...
object ColorUtils {
  ...
  fun parseHsl(hsl: String): Int? {
    val hslRegex =
        Regex(
            """hsla?\(\s*([0-9.]+)\s*(?:,|\s)\s*([0-9.]+%)\s*(?:,|\s)\s*([0-9.]+%)(?:\s*[/,]\s*([0-9.]+%?))?\s*\)"""
        )
  ...
  fun parseRgb(rgb: String): Int? {
    val rgbRegex =
        Regex(
            """rgba?\(\s*(\d{1,3}%?)\s*(?:,|\s)\s*(\d{1,3}%?)\s*(?:,|\s)\s*(\d{1,3}%?)(?:\s*[/,]\s*([0-9.]+%?))?\s*\)"""
        )
  ...
  /**
   * Parses a color string in any supported format (HSL, RGB, or Hex) into an Android [Color]
   * integer.
   *
