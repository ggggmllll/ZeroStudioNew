...
fun MarkdownCodeHighlighterRegistry.withEditorHighlighter(
    editorContextProvider: (languageName: LanguageName) -> Pair<Language, EditorColorScheme>?
) {
  ...
  override suspend fun highlightAsync(
    ...
    try {
      suspendCancellableCoroutine { continuation ->
        ...
        val receiver =
            object : EmptyStyleReceiver() {
              override fun setStyles(sourceManager: AnalyzeManager, styles: Styles?) {
                if (styles == null || !resumed.compareAndSet(false, true)) {
                  return
                }
                val result = runCatching { styles.toSpanned(content, editorSchema, codeTypeface) }
                continuation.resumeWith(result)
              }
            }
        continuation.invokeOnCancellation { analyzeManager.setReceiver(null) }
        analyzeManager.setReceiver(receiver)
        runCatching { analyzeManager.reset(ContentReference(content), Bundle()) }
            .onFailure { error ->
  ...
  private fun Styles.toSpanned(
    ...
    for (lineIndex in 0 until lineCount) {
      ...
      for (i in 0 until spanCount) {
        ...
        applySpan(builder, span, start, end.coerceAtMost(builder.length), colorScheme, typeface)
      }
    }
    return builder
  }
