...
class MarkdownCodeHighlighterRegistry {
  ...
  fun registerWithAlias(alias: LanguageAlias, highlighter: MarkdownCodeHighlighter): Disposable {
    ...
    alias.aliases.forEach { a -> aliases[a.lowercase()] = target }
    return Disposable {
      highlighters.remove(target)
      alias.aliases.forEach { a -> aliases.remove(a.lowercase()) }
    }
  }
  ...
  fun highlight(code: String, language: LanguageName?, codeTypeface: Typeface): HighlightResult {
    ...
    val highlighter =
        highlighters[resolved]
            ?: providers
                .firstNotNullOfOrNull { it.provide(resolved) }
  ...
  data class HighlightResult(val content: CharSequence, val needsAsync: Boolean)
