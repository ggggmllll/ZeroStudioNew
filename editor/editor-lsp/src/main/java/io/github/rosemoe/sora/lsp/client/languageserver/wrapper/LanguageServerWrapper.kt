...
class LanguageServerWrapper(
  ...
  fun start(throwException: Boolean = false) {
    synchronized(this) {
      ...
      try {
        ...
        eventHandler =
            EventHandler(serverDefinition.eventListener) {
              wrapperRef.get()?.isOffline()?.not() == true
            }

        client = DefaultLanguageClient(ServerWrapperBaseClientContext(this@LanguageServerWrapper))
        ...
        initializeFuture =
            connectedLanguageServer.initialize(initParams).thenApply { res ->
              ...
              val mergedCapabilities = mergeCapabilities(res.capabilities, fallbackCapabilities)
              ...
              eventHandler
                  ?.listener
                  ?.initialize(connectedLanguageServer, InitializeResult(mergedCapabilities))
  ...
  fun crashed(e: Exception) {
    crashCount += 1
    if (crashCount <= 3) {
      commonCoroutineScope.launch { restartAndReconnect() }
    } else {
      serverDefinition.eventListener.onHandlerException(
          LSPException(
              String.format(
      ...
      editor.textDocumentSyncKind =
          if (
              textDocumentSyncKind != TextDocumentSyncKind.Incremental ||
                  editor.textDocumentSyncKind != TextDocumentSyncKind.Incremental
          )
              TextDocumentSyncKind.Full
          else textDocumentSyncKind

      val completionTriggers = capabilities.completionProvider?.triggerCharacters ?: emptyList()
