...
object SimpleMarkdownRenderer {
  ...
  private fun build(
    ...
    for (block in blocks) {
      ...
      when (block) {
        ...
        is Block.CodeBlock -> {
          val start = builder.length
          val result = highlighterRegistry.highlight(block.content, block.language, codeTypeface)
        ...
        is Block.ListBlock -> {
          ...
          for (item in block.items) {
            ...
            builder.setSpan(
                LeadingMarginSpan.Standard(leadingMargin, leadingMargin + indentMargin),
                prefixStart,
                itemEnd,
                SPAN_MODE,
            )
    ...
    for (block in blocks) {
      ...
      when (block) {
        ...
        is Block.CodeBlock -> {
          val start = builder.length
          val highlighted =
              highlighterRegistry.highlightAsync(block.content, block.language, codeTypeface)
      ...
      when (inline) {
        ...
        is Inline.Image -> {
          ...
          if (drawable != null) {
            ...
            val end = builder.length
            builder.setSpan(ImageSpan(drawable, ImageSpan.ALIGN_CENTER), start, end, SPAN_MODE)
            builder.setSpan(URLSpan(inline.url), start, end, SPAN_MODE)
            continue
          }
  ...
  private fun parseInlines(text: String): List<Inline> {
    ...
    while (index < length) {
      ...
      if (current == '*' || current == '_') {
        val delimiter =
            if (index + 1 < length && text[index + 1] == current) {
              "$current$current"
            } else {
              "$current"
            }
  ...
  private fun normalize(input: String): String {
    ...
    text =
        preRegex.replace(text) { matchResult ->
          val body = matchResult.groupValues[1]
          "```\n$body\n```"
        }
    text =
        liRegex.replace(text) { matchResult ->
          val body = matchResult.groupValues[1].trim()
          "- $body"
        }
    text =
        linkRegex.replace(text) { matchResult ->
          val href = matchResult.groupValues[1]
          val label = matchResult.groupValues[2]
          "[$label]($href)"
        }
    text =
        autoLinkRegex.replace(text) { matchResult ->
  ...
  /**
   * Converts plain URLs in the input text into Markdown links, ignoring URLs that are already part
   * of existing Markdown links.
   *
   ...
   *
   * @param maxWidth Maximum width for returned Bitmaps. Images wider than this will be scaled down
   *   preserving aspect ratio.
   */
  open class DefaultImageProvider(private val maxWidth: Int = 800) : ImageProvider {
    override fun load(src: String): Drawable? {
      ...
      val bitmap =
          BitmapFactory.decodeByteArray(imageByteArray, 0, imageByteArray.size) ?: return null
      val scaledBitmap = scaleIfNeeded(bitmap, maxWidth)
      return BitmapDrawable(scaledBitmap)
    }

    /**
     * Scale down a bitmap to maxWidth preserving aspect ratio. If bitmap width is already <=
     * maxWidth, the original bitmap is returned.
     */
