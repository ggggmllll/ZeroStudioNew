  ...
  override fun onReceive(event: SelectionChangeEvent, unsubscribe: Unsubscribe) {
    ...
    editor.coroutineScope.launch(Dispatchers.IO) {
      editor.eventManager.emitAsync(EventType.documentHighlight) {
        put(DocumentHighlightEvent.DocumentHighlightRequest(event.left.fromThis()))
      }
    }
    ...
    if (
        (!originEditor.hasMouseHovering() &&
            (!hoverWindow.alwaysShowOnTouchHover || event.isSelected)) || isInCompletion
    ) {
      return
    }
