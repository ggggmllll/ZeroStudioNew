...
class LspEditor(
      ...
      withContext(Dispatchers.IO) {
        ...
        runCatching {
              ...
              currentLanguage?.let { language ->
                if (
                    capabilities.documentFormattingProvider?.left != false ||
              ...
              if (
                  capabilities.inlayHintProvider?.left != false ||
                      capabilities.inlayHintProvider?.right != null
              ) {
  ...
  private fun publishDiagnostics(diagnostics: List<Diagnostic>) {
    eventManager.emit(EventType.publishDiagnostics) { put("data", diagnostics) }
  }
