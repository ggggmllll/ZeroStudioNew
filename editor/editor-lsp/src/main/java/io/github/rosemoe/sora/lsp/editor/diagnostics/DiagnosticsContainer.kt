...
class DiagnosticsContainer {
  ...
  private fun findDiagnostic(uri: FileUri, line: Int): Diagnostic? {
    return diagnosticsMap[uri]?.find { it.range.start.line == line || it.range.end.line == line }
  }
  ...
  private fun findOrReplaceDiagnostic(uri: FileUri, newDiagnostic: Diagnostic) {
    ...
    findDiagnostic(uri, newDiagnostic.range.start.line)?.let { removeDiagnostic(uri, it) }
  ...
  fun findDiagnostics(uri: FileUri, range: Range): List<Diagnostic>? {
    ...
    val sortedDiagnostics =
        diagnostics.sortedWith(compareBy({ it.range.start.line }, { it.range.start.character }))
    ...
    if (index >= 0) {
      ...
      for (i in
          maxOf(0, insertionPoint - 1) until minOf(sortedDiagnostics.size, insertionPoint + 2)) {
        if (
            i in sortedDiagnostics.indices &&
                isPositionInRange(range.start, sortedDiagnostics[i].range)
        ) {
