...
@DslMarker annotation class LanguageServerDefinitionDslMarker
...
inline fun languageServerDefinition(
  ...
  return LanguageServerDefinitionDsl()
      .apply {
        ext(ext)
        block()
      }
...
class LanguageServerDefinitionDsl {
  ...
  /**
   * Provides a raw [CustomLanguageServerDefinition.ServerConnectProvider] when more control is
   * required.
   */
  ...
  fun build(): LanguageServerDefinition {
    val actualExtension =
        extension
            ?: throw IllegalStateException(
                "Extension must be provided when building a server definition"
            )
    val connection =
        connectProvider
            ?: throw IllegalStateException(
                "Connection must be provided when building a server definition"
            )
    ...
    return object :
      ...
      override fun getInitializationOptions(uri: URI?): Any? {
        return initializationOptionsProvider?.invoke(uri) ?: super.getInitializationOptions(uri)
      }
