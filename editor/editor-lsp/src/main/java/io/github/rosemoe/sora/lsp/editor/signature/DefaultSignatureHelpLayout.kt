...
class DefaultSignatureHelpLayout : SignatureHelpLayout {
  ...
  override fun createView(inflater: LayoutInflater): View {
    ...
    previousButton.setOnClickListener {
      signatureHelp?.let {
        ...
        currentIndex =
            (currentIndex - 1).let { idx -> if (idx < 0) it.signatures.lastIndex else idx }
  ...
  override fun applyColorScheme(colorScheme: EditorColorScheme, typeface: Typeface) {
    ...
    nextButton.apply { colorFilter = PorterDuffColorFilter(textColor, PorterDuff.Mode.SRC_ATOP) }
    val drawable =
  ...
  private fun updateDisplayedSignature() {
    ...
    val activeParameterIndex = signature.activeParameter ?: signatureHelp.activeParameter ?: -1
