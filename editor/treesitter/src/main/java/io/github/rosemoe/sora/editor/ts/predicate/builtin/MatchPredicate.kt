...
object MatchPredicate : TsPredicate {
  ...
  override fun doPredicate(
    ...
    val targetCaptureName = predicateSteps[1].content
    val regexPattern = predicateSteps[2].content
    ...
    for (i in 0 until capturesCount) {
      ...
      val startCharIndex = capture.node.startByte shr 1
      val endCharIndex = capture.node.endByte shr 1

      if (startCharIndex < 0 || endCharIndex > currentTextLength || startCharIndex > endCharIndex) {
        return PredicateResult.UNHANDLED
      }

      capturedTexts.add(text.subSequence(startCharIndex, endCharIndex).toString())
    }

    // 执行正则匹配逻辑
    try {
      var regex = cache[regexPattern]
      if (regex == null) {
