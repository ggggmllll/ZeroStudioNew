 ...
 * This class extends the standard [EditorRenderer] to wrap critical rendering methods with
 * `androidx.tracing.trace`. This allows developers to visualize the performance of the editor's
 * rendering pipeline using system tracing tools (Systrace, Perfetto).
 *
 * **Workflow**:
 * Every method overrides its counterpart in the superclass. Instead of executing directly,
 * it passes a section name (e.g., "drawView") to the [trace] inline function. This creates
 * a timed section in the system trace, executing the superclass logic within that section.
 *
 * **Usage**:
 * This renderer is automatically initialized and attached to the [CodeEditor] when debugging
 * is enabled or explicitly requested. It is not meant to be called directly by application code
 * for drawing purposes, but rather acts as the drawing engine for the editor widget.
 *
 * @author Akash Yadav
 * @author android_zero
 */
class TracingEditorRenderer(
  ...
  /**
   * **Purpose**: Updates the display list for a specific line using hardware acceleration (RenderNode).
   *
   * **Functionality**: On Android Q and above, this helps in caching the rendering commands for a line
   * to improve scrolling performance by avoiding re-recording drawing commands every frame.
   *
   ...
   *
   * **Usage**: Used when plugins or themes define specific background colors for the gutter
   * of certain lines (e.g., git diff indicators).
   *
   * @param canvas The canvas to draw on.
   ...
   *
   * **Functionality**: When "Sticky Scroll" is enabled, this draws the line numbers for the
   * code blocks that are currently stuck to the top of the viewport.
   *
   ...
   *
   * **Functionality**: Renders the text content of the scope headers (e.g., function definitions, classes)
   * that stick to the top of the screen when scrolling within their scope.
   *
   * @param canvas The canvas to draw on.
   ...
   *
   * **Functionality**: Draws a vertical line indicating where the code should ideally wrap
   * (e.g., at column 80 or 100).
   *
   * @param canvas The canvas to draw on.
   ...
   *
   * **Functionality**: Iterates through the layout to render text backgrounds, text content,
   * and prepares deferred drawing tasks for cursors and line numbers.
   *
   * @param canvas The canvas.
  ...
  /**
   * **Purpose**: Draws small graphical markers (e.g., for non-printable characters like newlines or tabs).
   *
   * @param canvas The canvas.
  ...
  /**
   * **Purpose**: Calculates the top Y coordinate for drawing a row's background.
   * **Note**: May differ from standard row top depending on settings (e.g., if background wraps text only).
   */
  ...
  /**
   * **Purpose**: Draws a specific background region within a row (e.g., selection, search highlight).
   *
   ...
   * @param tr The [TextRow] helper object used to measure text advances for accurate background placement.
   ...
   */
  override fun drawRowRegionBackground(canvas: Canvas, row: Int, tr: TextRow?, highlightStart: Int,
    highlightEnd: Int, rowStart: Int, rowEnd: Int, color: Int, borderColor: Int) = trace("drawRowRegionBackground") {
    super.drawRowRegionBackground(canvas, row, tr, highlightStart, highlightEnd, rowStart, rowEnd, color, borderColor)
  }
  ...
  /**
   * **Purpose**: Draws the bubble/panel that appears when dragging the scrollbar (shows current line info).
   *
   * @param canvas The canvas.
  ...
  /**
   * **Purpose**: Utility to patch/overlay a colored region on the text (used by snippet and delimiter highlighting).
   *
