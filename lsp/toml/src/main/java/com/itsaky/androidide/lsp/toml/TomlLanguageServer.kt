package com.itsaky.androidide.lsp.toml
...
class TomlLanguageServer : ILanguageServer {
  companion object { const val ID = "toml" }
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent = MarkupContent("toml symbol", MarkupKind.PLAIN)
  ...
  override suspend fun analyze(file: Path): DiagnosticResult {
    ...
    val hasStructure = content.lineSequence().any { it.trimStart().startsWith("[") || it.contains("=") }
    if (content.isBlank()) {
  ...
  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> = listOf(InlayHint(params.range.start, "toml", InlayHintKind.Type))
  override suspend fun documentLinks(file: Path): List<DocumentLink> = emptyList()
  override suspend fun codeLens(file: Path): List<CodeLens> = listOf(CodeLens(Range.pointRange(0,0), Command("Analyze", "analyze.toml"), null))
  override suspend fun callHierarchy(params: DefinitionParams): List<CallHierarchyItem> = listOf(CallHierarchyItem("toml.call", SymbolKind.Function, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
  override suspend fun typeHierarchy(params: DefinitionParams): List<TypeHierarchyItem> = listOf(TypeHierarchyItem("toml.type", SymbolKind.Class, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
