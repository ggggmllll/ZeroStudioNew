...
class TomlLanguageServer : ILanguageServer {
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent =
      MarkupContent("toml symbol", MarkupKind.PLAIN)
  ...
  override suspend fun analyze(file: Path): DiagnosticResult {
    ...
    val hasStructure =
        content.lineSequence().any { it.trimStart().startsWith("[") || it.contains("=") }
    if (content.isBlank()) {
      diagnostics +=
  ...
  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> =
      listOf(InlayHint(params.range.start, "toml", InlayHintKind.Type))
  ...
  override suspend fun codeLens(file: Path): List<CodeLens> =
      listOf(CodeLens(Range.pointRange(0, 0), Command("Analyze", "analyze.toml"), null))
