...
/**
 * 强大的 TOML 特性处理引擎。 纯字符串与正则分析实现，避免了复杂的 AST 解析开销，完美适配移动端 IDE， 专门针对 TOML（尤其是 Gradle
 * libs.versions.toml）进行了优化。
 *
 * @author android_zero
 */
object TomlFeatureEngine {
    ...
    for ((lineIdx, line) in lines.withIndex()) {
      ...
      for (match in matches) {
        if (match.value == targetWord) {
          val range =
              Range(Position(lineIdx, match.range.first), Position(lineIdx, match.range.last + 1))
  ...
  /**
   * 2. Definition (版本或变量定义跳转) 特别针对 Gradle Version Catalog 优化。如果在 `version.ref = "xxx"` 点击 `xxx`，会跳到
   *    `[versions]` 下的 `xxx`。
   */
  fun computeDefinition(content: String, uri: String, position: Position): List<Location> {
    ...
    for ((lineIdx, line) in lines.withIndex()) {
      if (defRegex.containsMatchIn(line)) {
        ...
        val range =
            Range(Position(lineIdx, startChar), Position(lineIdx, startChar + targetWord.length))
  ...
  /** 4. Document Symbol (文档大纲) 解析 `[table]` 和普通的 `key = value` */
  ...
  /** 5. Folding Range (代码折叠) 按 `[table]` 块进行折叠 */
  ...
  /** 7. Formatting (基础格式化) 删除尾部空格，并在 `=` 两侧加空格 */
  ...
  /** 8. Code Action (快速修复) TOML 结构简单，如果没有诊断错误，直接返回空。 */
  fun computeCodeAction(
      content: String,
