...
class ClangLanguageServer(initialSettings: ClangdServerSettings) : ILanguageServer {
  ...
  override fun shutdown() {
    ...
    fileVersions.keys.forEach { file -> ClangdNativeBridge.nativeDidClose(toFileUri(file)) }
  ...
  override fun complete(params: CompletionParams?): CompletionResult {
    ...
    val key =
        "completion:${params.file}:${params.position.line}:${params.position.column}:${params.prefix.orEmpty()}"
    resultCache.get(key)?.let {
      return parseCompletionResponse(it, params.prefix.orEmpty())
    }

    val requestId =
        ClangdNativeBridge.nativeRequestCompletion(
  ...
  override suspend fun findReferences(params: ReferenceParams): ReferenceResult {
    ...
    val requestId =
        ClangdNativeBridge.nativeRequestReferences(
            toFileUri(params.file),
            params.position.line,
            params.position.column,
  ...
  override suspend fun findDefinition(params: DefinitionParams): DefinitionResult {
    ...
    syncFile(params.file, null)
    val requestId =
        ClangdNativeBridge.nativeRequestDefinition(
            toFileUri(params.file),
            params.position.line,
            params.position.column,
        )
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent {
    syncFile(params.file, null)
    val requestId =
        ClangdNativeBridge.nativeRequestHover(
            toFileUri(params.file),
            params.position.line,
            params.position.column,
        )
  ...
  private fun parseCompletionResponse(raw: String, prefix: String): CompletionResult {
    ...
    for (i in 0 until itemsArray.length()) {
      ...
      val match =
          CompletionItem.matchLevel(candidate, matchToken, settings.completionFuzzyMatchMinRatio())
  ...
  private fun mapKind(kind: Int): CompletionItemKind =
      when (kind) {
        3,
        10 -> CompletionItemKind.FUNCTION
        6 -> CompletionItemKind.VARIABLE
        7,
        8 -> CompletionItemKind.CLASS
