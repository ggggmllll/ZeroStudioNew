...
object LspRequestDispatcher {
  ...
  private fun <T> submitToChannel(
    ...
    val channel =
        requestChannels.computeIfAbsent(fileUri) { key ->
  ...
  private class HoverRateLimiter(private val minIntervalMs: Long) {
    ...
    fun shouldAllow(key: String, caretSignature: String): Boolean {
      ...
      if (
          state == null ||
              state.signature != caretSignature ||
              now - state.timestamp >= minIntervalMs
      ) {
  ...
  private class FileRequestChannel(
      ...
      synchronized(this) {
        when {
          currentTask?.matches(type, identity) == true -> {
            @Suppress("UNCHECKED_CAST") (currentTask as MethodTask<T>).addCallback(callback)
            appended = true
          }
          pendingTasks[type]?.identity == identity -> {
            @Suppress("UNCHECKED_CAST") (pendingTasks[type] as MethodTask<T>).addCallback(callback)
            appended = true
          }
          else -> {
            val task =
    ...
    companion object {
      private val TASK_COMPARATOR =
          Comparator<MethodTask<*>> { a, b ->
            when {
