...
object LspService {
  ...
  // 诊断缓存
  private val diagnosticsCache =
      java.util.concurrent.ConcurrentHashMap<String, List<DiagnosticItem>>()
  ...
  @JvmStatic
  private external fun nativeRequestHover(fileUri: String, line: Int, character: Int): Long

  @JvmStatic
  private external fun nativeRequestCompletion(
      fileUri: String,
      line: Int,
      character: Int,
  ...
  @JvmStatic
  private external fun nativeRequestDefinition(fileUri: String, line: Int, character: Int): Long

  @JvmStatic
  private external fun nativeRequestReferences(
      fileUri: String,
      line: Int,
      character: Int,
  ...
  @JvmStatic
  private external fun nativeDidOpen(fileUri: String, content: String, languageId: String?)
  ...
  fun initialize(clangdPath: String = DEFAULT_CLANGD_PATH, workDir: String = "/"): Boolean {
    ...
    mainHandler.post { initializationListeners.forEach { it.onInitializationChanged(initialized) } }
  ...
  suspend fun requestHover(fileUri: String, line: Int, character: Int): HoverResult? =
      withContext(Dispatchers.IO) {
  ...
  suspend fun requestDefinition(fileUri: String, line: Int, character: Int): List<Location>? =
      withContext(Dispatchers.IO) {
  ...
  private suspend fun <T> awaitJsonResult(
    ...
    val cancellationHandle =
        ctx[Job]?.invokeOnCompletion { cause ->
  ...
  private fun parseCompletionItem(obj: JSONObject): CompletionItem? {
    return try {
      ...
      var insertText =
          obj.optJSONObject("textEdit")?.optString("newText", "")?.ifEmpty { null }
  ...
  private fun parseDocumentation(doc: Any?): String =
      when (doc) {
        is String -> doc
  ...
  private fun parseHoverResult(json: String): HoverResult? {
    return try {
      ...
      val contents =
          when (val c = result.opt("contents")) {
            is String -> c
            is JSONObject -> c.optString("value", "")
            is JSONArray ->
                (0 until c.length()).joinToString("\n") {
