...
object LspProjectManager {
  ...
  // 当前活动项目
  @Volatile private var currentProject: LspProject? = null
  ...
  // 是否已初始化 LSP 重启监听
  @Volatile private var lspListenerInitialized = false
  ...
  fun initialize() {
    ...
    LspService.addInitializationListener { initialized ->
      if (initialized) {
        ...
        managerScope.launch { resendAllDocuments() }
      }
    }
   ...
   */
  suspend fun openProject(context: Context, projectPath: String): LspProject =
  ...
  private suspend fun resendAllDocuments() {
    ...
    project.getOpenedFiles().forEach { filePath -> project.getEditor(filePath)?.resendDocument() }
  }
  ...
  private fun notifyProjectChanged(oldProject: LspProject?, newProject: LspProject?) {
    projectChangeListeners.forEach { it.onProjectChanged(oldProject, newProject) }
  }
  ...
  suspend fun createEditorForFile(
    ...
    // 确定项目路径
    val effectiveProjectPath = projectPath ?: File(absoluteFilePath).parent ?: return null

    // 获取或打开项目
    val project =
        if (currentProject?.projectPath == effectiveProjectPath) {
          currentProject!!
        } else {
