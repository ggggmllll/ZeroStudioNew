 ...
 * - 资源清理
 */
class LspProject(val context: Context, val projectPath: String) {
  companion object {
  ...
  suspend fun initialize(): Boolean = initMutex.withLock {
    ...
    // 初始化 LSP 服务
    val success = LspService.initialize(clangdPath = clangdPath!!, workDir = projectPath)
  ...
  fun removeEditor(filePath: String) {
    ...
    editors.remove(absolutePath)?.also { editor ->
      coroutineScope.launch { editor.dispose() }
...
class DiagnosticsContainer {
  ...
  fun clear() {
    ...
    diagnostics.clear()
    files.forEach { fileUri -> listeners.forEach { it.onDiagnosticsUpdated(fileUri, emptyList()) } }
  }
