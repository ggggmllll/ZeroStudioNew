 ...
 * - 编辑器事件监听
 */
class LspEditor(val project: LspProject, val filePath: String) {
  companion object {
  ...
  // 文档版本
  @Volatile private var version: Int = 1
  ...
  var editor: CodeEditor?
    ...
    set(value) {
      ...
      contentChangeSubscription =
          value.subscribeEvent<ContentChangeEvent> { _, _ -> scheduleSync() }
  ...
  suspend fun dispose() {
    ...
    // 取消事件订阅
    withContext(Dispatchers.Main) { contentChangeSubscription?.unsubscribe() }
