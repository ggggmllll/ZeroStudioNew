...
fun bumpPatchVersionString(version: String): String {
  val (numericPart, suffix) =
      version.split("-", limit = 2).let { parts ->
...
val versionBumpKeywords =
    listOf("assemble", "bundle", "install", "publish", "build", "autoIncrementVersion")
val shouldAutoIncrementVersion =
...
afterEvaluate {
  ...
  if (arm64Task != null && x86Task != null && mergeNativeTask != null) {
    buildAllAbiNative.configure { dependsOn(arm64Task, x86Task) }
