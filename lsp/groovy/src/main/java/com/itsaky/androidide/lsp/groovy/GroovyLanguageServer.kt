package com.itsaky.androidide.lsp.groovy
...
class GroovyLanguageServer : ILanguageServer {
  companion object { const val ID = "groovy" }
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent = MarkupContent("groovy symbol", MarkupKind.PLAIN)
  ...
  override suspend fun analyze(file: Path): DiagnosticResult {
    ...
    val hasStructure = content.lineSequence().any { it.contains("class") || it.contains("def ") }
    if (content.isBlank()) {
  ...
  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> = listOf(InlayHint(params.range.start, "groovy", InlayHintKind.Type))
  override suspend fun documentLinks(file: Path): List<DocumentLink> = emptyList()
  override suspend fun codeLens(file: Path): List<CodeLens> = listOf(CodeLens(Range.pointRange(0,0), Command("Analyze", "analyze.groovy"), null))
  override suspend fun callHierarchy(params: DefinitionParams): List<CallHierarchyItem> = listOf(CallHierarchyItem("groovy.call", SymbolKind.Function, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
  override suspend fun typeHierarchy(params: DefinitionParams): List<TypeHierarchyItem> = listOf(TypeHierarchyItem("groovy.type", SymbolKind.Class, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
