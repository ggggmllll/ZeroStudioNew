...
class GroovyLanguageServer : ILanguageServer {
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent =
      MarkupContent("groovy symbol", MarkupKind.PLAIN)
  ...
  override suspend fun analyze(file: Path): DiagnosticResult {
    ...
    if (content.isBlank()) {
      diagnostics +=
          DiagnosticItem("Empty document", "EMPTY", Range.NONE, ID, DiagnosticSeverity.INFO)
    }
  ...
  override suspend fun documentSymbols(file: Path): DocumentSymbolsResult {
    ...
    val symbols =
        file.toFile().readLines().mapIndexedNotNull { index, line ->
          val text = line.trim()
      ...
      if (!file.exists()) emptyList()
          ...
          listOf(
              FoldingRange(
                  0,
                  0,
                  file.toFile().readLines().lastIndex.coerceAtLeast(0),
                  0,
  ...
  override suspend fun semanticTokensFull(params: SemanticTokensParams): SemanticTokens =
      SemanticTokens(listOf(0, 0, 5, 0, 0), "1")
  ...
  override suspend fun semanticTokensDelta(params: SemanticTokensParams): SemanticTokensDelta =
      SemanticTokensDelta("2", listOf(SemanticTokensDeltaEdit(0, 0, listOf(0, 0, 5, 0, 0))))

  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> =
      listOf(InlayHint(params.range.start, "groovy", InlayHintKind.Type))
  ...
  override suspend fun codeLens(file: Path): List<CodeLens> =
      listOf(CodeLens(Range.pointRange(0, 0), Command("Analyze", "analyze.groovy"), null))
