...
abstract class BaseAstLanguageServer(
  ...
  override fun complete(params: CompletionParams?): CompletionResult {
    ...
    return CompletionResult(
        keywords.filter { it.startsWith(prefix, ignoreCase = true) }.map { kw ->
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent = MarkupContent("$serverId symbol", MarkupKind.PLAIN)
  ...
  override suspend fun analyze(file: Path): DiagnosticResult {
    ...
    if (content.isNotBlank() && !structureDetector(content)) {
      diagnostics += DiagnosticItem("No recognizable $serverId structure", "NO_STRUCTURE", Range.NONE, serverId, DiagnosticSeverity.HINT)
    }
    ...
    val symbols = lines.mapIndexedNotNull { index, line ->
      ...
      if (text.isEmpty()) null
          ...
          kind = when {
            ...
            text.contains("function", true) || text.contains("def ", true) -> SymbolKind.Function
  ...
  override suspend fun workspaceSymbols(query: String): WorkspaceSymbolsResult {
    ...
    root.toFile().walkTopDown().filter { it.isFile && it.extension.isNotBlank() }.take(200).forEach { f ->
  ...
  override suspend fun rename(params: RenameParams): WorkspaceEdit {
    val content = runCatching { params.file.toFile().readText() }.getOrNull() ?: return WorkspaceEdit()
    val line = content.lineSequence().drop(params.position.line).firstOrNull() ?: return WorkspaceEdit()
    val token = line.split(Regex("[^A-Za-z0-9_]+"))
        .firstOrNull { it.isNotBlank() } ?: return WorkspaceEdit()
    val edits = mutableListOf<TextEdit>()
    content.lineSequence().forEachIndexed { idx, ln ->
      var start = ln.indexOf(token)
      while (start >= 0) {
        edits += TextEdit(Range(Position(idx, start), Position(idx, start + token.length)), params.newName)
  ...
  override suspend fun foldingRanges(file: Path): List<FoldingRange> {
    ...
    return listOf(FoldingRange(startLine = 0, endLine = lines.lastIndex, kind = FoldingRangeKind.Region))
  }

  override suspend fun selectionRanges(params: SelectionRangesParams): List<SelectionRange> {
    return params.positions.map { pos -> SelectionRange(Range.pointRange(pos), SelectionRange(Range.NONE, null)) }
  }
  ...
  override suspend fun semanticTokensDelta(params: SemanticTokensParams): SemanticTokensDelta {
    return SemanticTokensDelta(resultId = "1", edits = listOf(SemanticTokensDeltaEdit(0, 0, listOf(0,0,5,0,0))))
  }

  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> {
    return listOf(InlayHint(position = params.range.start, label = "$serverId hint", kind = InlayHintKind.Type))
  }
  ...
  override suspend fun documentLinks(file: Path): List<DocumentLink> {
    ...
    return listOf(DocumentLink(Range(Position(line, start), Position(line, first.length)), first.substring(start).trim(), "External link"))
  }

  override suspend fun codeLens(file: Path): List<CodeLens> {
    return listOf(CodeLens(range = Range.pointRange(0, 0), command = Command("Run", "run.$serverId")))
  }

  override suspend fun callHierarchy(params: DefinitionParams): List<CallHierarchyItem> {
    return listOf(CallHierarchyItem("$serverId.call", SymbolKind.Function, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
  }

  override suspend fun typeHierarchy(params: DefinitionParams): List<TypeHierarchyItem> {
    return listOf(TypeHierarchyItem("$serverId.Type", SymbolKind.Class, Location(params.file, Range.pointRange(params.position.line, params.position.column))))
  }
}

class CmakeLanguageServer : BaseAstLanguageServer("cmake", listOf("cmake_minimum_required", "project", "add_executable", "add_library"), { it.contains("(") && it.contains(")") })
class Css3LanguageServer : BaseAstLanguageServer("css3", listOf("color", "display", "position", "font-size", "background"), { it.contains("{") && it.contains("}") })
class CsvLanguageServer : BaseAstLanguageServer("csv", listOf(",", "\"", "header", "delimiter"), { it.lineSequence().any { line -> line.contains(',') } })
class EcmascriptLanguageServer : BaseAstLanguageServer("ecmascript", listOf("function", "const", "let", "class", "import", "export"), { it.contains("function") || it.contains("=>") || it.contains("class") })
class HtmlLanguageServer : BaseAstLanguageServer("html", listOf("<div>", "<span>", "<script>", "<style>", "<body>"), { it.contains('<') && it.contains('>') })
class JavascriptLanguageServer : BaseAstLanguageServer("javascript", listOf("function", "const", "let", "class", "await", "Promise"), { it.contains("function") || it.contains("=>") })
class JsonLanguageServer : BaseAstLanguageServer("json", listOf("{", "}", "[", "]", "\"key\""), { it.trimStart().startsWith('{') || it.trimStart().startsWith('[') })
class Json5LanguageServer : BaseAstLanguageServer("json5", listOf("{", "}", "//", "/* */", "Infinity", "NaN"), { it.contains('{') && it.contains('}') })
class PropertiesLanguageServer : BaseAstLanguageServer("properties", listOf("key=value", "profile", "spring", "android"), { it.lineSequence().any { line -> line.contains('=') || line.contains(':') } })
class Protobuf3LanguageServer : BaseAstLanguageServer("protobuf3", listOf("syntax", "message", "enum", "service", "rpc"), { it.contains("syntax") || it.contains("message") })
