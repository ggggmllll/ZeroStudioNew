...
class SnippetTransformer {
  ...
  fun transformSnippet(insertText: String, parameterPairs: List<Pair<String, String>>?): String {
    ...
    result = SNIPPET_PLACEHOLDER_REGEX.replace(result) { matchResult ->
      val tabstop = matchResult.groupValues[1].toInt()
      val placeholder = matchResult.groupValues[2]

      // 提取对应位置的参数信息
      val paramIndex = tabstop - 1
      val paramInfo = if (paramIndex in parameterPairs.indices) parameterPairs[paramIndex] else Pair(placeholder, "")
  ...
  /**
   * 从函数签名中提取参数名和类型。
   * 示例输入: `(text: String, onClick: () -> Unit)`
   * 示例输出: `[(text, String), (onClick, () -> Unit)]`
   */
