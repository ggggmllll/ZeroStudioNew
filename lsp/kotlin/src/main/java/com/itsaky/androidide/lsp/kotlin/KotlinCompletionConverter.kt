 ...
 * 转换器：二次过滤、加工并注入导入修饰符。
 *
 * 作用：KLS 的 Completion 返回可能带有极多无效项且 Snippet 占位符并不完美兼容。 我们在这里进行清洗，拦截 KLS 原始补全参数将其转化为智能 Lambda / 具名参数，
...
class KotlinCompletionConverter {
  ...
  private fun convertItemFast(item: JsonObject, fileContent: String): CompletionItem {
    ...
    if (serverImportEdit != null) {
      val (line, importText) =
          importResolver.generateImportEdit(serverImportEdit.replace("import ", ""), fileContent)
      additionalEdits.add(
          TextEdit(
              range = Range(start = Position(line, 0), end = Position(line, 0)),
              newText = importText,
