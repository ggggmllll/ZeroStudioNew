...
class SmaliLanguageServer : ILanguageServer {
  ...
  override suspend fun hover(params: DefinitionParams): MarkupContent =
      MarkupContent("smali symbol", MarkupKind.PLAIN)
  ...
  override suspend fun inlayHints(params: InlayHintParams): List<InlayHint> =
      listOf(InlayHint(params.range.start, "smali", InlayHintKind.Type))
  ...
  override suspend fun codeLens(file: Path): List<CodeLens> =
      listOf(CodeLens(Range.pointRange(0, 0), Command("Analyze", "analyze.smali"), null))
