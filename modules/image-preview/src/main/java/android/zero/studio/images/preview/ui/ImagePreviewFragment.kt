...
/** The main entry point for Image Preview. Implemented as a FullScreen DialogFragment. */
class ImagePreviewFragment : DialogFragment() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    arguments?.getString(ARG_PATH)?.let { currentFile = File(it) }
  }
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    // Swipe Navigation
    binding.previewView.onFileSwitchRequest = { direction -> navigateFile(direction) }
  }
  ...
  private fun loadFileContext() {
    currentFile?.parentFile?.listFiles()?.let { files ->
      fileList = files.filter { it.isFile && ImageFileUtils.isSupportedImage(it) }.sorted()
    }
  }
  ...
  private fun updateBackgroundColor() {
    ...
    binding.previewView.backgroundColorVal =
        0 // Clear native bg to let container show through or set native?
