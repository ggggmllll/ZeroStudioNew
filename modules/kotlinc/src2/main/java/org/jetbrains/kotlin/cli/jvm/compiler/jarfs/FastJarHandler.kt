...
class FastJarHandler(val fileSystem: FastJarFileSystem, path: String) {
    ...
    init {
        ...
        RandomAccessFile(file, "r").use { randomAccessFile ->
            ...
            try {
                ...
                cachedManifest = entries.singleOrNull { StringUtil.equals(MANIFEST_PATH, it.relativePath) }?.let(mappedByteBuffer::contentsToByteArray)
            } finally {
                with(fileSystem) {
    ...
    private fun createFile(entry: ZipEntryDescription, directories: MutableMap<String, FastJarVirtualFile>): FastJarVirtualFile {
        ...
        if ("." == shortName) return parentFile

        return FastJarVirtualFile(this, shortName, if (entry.isDirectory) -1 else entry.uncompressedSize, parentFile, entry)
    }
    ...
    private fun CharSequence.splitPath(): Pair<CharSequence, CharSequence> {
        ...
        while (slashIndex >= 0 && this[slashIndex] != '/') slashIndex--
        if (slashIndex == -1) return Pair("", this)
        return Pair(subSequence(0, slashIndex), subSequence(slashIndex + 1, this.length))
    }

    fun findFileByPath(pathInJar: String): VirtualFile? = myRoot?.findFileByRelativePath(pathInJar)
