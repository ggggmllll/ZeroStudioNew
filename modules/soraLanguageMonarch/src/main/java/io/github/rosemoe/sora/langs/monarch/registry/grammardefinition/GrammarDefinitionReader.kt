...
abstract class GrammarDefinitionReader<T> : JsonAdapter<ParsedGrammarDefinitionList<T>>() {
  ...
  private fun readGrammarDefinition(reader: JsonReader): GrammarDefinition<T> {
    var name = ""
    val grammar: T?
          ...
          languageConfiguration =
              kotlin
                  .runCatching {
                    ...
                    val rawText = FileProviderRegistry.resolve(path)?.bufferedReader()?.readText()
    ...
    grammar =
        readGrammar(
            GrammarDefinition(name, Unit, embeddedLanguages, languageConfiguration, scopeName),
            grammarPath,
        )
