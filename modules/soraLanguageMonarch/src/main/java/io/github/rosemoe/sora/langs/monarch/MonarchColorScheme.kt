...
class MonarchColorScheme(themeModel: ThemeModel) : EditorColorScheme(), ThemeChangeListener {
  ...
  private fun applyThemeSettings(defaultColors: ThemeDefaultColors) {
    ...
    defaultColors["editor.foreground"]?.let { setColor(TEXT_NORMAL, Color.parseColor(it)) }
    ...
    val blockLineColor =
        (getColor(WHOLE_BACKGROUND) + getColor(TEXT_NORMAL)) / 2 and
            0x00FFFFFF or
            0x88000000.toInt()
  ...
  private fun applyTMTheme(defaultColors: ThemeDefaultColors) {
    ...
    defaultColors["caret"]?.let { setColor(SELECTION_INSERT, Color.parseColor(it)) }

    defaultColors["selection"]?.let { setColor(SELECTED_TEXT_BACKGROUND, Color.parseColor(it)) }

    defaultColors["invisibles"]?.let { setColor(NON_PRINTABLE_CHAR, Color.parseColor(it)) }

    defaultColors["lineHighlight"]?.let { setColor(CURRENT_LINE, Color.parseColor(it)) }
    ...
    defaultColors["foreground"]?.let { setColor(TEXT_NORMAL, Color.parseColor(it)) }
  ...
  override fun getColor(type: Int): Int {
    ...
    val newColor = if (color != null) Color.parseColor(color) else super.getColor(TEXT_NORMAL)
