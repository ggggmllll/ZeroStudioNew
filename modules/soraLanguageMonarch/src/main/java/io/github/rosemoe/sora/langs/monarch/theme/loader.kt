...
class TokenThemeAdapter : JsonAdapter<TokenTheme>() {
  ...
  private fun readThemeColors(reader: JsonReader, themeColorsMap: MutableMap<String, String>) {
    reader.beginObject()
  ...
  private fun readTokenColors(reader: JsonReader, tokenThemeRuleList: MutableList<TokenThemeRule>) {
    reader.beginArray()
  ...
  private fun readTokenColor(
    ...
    val processToken = { token: String ->
      if (token.contains(",")) {
        tokenList.addAll(token.split(",").map { it.trim() })
      } else {
        tokenList.add(token)
      }
    }
    ...
    for (token in tokenList) {
      tokenThemeRuleList.add(TokenThemeRule(token, foreground, background, fontStyle))
    }

    val updates =
        mapOf("foreground" to foreground, "background" to background, "fontStyle" to fontStyle)
