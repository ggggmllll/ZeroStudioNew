...
class MonarchSymbolPairMatch(
  ...
  fun updatePair() {
    ...
    val languageConfiguration: LanguageConfiguration = language.languageConfiguration ?: return

    removeAllPairs()

    val surroundingPairs = languageConfiguration.surroundingPairs ?: emptyList()
    ...
    autoClosingPairs.forEach {
      pairs[it.open] = SymbolPair(it.open, it.close, SymbolPairEx(it, true))
    }

    for (surroundingPair in surroundingPairs) {
      val originAutoClosingPair =
          autoClosingPairs.find {
            it.open == surroundingPair.open && it.close == surroundingPair.close
          } as? AutoClosingPairConditional

      val surroundingPairNotInList =
          if (surroundingPair is AutoClosingPairConditional) {
            surroundingPair.notIn
          } else emptyList()

      if (originAutoClosingPair == null) {
        pairs[surroundingPair.open] =
            SymbolPair(
      ...
      pairs[pair.open] = SymbolPair(pair.open, pair.close, SymbolPairEx(pair, true))
    }

    pairs.forEach { putPair(it.key, it.value) }
  }
  ...
  class SymbolPairEx(
    ...
    init {
      run {
        ...
        for (i in excludeTokenTypesArray.indices) {
          val excludeTokenName = excludeTokenTypeList[i].lowercase(Locale.getDefault())
    ...
    override fun shouldReplace(
      ...
      if (extra is Int) {
        val index = Arrays.binarySearch(excludedTokenTypes, extra)
        return index < 0
      }
