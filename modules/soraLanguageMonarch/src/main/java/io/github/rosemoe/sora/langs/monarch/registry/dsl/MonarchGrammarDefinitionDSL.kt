...
class MonarchLanguageDefinitionListBuilder :
  ...
  override fun build(): List<GrammarDefinition<Language>> = allBuilder.map { languageDefinition ->
    val monarchLanguage =
        languageDefinition.monarchLanguage
            ?: languageDefinition
                ...
                ?.getOrNull()
            ?: throw Exception("Failed to load monarch source")
    ...
    val languageConfiguration =
        languageDefinition.languageConfiguration?.let { configuration ->
          kotlin.runCatching { configuration.toLanguageConfiguration() }.getOrNull()
              ?: runCatching {
                    FileProviderRegistry.resolve(languageDefinition.languageConfiguration ?: "")
