...
object ThemeRegistry {
  ...
  @Synchronized
  @Throws(Exception::class)
  fun loadTheme(themeModel: ThemeModel, isCurrentTheme: Boolean) {
  ...
  fun setTheme(name: String): Boolean {
    var targetModel: ThemeModel? = findThemeByFileName(name)
  ...
  fun findTheme(name: String): ThemeModel? {
    return themes.firstOrNull { it.value.name == name }
  }
}
