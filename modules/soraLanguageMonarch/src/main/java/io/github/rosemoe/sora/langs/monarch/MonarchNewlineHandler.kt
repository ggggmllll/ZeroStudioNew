...
class MonarchNewlineHandler(private val language: MonarchLanguage) : NewlineHandler {
  ...
  fun getIndentForEnter(text: Content, position: CharPosition): Pair<String, String>? {
    ...
    val beforeEnterIndent = beforeEnterText.getLeadingWhitespace(0, beforeEnterText.length)
  ...
  override fun handleNewline(
    ...
    when (enterAction.indentAction) {
      ...
      Outdent -> {
        val indentation =
            enterAction.indentation.getIndentationFromWhitespace(
                language.tabSize,
                language.useTab(),
            )
        val outdentedText =
  ...
  fun getEnterAction(content: Content, position: CharPosition): CompleteEnterAction? {
    ...
    // range.startColumn);
    val onEnterSupport = this.enterSupport ?: return null
    ...
    if (appendText == null) {
      appendText =
          if (indentAction == IndentAction.Indent || indentAction == IndentOutdent) {
            "\t"
          } else {
            ""
          }
   ...
   * Result: -2: run into the boundary of embedded languages -1: every line above are invalid else:
  ...
  fun getPrecedingValidLine(content: WrapperContent, lineNumber: Int): Int {
    ...
    if (lineNumber > 0) {
      for (lastLineNumber in lineNumber - 1 downTo 0) {
        ...
        if (
            indentRulesSupport?.shouldIgnore(lineContent) == true ||
                precedingValidPattern.matches(lineContent) ||
                lineContent.isEmpty()
        ) {
          continue
        }
   ...
   * 2. If this line matches indentNextLinePattern or increaseIndentPattern, it means that the
   *    indent level of `lineNumber` should be 1 greater than this line.
   * 3. If this line doesn't match any indent rules a. check whether the line above it matches
   *    indentNextLinePattern b. If not, the indent level of this line is the result c. If so, it
   *    means the indent of this line is *temporary*, go upward utill we find a line whose indent is
   *    not temporary (the same workflow a -> b -> c).
   * 4. Otherwise, we fail to get an inherited indent from aboves. Return null and we should not
   *    touch the indent of `lineNumber` <p> This function only return the inherited indent based on
   *    above lines, it doesn't check whether current line should decrease or not.
   */
  private fun getInheritIndentForLine(
    ...
    if (
      ...
      if (
          ((previousLineIndentMetadata and
              (IndentRulesSupport.IndentConsts.INCREASE_MASK or
                  IndentRulesSupport.IndentConsts.DECREASE_MASK)) === 0) &&
              ((previousLineIndentMetadata and
                  IndentRulesSupport.IndentConsts.INDENT_NEXTLINE_MASK) === 0) &&
        ...
        return InheritIndentResult(
            model.getLineContent(stopLine + 1).getLeadingWhitespace(),
            IndentAction.None,
            stopLine + 1,
        )
      }
      ...
      for (i in precedingUnIgnoredLine downTo 1) {
        ...
        if (indentRulesSupport.shouldIncrease(lineContent)) {
          return InheritIndentResult(lineContent.getLeadingWhitespace(), IndentAction.Indent, i)
          ...
          return InheritIndentResult(lineContent.getLeadingWhitespace(), IndentAction.None, i)
        }
      }

      return InheritIndentResult(
          model.getLineContent(1).getLeadingWhitespace(),
          IndentAction.None,
          1,
      )
    }
