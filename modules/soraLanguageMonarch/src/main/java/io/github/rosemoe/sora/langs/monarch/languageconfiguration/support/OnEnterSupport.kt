...
// See
// https://github.com/microsoft/vscode/blob/aa31bfc9fd1746626b3efe86f41b9c172d5f4d23/src/vs/editor/common/languages/supports/onEnter.ts#
...
class OnEnterSupport(brackets: List<CharacterPair>?, onEnterRules: List<OnEnterRule>?) {
  ...
  init {
    val brackets =
        brackets
            ?: listOf(CharacterPair("(", ")"), CharacterPair("[", "]"), CharacterPair("{", "}"))
    ...
    brackets.forEach {
      ...
      if (openRegExp != null && closeRegExp != null) {
        processedBracketPairs.add(
            ProcessedBracketPair(it.first, it.second, openRegExp, closeRegExp)
        )
      }
    }
  ...
  fun onEnter(
    ...
    for ((beforeText, afterTextPattern, previousLinePattern, action) in regExpRules) {
      ...
      if (
          previousLinePattern != null &&
              !previousLinePattern.matchesPartially(previousLineText ?: "")
      )
          continue
    ...
    if (beforeEnterText.isNotEmpty() && afterEnterText.isNotEmpty()) {
      for (bracket in brackets) {
        if (
            bracket.openRegExp.matches(beforeEnterText) &&
                bracket.closeRegExp.matches(afterEnterText)
        ) {
  ...
  companion object {
    ...
    private fun createOpenBracketRegExp(bracket: String): Regex? {
      ...
      if (!B_REGEXP.matcher(firstChar).find()) {
