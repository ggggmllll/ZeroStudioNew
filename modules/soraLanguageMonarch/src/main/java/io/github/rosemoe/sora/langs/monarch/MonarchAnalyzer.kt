...
class MonarchAnalyzer(
...
) : AsyncIncrementalAnalyzeManager<MonarchState, Span>(), FoldingHelper, ThemeChangeListener {
  ...
  init {
    ...
    if (languageConfiguration != null) {
      ...
      if (!pairs.isNullOrEmpty()) {
        ...
        val pairArr =
            CharArray(filteredPairs.size * 2) { index ->
  ...
  override fun computeBlocks(
    ...
    if (delegate.isNotCancelled) {
      withReceiver { it.updateBracketProvider(this, bracketsProvider) }
    }
    return list
  }
  ...
  override fun tokenizeLine(
    ...
    val lineTokens =
        tokenization.tokenizeEncoded(
            line,
            false,
            state?.tokenizeState ?: initialState.tokenizeState,
        )
    ...
    for (index in 0 until tokensLength) {
      val startIndex = line.convertUnicodeOffsetToUtf16(lineTokens.tokens[2 * index], surrogate)
      ...
      if (language.createIdentifiers && tokenType == StandardTokenType.Other) {
        var end =
            if (index + 1 == tokensLength) lineC.length
            else line.convertUnicodeOffsetToUtf16(lineTokens.tokens[2 * (index + 1)], surrogate)
    ...
    return LineTokenizeResult(
        MonarchState(
            lineTokens.endState,
            cachedFoldingRegExp?.search(line, 0),
  ...
  override fun onAddState(state: MonarchState) {
    ...
    if (language.createIdentifiers) {
      state.identifiers?.forEach { identifier -> syncIdentifiers.identifierIncrease(identifier) }
    }
  }
  ...
  override fun onAbandonState(state: MonarchState) {
    ...
    if (language.createIdentifiers) {
      state.identifiers?.forEach { identifier -> syncIdentifiers.identifierDecrease(identifier) }
    }
  }
