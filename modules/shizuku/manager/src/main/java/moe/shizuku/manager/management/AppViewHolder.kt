...
class AppViewHolder(private val binding: AppListItemBinding) :
  ...
  private val switchWidget
    get() = binding.switchWidget

  private val root
    get() = binding.requiresRoot
  ...
  private inline val packageName
    get() = data.packageName

  private inline val ai
    get() = data.applicationInfo!!

  private inline val uid
    get() = ai.uid
  ...
  override fun onBind() {
    ...
    root.visibility =
        if (ai.metaData != null && ai.metaData.getBoolean("moe.shizuku.client.V3_REQUIRES_ROOT"))
            View.VISIBLE
        else View.GONE
