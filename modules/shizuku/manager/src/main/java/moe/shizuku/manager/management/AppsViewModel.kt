...
@MainThread fun ComponentActivity.appsViewModel() = viewModels { AppsViewModel(this) }

@MainThread fun Fragment.appsViewModel() = activityViewModels { AppsViewModel(requireContext()) }
...
class AppsViewModel(context: Context) : ViewModel() {
  ...
  fun load(onlyCount: Boolean = false) {
    viewModelScope.launch(Dispatchers.IO) {
      ...
      } catch (e: CancellationException) {} catch (e: Throwable) {
