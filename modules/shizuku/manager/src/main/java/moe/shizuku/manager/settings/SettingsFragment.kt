...
class SettingsFragment : PreferenceFragmentCompat() {
  ...
  override fun onCreateRecyclerView(
    ...
    if (lp is FrameLayout.LayoutParams) {
      lp.rightMargin =
          recyclerView.context.resources.getDimension(R.dimen.rd_activity_horizontal_margin).toInt()
  ...
  private fun setupLocalePreference() {
    ...
    for ((index, displayLocale) in displayLocaleTags.withIndex()) {
      ...
      val localeName =
          if (!TextUtils.isEmpty(locale.script)) locale.getDisplayScript(locale)
