...
/**
 * 命令行直调模式的 D8 DEX 转换器封装。 作为 CompilerDaemon 的备用方案。
 *
 * @author android_zero
 */
class ComposeDexCompiler(private val classpathManager: ComposeClasspathManager) {
  ...
  suspend fun compileToDex(classesDir: File, outputDir: File): DexCompilationResult =
      withContext(Dispatchers.IO) {
        ...
        try {
          val processBuilder =
              ProcessBuilder(command).directory(classesDir).redirectErrorStream(true)
          ...
          DexCompilationResult(
              success,
              if (success) dexFile else null,
              if (!success) output else "",
          )
        } catch (e: Exception) {
