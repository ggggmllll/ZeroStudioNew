...
/**
 * 提取项目上下文。 为 Compose 预览服务提供必要的局部 Classpath 和 R.class 等依赖环境。
 *
 * @author android_zero
 */
...
class ProjectContextSource {
  ...
  fun resolveContext(filePath: String): ProjectContext {
    ...
    val compileClasspaths =
        (module.getCompileClasspaths() + intermediateClasspaths).distinct().filter { it.exists() }
    ...
    LOG.info(
        "Hot-Reload Context - Module: {}, Variant: {}, CompileJars: {}, ProjectDex: {}, NeedsBuild: {}",
        module.path,
        variantName,
