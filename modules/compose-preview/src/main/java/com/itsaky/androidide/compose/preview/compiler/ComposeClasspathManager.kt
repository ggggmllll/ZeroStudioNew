 ...
 * 极简版依赖提取器。
 *
 * <p>基于 AndroidIDE 架构：我们摒弃了耗时的 Assets 解压方式。宿主环境中已经具备 Compose 和 Kotlin 编译器依赖， 此工具只需负责从本地缓存精准查找
 * <code>kotlin-compiler</code>、<code>compose-compiler-plugin</code> 和 <code>d8.jar</code>。</p>
 *
 * @author android_zero
 */
class ComposeClasspathManager(private val context: Context) {
  ...
  private val composePluginArtifacts =
      listOf(
  ...
  /**
   * Finds a JAR file matching the specified artifact group/name path in the Gradle cache. Takes the
  ...
  fun getCompilerPlugin(): File {
    ...
    throw IllegalStateException(
        "Compose Compiler Plugin not found. Make sure it's defined in your libs.versions.toml and synced."
    )
  }
