      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (ctx.needsBuild && ctx.modulePath != null) {
            LOG.warn(
                "No intermediate classes found - Full/Incremental build required before preview initialization"
            )
          ...
          try {
            ...
            LOG.warn(
                "Failed to pre-start compiler daemon (non-fatal, will fallback to CLI mode)",
                e,
            )
          }
  ...
  override suspend fun compilePreview(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          val fullClassName =
              if (parsedSource.packageName.isNotBlank())
                  "${parsedSource.packageName}.$generatedClassName"
          ...
          if (cached != null) {
            LOG.info(
                "Hot-Reload: Cache HIT for hash: {}, projectDexFiles={}",
                sourceHash,
          ...
          val packageDir =
              if (parsedSource.packageName.isNotBlank())
                  File(sourceDir, parsedSource.packageName.replace('.', '/'))
              else sourceDir
          packageDir.mkdirs()
          ...
          LOG.info(
              "Hot-Reload Preview Ready: {} with {} previews, {} project DEX files",
              fullClassName,
