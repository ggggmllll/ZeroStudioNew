...
open class TsAnalyzeManager(val languageSpec: TsLanguageSpec, var theme: TsTheme) :
  ...
  fun updateTheme(theme: TsTheme) {
    ...
    spans?.let { if (it is LineSpansGenerator) it.theme = theme }
  }
  ...
  override fun insert(start: CharPosition, end: CharPosition, insertedContent: CharSequence) {
    ...
    (styles.spans as LineSpansGenerator?)?.apply {
      ...
      safeTree.accessTreeIfAvailable { it.edit(newTSInputEdit(start, start, end)) }
    }
  }
  ...
  override fun delete(start: CharPosition, end: CharPosition, deletedContent: CharSequence) {
    thread?.offerMessage(
        MSG_MOD,
        TextModification(start.index, end.index, newTSInputEdit(start, end, start), null),
    )
    (styles.spans as LineSpansGenerator?)?.apply {
      lineCount = reference!!.lineCount
      safeTree.accessTreeIfAvailable { it.edit(newTSInputEdit(start, end, start)) }
    }
  }
  ...
  companion object {
    ...
    @Synchronized fun nextThreadId() = ++threadId
  }
  ...
  inner class TsLooperThread : Thread() {
    ...
    private val parser = TSParser.create().also { it.language = languageSpec.language }
    ...
    fun updateCodeBlocks() {
      ...
      TSQueryCursor.create().use {
        ...
        while (match != null) {
          if (
              languageSpec.blocksPredicator.doPredicate(languageSpec.predicates, localText, match)
          ) {
            match.captures.forEach {
              val block =
                  CodeBlock().also { block ->
                    var node = it.node
