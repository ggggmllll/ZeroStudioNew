...
/**
 * Safe accessor for [TSTree] instances. single [TSTree] is not thread-safe. This class adds lock to
 * make thread-safe access to the tree.
 *
 * @author Rosemoe
 */
class SafeTsTree(private val tree: TSTree) : AutoCloseable {
  ...
  /**
   * Make access to the tree. All access to the tree must be in the param [block]. The
   * [TreeAccessor] object will be invalid immediately after the given [block] is executed.
   *
  ...
  inner class TreeAccessor(internal var accessible: Boolean = true) {
    ...
    private fun checkAccess() {
      if (closed) throw IllegalStateException("executing operation on dead accessor")
    }
