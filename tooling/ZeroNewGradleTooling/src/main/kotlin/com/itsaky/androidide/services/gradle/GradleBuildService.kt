...
/** 一个运行 Gradle Tooling API 的 Android 前台服务。 负责执行构建任务、获取任务列表，并通过 Flow 分发日志和状态。 */
class GradleService : LifecycleService() {
    ...
    currentJob =
        lifecycleScope.launch(Dispatchers.IO) {
          ...
          try {
            ...
            val connector = GradleConnector.newConnector().forProjectDirectory(request.projectDir)
          ...
          try {
            ...
            buildLauncher.addProgressListener(
                { event ->
                  ...
                  lifecycleScope.launch { _events.emit(BuildEvent.Progress(event.description, -1)) }
                },
                OperationType.TASK,
            )
            ...
            emitLog(
                "Executing command: ./gradlew ${request.tasks.joinToString(" ")} ${args.joinToString(" ")}"
            )
  ...
  /** 日志输出流适配器。 将 Tooling API 的 OutputStream 转换为字符串并按行分发。 */
