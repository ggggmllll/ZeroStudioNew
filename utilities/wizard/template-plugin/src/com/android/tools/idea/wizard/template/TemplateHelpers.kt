...
fun classToResource(name: String): String =
    if (name.isNotEmpty()) AssetNameConverter(Type.CLASS_NAME, name).getValue(Type.RESOURCE) else ""

/**
 * Converts a String to Camel Case. The return will not contain any two consecutive upper case
 * characters For example: MyCLASsName to MyClassName my_class_name to MyClassName URL to Url
 */
...
fun underscoreToCamelCase(string: String): String =
    CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, string)

fun underscoreToLowerCamelCase(string: String): String =
    CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL, string)

fun escapeKotlinIdentifier(identifier: String): String =
    identifier.split(".").joinToString(".") { if (it in kotlinKeywords) "`$it`" else it }

/**
 * Creates a Java class name out of the given string, if possible. For example, "My Project" becomes
 * "MyProject", "hello" becomes "Hello", "Java's" becomes "Javas", and so on.
 *
...
fun extractClassName(string: String): String? {
  val javaIdentifier =
      string
          .dropWhile { !Character.isJavaIdentifierStart(it.toUpperCase()) }
...
fun getMaterialComponentName(mavenCoordinate: String, useAndroidX: Boolean): String {
  ...
  val mapping =
      mapOf(
          ...
          "android.support.v4.app.ActivityOptionsCompat" to
          ...
          "android.support.v4.app.NotificationChannelCompat" to
...
/**
 * Finds a resource given a file path and a context class. The implementation takes cares of the
 * details of different file path separators in different operating systems.
 */
