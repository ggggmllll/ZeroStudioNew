...
interface RecipeExecutor {
   ...
   * @param `stripPaths` ignores the first N directory paths within the compressed file (default is
   *   0).
   *
   * Usage demonstration: '''kotlin val mainDir = File(data.projectDir, "src/main")
  ...
  fun extractAsset(path: String, destDir: File, stripPaths: Int = 0) {
    openAsset(path).use { inputStream -> TarXzExtractor.extract(inputStream, destDir, stripPaths) }
  }

  /**
   * Unzip public method with force wait dialog. Prioritize blocking background threads and wait for
   * the UI rendering dialog to complete before performing decompression. Regardless of whether the
   * decompression is successful or failed, the dialog box will be automatically destroyed. If an
   * exception occurs, it will block subsequent project builds (preventing intent jumps).
   *
  ...
  fun extractAssetWithDialog(
    ...
    if (topActivity != null) {
      ...
      ThreadUtils.runOnUiThread {
        dialog =
            MaterialAlertDialogBuilder(topActivity)
