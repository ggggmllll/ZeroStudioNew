...
/**
 * Represents formatting options for the Kotlin code formatter. This class acts as a bridge between
 * IDE settings and the underlying ktfmt formatter options.
 *
 * @property indentSize The number of spaces to use for a single indentation level. Defaults to 4.
 * @property maxLineLength The maximum line length before the formatter attempts to wrap. Defaults
 *   to 100.
 * @property organizeImports If true, unused imports will be removed and the remaining ones will be
...
/**
 * A powerful code formatter for Kotlin files that uses the 'ktfmt' library from Facebook. It is
 * designed to produce consistent, idiomatic Kotlin code and handles complex reflowing and chained
 * calls gracefully.
 *
 * This implementation is resilient to parsing errors, returning the original source code if the
 * formatter encounters syntax that it cannot handle.
 *
 * @param options Configuration for the formatter. See [KotlinFormatOptions] for details.
 * @param errorStream A [PrintStream] to which formatting errors will be logged. Defaults to
 *   `System.err`.
 */
class KotlinFormatter(
   ...
   *
   * If the source code contains syntax errors that prevent `ktfmt` from parsing it, this method
   * will catch the exception, log an error message, and return the original, unformatted source
   * string.
   *
    ...
    return try {
      ...
      val ktfmtOptions =
          KtfmtOptions(
              ...
              continuationIndent =
                  options.indentSize *
                      2, // A common and readable convention is twice the block indent.
      ...
      errorStream.println(
          "ktfmt formatting failed due to a parsing error. This is often expected for incomplete code. Details: ${e.message}"
      )
      source
    } catch (e: Exception) {
      // Catch any other unexpected exceptions from the formatter.
      errorStream.println(
          "An unexpected error occurred during ktfmt formatting. Returning original source."
      )
