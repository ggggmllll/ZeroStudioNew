...
class JavaToKotlinAction(context: Context, override val order: Int) : EditorRelatedAction() {
  ...
  override suspend fun execAction(data: ActionData): Boolean {
    ...
    activity.lifecycleScope.launchAsyncWithProgress(
        configureFlashbar = { builder, _ -> builder.message(R.string.action_editor_java_to_kotlin) }
    ) { _, _ ->
      val code = editor.text.toString()
      val classpaths = module?.getCompileClasspaths() ?: emptySet()

      val result =
          withContext(Dispatchers.IO) { J2kConverterHelper.convert(file, code, classpaths) }
          ...
          try {
            ...
            val msg =
                if (result.backupPath != null) {
                  activity.getString(
                      R.string.action_java_to_kotlin_success_with_backup,
                      newFile.name,
                  )
                } else {
            ...
            activity.flashError(
                activity.getString(R.string.action_editor_java_to_kotlin_failed_write, errorMsg)
            )
          }
        }
      }
