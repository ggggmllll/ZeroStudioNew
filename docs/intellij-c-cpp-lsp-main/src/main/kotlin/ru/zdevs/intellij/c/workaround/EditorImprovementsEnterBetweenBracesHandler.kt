...
/**
 * ****************************************************************************
 * Copyright (c) 2025 Red Hat, Inc. Distributed under license by Red Hat, Inc. All rights reserved.
 * This program is made available under the terms of the Eclipse Public License v2.0 which
...
class EditorImprovementsEnterBetweenBracesHandler : EnterBetweenBracesFinalHandler() {
  ...
  override fun postProcessEnter(
    ...
    if (
      ...
      if (enterAfterOpenBrace && enterBeforeCloseBrace) {
        ...
        val bracedPairStartOffset =
            StringUtil.lastIndexOf(documentChars, openBraceCharacter!!, 0, offset + 1)
        var bracedPairEndOffset = StringUtil.indexOf(documentChars, closeBraceCharacter!!, offset)
        if ((bracedPairStartOffset > -1) && (bracedPairEndOffset > -1)) {
          ...
          if (bracedPairEndLineEndOffset > bracedPairEndOffset) {
            ...
            if (
                afterBracedPairEndText.isNotEmpty() &&
                    afterBracedPairEndText.trim { it <= ' ' }.isEmpty()
            ) {
        ...
        // Find the first non-whitespace character in the line as we'll only replace up to that
        // point
        val lineChars = documentChars.subSequence(lineStartOffset, lineEndOffset)
        val lineText = lineChars.toString()
        val firstNonWhitespaceCharacterIndex =
            StringUtil.findFirst(lineText) { lineCharacter: Char ->
              !Character.isWhitespace(lineCharacter)
            }
  ...
  companion object {
    private fun getCurrentIndentSize(file: PsiFile, document: Document, offset: Int): Int {
