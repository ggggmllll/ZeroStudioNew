...
class CModuleConfigurationEditor(val module: Module) : ModuleConfigurationEditor {
  ...
  init {
    buildSystem =
        ComboBox<String>(arrayOf(CProject.BUILD_SYSTEM_MAKEFILE, CProject.BUILD_SYSTEM_CMAKE))

    val content =
        FormBuilder.createFormBuilder().addLabeledComponent("Build system:", buildSystem).panel
