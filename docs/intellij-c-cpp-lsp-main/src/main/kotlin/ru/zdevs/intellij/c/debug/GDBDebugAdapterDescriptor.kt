...
class GDBDebugAdapterDescriptor(
  ...
  override fun startServer(): ProcessHandler {
    ...
    if (
        command.isNullOrEmpty() || LLDBDebugAdapterDescriptor.DEBUGGER_EXEC_NAME.contains(command)
    ) {
      command = Utils.findExecutableInPATH(DEBUGGER_EXEC_NAME)
    }
    if (command.isNullOrEmpty()) {
      throw ExecutionException(
          "GDB not found. Make sure it is installed properly (and `${DEBUGGER_EXEC_NAME[0]}` available in PATH), and restart the IDE."
      )
    }
