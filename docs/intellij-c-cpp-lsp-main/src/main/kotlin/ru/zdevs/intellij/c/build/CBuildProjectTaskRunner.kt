...
class CBuildProjectTaskRunner : ProjectTaskRunner() {
  ...
  override fun run(
    ...
    tasks.forEach { task ->
      ...
      invokeLater { start(task.module, resultPromise, !task.isIncrementalBuild) }
    }
  ...
  private fun startConfigure(
      module: Module,
      workDirectory: String?,
      resultPromise: AsyncPromise<Result>,
  ) {
    val commandLine = getCommandLine(workDirectory).withExePath("cmake").withParameters("..")
    ...
    handler.addProcessListener(
        CBuildAdapter(
            ...
            module,
            "Run CMake",
            CMakeBuildParser(ctx, module.moduleFile?.parent?.path),
        )
    )
    handler.addProcessListener(
  ...
  private fun startBuild(
    ...
    if (cmake) {
      commandLine.withExePath("cmake").withParameters("--build", ".")
    } else {
      val bear = findBearExecutableInPATH()
      if (bear != null) {
        // use bear for compile_commands.json generation
        commandLine.withExePath("bear").withParameters("--append", "--", "make", "all")
      } else {
        commandLine.withExePath("make").withParameters("all")
      }
    }
    ...
    val ctx = CBuildContext(handler)
    handler.addProcessListener(
        CBuildAdapter(ctx, module, "Run C/C++ Build", MakeBuildParser(ctx, workDirectory))
    )
    handler.addProcessListener(
        object : ProcessAdapter() {
          override fun processTerminated(event: ProcessEvent) {
  ...
  private fun startClean(
      module: Module,
      resultPromise: AsyncPromise<Result>,
      settings: CModuleSettings,
  ) {
    val workDirectory = module.moduleFile?.parent?.path

    val commandLine = getCommandLine(workDirectory)
    if (settings.buildSystem == CProject.BUILD_SYSTEM_CMAKE) {
      commandLine.withExePath("cmake").withParameters("--build", "build", "--target", "clean")
    } else {
      commandLine.withExePath("make").withParameters("clean")
    }

    val handler = BlockedProcessHandler(commandLine)
    val ctx = CBuildContext(handler)
    handler.addProcessListener(
        CBuildAdapter(ctx, module, "Run clean", CMakeBuildParser(ctx, workDirectory))
    )
    handler.addProcessListener(
        object : ProcessAdapter() {
          override fun processTerminated(event: ProcessEvent) {
