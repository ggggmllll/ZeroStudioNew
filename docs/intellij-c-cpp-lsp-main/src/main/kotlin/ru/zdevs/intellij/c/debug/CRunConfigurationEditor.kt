...
class CRunConfigurationEditor : SettingsEditor<DAPRunConfiguration>() {
  ...
  init {
    ...
    command.addBrowseFolderListener(
    ...
    workDir.addBrowseFolderListener(
        TextBrowseFolderListener(
            FileChooserDescriptorFactory.createSingleFolderDescriptor()
                .withTitle("Select Working Directory")
        )
    )
    executableFileChooser =
        FileChooserDescriptorFactory.createSingleFileDescriptor()
            .withTitle("Select Target Executable")
            .withFileFilter { file -> !SystemInfo.isWindows || file.extension == "exe" }
    ...
    val args = FormBuilder.createFormBuilder().addComponentFillVertically(arguments, 0).panel
  ...
  override fun resetEditorFrom(configuration: DAPRunConfiguration) {
    ...
    command.text =
        if (configuration.command.isNullOrEmpty()) SERVER_COMMAND[serverIndex][0]
    ...
    workDir.whenTextChanged {
      executableFileChooser.withRoots(
          LocalFileSystem.getInstance().findFileByIoFile(File(workDir.text))
      )
    }
    executableFileChooser.withRoots(
        LocalFileSystem.getInstance().findFileByIoFile(File(workDir.text))
  ...
  companion object {
    ...
    val SERVER_COMMAND =
        arrayOf(
            LLDBDebugAdapterDescriptor.DEBUGGER_EXEC_NAME,
            GDBDebugAdapterDescriptor.DEBUGGER_EXEC_NAME,
        )
  }
}
