  ...
  class Step(parent: NewProjectWizardStep) : AbstractNewProjectWizardStep(parent) {
    ...
    override fun setupUI(builder: Panel) {
      ...
      with(builder) {
        row("Build system:") {
          buildSystem =
              segmentedButton(listOf(CProject.BUILD_SYSTEM_MAKEFILE, CProject.BUILD_SYSTEM_CMAKE)) {
                text = it
              }
        }
        row("Language:") {
          languageType =
              segmentedButton(listOf(CProject.LANGUAGE_C, CProject.LANGUAGE_CPP)) { text = it }
        }
        row("Compiler:") {
          val ui =
              cell(ComponentWithBrowseButton<ComboBox<String>>(toolChain, {}))
                  .align(Align.FILL)
                  .component
          ...
          choose.withFileFilter { file ->
            file.name.endsWith("gcc$EXEC_EXTENSION") or
                file.name.endsWith("g++$EXEC_EXTENSION") or
    ...
    override fun setupProject(project: Project) {
      ...
      val templateMainName =
          if (languageType.selectedItem == CProject.LANGUAGE_CPP) "main.cpp" else "main.c"
      ...
      val templateMakeName =
          if (buildSystem.selectedItem == CProject.BUILD_SYSTEM_MAKEFILE) "Makefile"
          else "CMakeLists.txt"
      val templateMake = templateManager.getInternalTemplate(templateMakeName)
      FileUtil.writeToFile(
          Path.of(project.basePath, templateMakeName).toFile(),
          templateMake.getText(properties),
      )

      FileUtil.writeToFile(
          Path.of(project.basePath, ".gitignore").toFile(),
          """
          *.o
          *.so
          *.exe
      ...
      if (
          toolChain.selectedIndex > 1 && !state.toolChain.contains(toolChain.selectedItem as String)
      )
