...
class MakeBuildParser(ctx: CBuildContext, private val path: String?) :
    BuildOutputParser, BaseBuildParser(ctx) {
  private val workingDirection: Stack<String> = Stack()

  private val parsers: Array<IErrorParser> =
      arrayOf(
          // GCC / Clang
          GCCErrorParser(
              "(.*?):(\\d+):((\\d+):)?\\s*(fatal )?(([Ee]rror)|(ERROR)): (.*)",
          ...
          GCCErrorParser(
              "(.*?):(\\d+):((\\d+):)?\\s*(([Nn]ote)|(NOTE)|([Ii]nfo)|(INFO)|(remark)): (.*)",
          ...
          CallbackErrorParser("make\\[(\\d+)\\]: Entering directory '(.*)'") {
              _: BaseBuildParser,
              matcher: Matcher ->
  ...
  private fun getWorkingDir(): String {
    return if (workingDirection.isEmpty()) path ?: "/" else workingDirection.peek()
  }

  override fun getFilePath(file: String): File {
    return if (file.startsWith("/")) File(file) else File(getWorkingDir(), file)
  }
  ...
  companion object {
    ...
    fun withNewLine(str: String): String =
        if (StringUtil.endsWithLineBreak(str)) str else str + '\n'
