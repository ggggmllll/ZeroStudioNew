...
/**
 * A <tt>MemoryUsage</tt> object represents a snapshot of memory usage. Instances of the
 * <tt>MemoryUsage</tt> class are usually constructed by methods that are used to obtain memory
 * usage information about individual memory pool of the Java virtual machine or the heap or
 * non-heap memory of the Java virtual machine as a whole.
 *
 ...
 *
 * <tt>MemoryUsage</tt> is mapped to a {@link CompositeData CompositeData} with attributes as
 * specified in the {@link #from from} method.
 *
 * @author Mandy Chung
 * @since 1.5
 */
public class MemoryUsage {
   ...
   * @param max the maximum amount of memory in bytes that can be used; or <tt>-1</tt> if undefined.
   * @throws IllegalArgumentException if
   *     <ul>
   *       <li>the value of <tt>init</tt> or <tt>max</tt> is negative but not <tt>-1</tt>; or
   *       <li>the value of <tt>used</tt> or <tt>committed</tt> is negative; or
   *       <li><tt>used</tt> is greater than the value of <tt>committed</tt>; or
   *       <li><tt>committed</tt> is greater than the value of <tt>max</tt> <tt>max</tt> if defined.
   *     </ul>
   */
   ...
   * operating system for memory management. This method returns <tt>-1</tt> if the initial memory
   * size is undefined.
   *
   ...
   * method returns <tt>-1</tt> if the maximum memory size is undefined.
   *
   * <p>This amount of memory is not guaranteed to be available for memory management if it is
   * greater than the amount of committed memory. The Java virtual machine may fail to allocate
