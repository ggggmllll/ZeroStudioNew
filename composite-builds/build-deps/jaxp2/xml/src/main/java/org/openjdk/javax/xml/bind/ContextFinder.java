...
class ContextFinder {
  ...
  private static final String PLATFORM_DEFAULT_FACTORY_CLASS =
      "org.openjdk.com.sun.xml.internal.bind.v2.ContextFactory";

  // previous value of JAXBContext.JAXB_CONTEXT_FACTORY, using also this to ensure backwards
  // compatibility
  private static final String JAXB_CONTEXT_FACTORY_DEPRECATED =
      "org.openjdk.javax.xml.bind.context.factory";
   ...
   * the wrapped exception. Otherwise returns exception to be wrapped for further processing.
   */
  private static Throwable handleInvocationTargetException(InvocationTargetException x)
  ...
  private static JAXBException handleClassCastException(Class originalType, Class targetType) {
    ...
    return new JAXBException(
        Messages.format(
            ...
            // we don't care where the impl class is, we want to know where JAXBContext lives in the
            // impl
            // class' ClassLoader
  ...
  static JAXBContext newInstance(
    ...
    try {
      Class spFactory =
          ServiceLoaderUtil.safeLoadClass(className, PLATFORM_DEFAULT_FACTORY_CLASS, classLoader);
  ...
  private static Object instantiateProviderIfNecessary(final Class<?> implClass)
      throws JAXBException {
    try {
  ...
  static JAXBContext newInstance(Class[] classes, Map properties, String className)
    ...
    try {
      spi =
          ServiceLoaderUtil.safeLoadClass(
              className, PLATFORM_DEFAULT_FACTORY_CLASS, getContextClassLoader());
  ...
  static JAXBContext find(
    ...
    if (factoryClassName != null) {
      return newInstance(
          contextPath, contextPathClasses, factoryClassName, classLoader, properties);
    }
    ...
    Class ctxFactory =
        (Class)
            ServiceLoaderUtil.lookupUsingOSGiServiceLoader("javax.xml.bind.JAXBContext", logger);
    ...
    return newInstance(
        contextPath, contextPathClasses, PLATFORM_DEFAULT_FACTORY_CLASS, classLoader, properties);
  }
  ...
  static JAXBContext find(Class<?>[] classes, Map<String, ?> properties) throws JAXBException {
    ...
    for (final Class c : classes) {
      ...
      // classes from the same package might come from different class loades, so it might be a bad
      // idea
  ...
  private static String classNameFromPackageProperties(
      URL packagePropertiesUrl, String... factoryIds) throws JAXBException {
  ...
  @Deprecated
  static String firstByServiceLoaderDeprecated(Class spiClass, ClassLoader classLoader)
      throws JAXBException {
  ...
  private static String jaxbProperties(
      String contextPath, ClassLoader classLoader, String factoryId) throws JAXBException {
    ...
    for (String pkg : packages) {
      ...
      if (jaxbPropertiesUrl != null) {
        return classNameFromPackageProperties(
            jaxbPropertiesUrl, factoryId, JAXB_CONTEXT_FACTORY_DEPRECATED);
      }
    }
    return null;
  }

  private static String jaxbProperties(Class[] classesFromContextPath, String factoryId)
      throws JAXBException {
