...
public interface Serializer {
   ...
   * called more, or over-riding property values, can be specified. One property value that can not
   * be changed is that of the "method" property key.
   *
   ...
   *
   * <p>If the "cdata-section-elements" key is specified on multiple calls to this method the set of
   * elements specified in the value is not replaced from one call to the next, but it is cumulative
   * across the calls.
   *
  ...
  /**
   * Return a {@link ContentHandler} interface to provide SAX input to. Through the returned object
   * the document to be serailized, as a series of SAX events, can be provided to the serialzier. If
   * the serializer does not support the {@link ContentHandler} interface, it will return null.
   *
  ...
  /**
   * Return a {@link DOMSerializer} interface into this serializer. Through the returned object the
   * document to be serialized, a DOM, can be provided to the serializer. If the serializer does not
  ...
  /**
   * This method resets the serializer. If this method returns true, the serializer may be used for
   * subsequent serialization of new documents. It is possible to change the output format and
   * output stream prior to serializing, or to reuse the existing output format and output stream or
   * writer.
   *
