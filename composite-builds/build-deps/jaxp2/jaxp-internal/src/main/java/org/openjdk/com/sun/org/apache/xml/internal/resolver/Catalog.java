...
public class Catalog {
   ...
   * CATALOG entry. If one catalog file includes another with a CATALOG entry, the included catalog
   * logically occurs <i>at the end</i> of the including catalog, and after any preceding CATALOG
   * entries. In other words, the CATALOG entry cannot insert anything into the middle of a catalog
   * file.
   *
   * <p>When processing reaches the end of each catalog files, any elements on this vector are added
   * to the front of the <code>catalogFiles</code> vector.
   *
   * @see #catalogFiles
   */
   ...
   *
   * <p>Catalogs that are processed as the result of CATALOG or DELEGATE* entries are subordinate to
   * the catalog that contained them, but they may in turn have subordinate catalogs.
   *
   ...
   *
   * <p>This two-level system has two purposes; first, it allows us to sort the DELEGATE* entries by
   * the length of the partial public identifier so that a linear search encounters them in the
   * correct order and second, it puts them all at the end of the Catalog.
   *
   * <p>When processing reaches the end of each catalog file, any elements on this vector are added
   * to the end of the <code>catalogEntries</code> vector. This assures that matching PUBLIC
   ...
   *
   * <p>This vector contains all of the readers in the order that they were added. In the event that
   * a catalog is read from a file, where the MIME type is unknown, each reader is attempted in turn
   * until one succeeds.
   */
   ...
   *
   * <p>In the absence of a mimeType (e.g., when reading a catalog directly from a file on the local
   * system), the readers are attempted in the order that you add them to the Catalog.
   *
   * <p>Note that subordinate catalogs (created by CATALOG or DELEGATE* entries) get a copy of the
   * set of readers present in the primary catalog when they are created. Readers added subsequently
   * will not be available. For this reason, it is best to add all of the readers before the first
   * call to parse a catalog.
   *
   ...
   *
   * <p>This method is used internally when constructing a new catalog. It copies the current reader
   * associations over to the new catalog.
   *
   ...
   *
   * <p>This method constructs a new instance of the running Catalog class (which might be a subtype
   * of com.sun.org.apache.xml.internal.resolver.Catalog). All new catalogs are managed by the same
   * CatalogManager.
   *
   * <p>N.B. All Catalog subtypes should call newCatalog() to construct a new Catalog. Do not simply
   * use "new Subclass()" since that will confuse future subclasses.
   */
   ...
   *
   * <p>All catalog files loaded by this catalog will have the initial override setting specified by
   * this default.
   */
   ...
   *
   * <p>The method adds all of the catalogs specified in the <tt>xml.catalog.files</tt> property to
   * the Catalog list.
   *
   ...
   * Parse a catalog file, augmenting internal data structures.
   *
   * <p>Catalogs retrieved over the net may have an associated MIME type. The MIME type can be used
   * to select an appropriate reader.
   *
   ...
   * throw an exception, you can be confident that no subsequent call to any resolve*() method will
   * either, with two possible exceptions:
   *
   * <ol>
   *   <li>
   *       <p>Delegated catalogs are re-parsed each time they are needed (because a variable list of
   *       them may be needed in each case, depending on the length of the matching partial public
   *       identifier).
   *       <p>But they are parsed by this method, so as long as they don't change or disappear while
   *       the program is running, they shouldn't generate errors later if they don't generate
   *       errors now.
   *   <li>
  ...
  public String resolveDoctype(String entityName, String publicId, String systemId)
    ...
    if (systemId != null && systemId.startsWith("urn:publicid:")) {
      ...
      if (publicId != null && !publicId.equals(systemId)) {
        catalogManager.debug.message(
            1,
            "urn:publicid: system identifier differs from public identifier; using public"
                + " identifier");
        systemId = null;
      } else {
        publicId = systemId;
   ...
   * system or public identifiers. If no appropriate PUBLIC or SYSTEM entry is found in the Catalog,
   * null is returned.
   *
   ...
   *     if a match is not found in the catalog, instead null is returned to indicate that no match
   *     was found.
   */
   ...
   *   <li>If the system identifier is null, then all of the PUBLIC entries are interrogated and the
   *       first matching entry is returned. This may not be the same as the preceding case, if some
   *       PUBLIC entries are encountered when OVERRIDE NO is in effect. In XML, the only place
   *       where a public identifier may occur without a system identifier is in a notation
   *       declaration.
   *   <li>Finally, if the public identifier matches one of the partial public identifiers specified
   *       in a DELEGATE* entry in the Catalog, the delegated catalog is interrogated. The first
   *       time that the delegated catalog is required, it will be retrieved and parsed. It is
   *       subsequently cached.
   * </ul>
   *
   ...
   *
   * <p>If a SYSTEM entry exists in the catalog file for the system ID specified, return the mapped
   * value.
   *
   * @param systemId The system ID to locate in the catalog
   ...
   *
   * <p>This method searches the Catalog and returns the system identifier specified for the given
   * entity type with the given name, public, and system identifiers. In some contexts, these may be
   * null.
   *
   * @param entityType The CatalogEntry type for which this query is being conducted. This is
