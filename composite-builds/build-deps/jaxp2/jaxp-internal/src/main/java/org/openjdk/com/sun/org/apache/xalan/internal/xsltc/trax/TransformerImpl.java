...
public final class TransformerImpl extends Transformer implements DOMCache, ErrorListener {
  ...
  public SerializationHandler getOutputHandler(Result result) throws TransformerException {
    ...
    try {
      ...
      if (result instanceof SAXResult) {
        ...
        /**
         * Fix for bug 24414 If the lexicalHandler is set then we need to get that for obtaining the
         * lexical information
         */
  ...
  /**
   * Implements JAXP's Transformer.getErrorListener() Get the error event handler in effect for the
   * transformation.
   *
  ...
  /**
   * Implements JAXP's Transformer.setErrorListener() Set the error event listener in effect for the
   * transformation. Register a message handler in the translet in order to forward xsl:messages to
   * error listener.
   *
   ...
   * properties set by calls to setOutputProperty() and setOutputProperties() on this class, and the
   * output settings defined in the stylesheet's <xsl:output> element makes up the second level,
   * while the default XSLT output settings are returned on the third level.
   *
   ...
   * the transformation. The property specified may be a property that was set with
   ...
   * transformation. These properties will override properties set in the Templates with xsl:output.
   ...
   * the default layer and the base layer. The latter contains properties defined in the stylesheet
   * or by the user using this API.
   */
  ...
  /**
   * Implements JAXP's Transformer.getParameter() Returns the value of a given parameter. Note that
   * the translet will not keep values for parameters that were not defined in the stylesheet.
   *
   * @param name The name of the parameter
  ...
  /**
   * Implements JAXP's Transformer.setURIResolver() Set an object that will be used to resolve URIs
   * used in document().
   *
   ...
   * interface and XSLTC's DOMCache interface. This approach is simple, but removes the possibility
   * of using external document caches with XSLTC.
   *
