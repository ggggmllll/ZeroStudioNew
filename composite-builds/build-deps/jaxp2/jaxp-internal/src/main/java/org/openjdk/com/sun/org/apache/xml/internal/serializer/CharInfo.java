...
final class CharInfo {
  ...
  /**
   * This flag is an optimization for HTML entities. It false if entities other than quot (34), amp
   * (38), lt (60) and gt (62) are defined in the range 0 to 127.
   *
   * @xsl.usage internal
   */
   ...
   * is complete, the isSpecialAttrASCII array is used first because access to its values is common
   * and faster.
   */
  ...
  /**
   * Defines a new character reference. The reference's name and value are supplied. Nothing happens
   * if the character reference is already defined.
   *
   ...
   * fully decorated entity name "&lt;". Strings for entity references are loaded from a properties
   * file, but additional mappings defined through calls to defineChar2String() are possible. Such
   ...
   * that introduces a threading risk. Simplest fix for now is to make it a synchronized method, or
   * to give up the reuse; I see very little performance difference between them. Long-term solution
   * would be to replace the hashtable with a sparse array keyed directly from the character's
  ...
  /**
   * This method is used to determine if an ASCII character in a text node (not an attribute value)
   * is "clean".
   *
   ...
   *
   * <p>This implementation uses an array of integers with 32 bits per integer. If a bit is set to 1
   * the corresponding integer is in the set of integers.
   *
