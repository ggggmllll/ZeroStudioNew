...
final class TestSeq {
  ...
  /**
   * Returns a string representation of this test sequence. Notice that test sequences are mutable,
   * so the value returned by this method is different before and after calling reduce().
   */
  ...
  /**
   * Return the highest priority for a pattern in this test sequence. This is either the priority of
   * the first or of the default pattern.
   */
  ...
  /**
   * Get the instruction handle to a template's code. This is used when a single template occurs in
   * several test sequences; that is, if its pattern is a union of patterns (e.g. match="A/B |
   * A/C").
   */
  ...
  /**
   * Compile the code for this test sequence. Compile patterns from highest to lowest priority. Note
   * that since patterns can be share by multiple test sequences, instruction lists must be copied
   * before backpatching.
   */
