...
class RegexParser {
      ...
      if (locale != null) {
        this.resources =
            SecuritySupport.getResourceBundle(
                "org.openjdk.com.sun.org.apache.xerces.internal.impl.xpath.regex.message", locale);
      } else {
        this.resources =
            SecuritySupport.getResourceBundle(
  ...
  */
   ...
   * '\B' | '\<' | '\>' | atom (('*' | '+' | '?' | minmax ) '?'? )?) | '(?=' regex ')' | '(?!' regex
   * ')' | '(?&lt;=' regex ')' | '(?&lt;!' regex ')' atom ::= char | '.' | range | '(' regex ')' |
   ...
   * | minmax ) '?'? )?) | '(?=' regex ')' | '(?!' regex ')' | '(?&lt;=' regex ')' | '(?&lt;!' regex
  ...
  Token parseAtom() throws ParseException {
    ...
    switch (ch) {
       ...
       * bmp-code | character-2 bmp-char ::= '\' 'u' [0-9a-fA-F] [0-9a-fA-F] [0-9a-fA-F] [0-9a-fA-F]
       */
      case T_LBRACKET:
  ...
  protected RangeToken parseCharacterClass(boolean useNrange) throws ParseException {
    ...
    while ((type = this.read()) != T_EOF) {
      ...
      // POSIX Character class such as [:alnum:]
        ...
        RangeToken range =
            Token.getRange(name, positive, this.isSet(RegularExpression.XMLSCHEMA_MODE));
