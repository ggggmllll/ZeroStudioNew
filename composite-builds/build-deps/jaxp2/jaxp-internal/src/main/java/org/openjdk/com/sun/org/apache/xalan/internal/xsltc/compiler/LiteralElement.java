...
final class LiteralElement extends Instruction {
   ...
   * its attributes. The output must contain a definition for each namespace, so we stuff them in a
   * hashtable.
   */
   ...
   * xsl:stylesheet. Also registers a QName to assure that the output element contains the necessary
  ...
  /**
   * Type-check the contents of this element. The element itself does not need any type checking as
   * it leaves nothign on the JVM's stack.
   */
  ...
  /**
   * Return the ElemDesc object for an HTML element. Return null if the output method is not HTML or
   ...
   * an element. Only return false when we are sure that no attribute node is produced. Return true
   * if we are not sure. If the flag ignoreXslAttribute is true, the direct <xsl:attribute> children
