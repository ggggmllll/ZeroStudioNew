...
public class XSConstraints {
  ...
  /**
   * check whether complex type derived is valid derived from base, given a subset of {restriction,
   * extension}.
   */
  ...
  /**
   * Note: this will be a private method, and it assumes that derived is not anySimpleType, and base
   * is not anyType. Another method will be introduced for public use, which will call this method.
   */
  private static boolean checkSimpleDerivation(
    ...
    // 2.2.2 D's base type definition is not the simple ur-type definition and is validly derived
  ...
  /**
   * Note: this will be a private method, and it assumes that derived is not anyType. Another method
   * will be introduced for public use, which will call this method.
   */
  ...
  public static Object ElementDefaultValidImmediate(
    ...
    // true:
      ...
      // 2.2.1 If the {content type} is a simple type definition, then the string must be valid with
      // respect to that simple type definition as defined by String Valid (3.14.4).
      if (ctype.fContentType == XSComplexTypeDecl.CONTENTTYPE_SIMPLE) {
        dv = ctype.fXSSimpleType;
      }
      // 2.2.2 If the {content type} is mixed, then the {content type}'s particle must be emptiable
  ...
  static void reportSchemaError(
    ...
    if (loc != null) {
      errorReporter.reportError(
          loc, XSMessageFormatter.SCHEMA_DOMAIN, key, args, XMLErrorReporter.SEVERITY_ERROR);
    } else {
      errorReporter.reportError(
  ...
  /**
   * used to check the 3 constraints against each complex type (should be each model group): Unique
   * Particle Attribution, Particle Derivation (Restriction), Element Declrations Consistent.
   */
  ...
  */
      ...
      case XSParticleDecl.PARTICLE_ELEMENT:
        {
          switch (bType) {
            ...
            case XSModelGroupImpl.MODELGROUP_CHOICE:
              {
                ...
                checkRecurseLax(
                    dChildren, 1, 1, dSGHandler, bChildren, bMinOccurs, bMaxOccurs, bSGHandler);
                return bExpansionHappened;
              }
            ...
            default:
              {
                throw new XMLSchemaException(
                    "Internal-Error", new Object[] {"in particleValidRestriction"});
              }
          }
        }
      ...
      case XSParticleDecl.PARTICLE_WILDCARD:
        {
          switch (bType) {
            ...
            case XSParticleDecl.PARTICLE_ELEMENT:
              {
                throw new XMLSchemaException(
                    "cos-particle-restrict.2", new Object[] {"any:choice,sequence,all,elt"});
              }
      ...
      case XSModelGroupImpl.MODELGROUP_ALL:
        {
          switch (bType) {
            ...
            case XSParticleDecl.PARTICLE_ELEMENT:
              {
                throw new XMLSchemaException(
                    "cos-particle-restrict.2", new Object[] {"all:choice,sequence,elt"});
              }
      ...
      case XSModelGroupImpl.MODELGROUP_CHOICE:
        {
          switch (bType) {
            ...
            case XSParticleDecl.PARTICLE_ELEMENT:
              {
                throw new XMLSchemaException(
                    "cos-particle-restrict.2", new Object[] {"choice:all,sequence,elt"});
              }
  ...
  private static void checkNameAndTypeOK(
    ...
    if (dElement.fName != bElement.fName
      ...
      throw new XMLSchemaException(
          ...
          new Object[] {
            dElement.fName, dElement.fTargetNamespace, bElement.fName, bElement.fTargetNamespace
          });
    }
    ...
    if (bElement.getConstraintType() == XSConstants.VC_FIXED) {
      ...
      if (!isSimple && !bElement.fDefault.normalizedValue.equals(dElement.fDefault.normalizedValue)
        ...
        throw new XMLSchemaException(
            ...
            new Object[] {
              dElement.fName, dElement.fDefault.stringValue(), bElement.fDefault.stringValue()
            });
      }
    }
  ...
  private static boolean checkOccurrenceRange(int min1, int max1, int min2, int max2) {
    ...
    if ((min1 >= min2)
        && ((max2 == SchemaSymbols.OCCURRENCE_UNBOUNDED)
            || (max1 != SchemaSymbols.OCCURRENCE_UNBOUNDED && max1 <= max2))) return true;
    else return false;
  }
  ...
  private static void checkNSSubset(
      XSWildcardDecl dWildcard, int min1, int max1, XSWildcardDecl bWildcard, int min2, int max2)
      throws XMLSchemaException {
    ...
    if (dWildcard.weakerProcessContents(bWildcard)) {
      throw new XMLSchemaException(
          "rcase-NSSubset.3",
          new Object[] {
            dWildcard.getProcessContentsAsString(), bWildcard.getProcessContentsAsString()
          });
    }
  }
