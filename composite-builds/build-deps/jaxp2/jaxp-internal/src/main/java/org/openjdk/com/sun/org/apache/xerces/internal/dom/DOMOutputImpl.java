...
public class DOMOutputImpl implements LSOutput {
   ...
   * encoding attribute. Setting the encoding in this way will override any encoding specified in an
   * XML declaration in the data.
   */
  ...
  /**
   * Depending on the language binding in use, this attribute may not be available. An attribute of
   * a language and binding dependent type that represents a writable stream to which 16-bit units
   * can be output. The application must encode the stream using UTF-16 (defined in [Unicode] and
   ...
   * application knows the character encoding of the object pointed to by the system identifier, it
   * can set the encoding using the encoding attribute. If the system ID is a relative URI reference
   * (see section 5 in [IETF RFC 2396]), the behavior is implementation dependent.
   */
   ...
   * declaration ([XML 1.0] section 4.3.3 "Character Encoding in Entities"). This attribute has no
   * effect when the application provides a character stream or string data. For other sources of
   * input, an encoding specified by means of this attribute will override any encoding specified in
   * the XML declaration or the Text declaration, or an encoding obtained from a higher level
   * protocol, such as HTTP [IETF RFC 2616].
   */
