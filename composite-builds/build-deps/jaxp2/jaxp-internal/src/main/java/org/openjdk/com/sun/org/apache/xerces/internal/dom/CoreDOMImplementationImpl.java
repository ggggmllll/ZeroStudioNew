...
public class CoreDOMImplementationImpl implements DOMImplementation, DOMImplementationLS {
  ...
  //
  /**
   * Test if the DOM implementation supports a specific "feature" -- currently meaning language and
   * level thereof.
   *
   * @param feature The package name of the feature to test. In Level 1, supported values are "HTML"
   *     and "XML" (case-insensitive). At this writing, com.sun.org.apache.xerces.internal.dom
   *     supports only XML.
   ...
   * parser may then be configured by means of its <code>DOMConfiguration</code> object, and used to
   * parse documents by means of its <code>parse</code> method.
   *
   ...
   *     lexical checking is done on the absolute URI. In order to create a <code>LSParser</code>
   *     for any kind of schema types (i.e. the LSParser will be free to use any schema found), use
   ...
   *     the <code>"error-handler"</code> configuration parameter on the new created <code>LSParser
   *     </code> contains a reference to the default error handler.
