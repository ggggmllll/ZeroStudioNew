...
public abstract class SyntaxTreeNode implements Constants {
   ...
   * checks namespace mappings for this element, and if necessary for ancestor elements as well (ie.
   * if the URI is declared within the current scope then you'll definately get the prefix from this
   * method). Note that this is a very slow method and consequentially it should only be used
  ...
  /**
   * Set this node's parser. The parser (the XSLT parser) gives this syntax tree node access to the
   * symbol table and XPath parser.
   *
   ...
   * Note that this method will return 'null' for nodes that represent top-level elements.
   *
   ...
   * functions). The default behaviour is to parser the syntax tree node's children (since there are
   * no common expressions, patterns, etc. that can be handled in this base class.
   *
  ...
  /**
   * Type check the children of this node. The type check phase may add coercions (CastExpr) to the
   * AST.
   *
   ...
   *
   * <p>A node is an adaptive RTF if each children is a Text element or it is <xsl:call-template> or
   * <xsl:apply-templates>.
   *
   * @param node A node
   ...
   *
   * <p>A node is a Text element if it is Text, xsl:value-of, xsl:number, or a combination of these
   * nested in a control instruction (xsl:if or xsl:choose).
   *
  ...
  protected void compileResultTree(ClassGenerator classGen, MethodGenerator methodGen) {
    ...
    if (stylesheet.callsNodeset() && !DOM_CLASS.equals(DOM_IMPL_CLASS)) {
      ...
      if (!stylesheet.callsNodeset()) {
        ...
        il.append(ALOAD_0);
        il.append(
            new GETFIELD(cpg.addFieldref(TRANSLET_CLASS, NAMESPACE_INDEX, NAMESPACE_INDEX_SIG)));
