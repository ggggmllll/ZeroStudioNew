...
public class MethodGenerator extends MethodGen
  ...
  /**
   * A mapping between patterns and instruction lists used by test sequences to avoid compiling the
   * same pattern multiple times. Note that patterns whose kernels are "*", "node()" and "@*" can
   * between shared by test sequences.
   */
  ...
  public MethodGenerator(
    ...
    final int startElement =
        cpg.addInterfaceMethodref(TRANSLET_OUTPUT_INTERFACE, "startElement", START_ELEMENT_SIG);
    ...
    final int attribute =
        cpg.addInterfaceMethodref(
            TRANSLET_OUTPUT_INTERFACE, "addAttribute", "(" + STRING_SIG + STRING_SIG + ")V");
    _attribute = new INVOKEINTERFACE(attribute, 3);

    final int uniqueAttribute =
        cpg.addInterfaceMethodref(
            TRANSLET_OUTPUT_INTERFACE, "addUniqueAttribute", "(" + STRING_SIG + STRING_SIG + "I)V");
   ...
   * addLocalVariable2() so that the new variable is known to the allocator. Failing to do this may
   * cause the allocator to return a slot that is already in use.
   */
   ...
   * </code> and {@link
   * MethodGen#addLocalVariable(String,Type,int,InstructionHandle,InstructionHandle)}</code> methods
   * of {@link MethodGen} will only keep track of {@link LocalVariableGen} object until it'ss
   * removed by a call to {@link MethodGen#removeLocalVariable(LocalVariableGen)}.
   *
   * <p>In order to support efficient copying of local variables to outlined methods by {@link
   * #outline(InstructionHandle,InstructionHandle,String,ClassGenerator)}, this class keeps track of
   * all local variables defined by the method.
   */
  protected class LocalVariableRegistry {
    /**
     * A <code>java.lang.ArrayList</code> of all {@link LocalVariableGen}s created for this method,
     * indexed by the slot number of the local variable. The JVM stack frame of local variables is
     * divided into "slots". A single slot can be used to store more than one variable in a method,
     * without regard to type, so long as the byte code keeps the ranges of the two disjoint.
     *
     * <p>If only one registration of use of a particular slot occurs, the corresponding entry of
     * <code>_variables</code> contains the <code>LocalVariableGen</code>; if more than one occurs,
     * the corresponding entry contains all such <code>LocalVariableGen</code>s registered for the
     ...
     *       instructions for any <code>LocalVariableGen</code> with the same slot index previously
     *       registered for this method. <b><em>(Unchecked.)</em></b>
     * </ul>
     *
    ...
    /**
     * Find which {@link LocalVariableGen}, if any, is registered for a particular JVM local stack
     * frame slot at a particular position in the byte code for the method.
     *
     * <p><b>Preconditions:</b>
     *
     * <ul>
    ...
    /**
     * Set up a mapping of the name of the specified {@link LocalVariableGen} object to the <code>
     * LocalVariableGen</code> itself.
     *
     * <p>This is a bit of a hack. XSLTC is relying on the fact that the name that is being looked
     * up won't be duplicated, which isn't guaranteed. It replaces code which used to call {@link
     * MethodGen#getLocalVariables()} and looped through the <code>LocalVariableGen</code> objects
     * it contained to find the one with the specified name. However, <code>getLocalVariables()
     * </code> has the side effect of setting the start and end for any <code>LocalVariableGen
     * </code> which did not already have them set, which causes problems for outlining..
     *
     * <p>See also {@link #lookUpByName(String)} and {@link #removeByNameTracking(LocalVariableGen)}
     * LocalVariableGen</code>
     */
     ...
     * setting the start and end range for any <code>LocalVariableGen</code> if either was <code>
     * null</code>. That side-effect causes problems for outlining of code in XSLTC.
     *
  ...
  /**
   * Determines whether a particular variable is in use at a particular offset in the byte code for
   * this method.
   *
   * <p><b>Preconditions:</b>
   *
  ...
  /**
   * Get the instruction list for a pre-compiled pattern. Used by test sequences to avoid compiling
  ...
  private class Chunk implements Comparable {
    ...
    /**
     * {@link org.apache.bcel.generic.InstructionHandle} of the first instruction in the outlineable
     * chunk.
     */
     ...
     * </ul>
     *
     * @param start The {@link InstructionHandle} of the first instruction in the outlineable chunk.
    ...
    /**
     * Determines whether this outlineable {@link MethodGenerator.Chunk} is followed immediately by
     * the argument <code>MethodGenerator.Chunk</code>, with no other intervening instructions,
     * including {@link OutlineableChunkStart} or {@link OutlineableChunkEnd} instructions.
     *
     * @param neighbour an outlineable {@link MethodGenerator.Chunk}
     * @return <code>true</code> if and only if the argument chunk immediately follows <code>this
     *     </code> chunk
     */
     ...
     * @return
     *     <ul>
     *       <li>A positive <code>int</code> if the length of <code>this</code> chunk in bytes is
     *           greater than that of <code>comparand</code>
     *       <li>A negative <code>int</code> if the length of <code>this</code> chunk in bytes is
     *           less than that of <code>comparand</code>
     *       <li>Zero, otherwise.
     *     </ul>
     */
  ...
  /**
   * Find the outlineable chunks in this method that would be the best choices to outline, based on
   * size and position in the method.
   *
      ...
      for (int run = 0; run < numAdjacentRuns; run++) {
        ...
        for (int mergeStart = runStart;
          ...
          if (mergeSize <= TARGET_METHOD_SIZE) {
            ...
            mergedChunks.add(
                new Chunk(chunks[mergeStart].getChunkStart(), chunks[mergeEnd].getChunkEnd()));
  ...
  /**
   * Given an outlineable chunk of code in the current {@link MethodGenerator} move ("outline") the
   * chunk to a new method, and replace the chunk in the old method with a reference to that new
   * method. No {@link OutlineableChunkStart} or {@link OutlineableChunkEnd} instructions are
   * copied.
   *
          ...
          if (localVarMap.get(oldLVG) == null) {
            ...
            boolean copyInLocalValue =
                offsetInLocalVariableGenRange(
                    oldLVG, (outlineChunkStartOffset != 0) ? outlineChunkStartOffset - 1 : 0);
            ...
            if (copyInLocalValue || copyOutLocalValue) {
              ...
              if (copyInLocalValue) {
                ...
                InstructionHandle copyInLoad =
                    oldMethCopyInIL.append(loadLocal(oldLocalVarIndex, varType));
              ...
              if (copyOutLocalValue) {
                ...
                InstructionHandle copyOutStore =
                    oldMethCopyOutIL.append(storeLocal(oldLocalVarIndex, varType));
  ...
  /**
   * Track the number of outlineable chunks started but not yet ended. Used to detect imbalances in
   ...
   * MethodGen#getMethod()} only returns a single <code>Method</code> object. This method takes into
   * account the Java Virtual Machine Specification limit of 64KB on the size of a method, and may
   * return more than one <code>Method</code>.
   *
   * <p>If the code associated with the <code>MethodGenerator</code> would exceed the 64KB limit,
   * this method will attempt to split the code in the {@link InstructionList} associated with this
   * <code>MethodGenerator</code> into several methods.
   *
   ...
   *
   * <p>The Java Virtual Machine Specification permits the code portion of a method to be up to 64KB
   * in length. However, some control transfer instructions specify relative offsets as a signed
   * 16-bit quantity, limiting the range to a subset of the instructions that might be in a method.
   *
   ...
   *
   * <p>The <code>GOTO</code> and <code>JSR</code> instructions come in two forms, one of which uses
   * 16-bit relative offsets, and the other of which uses 32-bit relative offsets. The BCEL library
   * decides whether to use the wide form of <code>GOTO</code> or <code>JSR</code>instructions based
   * on the relative offset of the target of the instruction without any intervention by the user of
   * the library.
   *
   ...
   * relative branch offsets. The logic of each such instruction is inverted, and made to target the
   * instruction which follows it. An unconditional branch to the original target of the instruction
   * is then inserted between the conditional branch and the instruction which previously followed
   * it. The unconditional branch is permitted to have a 16-bit or a 32-bit relative offset, as
   ...
   *
   * <ul>
   *   <li>The {@link InstructionList#setPositions()} has been called for the <code>InstructionList
   *       </code> associated with this <code>MethodGenerator</code>.
   * </ul>
   *
      ...
      if (inst instanceof IfInstruction) {
        ...
        if ((relativeTargetOffset - maxOffsetChange < MIN_BRANCH_TARGET_OFFSET)
          ...
          try {
            ...
            String msg =
                new ErrorMsg(ErrorMsg.OUTLINE_ERR_DELETED_TARGET, tle.getMessage()).toString();
            throw new InternalError(msg);
          }
