...
public class XMLDTDScannerImpl extends XMLScanner
  ...
  public XMLDTDScannerImpl(
      SymbolTable symbolTable, XMLErrorReporter errorReporter, XMLEntityManager entityManager) {
  ...
  private final void scanChildren(String elName) throws IOException, XNIException {
    ...
    while (true) {
      ...
      if (childName == null) {
        reportFatalError(
            "MSG_OPEN_PAREN_OR_ELEMENT_TYPE_REQUIRED_IN_CHILDREN", new Object[] {elName});
        return;
      }
      // call handler
      if (fDTDContentModelHandler != null) {
  ...
  protected final void scanAttlistDecl() throws IOException, XNIException {
    ...
    if (!skipSeparator(true, !scanningInternalSubset())) {
      ...
      if (fEntityScanner.skipChar('>')) {
        ...
        reportFatalError(
            "MSG_SPACE_REQUIRED_BEFORE_ATTRIBUTE_NAME_IN_ATTDEF", new Object[] {elName});
      }
    }
    ...
    while (!fEntityScanner.skipChar('>')) {
      ...
      if (!skipSeparator(true, !scanningInternalSubset())) {
        reportFatalError(
            "MSG_SPACE_REQUIRED_BEFORE_DEFAULTDECL_IN_ATTDEF", new Object[] {elName, name});
      }
  ...
  private final String scanAttType(String elName, String atName) throws IOException, XNIException {
    ...
    if (fEntityScanner.skipString("CDATA")) {
      ...
      if (!skipSeparator(true, !scanningInternalSubset())) {
        reportFatalError(
            "MSG_SPACE_REQUIRED_AFTER_NOTATION_IN_NOTATIONTYPE", new Object[] {elName, atName});
      }
      // open paren
  ...
  protected final String scanAttDefaultDecl(
    ...
    if (fEntityScanner.skipString("#REQUIRED")) {
      ...
      if (fEntityScanner.skipString("#FIXED")) {
        ...
        if (!skipSeparator(true, !scanningInternalSubset())) {
          reportFatalError(
              "MSG_SPACE_REQUIRED_AFTER_FIXED_IN_DEFAULTDECL", new Object[] {elName, atName});
        }
      }
      // AttValue
  ...
  private final void scanEntityDecl() throws IOException, XNIException {
    ...
    if (!isPEDecl && fEntityScanner.skipString("NDATA")) {
      ...
      if (!sawSpace) {
        reportFatalError(
            "MSG_SPACE_REQUIRED_BEFORE_NDATA_IN_UNPARSED_ENTITYDECL", new Object[] {name});
      }

      // spaces
      if (!skipSeparator(true, !scanningInternalSubset())) {
        reportFatalError(
            "MSG_SPACE_REQUIRED_BEFORE_NOTATION_NAME_IN_UNPARSED_ENTITYDECL", new Object[] {name});
      }
  ...
  private final void scanNotationDecl() throws IOException, XNIException {
    ...
    fResourceIdentifier.setValues(
        publicId, systemId, baseSystemId, XMLEntityManager.expandSystemId(systemId, baseSystemId));
  ...
  /**
   * Scans a conditional section. If it's a section to ignore the whole section gets scanned through
   * and this method only returns after the closing bracket has been found. When it's an include
   * section though, it returns to let the main loop take care of scanning it. In that case the end
   * of the section if handled by the main loop (scanDecls).
   *
   * <p>
   *
   * <pre>
          ...
          if (fEntityScanner.skipChar(']')) {
            ...
            if (fEntityScanner.skipChar('>')) {
              if (fIncludeSectDepth-- == initialDepth) {
                ...
                if (fDTDHandler != null) {
                  fLiteral.setValues(
                      fIgnoreConditionalBuffer.ch, 0, fIgnoreConditionalBuffer.length - 2);
