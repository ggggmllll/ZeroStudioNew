...
public class DocumentImpl extends CoreDocumentImpl
  ...
  /**
   * Create and return a NodeIterator. The NodeIterator is added to a list of NodeIterators so that
   * it can be removed to free up the DOM Nodes it references.
   *
   * @param root The root of the iterator.
   * @param whatToShow The whatToShow mask.
   * @param filter The NodeFilter installed. Null means no filter.
  ...
  /**
   * This is not called by the developer client. The developer client uses the detach() function on
   * the NodeIterator itself.
   *
  ...
  /**
   * Not a client function. Called by Range.detach(), so a Range can remove itself from the list of
   * Ranges.
   */
   ...
   *
   * @param type The eventType parameter specifies the type of Event interface to be created. If the
   *     Event interface specified is supported by the implementation this method will return a new
   *     Event of the interface type requested. If the Event is to be dispatched via the
   ...
   * references! Indeed, the node here won't be GC'ed as long as some listener is registered on it,
   ...
   * both Capturing and Bubbling, but may only be registered once per role; redundant registrations
   * are ignored.
   *
   ...
   *       event's stopPropagation flag is cleared when dispatch begins; thereafter, if it has been
   *       set before processing of a node commences, we instead immediately advance to the DEFAULT
   *       phase.
   ...
   *       alters the document tree, that does not change which nodes will be informed of the event.
   ...
   *   <li>Default processing: Some DOMs have default behaviors bound to specific nodes. If this DOM
   *       does, and if the event's preventDefault flag has not been set, we now return to the
   *       target node and process its default handler for this event, if any.
   * </ol>
   *
   * <p>Note that registration of handlers during processing of an event does not take effect during
   * this phase of this event; they will not be called until the next time this node is visited by
   * dispatchEvent. On the other hand, removals take effect immediately.
   *
   * <p>If an event handler itself causes events to be dispatched, they are processed synchronously,
   * before processing resumes on the event which triggered them. Please be aware that this may
   * result in events arriving at listeners "out of order" relative to the actual sequence of
   * requests.
   *
  ...
  /**
   * NON-DOM INTERNAL: DOMNodeInsertedIntoDocument and ...RemovedFrom... are dispatched to an entire
   * subtree. This is the distribution code therefor. They DO NOT bubble, thanks be, but may be
   * captured.
   *
   * <p>Similar to code in dispatchingEventToSubtree however this method is only used on the target
   * node and does not start a dispatching chain on the sibling of the target node as this is not
   * part of the subtree ***** At the moment I'm being sloppy and using the normal capture
   * dispatcher on every node. This could be optimized hugely by writing a capture engine that
   * tracks our position in the tree to update the capture chain without repeated chases up to root.
   *
  ...
  /**
   * NON-DOM INTERNAL: Convenience wrapper for calling dispatchAggregateEvents when the context was
   * established by <code>savedEnclosingAttr</code>.
   *
   * @param node node to dispatch to
   ...
   * makes multiple changes to a single Attr before returning, it would be nice to generate only one
   * DOMAttrModified, and multiple changes over larger scope but within a recognizable single
   ...
   * make an explicit call to this routine at the higher level. Some examples now exist in our code.
   *
  ...
  protected void dispatchAggregateEvents(
    ...
    if (lc.total > 0) {
      ...
      me.initMutationEvent(
          MutationEventImpl.DOM_SUBTREE_MODIFIED, true, false, null, null, null, null, (short) 0);
    ...
    if (lc.total > 0) {
      ...
      while (true) {
        ...
        if (type == ATTRIBUTE_NODE) {
          ...
          return;
        } else if (type == ENTITY_REFERENCE_NODE) eventAncestor = eventAncestor.parentNode();
  ...
  void insertedNode(NodeImpl node, NodeImpl newInternal, boolean replace) {
    if (mutationEvents) {
      ...
      if (lc.total > 0) {
        MutationEventImpl me = new MutationEventImpl();
        me.initMutationEvent(
            MutationEventImpl.DOM_NODE_INSERTED, true, false, node, null, null, null, (short) 0);
  ...
  void removingNode(NodeImpl node, NodeImpl oldChild, boolean replace) {
    ...
    if (mutationEvents) {
      ...
      if (lc.total > 0) {
        MutationEventImpl me = new MutationEventImpl();
        me.initMutationEvent(
            MutationEventImpl.DOM_NODE_REMOVED, true, false, node, null, null, null, (short) 0);
