...
final class Mode implements Constants {
  ...
  private void addPattern(int kernelType, LocationPathPattern pattern) {
    ...
    if (kernelType == DOM.NO_TYPE) {
      if (pattern.getAxis() == Axis.ATTRIBUTE) {
        patterns =
            (_attribNodeGroup == null) ? (_attribNodeGroup = new Vector(2)) : _attribNodeGroup;
      } else {
  ...
  /**
   * Build test sequences. The first step is to complete the test sequences by including patterns of
   * "*" and "node()" kernel to all element test sequences, and of "@*" to all attribute test
   * sequences.
   */
  ...
  public void compileApplyTemplates(ClassGenerator classGen) {
    ...
    targets[DTM.ROOT_NODE] =
        _rootPattern != null ? getTemplateInstructionHandle(_rootPattern.getTemplate()) : ihRecurse;
  ...
  private void peepHoleOptimization(MethodGenerator methodGen) {
    ...
    // changed to lower case - changing to all lower case although only the instruction with capital
    // I
