...
public class ElementImpl extends ParentNode implements Element, TypeInfo {
   ...
   *
   * <p>Note: Attributes may contain complex node trees. This method returns the "flattened" string
   * obtained from Attribute.getValue(). If you need the structure information, see
   * getAttributeNode().
   */
   ...
   *
   * <p>Note: NodeList is a "live" view of the DOM. Its contents will change as the DOM changes, and
   * alterations made to the NodeList will be reflected in the DOM.
   *
  ...
  /**
   * Returns the name of the Element. Note that Element.nodeName() is defined to also return the tag
   * name.
   *
  ...
  /**
   * Remove the named attribute from this Element. If the removed Attribute has a default value, it
   * is immediately replaced thereby.
   *
   * <p>The default logic is actually implemented in NamedNodeMapImpl. PR-DOM-Level-1-19980818
   * doesn't fully address the DTD, so some of this behavior is likely to change in future versions.
   * ?????
   *
  ...
  /**
   * Remove the specified attribute/value pair. If the removed Attribute has a default value, it is
   * immediately replaced.
   *
   * <p>NOTE: Specifically removes THIS NODE -- not the node with this name, nor the node with these
   * contents. If the specific Attribute object passed in is not stored in this Element, we throw a
   * DOMException. If you really want to remove an attribute by name, use removeAttribute().
   *
   ...
   *
   * <p>Note: this method supports only the simplest kind of Attribute, one whose value is a string
   * contained in a single Text node. If you want to assert a more complex value (which XML permits,
   ...
   * local name and namespace URI is already present on the element, its prefix is changed to be the
   * prefix part of the qualifiedName, and its value is changed to be the value parameter. This
   * value is a simple string, it is not parsed as it is being set. So any markup (such as syntax to
   * be recognized as an entity reference) is treated as literal text, and needs to be appropriately
   * escaped by the implementation when it is written out. In order to assign an attribute value
   * that contains entity references, the user must create an Attr node plus any Text and
   * EntityReference nodes, build the appropriate subtree, and use setAttributeNodeNS or
   ...
   *     qualifiedName has a prefix different from "xml" and "xmlns" and the namespaceURI is null or
   *     an empty string.
   * @since WD-DOM-Level-2-19990923
   */
  public void setAttributeNS(String namespaceURI, String qualifiedName, String value) {
    ...
    if (newAttr == null) {
      ...
      if (newAttr instanceof AttrNSImpl) {
        ...
        newAttr =
            new AttrNSImpl(
                (CoreDocumentImpl) getOwnerDocument(), namespaceURI, qualifiedName, localName);
        attributes.setNamedItemNS(newAttr);
      }
   ...
   * Introduced in DOM Level 2.
   *
   * <p>Removes an attribute by local name and namespace URI. If the removed attribute has a default
   * value it is immediately replaced. The replacing attribute has the same namespace URI and local
   * name, as well as the original prefix.
   *
   * <p>
   *
   ...
   * @param namespaceURI The namespace URI of the attribute to retrieve.
   * @param localName The local name of the attribute to retrieve.
   * @return Attr The Attr node with the specified attribute local name and namespace URI or null if
   ...
   * order in which they would be encountered in a preorder traversal of the Document tree, starting
   * from this node.
   *
  ...
  /**
   * DOM Level 3 WD- Experimental. Override inherited behavior from NodeImpl and ParentNode to check
   * on attributes
   */
