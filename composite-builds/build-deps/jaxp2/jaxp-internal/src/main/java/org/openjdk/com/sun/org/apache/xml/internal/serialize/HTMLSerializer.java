...
public class HTMLSerializer extends BaseMarkupSerializer {
  ...
  /**
   * Constructs a new HTML/XHTML serializer depending on the value of <tt>xhtml</tt>. The serializer
   * cannot be used without calling {@link #setOutputCharStream} or {@link #setOutputByteStream}
   * first.
   *
  ...
  public void endElementIO(String namespaceURI, String localName, String rawName)
    ...
    if (state.namespaceURI == null || state.namespaceURI.length() == 0) htmlName = state.rawName;
    else {
   ...
   * must name the root element, but the root element is only known when that element is serialized,
   * and not at the start of the document.
   *
   * <p>This method will check if it has not been called before ({@link #_started}), will serialize
   * the document type declaration, and will serialize all pre-root comments and PIs that were
   * accumulated in the document (see {@link #serializePreRoot}). Pre-root will be serialized even
   * if this is not the first root element of the document.
   */
