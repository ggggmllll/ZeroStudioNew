...
public class DOMSerializerImpl implements LSSerializer, DOMConfiguration {
  ...
  public void setParameter(String name, Object value) throws DOMException {
    if (value instanceof Boolean) {
      ...
      if (name.equalsIgnoreCase(Constants.DOM_INFOSET)) {
        ...
        features =
            (short) (state ? features | FORMAT_PRETTY_PRINT : features & ~FORMAT_PRETTY_PRINT);
        ...
        if (state) {
          String msg =
              DOMMessageFormatter.formatMessage(
                  DOMMessageFormatter.DOM_DOMAIN, "FEATURE_NOT_SUPPORTED", new Object[] {name});
   ...
   * <code>LSSerializer</code>. The result of serializing the node is returned as a string. Writing
   * a Document or Entity node produces a serialized form that is well formed XML. Writing other
   * node types produces a fragment of text in a form that is not fully defined by this document,
   * but that should be useful to a human for debugging or diagnostic purposes.
   *
   * @param wnode The node to be written.
   * @return Returns the serialized data
   * @exception DOMException DOMSTRING_SIZE_ERR: The resulting string is too long to fit in a <code>
   *     DOMString</code>.
   * @exception LSException SERIALIZE_ERR: Unable to serialize the node. DOM applications should
   *     attach a <code>DOMErrorHandler</code> using the parameter &quot;<i>error-handler</i>&quot;
   *     to get details on error.
   */
  public String writeToString(Node wnode) throws DOMException, LSException {
    ...
    try {
      ...
      String msg =
          DOMMessageFormatter.formatMessage(
              DOMMessageFormatter.DOM_DOMAIN, "STRING_TOO_LONG", new Object[] {ioe.getMessage()});
  ...
  /**
   * DOM L3 EXPERIMENTAL: The end-of-line sequence of characters to be used in the XML being written
   * out. The only permitted values are these:
   *
   * <dl>
   *   <dt><code>null</code>
   *   <dd>Use a default end-of-line sequence. DOM implementations should choose the default to
   *       match the usual convention for text files in the environment being used. Implementations
   *       must choose a default sequence that matches one of those allowed by 2.11 "End-of-Line
   *       Handling".
   *   <dt>CR
   ...
   * implementation can choose to remove the node from the stream or to terminate the serialization
   * early.
   */
          ...
          if (attributes != null) {
            for (int i = 0; i < attributes.getLength(); ++i) {
              ...
              DOMNormalizer.isAttrValueWF(
                  fErrorHandler, fError, fLocator, attributes, attr, attr.getValue(), xml11Version);
              if (verifyNames) {
      ...
      case Node.COMMENT_NODE:
        {
          ...
          if ((features & COMMENTS) != 0)
            DOMNormalizer.isCommentWF(
                fErrorHandler, fError, fLocator, ((Comment) node).getData(), xml11Version);
          break;
        }
