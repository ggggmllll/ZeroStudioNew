...
public final class XSLTC {
  ...
  /**
   * Set to true if template inlining is requested. Template inlining used to be the default, but we
   * have found that Hotspots does a better job with shorter methods, so the default is *not* to
   * inline now.
   */
  ...
  /**
   * Set a flag indicating if templates are to be inlined or not. The default is to do inlining, but
   * this causes problems when the stylesheets have a large number of templates (e.g. branch targets
  ...
  /**
   * Compiles a stylesheet pointed to by a URL. The result is put in a set of byte arrays. One byte
   * array for each generated class.
   *
   * @param name The name of the translet class to generate
   * @param input An InputSource that will pass in the stylesheet contents
   ...
   * Affects the DOM used by the translet.
   */
  ...
  /**
   * Retrieve a string representation of the character data to be stored in the translet as a <code>
   * char[]</code>. There may be more than one such array required.
   *
