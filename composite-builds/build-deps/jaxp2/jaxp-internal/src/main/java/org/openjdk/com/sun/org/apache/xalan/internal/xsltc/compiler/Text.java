...
final class Text extends Instruction {
    ...
    if (!_ignore) {
      ...
      if (!canLoadAsArrayOffsetLength()) {
        final int characters =
            cpg.addInterfaceMethodref(OUTPUT_HANDLER, "characters", "(" + STRING_SIG + ")V");
   ...
   * Text node, followed by the offset of the data from the start of the array, and then the length
   * of the data.
   *
   * <p>The pre-condition to calling this method is that canLoadAsArrayOffsetLength() returns true.
   *
  ...
  public void loadAsArrayOffsetLength(ClassGenerator classGen, MethodGenerator methodGen) {
    ...
    il.append(
        new GETSTATIC(
            cpg.addFieldref(xsltc.getClassName(), charDataFieldName, STATIC_CHAR_DATA_FIELD_SIG)));
