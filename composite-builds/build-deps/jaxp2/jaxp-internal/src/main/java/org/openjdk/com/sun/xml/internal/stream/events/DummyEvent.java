...
public abstract class DummyEvent implements XMLEvent {
  ...
  /**
   * Returns this event as an end element event, may result in a class cast exception if this event
   * is not a end element.
   */
  ...
  /**
   * Returns this event as a start element event, may result in a class cast exception if this event
  ...
  /**
   * This method will write the XMLEvent as per the XML 1.0 specification as Unicode characters. No
   * indentation or whitespace should be outputted.
   *
   * <p>Any user defined event type SHALL have this method called when being written to on an output
   * stream. Built in Event types MUST implement this method, but implementations MAY choose not
   * call these methods for optimizations reasons when writing out built in Events to an output
   * stream. The output generated MUST be equivalent in terms of the infoset expressed.
   *
