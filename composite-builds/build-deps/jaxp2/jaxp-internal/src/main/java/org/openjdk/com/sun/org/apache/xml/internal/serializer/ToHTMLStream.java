...
public final class ToHTMLStream extends ToStream {
  ...
  /**
   * Map that tells which XML characters should have special treatment, and it provides character to
   * entity name lookup.
   */
  ...
  static void initTagReference(Trie m_elementFlags) {
    ...
    m_elementFlags.put(
        "BR",
        new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.EMPTY | ElemDesc.BLOCK));
    ...
    m_elementFlags.put(
        "DIV", new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
    ...
    m_elementFlags.put(
        "HR",
        new ElemDesc(
            0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET | ElemDesc.EMPTY));
    m_elementFlags.put(
        "P", new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
    ...
    m_elementFlags.put(
        "DL", new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
    ...
    m_elementFlags.put(
        "INPUT", new ElemDesc(0 | ElemDesc.FORMCTRL | ElemDesc.INLINELABEL | ElemDesc.EMPTY));
    ...
    m_elementFlags.put(
        "SCRIPT",
        new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.HEADMISC | ElemDesc.RAW));
    m_elementFlags.put(
        "NOSCRIPT",
        new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
   ...
   * method="html" (it must be method="html" otherwise we shouldn't even have a ToHTMLStream object
   * here!)
   *
  ...
  public void setOutputFormat(Properties format) {
    ...
    m_specialEscapeURLs =
        OutputPropertyUtils.getBooleanProperty(OutputPropertiesFactory.S_USE_URL_ESCAPING, format);

    m_omitMetaTag =
        OutputPropertyUtils.getBooleanProperty(OutputPropertiesFactory.S_OMIT_META_TAG, format);
  ...
  /**
   * A Trie that is just a copy of the "static" one. We need this one to be able to use the faster,
   * but not thread-safe method Trie.get2(name)
   */
  private Trie m_htmlInfo = new Trie(m_elementFlags);

  /**
   * Calls to this method could be replaced with calls to getElemDesc(name), but this one should be
   * faster.
   */
  ...
  protected void processAttribute(
      java.io.Writer writer, String name, String value, ElemDesc elemDesc) throws IOException {
    writer.write(' ');
  ...
  /**
   * Make an integer into an HH hex value. Does no checking on the size of the input, since this is
   * only meant to be used locally by writeAttrURI.
   *
  ...
  public void writeAttrURI(final java.io.Writer writer, String string, boolean doURLEscaping)
    ...
    for (int i = 0; i < end; i++) {
      ...
      // identifier, we believe that it's OK to double escape quotes.
        ...
        // Dmitri Ilyin: to check if '%' number number is invalid. It must be checked if %xx is a
        // sign, that would be encoded
        // The encoded signes are in Hex form. So %xx my be in form %3C that is "<" sign. I will try
        // to change here a little.
  ...
  public void processAttributes(java.io.Writer writer, int nAttrs)
    ...
    for (int i = 0; i < nAttrs; i++) {
      processAttribute(
          writer, m_attributes.getQName(i), m_attributes.getValue(i), m_elemContext.m_elementDesc);
    }
  }
   ...
   * started with a startElement() and before and endElement(). startPrefixMapping(prefix,uri) would
   * be used before the startElement() call.
   *
  ...
  /**
   * This method is used to add an attribute to the currently open element. The caller has guaranted
   * that this attribute is unique, which means that it not been seen before and will not be seen
   * again.
   *
  ...
  static class Trie {
     ...
     * must be a 7-bit ASCII string The value may be any Java Object One can get an object stored in
     * a trie from its key, but the search is either case sensitive or case insensitive to the
     * characters in the key, and this choice of sensitivity or insensitivity is made when the Trie
     * is created, before any objects are put in it.
     *
    ...
    /**
     * Construct the trie from another Trie. Both the existing Trie and this new one share the same
     * table for lookup, and it is assumed that the table is fully populated and not changing
     * anymore.
     *
