...
class DurationImpl extends Duration implements Serializable {
   ...
   * non-zero parameters into {@link BigInteger} and {@link BigDecimal}. Zero value of int parameter
   * is equivalent of null value of the corresponding field.
   *
   ...
   *
   * <p>For example, this method parses strings like "P1D" (1 day), "-PT100S" (-100 sec.), "P1DT12H"
   * (1 days and 12 hours).
   *
   ...
   *   <li>{@link DatatypeConstants#EQUAL} if this <code>Duration</code> is equal to <code>duration
   *       </code> parameter
   ...
   *     if the order relation between date1 and date2 is EQUAL. If the strict parameter is true,
   *     return LESS_THAN if date1 is less than date2 and return GREATER_THAN if date1 is greater
   *     than date2. If the strict parameter is false, return LESS_THAN if date1 is less than OR
   *     equal to date2 and return GREATER_THAN if date1 is greater than OR equal to date2
   */
  ...
  public Number getField(DatatypeConstants.Field field) {
     ...
     * "#(getSet(DatatypeConstants.Field field) called with an unknown field: " + field.toString()
     * );
     */
    String methodName =
        "org.openjdk.javax.xml.datatype.Duration" + "#(getSet(DatatypeConstants.Field field)";
   ...
   * method.
   *
   * <p>Note that since this method returns <tt>int</tt>, this method will return an incorrect value
   * for {@link Duration}s with the year field that goes beyond the range of <tt>int</tt>. Use
   * <code>getField(YEARS)</code> to avoid possible loss of precision.
   *
   ...
   * </pre>
   *
   * <p>Since the {@link Duration} class is immutable, this method doesn't change the value of this
   * object. It simply computes a new Duration object and returns it.
   *
   ...
   *
   * <p>A field of the resulting {@link Duration} will be unset if and only if respective fields of
   * two input {@link Duration}s are unset.
   *
   ...
   * with values beyond the range of <tt>int</tt> in its fields will cause overflow/underflow to the
   * given {@link Calendar}. {@link XMLGregorianCalendar#add(Duration)} provides the same basic
