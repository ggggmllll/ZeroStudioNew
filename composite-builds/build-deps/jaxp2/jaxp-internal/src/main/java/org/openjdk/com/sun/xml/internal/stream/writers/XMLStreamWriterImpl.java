...
public final class XMLStreamWriterImpl extends AbstractMap implements XMLStreamWriter {
   ...
   * writer. For example, an 8-bit char from the extended ASCII set is not encodable by 7-bit ASCII
   * encoder. Unencodable chars are escaped using XML numeric entities.
   */
   ...
   * default namespace; It's added to avoid changing the current impl. which has many redundant code
   * for the repair mode
   */
  ...
  /**
   * Creates a new instance of XMLStreamWriterImpl using a StreamResult. A StreamResult encasupates
   * an OutputStream, a Writer or a SystemId.
   *
   * @param writer Underlying writer to which chars are written
   * @param props Properties used by this writer
   */
  ...
  /**
   * Reset this instance so that it can be re-used. Do not read properties again. The method <code>
   * setOutput(StreamResult, encoding)</code> must be called after this one.
   */
  public void reset() {
    reset(false);
  }
   ...
   * context passed to the method as the root context for resolving namespaces. This method may only
   * be called once at the start of the document. It does not cause the namespaces to be declared.
   * If a namespace URI to prefix mapping is found in the namespace context it is treated as
   * declared and the prefix may be used by the <code>XMLStreamWriter</code>.
   *
  ...
  private void correctPrefix(QName attr, int type) {
    ...
    if (prefix == null || prefix.equals("")) {
      ...
      if (prefix == XMLConstants.DEFAULT_NS_PREFIX && uri == XMLConstants.DEFAULT_NS_PREFIX) return;
  ...
  protected class ElementStack {
     ...
     * value for every end element we encouter. For Well formedness checks we can have the same
     * QName object that was pushed. the values will be set only if application need to know about
