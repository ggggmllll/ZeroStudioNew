...
public class XPathMatcher {
  ...
  /**
   * No match depth. The value of this field will be zero while matching is successful for the given
   * xpath expression.
   */
  ...
  public boolean isMatched() {
    ...
    for (int i = 0; i < fLocationPaths.length; i++)
      if (((fMatched[i] & MATCHED) == MATCHED)
          ...
          && ((fNoMatchDepth[i] == 0)
              || ((fMatched[i] & MATCHED_DESCENDANT) == MATCHED_DESCENDANT))) return true;
              ...
              if (fCurrentStep[i] == steps.length) {
                ...
                if (j == i) {
                  AttributePSVI attrPSVI =
                      (AttributePSVI)
                          attributes.getAugmentations(aIndex).getItem(Constants.ATTRIBUTE_PSVI);
   ...
   *     that this may not be the type declared in the element declaration, but it is "the actual
   *     type". For example, if the XML is &lt;foo xsi:type="xs:string">aaa&lt;/foo>, this parameter
   *     will be "xs:string".
