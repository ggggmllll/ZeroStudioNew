...
public class AttrImpl extends NodeImpl implements Attr, TypeInfo {
  ...
  /**
   * Implicit in the rerouting of getNodeValue to getValue is the need to redefine setNodeValue, for
   * symmetry's sake. Note that since we're explicitly providing a value, Specified should be set
  ...
  /**
   * The "string value" of an Attribute is its text representation, which in turn is a concatenation
   * of the string values of its children.
   */
  ...
  /**
   * The "specified" flag is true if and only if this attribute's value was explicitly specified in
   * the original document. Note that the implementation, not the user, is in charge of this
  ...
  /**
   * Checks the normalized state of this node after inserting a child. If the inserted child causes
   * this node to be unnormalized, then this node is flagged accordingly. The conditions for
   * changing the normalized state are:
   *
   * <ul>
   ...
   * node to be unnormalized, then this node is flagged accordingly. The conditions for changing the
   * normalized state are:
   *
   * <ul>
