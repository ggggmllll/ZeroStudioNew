...
public class RangeImpl implements Range {
  ...
  public void insertNode(Node newNode) throws DOMException, RangeException {
    ...
    if (fStartContainer.getNodeType() == Node.TEXT_NODE) {
      ...
      ((TextImpl) fStartContainer)
          .setNodeValueInternal((fStartContainer.getNodeValue()).substring(0, fStartOffset));
  ...
  /**
   * This function is called from the DOM. This node has already been inserted into the DOM. Fix-up
   * any offsets.
   */
  ...
  /**
   * This function must be called by the DOM _BEFORE_ a node is deleted, because at that time it is
   ...
   *     </ol>
   *
   * @return Returns a document fragment containing any copied or extracted nodes. If the <code>how
   *     </code> parameter was <code>DELETE_CONTENTS</code>, the return value is null.
   */
  ...
  /**
   * Visits the nodes selected by this range when we know a-priori that the start and end containers
   * are the same. This method is invoked by the generic <code>traverse</code> method.
   *
   * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
   *     Legal values for this argument are:
   *     <ol>
  ...
  /**
   * Visits the nodes selected by this range when we know a-priori that the start and end containers
   * are not the same, but the start container is an ancestor of the end container. This method is
  ...
  /**
   * Visits the nodes selected by this range when we know a-priori that the start and end containers
   * are not the same, but the end container is an ancestor of the start container. This method is
   * invoked by the generic <code>traverse</code> method.
   *
  ...
  /**
   * Visits the nodes selected by this range when we know a-priori that the start and end containers
   * are not the same, and we also know that neither the start nor end container is an ancestor of
   * the other. This method is invoked by the generic <code>traverse</code> method.
   *
   * @param startAncestor Given a common ancestor of the start and end containers, this parameter is
   *     the ancestor (or self) of the start container that is a direct child of the common
   *     ancestor.
  ...
  /**
   * Traverses the "right boundary" of this range and operates on each "boundary node" according to
   * the <code>how</code> parameter. It is a-priori assumed by this method that the right boundary
   * does not contain the range's start container.
   *
   ...
   * both the start and end offsets. Such nodes should have been previously detected and been routed
   * to traverseTextNode.
   *
   ...
   *     Note that although the DOM 2 specification says that a text node that is boththe start and
   *     end container is not selected, we treat it here as if it were partially selected.
   * @param isLeft Is true if we are traversing the node as part of navigating the "left boundary"
   *     of the range. If this value is false, it implies we are navigating the "right boundary" of
   *     the range.
   * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
   *     Legal values for this argument are:
   *     <ol>
   ...
   *       <li><code>DELETE_CONTENTS</code> - will delete the node from it's parent, but will return
   *           null.
   *     </ol>
   *
  ...
  /**
   * Finds the root container for the given node and determines if that root container is legal with
   * respect to the DOM 2 specification. At present, that means the root container must be either an
   * attribute, a document, or a document fragment.
   */
   ...
   * find out which node is selected by the given index. Note that if the index is greater than the
   * number of children, this implies that the first node selected is the parent node itself.
   *
