...
public abstract class NodeCounter {
  ...
  /**
   * Set the start node for this counter. The same <tt>NodeCounter</tt> object can be used multiple
   * times by resetting the starting node.
   */
   ...
   * NumberFormatException, it returns 0. This method is used instead of Integer.parseInt() since it
   * does not incur the overhead of throwing an Exception which is expensive.
   *
  ...
  /**
   * Format a single value based on the appropriate formatting token. This method is based on saxon
   * (Michael Kay) and only implements lang="en".
   */
