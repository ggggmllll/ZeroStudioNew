...
public abstract class ParentNode extends ChildNode {
  ...
  /**
   * No public constructor; only subclasses of ParentNode should be instantiated, and those normally
   * via a Document's factory methods
   */
   ...
   * getLength() or item(int). ChildAndParentNode optimizes getChildNodes() by implementing NodeList
   * itself. However if a subclass Element implements methods with the same name as the NodeList
  ...
  /**
   * Override default behavior to call normalize() on this Node's children. It is up to implementors
   * or Node to override normalize() to take action.
   */
