  ...
  static final class XPrecisionDecimal {
    ...
    void initD(String content) throws NumberFormatException {
      ...
      for (int fracPos = fracStart; fracPos < fracEnd; fracPos++) {
        if (!TypeValidator.isDigit(content.charAt(fracPos))) throw new NumberFormatException();
      }
    ...
    private int intComp(XPrecisionDecimal val) {
      if (intDigits != val.intDigits) return intDigits > val.intDigits ? GREATER_THAN : LESS_THAN;
    ...
    public boolean isIdentical(XPrecisionDecimal decimal) {
      if (ivalue.equals(decimal.ivalue)
          && (ivalue.equals("INF") || ivalue.equals("-INF") || ivalue.equals("NaN"))) return true;
