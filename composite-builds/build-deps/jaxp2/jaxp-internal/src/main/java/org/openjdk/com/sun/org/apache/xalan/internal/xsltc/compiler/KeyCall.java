...
final class KeyCall extends FunctionCall {
  ...
  /**
   * Get the parameters passed to function: key(String name, String value) key(String name, NodeSet
   * value) The 'arguments' vector should contain two parameters for key() calls, one holding the
   * key name and one holding the value(s) to look up. The vector has only one parameter for id()
   * calls (the key name is always "##id" for id() calls).
   *
   ...
   *
   * <p>and assuming this class represents "key('x', 1)", add a reference between variable y and key
   * x. Note that if 'x' is unknown statically in key('x', 1), there's nothing we can do at this
   * point.
   */
  ...
  /**
   * Type check the parameters for the id() or key() function. The index name (for key() call only)
   * must be a string or convertable to a string, and the lookup-value must be a string or a
   * node-set.
   *
   ...
   *
   * <p>This method will generate byte code that produces an iterator for the nodes in the node set
   * for the key or id function call.
   *
  ...
  public void translate(ClassGenerator classGen, MethodGenerator methodGen) {
    ...
    final int getKeyIndex =
        cpg.addMethodref(TRANSLET_CLASS, "getKeyIndex", "(Ljava/lang/String;)" + KEY_INDEX_SIG);
