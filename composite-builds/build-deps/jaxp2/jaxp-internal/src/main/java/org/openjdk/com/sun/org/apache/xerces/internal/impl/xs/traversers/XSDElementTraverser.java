...
class XSDElementTraverser extends XSDAbstractTraverser {
   ...
   * declarations. This method causes a number of data structures in the schema handler to be filled
   * in.
   *
   * @param elmDecl
   * @param schemaDoc
  ...
  protected void traverseLocal(
    ...
    if (elmDecl.getAttributeNode(SchemaSymbols.ATT_REF) != null) {
      if (refAtt != null) {
        element =
            (XSElementDecl)
                fSchemaHandler.getGlobalDecl(schemaDoc, XSDHandler.ELEMENT_TYPE, refAtt, elmDecl);
        ...
        if (child != null) {
          reportSchemaError(
              "src-element.2.2", new Object[] {refAtt.rawname, DOMUtil.getLocalName(child)}, child);
        }
      } else {
        element = null;
      }
  ...
  XSElementDecl traverseNamedElement(
    ...
    if (elementType == null && typeAtt != null) {
      elementType =
          (XSTypeDefinition)
              fSchemaHandler.getGlobalDecl(schemaDoc, XSDHandler.TYPEDECL_TYPE, typeAtt, elmDecl);
      if (elementType == null) {
    ...
    // 2.2 If ref is present, then all of <complexType>, <simpleType>, <key>, <keyref>, <unique>,
    // nillable, default, fixed, form, block and type must be absent, i.e. only minOccurs,
    ...
    // 2 If there is a {value constraint}, the canonical lexical representation of its value must be
    // valid with respect to the {type definition} as defined in Element Default Valid (Immediate)
    // (3.3.6).
    if (element.fDefault != null) {
      fValidationState.setNamespaceSupport(schemaDoc.fNamespaceSupport);
      if (XSConstraints.ElementDefaultValidImmediate(
              element.fType, element.fDefault.normalizedValue, fValidationState, element.fDefault)
          == null) {
        reportSchemaError(
            "e-props-correct.2", new Object[] {nameAtt, element.fDefault.normalizedValue}, elmDecl);
    ...
    // definition} is complex) or as defined in Type Derivation OK (Simple) (3.14.6) (if the {type
