...
public abstract class BaseMarkupSerializer
   ...
   * When leaving an element, it's state is not removed but reused when later returning to the same
   * nesting level.
   */
   ...
   * format was passed to the constructor, the default one for this document type will be used. The
   * format object is never changed by the serializer.
   */
  ...
  /**
   * Protected constructor can only be used by derived class. Must initialize the serializer before
   * serializing any document, by calling {@link #setOutputCharStream} or {@link
    ...
    if (output == null) {
      String msg =
          DOMMessageFormatter.formatMessage(
              DOMMessageFormatter.SERIALIZER_DOMAIN, "ArgumentIsNull", new Object[] {"output"});
  ...
  public void setOutputCharStream(Writer writer) {
    if (writer == null) {
      String msg =
          DOMMessageFormatter.formatMessage(
              DOMMessageFormatter.SERIALIZER_DOMAIN, "ArgumentIsNull", new Object[] {"writer"});
      throw new NullPointerException(msg);
    }
    _writer = writer;
    _output = null;
  ...
  public void setOutputFormat(OutputFormat format) {
    if (format == null) {
      String msg =
          DOMMessageFormatter.formatMessage(
              DOMMessageFormatter.SERIALIZER_DOMAIN, "ArgumentIsNull", new Object[] {"format"});
      throw new NullPointerException(msg);
    }
    _format = format;
    reset();
  ...
  /**
   * Serializes a node using the previously specified writer and output format. Throws an exception
  ...
  /**
   * Serializes the DOM document using the previously specified writer and output format. Throws an
   * exception only if an I/O exception occured while serializing.
   *
  ...
  /**
   * Must be called by a method about to print any type of content. If the element was just opened,
   * the opening tag is closed and will be matched to a closing tag. Returns the current element
   ...
   * capable of printing text as CDATA, it is used for that purpose as well. White space handling is
   * determined by the current element state. In addition, the output format can dictate whether the
   ...
   * accumulated inside a vector and serialized by calling this method. Will be called when the root
   * element is serialized and when the document finished serializing.
   *
   * @throws IOException An I/O exception occured while serializing
   */
        ...
        if (fDOMErrorHandler != null) {
          ...
          if ((features & DOMSerializerImpl.SPLITCDATA) == 0) {
            ...
            if ((features & DOMSerializerImpl.WELLFORMED) != 0) {
              // issue fatal error
              modifyDOMError(
                  msg, DOMError.SEVERITY_FATAL_ERROR, "wf-invalid-character", fCurrentNode);
              fDOMErrorHandler.handleError(fDOMError);
              throw new LSException(LSException.SERIALIZE_ERR, msg);
            } else {
              // issue error
              modifyDOMError(
                  msg, DOMError.SEVERITY_ERROR, "cdata-section-not-splitted", fCurrentNode);
  ...
  /**
   * Called to print additional text with whitespace handling. If spaces are preserved, the text is
   * printed as if by calling {@link #printText(String,boolean,boolean)} with a call to {@link
   * Printer#breakLine} for each new line. If spaces are not preserved, the text is broken at space
   * boundaries if longer than the line width; Multiple spaces are printed as such, but spaces at
   * beginning of line are removed.
   *
   * @param text The text to print
  ...
  /**
   * Leave the current element state and return to the state of the parent element. If this was the
   * root element, return to the state of the document.
   *
