...
public class DOMHelper {
  ...
  /**
   * DOM Level 1 did not have a standard mechanism for creating a new Document object. This function
   * provides a DOM-implementation-independent abstraction for that for that concept. It's typically
   * used when outputting a new DOM as the result of an operation.
   *
   * <p>TODO: This isn't directly compatable with DOM Level 2. The Level 2 createDocument call also
   * creates the root element, and thus requires that you know what that element will be before
   * creating the Document. We should think about whether we want to change this code, and the
   * callers, so we can use the DOM's own method. (It's also possible that DOM Level 3 may relax
   * this sequence, but you may give up some intelligence in the DOM by doing so; the intent was
   * that knowing the document type and root element might let the DOM automatically switch to a
   * specialized subclass for particular kinds of documents.)
   *
   ...
   * DOM Node.
   *
   * <p>Warning: The base implementation uses the Node object's hashCode(), which is NOT guaranteed
   * to be unique. If that method hasn't been overridden in this DOM ipmlementation, most Java
   * implementions will derive it from the object's address and should be OK... but if your DOM uses
   * a different definition of hashCode (eg hashing the contents of the subtree), or if your DOM may
   * have multiple objects that represent a single Node in the data structure (eg via proxying), you
   ...
   * whether an ID may be reused. Currently we're assuming that the input document is stable for the
   * duration of the XPath/XSLT operation, so this shouldn't arise in this context.
   *
   * <p>(DOM Level 3 is investigating providing a unique node "key", but that won't help Level 1 and
  ...
  /**
   * Given an XML Namespace prefix and a context in which the prefix is to be evaluated, return the
   * Namespace Name this prefix was bound to. Note that DOM Level 3 is expected to provide a version
   * of this which deals with the DOM's "early binding" behavior.
   *
   * <p>Default handling:
   *
  ...
  /**
   * Object to put into the m_NSInfos table that tells that a node has been processed, and has xmlns
   * namespace decls.
   */
   ...
   * using getNamespaceForPrefix in that it attempts to cache some of the data in NSINFO objects, to
   * avoid repeated lookup. TODO: Should we consider moving that logic into getNamespaceForPrefix?
   *
   * @param n Node to be examined.
          ...
          if ((false == ancestorsHaveXMLNS) && (null == parent)) {
            for (int i = 0; i < nCandidates; i += 2) {
              ...
              if (candidateInfo == m_NSInfoUnProcWithoutXMLNS) {
                m_NSInfos.put(
                    m_candidateNoAncestorXMLNS.elementAt(i), m_NSInfoUnProcNoAncestorXMLNS);
  ...
  /**
   * Returns the attribute name with the namespace prefix (if any) replaced by the Namespace URI it
   * was bound to. This is not a standard representation of a node name, but it allows convenient
   * single-string comparison of the "universal" names of two nodes.
   *
  ...
  /**
   * Tell if the node is ignorable whitespace. Note that this can be determined only in the context
   * of a DTD or other Schema, and that DOM Level 2 has nostandardized DOM API which can return that
   * information.
   *
   * @deprecated
   ...
   *     of) a DocumentFragment or an orphaned node, as can arise if the DOM has been edited rather
   *     than being generated by a parser.
   */
   ...
   *
   * <p>Note that DOM Level 1 had no ability to retrieve this information. DOM Level 2 introduced it
   * but does not promise that it will be supported in all DOMs; those which can't support it will
   * always return null.
   *
   ...
   * should be done here; currently that's pushed up to a higher levelof our application. (Note that
   * DOM Level 1 didn't store the document's base URI.) TODO: Consider resolving Relative URIs.
   *
  ...
  /**
   * Store the factory object required to create DOM nodes in the result tree. In fact, that's just
   * the result tree's Document node...
   *
  ...
  /**
   * Get the textual contents of the node. See getNodeData(Node,FastStringBuffer) for discussion of
   * how whitespace nodes are handled.
   *
