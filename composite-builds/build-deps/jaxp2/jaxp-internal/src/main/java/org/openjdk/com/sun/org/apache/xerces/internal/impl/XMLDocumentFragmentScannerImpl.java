...
public class XMLDocumentFragmentScannerImpl extends XMLScanner
  ...
  private static final String[] RECOGNIZED_FEATURES = {
    NAMESPACES, VALIDATION, NOTIFY_BUILTIN_REFS, NOTIFY_CHAR_REFS, Constants.STAX_REPORT_CDATA_EVENT
  };
  ...
  // xxx do we need to create an extra XMLString object... look for using fTempString for collecting
  // all the data values
  ...
  /**
   * comma-delimited list of protocols that are allowed for the purpose of accessing external dtd or
   * entity references
   */
   ...
   *     the registered document handler. A value of false indicates that that the scanner should
   *     only scan the next portion of the document and return. A scanner instance is permitted to
   *     completely scan a document if it does not support this "pull" scanning model.
    ...
    do {
      switch (event) {
        ...
        case XMLStreamConstants.SPACE:
          // check if getCharacterData() is the right function to retrieve ignorableWhitespace
          // information.
  ...
  public void reset(XMLComponentManager componentManager) throws XMLConfigurationException {
    ...
    fExternalSubsetResolver =
        (resolver instanceof ExternalSubsetResolver) ? (ExternalSubsetResolver) resolver : null;
  ...
  public void reset(PropertyManager propertyManager) {
    ...
    // JAXP 1.5 features and properties
    XMLSecurityPropertyManager spm =
        (XMLSecurityPropertyManager) propertyManager.getProperty(XML_SECURITY_PROPERTY_MANAGER);
    fAccessExternalDTD = spm.getValue(XMLSecurityPropertyManager.Property.ACCESS_EXTERNAL_DTD);
  ...
  public void setProperty(String propertyId, Object value) throws XMLConfigurationException {
    ...
    if (propertyId.startsWith(Constants.XERCES_PROPERTY_PREFIX)) {
      ...
      if (suffixLength == Constants.ENTITY_RESOLVER_PROPERTY.length()
        ...
        fExternalSubsetResolver =
            (value instanceof ExternalSubsetResolver) ? (ExternalSubsetResolver) value : null;
        return;
      }
    }
  ...
  public void endEntity(String name, Augmentations augs) throws IOException, XNIException {
     ...
     * fDocumentHandler.characters(fStringBuffer, null); fStringBuffer.length = 0; // make sure we
     * know it's been flushed }
     */
   ...
   * contents of these variables will be destroyed. The caller should copy important information out
   * of these variables before calling this method. NB: Content in fAttributes is valid only till
   * the state of the parser is XMLEvent.START_ELEMENT
   *
  ...
  protected boolean scanStartElement() throws IOException, XNIException {
    ...
    if (fEmptyElement) {
      ...
      // We should not be popping out the context here in endELement becaause the namespace context
      // is still
  ...
  /**
   * protected void scanAttribute(AttributeIteratorImpl attributes) throws IOException, XNIException
  ...
  protected void scanAttribute(XMLAttributes attributes) throws IOException, XNIException {
    ...
    if (!fEntityScanner.skipChar('=')) {
      reportFatalError(
          "EqRequiredInAttribute", new Object[] {fCurrentElement.rawname, fAttributeQName.rawname});
    }
    fEntityScanner.skipSpaces();
    ...
    // WFC: Unique Att Spec
    // attributes count will be same if the current attribute  name already exists for this element
    // name.
    // this means there are two duplicate attributes.
    if (oldLen == attributes.getLength()) {
      reportFatalError(
          "AttributeNotUnique", new Object[] {fCurrentElement.rawname, fAttributeQName.rawname});
    }
  ...
  protected boolean scanCDATASection(XMLStringBuffer contentBuffer, boolean complete)
    ...
    while (true) {
      ...
      if (!fEntityScanner.scanData("]]>", contentBuffer)) {
        break;
        /**
         * We dont need all this code if we pass ']]>' as delimeter.. int brackets = 2; while
         * (fEntityScanner.skipChar(']')) { brackets++; }
         *
         * <p>//When we find more than 2 square brackets if (fDocumentHandler != null && brackets >
         * 2) { //we dont need to clear the buffer.. //contentBuffer.clear(); for (int i = 2; i <
         * brackets; i++) { contentBuffer.append(']'); } fDocumentHandler.characters(contentBuffer,
         * null); }
         *
         * <p>if (fEntityScanner.skipChar('>')) { break; } if (fDocumentHandler != null) { //we dont
         * need to clear the buffer now.. //contentBuffer.clear(); contentBuffer.append("]]");
   ...
   * variable will be destroyed. The caller should copy the needed information out of this variable
  ...
  protected int scanEndElement() throws IOException, XNIException {
    ...
    // We can skip this for now... In any case if the string doesn't match -- document is not well
    // formed.
    ...
    // We should not be popping out the context here in endELement becaause the namespace context is
    // still
    // valid when parser is at the endElement state.
   ...
   *     integrate with rest of Xerces2. SO maintaining the current behavior and still calling this
  ...
  protected class ElementStack2 {
    ...
    /**
     * This function doesn't increase depth. The function in this function is broken down into two
     * functions for efficiency. <@see>matchElement</see>. This function just returns the pointer to
  ...
  protected class ElementStack {
     ...
     * caller does <em>not</em> orphan the element to the stack. Also, the QName object returned is
     * <em>not</em> orphaned to the caller. It should be considered read-only.
     *
    ...
    public QName getNext() {
      ...
      if (DEBUG_SKIP_ALGORITHM) {
        System.out.println(
            "Element at fPosition = " + fPosition + " is " + fElements[fPosition].rawname);
      }
     ...
     * depth - because element was sucessfully skipped. 2. Store the position of the element token
     * in array "last opened tag" at depth. 3. increase the position counter so as to point to the
    ...
    public boolean matchElement(QName element) {
      ...
      if (fCount == fElements.length) {
        fSkip = false;
        fAdd = false;
        // reposition the stack -- it seems to be too complex document and there is no symmerty in
        // structure
        reposition();
        if (DEBUG_SKIP_ALGORITHM) {
    ...
    /**
     * Reposition the stack. fInt [] contains all the opened tags at particular depth. Transfer all
     * the opened tags starting from depth '2' to the current depth and reposition them as per the
     * depth.
     */
    ...
    /**
     * This function is as a result of optimization done for endElement -- we dont need to set the
     * value for every end element encouterd. For Well formedness checks we can have the same QName
     * object that was pushed. the values will be set only if application need to know about the
     * endElement -- neeraj.bajaj@sun.com
     */
   ...
   * next state/event.
   *
   * <p>Internally XML document is divided into several states. Each state represents a sections of
   * XML document. When this functions returns normally, it has read the section of XML document and
   * returns the state corresponding to section of document which has been read. For optimizations,
   * a particular driver can read ahead of the section of document (state returned) just read and
   ...
   *
   * <p>This class has been modified as per the new design which is more suited to efficiently build
   * pull parser. Lot of performance improvements have been done and the code has been added to
   * support stax functionality/features.
   *
   * @author Neeraj Bajaj, Sun Microsystems
   * @author Andy Clark, IBM
   * @author Eric Ye, IBM
  ...
  protected class FragmentContentDriver implements Driver {
     ...
     * the parser to one of its sub state. sub states are defined in the parser on the basis of
     * different XML component like SCANNER_STATE_ENTITY_REFERENCE , SCANNER_STATE_START_ELEMENT,
     * SCANNER_STATE_CDATA etc.. These sub states help the parser to have fine control over the
     * parsing. These are the different milepost, parser stops at each sub state (milepost). Based
     * on this state it is decided if paresr needs to stop at next milepost ??
     */
    ...
    /**
     * Drives the parser to the next state/event on the input. Parser is guaranteed to stop at the
     * next state/event. Internally XML document is divided into several states. Each state
     * represents a sections of XML document. When this functions returns normally, it has read the
     * section of XML document and returns the state corresponding to section of document which has
     * been read. For optimizations, a particular driver can read ahead of the section of document
        ...
        try {
          ...
          if (fIsCoalesce) {
            ...
            if (fLastSectionWasCharacterData) {
              ...
              // if we dont encounter any CDATA or ENITY REFERENCE and current state is also not
          ...
          switch (fScannerState) {
            ...
            case SCANNER_STATE_START_ELEMENT_TAG:
              {
                ...
                // xxx this function returns true when element is empty.. can be linked to end
                // element event.
            ...
            case SCANNER_STATE_CHARACTER_DATA:
              {
                ...
                // if last section was either entity reference or cdata or character data we should
                // be using buffer
                fUsebuffer =
                ...
                // When coalesce is set to true and last state was REFERENCE or CDATA or
                // CHARACTER_DATA, buffer should not be cleared.
                if (fIsCoalesce
                    && (fLastSectionWasEntityReference
                        || fLastSectionWasCData
                        || fLastSectionWasCharacterData)) {
                ...
                if (fEntityScanner.skipChar('<')) {
                  ...
                  // in case last section was either entity reference or cdata or character data --
                  // we should be using buffer
                  if (fUsebuffer) {
                    fContentBuffer.append(fTempString);
                    fTempString.length = 0;
                  }
                ...
                do {
                  ...
                  // ?
                    ...
                    if (XMLChar.isHighSurrogate(c)) {
                      ...
                      reportFatalError(
                          "InvalidCharInContent", new Object[] {Integer.toString(c, 16)});
                      fEntityScanner.scanChar();
                    }
                    break;
                  }
            ...
            case SCANNER_STATE_END_ELEMENT_TAG:
              {
                if (fEmptyElement) {
                  ...
                  if (elementDepthIsZeroHook()) {
                    ...
                    // the state shouldn't be set, because it is set by elementDepthIsZeroHook()
                    // function
            ...
            case SCANNER_STATE_CDATA:
              { // SCANNER_STATE_CDATA: {
                ...
                // we should not clear the buffer only when the last state was either
                // SCANNER_STATE_REFERENCE or
                ...
                // behave appropriately. When isCoalesce is set to true we dont need to reportCDATA
                // event
    ...
    /**
     * Scan for DOCTYPE hook. This method is a hook for subclasses to add code to handle scanning
     * for a the "DOCTYPE" string after the string "<!" has been scanned.
     *
    ...
    /**
     * Element depth iz zero. This methos is a hook for subclasses to add code to handle when the
     * element depth hits zero. When scanning a document fragment, an element depth of zero is
     * normal. However, when scanning a full XML document, the scanner must handle the trailing
     * miscellanous section of the document after the end of the document's root element.
     *
     ...
     * scanning for the root element. When scanning a document fragment, there is no "root" element.
     * However, when scanning a full XML document, the scanner must handle the root element
     * specially.
     *
     * @return True if the caller should stop and return true which allows the scanner to switch to
     *     a new scanning driver. A return value of false indicates that the content driver should
    ...
    /**
     * End of file hook. This method is a hook for subclasses to add code that handles the end of
     * file. The end of file in a document fragment is OK if the markup depth is zero. However, when
     * scanning a full XML document, an end of file is always premature.
     */
