...
public final class EncodingInfo extends Object {
  ...
  /**
   * A helper object that we can ask if a single char, or a surrogate UTF-16 pair of chars that form
   * a single character, is in this encoding.
   */
  ...
  /**
   * Create an EncodingInfo object based on the ISO name and Java name. If both parameters are null
   * any character will be considered to be in the encoding. This is useful for when the serializer
   * is in temporary output state, and has no assciated encoding.
   *
  ...
  /**
   * A simple interface to isolate the implementation. We could also use some new JRE 1.4 methods in
   * another implementation provided we use reflection with them.
   *
  ...
  private class EncodingImpl implements InEncoding {
    ...
    /**
     * m_first through m_last is the range of unicode values that this object will return an answer
     * on. It may delegate to a similar object with a different range
     */
    ...
    /**
     * The object, of the same type as this one, that handles unicode values in a range before the
     * range explictly handled by this object, and to which this object may delegate.
     */
    private InEncoding m_before;

    /**
     * The object, of the same type as this one, that handles unicode values in a range after the
     * range explictly handled by this object, and to which this object may delegate.
     */
    private InEncoding m_after;

    /**
     * The number of unicode values explicitly handled by a single EncodingInfo object. This value
     * is tuneable, but is set to 128 because that covers the entire low range of ASCII type chars
  ...
  /**
   * This is heart of the code that determines if a given high/low surrogate pair forms a character
   * that is in the given encoding. This method is probably expensive, and the answer should be
   * cached.
   *
   * <p>This method is not a public API, and should only be used internally within the serializer.
   *
