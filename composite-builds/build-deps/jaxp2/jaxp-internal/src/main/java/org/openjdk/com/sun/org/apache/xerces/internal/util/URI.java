...
public class URI implements Serializable {
   ...
   * accordingly - setting the scheme, userinfo, host,port, path, query string and fragment fields
   * as necessary. If the specification does not follow the "generic URI" syntax, the specification
   * is parsed into a scheme and scheme-specific part (stored as the path) only. Construct a
   * relative URI if boolean is assigned to "true" and p_uriSpec is not valid absolute URI, instead
    ...
    if (host.length() > 0) {
      ...
      if (hasPort) {
        ...
        if (portStr.length() > 0) {
          ...
          /**
           * for (int i = 0; i < portStr.length(); i++) { if (!isDigit(portStr.charAt(i))) { throw
           * new MalformedURIException( portStr + " is invalid. Port should only contain digits!");
           * } }*
           */
          // REVISIT: Remove this code.
   ...
   * port is set to -1.
   *
   * <p>Note: This method overwrites registry based authority if it previously existed in this URI.
   *
   ...
   * @exception MalformedURIException if p_queryString is not null and this URI does not conform to
   ...
   * @exception MalformedURIException if p_fragment is not null and this URI does not conform to the
  ...
  public static boolean isWellFormedIPv4Address(String address) {
    ...
    for (int i = 0; i < addrLength; i++) {
      ...
      // Check that this segment is not greater than 255.
        ...
        if (!(first < '2'
            || (first == '2' && (second < '5' || (second == '5' && testChar <= '5'))))) {
          return false;
        }
      }
    }
   ...
   * defined in RFC 2373. The IPv6 address is parsed according to Section 2.2 of RFC 2373, with the
   * additional constraint that the address be composed of 128 bits of information.
   *
   ...
   * was in fact removed from RFC 3513, the successor of RFC 2373.
   *
