...
public class XSDHandler {
  ...
  /**
   * This method initiates the parse of a schema. It will likely be called from the Validator and it
   * will make the resulting grammar available; it returns a reference to this object just in case.
   * A reset(XMLComponentManager) must be called before this methods is called.
   *
   * @param is
   * @param desc
   * @param locationPairs
  ...
  public SchemaGrammar parseSchema(XMLInputSource is, XSDDescription desc, Map locationPairs)
    ...
    if (is instanceof DOMInputSource) {
      ...
      schemaRoot =
          getSchemaDocument(
              schemaNamespace, is, referType == XSDDescription.CONTEXT_PREPARSE, referType, null);
    ...
    if (referType == XSDDescription.CONTEXT_PREPARSE) {
      ...
      if (grammar != null) {
        ...
        if (!fNamespaceGrowth
            || (schemaId != null && grammar.getDocumentLocations().contains(schemaId))) {
          return grammar;
        }
      }
  ...
  /**
   * First try to find a grammar in the bucket, if failed, consult the grammar pool. If a grammar is
   * found in the pool, then add it (and all imported ones) into the bucket.
   */
  ...
  protected XSDocumentInfo constructTrees(
    ...
    for (Element child = DOMUtil.getFirstChildElement(rootNode);
      ...
      if (localName.equals(SchemaSymbols.ELT_ANNOTATION)) continue;
        ...
        // if ((!fHonourAllSchemaLocations && findGrammar(fSchemaGrammarDescription) != null) ||
        ...
        if (!alreadyTraversed) {
          newSchemaRoot =
              resolveSchema(schemaSource, fSchemaGrammarDescription, mustResolve, child);
      ...
      if (fLastSchemaWasDuplicate) {
        ...
        newSchemaInfo =
            constructTrees(newSchemaRoot, schemaHint, fSchemaGrammarDescription, importCollision);
      }
   ...
   * Namespace growth
   *
   * <p>Go through the import list of a given grammar and for each imported grammar, check to see if
   * the grammar bucket has a newer version. If a new instance is found, we update the import list
   ...
   * Namespace growth
   *
   * <p>Go throuth the grammar bucket, and for each grammar in the bucket check the import list. If
   * there exists a grammar in import list that has the same namespace as newGrammar, but a
          ...
          for (Element redefineComp = DOMUtil.getFirstChildElement(globalComp);
            ...
            if (componentType.equals(SchemaSymbols.ELT_ATTRIBUTEGROUP)) {
              ...
              if (componentType.equals(SchemaSymbols.ELT_COMPLEXTYPE)) {
                renameRedefiningComponents(
                    currSchemaDoc, redefineComp, SchemaSymbols.ELT_COMPLEXTYPE, lName, targetLName);
              } else { // must be simpleType
                renameRedefiningComponents(
                    currSchemaDoc, redefineComp, SchemaSymbols.ELT_SIMPLETYPE, lName, targetLName);
              }
              ...
              renameRedefiningComponents(
                  currSchemaDoc, redefineComp, SchemaSymbols.ELT_GROUP, lName, targetLName);
            }
    ...
    while (!schemasToProcess.empty()) {
      ...
      for (Element globalComp = DOMUtil.getFirstVisibleChildElement(currRoot, fHiddenNodes);
        ...
        if (DOMUtil.getLocalName(globalComp).equals(SchemaSymbols.ELT_REDEFINE)) {
          ...
          currSG.addAnnotation(
              fElementTraverser.traverseAnnotationDecl(
                  globalComp, currSchemaDoc.getSchemaAttrs(), true, currSchemaDoc));
          sawAnnotation = true;
        } else {
          reportSchemaError(
              "s4s-elt-invalid-content.1",
              new Object[] {SchemaSymbols.ELT_SCHEMA, DOMUtil.getLocalName(globalComp)},
              globalComp);
        }
      } // end for
      ...
      if (!sawAnnotation) {
        ...
        if (text != null) {
          currSG.addAnnotation(
              fElementTraverser.traverseSyntheticAnnotation(
                  currRoot, text, currSchemaDoc.getSchemaAttrs(), true, currSchemaDoc));
        }
      }
  ...
  protected Object getGlobalDecl(
    ...
    if (sGrammar == null) {
      if (needReportTNSError(declToTraverse.uri))
        reportSchemaError(
            "src-resolve", new Object[] {declToTraverse.rawname, COMP_TYPE[declType]}, elmNode);
      return null;
    }
    ...
    if (DOMUtil.isHidden(decl, fHiddenNodes)) {
      if (retObj == null) {
        ...
        reportSchemaError(
            code, new Object[] {declToTraverse.prefix + ":" + declToTraverse.localpart}, elmNode);
      }
      return retObj;
    }
  ...
  private Element resolveSchema(
    ...
    try {
      ...
      if (mustResolve) {
        reportSchemaError(
            "schema_reference.4", new Object[] {desc.getLocationHints()[0]}, referElement);
      } else {
        reportSchemaWarning(
            "schema_reference.4", new Object[] {desc.getLocationHints()[0]}, referElement);
      }
    ...
    return getSchemaDocument(
        desc.getTargetNamespace(), schemaSource, mustResolve, desc.getContextType(), referElement);
  } // getSchema(String, String, String, boolean, short):  Document

  private Element resolveSchema(
      XMLInputSource schemaSource, XSDDescription desc, boolean mustResolve, Element referElement) {
          ...
          if (referType == XSDDescription.CONTEXT_IMPORT
            ...
            String accessError =
                SecuritySupport.checkAccess(
                    schemaId, fAccessExternalSchema, Constants.ACCESS_EXTERNAL_ALL);
            if (accessError != null) {
    ...
    try {
      if (inputSource != null
        ...
        if (referType != XSDDescription.CONTEXT_PREPARSE) {
          schemaId =
              XMLEntityManager.expandSystemId(
                  inputSource.getSystemId(), schemaSource.getBaseSystemId(), false);
          key = new XSDKey(schemaId, referType, schemaNamespace);
          if ((schemaElement = (Element) fTraversed.get(key)) != null) {
            fLastSchemaWasDuplicate = true;
            return schemaElement;
        ...
        if (parser != null) {
          ...
          try {
            ...
            System.err.println("Warning: " + parser.getClass().getName() + ": " + exc.getMessage());
          }
        }
  ...
  private Element getSchemaDocument1(
    ...
    if (mustResolve) {
      if (hasInput) {
        reportSchemaError(
            "schema_reference.4", new Object[] {schemaSource.getSystemId()}, referElement, ioe);
      } else {
        reportSchemaError(
            "schema_reference.4",
    ...
    switch (componentType) {
      case XSConstants.TYPE_DEFINITION:
        expandRelatedTypeComponents(
            (XSTypeDefinition) component, componentList, component.getNamespace(), dependencies);
        break;
      case XSConstants.ATTRIBUTE_DECLARATION:
  ...
  public void reset(XMLComponentManager componentManager) {
    ...
    try {
      fSchemaParser.setFeature(
          CONTINUE_AFTER_FATAL_ERROR, fErrorReporter.getFeature(CONTINUE_AFTER_FATAL_ERROR));
    } catch (XMLConfigurationException e) {
    }
    ...
    XMLSecurityPropertyManager securityPropertyMgr =
        (XMLSecurityPropertyManager) componentManager.getProperty(XML_SECURITY_PROPERTY_MANAGER);
    // Passing on the setting to the parser
    fSchemaParser.setProperty(XML_SECURITY_PROPERTY_MANAGER, securityPropertyMgr);

    fAccessExternalDTD =
        securityPropertyMgr.getValue(XMLSecurityPropertyManager.Property.ACCESS_EXTERNAL_DTD);

    fAccessExternalSchema =
        securityPropertyMgr.getValue(XMLSecurityPropertyManager.Property.ACCESS_EXTERNAL_SCHEMA);
  } // reset(XMLComponentManager)

  /**
   * Traverse all the deferred local elements. This method should be called by traverseSchemas after
   * we've done with all the global declarations.
   */
    ...
    if ((objElem = registry.get(qName)) == null) {
      ...
      if (redefinedSchema != null) { // redefinition involved somehow
        ...
        if (redefinedSchema == currSchema) { // object comp. okay here
          ...
          if (currSchema.fTargetNamespace == null)
            checkForDuplicateNames(
                "," + newName, declType, registry, registry_sub, currComp, currSchema);
          else
            checkForDuplicateNames(
  ...
  private void renameRedefiningComponents(
    ...
    if (componentType.equals(SchemaSymbols.ELT_SIMPLETYPE)) {
          ...
          if (greatGrandKid == null) {
            ...
            if (greatGrandKid == null) {
              ...
              if (!greatGrandKidName.equals(SchemaSymbols.ELT_RESTRICTION)
                ...
                reportSchemaError(
                    "src-redefine.5.b.c", new Object[] {greatGrandKidName}, greatGrandKid);
              } else {
                Object[] attrs =
      ...
      int attGroupRefsCount =
          changeRedefineGroup(processedBaseName, componentType, newName, child, currSchema);
      ...
      reportSchemaError(
          "Internal-Error",
          new Object[] {
            "could not handle this particular <redefine>; please submit your schemas and instance"
                + " document in a bug report!"
          },
          child);
    }
   ...
   * grammar bucket. It will also return an object for the schema for schemas which will contain at
   * least the relevant declarations for annotations.
   */
      ...
      if (grammarType == XMLGrammarDescription.XML_SCHEMA) {
        if (fInitialGrammarSet == null) {
          if (fGrammarBucket == null) {
            ...
            SchemaGrammar[] schemaGrammars = fGrammarBucket.getGrammars();
            /**
             * If the grammar bucket already contains the schema for schemas then we already have
             * the definitions for the parts relevant to annotations.
             */
            for (int i = 0; i < schemaGrammars.length; ++i) {
  ...
  /**
   * used to identify a reference to a schema document if the same document is referenced twice with
   * the same key, then we only need to parse it once.
   *
   * <p>When 2 XSDKey's are compared, the following table can be used to determine whether they are
   * equal: inc red imp pre ins inc N/L ? N/L N/L N/L red ? N/L ? ? ? imp N/L ? N/P N/P N/P pre N/L
   * ? N/P N/P N/P ins N/L ? N/P N/P N/P
   *
   * <p>Where: N/L: duplicate when they have the same namespace and location. ? : not clear from the
   * spec. REVISIT: to simplify the process, also considering it's very rare, we treat them as not
   * duplicate. N/P: not possible. imp/pre/ins are referenced by namespace. when the first time we
   * encounter a schema document for a namespace, we create a grammar and store it in the grammar
   * bucket. when we see another reference to the same namespace, we first check whether a grammar
   * with the same namespace is already in the bucket, which is true in this case, so we won't
   * create another XSDKey.
   *
