...
public class GetOpt {
  ...
  /**
   * gets the next option found in the commandline. Distinguishes between two bad cases, one case is
   * when an illegal option is found, and then other case is when an option takes an argument but no
   * argument was found for that option. If the option found was not declared in the optString, then
   * an IllegalArgumentException will be thrown (case 1). If the next option found has been declared
   * to take an argument, and no such argument exists, then a MissingOptArgException is thrown (case
   * 2).
   *
   * @param none
