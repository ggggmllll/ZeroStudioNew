...
public class XMLEntityScanner implements XMLLocator {
  ...
  /**
   * Returns the XML version of the current entity. This will normally be the value from the XML or
   * text declaration or defaulted by the parser. Note that that this value may be different than
   * the version of the processing rules applied to the current entity. For instance, an XML 1.1
   * document may refer to XML 1.0 entities. In such a case the rules of XML 1.1 are applied to the
   * entire document. Also note that, for a given entity, this value can only be considered final
   * once the XML or text declaration has been read or once it has been determined that there is no
   * such declaration.
   */
  ...
  /**
   * Returns the encoding of the current entity. Note that, for a given entity, this value can only
   * be considered final once the encoding declaration has been read (or once it has been determined
   * that there is no such declaration) since, no encoding having been specified on the
   ...
   *
   * <p><strong>Note:</strong> The underlying character reader on the current entity will be changed
   * to accomodate the new encoding. However, the new encoding is ignored if the current reader was
   * not constructed from an input stream (e.g. an external entity that is resolved directly to the
   ...
   *
   * <p><strong>Note:</strong> The string returned must be a symbol. The SymbolTable can be used for
   * this purpose.
   *
   * @throws IOException Thrown if i/o error occurs.
   * @throws EOFException Thrown on end of file.
   ...
   * <p><strong>Note:</strong> The characters are consumed.
   *
   * <p><strong>Note:</strong> This method does not guarantee to return the longest run of attribute
   * value data. This method may return before the quote character due to reaching the end of the
   * input buffer or any other reason.
   *
   * <p><strong>Note:</strong> The fields contained in the XMLString structure are not guaranteed to
   * remain valid upon subsequent calls to the entity scanner. Therefore, the caller is responsible
   * for immediately using the returned character data or making a copy of the character data.
   *
   ...
   * <p><strong>Note:</strong> The characters are consumed.
   *
   * <p><strong>Note:</strong> This assumes that the length of the delimiter and that the delimiter
   * contains at least one character.
   *
   * <p><strong>Note:</strong> This method does not guarantee to return the longest run of character
   * data. This method may return before the delimiter due to reaching the end of the input buffer
   * or any other reason.
   *
   * <p>
   *
   ...
   *
   * <p><strong>Note:</strong> The character is consumed only if it matches the specified character.
   *
   ...
   *     Boolean which is true iff the document is big endian, false if it's little-endian, and null
   *     if the distinction isn't relevant.
   */
   ...
   * scanning structures such as the XMLDecl and TextDecl that can only contain US-ASCII characters.
   *
