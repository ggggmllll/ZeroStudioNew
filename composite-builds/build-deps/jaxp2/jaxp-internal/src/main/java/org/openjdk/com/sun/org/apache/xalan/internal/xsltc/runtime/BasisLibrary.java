...
public final class BasisLibrary {
  ...
  /**
   * External functions that cannot be resolved are replaced with a call to this method. This method
   * will generate a runtime errors. A good stylesheet checks whether the function exists using
   * conditional constructs, and never really tries to call it if it doesn't exist. But simple
   * stylesheets may result in a call to this method. The compiler should generate a warning if it
   * encounters a call to an unresolved external function.
   */
   ...
   *
   * <p>This is only used in forward-compatibility mode, when the control flow cannot be determined.
   * In 1.0 mode, the error message is emitted at compile time.
   */
   ...
   * Uses a specialized formatter object for very large and very small numbers that ignores locales,
   * thus using always using "." as a decimal separator.
   */
  ...
  /**
   * In a perfect world, this would be the implementation for nodeList2Iterator. In reality, though,
   * this causes a ClassCastException in getDTMHandleFromNode because SAXImpl is not an instance of
   * DOM2DTM. So we use the more lengthy implementation below until this issue has been addressed.
   *
  ...
  /**
   * Utility function to check if xsl:attribute has a valid qname This method should only be invoked
   * if the name attribute is an AVT
   */
  ...
  /**
   * Utility function to check if a name is a valid ncname This method should only be invoked if the
   * attribute value is an AVT
   */
  ...
  /**
   * Utility function to check if a name is a valid qname This method should only be invoked if the
   * attribute value is an AVT
   */
  ...
  /**
   * Utility method to allow setting parameters of the form {namespaceuri}localName which get mapped
   * to an instance variable in the class Hence a parameter of the form "{http://foo.bar}xyz" will
   * be replaced with the corresponding values by the BasisLibrary's utility method
  ...
  public static String mapQNameToJavaName(String base) {
    return replace(
        ...
        ".-:/{}?#%*",
        new String[] {
          "$dot$", "$dash$", "$colon$", "$slash$", "", "$colon$", "$ques$", "$hash$", "$per$",
          "$aster$"
        });
  }
