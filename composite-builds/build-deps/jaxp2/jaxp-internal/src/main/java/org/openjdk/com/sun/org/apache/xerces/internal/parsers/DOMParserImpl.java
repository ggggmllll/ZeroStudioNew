...
public class DOMParserImpl extends AbstractDOMParser implements LSParser, DOMConfiguration {
    ...
    if (schemaType != null) {
      if (schemaType.equals(Constants.NS_DTD)) {
        ...
        fConfiguration.setProperty(
            Constants.JAXP_PROPERTY_PREFIX + Constants.SCHEMA_LANGUAGE, Constants.NS_DTD);
        fSchemaType = Constants.NS_DTD;
      } else if (schemaType.equals(Constants.NS_XMLSCHEMA)) {
        // XML Schem validation
        fConfiguration.setProperty(
            Constants.JAXP_PROPERTY_PREFIX + Constants.SCHEMA_LANGUAGE, Constants.NS_XMLSCHEMA);
      }
    }
  }
   ...
   * completion of the construction of each <code>Element</code> node. The filter implementation can
   * choose to remove the element from the document being constructed (unless the element is the
   * document element) or to terminate the parse early. If the document is being validated when it's
   * loaded the validation happens before the filter is called.
   */
    ...
    if (value instanceof Boolean) {
      ...
      if (name.equalsIgnoreCase(Constants.DOM_ERROR_HANDLER)) {
        ...
        if (value instanceof LSResourceResolver || value == null) {
          try {
            fConfiguration.setProperty(
                ENTITY_RESOLVER, new DOMEntityResolverWrapper((LSResourceResolver) value));
          } catch (XMLConfigurationException e) {
          }
        } else {
          // REVISIT: type mismatch
   ...
   * contextNode</code> and <code>action</code> arguments. When parsing the input stream the context
   ...
   *     being inserted and the existing children of the context node. The set of possible actions
   *     is defined above.
