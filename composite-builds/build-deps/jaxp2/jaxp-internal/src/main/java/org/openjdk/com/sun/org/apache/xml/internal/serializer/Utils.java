...
class Utils {
   ...
   *
   * <p>This implementation, for performance reasons, caches all classes loaded by name and returns
   * the cached Class object if it can previously loaded classes that were load by name. If not
   * previously loaded an attempt is made to load with Class.forName(classname)
   *
