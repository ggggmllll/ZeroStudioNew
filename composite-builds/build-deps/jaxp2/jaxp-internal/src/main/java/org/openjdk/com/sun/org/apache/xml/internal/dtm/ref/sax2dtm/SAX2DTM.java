...
public class SAX2DTM extends DTMDefaultBaseIterators
  ...
  /**
   * Vector of entities. Each record is composed of four Strings: publicId, systemID, notationName,
   * and name.
   */
  ...
  public ContentHandler getContentHandler() {
    ...
    if (m_incrementalSAXSource
        .getClass()
        .getName()
        .equals("org.openjdk.com.sun.org.apache.xml.internaldtm.ref.IncrementalSAXSource_Filter"))
    ...
    if (m_textPendingStart >= 0) // -1 indicates no-text-in-progress
    {
      ...
      if (doStrip) {
        ...
        if (length > 0) {
          ...
          m_previous =
              addNode(m_coalescedTextType, exName, m_parents.peek(), m_previous, dataIndex, false);
  ...
  public void processingInstruction(String target, String data) throws SAXException {
    ...
    int exName =
        m_expandedNameTable.getExpandedTypeID(null, target, DTM.PROCESSING_INSTRUCTION_NODE);
   ...
   *
   * <p><strong>NOTE:</entity> entity references in attribute values -- and the start and end of the
   * document entity -- are never reported.
   *
   * <p>The start and end of the external DTD subset are reported using the pseudo-name "[dtd]". All
   ...
   *
   * <p>%REVIEW% Now that we no longer use this method to support getSourceLocatorFor, can we remove
   * it?
   *
   ...
   *
   * <p>(We _could_ return a locator with the document's base URI and bogus line/column information.
   * Trying that; see the else clause.)
   */
