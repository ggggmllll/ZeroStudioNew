...
public final class AttributesImplSerializer extends AttributesImpl {
  /**
   * Hash table of qName/index values to quickly lookup the index of an attributes qName. qNames are
   * in uppercase in the hash table to make the search case insensitive.
   *
  ...
  /**
   * This method adds the attribute, but also records its qName/index pair in the hashtable for fast
   * lookup by getIndex(qName).
   *
  ...
  /**
   * We are switching over to having a hash table for quick look up of attributes, but up until now
   * we haven't kept any information in the Hashtable, so we now update the Hashtable. Future
