 ...
 * ManagementFactory#getCompilationMXBean} method or from the {@link
 * ManagementFactory#getPlatformMBeanServer platform <tt>MBeanServer</tt>} method.
 *
 * <p>The <tt>ObjectName</tt> for uniquely identifying the MXBean for the compilation system within
 * an MBeanServer is:
 *
 * <blockquote>
 *
...
public interface CompilationMXBean extends PlatformManagedObject {
   ...
   * multiple threads are used for compilation, this value is summation of the approximate time that
   * each thread spent in compilation.
   *
   * <p>This method is optionally supported by the platform. A Java virtual machine implementation
   * may not support the compilation time monitoring. The {@link
   * #isCompilationTimeMonitoringSupported} method can be used to determine if the Java virtual
   * machine supports this operation.
   *
   * <p>This value does not indicate the level of performance of the Java virtual machine and is not
