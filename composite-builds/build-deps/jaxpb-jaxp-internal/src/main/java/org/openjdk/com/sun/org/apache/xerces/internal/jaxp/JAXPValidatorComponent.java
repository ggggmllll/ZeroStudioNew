...
final class JAXPValidatorComponent extends TeeXMLDocumentFilterImpl implements XMLComponent {
  ...
  /**
   * Used to store the {@link Augmentations} associated with the current event, so that we can pick
   * it up again when the event is forwarded by the {@link ValidatorHandler}.
   *
   * <p>UGLY HACK.
   */
    ...
    validator.setResourceResolver(
        new LSResourceResolver() {
          public LSInput resolveResource(
              String type, String ns, String publicId, String systemId, String baseUri) {
   ...
   *
   * <p>We only forward certain events from a {@link ValidatorHandler}. Other events should go "the
   * 2nd direct route".
   */
