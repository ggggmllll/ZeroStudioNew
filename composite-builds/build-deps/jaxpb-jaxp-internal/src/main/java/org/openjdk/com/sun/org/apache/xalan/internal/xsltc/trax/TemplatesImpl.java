...
public final class TemplatesImpl implements Templates, Serializable {
  ...
  /**
   * Contains the translet class definition(s). These are created when this Templates is created or
   * when it is read back from disk.
   */
  ...
  /**
   * Cache the DTM for the stylesheet in a thread local variable, which is used by the document('')
   * function. Use ThreadLocal because a DTM cannot be shared between multiple threads. Declaring it
   * transient to fix bug 22438
   */
   ...
   *
   * <p>Note: This method is private for security reasons. See CR 6537898. When merging with Apache,
   * we must ensure that the privateness of this method is maintained (that is why it wasn't
   * removed).
   */
  ...
  /**
   * Defines the translet class and auxiliary classes. Returns a reference to the Class object that
  ...
  /**
   * Implements JAXP's Templates.getOutputProperties(). We need to instanciate a translet to get the
   * output settings, so we might as well just instanciate a Transformer and use its implementation
   * of this method.
   */
