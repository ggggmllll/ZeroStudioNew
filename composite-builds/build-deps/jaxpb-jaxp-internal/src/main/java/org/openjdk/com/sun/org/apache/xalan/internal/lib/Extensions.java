...
public class Extensions {
   ...
   *     <p>Note: The usage of this extension function in the xalan namespace is deprecated. Please
   *     use the same function in the EXSLT sets extension (http://exslt.org/sets).
   */
  ...
  /**
   * Returns the result of evaluating the argument as a string containing an XPath expression. Used
   * where the XPath expression is not known until run-time. The expression is evaluated as if the
   * run-time value of the argument appeared in place of the evaluate function call at compile time.
   *
   ...
   *     <p>Note: The usage of this extension function in the xalan namespace is deprecated. Please
   *     use the same function in the EXSLT dynamic extension (http://exslt.org/dynamic).
   */
  ...
  /**
   * Returns a NodeSet containing one text node for each token in the first argument. Delimiters are
   * whitespace. That is, the delimiters that are used are tab (&#x09), linefeed (&#x0A), return
   * (&#x0D), and space (&#x20). Tokens are determined by a call to <code>StringTokenizer</code>. If
   * the first argument is an empty string or contains only delimiters, the result will be an empty
   * NodeSet.
   *
   * <p>Contributed to XalanJ1 by <a href="mailto:benoit.cerrina@writeme.com">Benoit Cerrina</a>.
   *
   ...
   * utility to grab info about the Java environment and CLASSPATH, etc., and then returns the
   * resulting Node. Stylesheets can then maniuplate this data or simply xsl:copy-of the Node. Note
   * that we first attempt to load the more advanced org.apache.env.Which utility by reflection;
   * only if that fails to we still use the internal version. Which is available from <a
