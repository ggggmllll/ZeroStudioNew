...
public abstract class SerializerBase implements SerializationHandler, SerializerConstants {
  ...
  /**
   * Flag to tell that we need to add the doctype decl, which we can't do until the first element is
   * encountered.
   */
   ...
   * cdata-section-elements attribute. Every second one is a local name, and every other second one
   * is the URI for the local name.
   */
  ...
  /**
   * A reference to "stack frame" corresponding to the current element. Such a frame is pushed at a
   * startElement() and popped at an endElement(). This frame contains information about the
   ...
   *     this information for reporting its own errors (such as character content that does not
   *     match an application's business rules). The information returned by the locator is probably
   ...
   * the element has not yet been written to the output. In the stream case this means that we have
   * not yet been forced to close the elements opening tag by another notification, such as a
  ...
  public void addAttribute(
      String uri, String localName, String rawName, String type, String value, boolean XSLAttribute)
      throws SAXException {
  ...
  public void addAttributes(Attributes atts) throws SAXException {
    ...
    for (int i = 0; i < nAtts; i++) {
      ...
      addAttributeAlways(
          uri, atts.getLocalName(i), atts.getQName(i), atts.getType(i), atts.getValue(i), false);
    }
  }
  ...
  /**
   * Flush and close the underlying java.io.Writer. This method applies to ToStream serializers, not
   * ToSAXHandler serializers.
   *
   * @see ToStream
   */
  public void close() {
   ...
   *     be included in the output document. This method remembers if the value was explicitly set
   *     using this method, verses if the value is the default value.
   */
   ...
   * these trace events: EVENTTYPE_OUTPUT_PSEUDO_CHARACTERS EVENTTYPE_OUTPUT_CHARACTERS which trace
