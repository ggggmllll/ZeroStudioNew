...
class XSDAttributeTraverser extends XSDAbstractTraverser {
  ...
  protected XSAttributeUseImpl traverseLocal(
    ...
    if (defaultAtt != null && attrUse != null) {
      // 2 if there is a {value constraint}, the canonical lexical representation of its value must
      // be valid with respect to the {type definition} as defined in String Valid (3.14.4).
      ...
      // 2 If the {attribute declaration} has a fixed {value constraint}, then if the attribute use
      // itself has a {value constraint}, it must also be fixed and its value must match that of the
  ...
  XSAttributeDecl traverseNamedAttr(
    ...
    if (attrType == null && typeAtt != null) {
      XSTypeDefinition type =
          (XSTypeDefinition)
              fSchemaHandler.getGlobalDecl(schemaDoc, XSDHandler.TYPEDECL_TYPE, typeAtt, attrDecl);
      if (type != null && type.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE) {
        attrType = (XSSimpleType) type;
      } else {
        reportSchemaError(
            "src-resolve", new Object[] {typeAtt.rawname, "simpleType definition"}, attrDecl);
        if (type == null) {
    ...
    // valid with respect to the {type definition} as defined in String Valid (3.14.4).
    if (attDefault != null) {
      fValidationState.setNamespaceSupport(schemaDoc.fNamespaceSupport);
      try {
        ...
        reportSchemaError(ide.getKey(), ide.getArgs(), attrDecl);
        reportSchemaError(
            "a-props-correct.2", new Object[] {nameAtt, attDefault.normalizedValue}, attrDecl);
        // Recover by removing the default value
        attDefault = null;
