...
class XPathResultImpl implements XPathResult, EventListener {
  ...
  XPathResultImpl(short type, XObject result, Node contextNode, XPath xpath) {
    ...
    if (null == result) {
      String fmsg =
          XPATHMessages.createXPATHMessage(XPATHErrorResources.ER_EMPTY_XPATH_RESULT, null);
    ...
    if ((m_resultType == ORDERED_NODE_ITERATOR_TYPE)
      ...
      try {
        ...
        throw new XPathException(
            ...
            fmsg); // "The XPathResult of XPath expression {0} has an XPathResultType of {1} which
        // cannot be coerced into the specified XPathResultType of {2}."},
      }
  ...
  public double getNumberValue() throws XPathException {
    if (getResultType() != NUMBER_TYPE) {
      ...
      //              "The XPathResult of XPath expression {0} has an XPathResultType of {1} which
  ...
  private short getTypeFromXObject(XObject object) {
    switch (object.getType()) {
      ...
      //          case XObject.CLASS_YMDURATION: return STRING_TYPE; // treat all date types as
      // strings?
