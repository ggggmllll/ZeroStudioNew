...
public class NamedNodeMapImpl implements NamedNodeMap, Serializable {
  ...
  /**
   * Report how many nodes are currently stored in this NamedNodeMap. Caveat: This is a count rather
   * than an index, so the highest-numbered node at any time can be accessed via
   * item(getLength()-1).
   */
   ...
   *     will always refer to the same entry, or that there is any stable ordering of entries... and
   *     be prepared for double-reporting or skips as insertion and deletion occur.
   ...
   *     point at which to insert the item, encoded as -1-index and hence reconvertable by
   *     subtracting it from -1. (Encoding because I don't want to recompare the strings but don't
