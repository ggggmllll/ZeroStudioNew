...
final class XslElement extends Instruction {
  ...
  public void parseContents(Parser parser) {
    ...
    if (_isLiteralName) {
      ...
      _namespace =
          (namespace == EMPTYSTRING) ? null : new AttributeValueTemplate(namespace, parser, this);
    }
   ...
   * there is no need to inspect the element name at runtime to determine if a prefix exists, needs
   * to be generated, etc.
   */
   ...
   * name, (ii) checks for a prefix in the name (iii) generates a new prefix and create a new qname
   * when necessary (iv) calls startElement() on the handler (v) looks up a uri in the XML when the
   * prefix is not known at compile time (vi) calls namespace() on the handler (vii) evaluates the
   * contents (viii) calls endElement().
   */
