...
public class TextImpl extends CharacterDataImpl implements CharacterData, Text {
  ...
  /**
   * internal method taking a StringBuffer in parameter and inserts the text content at the start of
   * the buffer
   *
   * @param buf
   */
  ...
  /**
   * Replaces the text of the current node and all logically-adjacent text nodes with the specified
   * text. All logically-adjacent text nodes are removed including the current node unless it was
   * the recipient of the replacement text.
   *
   ...
   * CDATASection nodes, the replaceWholeText method must fail before performing any modification of
   * the document, raising a DOMException with the code NO_MODIFICATION_ALLOWED_ERR. Traverse
   * previous siblings of the node to be replaced. If a previous sibling is an EntityReference node,
   * get it's last child. If the last child was a Text or CDATASection node and its previous
   * siblings are neither a replaceable EntityReference or Text or CDATASection nodes, return false.
   * IF the last child was neither Text nor CDATASection nor a replaceable EntityReference Node,
   * then return true. If the last child was a Text or CDATASection node any its previous sibling
   * was not or was an EntityReference that did not contain only Text or CDATASection nodes, return
   * false. Check this recursively for EntityReference nodes.
   *
   * @param node
   ...
   * the document, raising a DOMException with the code NO_MODIFICATION_ALLOWED_ERR. Traverse
   * previous siblings of the node to be replaced. If a previous sibling is an EntityReference node,
   * get it's last child. If the first child was a Text or CDATASection node and its next siblings
   * are neither a replaceable EntityReference or Text or CDATASection nodes, return false. IF the
   * first child was neither Text nor CDATASection nor a replaceable EntityReference Node, then
   * return true. If the first child was a Text or CDATASection node any its next sibling was not or
   * was an EntityReference that did not contain only Text or CDATASection nodes, return false.
   * Check this recursively for EntityReference nodes.
   *
   * @param node
   * @return true - can replace text false - can't replace exception must be raised
   */
