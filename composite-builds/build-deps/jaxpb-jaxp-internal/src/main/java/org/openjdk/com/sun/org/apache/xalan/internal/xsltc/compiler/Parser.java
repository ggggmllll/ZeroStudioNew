...
public class Parser implements Constants, ContentHandler {
  ...
  public QName getQNameSafe(final String stringRep) {
    ...
    if (colon != -1) {
      ...
      final String uri =
          stringRep.equals(XMLNS_PREFIX) ? null : _symbolTable.lookupNamespace(EMPTYSTRING);
  ...
  public QName getQName(String namespace, String prefix, String localname) {
    if (namespace == null || namespace.equals(EMPTYSTRING)) {
      ...
      String lexicalQName =
          (prefix == null || prefix.length() == 0) ? localname : (prefix + ':' + localname);
  ...
  /**
   * Create an instance of the <code>Stylesheet</code> class, and then parse, typecheck and compile
   * the instance. Must be called after <code>parse()</code>.
   */
  ...
  public SyntaxTreeNode parse(InputSource input) {
    try {
      ...
      try {
        parser.setProperty(
            XMLConstants.ACCESS_EXTERNAL_DTD, _xsltc.getProperty(XMLConstants.ACCESS_EXTERNAL_DTD));
      } catch (SAXNotRecognizedException e) {
        ErrorMsg err =
            new ErrorMsg(ErrorMsg.WARNING_MSG, parser.getClass().getName() + ": " + e.getMessage());
  ...
  /**
   * Extracts the DOM for the stylesheet. In the case of an embedded stylesheet, it extracts the DOM
   * subtree corresponding to the embedded stylesheet that has an 'id' attribute whose value is the
   * same as the value declared in the <?xml-stylesheet...?> processing instruction (P.I.). In the
   * xml-stylesheet P.I. the value is labeled as the 'href' data of the P.I. The extracted DOM
        ...
        // Check if this is an extension of some other XSLT processor
          ...
          if ((sheet != null) && (sheet.isExtension(uri))) {
            if (sheet != (SyntaxTreeNode) _parentStack.peek()) {
              ...
              ErrorMsg msg =
                  new ErrorMsg(ErrorMsg.UNSUPPORTED_EXT_ERR, getLineNumber(), prefix + ":" + local);
  ...
  /**
   * SAX2: Begin the scope of a prefix-URI Namespace mapping. This has to be passed on to the symbol
   * table!
   */
  ...
  /**
   * SAX2: Receive notification of the beginning of an element. The parser may re-use the attribute
   * list that we're passed so we clone the attributes in our own Attributes implementation
   */
