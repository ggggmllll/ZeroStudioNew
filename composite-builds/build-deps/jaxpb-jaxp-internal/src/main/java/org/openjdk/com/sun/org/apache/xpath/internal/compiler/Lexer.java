...
class Lexer {
  ...
  void tokenize(String pat, Vector targetStrings)
    ...
    m_compiler.m_opMap =
        new OpMapVector(
            OpMap.MAXTOKENQUEUESIZE * 5, OpMap.BLOCKTOKENQUEUESIZE * 5, OpMap.MAPINDEX_LENGTH);
  ...
  private int mapNSTokens(String pat, int startSubstring, int posOfNSSep, int posOfScan)
    ...
    if ((null != uName) && (uName.length() > 0)) {
       ...
       * "+prefix); // m_processor.error(XPATHErrorResources.ER_PREFIX_MUST_RESOLVE, // new String[]
       * {prefix}); //"Prefix must resolve to a namespace: {0}";
       */
