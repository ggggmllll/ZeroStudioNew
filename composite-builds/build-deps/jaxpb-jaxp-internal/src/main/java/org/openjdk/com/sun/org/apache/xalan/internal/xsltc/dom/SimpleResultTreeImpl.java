...
public class SimpleResultTreeImpl extends EmptySerializer implements DOM, DTM {
  ...
  /**
   * The SimpleIterator is designed to support the nodeset() extension function. It has a traversal
   * direction parameter. The DOWN direction is used for child and descendant axes, while the UP
   * direction is used for parent and ancestor axes.
   *
  ...
  /**
   * We only need to override the endDocument, characters, and setEscaping interfaces. A simple RTF
   * does not have element nodes. We do not need to touch startElement and endElement.
   */
  ...
  /**
   * The DTM interfaces are not used in this class. Implementing the DTM interface is a requirement
   * from MultiDOM. If we have a better way of handling multiple documents, we can get rid of the
   * DTM dependency.
   *
