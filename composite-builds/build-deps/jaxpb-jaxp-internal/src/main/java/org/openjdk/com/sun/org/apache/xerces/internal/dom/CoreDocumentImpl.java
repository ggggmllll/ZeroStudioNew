...
public class CoreDocumentImpl extends ParentNode implements Document {
  ...
  /**
   * Number of alterations made to this document since its creation. Serves as a "dirty bit" so that
   * live objects such as NodeList can recognize when an alteration has been made and discard its
   ...
   * in the DOM's design.)
   *
   * <p>Note that changes which do not affect the tree's structure -- changing the node's name, for
   * example -- do _not_ have to call changed().
   *
   * <p>Alternative implementation would be to use a cryptographic Digest value rather than a count.
   * This would have the advantage that "harmless" changes (those producing equal() trees) would not
   * force NodeList to resynchronize. Disadvantage is that it's slightly more prone to "false
   * negatives", though that's the difference between "wildly unlikely" and "absurdly unlikely". IF
   ...
   * cost of having an int on every (parent) node plus having to walk up the tree all the way to the
   * root to mark the branch as changed everytime a node is changed. So we now have a single counter
   * global to the document. It means that some objects may flush their cache more often than
   * necessary, but this makes nodes smaller and only the document needs to be marked as changed.
   */
  ...
  static {
    ...
    kidOK[DOCUMENT_TYPE_NODE] =
        kidOK[PROCESSING_INSTRUCTION_NODE] =
            kidOK[COMMENT_NODE] =
                kidOK[TEXT_NODE] = kidOK[CDATA_SECTION_NODE] = kidOK[NOTATION_NODE] = 0;
  } // static
  ...
  /**
   * Since a Document may contain at most one top-level Element child, and at most one DocumentType
   * declaraction, we need to subclass our add-children methods to implement this constraint. Since
   * appendChild() is implemented as insertBefore(,null), altering the latter fixes both.
   *
  ...
  /**
   * Since insertBefore caches the docElement (and, currently, docType), removeChild has to know how
   * to undo the cache
   *
   * <p>REVISIT: According to the spec it is not allowed to alter neither the document element nor
   * the document type in any way
  ...
  /**
   * Sets whether the DOM implementation performs error checking upon operations. Turning off error
   * checking only affects the following DOM checks:
   *
   * <ul>
   ...
   *
   * <p>An attribute specifying the encoding used for this document at the time of the parsing. This
  ...
  /**
   * DOM Level 3 WD - Experimental. The location of the document or <code>null</code> if undefined.
   * <br>
  ...
  /**
   * Returns the absolute base URI of this node or null if the implementation wasn't able to obtain
   * an absolute URI. Note: If the URI is malformed, a null is returned.
   *
   ...
   * Setting the value of this attribute might throw NOT_SUPPORTED_ERR if the implementation doesn't
   * support the mode the attribute is being set to. Should the DOM spec define the default value of
   * this property? What if implementing both async and sync IO is impractical in some systems?
   * 2001-09-14. default is <code>false</code> but we need to check with Mozilla and IE.
   */
   ...
   * DOM Level 3 WD - Experimental.
   *
   * <p>Replaces the content of the document with the result of parsing the given URI. Invoking this
   ...
   * </code> , <code>"normalize-characters"</code>, <code>"check-character-normalization"</code> are
   ...
   *     <code>load</code> returns <code>true</code> if the document was successfully loaded and
   *     parsed. If an error occurred when either loading or parsing the URI, <code>load</code>
  ...
  /**
   * DOM Level 3 WD - Experimental. Save the document or the given node and all its descendants to a
   * string (i.e. serialize the document or node). <br>
   ...
   * @exception DOMException WRONG_DOCUMENT_ERR: Raised if the node passed in as the node parameter
   *     is from an other document.
   */
   ...
   * an element is being imported, a check is done for an associated identifier. If one exists, the
   ...
   * ID. Attributes with the name "ID" are not of type ID unless so defined. Implementations that do
   ...
   *
   * <p>Creates an element of the given qualified name and namespace URI. If the given namespaceURI
   * is null or an empty string and the qualifiedName has a prefix that is "xml", the created
   * element is bound to the predefined namespace "http://www.w3.org/XML/1998/namespace"
   * [Namespaces].
   *
   ...
   *     "http://www.w3.org/XML/1998/namespace", or if the qualifiedName has a prefix different from
   *     "xml" and the namespaceURI is null or an empty string.
   ...
   * @return Element A new Element object with the following attributes:
   * @exception DOMException INVALID_CHARACTER_ERR: Raised if the specified name contains an invalid
   *     character.
   */
  ...
  public static final boolean isValidQName(String prefix, String local, boolean xml11Version) {
    ...
    if (!xml11Version) {
      validNCName =
          (prefix == null || XMLChar.isValidNCName(prefix)) && XMLChar.isValidNCName(local);
    } else {
      validNCName =
  ...
  /**
   * Puts the given NodeListCache in the free list. Note: The owner node can keep using it until we
   * reuse it
   */
   ...
   * reference to them that prevents them from being gc'ed until the document is. At the same time,
   * doing it here has the advantage of avoiding a finalize() method on Node, which would affect all
  ...
  /**
   * We could have more xml versions in future , but for now we could do with this to handle XML 1.0
   * and 1.1
   */
   ...
   * place where we could use weak references! Indeed, the node here won't be GC'ed as long as some
   * user data is attached to it, since the userData table will have a reference to the node.
   */
  ...
  /**
   * A method to be called when some text was deleted from a text node, so that live objects can be
   * notified.
   */
  void deletedText(NodeImpl node, int offset, int count) {}

  /**
   * A method to be called when some text was inserted into a text node, so that live objects can be
   * notified.
   */
