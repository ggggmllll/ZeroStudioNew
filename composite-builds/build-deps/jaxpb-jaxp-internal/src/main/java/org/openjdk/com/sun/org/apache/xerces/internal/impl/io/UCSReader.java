...
public class UCSReader extends Reader {
  ...
  /**
   * Constructs an ASCII reader from the specified input stream and buffer size. The Endian-ness and
   * whether this is UCS-2 or UCS-4 needs also to be known in advance.
   *
   * @param inputStream The input stream.
  ...
  public int read(char ch[], int offset, int length) throws IOException {
    ...
    for (int i = 0; i < numChars; i++) {
      ...
      if (fEncoding >= 4) {
        ...
        if (fEncoding == UCS4BE) ch[offset + i] = (char) ((b0 << 24) + (b1 << 16) + (b2 << 8) + b3);
