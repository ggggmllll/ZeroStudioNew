...
public final class XSModelImpl extends AbstractList implements XSModel, XSNamespaceItemList {
   ...
   * one for each namespace name which appears as the target namespace of any schema component in
   * the schema used for that assessment, and one for absent if any schema component in the schema
   ...
   *     <code>XSTypeDefinition.SIMPLE_TYPE</code> and <code>XSTypeDefinition.COMPLEX_TYPE</code>
   *     can also be used as the <code>objectType</code> to retrieve only complex types or simple
  ...
  /**
   * Convenience method. Returns a list of top-level component declarations that are defined within
   * the specified namespace, i.e. element declarations, attribute declarations, etc.
   *
  ...
  public synchronized XSObjectList getAnnotations() {
    ...
    for (int i = 0; i < fGrammarCount; i++) {
      ...
      if (currGrammar.fNumAnnotations > 0) {
        System.arraycopy(
            currGrammar.fAnnotations, 0, annotations, currPos, currGrammar.fNumAnnotations);
  ...
  /**
   * REVISIT: to expose identity constraints from XSModel. For now, we only expose whether there are
   * any IDCs. We also need to add these methods to the public XSModel interface.
   */
   ...
   *     XSElementDeclaration</code> or an empty <code>XSObjectList</code> if the substitution group
   *     contains no members.
   */
