...
public class DocumentBuilderImpl extends DocumentBuilder implements JAXPConstants {
  ...
  DocumentBuilderImpl(
    ...
    if (dbf.isValidating()) {
      fInitErrorHandler =
          new DefaultValidationErrorHandler(domParser.getXMLParserConfiguration().getLocale());
    ...
    if (secureProcessing) {
      /**
       * If secure processing is explicitly set on the factory, the access properties will be set
       * unless the corresponding System Properties or jaxp.properties are set
       */
      if (features != null) {
        ...
        if (temp != null) {
          ...
          if (value && Constants.IS_JDK8_OR_ABOVE) {
            fSecurityPropertyMgr.setValue(
                Property.ACCESS_EXTERNAL_DTD, State.FSP, Constants.EXTERNAL_ACCESS_DEFAULT_FSP);
            fSecurityPropertyMgr.setValue(
                Property.ACCESS_EXTERNAL_SCHEMA, State.FSP, Constants.EXTERNAL_ACCESS_DEFAULT_FSP);
          }
        }
      }
    }
