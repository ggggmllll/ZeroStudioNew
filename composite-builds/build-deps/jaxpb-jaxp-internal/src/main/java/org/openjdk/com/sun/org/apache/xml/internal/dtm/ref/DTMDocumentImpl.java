...
public class DTMDocumentImpl
  ...
  /**
   * If we're building the model incrementally on demand, we need to be able to tell the source when
   ...
   *     they came from as well as a location within the document. It is the DTMManager's
   ...
   * built, we will ask this object to send us more events, and it will manage interactions with its
   * data sources.
   *
   * <p>Note that we do not actually build the IncrementalSAXSource, since we don't know what source
   * it's reading from, what thread that source will run in, or when it will run.
   *
  ...
  /**
   * Set a reference pointer to the prefix name symbol table. %REVIEW% Should this really be Public?
   * Changing it while DTM is in use would be a disaster.
   *
   * @param poolRef DTMStringPool reference to an instance of table.
   */
  ...
  //      /**
  //       * Factory method; creates an Element node not associated with a given name space in this
  // document.
  //       *
  //       * The node created will be chained according to its natural order of request
  //       * received.  %TBD% It can be rechained later via the optional DTM writable interface.
  ...
  // interface.
  //              // The raw string values are stored into the current DTM name symbol tables.  The
  // method
  ...
  //              // create a Text Node
  //              // %TBD% may be possible to combine with appendNode()to replace the next chunk of
  // code
  //              int w0 = TEXT_NODE;
  //              // W1: Parent
  //              int w1 = currentParent;
  ...
  /**
   * Given a node handle, advance to its next descendant. If not yet resolved, waits for more nodes
  ...
  /**
   * Given a node handle, find the owning document node. This has the DTM semantics; a Document node
   * is its own owner.
   *
   * <p>%REVIEW% Since this is DOM-specific, it may belong at the DOM binding layer. Included here
   * as a convenience function and to aid porting of DOM code to DTM.
   ...
   * not be the same as the number of SAX characters() events that caused the text node to be built
   * in the first place, since m_chars buffering may be on different boundaries than the parser's
   * buffers.
   *
   * @param nodeHandle The node ID.
   ...
   * unit. PLEASE NOTE that this may not be the same granularityas the SAX characters() events that
   * caused the text node to be built in the first place, since m_chars buffering may be on
   ...
   *     specified, supporting any version of the feature will cause the method to return <code>true
   *     </code>.
   ...
   * the DTM document.
   *
   * <p>%REVEIW% Presumes a 1:1 mapping from DTM to Document, since we aren't saying which Document
   * to query...?
   */
  ...
  /**
   * Append a text child at the current insertion point. Assumes that the actual content of the text
   * has previously been appended to the m_char buffer (shared with the builder).
   *
   ...
   *     namespace declaration to also be declared as being a node ID. I don't really want to
  ...
  /**
   * This is a shortcut to the iterators that implement the supported XPath axes (only namespace::)
   * is not supported. Returns a bare-bones iterator that must be initialized with a start node
  ...
  /**
   * Terminate the element currently acting as an insertion point. Subsequent insertions will occur
   * as the last child of this element's parent.
   */
