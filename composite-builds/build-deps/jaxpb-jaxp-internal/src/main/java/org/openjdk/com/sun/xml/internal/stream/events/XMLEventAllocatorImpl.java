...
public class XMLEventAllocatorImpl implements XMLEventAllocator {
  ...
  public org.openjdk.javax.xml.stream.events.XMLEvent allocate(
    ...
    //        allocate is not supposed to change the state of the reader so we shouldn't be calling
    // next.
      ...
      case XMLEvent.START_ELEMENT:
        {
          ...
          // we might have different XMLStreamReader so check every time for the namespace aware
          // property
      ...
      case XMLEvent.PROCESSING_INSTRUCTION:
        {
          ProcessingInstructionEvent piEvent =
              new ProcessingInstructionEvent(streamReader.getPITarget(), streamReader.getPIData());
  ...
  protected void fillAttributes(StartElementEvent event, XMLStreamReader xmlr) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      // so we can be sure that there wont be any namespace declaration as part of this function
      // call
