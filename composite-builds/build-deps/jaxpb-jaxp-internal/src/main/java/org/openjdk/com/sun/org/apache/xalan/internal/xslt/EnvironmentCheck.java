...
public class EnvironmentCheck {
   ...
   * properties and logs them out; it will report a problem if it finds a setting or .jar file that
   * is <i>likely</i> to cause problems.
   *
   * <p>Advanced users can peruse the code herein to help them investigate potential environment
   * problems found; other users may simply send the output from this tool along with any bugs they
   * submit to help us in the debugging process.
   *
   ...
   * description of the current state of that item as the value. Any serious problems will be put in
   * with a key that is prefixed with {@link #ERROR 'ERROR.'} so it stands out in any resulting
   ...
   *
   * <p>This dumps a simple header and then each of the entries in the Hashtable to our PrintWriter;
   * it does special processing for entries that are .jars found in the classpath.
   *
   ...
   *
   * <p>Copy of writeEnvironmentReport that creates a Node suitable for other processing instead of
   * a properties-like text output.
   *
   ...
   *
   * <p>Currently does a lookup into a local table of some recent shipped Xalan builds to determine
   * where the .jar probably came from. Note that if you recompile Xalan or Xerces yourself this
   * will likely report a potential error, since we can't certify builds other than the ones we
   * ship. Only reports against selected posted Xalan-J builds.
   *
  ...
  protected void checkProcessorVersion(Hashtable h) {
    ...
    try {
      ...
      final String XALAN2_2_VERSION_CLASS = "org.openjdk.com.sun.org.apache.xalan.internal.Version";
   ...
   * key=jarsize, value=jarname ' from ' distro name Note assumption: two jars cannot have the same
   * size!
   *
   * @see #getApparentVersion(String, long)
   */
  ...
  static {
    ...
    jarVersions.put(
        new Long(109049), "xml-apis.jar from xalan-j_2_4_0 from xml-commons RIVERCOURT1 branch");
    jarVersions.put(
        new Long(113749),
    ...
    jarVersions.put(
        new Long(1730053), "xercesImpl.jar from xalan-j_2_3_0 or xalan-j_2_3_1 from xerces-2_0_0");
