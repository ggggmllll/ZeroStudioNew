...
public class XIncludeHandler implements XMLComponent, XMLDocumentFilter, XMLDTDFilter {
  ...
  public static final QName NEW_NS_ATTR_QNAME =
      new QName(
          XMLSymbols.PREFIX_XMLNS, "", XMLSymbols.PREFIX_XMLNS + ":", NamespaceContext.XMLNS_URI);
  ...
  public void endElement(QName element, Augmentations augs) throws XNIException {
    ...
    if (isFallbackElement(element)) {
      ...
      // element
      // now that we're done processing it, we should ignore all the other children of the include
      // element
      if (getState() == STATE_NORMAL_PROCESSING) {
        setState(STATE_IGNORE);
      }
  ...
  protected boolean handleIncludeElement(XMLAttributes attributes) throws XNIException {
    ...
    if (includedSource == null) {
      ...
      if (accept != null || acceptLanguage != null) {
        includedSource =
            createInputSource(
                null, href, fCurrentBaseURI.getExpandedSystemId(), accept, acceptLanguage);
      } else {
    ...
    if (parse.equals(XINCLUDE_PARSE_XML)) {
      ...
      if ((xpointer != null && fXPointerChildConfig == null)
        ...
        // use the same symbol table, error reporter, entity resolver, security manager and buffer
        // size.
      ...
      // encoding errors
        ...
        fErrorReporter.reportError(
            ex.getDomain(), ex.getKey(), ex.getArguments(), XMLErrorReporter.SEVERITY_FATAL_ERROR);
  ...
  /**
   * Returns true if the current [base URI] is the same as the [base URI] that was in effect on the
   * include parent. This method should <em>only</em> be called when the current element is a top
   * level included element, i.e. the direct child of a fallback element, or the root elements in an
   * included document. The "include parent" is the element which, in the result infoset, will be
   * the direct parent of the current element.
   *
   ...
   * "include parent" is the element which, in the result infoset, will be the direct parent of the
   * current element.
   *
  ...
  protected boolean searchForRecursiveIncludes(XMLLocator includedSource) {
    ...
    if (includedSystemId == null) {
      try {
        includedSystemId =
            XMLEntityManager.expandSystemId(
                includedSource.getLiteralSystemId(), includedSource.getBaseSystemId(), false);
   ...
   *
   * <ul>
   *   <li>If the element is a top level included item whose [base URI] is different from the [base
   *       URI] of the include parent, then an xml:base attribute is added to specify the true [base
   *       URI]
  ...
  protected XMLAttributes processAttributes(XMLAttributes attributes) {
    if (isTopLevelIncludedItem()) {
      ...
      if (fFixupLanguage && !sameLanguageAsIncludeParent()) {
        ...
        int index =
            attributes.addAttribute(XML_LANG_QNAME, XMLSymbols.fCDATASymbol, fCurrentLanguage);
        attributes.setSpecified(index, true);
      }
      ...
      while (inscopeNS.hasMoreElements()) {
        ...
        if (parentURI != uri && attributes != null) {
          if (prefix == XMLSymbols.EMPTY_STRING) {
            if (attributes.getValue(NamespaceContext.XMLNS_URI, XMLSymbols.PREFIX_XMLNS) == null) {
              ...
              int index =
                  attributes.addAttribute(
                      ns, XMLSymbols.fCDATASymbol, uri != null ? uri : XMLSymbols.EMPTY_STRING);
              attributes.setSpecified(index, true);
            ...
            ns.rawname += prefix;
            ns.rawname =
                (fSymbolTable != null) ? fSymbolTable.addSymbol(ns.rawname) : ns.rawname.intern();
            int index =
                attributes.addAttribute(
                    ns, XMLSymbols.fCDATASymbol, uri != null ? uri : XMLSymbols.EMPTY_STRING);
            attributes.setSpecified(index, true);
  ...
  protected String getRelativeBaseURI() throws MalformedURIException {
    ...
    if (isRootDocument()) {
      ...
      if (includeParentDepth == 0) {
        ...
        /**
         * The scheme and authority components are equal, return the path and the possible query
         * and/or fragment which follow.
         */
   ...
   * element is a top level included item. Returning 0 indicates that the top level element in this
   * document was an include element.
   *
  ...
  /**
   * Modify the augmentations. Add an [included] infoset item, if the current element is a top level
   * included item.
   *
  ...
  /**
   * Modify the augmentations. Add an [included] infoset item, if <code>force</code> is true, or if
   * the current element is a top level included item.
   *
   * @param augs the Augmentations to modify.
  ...
  /**
   * Records that an &lt;fallback&gt; was encountered at the specified depth, as an ancestor of the
   * current element, or as a sibling of an ancestor of the current element.
   *
   * @param depth
   * @param val
   */
  ...
  /**
   * Return whether an &lt;include&gt; was encountered at the specified depth, as an ancestor of the
   * current item.
   *
   * @param depth
   * @return
  ...
  /**
   * Checks if an UnparsedEntity with the given name was declared in the DTD of the document for the
   * current pipeline. If so, then the notation for the UnparsedEntity is checked. If that turns out
   * okay, then the UnparsedEntity is passed to the root pipeline to be checked for conflicts, and
   * sent to the root DTDHandler.
   *
  ...
  protected static class UnparsedEntity {
    ...
    public boolean equals(Object obj) {
      return obj == this
          || obj instanceof UnparsedEntity && Objects.equals(name, ((UnparsedEntity) obj).name);
    }
  ...
  private XMLInputSource createInputSource(
      String publicId, String systemId, String baseSystemId, String accept, String acceptLanguage) {
