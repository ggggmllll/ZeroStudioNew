...
class VariableBase extends TopLevelElement {
  ...
  /**
   * Add a reference to this variable. Called by VariableRef when an expression contains a reference
   * to this variable.
   */
  ...
  /**
   * When a variable is overriden by another, e.g. via xsl:import, its references need to be copied
   * or otherwise it may be compiled away as dead code. This method can be used for that purpose.
   */
  ...
  /**
   * Compile the value of the variable, which is either in an expression in a 'select' attribute, or
   * in the variable elements body
   */
  public void translateValue(ClassGenerator classGen, MethodGenerator methodGen) {
    ...
    if (_select != null) {
      ...
      if (_select.getType() instanceof NodeSetType) {
        ...
        final int initCNI =
            cpg.addMethodref(
                CACHED_NODE_LIST_ITERATOR_CLASS, "<init>", "(" + NODE_ITERATOR_SIG + ")V");
