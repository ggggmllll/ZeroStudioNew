...
public class SubstitutionGroupHandler {
  ...
  protected boolean substitutionGroupOK(
    ...
    // extension, restriction}, the value of a {disallowed substitutions}) one of the following must
    // be true:
    ...
    // 2.2 There is a chain of {substitution group affiliation}s from D to C, that is, either D's
    // {substitution group affiliation} is C, or D's {substitution group affiliation}'s
    ...
    // from C's {type definition} does not intersect with the union of the blocking constraint, C's
    // {prohibited substitutions} (if C is complex, otherwise the empty set) and the {prohibited
    // substitutions} (respectively the empty set) of any intermediate {type definition}s in the
      ...
      if (base.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE) {
        ...
        if (st.getVariety() == XSSimpleTypeDefinition.VARIETY_UNION) {
          ...
          for (int i = 0; i < length; ++i) {
            if (typeDerivationOK(
                derived, (XSTypeDefinition) memberTypes.item(i), blockingConstraint)) {
              return true;
            }
          }
        }
  ...
  public boolean inSubstitutionGroup(XSElementDecl element, XSElementDecl exemplar) {
    // [Definition:]  Every element declaration (call this HEAD) in the {element declarations} of a
    // schema defines a substitution group, a subset of those {element declarations}, as follows:
    ...
    // declaration in the {element declarations} has a {substitution group affiliation} in PSG, then
    // it is also in PSG itself.
    // HEAD's actual substitution group is then the set consisting of each member of PSG such that
    // all of the following must be true:
    // 1 Its {abstract} is false.
    // 2 It is validly substitutable for HEAD subject to an empty blocking constraint, as defined in
   ...
   *
   * <p>from the spec, substitution group of a given element decl also contains the element itself.
   * but the array returned from this method doesn't containt this element.
   */
