...
public abstract class ToStream extends SerializerBase {
  ...
  /**
   * Method reference to the sun.io.CharToByteConverter#canConvert method for this encoding. Invalid
   * if m_charToByteConverter is null.
   */
   ...
   *
   * <p>Used only in shouldIndent() but only if m_doIndent is true. If m_doIndent is false this flag
   * has no impact.
   */
   ...
   * whitespace is removed,and will include the enclosing parentheses.
   *
  ...
  private synchronized void init(
    ...
    setIndentAmount(
        OutputPropertyUtils.getIntProperty(OutputPropertiesFactory.S_KEY_INDENT_AMOUNT, format));
  ...
  /**
   * Set if the operating systems end-of-line line separator should be used when serializing. If set
   * false NL character (decimal 10) is left alone, otherwise the new-line will be replaced on
   * output with the systems line separator. For example on UNIX this is NL, while on Windows it is
   * two characters, CR NL, where CR is the carriage-return (decimal 13).
   *
  ...
  public void setOutputStream(OutputStream output) {
    ...
    try {
      Properties format;
      if (null == m_format) format = OutputPropertiesFactory.getDefaultMethodProperties(Method.XML);
  ...
  /**
   * Once a surrogate has been detected, write out the pair of characters if it is in the encoding,
   * or if there is no encoding, otherwise write out an entity reference of the value of the unicode
   * code point of the character represented by the high/low surrogate pair.
   *
   * <p>An exception is thrown if there is no low surrogate in the pair, because the array ends
   * unexpectely, or if the low char is there but its value is such that it is not a low surrogate.
   *
    ...
    if (i + 1 >= end) {
      throw new IOException(
          org.openjdk.com.sun.org.apache.xml.internal.serializer.utils.Utils.messages.createMessage(
              MsgKey.ER_INVALID_UTF16_SURROGATE, new Object[] {Integer.toHexString((int) c)}));
    }
      ...
      for (; i < end; i++) {
        {
          ...
          // to optimize.
          char ch2;
          while (i < end && ((ch2 = chars[i]) < 127) && m_charInfo.isTextASCIIClean(ch2)) i++;
          if (i == end) break;
        }
  ...
  /**
   * This method checks if a given character either NEL (0x85) or LSEP (0x2028) These are new end of
   * line charcters added in XML 1.1. These characters must be written as Numeric Character
  ...
  protected int accumDefaultEscape(
      Writer writer, char ch, int i, char[] chars, int len, boolean fromTextNode, boolean escLF)
      throws IOException {
        ...
        if (i + 1 >= len) {
          throw new IOException(
              org.openjdk.com.sun.org.apache.xml.internal.serializer.utils.Utils.messages
                  .createMessage(
                      MsgKey.ER_INVALID_UTF16_SURROGATE, new Object[] {Integer.toHexString(ch)}));
   ...
   * namespace or attribute information can occur before or after this call, that is associated with
   * this element.
   *
   ...
   * Need to close any open start tag to make sure than any name space attributes due to this event
   * are associated wih the up comming element, not the current one.
   *
  ...
  /**
   * Receive notification of an XML comment anywhere in the document. This callback will be used for
   * comments inside or outside the document element, including comments in the external DTD subset
   * (if read).
   *
   ...
   *
   * <p>The start and end of the document entity are not reported. The start and end of the external
   * DTD subset are reported using the pseudo-name "[dtd]". All other events must be properly nested
  ...
  /**
   * Tell if, based on space preservation constraints and the doIndent property, if an indent should
   * occur.
   *
  ...
  /**
   * Remembers the cdata sections specified in the cdata-section-elements. The "official way to set
   * URI and localName pairs. This method should be used by both Xalan and XSLTC.
   *
    ...
    if (ns != null && ns.length() > 0) {
      ...
      if (index > 0) {
        ...
        if (prefix == null) {
          ...
          this.addAttribute(
              "http://www.w3.org/2000/xmlns/", prefix, "xmlns:" + prefix, "CDATA", ns, false);
        }
  ...
  /**
   * This method flushes any pending events, which can be startDocument() closing the opening tag of
   * an element, or closing an open CDATA section.
   */
   ...
   * This is useful if a SAX startPrefixMapping() should need to add an attribute before the element
   * name is seen.
   *
  ...
  /**
   * To fire off the pseudo characters of attributes, as they currently exist. This method should be
   * called everytime an attribute is added, or when an attribute value is changed, or an element is
   * created.
   */
  ...
  public void setEncoding(String encoding) {
    ...
    if (old == null || !old.equals(encoding)) {
      ...
      if (encoding != null && m_encodingInfo.name == null) {
        ...
        String msg =
            Utils.messages.createMessage(MsgKey.ER_ENCODING_NOT_SUPPORTED, new Object[] {encoding});
        try {
  ...
  /**
   * If set to false the serializer does not expand DTD entities, but leaves them as is, the default
   * value is true;
   */
