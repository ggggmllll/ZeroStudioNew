...
public final class SAXImpl extends SAX2DTM2 implements DOMEnhancedForDTM, DOMBuilder {
  ...
  /**
   * Lookup a namespace URI from a prefix starting at node. This method is used in the execution of
   * xsl:element when the prefix is not known at compile time.
   */
  ...
  /**
   * Create an org.w3c.dom.Node from a node in an iterator The iterator most be started before this
   * method is called
   */
   ...
   * It cannot be made to point to any other node through setStartNode().
   */
  ...
  /**
   * Do not think that this returns an iterator for the namespace axis. It returns an iterator with
   * nodes that belong in a certain namespace, such as with <xsl:apply-templates
   * select="blob/foo:*"/> The 'axis' specifies the axis for the base iterator from which the nodes
   * are taken, while 'ns' specifies the namespace URI type.
   */
  ...
  /**
   * Iterator that handles node tests that test for a namespace, but have a wild card for the local
   * name of the node, i.e., node tests of the form <axis>::<prefix>:*
   */
   ...
   *
   * <p>TODO: Copy namespace declarations. Can't be done until we add namespace nodes and keep track
   * of NS prefixes TODO: Copy comment nodes
   */
    ...
    if (rtfType == DOM.SIMPLE_RTF) {
      if (addToManager) {
        ...
        SimpleResultTreeImpl rtf =
            new SimpleResultTreeImpl(_dtmManager, dtmPos << DTMManager.IDENT_DTM_NODE_BITS);
      ...
      return (DOM)
          _dtmManager.getDTM(null, true, m_wsfilter, true, false, false, initSize, m_buildIdIndex);
    }
  }
