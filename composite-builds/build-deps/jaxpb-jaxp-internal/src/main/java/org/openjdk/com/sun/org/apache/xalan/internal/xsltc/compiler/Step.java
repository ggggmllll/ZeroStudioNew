...
final class Step extends RelativeLocationPath {
   ...
   * be removed from the above vector as they are handled. We use this boolean to remember if we did
   * have any predicates.
   */
  ...
  /**
   * Returns 'true' if this step has a parent pattern. This method will return 'false' if this step
   * occurs on its own under an element like <xsl:for-each> or <xsl:apply-templates>.
   */
  ...
  /**
   * Type check this step. The abbreviated steps '.' and '@attr' are assigned type node if they have
   * no predicates. All other steps have type node-set.
   */
   ...
   * and '@attr' do not create new iterators if they are not part of a LocationPath and have no
   * filters. In these cases a node index instead of an iterator is pushed onto the stack.
   */
  ...
  private void translateStep(
    ...
    if (predicateIndex >= 0) {
      ...
      if (_axis == Axis.ATTRIBUTE
          ...
          && star == 0) {
        int iter =
            cpg.addInterfaceMethodref(DOM_INTF, "getTypedAxisIterator", "(II)" + NODE_ITERATOR_SIG);
          ...
          if (star > 1) {
            ...
            final int ns =
                cpg.addInterfaceMethodref(
                    DOM_INTF, "getNamespaceAxisIterator", "(II)" + NODE_ITERATOR_SIG);
            il.append(methodGen.loadDOM());
            il.append(new PUSH(cpg, _axis));
  ...
  /**
   * Translate a sequence of predicates. Each predicate is translated by constructing an instance of
   * <code>CurrentNodeListIterator</code> which is initialized from another iterator (recursive
    ...
    if (predicateIndex < 0) {
      ...
      // Handle 'elem[n]' expression
        ...
        LocalVariableGen filterTemp =
            methodGen.addLocalVariable(
                "step_tmp2", Util.getJCRefType(CURRENT_NODE_LIST_FILTER_SIG), null, null);
