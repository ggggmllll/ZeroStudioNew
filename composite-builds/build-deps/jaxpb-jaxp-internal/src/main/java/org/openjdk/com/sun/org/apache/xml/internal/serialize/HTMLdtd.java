...
public final class HTMLdtd {
  ...
  /**
   * Returns true if element is declared to be empty. HTML elements are defines as empty in the DTD,
   * not by the document syntax.
   *
  ...
  /**
   * Returns true if element's closing tag is generally not printed. For example, <tt>LI</tt> should
   * not print the closing tag.
   *
   * @param tagName The element tag name (upper case)
  ...
  /**
   * Returns the value of an HTML character reference by its name. If the reference is not found or
   * was not defined as a character reference, returns EOF (-1).
   *
   ...
   * accessible by name or character value and is optimized for character substitution. This method
   * may be called any number of times but will execute only once.
   */
