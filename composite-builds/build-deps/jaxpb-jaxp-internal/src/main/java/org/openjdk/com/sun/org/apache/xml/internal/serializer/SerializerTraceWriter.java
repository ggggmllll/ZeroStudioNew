...
final class SerializerTraceWriter extends Writer implements WriterChain {
  ...
  /**
   * Constructor. If the writer passed in is null, then this SerializerTraceWriter will only signal
   * trace events of what would have been written to that writer. If the writer passed in is not
   * null then the trace events will mirror what is going to that writer. In this way tools, such as
  ...
  /**
   * Flush out the collected characters by sending them to the trace listener. These characters are
   * never written to the real writer (m_writer) because that has already happened with every method
   * call. This method simple informs the listener of what has already happened.
   *
   * @throws IOException
   */
