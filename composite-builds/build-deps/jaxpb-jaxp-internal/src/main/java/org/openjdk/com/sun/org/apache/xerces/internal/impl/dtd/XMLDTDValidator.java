...
public class XMLDTDValidator
   ...
   * from the root to the current element depth. This structure never gets "deeper" than the deepest
   * element. Space is re-used once each element is closed.
   *
   * <p><strong>Note:</strong> This is much more efficient use of memory than creating new arrays
   * for each element depth.
   *
   * <p><strong>Note:</strong> The use of this data structure is for validation "on the way out". If
   * the validation model changes to "on the way in", then this data structure is not needed.
   */
      ...
      case XMLSimpleType.TYPE_IDREF:
        {
          boolean isAlistAttribute =
              attributeDecl
                  .simpleType
                  .list; // Caveat - Save this information because invalidStandaloneAttDef
   ...
   * structurally validated, so it calls this method. The index of the element being checked (in the
   * decl pool), is provided as well as an array of element name indexes of the children. The
   * validator must confirm that this element can have these children in this order.
   *
   * <p>This can also be called to do 'what if' testing of content models just to see if they would
   * be valid.
   *
   ...
   *     the value returned is equal to the number of children, then additional content is required
   *     to reach a valid ending state.
  ...
  protected boolean handleStartElement(QName element, XMLAttributes attributes, Augmentations augs)
    ...
    if (fPerformValidation) {
      ...
      if (fElementChildrenOffsetStack.length <= fElementDepth) {
        ...
        System.arraycopy(
            fElementChildrenOffsetStack, 0, newarray, 0, fElementChildrenOffsetStack.length);
      ...
      if (elementIndex != -1 && fCurrentContentSpecType != -1) {
        ...
        if (result != -1) {
          ...
          if (fTempElementDecl.type == XMLElementDecl.TYPE_EMPTY) {
            ...
            String messageKey =
                result != childrenLength ? "MSG_CONTENT_INVALID" : "MSG_CONTENT_INCOMPLETE";
            fErrorReporter.reportError(
                XMLMessageFormatter.XML_DOMAIN,
                messageKey,
