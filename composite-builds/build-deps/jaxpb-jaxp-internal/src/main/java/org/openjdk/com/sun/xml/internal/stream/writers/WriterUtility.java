...
public class WriterUtility {
   ...
   * however, if the feature is set to false characters wont be escaped. This feature has no effect
   * when writing Attribute values, character would still be escaped. I can't think of any reason
   * why this would be useful when writing attribute values. However, this can be reconsidered if
   * there is any usecase.
   */
  ...
  private void writeXMLContent(char[] content, int start, int length, boolean escapeCharacter)
    ...
    // define startWritePos to track the position from where the character array data needs to be
    // written
    ...
    if (DEBUG_XML_CONTENT) {
      System.out.println(
          "out of the loop, writing " + new String(content, startWritePos, end - startWritePos));
    }
