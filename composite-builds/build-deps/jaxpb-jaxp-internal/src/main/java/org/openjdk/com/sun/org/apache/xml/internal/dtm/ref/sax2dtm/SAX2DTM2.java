...
public class SAX2DTM2 extends SAX2DTM {
   ...
   * encoded value into an int, using 10 bits (max. 1024) for length and 21 bits for offset. We can
   * save two SuballocatedIntVector calls for each getStringValueX() and dispatchCharacterEvents()
   * call by doing this.
   */
   ...
   *
   * <p>This one is less efficient than _exptype2. It is only used during DTM building. _exptype2 is
   * used after the document is fully built.
   */
   ...
   *
   * <p>This one is only used by DOMAdapter.getExpandedTypeID(int), which is mostly called from the
   * compiled translets.
   */
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    int prefixIndex =
        (qName.length() != localName.length()) ? m_valuesOrPrefixes.stringToIndex(qName) : 0;
    ...
    if (null != m_wsfilter) {
      ...
      boolean shouldStrip =
          (DTMWSFilter.INHERIT == wsv) ? getShouldStripWhitespace() : (DTMWSFilter.STRIP == wsv);
  ...
  public void comment(char ch[], int start, int length) throws SAXException {
    ...
    m_previous =
        addNode(DTM.COMMENT_NODE, DTM.COMMENT_NODE, m_parents.peek(), m_previous, dataIndex, false);
  }
   ...
   *
   * <p>Given a node handle, return the XPath node name. This should be the name as described by the
   ...
   * XMLString.
   *
   * <p>Get the string-value of a node as a String object (see http://www.w3.org/TR/xpath#data-model
      ...
      if (dataIndex >= 0) {
        if (m_xstrf != null)
          ...
          return new XMLStringDefault(
              m_chars.getString(dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX));
      } else {
        if (m_xstrf != null)
          return m_xstrf.newstr(
              m_chars, m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
        else
          return new XMLStringDefault(
   ...
   *
   * <p>%OPT% This is one of the most often used interfaces. Performance is critical here. This one
   * is different from SAX2DTM.getStringValue(int) in that it returns a String instead of a
   * XMLString.
   *
   * <p>Get the string- value of a node as a String object (see http: //www. w3. org/TR/xpath#data-
   * model for the definition of a node's string- value).
   *
   * @param nodeHandle The node ID.
   * @return A string object that represents the string-value of the given node.
   */
  ...
  public String getStringValue() {
    ...
    if ((_exptype2(child) == DTM.TEXT_NODE) && (_nextsib2(child) == DTM.NULL)) {
      ...
      if (dataIndex >= 0)
        return m_chars.getString(dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
      else return m_chars.getString(m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
   ...
   *
   * <p>Directly call the characters method on the passed ContentHandler for the string-value of the
