...
public final class NodeSetType extends Type {
  ...
  /**
   * Translates a node-set into an object of internal type <code>type</code>. The translation to int
   * is undefined since node-sets are always converted to reals in arithmetic expressions.
   *
  ...
  /**
   * Translates an external Java Class into an internal type. Expects the Java object on the stack,
   * pushes the internal type
   */
  ...
  /**
   * Translates a node-set into a synthesized boolean. The boolean value of a node-set is "true" if
   * non-empty and "false" otherwise. Notice that the function getFirstNode() is called in
  ...
  /**
   * Expects a node-set on the stack and pushes a boxed node-set. Node sets are already boxed so the
