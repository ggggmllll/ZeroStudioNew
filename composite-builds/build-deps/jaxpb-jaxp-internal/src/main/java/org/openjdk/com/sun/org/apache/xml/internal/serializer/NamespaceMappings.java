...
public class NamespaceMappings {
  ...
  /**
   * Each entry (prefix) in this hashmap points to a Stack of URIs This maps a prefix (String) to a
   * Stack of prefix mappings. All mappings in that retrieved stack have the same prefix, though
   * possibly different URI's or depths. Such a stack must have mappings at deeper depths push later
   * on such a stack. Mappings pushed earlier on the stack will have smaller values for
   * MappingRecord.m_declarationDepth.
   */
  private HashMap m_namespaces = new HashMap();

  /**
   * The top of this stack contains the MapRecord of the last declared a namespace. Used to know how
   * many prefix mappings to pop when leaving the current element depth. For every prefix mapping
   * the current element depth is pushed on this stack. That way all prefixes pushed at the current
   * depth can be removed at the same time. Used to ensure prefix/uri map scopes are closed
   * correctly
   */
  ...
  /**
   * Given a namespace uri, and the namespaces mappings for the current element, return the current
   * prefix for that uri.
   *
