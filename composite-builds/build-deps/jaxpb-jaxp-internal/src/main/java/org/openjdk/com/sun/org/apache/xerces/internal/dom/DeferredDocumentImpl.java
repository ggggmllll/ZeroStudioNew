...
public class DeferredDocumentImpl extends DocumentImpl implements DeferredNode {
  ...
  public int createDeferredEntity(
      String entityName, String publicId, String systemId, String notationName, String baseURI) {
    // create node
   ...
   */
  public int setDeferredAttribute(
      int elementNodeIndex, String attrName, String attrURI, String attrValue, boolean specified) {
    // create attribute
    int attrNodeIndex =
  ...
  /**
   * Returns the <i>real</i> prev sibling of the given node, directly from the data structures. Used
   * by TextImpl#getNodeValue() to normalize values.
   */
  ...
  public String getNodeValueString(int nodeIndex, boolean free) {
    ...
    int index = nodeIndex & CHUNK_MASK;
    String value =
        free ? clearChunkValue(fNodeValue, chunk, index) : getChunkValue(fNodeValue, chunk, index);
    if (value == null) {
      return null;
    }
    ...
    if (DEBUG_IDS) {
      System.out.println(
          "putIdentifier("
              ...
              + " // "
              + getChunkValue(
                  fNodeName, elementNodeIndex >> CHUNK_SHIFT, elementNodeIndex & CHUNK_MASK));
    }
   ...
   * Synchronizes the node's children with the internal structure. Fluffing the children at once
   * solves a lot of work to keep the two structures in sync. The problem gets worse when editing
   * the tree -- this makes it a lot easier. This is not directly used in this class but this method
   * is here so that it can be shared by all deferred subclasses of AttrImpl.
   */
   ...
   * solves a lot of work to keep the two structures in sync. The problem gets worse when editing
   * the tree -- this makes it a lot easier. This is not directly used in this class but this method
   * is here so that it can be shared by all deferred subclasses of ParentNode.
   */
  ...
  /**
   * Performs a binary search for a target value in an array of values. The array of values must be
   * in ascending sorted order before calling this method and all array values must be non-negative.
   *
  ...
  /**
   * Clears the specified value in the given data at the chunk and index. Note that this method will
   * clear the given chunk if the reference count becomes zero.
   *
   * @return Returns the old value.
   */
