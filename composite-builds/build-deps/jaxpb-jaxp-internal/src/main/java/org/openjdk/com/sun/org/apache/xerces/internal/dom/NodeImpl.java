...
public abstract class NodeImpl implements Node, NodeList, EventTarget, Cloneable, Serializable {
  ...
  /**
   * The two nodes are the same. Two nodes that are the same have an equivalent position, though the
  ...
  /**
   * No public constructor; only subclasses of Node should be instantiated, and those normally via a
   ...
   *     _not_ clone those children unless a "deep clone" has been requested. A shallow clone of an
   *     Attr node will yield an empty Attr of the same name.
  ...
  /**
   * Obtain the DOM-tree parent of this node, or null if it is not currently active in the DOM tree
   * (perhaps because it has just been created or removed). Note that Document, DocumentFragment,
   * and Attribute will never have parents.
   */
   ...
   * </code> nodes, i.e., there are no adjacent <code>Text</code> nodes. This can be used to ensure
   * that the DOM view of a document is the same as if it were saved and re-loaded, and is useful
   * when operations (such as XPointer lookups) that depend on a particular document tree structure
   * are to be used.In cases where the document contains <code>CDATASections</code>, the normalize
   ...
   *
   * <p>Note that this implementation simply calls normalize() on this Node's children. It is up to
   ...
   * Introduced in DOM Level 2.
   *
   * <p>The namespace URI of this node, or null if it is unspecified. When this node is of any type
   * other than ELEMENT_NODE and ATTRIBUTE_NODE, this is always null and setting it has no effect.
   *
   ...
   * When the node is a <code>Notation</code>, an <code>Entity</code>, or an <code>EntityReference
   * </code>, this attribute represents the properties [declaration base URI] in the . How will this
   * be affected by resolution of relative namespace URIs issue?It's not.Should this only be on
   * Document, Element, ProcessingInstruction, Entity, and Notation nodes, according to the infoset?
   * If not, what is it equal to on other nodes? Null? An empty string? I think it should be the
   * parent's.No.Should this be read-only and computed or and actual read-write attribute?Read-only
   * and computed (F2F 19 Jun 2000 and teleconference 30 May 2001).If the base HTML element is not
   * yet attached to a document, does the insert change the Document.baseURI? Yes. (F2F 26 Sep 2001)
   *
   * @since DOM Level 3
   */
   ...
   * the same type.The following string attributes are equal: <code>nodeName</code>, <code>localName
   * </code>, <code>namespaceURI</code>, <code>prefix</code>, <code>nodeValue</code> , <code>baseURI
   * </code>. This is: they are both <code>null</code>, or they have the same length and are
   * character for character identical. The <code>attributes</code> <code>NamedNodeMaps</code> are
   * equal. This is: they are both <code>null</code>, or they have the same length and for each node
   * that exists in one map there is a node that exists in the other map and is equal, although not
   * necessarily at the same index.The <code>childNodes</code> <code>NodeLists</code> are equal.
   * This is: they are both <code>null</code>, or they have the same length and contain equal nodes
   * at the same index. This is true for <code>Attr</code> nodes as for any other type of node. Note
   * that normalization can affect equality; to avoid this, nodes should be normalized before being
   * compared. <br>
   ...
   * @return If the nodes, and possibly subtrees are equal, <code>true</code> otherwise <code>false
   *     </code>.
   * @since DOM Level 3
   */
   ...
   * that are no longer used. Failure to do so will prevent the nodes, your data is attached to, to
   * be garbage collected until the whole document is.
   *
  ...
  /**
   * For non-child nodes, the node which "points" to this node. For example, the owning element for
   * an attribute
   */
