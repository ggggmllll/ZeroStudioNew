...
public class XMLDocumentScannerImpl extends XMLDocumentFragmentScannerImpl {
  ...
  public String[] getRecognizedProperties() {
    ...
    System.arraycopy(
        RECOGNIZED_PROPERTIES, 0, combinedPropertyIds, length, RECOGNIZED_PROPERTIES.length);
  ...
  protected boolean scanDoctypeDecl(boolean supportDTD) throws IOException, XNIException {
    ...
    if (supportDTD && !fHasExternalDTD && fExternalSubsetResolver != null) {
      fDTDDescription.setValues(
          null, null, fEntityManager.getCurrentResourceIdentifier().getExpandedSystemId(), null);
   ...
   *
   * <p>This class has been modified as per the new design which is more suited to efficiently build
   * pull parser. Lots of performance improvements have been done and the code has been added to
  ...
  protected final class PrologDriver implements Driver {
    ...
    public int next() throws IOException, XNIException {
      ...
      try {
            ...
            case SCANNER_STATE_START_OF_MARKUP:
              {
                ...
                if (fEntityScanner.skipChar('?')) {
                  ...
                  // from now onwards this would be handled by fContentDriver,in the same next()
                  // call
        ...
        switch (fScannerState) {
          /**
           * //this part is handled by FragmentContentHandler case SCANNER_STATE_ROOT_ELEMENT: {
           * //we have read '<' and beginning of reading the start element tag
           * setScannerState(SCANNER_STATE_START_ELEMENT_TAG); setDriver(fContentDriver); //from now
           * onwards this would be handled by fContentDriver,in the same next() call return
           * fContentDriver.next(); }
           */
          ...
          case SCANNER_STATE_DOCTYPE:
            {
              ...
              if (fDoctypeSystemId != null) {
                ...
                if (((fValidation || fLoadExternalDTD)
                  ...
                  fExternalSubsetSource = null;
                  if (fSupportDTD) setScannerState(SCANNER_STATE_DTD_EXTERNAL_DECLS);
              ...
              // Send endDTD() call if:
              // a) systemId is null or if an external subset resolver could not locate an external
              // subset.
  ...
  protected final class DTDDriver implements Driver {
    ...
    public int next() throws IOException, XNIException {
      ...
      if (fPropertyManager != null) {
        dtdGrammarUtil =
            new DTDGrammarUtil(
                ((XMLDTDScannerImpl) fDTDScanner).getGrammar(), fSymbolTable, fNamespaceContext);
      }
            ...
            case SCANNER_STATE_DTD_INTERNAL_DECLS:
              {
                ...
                boolean moreToScan =
                    fDTDScanner.scanDTDInternalSubset(
                        completeDTD, fStandalone, fHasExternalDTD && fLoadExternalDTD);
                Entity entity = fEntityScanner.getCurrentEntity();
                if (entity instanceof Entity.ScannedEntity) {
                  fEndPos = ((Entity.ScannedEntity) entity).position;
                }
            ...
            case SCANNER_STATE_DTD_EXTERNAL:
              {
                 ...
                 * fDTDDescription.setRootName(fDoctypeName); XMLInputSource xmlInputSource =
                 * fEntityManager.resolveEntity(fDTDDescription);
                 * fDTDScanner.setInputSource(xmlInputSource);
                 * setScannerState(SCANNER_STATE_DTD_EXTERNAL_DECLS); again = true; break;
                 */
                ...
                // Check access permission. If the source is resolved by a resolver, the check is
                // skipped.
