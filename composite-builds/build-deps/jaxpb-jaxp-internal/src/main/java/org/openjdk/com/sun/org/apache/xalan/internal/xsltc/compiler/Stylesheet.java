...
public final class Stylesheet extends SyntaxTreeNode {
  ...
  /**
   * Used to cache result of <code>getAllValidTemplates()</code>. Only set in top-level stylesheets
   * that include/import other stylesheets.
   */
  ...
  public int getMinimumDescendantPrecedence() {
    if (_minimumDescendantPrecedence == -1) {
      ...
      for (int i = 0; i < inclImpCount; i++) {
        int prec =
            ((Stylesheet) _includedStylesheets.elementAt(i)).getMinimumDescendantPrecedence();
  ...
  private String compileTopLevel(ClassGenerator classGen) {
    ...
    final LocalVariableGen current =
        toplevel.addLocalVariable(
            "current", org.openjdk.com.sun.org.apache.bcel.internal.generic.Type.INT, null, null);
