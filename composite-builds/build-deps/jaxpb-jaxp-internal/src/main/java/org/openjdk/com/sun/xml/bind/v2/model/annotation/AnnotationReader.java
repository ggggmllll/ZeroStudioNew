 ...
 *
 * <p>This is the lowest abstraction that encapsulates the difference between reading inline
 * annotations and external binding files.
 *
 * <p>Because the former operates on a {@link Field} and {@link Method} while the latter operates on
 * a "property", the methods defined on this interface takes both, and the callee gets to choose
 * which to use.
 *
 * <p>Most of the get method takes {@link Locatable}, which points to the place/context in which the
 * annotation is read. The returned annotation also implements {@link Locatable} (so that it can
 * point to the place where the annotation is placed), and its {@link Locatable#getUpstream()} will
 * return the given {@link Locatable}.
 *
 * <p>Errors found during reading annotations are reported through the error handler. A valid {@link
 * ErrorHandler} must be registered before the {@link AnnotationReader} is used.
 *
...
public interface AnnotationReader<T, C, F, M> {
  ...
  <A extends Annotation> A getMethodAnnotation(
      Class<A> annotation, M getter, M setter, Locatable srcpos);
   ...
   *
   * @param srcPos the location from which this annotation is read.
   */
   ...
   *
   * <p>Depending on the underlying reflection library, you can't always obtain the {@link Class}
   * object directly (see the Annotation Processing MirrorTypeException for example), so use this
   * method to avoid that.
   *
   * @param name The name of the annotation parameter to be read.
   */
  T getClassValue(Annotation a, String name);

  /**
   * Similar to {@link #getClassValue(Annotation, String)} method but obtains an array parameter.
   */
