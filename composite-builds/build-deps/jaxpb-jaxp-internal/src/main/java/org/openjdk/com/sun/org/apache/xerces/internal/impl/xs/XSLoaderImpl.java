...
public final class XSLoaderImpl implements XSLoader, DOMConfiguration {
   ...
   * configuration, it is possible to change the behavior of the load methods. The configuration may
   * support the setting of and the retrieval of the following non-boolean parameters defined on the
   * <code>DOMConfiguration</code> interface: <code>error-handler</code> (<code>DOMErrorHandler
   * </code>) and <code>resource-resolver</code> (<code>LSResourceResolver</code>). <br>
  ...
  /**
   * Parses the content of XML Schema documents specified as the list of URI references. If the URI
  ...
  /**
   * Parse an XML Schema document from a location identified by a URI reference. If the URI contains
   * a fragment identifier, the behavior is not defined by this specification.
   *
  ...
  /**
   * Grammar pool which merges grammars from the same namespace into one. This eliminates duplicate
   * named components. It doesn't ensure that the grammar is consistent, however this no worse than
   * than the behaviour of XMLSchemaLoader alone when used as an XSLoader.
   */
  private static final class XSGrammarMerger extends XSGrammarPool {
    ...
    private void mergeSchemaGrammars(SchemaGrammar cachedGrammar, SchemaGrammar newGrammar) {
      ...
      /**
       * Add all annotations. Since these components are not named it's possible we'll add duplicate
       * components. There isn't much we can do. It's no worse than XMLSchemaLoader when used as an
       * XSLoader.
       */
