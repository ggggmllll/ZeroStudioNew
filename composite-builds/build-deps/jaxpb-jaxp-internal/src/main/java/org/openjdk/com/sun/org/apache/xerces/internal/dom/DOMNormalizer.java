...
public class DOMNormalizer implements XMLDocumentHandler {
   ...
   * document in a "normal" form. The actual result depends on the features being set and governing
   * what operations actually take place. See setNormalizationFeature for details. Noticeably this
   * method normalizes Text nodes, makes the document "namespace wellformed", according to the
   * algorithm described below in pseudo code, by adding missing namespace declaration attributes
              ...
              if (fNamespaceValidation) {
                wellformed =
                    CoreDocumentImpl.isValidQName(
                        node.getPrefix(), node.getLocalName(), fDocument.isXML11Version());
              } else {
                wellformed =
      ...
      case Node.ENTITY_REFERENCE_NODE:
        {
          ...
          if ((fConfiguration.features & DOMConfigurationImpl.ENTITIES) == 0) {
            ...
            parent.removeChild(node);
            Node next =
                (prevSibling != null) ? prevSibling.getNextSibling() : parent.getFirstChild();
      ...
      case Node.CDATA_SECTION_NODE:
        {
          ...
          if ((fConfiguration.features & DOMConfigurationImpl.SPLITCDATA) != 0) {
            ...
            if (fDocument.errorChecking) {
              isXMLCharWF(
                  fErrorHandler, fError, fLocator, node.getNodeValue(), fDocument.isXML11Version());
            }
            while ((index = value.indexOf("]]>")) >= 0) {
              ...
              String msg =
                  DOMMessageFormatter.formatMessage(
                      DOMMessageFormatter.DOM_DOMAIN, "cdata-sections-splitted", null);
              reportDOMError(
                  fErrorHandler,
                  fError,
                  fLocator,
      ...
      case Node.PROCESSING_INSTRUCTION_NODE:
        {
          ...
          // do the well-formed valid PI target name , data check when application has set the value
          // of well-formed feature to true
          if (fDocument.errorChecking
              && (fConfiguration.features & DOMConfigurationImpl.WELLFORMED) != 0) {
            ...
            isXMLCharWF(
                fErrorHandler, fError, fLocator, pinode.getData(), fDocument.isXML11Version());
          }
  ...
  protected final void namespaceFixUp(ElementImpl element, AttributeMap attributes) {
    if (DEBUG) {
      System.out.println(
          "[ns-fixup] element:" + element.getNodeName() + " uri: " + element.getNamespaceURI());
    }
      ...
      for (int k = 0; k < attributes.getLength(); ++k) {
        ...
        if (uri != null && uri.equals(NamespaceContext.XMLNS_URI)) {
          ...
          if (fDocument.errorChecking && value.equals(NamespaceContext.XMLNS_URI)) {
            ...
            reportDOMError(
                fErrorHandler, fError, fLocator, msg, DOMError.SEVERITY_ERROR, "CantBindXMLNS");
          } else {
      ...
      for (int i = 0; i < fAttributeList.size(); i++) {
        ...
        if (uri != null) { // attribute has namespace !=null
          ...
          if (fDocument.errorChecking
            ...
            if (fDocument.isXMLVersionChanged()) {
              boolean wellformed =
                  CoreDocumentImpl.isXMLName(attr.getNodeName(), fDocument.isXML11Version());
              if (!wellformed) {
                String msg =
                    DOMMessageFormatter.formatMessage(
                        DOMMessageFormatter.DOM_DOMAIN,
  ...
  /**
   * Adds a namespace attribute or replaces the value of existing namespace attribute with the given
   * prefix and value for URI. In case prefix is empty will add/update default namespace
   * declaration.
   *
   * @param prefix
   * @param uri
    ...
    if (isXML11Version) {
      ...
      while (i < datalength) {
        ...
        if (XML11Char.isXML11Invalid(c)) {
          ...
          reportDOMError(
              errorHandler, error, locator, msg, DOMError.SEVERITY_ERROR, "wf-invalid-character");
        } else if (c == ']') {
          int count = i;
   ...
   *     the parsing of this document. However, it is <em>strongly</em> recommended that a locator
   *     be supplied that can at least report the system identifier of the document.
