...
public class XSDFACM implements XSCMValidator {
   ...
   * do the actual validation. Note tat since either XSElementDecl or XSParticleDecl object can live
   * here, we've got to use an Object.
   */
  ...
  /**
   * This is an array of booleans, one per state (there are fTransTableSize states in the DFA) that
   * indicates whether that state is a final state.
   */
  private boolean fFinalStateFlags[] = null;

  /**
   * The list of follow positions for each NFA position (i.e. for each non-epsilon leaf node.) This
   * is only used during the building of the DFA, and is let go afterwards.
   */
  ...
  /**
   * The count of leaf nodes. This is an important number that set some limits on the sizes of data
   * structures in the DFA process.
   */
   ...
   * array of arrays of ints. The first dimension is the number of states we end up with in the DFA.
   * The second dimensions is the number of unique elements in the content model (fElemMapSize).
   * Each entry in the second dimension indicates the new state given that input for the first
   * dimension's start state.
   *
   * <p>The fElemMap array handles mapping from element indexes to positions in the second dimension
  ...
  /**
   * Array of counters for all the for elements (or wildcards) of the form a{n,m} where n > 1 and m
   * <= unbounded. Used to count the a's to later check against n and m. Counter set to -1 if
      ...
      for (int j = 0; j < fElemMapSize; j++) {
        if (conflictTable[i][j] == 1) {
          ...
          throw new XMLSchemaException(
              "cos-nonambig", new Object[] {fElemMap[i].toString(), fElemMap[j].toString()});
        }
      }
    }
  ...
  /**
   * Check which elements are valid to appear at this point. This method also works if the state is
   * in error, in which case it returns what should have been seen.
   *
  ...
  /**
   * Used by constant space algorithm for a{n,m} for n > 1 and m <= unbounded. Called by a validator
   * if validation of countent model succeeds after subsuming a{n,m} to a* (or a+) to check the n
   * and m bounds. Returns <code>null</code> if validation of bounds is successful. Returns a list
   * of strings with error info if not. Even entries in list returned are error codes (used to look
   * up properties) and odd entries are parameters to be passed when formatting error message. Each
   * parameter is associated with the error code that preceeds it in the list.
   */
