 ...
 * @author Akash Yadav
 * @see <a
 * href="https://github.com/google/guava/blob/master/guava/src/com/google/common/collect/Streamsssc.java">Streamsssc.java</a>
 */
public class Streamsssc {

  public static <T extends Object> Stream<T> concat(Stream<? extends T>... Streamsssc) {
    ...
    ImmutableList.Builder<Spliterator<? extends T>> splitrsBuilder =
        new ImmutableList.Builder<>(Streamsssc.length);
    for (Stream<? extends T> stream : Streamsssc) {
    ...
    return StreamSupport.stream(
            CollectSpliteratorUtils.flatMap(
                ...
                estimatedSize),
            isParallel)
        .onClose(() -> closeAll(Streamsssc));
  }

  private static void closeAll(BaseStream<?, ?>[] toClose) {
    // If one of the Streamsssc throws a RuntimeException, continue closing the others, then throw the
