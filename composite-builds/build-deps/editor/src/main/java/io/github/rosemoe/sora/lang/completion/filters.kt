...
private data class Scratch(
    ...
    val diag: Array<IntArray> =
        Array(MAX_LEN) { IntArray(MAX_LEN) }, // the length of a contiguous diagonal match
...
internal fun fillInMaxWordMatchPos(
  ...
  withScratch {
    this.fillInMaxWordMatchPos(patternLen, wordLen, patternStart, wordStart, patternLow, wordLow)
  }
}
...
data class FuzzyScoreOptions(
  ...
  companion object {
    @JvmStatic val default = FuzzyScoreOptions(boostFullMatch = true, firstMatchCanBeWeak = true)
  }
}
  ...
  return withScratch {
    ...
    while (patternPos < patternLen) {
      ...
      while (wordPos < nextMaxWordMatchPos) {
        ...
        val leftScore =
            if (canComeLeft)
                tableMatrix[row][column - 1] + (if (diagMatrix[row][column - 1] > 0) -5 else 0)
            else 0 // penalty for a gap start

        val canComeLeftLeft = wordPos > minWordMatchPos + 1 && diagMatrix[row][column - 1] > 0
        val leftLeftScore =
            if (canComeLeftLeft)
                tableMatrix[row][column - 2] + (if (diagMatrix[row][column - 2] > 0) -5 else 0)
            else 0 // penalty for a gap start
        ...
        if (
          ...
          // always prefer choosing left left to jump over a diagonal because that means a match is
          // earlier in the word
...
internal fun doScore(
  ...
  if (patternPos == patternStart) { // first character in pattern
    ...
    if (newMatchStart) {
      ...
      // this is part of a contiguous match, so give it a slight bonus, but do so only if it would
      // not be a preferred gap location
