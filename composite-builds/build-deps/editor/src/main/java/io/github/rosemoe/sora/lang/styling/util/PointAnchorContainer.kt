...
/**
 * Container text position based objects. The total amount of objects held by the container is
 * expected to be not very large.
 *
...
open class PointAnchoredContainer<T : PointAnchoredObject> {
  companion object {
    val comparator =
        object : Comparator<PointAnchoredObject> {
          override fun compare(o1: PointAnchoredObject?, o2: PointAnchoredObject?): Int {
  ...
  private fun getInsertionPointLast(e: PointAnchoredObject): Int {
    ...
    while (
        index + 1 < objects.size && comparator.compare(objects[index], objects[index + 1]) == 0
    ) {
      index++
    }
    return index
  ...
  private class Anchor(override var line: Int, override var column: Int) : PointAnchoredObject
}
