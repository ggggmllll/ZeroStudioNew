...
fun filterCompletionItems(
  ...
  for (originItem in completionItemList) {
    ...
    if (word.length != wordLen) {
      word =
          if (wordLen == 0) ""
          else sourceLine.substring(cursorPosition.column - wordLen, cursorPosition.column)
...
fun createCompletionItemComparator(
    completionItemList: Collection<CompletionItem>
): Comparator<CompletionItem> {
  if (
      completionItemList.isNotEmpty() &&
          completionItemList.first().extra != null &&
          completionItemList.first().extra !is SortedCompletionItem
  ) {
...
fun List<CompletionItem>.highlightMatchLabel(
    colorSchema: EditorColorScheme?
): List<CompletionItem> {
