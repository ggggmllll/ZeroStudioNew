...
class ContentShallowCopyTest {
  ...
  fun `test shallow copy performance`() {
    ...
    val timeDeep = measureTime { repeat(iterationCount) { text.copyText(false) } }
    val timeShallow = measureTime { repeat(iterationCount) { text.copyTextShallow() } }
    println(
  ...
  fun `test shallow copy concurrent modification`() {
    ...
    val editsText = prepareEdits(text.length, editCount = 100000, insertionLen = 100, seed = 12345)
    val editsCopy = prepareEdits(copy.length, editCount = 100000, insertionLen = 100, seed = 123456)
    ...
    val thread = Thread {
      runCatching { testOnEdits(sbCopy, copy, editsCopy) }.onFailure { err.set(it) }
    }
    thread.start()
  ...
  private fun prepareEdits(
    ...
    for (i in 0 until editCount) {
      ...
      if (r.nextFloat() < insertionProbability || index == len || len == 0) {
        edits.add(Insert(r.nextInt(len + 1), String(CharArray(insertionLen) { charset.random(r) })))
        len += insertionLen
      } else {
