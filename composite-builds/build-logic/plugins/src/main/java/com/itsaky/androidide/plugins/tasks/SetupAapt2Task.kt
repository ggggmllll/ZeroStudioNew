 ...
 * 2. 标准构建模式： a. 遍历所有目标 CPU 架构。 b. [首选方案] 尝试从远程 URL 下载 aapt2 二进制文件并进行校验和验证。 c. [备用方案]
 *    如果下载失败，则自动切换到从项目根目录的 `docs/aapt2/aapt2-[arch]` 路径复制本地文件。 d. 对最终获取的 aapt2 文件进行 ELF ABI
 *    断言，确保架构匹配。
 */
abstract class SetupAapt2Task : DefaultTask() {
  ...
  fun setupAapt2() {
    ...
    AAPT2_CHECKSUMS.forEach { (arch, checksum) ->
      ...
      // } catch (e: Exception) {
      // logger.warn("Failed to download aapt2 for '{}'. Falling back to local copy from
      // 'docs/aapt2/'.", arch, e)
      ...
      // "Failed to download aapt2 for '$arch' and the local fallback file could not be found at:
      // ${localAapt2File.path}"
      // )
      // }
