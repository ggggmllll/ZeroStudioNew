 ...
 * Configures the shell environment variables including PATH, HOME, PREFIX, and JDK paths.
 * 
 * @author android_zero
 */
public class TermuxShellEnvironment extends AndroidShellEnvironment {
    ...
    public HashMap<String, String> getEnvironment(@NonNull Context currentPackageContext, boolean isFailSafe) {
        ...
        if (!isFailSafe) {
            ...
            // [Modified] Construct PATH to include JDK binaries
            // 1. Termux bin prefix ($PREFIX/bin)
            // 2. JAVA_HOME bin (for java, javac, etc)
            // 3. Android SDK cmdline-tools
            String javaBinPath = new File(Environment.JAVA_HOME, "bin").getAbsolutePath();
            
            String newPath = TermuxConstants.TERMUX_BIN_PREFIX_DIR_PATH + 
                             ":" + javaBinPath + 
                             String.format(":%s/cmdline-tools/latest/bin", ANDROID_HOME.getAbsolutePath());
                             
            environment.put(ENV_PATH, newPath);
