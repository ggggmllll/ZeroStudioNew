...
public class PackageUtils {
    ...
    public static Context getContextForPackageOrExitApp(@NonNull Context context, String packageName,
        ...
        if (packageContext == null && exitAppOnError) {
            String errorMessage = context.getString(string.error_get_package_context_failed_message,
                packageName);
            if (!DataUtils.isNullOrEmpty(helpUrl))
                errorMessage += "\n" + context.getString(string.error_get_package_context_failed_help_url_message, helpUrl);
            Logger.logError(LOG_TAG, errorMessage);
            MessageDialogUtils.exitAppWithErrorMessage(context,
                context.getString(string.error_get_package_context_failed_title),
                errorMessage);
        }
    ...
    public static String isAppInstalled(@NonNull final Context context, String appName, String packageName) {
        ...
        if (!isAppEnabled)
            errmsg = context.getString(string.error_app_not_installed_or_disabled_warning, appName, packageName);
    ...
    public static String setComponentState(@NonNull final Context context, @NonNull String packageName,
        ...
        try {
            ...
            String errmsg = context.getString(
                newState ? string.error_enable_component_failed : string.error_disable_component_failed,
    ...
    public static Boolean isComponentDisabled(@NonNull final Context context, @NonNull String packageName,
        ...
        try {
            ...
            if (logErrorMessage)
                Logger.logStackTraceWithMessage(LOG_TAG, context.getString(string.error_get_component_state_failed, packageName, className), e);
        }
