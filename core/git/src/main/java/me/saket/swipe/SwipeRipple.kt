...
internal class SwipeRippleState {
  ...
  suspend fun animate(
    ...
    // Reverse animation feels faster (especially for larger swipe distances) so slow it down
    // further.
    ...
    coroutineScope {
      ...
      launch {
        Animatable(initialValue = if (action.isUndo) 0f else 0.25f)
            .animateTo(
  ...
  fun draw(scope: DrawScope) {
    ripple.value?.run {
      scope.clipRect {
        val size = scope.size
        // Start the ripple with a radius equal to the available height so that it covers the entire
        // edge.
