 ...
 *   weighted siblings. [SwipeableActionsBox] will divide its horizontal space and distribute it to
 *   actions according to their weight.
 * @param isUndo Determines the direction in which a ripple is drawn when this action is swiped.
 *   When false, the ripple grows from this action's position to consume the entire composable, and
 *   vice versa. This can be used for actions that can be toggled on and off.
 */
class SwipeAction(
    val onSwipe: () -> Unit,
...
fun SwipeAction(
  ...
  return SwipeAction(
      icon = {
        Image(modifier = Modifier.padding(16.dp), painter = icon, contentDescription = null)
      },
      background = background,
      weight = weight,
      onSwipe = onSwipe,
