...
fun ResetDialog(
  ...
  AlertDialog(
      title = { DialogTitle(stringResource(R.string.reset)) },
      text = {
        ScrollableColumn {
          if (fullOidOrBranchOrTag != null) {
            TextField(
                ...
                singleLine = true,
                onValueChange = { fullOidOrBranchOrTag.value = it },
                label = { Text(stringResource(R.string.hash_branch_tag)) },
            )
      ...
      confirmButton = {
        TextButton(
              ...
              if (onOk == null) {
                ...
                doJobThenOffLoading job@{
                  Repository.open(repoFullPath).use { repo ->
                    ...
                    val type =
                        if (selectedOpt.intValue == optSoft) Reset.ResetT.SOFT
                        else if (selectedOpt.intValue == optMixed) Reset.ResetT.MIXED
                        else Reset.ResetT.HARD
                    val commitRet =
                        Libgit2Helper.resolveCommitByHashOrRef(
                            repo,
                            fullOidOrBranchOrTag?.value ?: "",
                        )
                    ...
                    if (commitRet.hasError()) {
                      ...
                      createAndInsertError(
                          repoId,
                          "Reset $type err, resolve commit failed: " + commitRet.msg,
                      )
                      return@job
                    }
          ...
          Text(text = stringResource(R.string.reset), color = MyStyleKt.TextColor.danger())
        }
      },
  )
}
