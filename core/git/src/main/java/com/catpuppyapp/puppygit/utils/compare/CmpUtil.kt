...
object CmpUtil {
   ...
   *   总之，因为感觉匹配率高所以我加了这个参数，若传true，则会在比较前调用add和del， 并在返回结果前再调换回来，调用者只需正常传add和del即可，无需再手动调换；
  ...
  fun <T : CharSequence> compare(
      ...
      swap: Boolean = false,
      degradeMatchByWordsToMatchByCharsIfNonMatched: Boolean =
    ...
    return try {
      ...
      val result =
          SimilarCompare.INSTANCE.doCompare(
              ...
              degradeToCharMatchingIfMatchByWordFailed =
                  degradeMatchByWordsToMatchByCharsIfNonMatched,
              treatNoWordMatchAsNoMatchedWhenMatchByWord =
                  treatNoWordMatchAsNoMatchedWhenMatchByWord,
          )
   ...
   *
   * @param targetMatchCount if has these chars matched, will abort matching and return matched
   *   count. larger for more text matching(bad performance), smaller for more faster handling(good
   *   performance).
   * @param targetMatchCount
   *   匹配到此变量所代表的字数将终止匹配并返回匹配数（这个变量的值）。这个值越大，性能越差，但判断两个字符串重合度越越准；值越小，性能越好，但判断结果则更不准
   * @return at least matched chars count. greater than 0 means matched, non-matched otherwise, the
   *   returned value may grater than `targetMatchCount`
   */
  fun roughlyMatch(str1NoLineBreak: String, str2NoLineBreak: String, targetMatchCount: Int): Int {
    ...
    //        if(((str1NonBlankEndIndex - str1NonBlankStartIndex) < targetMatchCount) ||
    // ((str2NonBlankEndIndex - str2NonBlankStartIndex) < targetMatchCount)) {
    //            return false
    //        }

    // long str contains part of short str
    val containsCount =
        longerContainsPartOfShorter(
            str1NoLineBreak,
            str2NoLineBreak,
            targetMatchCount,
            IntRange(str1NonBlankStartIndex, str1NonBlankEndIndex),
            IntRange(str2NonBlankStartIndex, str2NonBlankEndIndex),
        )
    //        if(containsCount > 0) {
    ...
    // match starts with
    val startsMatchedCount =
        matchStartsOrEndsWith(
            str1NoLineBreak,
            str2NoLineBreak,
            false,
            targetMatchCount,
            str1NonBlankStartIndex,
            str2NonBlankStartIndex,
        )
    ...
    // match ends with
    val endsMatchedCount =
        matchStartsOrEndsWith(
            str1NoLineBreak,
            str2NoLineBreak,
            true,
            targetMatchCount,
            str1NonBlankEndIndex,
            str2NonBlankEndIndex,
        )
   ...
   * @param targetMatchCount match how many chars as matched success, if this value greater than
   *   str1 or str2 's length after trimStarts/Ends, this method will return false as not matched
   * @return matched chars count, range is [0, targetMatchCount], both are inclusive
   */
  private fun matchStartsOrEndsWith(
      str1: String,
      str2: String,
      reverse: Boolean,
  ...
  /**
   * @return matched chars count, 0 means non-matched or grater than 0 means matched. and, the
   *   returned value may grater than `targetMatchCount`
   */
  private fun longerContainsPartOfShorter(
      str1: String,
      str2: String,
      targetMatchCount: Int,
      str1NonBlankRange: IntRange,
      str2NonBlankRange: IntRange,
  ): Int {
    ...
    val (longer, shorter) =
        if (
            str1NonBlankRange.let { it.endInclusive - it.start } >=
                str2NonBlankRange.let { it.endInclusive - it.start }
        ) {
          Pair(str1, str2)
        } else {
    ...
    // 不足期望的匹配字符数，当作无匹配
    // targetMatchCount通常大于0，所以如果end - start小于等于0，会在这返回，而等于0即涵盖了subString为空字符串的情况，所以后面不需要专门在sub
    // string为empty时专门返回false了，
    // 另一个这里不判断empty的原因是为了完全遵从targetMatchCount，不然当期望匹配0则当作两个字符串匹配时（只要调用此函数就当作匹配而不管是否真的匹配），代码逻辑会不对
    //        if((end - start) < targetMatchCount) {
    //            return false
    //        }

    // if end-start equals to 0, has 1 chars; if greater than 0, more than 1 char; else, bad range,
    // return 0
    if ((end - start) < 0) {
      return 0
    }
    ...
    // if end - start == 0, means subShorter has 1 char, and longer contains it, so should return 1
    // as matched count rather than 0
