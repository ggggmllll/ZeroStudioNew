...
// private val highlightMapShared: MutableMap<String, Map<String, AnnotatedStringResult>> =
// ConcurrentMap()
...
class MyCodeEditor(
  ...
  private fun genNewStyleDelegate(editorState: TextEditorState?) =
      MyEditorStyleReceiver(this, Theme.inDarkTheme, stylesMap, editorState, languageScope)
  ...
  private fun updatePlScopeIfNeeded(plScope: MutableState<PLScope>, fileName: String) {
    // if was detected language or selected by user, then will not update program language scope
    // again
    if (plScope.value == PLScope.AUTO) {
      plScope.value =
          if (SettingsUtil.isEditorSyntaxHighlightEnabled()) {
  ...
  fun obtainCachedStyles(editorState: TextEditorState): StylesResult? {
    ...
    return if (isGoodStyles(cachedStyles, editorState)) {
      ...
      // when switched theme, maybe need remove another themes cached styles, if not clear is ok
      // too,
  ...
  fun sendUpdateStylesRequest(
    ...
    doJobThenOffLoading job@{
      stylesRequestLock.withLock {
        ...
        val targetEditorState =
            if (stylesUpdateRequest.ignoreThis) {
              if (AppModel.devModeOn) {
                // 忽略这次的样式，可能是阶段性修改，比如删除很多行，刚删除了一部分，删到最后一个条目时才会应用一次样式
                MyLog.i(
                    TAG,
                    "ignore styles for fieldsId: ${stylesUpdateRequest.targetEditorState.fieldsId}, the styles maybe update by later calls",
                )
              }
              null
            } else {
        ...
        try {
          ...
          if (AppModel.devModeOn) {
            MyLog.i(
                TAG,
                "send syntax highlight analyze request for fieldsId: ${targetEditorState?.fieldsId}",
            )
          }
        } catch (e: Exception) {
          ...
          MyLog.e(
              TAG,
              "#sendUpdateStylesRequest() err: call `TextMateUtil.setReceiverThenDoAct()` err: targetFieldsId=${receiver.editorState?.fieldsId}, err=${e.stackTraceToString()}",
          )
        }
      }
    }
  ...
  fun getLatestStylesResultIfMatch(editorState: TextEditorState) =
      if (isGoodStyles(latestStyles, editorState)) latestStyles else null
  ...
  fun analyze(
    ...
    doJobThenOffLoading { analyzeLock.withLock { doAnalyzeNoLock(editorState, plScope) } }
  }
  ...
  private fun doAnalyzeNoLock(
    ...
    // PLScope.scopeInvalid(plScope.scope)) {
    ...
    if (scopeChanged) {
      ...
      if (cachedStyles != null) {
        ...
        doJobThenOffLoading { editorState.applySyntaxHighlighting(cachedStyles) }
        return
      }
    }
    ...
    //            lang.isAutoCompleteEnabled = false  // enable this is fine, and can avoid store
    // identifiers for auto complete (reduce mem use and improve performance), but, already disabled
    // it when create Language, so don't need disable again at here
    //            lang.tabSize = 0  // enable this maybe will cause err when open git config file or
    // other file which have tab size not equals to 0 spaces? I am not sure.
  ...
  fun ifSameReturnCachedAnnotatedString(
      syntaxHighlightId: String,
      annotatedStringToCheck: AnnotatedString,
  ): AnnotatedString {
    return annotatedStringCachedMap.get(syntaxHighlightId).let {
      if (it == annotatedStringToCheck) {
        ...
        annotatedStringToCheck.also { annotatedStringCachedMap.put(syntaxHighlightId, it) }
      }
    }
  }
  ...
  // set checkTimes to control check how many times, that should not too large, else may have many
  // tasks, that's bad
  fun startAnalyzeWhenUserStopInputForAWhile(
      initState: TextEditorState,
      delayInSec: Int = 1,
      checkTimes: Int = 5,
  ) {
    if (plScopeStateInvalid()) {
    ...
    doJobThenOffLoading task@{
      ...
      delayAnalyzingTaskLock.withLock {
        ...
        while (count++ < checkTimes) {
          ...
          if (
              editorState != null &&
                  editorState.value.fieldsId.let { it.isNotBlank() && it == initState.fieldsId }
          ) {
  ...
  fun releaseAndClearUndoStack() {
    ...
    doJobThenOffLoading { undoStack.value.reset("", force = true, cleanUnusedStyles = false) }
  }
  ...
  suspend fun doActWithLatestEditorState(owner: String, act: suspend (TextEditorState) -> Unit) {
    ...
    textEditorStateOnChangeLock.withLock { act(editorState.value) }
  ...
  fun doActWithLatestEditorStateInCoroutine(owner: String, act: suspend (TextEditorState) -> Unit) {
    doJobThenOffLoading { doActWithLatestEditorState(owner, act) }
  }

  fun generateAnnotatedStringForLine(
    ...
    return buildAnnotatedString {
      TextMateUtil.forEachSpanResult(rawText, spans) { start, end, style ->
        withStyle(style) { append(rawText.substring(start, end)) }
      }
    }
  }
}
...
data class StylesResult(
  ...
  fun copyForEditorState(newFieldsId: String) =
      copy(
          styles = styles,
...
class SyntaxHighlightResult(
  ...
  fun obtainAnnotatedString(syntaxHighlightId: String) =
      if (inDarkTheme == Theme.inDarkTheme) storage.get(syntaxHighlightId) else null
}
...
fun MyCodeEditor?.scopeMatched(scope: String?) =
    this != null && scope != null && languageScope.scope == scope && !this.scopeInvalid()

fun MyCodeEditor?.isGoodStyles(stylesResult: StylesResult?, editorState: TextEditorState): Boolean {
  return !(this == null ||
      stylesResult == null ||
      stylesResult.fieldsId != editorState.fieldsId ||
