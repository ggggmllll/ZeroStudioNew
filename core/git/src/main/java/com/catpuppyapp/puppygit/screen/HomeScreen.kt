...
fun HomeScreen(
  ...
  val repoPageRepoList =
      mutableCustomStateListOf(stateKeyTag, "repoPageRepoList", listOf<RepoEntity>())

  val changeListRefreshRequiredByParentPage = rememberSaveable {
    SharedState.homeChangeList_Refresh
  }
  ...
  val changeListCachedEditorPath =
      mutableCustomBoxOf(stateKeyTag, "changeListCachedEditorPath") { "" }
  ...
  val changeListNaviTarget = rememberSaveable {
    mutableStateOf(Cons.ChangeListNaviTarget_InitValue)
  }

  val changeListPageFilterKeyWord =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
  ...
  //    val editorPageRequireOpenFilePath = StateUtil.getRememberSaveableState(initValue = "") //
  // canonicalPath
  ...
  val filesPageSelectedItems =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "filesPageSelectedItems",
          initValue = listOf<FileItemDto>(),
      )

  val reposPageIsSelectionMode = rememberSaveable { mutableStateOf(false) }
  val reposPageSelectedItems =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageSelectedItems",
          initValue = listOf<RepoEntity>(),
      )
  val reposPageUnshallowItems =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageUnshallowItems",
          initValue = listOf<RepoEntity>(),
      )
  val reposPageDeleteItems =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageDeleteItems",
          initValue = listOf<RepoEntity>(),
      )
  val reposPageUserInfoRepoList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageUserInfoRepoList",
          initValue = listOf<RepoEntity>(),
      )
  val reposPageUpstreamRemoteOptionsList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageUpstreamRemoteOptionsList",
          initValue = listOf<String>(),
      )

  // 指定一个待刷新的仓库列表，若为空，则刷新全部仓库
  val reposPageSpecifiedRefreshRepoList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "reposPageSpecifiedRefreshRepoList",
          initValue = listOf<RepoEntity>(),
      )
  ...
  val filesPageFilterOn = {
    ...
    if (text.isNotEmpty()) {
      filesPageFilterKeyword.value =
          filesPageFilterKeyword.value.copy(
  ...
  val filesPageGetFilterMode = { filesPageFilterMode.intValue }
  ...
  val automationPageScrolled = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.showNaviButtons)
  }

  val filesPageScrolled = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.showNaviButtons)
  }
  val filesPageListState =
      mutableCustomStateOf(stateKeyTag, "filesPageListState", initValue = LazyListState(0, 0))

  val filesPageSimpleFilterOn = rememberSaveable { mutableStateOf(false) }
  val filesPageSimpleFilterKeyWord =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
  ...
  val filesGetCurrentPath = { filesPageCurrentPath.value }
  val filesUpdateCurrentPath = { path: String -> filesPageCurrentPath.value = path }
  ...
  val filesPageCurPathFileItemDto =
      mutableCustomStateOf(stateKeyTag, "filesPageCurPathFileItemDto") { FileItemDto() }
  val filesPageCurrentPathBreadCrumbList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "filesPageCurrentPathBreadCrumbList",
          initValue = listOf<FileItemDto>(),
      )
  ...
  val reposPageFilterList =
      mutableCustomStateListOf(stateKeyTag, "reposPageFilterList", listOf<RepoEntity>())
  val filesPageFilterList =
      mutableCustomStateListOf(stateKeyTag, "filesPageFilterList", listOf<FileItemDto>())
  val changeListFilterList =
      mutableCustomStateListOf(stateKeyTag, "changeListFilterList", listOf<StatusTypeEntrySaver>())

  val changeListLastClickedItemKey = rememberSaveable {
    SharedState.homeChangeList_LastClickedItemKey
  }

  //    val changelistFilterListState = mutableCustomStateOf(keyTag = stateKeyTag, keyName =
  // "changelistFilterListState", LazyListState(0,0))
  val changelistFilterListState = rememberLazyListState()

  //    val filesFilterListState = mutableCustomStateOf(keyTag = stateKeyTag, keyName =
  // "filesFilterListState", LazyListState(0,0));
  //    val repoFilterListState = mutableCustomStateOf(keyTag = stateKeyTag, keyName =
  // "repoFilterListState", LazyListState(0,0))
  val filesFilterListState = rememberLazyListState()
  val repoFilterListState = rememberLazyListState()

  val editorDisableSoftKb = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.editor.disableSoftwareKeyboard)
  }

  val editorRecentFileList =
      mutableCustomStateListOf(stateKeyTag, "recentFileList") { listOf<FileDetail>() }
  val editorSelectedRecentFileList =
      mutableCustomStateListOf(stateKeyTag, "editorSelectedRecentFileList") { listOf<FileDetail>() }
  ...
  val editorFilterRecentList =
      mutableCustomStateListOf(stateKeyTag, "editorFilterRecentList") { listOf<FileDetail>() }
  ...
  val editorFilterRecentListKeyword =
      mutableCustomStateOf(stateKeyTag, "editorFilterRecentListKeyword") { TextFieldValue("") }
  ...
  val getActuallyRecentFilesList = {
    if (editorEnableRecentListFilter.value) editorFilterRecentList.value
    else editorRecentFileList.value
  }
  ...
  val getActuallyRecentFilesListLastPosition = {
    if (editorEnableRecentListFilter.value) editorRecentListFilterLastScrollPosition
    else editorRecentListLastScrollPosition
  }

  val editorPreviewFileDto =
      mutableCustomStateOf(stateKeyTag, "editorPreviewFileDto") { FileSimpleDto() }
  ...
  val changeListCurRepoState = rememberSaveable {
    mutableIntStateOf(StateT.NONE.bit)
  } // 初始状态是NONE，后面会在ChangeListInnerPage检查并更新状态，只要一创建innerpage或刷新（重新执行init），就会更新此状态
  val changeListPageFromTo = Cons.gitDiffFromIndexToWorktree
  val changeListPageItemList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "changeListPageItemList",
          initValue = listOf<StatusTypeEntrySaver>(),
      )
  val changeListPageItemListState = rememberLazyListState()
  val changeListPageSelectedItemList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "changeListPageSelectedItemList",
          initValue = listOf<StatusTypeEntrySaver>(),
      )
  ...
  val editorPreviewPathChanged = rememberSaveable {
    mutableStateOf("")
  } // 由于有可能重复入栈相同路径，例如从a->a，这时previewPath不会变化，导致其关联的检测当前页面是否是home页面的代码也不会被触发，所以，在修改previewPath的地方，同时给这个变量赋值随机数以触发页面刷新
  ...
  val editorPreviewNavStack =
      mutableCustomStateOf(stateKeyTag, "editorPreviewNavStack") { EditorPreviewNavStack("") }
  ...
  val intentConsumed = rememberSaveable {
    IntentHandler.intentConsumed
  } // 此变量用来确保导入模式只启动一次，避免以导入模式进入app后，进入子页面再返回再次以导入模式进入Files页面
  val filesPageRequireImportUriList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "filesPageRequireImportUriList",
          initValue = listOf<Uri>(),
      )
  val filesPageCurrentPathFileList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "filesPageCurrentPathFileList",
          initValue = listOf<FileItemDto>(),
      ) // 路径字符串，用路径分隔符分隔后的list
  ...
  val filesPageSelectedRepo =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "filesPageSelectedRepo",
          RepoEntity(id = ""),
      )

  val howToDealWithSingleSend = rememberSaveable {
    mutableStateOf(SingleSendHandleMethod.NEED_ASK.code)
  }
  ...
  if (showAskHandleSingleSendMethod.value) {
    ConfirmDialog3(
        ...
        customOk = {
          ScrollableRow {
            TextButton(onClick = { okAskHandleSingleSendMethod(SingleSendHandleMethod.EDIT) }) {
              Text(stringResource(id = R.string.edit))
            }

            TextButton(onClick = { okAskHandleSingleSendMethod(SingleSendHandleMethod.IMPORT) }) {
  ...
  val editorPageCloseDialogCallback =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
  ...
  val editorPageShowingFileDto =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "editorPageShowingFileDto",
          FileSimpleDto(),
      )
  val editorPageSnapshotedFileInfo =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "editorPageSnapshotedFileInfo",
          FileSimpleDto(),
      )
  val editorPageLastScrollEvent =
      mutableCustomStateOf<ScrollEvent?>(
          keyTag = stateKeyTag,
          keyName = "editorPageLastScrollEvent",
      ) {
        null
      } // 这个用remember就行，没必要在显示配置改变时还保留这个滚动状态，如果显示配置改变，直接设为null，从配置文件读取滚动位置重定位更好
  val editorPageLazyListState = rememberLazyListState()
  val editorPageIsInitDone = rememberSaveable {
    mutableStateOf(false)
  } // 这个也用remember就行，无需在配置改变时保存此状态，直接重置成false就行
  val editorPageSearchMode = rememberSaveable { mutableStateOf(false) }
  val editorPageSearchKeyword =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "editorPageSearchKeyword",
          TextFieldValue(""),
      )
  val editorPageMergeMode = rememberSaveable { mutableStateOf(false) }
  val editorPagePatchMode =
      rememberSaveable(settingsSnapshot.value.editor.patchModeOn) {
        mutableStateOf(settingsSnapshot.value.editor.patchModeOn)
      }
  val editorReadOnlyMode = rememberSaveable { mutableStateOf(false) }

  val editorShowLineNum = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.editor.showLineNum)
  }
  val editorLineNumFontSize = rememberSaveable {
    mutableIntStateOf(settingsSnapshot.value.editor.lineNumFontSize)
  }
  val editorLastSavedLineNumFontSize = rememberSaveable {
    mutableIntStateOf(editorLineNumFontSize.intValue)
  } // 用来检查，如果没变，就不执行保存，避免写入硬盘
  val editorFontSize = rememberSaveable {
    mutableIntStateOf(settingsSnapshot.value.editor.fontSize)
  }
  ...
  val editorShowUndoRedo = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.editor.showUndoRedo)
  }
  //    val editorUndoStack = remember(editorPageShowingFilePath.value){ derivedStateOf {
  // UndoStack(filePath = editorPageShowingFilePath.value.ioPath) } }
  ...
  val codeEditor =
      mutableCustomStateOf(stateKeyTag, "codeEditor") {
            MyCodeEditor(
  ...
  val requireInnerEditorOpenFileWithFileName = r@{ fullPath: String, expectReadOnly: Boolean ->
    ...
    editorPageMergeMode.value =
        false // 这个页面不负责打开ChangeList页面的diff条目，所以MergeMode状态直接初始化为关即可，用户需要用的时候打开文件后再手动开即可
    // 如果路径是app内置禁止修改的目录，则强制开启readonly，否则使用入参值
    //        editorReadOnlyMode.value = if(FsUtils.isReadOnlyDir(fullPath)) true else
    // expectReadOnly
    editorReadOnlyMode.value = expectReadOnly

    changeStateTriggerRefreshPage(
        needRefreshEditorPage
    ) // 这个其实可有可无，因为一切换页面，组件会重建，必然会执行一次LaunchedEffect，也就起到了刷新的作用
  }
  ...
  val filesGoToPath =
      getFilesGoToPath(
          filesPageLastPathByPressBack,
          filesGetCurrentPath,
          filesUpdateCurrentPath,
  ...
  val editorPreviewPageScrolled = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.showNaviButtons)
  }
  val changelistPageScrolled = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.showNaviButtons)
  }
  val repoPageScrolled = rememberSaveable { mutableStateOf(settingsSnapshot.value.showNaviButtons) }
  val editorRecentListScrolled = rememberSaveable {
    mutableStateOf(settingsSnapshot.value.showNaviButtons)
  }

  // two usages: 1. re query repo list when click title;  2. after imported submodules at ChangeList
  // page
  val needReQueryRepoListForChangeListTitle = rememberSaveable { mutableStateOf("") }
  val changeListRepoList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "changeListRepoList",
          initValue = listOf<RepoEntity>(),
      )
  ...
  val showWelcomeToNewUser = rememberSaveable {
    mutableStateOf(PrefMan.isFirstUse(activityContext))
  }
  ...
  // stringResource(R.string.try_long_press_icon_get_hints))
  //                    }
  //
  //                    // whatever, the master password actually
  // 没什么卵用，每次都要输麻烦，若存，就不绝对安全，而且很多时候没必要啊，数据库在用户手机本地，app空间内，一般不会泄漏，不建议设了，虽然设了还是比不设好一点点
  ////                    Row (modifier = Modifier.padding(top = 15.dp)){
  ////                        Text(text =
  // stringResource(R.string.recommend_go_to_settings_screen_set_a_master_password), color =
  ...
  //        showToast(context = appContext, appContext.getString(R.string.file_saved),
  // Toast.LENGTH_SHORT)
      ...
      listOf(
          ...
          clickChangeListPage@{
            doJobThenOffLoading {
              ...
              if (
                  editorShowingPath.isNotBlank() &&
                      oldChangeListCachedEditorPath != editorShowingPath
              ) {
  ...
  val openDrawer = { // 打开侧栏(抽屉)
    scope.launch { drawerState.apply { if (isClosed) open() } }
  ...
  ModalNavigationDrawer(
      ...
      drawerContent = {
        ModalDrawerSheet(
            ...
            //                modifier= if(drawerState.isOpen) Modifier.fillMaxWidth(.8F) else
            // Modifier,
    ...
    Scaffold(
              ...
              title = {
                ...
                if (currentHomeScreen.intValue == Cons.selectedItem_Repos) {
                  if (repoPageFilterModeOn.value) {
                    FilterTextField(
                        filterKeyWord = repoPageFilterKeyWord,
                        loading = reposSearching.value,
                    )
                  } else {
                    ReposTitle(
                  ...
                  EditorTitle(
                      ...
                      getActuallyRecentFilesListLastPosition =
                          getActuallyRecentFilesListLastPosition,
                  ...
                  if (changeListPageFilterModeOn.value) {
                    FilterTextField(
                        filterKeyWord = changeListPageFilterKeyWord,
                        loading = changeListSearching.value,
                    )
                  } else {
                    ChangeListTitle(
                  ...
                  ScrollableTitle(
                      ...
                      lastPosition = serviceLastPosition,
                  )
                  ...
                  ScrollableTitle(
                      ...
                      lastPosition = automationLastPosition,
                  )
                } else {
                  SimpleTitle()
                }
              ...
              navigationIcon = {
                ...
                if (
                  ...
                  LongPressAbleIconBtn(
                    ...
                    currentHomeScreen.intValue == Cons.selectedItem_ChangeList &&
                        changeListPageFilterModeOn.value
                ) {
                  LongPressAbleIconBtn(
                      tooltipText = stringResource(R.string.close),
                      icon = Icons.Filled.Close,
                  ...
                  LongPressAbleIconBtn(
                    ...
                    currentHomeScreen.intValue == Cons.selectedItem_Editor &&
                        (editorIsPreviewModeOn.value ||
                            editorPageSearchMode.value ||
                            editorAdjustFontSizeMode.value ||
                            editorAdjustLineNumFontSizeMode.value ||
                  ...
                  LongPressAbleIconBtn(
                    ...
                    if (editorIsPreviewModeOn.value) {
                      ...
                      editorPageRequestFromParent.value =
                          PageRequest.requireSaveFontSizeAndQuitAdjust
                    } else if (editorAdjustLineNumFontSizeMode.value) {
                      editorPageRequestFromParent.value =
                          PageRequest.requireSaveLineNumFontSizeAndQuitAdjust
              ...
              actions = {
                if (currentHomeScreen.intValue == Cons.selectedItem_Repos) {
                  ...
                  FilesPageActions(
                      ...
                      // showCreateFileOrFolderDialog,
                      refreshPage = { changeStateTriggerRefreshPage(needRefreshFilesPage) },
        ...
        floatingActionButton = {
          ...
          if (
              currentHomeScreen.intValue == Cons.selectedItem_Editor &&
                  editorIsPreviewModeOn.value &&
                  editorPreviewPageScrolled.value
          ) {
            ...
            SmallFab(
                modifier =
                    MyStyleKt.Fab.getFabModifierForEditor(
                        editorPageTextEditorState.value.isMultipleSelectionMode,
                        UIHelper.isPortrait(),
                    ),
                icon = Icons.Filled.Save,
                iconDesc = stringResource(id = R.string.save),
            ) {
              editorPageRequestFromParent.value = PageRequest.requireSave
            }
          } else if (
              currentHomeScreen.intValue == Cons.selectedItem_Editor &&
                  editorInRecentFilesPage.value &&
                  editorRecentListScrolled.value
          ) {
            GoToTopAndGoToBottomFab(
            ...
            if (changeListHasErr.value) {
              ...
              currentHomeScreen.intValue == Cons.selectedItem_Repos && repoPageScrolled.value
          ) {
            GoToTopAndGoToBottomFab(
              ...
              currentHomeScreen.intValue == Cons.selectedItem_Files && filesPageScrolled.value
          ) {
            if (filesPageHasErr()) {
              GoToTopAndGoToBottomFab(
                  scope = scope,
      ...
      if (AppModel.showChangelogDialog.value) {
        ChangelogDialog(onClose = { AppModel.showChangelogDialog.value = false })
      }
      ...
      //                    if(editorPageIsLoading.value) loadingText.value else
      ...
      if (currentHomeScreen.intValue == Cons.selectedItem_Repos) {
        ...
        EditorInnerPage(
            ...
            loadingOff = editorPageLoadingOff,
            saveOnDispose =
                true, // 销毁时保存处理起来比切换抽屉处理要好实现一些，所以在这里请求销毁时保存，但如果是子页面之类比较好处理的情况，这个值应该传false，由父页面负责保存
            doSave = doSave,
        ...
        ChangeListInnerPage(
            //                    stateKeyTag = Cache.combineKeys(stateKeyTag,
            // "ChangeListInnerPage"),
            stateKeyTag = stateKeyTag,
            ...
            // index..worktree, need not pass params, because `fromTo` already implicit
            // `indexToWorktree` or `headToIndex`
        ...
        SubscriptionPage(
            contentPadding = contentPadding,
            needRefresh = subscriptionPageNeedRefresh,
            openDrawer = openDrawer,
        )
      } else if (currentHomeScreen.intValue == Cons.selectedItem_Service) {
        ServiceInnerPage(
  ...
  LaunchedEffect(initDone.value, currentHomeScreen.intValue) {
    ...
    settingsSnapshot.value =
        newSettings // 确保在侧栏切换页面后重新获取最新的设置项，因为侧栏切换的本质上是同一个页面，所以默认不会重新执行remember，重新进入子页面再返回，所以这里更新下设置项避免使用旧的
    val settingsSnapshot = newSettings

    if (
        settingsSnapshot.startPageMode == SettingsCons.startPageMode_rememberLastQuit &&
            currentHomeScreen != settingsSnapshot.lastQuitHomeScreen &&
            initDone.value &&
            currentHomeScreen != Cons.selectedItem_Never &&
      ...
      // 写入配置文件
      SettingsUtil.update { it.lastQuitHomeScreen = currentHomeScreen }
    }
  }
  ...
  LaunchedEffect(needRefreshHome.value) {
    // test
    //        delay(30*1000)
    //        throw RuntimeException("test save when exception")  // passed, it can save when
    // exception threw, even in android 8, still worked like a charm
    // test
    ...
    doJobThenOffLoading {
      refreshLock.withLock {
        try {
          ...
          if (showWelcomeToNewUser.value) {
            if (
                PrefMan.isFirstUse(activityContext) &&
          ...
          if (
              true
          ) { // 检查activity是否为null没意义了，若为null怎么会执行这个页面的这块代码？之前主要是需要通过activity获取intent，现在不用了，所以直接设为true即可，但为了方便日后修改，不删除此if代码块
            ...
            if (intent != null) {
              ...
              MyLog.d(
                  TAG,
                  "curIntentConsumed: $curIntentConsumed, intent.action: ${intent.action}, intent.extras==null: ${intent.extras == null}, intent.data==null: ${intent.data == null}",
              )
              ...
              val requireEditFile =
                  intent.action.let { it == Intent.ACTION_VIEW || it == Intent.ACTION_EDIT }
              val extras = intent.extras

              // importListConsumed 确保每个Activity的文件列表只被消费一次(20240706:
              // 修复以导入模式启动app再进入子页面再返回会再次触发导入模式的bug)
              if (requireEditFile || extras != null) {
                // if need import files, go to Files, else go to page which matched to the page id
                // in the extras
                // if need import files, go to Files, else go to page which matched to the page id
                // in the extras
                ...
                if (
                    !curIntentConsumed
                ) { // 如果列表已经消费过一次，不重新导入，有可能通过系统分享菜单启动app，然后切换到后台，然后再从后台启动app会发生这种情况，但用户有可能已经在上次进入app后点击进入其他页面，所以这时再启动导入模式是不合逻辑的
                  // 是否请求编辑文件
                  if (requireEditFile) {
                  ...
                  var importFiles =
                      false // after jump to Files, it will be set to true, else keep false
                  // 20240706: fixed: if import mode in app then go to any sub page then back, will
                  // duplicate import files list
                  ...
                  //                                val howToDealWithSingleSend =
                  // howToDealWithSingleSend.value
                  ...
                  if (startPage.isNotBlank()) {
                    if (startPage == Cons.selectedItem_ChangeList.toString()) {
                      ...
                      if (startRepoId.isNotBlank()) { // 参数中携带的目标仓库id，查一下子，有就有，没就拉倒
                        startRepo =
                            AppModel.dbContainer.repoRepository.let {
                              it.getById(startRepoId) ?: it.getByName(startRepoId) ?: startRepo
                            }
                      }
  ...
  LifecycleEventEffect(Lifecycle.Event.ON_RESUME) {
    ...
    doJobThenOffLoading { Cache.clearAllSubPagesStates() }
  }
}
