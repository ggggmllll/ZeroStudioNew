...
data class DiffSettings(
    /**
     * if ture, will show delLine and addLine closer, else, maybe will split e.g. if true, show: -1
     * abc1 +1 abc2 -2 def3 +2 def4
     *
     * if false, show: -1 abc1 -2 def3 +1 abc2 +2 def4
     */
    var groupDiffContentByLineNum: Boolean = false,
    var diffContentSizeMaxLimit: Long =
        0L, // 0=no limit, unit is Byte, e.g. 1MB should set to 1000000L,

    /** load how many lines check once abort signal */
    var loadDiffContentCheckAbortSignalLines: Int = 1000,
    /**
     * load how much size will check once abort signal, when this or
     * `loadDiffContentCheckAbortSignalLines` reached will check abort signal
     */
    ...
    /** matching lines by words 按单词比较行 */
    var matchByWords: Boolean = false,

    /** enable select line for compare 启用选择行以比较，可以自定义比较两个行号不同的行，显示它们的区别 */
    ...
    /**
     * read only mode (was called "copyMode") note: even this value false, it may still enable in
     * some cases (e.g. compare two commits)，usually this value switchable when compare to
     * local(worktree) 注意：即使此值为false，只读模式仍可能强制启用（例如比较两个提交时），一般只有和本地文件(worktree)比较时此值才可切换
     */
    ...
    /** true, enable added and deleted line compare; false, disable true, 启用添加和删除行比较；否则禁用。 */
