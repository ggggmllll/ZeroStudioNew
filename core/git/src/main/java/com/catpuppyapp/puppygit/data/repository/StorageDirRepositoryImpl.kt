...
class StorageDirRepositoryImpl(private val dao: StorageDirDao) : StorageDirRepository {
  ...
  //        val funName = "insert"
  //
  //        StorageDirUtil.throwIfNameBad(item, RuntimeException("#$funName err: bad name, plz del
  // illegal chars"))
  //
  //        //检查是否和内部id重名
  //        StorageDirUtil.throwIfIdAlreadyExistsInVirtualSdList(item, RuntimeException("#$funName
  // err: id already exists"))
  //
  //        //fullPath必须唯一，若存在，禁止插入
  //        if(isNameOrFullPathExists(name=item.name, fullPath = item.fullPath)) {
  //            MyLog.w(TAG, "#$funName: warn: item's name or fullPath already exists! operation
  // abort...")
  ...
  //    }
  //
  //    override suspend fun delete(item: StorageDirEntity, requireDelFilesOnDisk:Boolean,
  ...
  //            MyLog.w(TAG, "#$funName: warn: StorageDir '${item.name}' is not allow to del,
  // operation abort")
  ...
  //                    "/ 感觉禁止删除是最省事也最不容易出错的，但对用户来说最不方便。
  // (如果采用禁止删除的方案，需要：提供能分别删除无子sd的sd和仓库的途径，要方便，要能批量删，那样的话，就可以用这个机制) /" +
  //                    "1: 禁止删除 (配套： “方便批量删除仓库和无子sd的sd的机制”)" +
  //                    "2: 递归删除所有子sd和所有与当前sd和子sd或子sd的子sd关联的所有仓库，并在ui上提供“删除硬盘上的文件”的勾选框，若勾选则删除硬盘文件" +
  //                    "3: 把当前模块的所有子模块和仓库都关联到other
  // sd。(ps:这种实现需要在在通过路径反向查找关联的sd时优先匹配仓库表条目的fullpath，若匹配成功则使用仓库的sdid，若匹配失败，才去匹配sd的fullpath，还匹配失败就判定路径无效，重定向到虚拟root目录，不然可能出现越狱访问的漏洞")
  //        }
  //
  ...
  //        StorageDirUtil.throwIfIdAlreadyExistsInVirtualSdList(item, RuntimeException("$funName
  // err: item not allow update"))
  //
  //        //这个检测只是兜底，调用者应在调用update前自行检查是否已存在相同fullpath或name
  //        if(isNameOrFullPathAlreadyUsedByOtherItem(name=item.name, fullPath = item.fullPath,
  // excludeId = item.id)) {
  //            MyLog.w(TAG, "#$funName: warn: item's name or fullPath already used by other item!
  // operation abort...")
  //
  //            throw RuntimeException("#$funName err: name/fullPath already exists")
  //        }
  ...
  //        val all = getAll()
  //        return all.filter {
  // !StorageDirCons.DefaultStorageDir.idListUnusedForMatchPath.contains(it.id) }
  //    }
  //
  ...
  //
  //
  //    override suspend fun getByNameOrFullPathExcludeId(name: String, fullPath: String,
  // excludeId:String): StorageDirEntity? {
  //        var item:StorageDirEntity? = null
  //
  //        //查内置sd
  //        for(it in StorageDirCons.DefaultStorageDir.listOfAllDefaultSds) {
  ...
  //     */
  //    override suspend fun isNameOrFullPathAlreadyUsedByOtherItem(name: String, fullPath: String,
  ...
  //        //获取一手资料，不用缓存，以免出错，但其实这样也有可能出错，呵呵
  //        // key=item.id, value=item,转换list为map，如果用associate则可自定义kv
  //        val allMap = getAll().associateBy { it.id }
  // //这个是包含默认条目的，真正的all，用来生成virtualpath，必须包含所有条目才行
  ...
  //                try {
  //                    it.virtualPath = StorageDirUtil.genVirtualPathByParentsName(it, allMap,
  // root)
