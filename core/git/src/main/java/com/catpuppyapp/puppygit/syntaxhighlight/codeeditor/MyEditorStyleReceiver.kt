...
class MyEditorStyleReceiver(
  ...
  override fun handleStyles(styles: Styles) {
    ...
    // this check can save memory, but may cause performance issue, if happened, disabled it is ok,
    // just maybe will use more memories
    ...
    val isUnusedFieldsId =
        (latestFieldsId != carriedFieldsId
        // 这个条件有可能不准，导致存上几个无效状态，例如：正在分支状态2的styles，
        //   然后执行undo，最新状态回退到状态1，然后状态2分析完毕，调用此方法，
        //   执行到这里，发现其时间戳大于状态1的，这时这个判断就会失效
        // this check may invalid after undo did, but not big problem, just will save few invalid
        // styles
    ...
    //        (resolved) 不行，有问题：
    //        lang + editorState1 调用分析，未结束时，lang + editorState2 重新设置了receiver，然后
    // 组合1的结果出来了，就被组合2拿到了，而组合2无法得知组合1 到底是谁
    ...
    val stylesResult =
        StylesResult(
            inDarkTheme,
            styles,
            StylesResultFrom.CODE_EDITOR,
            fieldsId = editorState.fieldsId,
            languageScope = languageScope,
        )
    if (codeEditor.isGoodStyles(stylesResult, editorState).not()) {
      MyLog.i(
          TAG,
          "`stylesResult` doesn't match with editor state: stylesResult=$stylesResult, styles.spans.lineCount=${styles.spans.lineCount}, editorState.fields.size=${editorState.fields.size}",
      )
      return
    }
