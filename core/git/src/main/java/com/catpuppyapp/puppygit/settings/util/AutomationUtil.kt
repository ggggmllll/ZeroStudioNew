...
object AutomationUtil {
  ...
  suspend fun getRepos(
    ...
    return try {
      val repoList =
          AppModel.dbContainer.repoRepository.getAll(updateRepoInfo = false).filter {
            bindRepoIds.contains(it.id)
          }
      val settings = SettingsUtil.getSettingsSnapshot()

      // 仅更新有可能用到的仓库的信息
      repoList.forEachBetter { Libgit2Helper.updateRepoInfo(it, settings = settings) }
  ...
  fun getSelectedAndUnSelectedAppList(
      context: Context,
      automationSettings: AutomationSettings,
  ...
  fun getAppAndRepoSpecifiedSettings(
      ...
      settings.automation.packageNameAndRepoAndSettingsMap.get(
          PackageNameAndRepo(appPackageName, repoId).toKey()
  ...
  fun groupReposByPushDelayTime(
    ...
    repos.forEachBetter {
      val pushDelayInSec =
          getAppAndRepoSpecifiedSettings(appPackageName, it.id, settings)
              .getPushDelayActuallyValue(settings)
