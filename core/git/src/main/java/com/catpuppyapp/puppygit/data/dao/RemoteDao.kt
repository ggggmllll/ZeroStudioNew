...
interface RemoteDao {
  ...
  @Query("SELECT * from remote ORDER BY id ASC") fun getAllStream(): Flow<List<RemoteEntity?>>

  @Query("SELECT * from remote WHERE id = :id") fun getStream(id: String): Flow<RemoteEntity?>

  @Query("SELECT * from remote WHERE id = :id") fun getById(id: String): RemoteEntity?

  @Insert suspend fun insert(item: RemoteEntity)

  @Update suspend fun update(item: RemoteEntity)

  @Delete suspend fun delete(item: RemoteEntity)

  @Query("DELETE FROM remote WHERE repoId = :repoId") suspend fun deleteByRepoId(repoId: String)
  ...
  // 可实现解除某个凭据的所有关联，或者将某个凭据所有关联条目关联到另一个凭据
  @Query(
      "update remote set pushCredentialId = :newCredentialId where pushCredentialId = :oldCredentialId"
  )
  suspend fun updatePushCredentialIdByCredentialId(oldCredentialId: String, newCredentialId: String)

  @Query(
      "update remote set credentialId = :fetchCredentialId , pushCredentialId = :pushCredentialId"
  )
  ...
  */
  ...
  @Query(
      "UPDATE remote set baseCreateTime = baseCreateTime-(:offsetInSec), baseUpdateTime = baseUpdateTime-(:offsetInSec)"
  )
