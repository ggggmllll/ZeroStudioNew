...
/**
 * fileName: 主要是用来检测mime类型的 filePath: 用来生成uri showOpenInEditor: 是否显示在内部编辑器打开的选项 openInEditor:
 * 执行在内部editor打开的函数 openSuccessCallback: 打开成功的回调，不管用内部editor打开还是用外部程序打开，只要打开成功就会调这个回调 close: 关闭弹窗
 */
@Composable
fun OpenAsDialog(
  ...
  val mimeTypeList =
      remember(fileName) {
        FsUtils.FileMimeTypes.typeList.toMutableList().let {
          ...
          // 我的app里只允许对文件使用 open
          // as，所以这里的filePath必然是文件（除非有bug），所以这里调用guessFromPath()无需判断路径是否是文件夹，也不需要写若是文件夹则在末尾追加分隔符的逻辑
          it.add(
      ...
      Row(
          modifier =
              Modifier.height(itemHeight).fillMaxWidth().clickable {
                val expectReadOnly =
                    readOnly.value // 期望的readonly模式，若文件路径不属于app内置禁止编辑的目录，则使用此值作为readonly的初始值
    ...
    mimeTypeList.forEachBetter { (mimeType, text) ->
      Row(
          modifier =
              Modifier.height(itemHeight).fillMaxWidth().clickable {
                val openSuccess =
                    FsUtils.openFile(activityContext, File(filePath), mimeType, readOnly.value)
