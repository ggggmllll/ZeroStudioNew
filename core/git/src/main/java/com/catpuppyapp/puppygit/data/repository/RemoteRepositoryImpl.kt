...
class RemoteRepositoryImpl(private val dao: RemoteDao) : RemoteRepository {
  ...
  override suspend fun getLinkedRemoteDtoForCredentialList(
      credentialId: String
  ...
  override suspend fun getUnlinkedRemoteDtoForCredentialList(
      credentialId: String
  ): List<RemoteDtoForCredential> {
    ...
    for (it in dtoList) {
      try {
        ...
        if (repoFromDb == null) {
          MyLog.w(
              TAG,
              "#setRemoteInfoForDtoList warn: an invalid repo id '${it.repoId}' related by remote '${it.remoteName}', remote id =${it.remoteId}",
          )
          continue
        }
        ...
        MyLog.e(
            TAG,
            "#setRemoteInfoForDtoList err: update info for remote '${it.remoteName}' err, remoteId=${it.remoteId}, err=${e.stackTraceToString()}",
        )
      }
    }
  }
  ...
  override suspend fun getRemoteDtoListByRepoId(repoId: String): List<RemoteDto> {
    ...
    val repoDb = AppModel.dbContainer.repoRepository
    val repoFromDb =
        repoDb.getById(repoId) ?: throw RuntimeException("no repo found by repoId '$repoId'")
    Repository.open(repoFromDb.fullSavePath).use { repo ->
      ...
      val willInsertToDb = listFromGit.filter { gitRemote ->
        listFromDb.none { it.remoteName == gitRemote }
      }
  ...
  private suspend fun syncRemoteInfoWithGit(remoteFromDb: RemoteEntity) {
    val repoDb = AppModel.dbContainer.repoRepository
    val repoFromDb =
        repoDb.getById(remoteFromDb.repoId)
            ?: throw RuntimeException(
                "no repo found by remoteFromDb.repoId '${remoteFromDb.repoId}'"
            )
    Repository.open(repoFromDb.fullSavePath).use { repo ->
      val remoteFromGit =
          Libgit2Helper.resolveRemote(repo, remoteFromDb.remoteName)
              ?: throw RuntimeException(
                  "resolve remote '${remoteFromDb.remoteName}' from git failed"
              )
  ...
  override suspend fun updateRemoteUrlById(
      id: String,
      remoteUrl: String,
    ...
    if (requireTransaction) {
      AppModel.dbContainer.db.withTransaction { act() }
    } else {
      act()
    }
  }
  ...
  override suspend fun updatePushCredentialIdByRemoteId(remoteId: String, credentialId: String) {
  ...
  override suspend fun updateAllFetchAndPushCredentialId(
      fetchCredentialId: String,
      pushCredentialId: String,
  ) {
