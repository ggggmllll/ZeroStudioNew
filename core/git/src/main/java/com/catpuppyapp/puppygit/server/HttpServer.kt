...
/** 如果ip同时在白名单和黑名单，将允许连接 如果设置项中的token为空字符串，将允许所有连接 */
private fun isGoodTokenAndIp(token: String?, requestIp: String, settings: AppSettings): Ret<Unit?> {
  ...
  if (
      whiteList.isEmpty() ||
          requestIp.isBlank() ||
          whiteList.find { it == matchAllIpSign || it == requestIp } == null
  ) {
    return errRet
  }
...
internal class HttpServer(val host: String, val port: Int) {
  private var server:
  ...
  private fun sendSuccessNotificationIfEnable(serviceNotify: ServiceNotify, settings: AppSettings) =
      { title: String?, msg: String?, startPage: Int?, startRepoId: String? ->
        if (settings.httpService.showNotifyWhenProgress) {
          serviceNotify.sendSuccessNotification(title, msg, startPage, startRepoId)
        }
      }

  private fun sendErrNotificationIfEnable(serviceNotify: ServiceNotify, settings: AppSettings) =
      { title: String, msg: String, startPage: Int, startRepoId: String ->
        if (settings.httpService.showNotifyWhenProgress) {
  ...
  private fun sendProgressNotificationIfEnable(
      serviceNotify: ServiceNotify,
      settings: AppSettings,
  ) = { repoNameOrId: String, progress: String ->
    if (settings.httpService.showNotifyWhenProgress) {
    ...
    try {
      server =
          embeddedServer(Netty, host = host, port = port) {
                ...
                routing {
                  /** 用于检测服务器是否在线 */
                  get("/status") { call.respond(createSuccessResult("online")) }
                  ...
                  /**
                   * query params: repoNameOrId: repo name or id, match by name first, if none, will
                   * match by id gitUsername: using for create commit, if null, will use PuppyGit
                   * settings gitEmail: using for create commit, if null, will use PuppyGit settings
                   * forceUseIdMatchRepo: 1 enable or 0 disable, default 0, if enable, will force
                   * match repo by repo id, else will match by name first, if no match, then match
                   * by id token: token is required
                   *
                   * e.g. request: http://127.0.0.1/pull?repoNameOrId=abc
                   */
                  get("/pull") {
                    ...
                    try {
                      ...
                      // 取参数
                      val forceUseIdMatchRepo =
                          call.request.queryParameters.get("forceUseIdMatchRepo") == "1"
                      // get git username and email for merge, if request doesn't contains them,
                      // will use PuppyGit app settings
                      ...
                      val pullWithRebase =
                          call.request.queryParameters.get("pullWithRebase")?.let { it == "1" }
                              ?: PrefUtil.getGlobalGitConfigPullWithRebase(AppModel.realAppContext)

                      // 从数据库查询仓库列表
                      val validRepoListFromDb =
                          getRepoListFromDb(
                      ...
                      doJobThenOffLoading {
                        ...
                        MyLog.d(
                            TAG,
                            "will do pull for ${validRepoListFromDb.size} repos: $validRepoListFromDb",
                        )
                        pullRepoList(
                            sessionId = sessionId,
                      ...
                      MyLog.e(
                          TAG,
                          "method:GET, route:$routeName, sessionId=$sessionId, repoNameOrId.size=${repoNameOrIdForLog.size}, repoNameOrId=$repoNameOrIdForLog, err=${e.stackTraceToString()}",
                      )
                    }
                  }

                  /**
                   * query params: repoNameOrId: repo name or id，优先查name，若无匹配，查id gitUsername: using
                   * for create commit, if null, will use PuppyGit settings gitEmail: using for
                   * create commit, if null, will use PuppyGit settings force: force push, 1 enable
                   * , 0 disable, if null, will disable (as 0) forceUseIdMatchRepo: 1 enable or 0
                   * disable, default 0, if enable, will force match repo by repo id, else will
                   * match by name first, if no match, then match by id token: token is required
                   * autoCommit: 1 enable or 0 disable, default 1: if enable and no conflict items
                   * exists, will auto commit all changes, and will check index, if index empty,
                   * will not pushing; if disable, will only do push, no commit changes, no index
                   * empty check, no conflict items check.
                   *
                   * e.g. request: http://127.0.0.1/push?repoNameOrId=abc
                   */
                  get("/push") {
                    val sessionId = generateRandomString()
                    ...
                    try {
                      ...
                      doJobThenOffLoading {
                        ...
                        MyLog.d(
                            TAG,
                            "will do push for ${validRepoListFromDb.size} repos: $validRepoListFromDb",
                        )
                   ...
                   * forceUseIdMatchRepo: 1 enable or 0 disable, default 0, if enable, will force
                   * match repo by repo id, else will match by name first, if no match, then match
                   * by id token: token is required autoCommit: 1 enable or 0 disable, default 1: if
                   * enable and no conflict items exists, will auto commit all changes, and will
                   * check index, if index empty, will not pushing; if disable, will only do push,
                   * no commit changes, no index empty check, no conflict items check.
                   */
                  get("/sync") {
                    val sessionId = generateRandomString()
                    val repoNameOrIdForLog = mutableListOf<String>()
                    ...
                    try {
                      ...
                      doJobThenOffLoading {
                        ...
                        MyLog.d(
                            TAG,
                            "will do sync for ${validRepoListFromDb.size} repos: $validRepoListFromDb",
                        )
                  ...
                  /**
                   * query params: gitUsername: using for create commit, if null, will use PuppyGit
                   * settings gitEmail: using for create commit, if null, will use PuppyGit settings
                   * token: token is required
                   *
                   * e.g. request:
                   * http://127.0.0.1/pullAll?gitUsername=username&gitEmail=email&token=your_token
                   */
                  get("/pullAll") {
                    val sessionId = generateRandomString()
                    ...
                    try {
                      ...
                      MyLog.e(
                          TAG,
                          "method:GET, route:$routeName, sessionId=$sessionId, err=${e.stackTraceToString()}",
                      )
                    }
                  }
                   ...
                   * query params: gitUsername: using for create commit, if null, will use PuppyGit
                   * settings gitEmail: using for create commit, if null, will use PuppyGit settings
                   * autoCommit: same as '/push' force: 1 enable , 0 disable, default 0 token: token
                   * is required
                   *
                   * e.g. request: http://127.0.0.1/pushAll?token=your_token
                   */
                  get("/pushAll") {
                    val sessionId = generateRandomString()
  ...
  private suspend fun getRepoListFromDb(
    ...
    MyLog.d(
        TAG,
        "raw repoNameOrId list size is: ${repoNameOrIdList.size}, values are: $repoNameOrIdList",
    )
  ...
  private fun tokenAndIpPassedOrThrowException(
      call: RoutingCall,
      routeName: String,
      settings: AppSettings,
  ) {
    ...
    if (tokenCheckRet.hasError()) {
      ...
      MyLog.e(
          TAG,
          "request rejected: routeName=$routeName, requestIp=$requestIp, token=$token, reason=${tokenCheckRet.msg}",
      )
  ...
  fun isServerRunning(): Boolean {
    ...
    //        val settings = SettingsUtil.getSettingsSnapshot()
    //        return checkApiRunning("${genHttpHostPortStr(settings.httpService.listenHost,
    // settings.httpService.listenPort)}/ping", 2)
  }
