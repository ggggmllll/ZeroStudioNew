  ...
  if (filesPageSimpleFilterOn) {
    ...
    val switchDropDownMenu = {
      if (dropDownMenuExpandState.value) { // need close
        ...
        doJobThenOffLoading {
          NameAndPath.getListForFilesManager(
              activityContext,
              storages.value.list,
              storages.value.updateLock,
          )
        }
    ...
    val getTitleColor = { UIHelper.getTitleColor(enabled = enableAction) }
          ...
          for ((idx, it) in storages.value.list.withIndex()) {
            ...
            if (
                it.type != NameAndPathType.FIRST_REPOS_STORAGE_PATH &&
                    it.type != NameAndPathType.REPOS_STORAGE_PATH
            ) {
              break
            }
    ...
    if (showDeleteStoragePathListDialog.value) {
        ...
        if (listMaybeChanged) {
          ...
          if (target.type == NameAndPathType.FIRST_REPOS_STORAGE_PATH) {
            ...
            if (itemAfterTarget?.type == NameAndPathType.REPOS_STORAGE_PATH) {
              storagePathList.set(
                  idxAfterTarget,
                  itemAfterTarget.copy(type = NameAndPathType.FIRST_REPOS_STORAGE_PATH),
              )
            }
          }
      ...
      ConfirmDialog2(
        ...
        doJobThenOffLoading {
          storages.value.updateLock.withLock { deleteStoragePath(targetIndex) }
        }
      }
    }
    ...
    TitleDropDownMenu(
          ...
          Text(
              text =
                  replaceStringResList(
                      ...
                      listOf("" + curPathItemDto.folderCount, "" + curPathItemDto.fileCount),
                  ),
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
        ...
        titleRightIcon = {
          Icon(
              ...
              tint =
                  if (enableAction) LocalContentColor.current
                  else UIHelper.getDisableBtnColor(inDarkTheme),
          )
        },
        isItemSelected = { false },
        menuItem = { element, selected ->
          var basePadding = PaddingValues(10.dp)
          val firstItemBasePadding =
              PaddingValues(top = 26.dp, bottom = 10.dp, start = 10.dp, end = 10.dp)
          ...
          if (
              element.type == NameAndPathType.FIRST_REPOS_STORAGE_PATH ||
                  element.type == NameAndPathType.REPOS_STORAGE_PATH
          ) {
            DropDownMenuItemText(
                ...
                trailIconOnClick = { initDeleteStoragePathListDialog(element.path) },
            )
          } else {
            DropDownMenuItemText(text1 = text1, text2 = text2, basePadding = basePadding)
          }
        },
        titleOnLongClick = { requestFromParent.value = PageRequest.requireShowPathDetails },
        itemOnClick = { goToPath(it.path) },
    )
    ...
    //            ScrollableRow {
    //                Text(
    //                    text = replaceStringResList(stringResource(R.string.folder_n_file_m),
    // listOf(""+curPathItemDto.folderCount, ""+curPathItemDto.fileCount)),
    //                    fontSize = MyStyleKt.Title.secondLineFontSize
    //                )
    //            }
