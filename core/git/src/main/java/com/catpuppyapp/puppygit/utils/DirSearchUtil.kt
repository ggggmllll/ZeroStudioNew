...
object DirSearchUtil {
  ...
  /**
   * 虚假的广度优先搜索，针对顶级目录和普通过滤功能性能一样（所以凑合用了一段时间，感觉也够用）， 但后续针对子目录的搜索并非广度优先， 实际会深入到一个子目录的最底层后才会查找
   * 其他的子目录（同时每个子目录也同样会先对目录内所有文件和文件夹进行匹配， 再分别深入其子目录，这个过程是递归的），所以并非真正的广度优先搜索。
   *
