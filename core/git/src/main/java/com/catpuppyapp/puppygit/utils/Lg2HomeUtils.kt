...
object Lg2HomeUtils {
  ...
  private const val sshKnownHostsVersionFileName =
      "version_known_hosts" // storage version of the file
  ...
  // user's known_hosts user difference format with well-known know_hosts file, each line is a
  // `SshCert.toDbString()`
  ...
  private fun createKnownHostsIfNonExists(appContext: Context) {
    ...
    // 从app res读取内容到文件
    FsUtils.copy(
        appContext.resources.openRawResource(known_hostsRawId),
        knownHostsFile.outputStream(),
    )
  ...
  private fun readItemFromUserKnownHostsFile() {
    doJobThenOffLoading {
      userKnownHostsFileLock.withLock {
        ...
        f.bufferedReader().use {
          while (true) {
            ...
            //                        if(sshCert!=null &&
            // userKnownHostItems.contains(sshCert).not()) { // for list
      ...
      MyLog.d(
          TAG,
          "read user SshCertList from file: size=${userKnownHostItems.size} items=$userKnownHostItems",
      )
    }
  }
  ...
  fun itemInUserKnownHostsFile(item: SshCert): Boolean {
    ...
    MyLog.d(
        TAG,
        "sshCert already contained: $contained, sshCertNeedCheck=$item, allCertCount=${set.size}, allCert=$set",
    )
    ...
    doJobThenOffLoading {
      userKnownHostsFileLock.withLock {
        userKnownHostsFile.outputStream().bufferedWriter().use { writer ->
          userKnownHostItems.forEach { writer.write("${it.toDbString()}\n") }
        }
      }
    }
  }
  ...
  fun addItemToUserKnownHostsFile(item: SshCert) {
    ...
    if (userKnownHostItems.add(item)) {
      // user's rules, shouldn't over 1000, so just write all, no need randam access or append to
      // file
