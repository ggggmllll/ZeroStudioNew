...
/**
 * difference with `ConfirmDialog`: this support hidden onOk/onCancel Button and custom
 * onDismiss(default do onCancel) and the default confirm text is "OK", denied text is "Cancel"
 */
@Composable
fun ConfirmDialog3(
    ...
    // click outside of dialog will call this function, recommend use `onCancel` instead passing
    // this param,
  ...
  AlertDialog(
      ...
      text = {
        if (requireShowTextCompose) {
          ...
          CopyScrollableColumn { Row { Text(text) } }
        }
      },
