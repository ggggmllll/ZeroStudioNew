...
interface DomainCredentialDao {
  ...
  @Query(
      "SELECT d.id as domainCredId, d.domain as domain, c.name as credName, c.id as credId, sshc.name as sshCredName, sshc.id as sshCredId from domain_credential d left join credential c on c.id= d.credentialId left join credential sshc on sshc.id=d.sshCredentialId order by d.baseCreateTime DESC"
  )
  ...
  @Insert suspend fun insert(item: DomainCredentialEntity)

  @Update suspend fun update(item: DomainCredentialEntity)

  @Delete suspend fun delete(item: DomainCredentialEntity)

  @Query("SELECT * from domain_credential WHERE id = :id")
  fun getById(id: String): DomainCredentialEntity?

  @Query(
      "UPDATE domain_credential set baseCreateTime = baseCreateTime-(:offsetInSec), baseUpdateTime = baseUpdateTime-(:offsetInSec)"
  )
