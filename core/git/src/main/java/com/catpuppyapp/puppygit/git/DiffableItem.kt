...
data class DiffableItem(
  ...
  companion object {
    ...
    fun anInvalidInstance(): DiffableItem {
      return DiffableItem(
          repoIdFromDb = "an_invalid_DiffableItem_30bc0f41-63e8-461a-b48b-415d5584740d"
      )
    }
  }
  ...
  override fun getItemKey(): String {
    ...
    return if (isChangeListItem)
        StatusTypeEntrySaver.generateItemKey(repoIdFromDb, relativePath, changeType, itemType)
    else FileHistoryDto.generateItemKey(commitId)
  }
  ...
  fun copyForLoading(): DiffableItem {
    ...
    return copy(
        loading = true,
        visible = true,
        stringPairMap = mutableStateMapOf(),
        compareLinePair = CompareLinePair(),
        submoduleIsDirty = false,
        errMsg = "",
        loadChannel = Channel(),
    )
  }
    ...
    return buildAnnotatedString {
      ...
      if (maybeLoadedAtLeastOnce()) {
        if (diffItemSaver.addedLines > 0) {
          withStyle(style = SpanStyle(color = Theme.mdGreen)) {
            append("+" + diffItemSaver.addedLines)
          }
          withStyle(style = SpanStyle(color = Theme.Gray1)) { append(", ") }
        }

        if (diffItemSaver.deletedLines > 0) {
          withStyle(style = SpanStyle(color = Theme.mdRed)) {
            append("-" + diffItemSaver.deletedLines)
          }
          withStyle(style = SpanStyle(color = Theme.Gray1)) { append(", ") }
        }
      }
      ...
      withStyle(style = SpanStyle(color = colorOfChangeType)) {
        append(fileParentPathOfRelativePath)
      }
    }
  }
