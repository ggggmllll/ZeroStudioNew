...
data class PackageNameAndRepoSettings(
  ...
  fun getPullIntervalActuallyValue(settings: AppSettings) =
      parseLongOrDefault(pullInterval, settings.automation.pullIntervalInSec) ?: 0L

  fun getPushDelayActuallyValue(settings: AppSettings) =
      parseLongOrDefault(pushDelay, settings.automation.pushDelayInSec) ?: 0L

  companion object {
    // if is not a valid number, save empty string to settings
    fun formatPullIntervalBeforeSaving(value: String) =
        parseLongOrDefault(value, null)?.toString() ?: ""

    fun formatPushDelayBeforeSaving(value: String) =
        parseLongOrDefault(value, null)?.toString() ?: ""
  }
}
