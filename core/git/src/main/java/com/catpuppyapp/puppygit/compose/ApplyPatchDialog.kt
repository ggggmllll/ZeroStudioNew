...
fun ApplyPatchDialog(
  ...
  ConfirmDialog2(
      okBtnEnabled =
          loadingRepoList.not() && repoList.isNotEmpty() && selectedRepo.value.id.isNotBlank(),
            ...
            Row(
              ...
              MySelectionContainer {
                if (hasErr) {
                  ...
                  Text(
                      stringResource(
                          if (loadingRepoList) R.string.loading else R.string.repo_list_is_empty
                      )
                  )
                }
              }
      ...
      try {
        Repository.open(selectedRepo.value.fullSavePath).use { repo ->
          ...
          */
          ...
          val ret =
              Libgit2Helper.applyPatchFromFile(
                  inputFile,
                  repo,
