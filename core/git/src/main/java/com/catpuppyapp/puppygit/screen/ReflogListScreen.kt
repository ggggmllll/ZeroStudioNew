...
fun ReflogListScreen(
  ...
  val curClickItem =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curClickItem",
          initValue = ReflogEntryDto(),
      )
  val curLongClickItem =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curLongClickItem",
          initValue = ReflogEntryDto(),
      )
  ...
  val list =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "list",
          initValue = listOf<ReflogEntryDto>(),
      )
  val allRefList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
  ...
  if (showCheckoutDialog.value) {
    val id =
        (if (checkoutNew.value) curLongClickItem.value.idNew else curLongClickItem.value.idOld)
            ?: Cons.git_AllZeroOid
  ...
  if (showResetDialog.value) {
    val id =
        (if (resetNew.value) curLongClickItem.value.idNew else curLongClickItem.value.idOld)
            ?: Cons.git_AllZeroOid
    ...
    if (id.isNullOrEmptyOrZero) { // id无效，关弹窗，显提示
      ...
      ResetDialog(
          ...
          refreshPage = { _, _, _ -> changeStateTriggerRefreshPage(needRefresh) },
      )
    }
  }
  ...
  if (showTitleInfoDialog.value) {
    RepoInfoDialog(
        prependContent = {
          Row { Text(stringResource(R.string.reference) + ": " + refName.value) }
        },
        curRepo = curRepo.value,
  ...
  Scaffold(
            ...
            title = {
              if (filterModeOn.value) {
                ...
                SimpleTitleDropDownMenu(
                    ...
                    titleSecondLineFormatter = {
                      Libgit2Helper.getRepoOnBranchOrOnDetachedHash(curRepo.value)
                    },
    ...
    PullToRefreshBox(
      ...
      if (showBottomSheet.value) {
        ...
        val title =
            Libgit2Helper.getShortOidStrByFull(
                (curLongClickItem.value.idOld ?: Cons.git_AllZeroOid).toString()
            ) +
                ".." +
                Libgit2Helper.getShortOidStrByFull(
                    (curLongClickItem.value.idNew ?: Cons.git_AllZeroOid).toString()
                )
      ...
      if (list.value.isEmpty()) {
        FullScreenScrollableColumn(contentPadding) {
          Text(
              stringResource(
                  if (isInitLoading.value) R.string.loading else R.string.item_list_is_empty
              )
          )
        }
      } else {
            ...
            filterTheList(
                ...
                match = { idx: Int, it: ReflogEntryDto ->
                  it.username.contains(keyword, ignoreCase = true) ||
                      ...
                      formatMinutesToUtc(it.originTimeZoneOffsetInMinutes)
        ...
        MyLazyColumn(
          ...
          ReflogItem(
              ...
              curLongClickItem,
              lastClickedItemKey,
              shouldShowTimeZoneInfo,
              it,
          ) { // onClick
            val suffix = "\n\n"
            val sb = StringBuilder()
            sb.append(activityContext.getString(R.string.new_oid))
                .append(": ")
                .append(it.idNew)
                .append(suffix)
            sb.append(activityContext.getString(R.string.old_oid))
                .append(": ")
                .append(it.idOld)
                .append(suffix)
            sb.append(activityContext.getString(R.string.date))
                .append(": ")
                .append(
                    it.date + " (${formatMinutesToUtc(it.actuallyUsingTimeZoneOffsetInMinutes)})"
                )
                .append(suffix)
            sb.append(activityContext.getString(R.string.timezone))
                .append(": ")
                .append(formatMinutesToUtc(it.originTimeZoneOffsetInMinutes))
                .append(suffix)
            sb.append(activityContext.getString(R.string.author))
                .append(": ")
                .append(Libgit2Helper.getFormattedUsernameAndEmail(it.username, it.email))
                .append(suffix)
