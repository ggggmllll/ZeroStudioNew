...
/**
 * 注意：这个组件会在LazyColumn的item里使用，所以不能用rememberSaveable， 否则有概率崩溃，报 "java.lang.ClassCastException"
 * ，是个bug，参见：https://issuetracker.google.com/issues/181880855
 *
...
fun DiffRow(
    ...
    //    reForEachDiffContent:()->Unit,  //这变量没什么用了，但暂且保留位置以免日后有用。之所以没用了是因为现在修改 pair
    // buffer后就会拷贝元素更新list，附带刷新效果了
  ...
  // 如果为null或空 或者 所有元素都是modified，则不使用string
  // part，否则使用。缺点：如果某行只有空格且类型为modified，则看不出到底有几个空格，因为整行都是绿色或红色的。
  val useStringPartList =
      !(stringPartList.isNullOrEmpty() || (stringPartList.indexOfFirst { it.modified.not() } == -1))
  ...
  val lineNum =
      paddingLineNumber(
          if (line.lineNum == LineNum.EOF.LINE_NUM) LineNum.EOF.TEXT else line.lineNum.toString(),
          lineNumExpectLength,
      )

  //    var prefix = ""
  val content = line.getContentNoLineBreak()
  // 我发现明明新旧都没末尾行，但是originType却是添加了末尾行 '>'， 很奇怪，所以把行相关的背景颜色改了，文字颜色一律灰色，另外，因为patch输出会包含 no new line
  // at end 之类的东西，所以不需要我再特意添加那句话了
  // 只显示新增换行符、删除换行符、新旧文件都没换行符、新增行、删除行、上下文
  //                    if (line.originType == Diff.Line.OriginType.DEL_EOFNL.toString() ||
  // line.originType == Diff.Line.OriginType.CONTEXT_EOFNL.toString()) { //新文件删除了换行符 和 新旧都没换行符
  ////                            prefix=line.originType+ ":"
  ////                            content = stringResource(R.string.no_new_line_at_end)
  // //不赋值的话，content什么都没有，用户看到的就是个带箭头(代表没新行的originType)的红色空行
  ...
  //    prefix = line.originType + lineNum + ":"  // show add or del and line num, e.g. "+123:" or
  // "-123:"
  //    var prefix = if(showOriginType) line.originType else ""
  //    prefix += (if(showLineNum) "$lineNum:" else ":")  // only show line num (can use color
  // figure add or del), e.g. "123:"

  val prefix =
      if (showOriginType && showLineNum.not()) {
        "${line.originType}:"
  ...
  //    val expandedMenu = mutableCustomStateOf(keyTag = stateKeyTag, keyName = "expandedMenu") {
  // false }
  ...
  val compareToClipboard =
      label@{
          content: String,
          line: PuppyLine,
          trueContentToClipboardFalseClipboardToContent: Boolean ->
        //        if(content.isEmpty()) {
        //
        ...
        if (line.originType == Diff.Line.OriginType.CONTEXT.toString()) {
          ...
          Msg.requireShowLongDuration(
              activityContext.getString(R.string.can_t_compare_clipboard_to_context_line)
          )
          return@label
        }
        ...
        doJobThenOffLoading {
          val newcp =
              if (trueContentToClipboardFalseClipboardToContent) { // content to clipboard
                CompareLinePair(
                    line1 = content,
                    line1OriginType = line.originType,
                    line1Num = line.lineNum,
  ...
  // this check include `originType` check, it is redundant actually, because when into this page,
  // the line originType always one of CONTEXT or ADDITION or DELETION,
  //  because ADD_EOFNL/DEL_EOFNL already trans to ADDITION/DELETION, and the CONTEXT_EOFNL is
  // deleted, will not shown
  ...
  Row(
      modifier =
          (if (lineClickable) {
              ...
              // alpha值越大越不透明
              //            .background(if (useStringPartList)
              // Libgit2Helper.getMatchedTextBgColorForDiff(inDarkTheme, line) else bgColor)
      ...
      ////                                    showToast(
      ////                                        appContext,
      ////                                        "$lineTypeStr,$oldLineAt:
      // ${line.oldLineNum},$newLineAt: ${line.newLineNum}",
      ////                                        Toast.LENGTH_SHORT
      ////                                    )
      //                            },
  ) {
    ...
    Row(
        ...
        modifier =
            (if (useStringPartList) Modifier else Modifier.background(bgColor))
                // require parent container set `Modifier.height(IntrinsicSize.Max)`, else
                // fillMaxHeight may not work
                .fillMaxHeight()

                // 首行加顶部padding，其余不加
                // x 已经解决，给每个行计算了虚拟的索引，然后就解决了）按行分组时，首行若是一对
                // add/del，两个都会加顶部padding，但看起来感觉并不难受，所以不用改，就这样吧
                .addTopPaddingIfIsFirstLine(index)
    ) {
      // show add/del and line number, e.g. +123, or only show line num e.g. 123, it should make a
      ...
      Text(
          ...
          fontFamily =
              PLFont
                  .codeFont, // line number always use mono font for align +-（行号总是使用等宽字体，不然+-符号对不齐）
          modifier =
              Modifier
    ...
    val contentModifier =
        Modifier
            // if fill max width, will not be able to saw the spaces at the end, because the
            // background will fill the whole line;
    ...
    if (useStringPartList) {
      SelectionRow(modifier = contentModifier) {
                ...
                try {
                  buildAnnotatedString {
                    obtainStylePartList()?.let { stylePartList ->
                      PuppyLine.mergeStringAndStylePartList(
                              stringPartList,
                              stylePartList,
                              bgColorSpanStyle,
                          )
                          .forEachBetter {
                            withStyle(it.style) { append(content.substring(it.start, it.end)) }
                          }
                    }
                  ...
                  MyLog.e(
                      TAG,
                      "DiffRow create substring err: lineNum=$lineNum, positionCode=1914714811075084, err=${e.localizedMessage}",
                  )
                  e.printStackTrace()

                  buildAnnotatedString { withStyle(bgColorSpanStyle) { append(content) } }
                },
      ...
      SelectionRow(modifier = Modifier.background(bgColor).then(contentModifier)) {
        // 文本内容
        Text(
            text =
                try {
                  ...
                  MyLog.e(
                      TAG,
                      "DiffRow create substring err: lineNum=$lineNum, positionCode=1855426513892273, err=${e.localizedMessage}",
                  )
                  e.printStackTrace()
      ...
      DisableSelection {
        DropdownMenu(
          ...
          if (enableLineEditActions) {
            ...
            // EOFNL status maybe wrong, before Edit or Del, must check it actually exists or is
            // not, when edit line num is EOF and EOFNL is not exists, then prepend a LineBreak
            // before users input
            // 编辑或删除前，如果行号是EOF，必须检查EOF
            // NL是否实际存在，如果EOFNL不存在，则先添加一个空行，再写入用户的实际内容，如果执行删除EOF且文件末尾无空行，则不执行任何删除；
            // 如果EOF为删除，则不用检查，点击恢复后直接在文件末尾添加一个空行即可
            if (
                line.originType == Diff.Line.OriginType.ADDITION.toString() ||
                  ...
                  Text(
                      if (line1ready)
                          replaceStringResList(
                              stringResource(R.string.compare_to_origintype_linenum),
                              listOf(cp.line1OriginType + cp.line1Num),
                          )
                      else {
                        stringResource(R.string.select_compare)
                      }
                  )
                },
                onClick = label@{
                      ...
                      if (line1ready) {
                        ...
                        // (20241114 change to no check, force re-compare, cause sometimes, maybe "a
                        // compare to b" has difference result with "b compare to a")
                        // same line num already compared in normal procudure
                        //                                    if(line.lineNum == cp.line1Num && (
                        //                                                (line.originType ==
                        // OriginType.ADDITION.toString() && cp.line1OriginType ==
                        // OriginType.DELETION.toString())
                        //                                                        ||
                        // (line.originType == OriginType.DELETION.toString() && cp.line1OriginType
                        ...
                        if (
                          ...
                          Msg.requireShow(
                              activityContext.getString(
                                  R.string.can_t_compare_both_context_type_lines
                              )
                          )
                          return@label
                        }
                        ...
                        Msg.requireShow(
                            replaceStringResList(
                                activityContext.getString(R.string.added_line_for_compare),
                                listOf(line.originType + lineNum),
                            )
                        )
                      }
                    },
