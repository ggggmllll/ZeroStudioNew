...
fun TagListScreen(
  ...
  // 获取假数据
  val list =
      mutableCustomStateListOf(keyTag = stateKeyTag, keyName = "list", initValue = listOf<TagDto>())

  val filterList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "filterList",
          initValue = listOf<TagDto>(),
      )
  ...
  val selectedItemList =
      mutableCustomStateListOf(keyTag = stateKeyTag, keyName = "selectedItemList", listOf<TagDto>())
  ...
  val isItemInSelected = { item: TagDto -> selectedItemList.value.contains(item) }
  // 多选模式相关函数，结束
  ...
  // 初始化 checkout对话框
  val initCheckoutDialogComposableVersion = { showCheckoutDialog.value = true }
  ...
  val remoteList =
      mutableCustomStateListOf(keyTag = stateKeyTag, keyName = "remoteList", listOf<String>())
  val selectedRemoteList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
  ...
  if (showTagFetchPushDialog.value) {
    TagFetchPushDialog(
        ...
        enableOk =
            if (requireDel.value) true
            else
                selectedRemoteList.value
                    .isNotEmpty(), // 如果是删除模式，可能只删本地也可能删本地和远程，而显示此弹窗有必须至少选中一个条目的前置判断，所以执行到这里一律启用ok即可；如果是fetch/push，则必须至少选一个remote，否则禁用ok
        ...
        onSuccess = { Msg.requireShow(activityContext.getString(R.string.success)) },
        onErr = { e ->
          ...
          MyLog.e(
              TAG,
              "#TagFetchPushDialog onOK error when '${fetchPushDialogTitle.value}': ${e.stackTraceToString()}",
          )
        },
        ...
        pushFailedListHandler = { pushFailedList ->
          ...
          pushFailedList.forEachBetter {
            ...
            repoLogMsg
                .append("remoteName='${it.remoteName}', err=${it.exception?.localizedMessage}")
                .append(spliter)
            logMsg
                .append("remoteName='${it.remoteName}', err=${it.exception?.stackTraceToString()}")
                .append(spliter)
          }
          ...
          createAndInsertError(
              curRepo.value.id,
              repoLogMsg.removeSuffix(spliter).toString() + "\n\n",
          )
  ...
  val showDetails = { selectedItemList: List<TagDto> ->
    ...
    selectedItemList.forEachBetter {
      ...
      sb.append(activityContext.getString(R.string.full_name))
          ...
          .append(it.name)
          .append(itemSuffix)
      sb.append(activityContext.getString(R.string.target))
          .append(": ")
          .append(it.targetFullOidStr)
          .append(itemSuffix)
      sb.append(activityContext.getString(R.string.type))
          .append(": ")
          .append(it.getType(activityContext, false))
          .append(itemSuffix)

      sb.append(Cons.flagStr)
          .append(": ")
          .append(it.getType(activityContext, true))
          .append(itemSuffix)

      if (it.isAnnotated) {
        sb.append(activityContext.getString(R.string.tag_oid))
            .append(": ")
            .append(it.fullOidStr)
            .append(itemSuffix)
        sb.append(activityContext.getString(R.string.author))
            .append(": ")
            .append(it.getFormattedTaggerNameAndEmail())
            .append(itemSuffix)
        sb.append(activityContext.getString(R.string.date))
            .append(": ")
            .append(it.getFormattedDate() + " (${it.getActuallyUsingTimeOffsetInUtcFormat()})")
            .append(itemSuffix)
        sb.append(activityContext.getString(R.string.timezone))
            .append(": ")
            .append(formatMinutesToUtc(it.originTimeOffsetInMinutes))
            .append(itemSuffix)
        sb.append(activityContext.getString(R.string.msg))
            .append(": ")
            .append(it.msg)
            .append(itemSuffix)
      }
      ...
      if (list.value.isEmpty()) { // 无条目，显示可创建或fetch
        FullScreenScrollableColumn(contentPadding) {
          if (isInitLoading.value) {
            ...
            Row { Text(text = stringResource(R.string.no_tags_found)) }
        ...
        if (multiSelectionMode.value) {
          ...
          val moreItemTextList =
              (listOf(
                  ...
                  // //可针对单个或多个条目查看details，多个时，用分割线分割多个条目的信息
              ))
          ...
          val iconOnClickList: List<() -> Unit> =
              listOf( // index页面的底栏选项
                  delete@{ initDelTagDialog() },
                  push@{ initPushTagDialog() },
                  details@{ showDetails(selectedItemList.value) },
                  selectAll@{
          ...
          val moreItemOnClickList: List<() -> Unit> =
              (listOf(
                  checkout@{ initCheckoutDialogComposableVersion() },
                  hardReset@{
        ...
        if (!repoId.isNullOrBlank()) {
          ...
          if (repoFromDb != null) {
            ...
            Repository.open(repoFromDb.fullSavePath).use { repo ->
              val tags =
                  Libgit2Helper.getAllTags(repoId, repo, settings).sortedByDescending {
