...
data class NameAndPath(
  ...
  companion object {
    fun genByPath(path: String, type: NameAndPathType, context: Context): NameAndPath {
      val name =
          if (
              type == NameAndPathType.REPOS_STORAGE_PATH ||
                  type == NameAndPathType.FIRST_REPOS_STORAGE_PATH
          ) {
    ...
    suspend fun getListForFilesManager(
      ...
      doActWithLockIfFree(lock, "NameAndPath#getListForFiles()") {
        ...
        newList.add(NameAndPath(StrCons.appData, FsUtils.getAppDataRootPathNoEndsWithSeparator()))
        ...
        if (AppModel.devModeOn) {
          ...
          AppModel.externalDataDir?.canonicalPath?.let {
            newList.add(NameAndPath(DevFeature.external_data_storage, it))
          }
        }

        // storage path
        newList.addAll(
            StoragePathsMan.get().storagePaths.mapIndexed { idx, it ->
              genByPath(
                  it,
                  if (idx == 0) NameAndPathType.FIRST_REPOS_STORAGE_PATH
                  else NameAndPathType.REPOS_STORAGE_PATH,
                  context,
              )
            }
        )
        ...
        newList.addAll(
            repoDb.getAll(updateRepoInfo = false).mapIndexed { idx, it ->
              NameAndPath(
                  it.repoName,
                  it.fullSavePath,
                  if (idx == 0) NameAndPathType.FIRST_REPO_WORKDIR_PATH
                  else NameAndPathType.REPO_WORKDIR_PATH,
              )
            }
        )
