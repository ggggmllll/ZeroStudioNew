...
object AppModel {
  ...
  // 废弃，改在compose里用 `SharedState.getCurrentConfigure()` 获取当前配置再获取屏幕宽高，那样获取的是响应式的
  // （参考案例：仓库页面，旋转屏幕后会根据屏幕宽度重新计算每行条目数）
  ...
  /**
   * long long ago, this is applicationContext get from Activity, but now, this maybe is baseContext
   * of Activity, baseContext bundled with Activity, save it's reference may cause memory leak;
   * applicationContext bundled with App (process maybe?), save it's reference more time is safe,
   * but it can't get properly resources in some cases, e.g. when call context.getString(),
   * baseContext can get string resource with correct language, but applicationContext maybe can't,
   * that's why I save baseContext rather than applicationContext
   *
   * update this reference in Activity#onCreate can reduce risk of mem leak, but maybe still will
   * make mem clean delay than usual
   *
   * now , actually this is Activity's Context, not the App
   */
  //    @Deprecated("use `LocalContext.current` instead, but this already many usages, so, keep it
  // for now")
  //    lateinit var activityContext:Context

  /**
   * real the App context, not activity, this may not be get strings resource with expect language,
   * but for show Toast or load raw resource stream, is fine
   */
  ...
  // //这个scope是全局的，生命周期几乎等于app的生命周期(？有待验证，不过因为是在根Compose创建的所以多半差不多是这样)，如果要执行和当前compose生命周期一致的任务，应该用
  // rememberCoroutineScope() 在对应compose重新获取一个scope

  lateinit var navController: NavHostController

  /** 系统时区偏移量，单位: 分钟 注意这是系统偏移量，不一定等于App实际使用的时区偏移量！ */
  ...
  /** key 分钟数 value UTC时区例如：UTC+8 UTC-7:30 UTC+0 */
  ...
  /**
   * 路径： app实际使用的私有目录/cache 父目录同 externalFilesDir，只是这个是cache目录，那个是files目录
   *
   * 必须存在
   */
  ...
  // 内部StorageDir存储目录，所有类型为“内部”的StorageDir都存储在这个路径下，默认在用户空间 Android/data/xxxxxx包名/files/StorageDirs
  // 路径。里面默认有之前的 allRepoParentDir 和 LogData 目录，且这两个目录不能删除
  ...
  // 20250218:
  // 禁用此变量，日后可能会删除相关代码。原因：这个debugModeOn最初是用来控制日志等级的，确保我在开发的时候能打印些调试信息到log文件，后来，设置页面有了专门控制日志等级的设置项，还加了个开发模式，此变量已经没用了
  // 外部应通过获取此文件来判断是否开启debug模式并通过DebugModeManager的set方法维护此变量和debugModeFlagFile
  // 此变量为true，则设置页面的debug模式状态为开启；false则为关闭。注：设置页面的debug模式开启或关闭仅与此变量有关，与debug
  // flag文件是否存在无关。例如：用户打开了debug模式，app创建了debug
  // flag文件，但随后，用户手动删除了flag文件，这时设置页面debug模式仍为开启，直到下次启动app时才会更新为关闭
  //    @Deprecated("use `DevFlag.isDebugModeOn()` instead")
  //    var debugModeOn = false  //这个变量改成让用户可配置，所以弄成变量，放到init_1里初始化，然后后面的日志之类的会用到它
  //        private set
  // //应通过AppModel.DebugModeManager修改debugModeOn的值，那个方法可持久化debug模式开启或关闭，也可临时设置此变量，一举两得
  ...
  /**
   * TODO 实现避免重复执行init的机制，并且如果app崩溃后recreate Activity，确保代码能继续正常工作。 TODO
   * 啊，对了，注意appContext这变量肯定是要在recreate Activity后重新赋值的，如果实现避免重入init，需要考虑哪些需要在recreate时重新赋值，哪些不需要
  ...
  /** 执行必须且无法显示界面的操作。 中量级，应该不会阻塞很久 */
  fun init_1(realAppContext: Context, exitApp: () -> Unit, initActivity: Boolean) {
    //        val funName = "init_1"

    // run once in app process life time start
    // （这些执行一次也行，多次也行，执行多次吧，不然如果开了辅助服务，就算从最近appp列表划掉app也不会杀进程，会导致某些状态不会更新）
    // 加载libgit2等库
    LibLoader.load()
    ...
    // disable dirs owner validation for libgit2, make it support access external storage path like
    // /sdcard or /storage/emulated/storage
    ...
    DevFeature.showRandomLaunchingText.state.value =
        PrefUtil.getShowRandomLaunchingText(realAppContext)
    ...
    //        }
    //            AppModel.mainActivity = mainActivity
    // //忘了这个干嘛的了，后来反正没用了，IDE提示什么Activity内存泄漏之类的，所以就注释了
    ...
    //            AppModel.logDir = createLogDirIfNonexists(externalCacheDir,
    // Cons.defaultLogDirName);

    // 20240527：禁用，sd相关 ，开始
    //            AppModel.internalStorageDirsParentDir = createDirIfNonexists(externalFilesDir,
    // Cons.defaultInternalStorageDirsParentDirName)

    // 设置repodir
    //            AppModel.allRepoParentDir =
    // createDirIfNonexists(AppModel.internalStorageDirsParentDir,
    // StorageDirCons.DefaultStorageDir.repoStorage1.name)
    //            StorageDirCons.DefaultStorageDir.repoStorage1.fullPath =
    // AppModel.allRepoParentDir.canonicalPath
    //
    //            //设置对用户可见的app工作目录
    //            AppModel.appDataUnderAllReposDir =
    // createDirIfNonexists(AppModel.internalStorageDirsParentDir,
    // StorageDirCons.DefaultStorageDir.puppyGitDataDir.name)
    //            StorageDirCons.DefaultStorageDir.puppyGitDataDir.fullPath =
    // AppModel.appDataUnderAllReposDir.canonicalPath
    // 20240527：禁用，sd相关 ，结束

    // 与sd相关代码互斥，开始
    // 设置repodir
    AppModel.allRepoParentDir =
        createDirIfNonexists(externalFilesDir, Cons.defaultAllRepoParentDirName)
    // test access external storage, passed
    //            AppModel.allRepoParentDir = createDirIfNonexists(File("/sdcard"),
    // "puppygit-repos")

    StorageDirCons.DefaultStorageDir.puppyGitRepos.fullPath =
        AppModel.allRepoParentDir.canonicalPath

    // 设置对用户可见的app工作目录
    AppModel.appDataUnderAllReposDir =
        createDirIfNonexists(
            AppModel.allRepoParentDir,
            Cons.defalutPuppyGitDataUnderAllReposDirName,
        )
    // 与sd相关代码互斥，结束
    ...
    */
    ...
    // 存放app内置证书的路径
    AppModel.certBundleDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, CertMan.defaultCertBundleDirName)
    AppModel.certUserDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, CertMan.defaultCertUserDirName)

    AppModel.fileSnapshotDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, Cons.defaultFileSnapshotDirName)
    // 创建editor cache目录
    AppModel.editCacheDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, Cons.defaultEditCacheDirName)

    // 创建git pathch 导出目录
    AppModel.patchDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, Cons.defaultPatchDirName)

    // create settings folder
    AppModel.settingsDir =
        createDirIfNonexists(AppModel.appDataUnderAllReposDir, Cons.defaultSettingsDirName)

    AppModel.submoduleDotGitBackupDir =
        createDirIfNonexists(
            AppModel.appDataUnderAllReposDir,
            Cons.defaultSubmoduleDotGitFileBakDirName,
        )
    ...
    // 设置文件快照目录
    //            AppModel.fileSnapshotDir =
    // createFileSnapshotDirIfNonexists(AppModel.allRepoParentDir, Cons.defaultFileSnapshotDirName)
    ...
    //        AppModel.debugModeFlagFile = File(AppModel.appDataUnderAllReposDir,
    // DebugModeManager.debugModeFlagFileName)
    // //debugMode检测模式是如果在特定目录下存在名为`debugModeFlagFileName`变量值的文件，则debugModeOn，否则off
    ...
    // 在设置页面添加相关选项“开启调试模式”，开启则在上面的目录创建debugModeOn文件，否则删除文件，这样每次启动app就能通过检查文件是否存在来判断是否开了debugMode了。(btw: 因为要在Settings初始化之前就读取到这个变量，所以不能放到Settings里)

    // for test unstable features
    dev_EnableUnTestedFeature =
        try {
  ...
  fun updateExitApp(exit: () -> Unit) {
    AppModel.exitApp = exit
  }

  /** 执行必须但已经可以显示界面的操作，所以这时候可以看到开发者设置的loading页面了，如果有的话。 可重可轻，有可能阻塞很久 */
  suspend fun init_2() {
    val funName = "init_2"
    ...
    /*
      init settings
    */
    //            val settingsSaveDir = AppModel.innerDataDir  // deprecated, move to use-visible
    // puppygit-data folder
    ...
    try {
      ...
      try {
        ...
        MyLog.w(
            TAG,
            "#$funName init bak settings err, will clear origin settings, user settings will lost!",
        )
        ...
        MyLog.w(
            TAG,
            "#$funName del settings success, will reInit settings, if failed, app will not work...",
        )
    ...
    DevFeature.treatNoWordMatchAsNoMatchedForDiff.state.value =
        settings.devSettings.treatNoWordMatchAsNoMatchedForDiff
    DevFeature.degradeMatchByWordsToMatchByCharsIfNonMatched.state.value =
        settings.devSettings.degradeMatchByWordsToMatchByCharsIfNonMatched
    DevFeature.showMatchedAllAtDiff.state.value = settings.devSettings.showMatchedAllAtDiff
    DevFeature.legacyChangeListLoadMethod.state.value =
        settings.devSettings.legacyChangeListLoadMethod
    // 结束：初始化开发者功能设置项
    ...
    //                    MyLog.w(TAG, "#$funName migrate password err, user's password may will be
    // invalid :(")
    //                }
    ...
    try {
      SnapshotUtil.init(
          enableContentSnapshotForEditorInitValue =
              settings.editor.enableContentSnapshot ||
                  File(AppModel.appDataUnderAllReposDir, FlagFileName.enableContentSnapshot)
                      .exists(),
          enableFileSnapshotForEditorInitValue =
              settings.editor.enableFileSnapshot ||
                  File(AppModel.appDataUnderAllReposDir, FlagFileName.enableFileSnapshot).exists(),
    ...
    try {
      ...
      //                val requireClearSettingsEditedHistory =
      // settings.editor.filesLastEditPosition.isNotEmpty()
    ...
    doJobThenOffLoading {
      ...
      try {
        ...
        //                 //   AppModel.getOrCreateFileSnapshotDir()  // is delete expired files,
        // is not del the folder, so no need call this make sure folder exist
      } catch (e: Exception) {
    ...
    AppVersionMan.init migrate@{ oldVer ->
      ...
      if (
          oldVer == AppVersionMan.err_fileNonExists ||
              oldVer == AppVersionMan.err_parseVersionFailed
      ) {
        return@migrate true
      }
      ...
      // 不是最新版本，执行迁移, if ver==1 do sth, else if ==2, do sth else ...
      // 最好用try...catch包裹，并且将迁移代码设置为幂等的，这样出错可再次重新调用
  ...
  fun init_3() {
    ...
    // //上推隐藏，下拉出现，TopAppBarState 可放到外部以保存状态，如果需要的话
  ...
  fun getAppIcon(context: Context, inDarkTheme: Boolean = Theme.inDarkTheme): ImageVector {
    //        return
    // context.packageManager.getApplicationIcon(appPackageName).toBitmap().asImageBitmap()
    return ImageVector.vectorResource(
        if (inDarkTheme) R.drawable.icon_dark_foreground else R.drawable.icon_light_foreground
    )
  }
   ...
   *
   * 快照目录路径：allRepoDir/PuppyGit-Data/FileSnapshot,
   * 目前(20240421)仅当文件保存失败且内存中的文件不为空时，会把文件存到这个目录，日后可能扩展使用范围
   */
  ...
  fun requireMasterPassword(settings: AppSettings = SettingsUtil.getSettingsSnapshot()): Boolean {
    ...
    // 用户输入的主密码和密码hash不匹配
    ||
        (settings.masterPasswordHash.isNotEmpty() &&
            masterPassword.value.isNotEmpty() &&
            !HashUtil.verify(masterPassword.value, settings.masterPasswordHash)))
  }
  ...
  /**
   * 获取App使用的时区ZoneOffset对象。 注意，这个是App实际使用的时区对象，并不是系统时区对象，若在设置页面为App指定了时区，系统和App使用的时区可能会不同
   *
   * @param settings
   *   一般不用传此参数，只有当更新过AppSettings但不确定SettingsUtil.getSettingsSnapshot()能否立刻获取到最新值时，才有必要传，传的一般是SettingsUtil.update(requireReturnUpdatedSettings=true)的返回值
   */
  ...
  fun reloadTimeZone(settings: AppSettings) {
    ...
    systemTimeZoneOffsetInMinutes =
        try {
          ...
          MyLog.e(
              TAG,
              "#reloadTimeZone() get system timezone offset in minutes err, will use UTC+0, err is: ${e.stackTraceToString()}",
          )
          // offset = 0, 即 UTC+0
          0
        }
    ...
    MyLog.d(
        TAG,
        "#reloadTimeZone(): new value of systemTimeZoneOffsetInMinutes=$systemTimeZoneOffsetInMinutes",
    )

    // 注：这里不能调用getSystemTimeZoneOffsetInMinutesCached，因为如果那个方法如果无结果时会调用此方法查询，若出bug，就死循环了
    // 更新App实际使用的时区对象
    timeZoneOffsetInMinutes =
        readTimeZoneOffsetInMinutesFromSettingsOrDefault(settings, systemTimeZoneOffsetInMinutes!!)
    ...
    MyLog.d(
        TAG,
        "#reloadTimeZone(): new value of App TimeZone: timeZoneMode=$timeZoneMode, timeZoneOffsetInMinutes=$timeZoneOffsetInMinutes, timeZoneOffset=$timeZoneOffset",
    )
  }
  ...
  // start: device configuration (include width/height, and rotate screen or do other actions will
  // update it)
