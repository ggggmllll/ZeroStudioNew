...
fun EditorInnerPage(
    ...
    editorFilterRecentListOn:
        MutableState<
            Boolean
        >, // filter on but may haven't a valid keyword, so actually not enabled filter
    editorEnableRecentListFilter:
        MutableState<Boolean>, // indicate filter mode actually enabled or not
    ...
    goToLine: Int =
        LineNum
            .lastPosition, // 若大于0，打开文件后跳转到指定行；否则还是旧逻辑（跳转到上次退出前的第一行）。SubPageEditor会用到此变量，HomeScreen跳转来Editor的话，目前用不到，以后若有需要再传参
  ...
  // 这样可在修改设置项后立刻生效
  val settings =
      remember(isPreviewModeOn.value) {
  ...
  val recentFilesLimit =
      remember(settings.editor.recentFilesLimit) { settings.editor.recentFilesLimit }

  val saveLock =
      remember(editorPageShowingFilePath.value.ioPath) {
        Cache.getSaveLockOfFile(editorPageShowingFilePath.value.ioPath)
      }
  //    val saveLock = Mutex()
  // //其实这样也行，不过根据路径创建锁更严谨，跨页面也适用，比如如果首页的Editor正在保存，然后打开子页面，这时子页面必须等首页保存完成，但如果用这个和页面生命周期一样的锁，就无法实现那种效果了，但和页面生命周期一样的锁其实也够用

  //    val isEdited = rememberSaveable{ mutableStateOf(false) }

  // BasicTextField用的变量
  //    val editorPageShowingFileText = rememberSaveable{ mutableStateOf("") }
  // //BasicTextField用的文本，用来存储打开的文件的所有内容
  //    val editorPageEditorFocusRequester = remember{ FocusRequester() }
  // //BasicTextField用的focusRequester
  //    val lastFilePath = StateUtil.getRememberSaveableState(initValue = "")
  val editorPageShowingFileHasErr = rememberSaveable {
    mutableStateOf(false)
  } // BasicTextField用的文本，用来存储打开的文件的所有内容
  val editorPageShowingFileErrMsg = rememberSaveable {
    mutableStateOf("")
  } // BasicTextField用的文本，用来存储打开的文件的所有内容
  ...
  // save last cursor at column when navigate line with keyboard Down/Up, will ignore less value
  // than current, and will reset when open file
  ...
  val resetLastCursorAtColumn = { lastCursorAtColumn.value = 0 }
  val getLastCursorAtColumnValue = { lastCursorAtColumn.value }
  ...
  //    val editorPageOpenedFileMap = rememberSaveable{ mutableStateOf("{}") }
  // //{canonicalPath:fileName}

  //    val editorPageShowingFileCanonicalPath = rememberSaveable{ mutableStateOf("") }
  // //当前展示的文件的真实路径
  ...
  val hasError = { editorPageShowingFileHasErr.value }
  ...
  val saveFontSizeAndQuitAdjust = {
    ...
    if (editorLastSavedFontSize.intValue != editorFontSize.intValue) {
      ...
      SettingsUtil.update { it.editor.fontSize = editorFontSize.intValue }
    }
  ...
  val saveLineNumFontSizeAndQuitAdjust = {
    ...
    if (editorLastSavedLineNumFontSize.intValue != editorLineNumFontSize.intValue) {
      ...
      SettingsUtil.update { it.editor.lineNumFontSize = editorLineNumFontSize.intValue }
    }
  ...
  val fileChangeListenerState =
      rememberFileChangeListenerState(editorPageShowingFilePath.value.ioPath)

  editorOpenFileErr.value =
      remember {
            derivedStateOf {
              editorPageShowingFileHasErr.value && !editorPageShowingFileIsReady.value
            }
          }
          .value

  val needAndReadyDoSave: () -> Boolean = {
    isEdited.value &&
        !readOnlyMode.value &&
        editorPageTextEditorState.value.fieldsId != lastSavedFieldsId.value
  } // 因为用了锁，只要拿到锁，肯定别人没在保存，所以无需判断isSaving，只判断文件是否编辑过即可，若没编辑过，说明之前保存成功后没动过，否则，应保存文件
    ...
    doJobThenOffLoading {
      saveLock.withLock {
        if (needAndReadyDoSave()) {
          ...
          MyLog.w(
              TAG,
              "#doSaveInCoroutine: will not save file, cause maybe other job already saved or saving",
          )
        }
      }
    }
  ...
  val doSaveNoCoroutine = suspend {
    ...
    saveLock.withLock {
      if (needAndReadyDoSave()) {
        ...
        MyLog.w(
            TAG,
            "#doSaveNoCoroutine: will not save file, cause maybe other job already saved or saving",
        )
      }
    }
  }
  ...
  val doSimpleSafeFastSaveInCoroutine =
      {
          requireShowMsgToUser: Boolean,
            ...
            if (needAndReadyDoSave()) {
              try {
                ...
                if (ret.success()) {
                  ...
                  editorPageShowingFileDto.value =
                      FileSimpleDto.genByFile(
                          editorPageShowingFilePath.value.toFuckSafFile(activityContext)
                      )
              ...
              MyLog.w(
                  TAG,
                  "#doSimpleSafeFastSaveInCoroutine: will not save file, cause maybe other job already saved or saving",
              )
            }
          }
        }
  ...
  val saveLastOpenPath = { path: String ->
    if (path.isNotBlank() && lastFilePath.value != path) {
      ...
      // 更新配置文件中记录的最后打开文件
      SettingsUtil.update { it.editor.lastEditedFilePath = path }
    }
  }
  ...
  val closeFile = {
    ...
    doJobThenOffLoading { undoStack.reset("", force = true) }
  ...
  val reloadFile = r@{ force: Boolean ->
    ...
    if (force) {
      ...
      try {
        val newDto =
            FileSimpleDto.genByFile(FuckSafFile(activityContext, FilePath(requireOpenFilePath)))
        val oldDto = editorPageShowingFileDto.value

        if (newDto == oldDto) {
          MyLog.d(
              TAG,
              "EditorInnerPage#reloadFile(force=false): file may not changed, skip reload, file path is '${requireOpenFilePath}'",
          )
          // 文件可能没改变，放弃加载
        ...
        MyLog.d(
            TAG,
            "EditorInnerPage#reloadFile(force=false): check file changes err, will reload file '${requireOpenFilePath}', err=${e.stackTraceToString()}",
        )
      }
    }
  ...
  val showInFiles = { path: FilePath ->
    if (path.ioPathType == PathType.ABSOLUTE) { // '/'开头的绝对路径
      ...
      Msg.requireShowLongDuration(
          activityContext.getString(R.string.file_path_invalid) + ": " + path.ioPath
      )
    }
  }
  ...
  val getTheLastOpenedFilePath = {
    ...
    lastFilePath.value.ifBlank { SettingsUtil.getSettingsSnapshot().editor.lastEditedFilePath }
  }
  ...
  if (showReloadDialog.value) {
    ...
    if (!needAndReadyDoSave()) {
      ...
      val newDto =
          FileSimpleDto.genByFile(editorPageShowingFilePath.value.toFuckSafFile(activityContext))

      if (newDto != editorPageShowingFileDto.value) {
        val fileName = editorPageShowingFileDto.value.name
        MyLog.d(
            TAG,
            "#showReloadDialog: file '${fileName}' may changed by external, will save content snapshot before reload",
        )
        ...
        doJobThenOffLoading {
          ...
          if (snapRet.hasError()) {
            MyLog.e(
                TAG,
                "#showReloadDialog: save content snapshot before reload, err: " + snapRet.msg,
            )
          }
        }
      }
  ...
  val editorAsUnsaved = {
    codeEditor.value.doActWithLatestEditorStateInCoroutine("#asUnsaved") { it.asUnsaved() }
  }
  ...
  if (showBackFromExternalAppAskReloadDialog.value) {
    OpenAsAskReloadDialog(
      ...
      if (newDto != editorPageShowingFileDto.value) {
        ...
        MyLog.d(
            TAG,
            "#showBackFromExternalAppAskReloadDialog: file '${fileName}' may changed by external, will save content snapshot before reload",
        )
        //                val content = editorPageTextEditorState.value.getAllText()
        //                val content = null
        val editorState = editorPageTextEditorState.value
        doJobThenOffLoading {
          val snapRet =
              SnapshotUtil.createSnapshotByContentAndGetResult(
                  ...
                  flag =
                      SnapshotFileFlag
                          .editor_content_BeforeReloadFoundSrcFileChanged_ReloadByBackFromExternalDialog,
              )
          if (snapRet.hasError()) {
            MyLog.e(
                TAG,
                "#showBackFromExternalAppAskReloadDialog: save content snapshot before reload, err: " +
                    snapRet.msg,
            )
          }
        }
  ...
  // `derivedStateOf` can auto capture state values, so, don't have to pass the `key` explicitly
  // actually, but pass is ok as well
  val openAsDialogFileName =
      remember(openAsDialogFilePath.value) {
  ...
  val previewLoadingOn = { previewLoading.value = true }
  val previewLoadingOff = { previewLoading.value = false }
  ...
  val previewNavBack = {
    runBlocking {
      ...
      if (last == null) {
        ...
        requestFromParent.value =
            if (isSubPageMode) PageRequest.requireInitPreviewFromSubEditor
            else PageRequest.requireInitPreview
      }
    }
  ...
  val previewLinkHandler: (link: String) -> Boolean = { link ->
    if (FsUtils.maybeIsRelativePath(link)) {
      ...
      runBlocking {
        ...
        val linkFullPath =
            FsUtils.getAbsolutePathIfIsRelative(
                path = link,
                basePathNoEndSlash = FsUtils.getParentPath(previewingFileFullPath),
            )

        val aheadFirstPath = previewNavStack.getAheadStackFirst()?.path
        if (
            aheadFirstPath == linkFullPath
        ) { // 当前条目已经在ahead栈中，不需要再push。这种可能是后退过再点击和前进目标相同的链接，就会导致两个路径一样
          previewNavAhead()
          true
        } else { // 当前条目不在栈中，push
          ...
          if (pushSuccess) {
            ...
            // push failed, maybe is not a valid relative path, then return false to let default
            // link handler to take it
            false
          }
        }
      }
  ...
  val updatePreviewDto = { previewPath: String ->
    editorPreviewFileDto.value =
        FileSimpleDto.genByFile(FuckSafFile(activityContext, FilePath(previewPath)))
  }
  ...
  val refreshPreviewPageNoCoroutine = { previewPath: String, force: Boolean ->
    val needRefresh =
        if (force) {
          ...
          if (newDto == oldDto) {
            MyLog.d(
                TAG,
                "EditorInnerPage#refreshPreviewPageNoCoroutine: file may not changed, skip reload, file path is '${previewPath}'",
            )
            false
          } else {
            true
    ...
    // 本来想顺便更新下editor
    // state，太麻烦，那个editor的fields得处理，不能简单读下文件再赋值，而且退出preview模式已经做了重载检测，所以顶多这里载入一次，返回编辑模式再载入一次，可接受，不改了。

  }

  val refreshPreviewPage = { previewPath: String, force: Boolean ->
    doJobThenOffLoading(loadingOn = { previewLoadingOn() }, loadingOff = { previewLoadingOff() }) {
  ...
  val loadingOffForRecentFileList = { loadingRecentFiles.value = false }

  val needRefreshRecentFileList = rememberSaveable { mutableStateOf("") }
  val reloadRecentFileList = { changeStateTriggerRefreshPage(needRefreshRecentFileList) }

  val tabIndentSpacesCount = rememberSaveable {
    mutableStateOf(SettingsUtil.editorTabIndentCount())
  }
  ...
  val initSetTabSizeDialog = {
    tabSizeBuf.value =
        tabIndentSpacesCount.value.toString().let {
          TextFieldValue(it, selection = TextRange(0, it.length))
        }
  ...
  if (showSetTabSizeDialog.value) {
    ...
    SetTabSizeDialog(
        ...
        onOk = ok@{ newSize ->
              ...
              SettingsUtil.update { it.editor.tabIndentSpacesCount = newSize }
            },
    )
  }
    ...
    SelectEncodingDialog(
      ...
      if (newCharset != editorCharset.value) {
        if (isConvertEncoding.value) { // convert encoding when write content to file
          ...
          initReloadDialogWithCallback { editorCharset.value = newCharset }
        }
      }
    }
  }
  ...
  if (requestFromParent.value == PageRequest.showLineBreakDialog) {
    PageRequest.clearStateThenDoAct(requestFromParent) { showLineBreakDialog.value = true }
  }

  if (requestFromParent.value == PageRequest.showSelectEncodingDialog) {
    PageRequest.clearStateThenDoAct(requestFromParent) { initSelectEncodingDialog(false) }
  }

  if (requestFromParent.value == PageRequest.convertEncoding) {
    PageRequest.clearStateThenDoAct(requestFromParent) { initSelectEncodingDialog(true) }
  }

  if (requestFromParent.value == PageRequest.selectSyntaxHighlighting) {
    PageRequest.clearStateThenDoAct(requestFromParent) { initSelectSyntaxHighlightingDialog() }
  }

  if (requestFromParent.value == PageRequest.showSetTabSizeDialog) {
    PageRequest.clearStateThenDoAct(requestFromParent) { initSetTabSizeDialog() }
  }

  if (requestFromParent.value == PageRequest.reloadRecentFileList) {
    PageRequest.clearStateThenDoAct(requestFromParent) { reloadRecentFileList() }
  }
  ...
  if (requestFromParent.value == PageRequest.requireClose) {
    PageRequest.clearStateThenDoAct(requestFromParent) { showCloseDialog.value = true }
  }

  if (requestFromParent.value == PageRequest.editorPreviewPageGoBack) {
    PageRequest.clearStateThenDoAct(requestFromParent) { previewNavBack() }
  }

  if (requestFromParent.value == PageRequest.editorPreviewPageGoForward) {
    PageRequest.clearStateThenDoAct(requestFromParent) { previewNavAhead() }
  }
  ...
  if (
      requestFromParent.value == PageRequest.requireInitPreview ||
      ...
      doJobThenOffLoading {
        ...
        if (
            switchFromEditPage &&
                ...
                previewNavStack.backStackOrAheadStackIsNotEmpty()
        ) {
        ...
        mdText.value =
            if (pathWillPreview == editorPageShowingFilePath) {
              curPreviewFileUsedCharset.value = editorCharset.value
              editorPageTextEditorState.value.getAllText()
            } else {
              val encoding =
                  EncodingUtil.detectEncoding(newInputStream = { FileInputStream(pathWillPreview) })
              curPreviewFileUsedCharset.value = encoding
  ...
  if (requestFromParent.value == PageRequest.requireSaveFontSizeAndQuitAdjust) {
    PageRequest.clearStateThenDoAct(requestFromParent) { saveFontSizeAndQuitAdjust() }
  }
  if (requestFromParent.value == PageRequest.requireSaveLineNumFontSizeAndQuitAdjust) {
    PageRequest.clearStateThenDoAct(requestFromParent) { saveLineNumFontSizeAndQuitAdjust() }
  }
    ...
    PageRequest.clearStateThenDoAct(requestFromParent) {
      if (editorPageShowingFilePath.value.isNotBlank()) {
        doJobThenOffLoading {
          ...
          // 请求外部程序打开文件
          //                    readOnlyForOpenAsDialog.value =
          // FsUtils.isReadOnlyDir(editorPageShowingFilePath.value)

          initOpenAsDialog(
              editorPageShowingFilePath.value.toFuckSafFile(activityContext).canonicalPath
          )
        }
      } else {
  ...
  if (requestFromParent.value == PageRequest.showInFiles) {
    PageRequest.clearStateThenDoAct(requestFromParent) { checkPathThenGoToFilesPage() }
  }
  ...
  if (requestFromParent.value == PageRequest.showDetails) {
    PageRequest.clearStateThenDoAct(requestFromParent) {
      ...
      val currentFile =
          FuckSafFile(
              activityContext,
              if (isPreviewModeOn.value) FilePath(previewPath) else editorPageShowingFilePath,
          )
      ...
      val (charsCount, linesCount) =
          if (showLinesCharsCount) editorPageTextEditorState.value.getCharsAndLinesCount()
          else Pair(0, 0)
      //            val lastModifiedTimeStr = getFormatTimeFromSec(sec=file.lastModified()/1000,
      // offset = getSystemDefaultTimeZoneOffset())
      val lastModifiedTimeStr =
          if (fileReadable) getFormattedLastModifiedTimeOfFile(currentFile) else ""

      val sb = StringBuilder()
      val suffix = "\n\n"
      sb.append(activityContext.getString(R.string.file_name) + ": " + fileName).append(suffix)
      sb.append(activityContext.getString(R.string.path) + ": " + currentFile.path.ioPath)
          .append(suffix)
      ...
      if (fileReadable) {
        ...
        sb.append(activityContext.getString(R.string.last_modified) + ": " + lastModifiedTimeStr)
            .append(suffix)
      }

      sb.append(
              activityContext.getString(R.string.encoding) +
                  ": " +
                  (if (isPreviewModeOn.value) curPreviewFileUsedCharset.value
                  else editorCharset.value)
          )
          .append(suffix)
      sb.append(
              activityContext.getString(R.string.line_break) +
                  ": " +
                  codeEditor.value.lineBreak.visibleValue
          )
          .append(suffix)
  ...
  //    val ifLastPathOkThenDoOkActElseDoNoOkAct:(okAct:(last: FuckSafFile)->Unit,
  // noOkAct:(last:String)->Unit)->Unit = { okAct, noOkAct ->
  ...
  //        //x 废弃，应在设置页面添加一个手动清除编辑器记录的位置信息的功能而不是一出异常就清除)
  // 注：这里不要判断文件是否存在，留到reload时判断，在那里如果发现文件不存在将清除文件的上次编辑位置等信息
  ////        var fuckSafFile: FuckSafFile? = null
  ////        if(last.isNotBlank() && FuckSafFile(activityContext, FilePath(last)).let { fuckSafFile
  // = it; it.exists() }) {
  //
  ...
  val notOpenFile =
      !editorPageShowingFileHasErr.value &&
          ...
          editorPageShowingFilePath.value.isBlank()
  val loadingFile =
      !editorPageShowingFileHasErr.value &&
          !editorPageShowingFileIsReady.value &&
  ...
  if (notOpenFile) { // 文件未就绪且无正在显示的文件且没错误
    ...
    val isItemInSelected = { item: FileDetail -> selectedRecentFileList.value.contains(item) }
    // 多选模式相关函数，结束

    LaunchedEffect(needRefreshRecentFileList.value) {
      doJobThenOffLoading(
          loadingOnForRecentFileList,
          loadingOffForRecentFileList,
    ...
    if (loadingRecentFiles.value) {
      ...
      if (recentFileList.value.isNotEmpty()) {
        ...
        LaunchedEffect(Unit) { inRecentFilesPage.value = true }

        DisposableEffect(Unit) {
          onDispose {
            inRecentFilesPage.value = false

            // this is possible, because when leave this code block, the recentFileList maybe
            // empty(e.g. removed all recent files)
        ...
        PullToRefreshBox(contentPadding = contentPadding, onRefresh = { reloadRecentFileList() }) {
          FileDetailList(
        ...
        // if is sub editor, after close a file, the closed file must available in the
        // `recentFileList`, so, no chance to reach `else` block
        ...
        if (!isSubPageMode) {
          ...
          PageCenterIconButton(
              ...
              onClick = { forceReloadFilePath(FilePath(getTheLastOpenedFilePath())) },
          )
        }
      }
        ...
        ConfirmDialog(
            ...
            textCompose = {
              ScrollableColumn {
                SelectionRow {
                  Text(
                      stringResource(R.string.will_delete_selected_items_are_u_sure),
            ...
            okTextColor =
                if (deleteFileOnDisk.value) MyStyleKt.TextColor.danger() else Color.Unspecified,
        ) {
              ...
              if (deleteFileOnDisk) {
                if (it.file.path.ioPathType == PathType.ABSOLUTE) {
                  try {
                    ...
                    MyLog.w(
                        TAG,
                        "remove file failed: ioPath=${it.file.path.ioPath}, err=${e.localizedMessage}",
                    )
                  }
                }
              }
      ...
      val iconOnClickList =
          listOf(
              delete@{ initDeleteRecentFilesDialog() },
              showInFiles@{
                selectedRecentFileList.value.firstOrNull()?.let { showInFiles(it.file.path) }
                Unit
              },
              openAs@{
                selectedRecentFileList.value.firstOrNull()?.let {
                  initOpenAsDialog(it.file.path.ioPath, showReloadDialog = false)
                }
                Unit
              },
              selectAll@{
      ...
      val getSelectedFilesCount = { selectedRecentFileList.value.size }
      val iconEnableList =
          listOf(
              delete@{ true },
      ...
      val countNumOnClickForBottomBar = { showSelectedItemsShortDetailsDialog.value = true }
      if (selectionMode.value) {
        BottomBar(
      ...
      if (editorOpenFileErr.value) { // 如果文件未加载就绪，加载出错显示错误，否则显示Loading...
        ...
        MySelectionContainer {
          Row {
            Text(
                text =
                    stringResource(id = R.string.open_file_failed) +
                        "\n" +
                        editorPageShowingFileErrMsg.value,
  ...
  if (isTimeShowEditor) {
    if (soraEditorComposeTestPassed) {
      ...
      FileEditor(
          ...
          refreshPreviewPage = {
            requestFromParent.value = PageRequest.editor_RequireRefreshPreviewPage
          },
    ...
    if (appPaused.value.not()) {
      FileChangeListener(
        ...
        if (
          ...
          doJobThenOffLoading {
            undoStack.clearRedoStackThenPushToUndoStack(
                editorPageTextEditorState.value,
                force = true,
            )
          }

          // set force to false to check last saved file info to avoid nonsense reload,
          //   but most time the file need a force reload when reached here, because we already
          // detected changes,
          ...
          MyLog.d(
              TAG,
              "file is changed by external, but currently app was modified file also, so will not reload it, $printFilePath",
          )
        }
      }
    }
  ...
  // 和on pause触发顺序相反，Activity的on
  // resume事件先调用，之后才调用compose的，因为必须得先有Activity才能有compose，后者依赖前者，所以销毁的时候，先销毁后面的，恢复的时候，先恢复前面的。
  LifecycleEventEffect(Lifecycle.Event.ON_RESUME) {
    ...
    doActIfIsExpectLifeCycle(MainActivityLifeCycle.ON_RESUME) {
      ...
      if (isPreviewModeOn.value) {
        ...
        MyLog.d(
            TAG,
            "#Lifecycle.Event.ON_RESUME: Preview Mode is On, will reload file: $previewPath",
        )
      }
    }
  ...
  LaunchedEffect(needRefreshEditorPage.value) {
    ...
    doJobThenOffLoading {
      try {
        ...
        doActWithLockIfFree(
            loadLock,
            "EditorInnerPage#Init#${needRefreshEditorPage.value}#${editorPageShowingFilePath.value.ioPath}",
        ) {
          doInit(
  ...
  DisposableEffect(Unit) {
    onDispose {
      ...
      //            editorPageTextEditorState.value =
      // editorPageTextEditorState.value.copy(focusingLineIdx = null)
...
private suspend fun doInit(
  ...
  if (!editorPageShowingFileIsReady.value) { // 从文件管理器跳转到editor 或 打开文件后从其他页面跳转到editor
    ...
    try {
      ...
      // reset if needed
      codeEditor.reset(
          FuckSafFile(AppModel.realAppContext, FilePath(requireOpenFilePath)),
          force = false,
      )
      ...
      // x 废弃 20240503 subPage为什么要百分百重载呢？再说subPage本来就是百分百重载啊，因为一关页面再开不就重载了吗？没必要在这特意区分是否subPage！)
      // subPage百分百重载文件；
      ...
      if (hasError().not()) {
        ...
        if (newDto == oldDto) {
          MyLog.d(
              TAG,
              "EditorInnerPage#loadFile: file may not changed, skip reload, file path is '${requireOpenFilePath}'",
          )
          // 文件可能没改变，放弃加载
          editorPageShowingFileIsReady.value = true
          return
      ...
      if (!file.isActuallyReadable()) {
        ...
        if (editorPageTextEditorState.value.contentIsEmpty().not() && !isContentSnapshoted.value) {
          MyLog.w(
              TAG,
              "#loadFile: file doesn't exist anymore, but content is not empty, will create snapshot for content",
          )
          ...
          doJobThenOffLoading {
            ...
            if (snapRet.hasError()) {
              MyLog.e(
                  TAG,
                  "#loadFile: create content snapshot for '$requireOpenFilePath' err: ${snapRet.msg}",
              )

              Msg.requireShowLongDuration(
                  "save content snapshot for '$fileName' err: " + snapRet.msg
              )
            } else {
      ...
      //            if (!file.isFile) {
      //                throw
      // RuntimeException(activityContext.getString(R.string.err_target_is_not_a_file))
      //            }
      ...
      if (isFileSizeOverLimit(file.length(), limit = maxSizeLimit)) {
        //                    editorPageSetShowingFileErrWhenLoading("Err: Doesn't support open file
        // over "+Cons.editorFileSizeMaxLimitForHumanReadable)
        throw RuntimeException(
            activityContext.getString(R.string.err_doesnt_support_open_file_over_limit) +
                "(" +
                getHumanReadableSizeStr(maxSizeLimit) +
                ")"
        )
      }

      //            if(debugModeOn) {
      //                println("editorPageShowingFileDto.value.fullPath:
      // "+editorPageShowingFileDto.value.fullPath)
      //            }
      ...
      // 读取文件内容
      //            editorPageTextEditorState.value =
      // TextEditorState.create(FsUtils.readFile(requireOpenFilePath))
      //                editorPageTextEditorState.value =
      // TextEditorState.create(FsUtils.readLinesFromFile(requireOpenFilePath))
      // 为新打开的文件创建全新的state
      if (soraEditorComposeTestPassed) {
        //                codeEditorState!!.content.value = Content(file.bufferedReader().use {
        // it.readText() })
      } else {
      ...
      doJobThenOffLoading {
        ...
        if (isSubPage) { // 作为子页面
          ...
          if (
            ...
            withMainContext { showTextAndUpdateTimeForPressBackBtn() }
          }
        }
      }
    }
