...
object PrefMan {
  ...
  object Key {
    /**
     * the value of this key: empty string: auto detect language country code: will use locale of
     * country code, e.g. us will use english
     *
     * 自动检测或指定国家代码
     */
    const val lang = "lang"
    /** e: err w: warn i: info d: debug v: verbose */
    const val logLevel = "log_level"
    /** log file keep in days 保留几天的日志 */
    const val logKeepDays = "log_keep_days"
    /**
     * 0 auto; 1 light; 2 dark. reference SettingsInnerPage.themeList, this value should match with
     * the themeList indices 这的值应该和设置页面的themeList索引匹配
     */
    ...
    /**
     * enabled to follow system color scheme, 0 disable, 1 enable, default 1 note: enable may cause
     * colors difficult to distinguish. see: https://github.com/catpuppyapp/PuppyGit/issues/66
     */
  ...
  private fun getPrefs(appContext: Context) =
      appContext.getSharedPreferences(fileName, MODE_PRIVATE)
