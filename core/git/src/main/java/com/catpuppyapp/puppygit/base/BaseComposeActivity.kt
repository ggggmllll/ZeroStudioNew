...
open class BaseComposeActivity : ComponentActivity() {
  ...
  protected fun setExceptionHandler(TAG: String, funName: String) {
    // catch exception, block start。( refer:
    // https://stackoverflow.com/questions/76061623/how-to-restart-looper-when-exception-throwed-in-jetpack-compose
        ...
        window.decorView.createLifecycleAwareWindowRecomposer(
            CoroutineExceptionHandler { coroutineContext, throwable ->
              try {
                ...
                showToast(
                    applicationContext,
                    getString(R.string.err_restart_app_may_resolve),
                    Toast.LENGTH_LONG,
                ) // 测试了下，能显示Toast
