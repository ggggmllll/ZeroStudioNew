...
private val knownSystemFilesManagerUris =
    listOf(
...
class FilePath(
  ...
  @IgnoredOnParcel private val rawPathType = PathType.getType(rawPath)

  @IgnoredOnParcel var ioPath: String = rawPath

  @IgnoredOnParcel var ioPathType = rawPathType
  ...
  private fun initIoPath() {
    ioPath =
        if (rawPathType == PathType.CONTENT_URI) {
  ...
  private fun tryResolveKnownUriToRealPath(): String {
    ...
    try {
      ...
      return if (maybeCanGetPathFromUri) {
        readableCanonicalPathOrDefault(
            File(FsUtils.getRealPathFromUri(Uri.parse(rawPath))),
            emptyPath,
        )
      } else {
        ...
        for (uriPrefix in knownUris) {
          ...
          if (indexOfPrefix == 0) {
            ...
            val realPathNoRootPrefix =
                uriStr.substring(uriPrefix.length) // 注：substring从length开始，其值为最大索引+1，所以不包含最后一个字符
            val file = File("/", realPathNoRootPrefix)
            // 打印的两个路径：第1个应该是没 / 前缀的， 第2个是有的
            MyLog.d(
                TAG,
                "resolved a known uri: realPathNoRootPrefix=$realPathNoRootPrefix, the path which will using is '${file.canonicalPath}'",
            )
      ...
      if (uriPath.isBlank()) {
        ...
        if (file.canRead()) {
          ...
          if (uriPath.startsWith(FsUtils.fileUriPathPrefix)) {
            ...
            val slashPrefixFileUriPath =
                Cons.slash +
                    FsUtils.fileUriPathPrefix // "/file://"，质感文件的uri解码到最后再取出uri.path返回的就是这样的路径
