...
fun <T> SelectedItemDialog(
    ...
    switchItemSelected:
        (
            T
        ) -> Unit, // 用switch而不是单纯的remove是为了日后可实现撤销删除方便，只要再把条目传给switchItemSelected函数，就能重新选中条目，但这样的话，恢复后的条目会在列表末尾，若想回到原位呢？难道删除后做整个列表的备份？或者这个函数改成能指定索引的？
 ...
 *
 * 为什么本方法是private: 之前直接用可复制弹窗实现，但后来改成了每行文本用各自的SelectionContainer，而被selection
 * container包围的内容无法用Box的对齐，导致文字无法居中，所以把这个方法改成private，然后在“必经之路” SelecteeItemDialog3上做手脚了
 */
@Composable
...
fun <T> SelectedItemDialog3(
    ...
    switchItemSelected: (T) -> Unit =
        {}, // 用switch而不是单纯的remove是为了日后可实现撤销删除方便，只要再把条目传给switchItemSelected函数，就能重新选中条目，但这样的话，恢复后的条目会在列表末尾，若想回到原位呢？难道删除后做整个列表的备份？或者这个函数改成能指定索引的？
  ...
  SelectedItemDialog2(
      ...
      trailIcon = {
        if (customTrailIcon != null) {
          ...
          Row(
            ...
            IconButton(onClick = { switchItemSelected(it) }) {
              Icon(
      ...
      onCopy = {
        ...
        doJobThenOffLoading {
          ...
          selectedItems.forEachBetter { sb.append(textFormatterForCopy(it)).append(lb) }
