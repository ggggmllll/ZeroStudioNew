...
enum class PLScope(val scope: String) {
  ...
  companion object {
    ...
    fun guessScopeType(fileName: String): PLScope {
      ...
      if (
          fileName.endsWith(".md") || fileName.endsWith(".mdown") || fileName.endsWith(".markdown")
      ) {
        return MARKDOWN
      }
      ...
      // c and cpp both use same header extension .h, but cpp is super set of c, so match to cpp
      // better
      ...
      if (
          fileName.endsWith(".rb") ||
              fileName.endsWith(".ruby")

              // Vagrantfile is ruby based
              || fileName == "vagrantfile" ||
              fileName.endsWith(".vagrantfile")
      ) {
        return RUBY
      }
      ...
      if (
          fileName.endsWith(".pl") ||
              fileName.endsWith(".perl") ||
              fileName.endsWith(".pm") ||
              fileName.endsWith(".psgi")
      ) {
        return PERL
      }

      if (
          fileName.endsWith(".pl6") ||
              fileName.endsWith(".p6") ||
              fileName.endsWith(".perl6") ||
              fileName.endsWith(".pm6") ||
              fileName.endsWith(".nqp") ||
              fileName.endsWith(".raku") ||
              fileName.endsWith(".rakudoc") ||
              fileName.endsWith(".rakumod")
      ) {
        return RAKU
      }
      ...
      if (
          fileName.endsWith(".rebase") ||
              fileName.endsWith(".git-rebase") ||
              fileName.endsWith(".gitrebase")
      ) {
        return GIT_REBASE
      }
