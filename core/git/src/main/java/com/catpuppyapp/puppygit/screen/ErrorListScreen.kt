...
fun ErrorListScreen(
  ...
  val curObjInState =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curObjInState",
          initValue = ErrorEntity(),
      )
  ...
  val getActuallyListState = { if (enableFilterState.value) filterListState else lazyListState }
  ...
  Scaffold(
    ...
    if (showBottomSheet.value) {
      BottomSheet(
          showBottomSheet,
          sheetState,
          stringResource(R.string.id) + ": " + curObjInState.value.id,
      ) {
        //                BottomSheetItem(sheetState=sheetState, showBottomSheet=showBottomSheet,
        // text=stringResource(R.string.view_msg)){
        //                    //弹窗显示错误信息，可复制
        //                }
        BottomSheetItem(
          ...
          doJobThenOffLoading {
            ...
            removeItemByPredicate { it.id == target.id }
    ...
    try {
      if (repoId.isNotBlank()) {
        doJobThenOffLoading {
          ...
          // here only need repo name, no need sync repo info with under git, and if query failed,
          // return a repo with empty id and empty name, but it shouldn't happen though
          curRepo.value =
              AppModel.dbContainer.repoRepository.getByIdNoSyncWithGit(repoId)
                  ?: RepoEntity(id = "")
