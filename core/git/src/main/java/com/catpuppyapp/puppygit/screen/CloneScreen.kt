...
fun CloneScreen(
  ...
  val depth = rememberSaveable {
    mutableStateOf("")
  } // 默认depth 为空，克隆全部；不为空则尝试解析，大于0，则传给git；小于0则克隆全部
  //    val credentialName = remember { mutableStateOf(TextFieldValue("")) }  //旋转手机，画面切换后值会被清，因为不是
  // rememberSaveable，不过rememberSaveable不适用于TextFieldValue，所以改用我写的自定义状态存储器了
  val credentialName =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
  ...
  //    val credentialHttpList = mutableCustomStateListOf(keyTag=stateKeyTag, keyName =
  // "credentialHttpList", initValue = listOf<CredentialEntity>())
  //    val credentialSshList = mutableCustomStateListOf(keyTag=stateKeyTag, keyName =
  // "credentialSshList", initValue = listOf<CredentialEntity>())
  // 这个用我写的自定义状态存储器没意义，因为如果屏幕旋转（手机的显示设置改变），本质上就会重新创建组件，重新加载列表，除非改成如果列表不为空，就不查询，但那样意义不大
  //    val curCredentialList:SnapshotStateList<CredentialEntity> = remember { mutableStateListOf()
  // }  //切换http和ssh后里面存对应的列表
  ...
  val requireFocusTo = rememberSaveable {
    mutableIntStateOf(focusToNone)
  } // 初始值0谁都不聚焦，修改后的值： 1聚焦url；2聚焦仓库名；3聚焦凭据名
  ...
  val credentialRadioOptions =
      listOf(noCredential, newCredential, selectCredential, matchCredentialByDomain) // 编号: 文本
  ...
  val setCredentialNameExistAndFocus: () -> Unit = {
    ...
    // credentialName.value.text.length)
    //                                          )
  ...
  val getStoragePathList = {
    ...
    val list =
        mutableListOf<NameAndPath>(
            NameAndPath(
                activityContext.getString(R.string.internal_storage),
                allRepoParentDir.canonicalPath,
    ...
    list.addAll(
        StoragePathsMan.get().storagePaths.map {
  ...
  val storagePathSelectedPath = rememberSaveable {
    mutableStateOf(
        StoragePathsMan.get().storagePathLastSelected.let { selectedPath ->
          storagePathList.value.find { it.path == selectedPath }
              ?: storagePathList.value.getOrNull(0)
              ?: NameAndPath()
        }
    )
  }

  val storagePathSelectedIndex = rememberSaveable {
    mutableIntStateOf(
        try {
          // if not found, just use first item, it is must existed internal storage
          storagePathList.value
              .indexOfFirst { storagePathSelectedPath.value.path == it.path }
              .coerceAtLeast(0)
        } catch (_: Exception) {
  ...
  if (showAddStoragePathDialog.value) {
    ConfirmDialog(
        ...
        onCancel = {
          ...
          // even close, still need reload list, because users maybe add storage path in the
          // FileChooser Screen
      ...
      doJobThenOffLoading {
        try {
          ...
          // here don't need check this, because storage paths included internal storage, so add it
          // just let it be selected, no bad effect
          //                    if(StoragePathsMan.allowAddPath(newPath).not()) {
          //                        throw RuntimeException("The path disallowed to add")
          //                    }

          // reload the storage path list, else, if added path in the File Chooser, here will lost
          // them
  ...
  val doSave: () -> Unit = {
    ...
    */
    ...
    doJobThenOffLoading launch@{
      ...
      //            val fullSavePath = if(storagePathSelectedIndex.intValue == 0) { // internal
      // storage
      //                allRepoParentDir.canonicalPath+ File.separator +repoNameText
      //            }else { // external storage, -1 or non-zero index, -1 only occured when edited
      // mode, the fullpath in db but is not in list
      //                storagePathSelectedPath.value.removeSuffix(File.separator) + File.separator
      // + repoNameText
      //            }
      ...
      if (
          isRepoNameExist || isPathExists(null, fullSavePath)
      ) { // 无论是否编辑模式，都需要判断下isPathExists()不然会错误覆盖其他目录
        focusRepoName()
      ...
      if (depth.value.isNotBlank()) {
        ...
        if (
            intDepth > 0
        ) { // 注：这里的状态只是预判，如果depth大于仓库实际的提交数，克隆后仓库依然是非shallow的，isShallow也会被更新为假，可通过检测仓库.git目录是否存在shallow文件来判断仓库是否处于shallowed状态，我已经在克隆仓库实现了这个功能
  ...
  Scaffold(
      ...
      topBar = {
        TopAppBar(
            ...
            title = { ScrollableTitle(stringResource(R.string.clone), listState, lastPosition) },
            navigationIcon = {
    ...
    Column(
        modifier =
            Modifier.baseVerticalScrollablePageModifier(contentPadding, listState)
      ...
      TextField(
          ...
          onValueChange = {
            ...
            val newCredentialType =
                Libgit2Helper.getCredentialTypeByGitUrlType(
                    newGitUrlType
                ) // 根据url类型获取credential类型（http or ssh）  //ssh
            ...
            // //为true代表url类型改变了，credential类型也需要跟着改变，并且重置一些状态变量
            //
          ...
          label = {
            Row {
              ...
              Text(
                  text = " (" + stringResource(id = R.string.http_https_ssh) + ")",
                  //                            modifier = Modifier.padding(start = 10.dp, bottom =
                  // 10.dp, end = 10.dp),
                  //                            fontSize = 11.sp
      ...
      //            改成启用未测试特性和 is shallowpassed控制功能是否显示，isPro决定功能是否enabled以及显示不同的文案( eg: depth and
      // depth(Pro))
      if (dev_EnableUnTestedFeature || shallowAndSingleBranchTestPassed) {
        ...
        Spacer(modifier = Modifier.padding(spacerPadding))
        //      depth输入框开始
        // 20240414 测试 发现depth有问题，虽能成功克隆，但之后莫名其妙出问题提示找不到object
        // id的概率非常大！而且在手机处理不了(不过电脑上的git虽也报错但能正常pull/push)，考虑过后，决定暂时放弃支持depth功能
      ...
      */
      ...
      SingleSelection(
          ...
          beforeShowItem = { idx, item ->
            // update credential type when click "New Credential" to make sure show private
            // key/passphrase for ssh url
  ...
  LaunchedEffect(Unit) {
    ...
    doJobThenOffLoading(
      ...
      if (isEditMode) { // 如果是编辑模式，查询仓库信息
        ...
        // if non-exists, it must not be the app's default internal storage path, so the type should
        // be repos storage path
        ...
        if (!credentialIdForClone.isNullOrBlank()) { // 更新credential相关字段
          if (credentialIdForClone == SpecialCredential.MatchByDomain.credentialId) {
            ...
            //                        MyLog.d(TAG, "#LaunchedEffect:credential==null:"
            // +(credential==null))
            ...
            if (credential == null) { // 要么没设置，要么设置了但被删除了，所以是无效id，这两种情况都会查不出对应的credential
              ...
              //                            curCredentialType.intValue = credential.type  //
              // deprecated
              curCredentialType.intValue =
                  Libgit2Helper.getCredentialTypeByUrl(repo.cloneUrl) // 设置当前credential类型
            }
          }
        }
      ...
      //            MyLog.d(TAG, "#LaunchedEffect:credentialHttpList.size=" +
      // credentialHttpList.value.size + ", credentialSshList.size=" + credentialSshList.value.size)
  ...
  isReadyForClone.value =
      ((gitUrl.value.isNotBlank() && repoName.value.text.isNotBlank()) &&
          ((credentialSelectedOption == optNumNoCredential ||
              credentialSelectedOption == optNumMatchCredentialByDomain) // 新凭据的情况
          ||
              ((credentialSelectedOption == optNumNewCredential &&
                  credentialName.value.text.isNotBlank()) // 必填字段
              // 要么是http且填了密码字段，要么是ssh且填了privatekey字段
              //                    && (curCredentialType.intValue==Cons.dbCredentialTypeHttp &&
              // credentialPass.value.isNotBlank()) ||
              // (curCredentialType.intValue==Cons.dbCredentialTypeSsh &&
              // credentialVal.value.isNotBlank())
              ) ||
