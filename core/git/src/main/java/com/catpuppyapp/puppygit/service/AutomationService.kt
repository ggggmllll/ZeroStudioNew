...
class AutomationService : BaseAccessibilityService() {
  companion object {
    ...
    // key is package name, value is time app last open time in millseconds, if null, means never
    // open
    ...
    // 包名若包含任意关键字，忽略
    private val ignorePackNameKeywords = listOf(".systemui", ".overlay", ".notification")
    ...
    private fun sendSuccessNotificationIfEnable(
    ...
    suspend fun pushRepoList(
      ...
      if (AppModel.devModeOn) {
        ...
        val notify = createNotify(NotifyUtil.genId())
        notify.sendNormalNotification(
            "auto push",
            "leave: $targetPackageName\nnow: $currentPackageName",
        )
      }
  ...
  override fun onCreate() {
    ...
    ) { // UPSIDE_DOWN_CAKE is sdk 34
      startForeground(
          serviceNotify.notifyId,
          getNotification(serviceNotify, settings),
  ...
  private fun getNotification(notifyBase: NotifyBase, settings: AppSettings): Notification {
    val builder =
        notifyBase.getNotificationBuilder(
            this,
            "PuppyGit Automation Service",
            "Auto pull/push when enter/leave apps",

            // 启动app并定位到Service页面
            NotifyUtil.createPendingIntentGoToSpecifiedPage(
                applicationContext,
  ...
  fun onAccessibilityEventNoCatch(event: AccessibilityEvent?) {
    ...
    if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
      ...
      if (AppModel.devModeOn) {
        MyLog.v(
            TAG,
            "TYPE_WINDOW_STATE_CHANGED: $packageName, className(activity name): $className",
        )
      }
      ...
      if (!activitiesOfPackage.contains(className)) {
        MyLog.d(
            TAG,
            "skip: app not contains className, packageName: $packageName, className: $className",
        )
        return
      }
        ...
        lock.withLock {
          ...
          if (targetPackageList.contains(packageName)) { // 是我们关注的app
            ...
            if (!targetOpened) { // was leave, now opened or first time opened
              ...
              repoList.forEachBetter {
                val pullIntervalInSec =
                    AutomationUtil.getAppAndRepoSpecifiedSettings(packageName, it.id, settings)
                        .getPullIntervalActuallyValue(settings)
                ...
                if (pullIntervalInSec >= 0L) {
                  ...
                  // 减法那个条件是检测时间间隔
                  if (
                      pullIntervalInMillSec == 0L ||
                          lastLeaveAt == 0L ||
                          (nowInMillSec - lastLeaveAt) > pullIntervalInMillSec
                  ) {
            ...
            if (lastTargetPackageName.isNotBlank()) { // 当前app不是我们关注的app，但上个是
              ...
              if (targetPackageList.contains(lastOpenedTarget)) {
                MyLog.d(
                    TAG,
                    "target packageName '$lastOpenedTarget' leaved, checking need push or no...",
                )
                // 这个应该百分百为真啊？
                val targetOpened =
                ...
                if (targetOpened) { // was opened, now leave
                  ...
                  AutomationUtil.groupReposByPushDelayTime(lastOpenedTarget, repoList, settings)
                      .forEachBetter mark@{ pushDelayInSec, repoList ->
                        if (repoList.isEmpty()) {
                          return@mark
                        }

                        // do push, one package may bind multi repos, start a coroutine do push for
                        // them
                        // 负数将禁用push
                          ...
                          doJobThenOffLoading pushTask@{
                            ...
                            if (pushDelayInMillSec > 0L) {
                              ...
                              while (true) {
                                ...
                                if (
                                    // app重新打开，当前任务取消
                                    targetPackageTrueOpenedFalseCloseNullNeverOpenedList[
                                        lastOpenedTarget] == true
                            ...
                            if (!taskCanceled) {
                              ...
                              if (curShowingPackageName.isNotBlank()) {
                                val repoListOfCurShowingPackage =
                                    AutomationUtil.getRepos(
                                        settings.automation,
                                        curShowingPackageName,
                                    )
                              ...
                              if (repoList.isEmpty()) {
                                ...
                                MyLog.d(
                                    TAG,
                                    "push cancelled, current app full-covered target repoList, will do push after current app leave",
                                )
                              } else {
                                pushRepoList(
                                    sessionId,
                                    settings,
                                    repoList,
                                    targetPackageName = lastOpenedTarget,
                ...
                MyLog.d(
                    TAG,
                    "target packageName '$lastOpenedTarget' was in targetList but removed, will not do push for it",
                )
                targetPackageTrueOpenedFalseCloseNullNeverOpenedList[lastOpenedTarget] = false
              }
            }
