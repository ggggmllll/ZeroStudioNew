...
fun CredentialManagerScreen(
  ...
  val onClickCurItem =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "onClickCurItem",
          initValue = CredentialEntity(id = ""),
      ) // 非点击跳转页面的情况下，点击条目后更新此变量
  val requireDoLink = rememberSaveable { mutableStateOf(false) }
  val targetAll = rememberSaveable { mutableStateOf(false) }
  val remoteDtoForCredential =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "remoteDtoForCredential",
          initValue = RemoteDtoForCredential(remoteId = remoteId),
      )
  ...
  if (showLinkOrUnLinkDialog.value) {
    LinkOrUnLinkCredentialAndRemoteDialog(
        ...
        onErrCallback = { e ->
          val errMsgPrefix =
              "${linkOrUnLinkDialogTitle.value} err: remote='${remote.value.remoteName}', credential=${onClickCurItem.value.name}, err="
                ...
                Column(
                  ...
                  if (remote.value.id.isNotEmpty()) { // 隐含 remoteId.isNotEmpty() 为 true
                    titleSecondaryString.value =
                        "${stringResource(R.string.link_mode)}: [${remote.value.remoteName} of ${curRepo.value.repoName}]"
        ...
        BottomSheet(showBottomSheet, sheetState, curCredential.value.name) {
          BottomSheetItem(
            ...
            navController.navigate(
                Cons.nav_CredentialNewOrEditScreen + "/" + curCredential.value.id
            )
          }
          ...
          CredentialItem(
            ...
            if (remoteId.isEmpty()) { // 若remoteId为空，跳转到remote和凭据绑定页面
              ...
              linkOrUnLinkDialogTitle.value =
                  activityContext.getString(R.string.link) + " '${it.name}'"
              // 显示弹窗
