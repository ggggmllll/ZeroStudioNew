...
fun EditorPageActions(
  ...
  */
  ...
  if (isPreviewModeOn) {
    val currentIsNotAtHome =
        remember(previewPathChanged) {
          derivedStateOf { runBlocking { previewNavStack.currentIsRoot().not() } }
        }
    val canGoBack =
        remember(previewPathChanged) {
          derivedStateOf { runBlocking { previewNavStack.backStackIsNotEmpty() } }
        }
    val canGoForward =
        remember(previewPathChanged) {
          derivedStateOf { runBlocking { previewNavStack.aheadStackIsNotEmpty() } }
        }
  ...
  if (showMenuIcon) {
    ...
    LongPressAbleIconBtn(
        ...
        onClick = {
          ...
          //   because the menu will cover the whole screen, so if user can click the 3dots icon,
          // the menu must be hidden,
  ...
  Row(modifier = Modifier.padding(top = MyStyleKt.TopBar.dropDownMenuTopPaddingSize)) {
    ...
    DropdownMenu(expanded = dropDownMenuExpandState.value, onDismissRequest = { closeMenu() }) {
      ...
      DropdownMenuItem(
          ...
          trailingIcon = { SimpleCheckBox(showUndoRedo.value) },
          onClick = {
            ...
            // 更新配置文件
            SettingsUtil.update { it.editor.showUndoRedo = newValue }
      ...
      DropdownMenuItem(
          ...
          onClick = {
            ...
            // 保存
            SettingsUtil.update { it.editor.showLineNum = newValue }
          },
      )
      ...
      val autoCloseSymbolPair = remember {
        mutableStateOf(SettingsUtil.isEditorAutoCloseSymbolPairEnabled())
      }
      DropdownMenuItem(
          // 非readOnly目录才允许开启或关闭readonly状态，否则强制启用readonly状态且不允许关闭
          enabled = enableMenuItem,
          text = { Text(stringResource(R.string.symbol_pair)) },
          trailingIcon = { SimpleCheckBox(autoCloseSymbolPair.value) },
          onClick = {
            //                    closeMenu()
            ...
            // 保存
            SettingsUtil.update { it.editor.autoCloseSymbolPair = newValue }
          },
      )
      ...
      if (proFeatureEnabled(editorEnableLineSelecteModeFromMenuTestPassed)) {
        ...
        DropdownMenuItem(
            ...
            trailingIcon = { SimpleCheckBox(selectModeOn) },
            onClick = {
              //                        closeMenu()
