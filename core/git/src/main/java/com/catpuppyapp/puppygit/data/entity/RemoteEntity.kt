...
data class RemoteEntity(
    ...
    @Deprecated(
        "改成动态查询git仓库的fetch refspecs了，然后根据数量来判断是自定义分支列表还是所有分支，参见：Libgit2Helper.getRemoteFetchBranchList(remote)"
    )
    ...
    var pushUrl: String =
        "", // 默认为空，git config无此字段，代表使用默认的url，例如 remote.origin.url，对应数据库的 remoteUrl 字段
