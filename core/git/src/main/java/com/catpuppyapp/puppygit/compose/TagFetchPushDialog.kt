...
fun TagFetchPushDialog(
    ...
    doJobThenOffLoading(loadingOn, loadingOff, loadingTextForFetchPushDialog.value) {
      try {
        Repository.open(curRepo.fullSavePath).use { repo ->
          ...
          // del remote tags
          val delRemote =
              requireDel && requireDelRemoteChecked.value && selectedRemoteList.isNotEmpty()
          ...
          val force =
              if (pushAndDel) {
                // 修改下loading text
                loadingTextForFetchPushDialog.value =
                    activityContext.getString(R.string.deleting_remote_tags)
          ...
          selectedRemoteList.forEachBetter {
            ...
            val fetchCredential =
                if (remote == null || remote.credentialId.isBlank()) null
                else
            ...
            val pushCredential =
                if (remote == null || remote.pushCredentialId.isBlank()) null
                else
                    credentialDb.getByIdWithDecryptAndMatchByDomain(
          ...
          if (trueFetchFalsePush) { // fetch
            ...
            selectedTagsList.forEachBetter {
              pushRefSpecs.add(if (delRemote) ":${it.name}" else "${it.name}:${it.name}")
            }
  ...
  AlertDialog(
      ...
      text = {
        ScrollableColumn {
              ...
              Text(
                  text =
                      if (trueFetchFalsePush)
                          stringResource(
                              R.string.note_as_default_remote_will_not_override_local_tags
                          )
          ...
          if (!requireDel || requireDelRemoteChecked.value) {
            ...
            RemoteCheckBoxList(
                itemList = remoteList,
                selectedList = selectedRemoteList,
                remoteCheckedList,
            )
              ...
              if (force.value) { // 显示勾选force后的覆盖提示
                SelectionRow {
                  DefaultPaddingText(
                      text =
                          if (trueFetchFalsePush)
                              stringResource(R.string.warn_remote_tag_override_local_tag)
      ...
      confirmButton = {
        TextButton(onClick = { onOk(force.value) }, enabled = enableOk) {
          Text(
              text =
                  stringResource(
                      if (requireDel) R.string.delete
                      else if (trueFetchFalsePush) R.string.fetch else R.string.push
                  ),
              color =
                  if (enableOk && requireDel) MyStyleKt.TextColor.danger() else Color.Unspecified,
          )
        }
      },
      dismissButton = {
