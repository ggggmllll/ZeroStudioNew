...
class FileHistoryDto(
    ...
    var dateTime: String = "",
    var originTimeOffsetInMinutes: Int =
        0, // commit的时间偏移量，单位分钟 (btw 实际上是 commit使用的是commit对象中的committer signature中的时间和偏移量)
  ...
  fun cachedShortCommitListStr(): String =
      cached_ShortCommitListStr
          ?: commitList
              .joinToString { Libgit2Helper.getShortOidStrByFull(it) }
              .let {
                cached_ShortCommitListStr = it
                it
              }
