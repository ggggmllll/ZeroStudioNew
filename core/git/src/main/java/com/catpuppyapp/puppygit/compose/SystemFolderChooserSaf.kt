...
/**
 * A Folder Chooser depend System File Chooser, may not work if system removed internal file picker,
 * in that case, can input path instead
 */
...
fun SystemFolderChooserSaf(
    ...
    chosenPathCallback: (uri: Uri?) -> Unit = { uri ->
      if (uri != null) {
        if (AppModel.devModeOn) {
          // example of output: uri.toString() == uri.path: false,
          // uri.toString()=content://com.android.externalstorage.documents/tree/primary%3ARepos,
          // uri.path=/tree/primary:Repos
          // toString更完整些，是我期望的safPath，uri.path缺乏一些信息，不行
          MyLog.d(
              TAG,
              "uri.toString() == uri.path: ${uri.toString() == uri.path}, uri.toString()=${uri.toString()}, uri.path=${uri.path}",
          )
        }
        ...
        MyLog.d(
            TAG,
            "#chooseDirLauncher: uri.toString()=${uri.toString()}, uri.path=${uri.path}, safEnabled=${safEnabled.value}, safPath=${safPath.value}, nonSafPath=${nonSafPath.value}",
        )
      }
    },
) {
  ...
  //        Text(stringResource(R.string.if_unable_choose_a_path_just_copy_paste_instead),
  ...
  if (showSafSwitchButton) {
    ...
    MyCheckBox(
        ...
        value = safEnabled,
        onValueChange = { newSafEnabledValue ->
          path.value =
              if (newSafEnabledValue) {
                safPath.value
              } else {
                nonSafPath.value
              }
