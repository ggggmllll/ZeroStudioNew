...
internal fun MyTextField(
  ...
  // NOTE: if the `value` is not equals to `BasicTextField` held value, then the ime state will
  // reset
  BasicTextField(
      ...
      onValueChange = ovc@{ newState ->
            ...
            if (indexOfLineBreak != -1) {
              // make sure only call contains new line once, else maybe cause paste content twice in
              // sometimes
              ...
              val newState =
                  keepStylesIfPossible(newState, lastState, textChangedCallback = scrollIfInvisible)
...
private fun keepStylesIfPossible(
  ...
  try {
    val textChanged =
        lastState.text.length != newState.text.length || lastState.text != newState.text
