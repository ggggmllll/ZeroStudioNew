...
object SettingsUtil {
  ...
  private val saveChannel =
      Channel<AppSettings>(capacity = channelBufferSize, onBufferOverflow = BufferOverflow.SUSPEND)
  ...
  private suspend fun readSettingsFromFile(useBak: Boolean) {
    ...
    if (settingsStr.isBlank()) { // 文件内容为空，可能刚新建的，往里面存入内容
      ...
      //            appSettingsReadOnly = newSettings.copy()
      // 新创建的肯定不用迁移，直接保存即可
      doJobThenOffLoading { saveChannel.send(newSettings) }
    } else { // 文件若存在，读取即可
  ...
  /**
   * usage: Settings.Util.update(false) { settings -> settings.abc=abc settings.def=def ... 其他修改 }
   * 就行了，这个调用执行完，会自动保存，如果想修改完后获取一份最新的设置项拷贝，传true即可，例如： val updatedNewSettings =
   * Settings.Util.update(true) { settings -> settings.abc=abc settings.def=def ... 其他修改 }
   */
  fun update(
      requireReturnSnapshotOfUpdatedSettings: Boolean = false,
      modifySettings: (AppSettings) -> Unit,
  ): AppSettings? {
    ...
    //        return if(requireReturnAnNewSettings) getSettingsSnapshot() else null
    // //返回最新的设置项，如果需要的话(否则的话，你的设置项可能已经过期，某些设置不是最新，但是，如果你需要用到的变量很少，并且就算不是最新也问题不大，就没必要获取最新设置项的拷贝了)

    // 返回的时候要创建拷贝，避免和update用同一个变量，不然外部一修改返回值里的数据，即将写入的状态也会被影响，就乱套了
    // 这里获取刚被修改过的实例的拷贝，相对更新一些，但可能和硬盘上的数据不一致，但正常来说，最终修改写入硬盘后，内容会一致
    return if (requireReturnSnapshotOfUpdatedSettings) getSettingsSnapshotFromSrc(settingsForUpdate)
    else
        null // 返回最新的设置项，如果需要的话(否则的话，你的设置项可能已经过期，某些设置不是最新，但是，如果你需要用到的变量很少，并且就算不是最新也问题不大，就没必要获取最新设置项的拷贝了)
  }
  ...
  fun updateSettingsIfNotEqualsWithOld(newSettings: AppSettings) { // 供外部调用的更新配置文件的方法
    doJobThenOffLoading {
      try {
        ...
        if (
            newSettings.version != tmpSettings.version || !newSettings.equals(tmpSettings)
        ) { // 如果新设置和旧的不一样，更新配置文件
        ...
        MyLog.e(
            TAG,
            "#updateSettingsIfNotEqualsWithOld(): update config err!\n" + e.stackTraceToString(),
        )
      }
    }
  }
  ...
  private suspend fun saveSettings(newSettings: AppSettings) {
    ...
    //        if(debugModeOn) {
    //            MyLog.d(TAG, "#saveSettings(): settings will
    // save:"+JsonUtil.j.encodeToString(appSettings))
    //        }
  ...
  fun editorThresholdLinesCountOfIncrementAnalyze() =
      appSettings.editor.thresholdLinesCountOfIncrementAnalyze
