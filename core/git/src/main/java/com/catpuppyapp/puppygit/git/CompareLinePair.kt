...
data class CompareLinePair(
  ...
  fun compare(
      ...
      map: MutableMap<String, CompareLinePairResult>,
  ) {
    // not ready
    ...
    // bad origin type
    if (
        line1OriginType == OriginType.CONTEXT.toString() &&
            line2OriginType == OriginType.CONTEXT.toString()
    ) {
      MyLog.w(
          TAG,
          "compare both Context type lines are nonsense: line1OriginType=$line1OriginType, line2OriginType=$line2OriginType",
      )
      return
    }

    //        val line1fakeType = if(line1OriginType != OriginType.ADDITION.toString() &&
    // line1!=OriginType.DELETION.toString()) {
    ...
    //        val add = if (line1OriginType == OriginType.ADDITION.toString() || line1OriginType ==
    // CompareLinePairHelper.clipboardLineOriginType || line1OriginType ==
    // OriginType.CONTEXT.toString()) {
  ...
  //        if(line1OriginType == OriginType.CONTEXT.toString() || line2OriginType ==
  // OriginType.CONTEXT.toString()
  //            || line1Num == CompareLinePairHelper.clipboardLineNum || line2Num ==
  // CompareLinePairHelper.clipboardLineNum
...
data class CompareLinePairResult(val stringPartList: List<IndexStringPart>? = null)
