...
fun createSimpleCommitDto(commit: Commit, repoId: String, settings: AppSettings): CommitDto =
    createCommitDto(
        commitOid = commit.id(),
        allBranchList = null,
        allTagList = null,
...
suspend fun updateRemoteDtoList(
    repo: Repository,
    remoteDtoList: List<RemoteDto>,
    onErr: (errRemote: RemoteDto, e: Exception) -> Unit = { r, e -> },
) {
...
fun updateRemoteDto(repo: Repository, remoteDto: RemoteDto) {
  ...
  // use isBlank() check the pushUrl is fine, if pushurl exists but has blank value, still will
  // using fetch url
...
fun createSubmoduleDto(
  ...
  val smFullPath =
      parentWorkdirPathNoSlashSuffix + Cons.slash + smRelativePath.removePrefix(Cons.slash)

  // [fixed, the reason was pass NULL to jni StringUTF method in c codes] if call submodule.url() it
  // will crashed when url invalid
  val smUrl = sm.url() ?: ""
...
fun genConfigDto(repoEntity: RepoEntity, settings: AppSettings): ConfigBean {
  ...
  return ConfigBean(
      ...
      api =
          ApiBean(
              ...
              // 少加点参数，少写少错
              pull_example =
                  "${genHttpHostPortStr(host, port.toString())}/pull?token=$token&repoNameOrId=${repoEntity.repoName}",
              push_example =
                  "${genHttpHostPortStr(host, port.toString())}/push?token=$token&repoNameOrId=${repoEntity.repoName}",
              sync_example =
                  "${genHttpHostPortStr(host, port.toString())}/sync?token=$token&repoNameOrId=${repoEntity.repoName}",
          ),
  )
}

fun rawAppInfoToAppInfo(
    rawAppInfo: ApplicationInfo,
    packageManager: PackageManager,
    selected: (AppInfo) -> Boolean,
): AppInfo? {
...
fun getCredentialName(
  ...
  // 若id为match by domain的id则显示match by
  // domain；否则，若name不为null则关联了有效的id，这时显示name；若name为null，则代表关联的凭据已被删除或者没关联凭据，这时实际处理时会当作无关联凭据，所以显示None
