...
fun AppScreenNavigator() {
  ...
  val currentHomeScreen = rememberSaveable {
    mutableIntStateOf(SettingsUtil.obtainLastQuitHomeScreen())
  }
  ...
  NavHost(navController = navController, startDestination = navStartScreen) {
    ...
    // 注意：带返回箭头的二级菜单用 navController.navigateUp()
    // api返回，这个和navipopup有点不同，如果在自己app里导航，两者一致，如果从外部app导航进本app，例如从其他app通过deeplink跳转到了本app，那么navigateUp会返回其他app，而popup则会尝试返回到操作系统为本app构建的虚拟栈中，按照这个理解，navigateUp期望返回进入本app的应用，而popup则期望返回本app内当前页面的上一页面（如果有的话）
    ...
    */
    composable(
        Cons.nav_CommitListScreen +
            "/{repoId}/{isHEAD}/{from}/{fullOidCacheKey}/{shortBranchNameCacheKey}"
    ) {
    ...
    composable(
        Cons.nav_DiffScreen +
            "/{repoId}/{fromTo}/{treeOid1Str}/{treeOid2Str}/{isDiffToLocal}/{curItemIndexAtDiffableList}/{localAtDiffRight}/{fromScreen}/{diffableListCacheKey}/{isMultiMode}"
    ) {
    ...
    composable(Cons.nav_CredentialNewOrEditScreen + "/{credentialId}") {
      CredentialNewOrEdit(
          ...
          naviUp = {
            ...
            //                            navController.navigate(Cons.nav_CredentialManagerScreen) {
            //
            // //这段代码的意思是导航到credential页面，但返回的时候，返回到home页面(而不是导航到credential的credential_new_or_edit页面)
    ...
    composable(
        Cons.nav_TreeToTreeChangeListScreen +
            "/{repoId}/{commit1OidStrCacheKey}/{commit2OidStrCacheKey}/{commitForQueryParentsCacheKey}/{titleCacheKey}"
    ) {
      ...
      val commitForQueryParentsCacheKey =
          it.arguments?.getString("commitForQueryParentsCacheKey") ?: ""
  ...
  LaunchedEffect(gotNewIntent.value) { IntentHandler.requireHandleNewIntent() }
}
