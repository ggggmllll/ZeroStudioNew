...
object MyStyleKt {
  ...
  object ClickableText {
    @Composable fun getStyle() = LocalTextStyle.current
    ...
    fun getColor(inDarkTheme: Boolean = Theme.inDarkTheme) =
        if (inDarkTheme) color_dark else color_light
    ...
    //        val modifier = Modifier.padding(start = 3.dp,top=0.dp, bottom = 0.dp,
    // end=1.dp).defaultMinSize(minClickableSize)
    val fontSize = 16.sp
  }
  ...
  object TextColor {
    ...
    fun getHighlighting(inDarkTheme: Boolean = Theme.inDarkTheme) =
        if (inDarkTheme) highlighting_green_dark else highlighting_green_light
    ...
    fun lineNumColor(inDarkTheme: Boolean = Theme.inDarkTheme, isFocused: Boolean): Color {
      return if (isFocused) {
        if (inDarkTheme) lineNum_focused_forEditorInDarkTheme
        else lineNum_focused_forEditorInLightTheme
      } else {
  ...
  object Fab {
    ...
    fun getFabModifierForEditor(isMultipleSelectionMode: Boolean, isPortrait: Boolean): Modifier {
      ...
      val kbOffset =
          naviPadding.calculateBottomPadding().value.toInt().let { if (it == 0) 0 else -it }
      ...
      return addNavPaddingIfNeed(
          ...
          Modifier.fabBasePadding()
              .then(
                  if (kbHeight.value > 0) { // 显示软键盘
                    Modifier.padding(bottom = kbHeight)
                  } else { // 不显示软键盘
                    if (isMultipleSelectionMode) Modifier.padding(bottom = BottomBar.height)
                    else Modifier
                  }
              ),
      )
    ...
    fun getFabModifier(isPortrait: Boolean, deviceWidthHeight: DeviceWidthHeight): Modifier {
      ...
      //            return addNavPaddingIfNeed(isPortrait, Modifier.padding(bottom =
      // BottomBar.height, end = (deviceWidthHeight.width * 0.1f).dp))
      // 底部加padding是为了高过多选模式的bottom bar，并不是所有页面都有多选模式的底栏，但懒得判断了，直接加固定高度
      // end 30.dp 是为了避免遮盖列表条目的菜单按钮
      return addNavPaddingIfNeed(
          isPortrait,
          Modifier.fabBasePadding().padding(bottom = BottomBar.fabBottomPadding, end = 30.dp),
      )
    }
  ...
  object TextSelectionColor {
    ...
    val customTextSelectionColors_cursorHandleVisible =
        TextSelectionColors(
  ...
  object TopBar {
    ...
    /**
     * when use this, be careful to make text in top bar colorful, if the color closer with this,
     * will difficult to distinguish
     */
    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    //        // note: this maybe cause colors too closer to difficult to distinguish when dynamic
    // color on
    fun getColors() =
  ...
  object DropDownMenu {
    ...
    @Composable fun selectedItemColor() = MaterialTheme.colorScheme.primary

    @Composable fun selectedItemContainerColor() = MaterialTheme.colorScheme.primaryContainer
  ...
  object ToggleButton {
    ...
    fun defaultColors(): IconToggleButtonColors {
      // 容器颜色都改成透明
      return IconButtonDefaults.filledTonalIconToggleButtonColors()
          .copy(
  ...
  object ConflictBlock {
    ...
    fun getConflictOursBlockBgColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) conflictOursBlockBgColorInDarkThem
      else conflictOursBlockBgColorInLightThem
    }
    ...
    fun getConflictTheirsBlockBgColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) conflictTheirsBgColorInDarkTheme
      else conflictTheirsBgColorInLightTheme
    }
    ...
    fun getConflictStartLineBgColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) conflictStartLineBgColorInDarkTheme
      else conflictStartLineBgColorInLightTheme
    }
    ...
    fun getConflictSplitLineBgColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) conflictSplitLineBgColorInDarkTheme
      else conflictSplitLineBgColorInLightTheme
    }
    ...
    fun getConflictEndLineBgColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) conflictEndLineBgColorInDarkTheme
      else conflictEndLineBgColorInLightTheme
    }
    ...
    fun getAcceptTheirsIconColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
      return if (inDarkTheme) acceptTheirsIconColorInDarkTheme
      else acceptTheirsIconColorInLightTheme
    }
  ...
  object LastClickedItem {
    ...
    fun getEditorLastClickedLineBgColor(inDarkTheme: Boolean) =
        if (inDarkTheme) editorLastClickedLineBgColorDark else editorLastClickedLineBgColorLight
  }
}
