  ...
  CopyableDialog2(
      ...
      okCompose = {
        TextButton(
            ...
            onClick = {
              // 执行用户传入的callback
              doJobThenOffLoading(loadingOn, loadingOff, loadingText) { onOk() }
            },
        ) {
          Text(
              text = okBtnText,
...
fun CopyableDialog2(
    ...
    // 若cancelCompose为null，则此函数为 cancel按钮 和 onDismiss 回调；否则仅为onDismiss回调。（p.s.
    // onDismiss就是弹窗显示的情况下，点击非弹窗区域触发的那个函数）
    onCancel: () -> Unit,
