...
fun RepoInfoDialog(
  ...
  InfoDialog(showTitleInfoDialog) {
    ...
    Row { Text(stringResource(id = R.string.repo) + ": " + curRepo.repoName) }
    RepoInfoDialogItemSpacer()

    if (dbIntToBool(curRepo.isDetached)) {
      Row { Text(stringResource(R.string.branch) + ": " + Cons.gitDetachedHead) }
    } else {
      Row { Text(stringResource(R.string.branch) + ": " + (curRepo.branch)) }

      if (curRepo.upstreamBranch.isNotBlank()) {
        RepoInfoDialogItemSpacer()

        Row { Text(stringResource(R.string.upstream) + ": " + (curRepo.upstreamBranch)) }
      }
    }

    RepoInfoDialogItemSpacer()
    Row { Text(stringResource(R.string.repo_state) + ": " + curRepo.getRepoStateStr(context)) }
...
fun ShortCommitInfo(title: String, name: String, commitDto: CommitDto, msgMaxLines: Int = 6) {
  ...
  if (
    ...
    Row {
      Text(
          stringResource(R.string.author) +
              ": " +
              Libgit2Helper.getFormattedUsernameAndEmail(commitDto.author, commitDto.email)
      )
    }
    RepoInfoDialogItemSpacer()
    Row {
      Text(
          stringResource(R.string.committer) +
              ": " +
              Libgit2Helper.getFormattedUsernameAndEmail(
                  commitDto.committerUsername,
                  commitDto.committerEmail,
              )
      )
    }
    RepoInfoDialogItemSpacer()
    ...
    Row {
      Text(
          stringResource(R.string.msg) + ": " + commitDto.msg,
...
fun CompareInfo(
    leftName: String,
    leftCommitDto: CommitDto,
    rightName: String,
    rightCommitDto: CommitDto,
) {
  Row {
    Text(
        stringResource(R.string.comparing_label) +
            ": " +
            Libgit2Helper.getLeftToRightDiffCommitsText(leftName, rightName, false)
    )
  }
