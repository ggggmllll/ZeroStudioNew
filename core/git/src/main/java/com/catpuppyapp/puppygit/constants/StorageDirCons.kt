...
object StorageDirCons {
  ...
  object Status {
    ...
    val disable =
        2 // 禁用，不算错误，用户可禁用某存储目录，被禁用的存储目录和其下的仓库将不会展示在Files和Repos和ChangeList页面，在克隆时也不可选择被禁用的存储目录作为仓库存储目录
  ...
  object DefaultStorageDir {
    ...
    @Deprecated(
        "保留，不会删，但也不会用，因为和旧版不兼容，目前实际的puppyGitData目录在 puppyGitRepos下，不是作为StorageDir，而是作为其中的一个普通文件夹"
    )
    val puppyGitDataDir =
        StorageDirEntity(
            ...
            baseFields =
                BaseFields(
                    //                baseStatus = Status.ok,
                    // //状态，是否已不存在或者被禁用啊之类的，禁用就不显示在仓库和Files页面了
                    baseStatus = Status.fakeDel, // 状态，是否已不存在或者被禁用啊之类的，禁用就不显示在仓库和Files页面了
                    baseIsDel =
                        Cons
                            .dbCommonTrue, // 这个状态，其实意义不大，和status含义冲突了，实现的时候最好写成“如果isDel为true就忽略status”，当然，保险起见，如果不放心，可同时把status设为fakeDel，那样不管用哪个判断，都会认为条目已删除
    ...
    val puppyGitRepos =
        StorageDirEntity(
            // 20240527
            // 交换了repoStorage1和otherRepos(原orphanRepos)的id，这样的话旧版数据库迁移的时候就直接指向repoStorage1的id，而已迁移的仓库也指向repoStorage1，最终结果就是之前已迁移的和即将迁移的所有旧版的仓库都关联上了这个条目
    ...
    /**
     * 此sd存储3种类型的目录（实际上只有仓库目录）： 1 通过fullpath反查sd时，没有任何仓库的sd与其fullpath匹配的仓库 2 没有任何sd的id与其sdid匹配的仓库 3
     * sdid为其id的仓库
     */
    ...
    val otherRepos =
        StorageDirEntity(
            ...
            type =
                Type
                    .other, // 检测到这个type，不会去查询其fullPath条目，而是简单列出当前storageDir关联的所有仓库，若是internal/external，则会直接把对应目录fullPath的所有条目列出来，实际就是对目录调用listFiles()。
            allowDel = Cons.dbCommonFalse, // 不允许删除
            parentId = rootDir.id,
            baseFields =
                BaseFields(
