...
class SearchOnm : Search() {
  /**
   * 时间复杂度 O(nm)，n 为addArr的长度，m为delArr的长度 和O(n)版算法主要区别在于能匹配以下几种情况: addArr: 12345或123b45，delArr:
   * 123a45 这种情况，此算法可完整匹配到123和45 addArr: 12345, delArr: a12345b，能匹配到12345 addArr: a12345a, delArr:
   * b12345b，能匹配到12345
   *
   * 另外：
   * 此算法如果正向无匹配，逆向必然也无匹配，所以在正向无匹配时调用逆向匹配意义不大，不过正向和逆向有可能匹配到不同的内容片段，如果想比较哪个匹配连续片段多就采用哪个结果的话，分别调用正向和逆向匹配是有意义的，但会消耗多一倍的时间。
   *
   * 注意：在能够匹配的情况下，正向匹配和逆向匹配的结果有可能不同，例如： 在 ababcdefef 中查找 abef
   * ，如果正向匹配，会匹配到第一个ab和ef；若逆向匹配则会匹配到最后一个ef和ab。
   */
    ...
    while (true) {
      if (addCur == addEnd && delCur == delEnd) {
        ...
        break
          ...
          addCur == addEnd
      ) { // 隐含  delCur!=delEnd  //如果add到结尾，del索引加1，add索引重置为addStart，未匹配内容添加到末尾；
        if (matching) {
          ...
          lastDel =
              addToList(
                  delStart,
                  if (reverse) delStart - 1 else delStart + 1,
                  true,
                  delList,
                  lastDel,
              )
