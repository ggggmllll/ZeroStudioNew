...
class Ret<T> private constructor(initData: T) {
  object ErrCode { // 小于SuccessCode.default的全是失败代码
    ...
    val mergeFailedByConfigIsFfOnlyButCantFfMustMerge =
        20 // 配置文件配置成仅限fast-forward但无法fast-forward必须合并才行(一般服务器上的git远程仓库会配置仅fast-forward（我猜的），让客户端必须解决冲突再推送，但客户端一般不会配置这个)
  ...
  fun <O> copyWithNewData(newData: O? = null): Ret<O?> {
    return create(data = newData, msg = msg, code = code, exception = exception)
  }

  companion object {
    fun <T> createErrorDefaultDataNull(
        errMsg: String,
        data: T? = null,
        errCode: Int = ErrCode.default,
    ...
    fun <T> createSuccessDefaultDataNull(
        data: T? = null,
        successMsg: String = "",
        successCode: Int = SuccessCode.default,
    ): Ret<T?> {
