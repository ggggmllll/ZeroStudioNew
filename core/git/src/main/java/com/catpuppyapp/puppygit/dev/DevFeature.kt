...
object DevFeature {
  ...
  val external_data_storage =
      appendDevPrefix("External Data") // app 在 外部存储路径/Android/data/data 下的私有目录
  ...
  val singleDiff =
      object :
          DevItem<Boolean>(
              ...
              desc = "Enable for better performance",
          ) {
        override fun update(newValue: Boolean, context: Context?) {
          ...
          // 写入配置文件
          SettingsUtil.update { it.devSettings.singleDiffOn = newValue }
        }
      }

  // treatNoWordMatchAsNoMatched when diff
  val treatNoWordMatchAsNoMatchedForDiff =
      object :
          DevItem<Boolean>(
              text = "Treat No Words Matched as Non-Matched",
              state = mutableStateOf(false),
              desc =
                  "Treat none words matched as non-matched when Diff contents and enabled match by words",
          ) {
        override fun update(newValue: Boolean, context: Context?) {
          // 更新状态变量，使用的时候就不用查配置文件了
          state.value = newValue
          ...
          // 写入配置文件
          SettingsUtil.update { it.devSettings.treatNoWordMatchAsNoMatchedForDiff = newValue }
        }
      }

  // degrade match by words to match by chars when no matched
  val degradeMatchByWordsToMatchByCharsIfNonMatched =
      object :
          DevItem<Boolean>(
              text = "Degrade Match by words",
              state = mutableStateOf(false),
              desc =
                  "Degrade to Match by chars if Match by words was non-matched, not good for space-split language matching (like English), but good for non-space-split language (like Chinese)",
          ) {
        override fun update(newValue: Boolean, context: Context?) {
          // 更新状态变量，使用的时候就不用查配置文件了
          state.value = newValue
  ...
  val showMatchedAllAtDiff =
      object :
          DevItem<Boolean>(
              text = "Show 'No/All Matched' at Diff Screen",
        ...
        override fun update(newValue: Boolean, context: Context?) {
          ...
          // 写入配置文件
          SettingsUtil.update { it.devSettings.showMatchedAllAtDiff = newValue }
        }
      }

  // random launching text
  val showRandomLaunchingText =
      object :
          DevItem<Boolean>(text = "Show Random Launching Text", state = mutableStateOf(false)) {
        override fun update(newValue: Boolean, context: Context?) {
          // 更新状态变量，使用的时候就不用查配置文件了
          state.value = newValue
  ...
  val legacyChangeListLoadMethod =
      object :
          DevItem<Boolean>(
              text = "Legacy ChangeList Load Method",
              state = mutableStateOf(true),
              desc = "Better enable this, new method are unstable and maybe slower",
          ) {
        override fun update(newValue: Boolean, context: Context?) {
          // 更新状态变量，使用的时候就不用查配置文件了
          state.value = newValue
