...
class UndoStack(
        ...
        undoLock.withLock {
          redoLock.withLock {
            ...
            for ((_, v) in codeEditor.stylesMap) {
              // clean if undo/redo stacks doesn't contains fieldsId and the fieldsId neither equals
              // to latest editor state fieldsId
  ...
  suspend fun undoStackPush(state: TextEditorState, force: Boolean = false) {
    undoLock.withLock { redoLock.withLock { undoStackPushNoLock(state, force) } }
  }
  ...
  private fun undoStackPushNoLock(state: TextEditorState, force: Boolean = false): Boolean {
    ...
    //        val selectModeChanged = headState.isMultipleSelectionMode !=
    // state.isMultipleSelectionMode
    ...
    if (
        force ||
            ...
            undoLastSaveAt == 0L ||
            (now - undoLastSaveAt) > undoSaveIntervalInSec
    ) {
      ...
      if (undoStack.size.let { it > 0 && it > sizeLimit }) {
        ...
        if (
            lastHead.fieldsId != state.fieldsId &&
    ...
    if (
        state.fieldsId != codeEditor?.editorState?.value?.fieldsId &&
  ...
  suspend fun undoStackPop(): TextEditorState? {
    return undoLock.withLock { undoStackPopNoLock() }
  }
  ...
  //            if(stack.isEmpty() || !(state.fields.size==1 && stack.last().fields.size == 1 &&
  // state.fields.first()==stack.last().fields.first()) || stack.last().fieldsId != state.fieldsId)
  // {
  ...
  suspend fun contains(fieldsId: String): Boolean {
    return undoLock.withLock { redoLock.withLock { containsNoLock(fieldsId) } }
  }
  ...
  suspend fun clear() {
    undoLock.withLock { undoStack.clear() }
    redoLock.withLock { redoStack.clear() }
  }
