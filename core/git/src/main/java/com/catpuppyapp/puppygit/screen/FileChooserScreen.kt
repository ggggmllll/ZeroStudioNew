...
@Composable
fun FileChooserScreen(type: FileChooserType, naviUp: () -> Unit) {
  ...
  val refreshPage = { changeStateTriggerRefreshPage(needRefreshFilesPage) }
  ...
  val filesPageCurrentPath = rememberSaveable {
    val initPath =
        runCatching {
              ...
              if (tmpPath.isBlank()) {
                ...
                File(tmpPath).let {
                  (if (it.isFile) it.canonicalFile.parent else it.canonicalPath) ?: ""
                }
              }
            }
            .getOrDefault("")
    ...
    initPath.ifBlank {
      settings.files.lastOpenedPath.ifBlank {
