...
@Deprecated("用 `UserUtil.isPro()` 替代") private val dev_ProModeOn = false
...
var dev_EnableUnTestedFeature =
    false // release版此变量应设为假，具体是否启用未测试特性应由对应功能的开关变量控制；开发模式下一般此变量为真以方便访问所有功能
...
val editorMergeModeTestPassed =
    true // editor两处：1 从cl页面进入subeditor时是否以merge mode启动；2 editor页面菜单项merge mode功能
...
fun treeToTreeBottomBarActAtLeastOneTestPassed() =
...
val bug_Editor_SelectColumnRangeOfLine_Fixed =
    true // editor，搜索，定位到某行，选中某个范围，有bug，跳转到对应行，会自动取消选中，应该是Editor实现有问题，那代码我暂时不想看，先禁用选中关键字功能
val bug_Editor_GoToColumnCantHideKeyboard_Fixed =
    true // editor，启动时或非启动时，定位到某列，强制弹出键盘，无法实现“打开文件定位光标到上次编辑行的上次编辑列但不弹出键盘”
 ...
 * 调试用的flag文件
 *
 * 注：文件位置：仓库根目录下的PuppyGit-Data目录。 注：如无特别说明，创建或修改文件后均需重启app生效。
 *
 * flag files for debug
 *
 * note: all flag files under PuppyGit-Data note: create or edit flag files need restart app for
 * effect
 */
object FlagFileName {
  ...
  /**
   * 启用编辑缓存，存在此文件或设置项启用editcache开关开启则启用editcache。 if this file exists or settings editcache switch
   * to on, then will enable editcache
   */
  @Deprecated("use `settings.editor.editCacheEnable` instead")
  const val enableEditCache = "enableEditCache"

  /**
   * 若此文件存在则启用内容快照，否则是否启用内容快照取决于settings.editor.enableContentSnapshot if this file exists then
   * enable content snapshot, else demand on settings.editor.enableContentSnapshot
   */
  const val enableContentSnapshot = "enableContentSnapshot"

  /**
   * 若此文件存在则启用文件快照，否则是否启用文件快照取决于settings.editor.enableFileSnapshot if this file exists then enable
   * file snapshot, else demand on settings.editor.enableFileSnapshot
   */
  ...
  fun flagFileExist(flagFileName: String): Boolean {
    return File(AppModel.getOrCreatePuppyGitDataUnderAllReposDir().canonicalPath, flagFileName)
        .exists()
  }
}
