...
fun DiffPageActions(
    ...
    readOnlyModeOn:
        MutableState<
            Boolean
        >, // `readOnlyMode` was named `copyMode`，当时想的是只能拷贝不能编辑，所以叫拷贝模式，但后来发现这不就是只读模式吗，所以就改成只读模式了
  ...
  //            iconColor =
  // UIHelper.getIconEnableColorOrNull(requireBetterMatchingForCompare.value),
  //            enabled = true,
  //        ) {
  ...
  ////                + (if(requireBetterMatchingForCompare.value)
  // appContext.getString(R.string.on_str) else appContext.getString(R.string.off_str))
  ////            )
  //
  //            SettingsUtil.update {
  ...
  DropdownMenu(
        ...
        text = {
          Text(
              stringResource(R.string.create_patch) +
                  (if (isMultiMode) " (${stringResource(R.string.all)})" else "")
          )
        },
        onClick = {
    ...
    DropdownMenuItem(
        ...
        trailingIcon = { SimpleCheckBox(showLineNum.value) },
        onClick = {
          showLineNum.value = !showLineNum.value

          SettingsUtil.update { it.diff.showLineNum = showLineNum.value }
    ...
    DropdownMenuItem(
        ...
        trailingIcon = { SimpleCheckBox(showOriginType.value) },
        onClick = {
          showOriginType.value = !showOriginType.value

          SettingsUtil.update { it.diff.showOriginType = showOriginType.value }
    ...
    DropdownMenuItem(
        ...
        trailingIcon = { SimpleCheckBox(groupDiffContentByLineNum.value) },
        onClick = {
    ...
    if (fileChangeTypeIsModified && proFeatureEnabled(detailsDiffTestPassed)) {
      DropdownMenuItem(
          ...
          trailingIcon = { SimpleCheckBox(requireBetterMatchingForCompare.value) },
          onClick = {
            requireBetterMatchingForCompare.value = !requireBetterMatchingForCompare.value
      ...
      DropdownMenuItem(
          ...
          trailingIcon = { SimpleCheckBox(matchByWords.value) },
          onClick = {
            matchByWords.value = !matchByWords.value

            SettingsUtil.update { it.diff.matchByWords = matchByWords.value }
    ...
    // disabled reason: almost useless and must update syntax highlighting for all items when
    // switched, maintenance it is too much...
    ...
    DropdownMenuItem(
        ...
        trailingIcon = { SimpleCheckBox(readOnlyModeOn.value) },
        onClick = {
          readOnlyModeOn.value = !readOnlyModeOn.value

          SettingsUtil.update { it.diff.readOnly = readOnlyModeOn.value }
    ...
    DropdownMenuItem(
        ...
        trailingIcon = { SimpleCheckBox(enableSelectCompare.value) },
        onClick = {
          enableSelectCompare.value = !enableSelectCompare.value

          SettingsUtil.update { it.diff.enableSelectCompare = enableSelectCompare.value }
