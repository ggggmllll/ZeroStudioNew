...
object EncodingUtil {
  ...
  private fun makeSureUseASupportedCharset(originCharset: String?): String {
    ...
    if (
        originCharset == Constants.CHARSET_X_ISO_10646_UCS_4_3412 ||
  ...
  fun detectEncoding(newInputStream: () -> InputStream): String {
    return try {
      ...
      if (AppModel.devModeOn) {
        MyLog.d(
            TAG,
            "#detectEncoding() err, will use '$defaultCharsetName', err msg=${e.localizedMessage}",
        )
        e.printStackTrace()
      }
  ...
  fun resolveCharset(charsetName: String?): Charset {
    try {
      ...
      MyLog.e(
          TAG,
          "#resolveCharset err, will use '$defaultCharsetName', param `charsetName`=$charsetName, err=${e.localizedMessage}",
      )
      e.printStackTrace()
   ...
   *
   * note: even isn't utf8bom, the `inputStream` still already read
   * (即使不是utf8bom，inputStream也已经被读取过了)
   */
  ...
  fun ignoreBomIfNeed(newInputStream: () -> InputStream, charsetName: String?): IgnoreBomResult {
    ...
    if (charsetName == Constants.CHARSET_UTF_16LE) {
      ...
      if (inputStream.read() == 0xFF && inputStream.read() == 0xFE) {
        return IgnoreBomResult(true, inputStream)
      }
    }
    ...
    if (charsetName == Constants.CHARSET_UTF_16BE) {
      ...
      if (inputStream.read() == 0xFE && inputStream.read() == 0xFF) {
        return IgnoreBomResult(true, inputStream)
      }
    }
