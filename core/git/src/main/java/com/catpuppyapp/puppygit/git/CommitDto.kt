...
data class CommitDto(
    ...
    var branchShortNameList: MutableList<String> =
        mutableListOf(), // 分支名列表，master origin/master 之类的，能通过看这个判断出是否把分支推送到远程了
    var parentOidStrList: MutableList<String> =
        mutableListOf(), // 父提交id列表，需要的时候可以根据这个oid取出父提交，然后可以取出父提交的树，用来diff
    ...
    var isGrafted: Boolean = false, // shallow root，此值为true，参考pcgit，在打印shallow仓库时，会对shallow
    // root添加grafted标识，不过需要注意，一个提交列表可能有多个isGrafted，这种情况发生于多个grafted是同一个提交的父提交的情况，换句话说，那个子提交是由多个父提交合并来的
  ...
  fun getActuallyUsingTimeZoneUtcFormat(settings: AppSettings): String {
    val minuteOffset =
        readTimeZoneOffsetInMinutesFromSettingsOrDefault(settings, originTimeOffsetInMinutes)

    return formatMinutesToUtc(minuteOffset)
  }

  // 在列表条目使用的缓存信息
  fun getCachedOneLineMsg(): String =
      (cached_OneLineMsg
          ?: Libgit2Helper.zipOneLineMsg(msg).let {
            cached_OneLineMsg = it
            it
          })

  fun cachedBranchShortNameList(): String =
      cached_BranchShortNameList
          ?: branchShortNameList
              .joinToString { it }
              .let {
                cached_BranchShortNameList = it
                it
              }

  fun cachedTagShortNameList(): String =
      cached_TagShortNameList
          ?: tagShortNameList
              .joinToString { it }
              .let {
                cached_TagShortNameList = it
                it
              }

  fun cachedParentShortOidStrList(): String =
      cached_ParentShortOidStrList
          ?: parentShortOidStrList
              .joinToString { it }
              .let {
                cached_ParentShortOidStrList = it
                it
              }

  // 在详情弹窗使用的缓存信息
  // 先换行，然后每行一个
  fun cachedLineSeparatedBranchList(): String =
      cached_LineSeparated_BranchShortNameList
          ?: branchShortNameList
              .joinToString(separator = "\n", prefix = "\n") { it }
              .let {
                cached_LineSeparated_BranchShortNameList = it
                it
              }

  fun cachedLineSeparatedTagList(): String =
      cached_LineSeparated_TagShortNameList
          ?: tagShortNameList
              .joinToString(separator = "\n", prefix = "\n") { it }
              .let {
                cached_LineSeparated_TagShortNameList = it
                it
              }

  fun cachedLineSeparatedParentFullOidList(): String =
      cached_LineSeparated_ParentFullOidStrList
          ?: parentOidStrList
              .joinToString(separator = "\n", prefix = "\n") { it }
              .let {
                cached_LineSeparated_ParentFullOidStrList = it
                it
              }

  fun getFormattedAuthorInfo() = Libgit2Helper.getFormattedUsernameAndEmail(author, email)

  fun getFormattedCommitterInfo() =
      Libgit2Helper.getFormattedUsernameAndEmail(committerUsername, committerEmail)
}
