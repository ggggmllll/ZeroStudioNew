...
fun FilesInnerPage(
  ...
  val settingsSnapshot =
      mutableCustomStateOf(keyTag = stateKeyTag, keyName = "settingsSnapshot") {
  ...
  //    val selFilePathListJsonObjStr = rememberSaveable{ mutableStateOf("{}") }
  ...
  ////    val selFilePathListJsonObj = JSONObject()
  // //想着复用同一个对象呢，不过没那个api，看下如果性能还行，就这样吧，不行的话，换别的json库
  ...
  val resetAct = { requireCancelAct.value = false }

  val cancelAct = { requireCancelAct.value = true }
  ...
  //    val repoList = mutableCustomStateListOf(keyTag = stateKeyTag, keyName = "repoList",
  // initValue = listOf<RepoEntity>())
  ...
  //    val currentPathBreadCrumbList = remember{ mutableIntStateOf(1) }
  // //为0时刚进页面，初始化，后续为1时读取list1修改list2，为2时读取list2修改list1，避免并发修改异常
  ...
  val switchItemSelected = { item: FileItemDto ->
    ...
    UIHelper.selectIfNotInSelectedListElseRemove(
        item,
        selectedItems.value,
        contains = containsForSelected,
    )
  }

  val selectItem = { item: FileItemDto ->
    isFileSelectionMode.value = true
    UIHelper.selectIfNotInSelectedListElseNoop(
        item,
        selectedItems.value,
        contains = containsForSelected,
    )
  ...
  val getSelectedFilesCount = { selectedItems.value.size }
  ...
  if (showGoToPathDialog.value) {
    ...
    val goToDialogOnOk = {
      ...
      doJobThenOffLoading {
        ...
        // handle path to absolute path, btw: internal path must before external path, because
        // internal actually starts with external, if swap order, code block of internal path will
        // ignore ever
        ...
        textCompose = {
          ScrollableColumn {
            TextField(
                ...
                modifier =
                    Modifier.fillMaxWidth().focusRequester(focusRequester).onPreviewKeyEvent { event
                      ->
                ...
                //                        singleLine = true,
                onValueChange = { pathToGo.value = it },
                label = { Text(stringResource(R.string.path)) },

                // 把软键盘回车替换成Go
                keyboardOptions = KeyboardOptions(imeAction = ImeAction.Go),
                // 点击Go直接跳转路径
                keyboardActions = KeyboardActions(onGo = { goToDialogOnOk() }),
            )
          }
        },
  ...
  val loadingRepoListJob =
      mutableCustomBoxOf<Job?>(stateKeyTag, "loadingRepoListJob") {
        null
      } // 如果不remember，这个值会被反复赋值，导致之前存的任务被冲掉
    ...
    loadingRepoListJob.value = doJob job@{
      val result = runCatching {
        ...
        if (listFromDb.isNotEmpty()) {
          ...
          // repo name match dir name first, it none match, check last selected repo id , if valid ,
          // use it, else use idx 0
          val repoNameMatchedDirNameIdx =
              listFromDb
                  .indexOfFirst { it.repoName == parentName }
                  .let { if (it < 0) listFromDb.indexOfFirst { selectedRepoId == it.id } else it }
  ...
  if (showApplyAsPatchDialog.value) {
    ApplyPatchDialog(
        ...
        onOkCallback = { Msg.requireShow(activityContext.getString(R.string.success)) },
    )
  }
  ...
  val fileNameForOpenAsDialog = remember {
    derivedStateOf { getFileNameFromCanonicalPath(openAsDialogFilePath.value) }
  }
  ...
  if (showOpenAsDialog.value) {
    OpenAsDialog(
        ...
        fileName = fileNameForOpenAsDialog.value,
        filePath = openAsDialogFilePath.value,
        showOpenInEditor = showOpenInEditor.value,
  ...
  val fileMenuKeyTextList =
      listOf(
          if (isFileChooser) ""
          else
              stringResource(
                  R.string.open
              ), // 用内部编辑器打开，得加个这个选项，因为如果自动判断文件该用外部程序打开，但又没对应的外部程序，用户又想用内部编辑器打开，但文件管理器又判断文件该用内部文件打开，就死局了，所以得加这个选项
          if (isFileChooser) "" else stringResource(R.string.open_as),
          stringResource(R.string.rename),
          if (isFileChooser) ""
          else if (proFeatureEnabled(applyPatchTestPassed)) stringResource(R.string.apply_as_patch)
          else "", // 应用patch，弹窗让用户选仓库，然后可对仓库应用patch
  ...
  //        copyThenShowCopied(getFilePathStrBasedRepoDir(realFullPath,
  // returnResultStartsWithSeparator = true))
  //    }
  ...
  val copyRepoRelativePath = { realFullPath: String ->
    try {
      ...
      if (repo == null) {
        ...
        repo.use {
          val realtivePath =
              Libgit2Helper.getRelativePathUnderRepo(
                  Libgit2Helper.getRepoWorkdirNoEndsWithSlash(it),
                  realFullPath,
              )

          // well, the repo already found, but the path got null, usually it happens when trying to
          // copy a repo-relative-path from a repo folder
  ...
  val details_ItemsSize = rememberSaveable {
    mutableLongStateOf(0L)
  } // this is not items count, is file size. this size is a recursive count
  val details_AllCount = rememberSaveable {
    mutableIntStateOf(0)
  } // selected items count(folder + files). note: this is not a recursive count
  val details_FilesCount = rememberSaveable {
    mutableIntStateOf(0)
  } // files count in selected items. not recursive count
  val details_FoldersCount = rememberSaveable {
    mutableIntStateOf(0)
  } // folders count in selected items. not recursive count
  val details_CountingItemsSize = rememberSaveable {
    mutableStateOf(false)
  } // indicate is calculating file size or finished
  val details_itemList =
      mutableCustomStateListOf(stateKeyTag, "details_itemList", listOf<FileItemDto>())

  val initDetailsDialog = { list: List<FileItemDto> ->
    val list =
        if (list.size == 1 && list.first().isDir) {
  ...
  fun addStoragePath(
    ...
    try {
      if (File(newPath).isDirectory.not()) {
        if (showMsg) {
          Msg.requireShowLongDuration(
              "err: " + activityContext.getString(R.string.path_is_not_a_dir)
          )
        }
      ...
      // save if not contains and not equals to app internal storage path(this is default repos
      // storage path, can't delete or add, it always at first item of the Storage Paths in the
      // CloneScreen)
  ...
  val listForDeleteDialog =
      mutableCustomStateListOf(stateKeyTag, "listForDeleteDialog") { listOf<FileItemDto>() }
  ...
  val initDelFileDialog = { list: List<FileItemDto> ->
    ...
    //        fileCountForDelDialog.value = allCountForDelDialog.value -
    // folderCountForDelDialog.value
    ...
    ConfirmDialog2(
        ...
        text =
            replaceStringResList(
                stringResource(R.string.n_items_will_be_deleted),
                listOf("" + allCountForDelDialog.value),
            ),
      ...
      doJobThenOffLoading(loadingOn = loadingOn, loadingOff = loadingOff) {
        ...
        targets.forEachBetter {
          ...
          //                        MyLog.w(TAG, "#DelFileDialog: may delete file under '.git'
          // folder, fullPath will del is: '${file.canonicalPath}'")
          //                    }
  ...
  val renameFile = { item: FileItemDto ->
    ...
    renameFileName.value =
        TextFieldValue(item.name, selection = getRangeForRenameFile(item.name)) // 旧文件名
  ...
  val fileMenuKeyActList =
      listOf<(FileItemDto) -> Unit>(
          ...
          applyAsPatch@{ item: FileItemDto ->
            ...
            //            Unit  // for make return type is Unit, or specify type at declare
            // statement
          },
          //        copyPath@{
          //            copyPath(it.fullPath)
          //        },
          fileHistory@{ goToFileHistory(it.fullPath, activityContext) },
          copyFullPath@{ copyThenShowCopied(it.fullPath) },
          copyRepoRelativePath@{ copyRepoRelativePath(it.fullPath) },
          details@{ initDetailsDialog(listOf(it)) },
          share@{ shareFiles(listOf(it)) },
          delete@{ initDelFileDialog(listOf(it)) },
  ...
  val dirMenuKeyActList =
      listOf(
          renameFile,
          copyFullPath@{ copyThenShowCopied(it.fullPath) },
          copyRepoRelativePath@{ copyRepoRelativePath(it.fullPath) },
          importAsRepo@{ initImportAsRepoDialog(listOf(it.fullPath)) },
          initRepo@{ initInitRepoDialog(listOf(it.fullPath)) },
          addStoragePath@{
        ...
        textCompose = {
          MySelectionContainer {
            ScrollableColumn {
              Row {
                Text(
                    text = stringResource(R.string.import_success) + ": " + successImportCount.value
                )
              }
              Row {
              ...
              Row(
                  modifier =
                      Modifier.clickable {
                        ...
                        // Msg.requierShow() 还得刷新页面才能看到信息，不如直接用Toast
                        //
                        // changeStateTriggerRefreshPage(needRefreshFilesPage)
                        // test
                      }
              ...
              Row { Text(text = stringResource(R.string.err_msg) + ":") }
              Row { Text(text = failedImportListStr.value, color = MyStyleKt.TextColor.error()) }
  ...
  if (showRenameDialog.value) {
    ...
    val onOk = {
      try {
        ...
        val fileOrFolderNameCheckRet =
            checkFileOrFolderNameAndTryCreateFile(newFileName, activityContext)
        if (fileOrFolderNameCheckRet.hasError()) { // 检测是否有坏字符，例如路径分隔符
          ...
          // 检测新旧文件名是否相同 以及 新文件名是否已经存在，两者都视为文件已存在
            ...
            newFileName == oldFileItemDto.name ||
                isPathExists(File(oldFileItemDto.fullPath).parent, newFileName)
        ) {
          renameHasErr.value = true
          ...
          doJobThenOffLoading(loadingOn = loadingOn, loadingOff = loadingOff) {
            try {
              ...
              MyLog.e(
                  TAG,
                  "rename failed: oldFile=${oldFileItemDto.fullPath}, newFileName=${newFileName}, err=${e.stackTraceToString()}",
              )
            }
          }
        }
        ...
        textCompose = {
          ScrollableColumn {
            TextField(
                ...
                onValueChange = { updateRenameFileName(it) },
                label = { Text(stringResource(R.string.file_name)) },
                placeholder = { Text(stringResource(R.string.input_new_file_name)) },
  ...
  val fileNameForCreateDialog =
      mutableCustomStateOf(stateKeyTag, "fileNameForCreateDialog") { TextFieldValue("") }
  //    val fileTypeOptionsForCreateDialog = remember
  // {listOf(activityContext.getString(R.string.file), activityContext.getString(R.string.folder))}
  // // idx: 0 1
              ...
              try {
                ...
                if (!File(currentPath()).exists()) {
                  throw RuntimeException(
                      activityContext.getString(R.string.current_dir_doesnt_exist_anymore)
                  )
                }

                val fileOrFolderNameCheckRet =
                    checkFileOrFolderNameAndTryCreateFile(fileOrFolderName, activityContext)
                  ...
                  if (file.exists()) { // 文件存在
                    ...
                    if (isCreateSuccess) { // 创建成功
                      ...
                      createFileOrFolderErrMsg.value =
                          errorStrRes // 设置错误信息为err，不过没有具体信息，用户虽然不知道出了什么错，但知道出错了，而且仍可点取消关闭弹窗，所以问题不大
                      return@f false
                    }
                  }
                }
                ...
                MyLog.e(
                    TAG,
                    "CreateFileOrFolderDialog in Files Page err: " + e.stackTraceToString(),
                )
  ...
  // filterListState.value.firstVisibleItemIndex else curListState.value.firstVisibleItemIndex } }
  //    ScrollListener(
  ...
  val findRepoThenGoToReposOrChangList =
      { fullPath: String, trueGoToReposFalseGoToChangeList: Boolean ->
        doJobThenOffLoading job@{
          try {
            ...
            MyLog.e(
                TAG,
                "#findRepoThenGoToReposOrChangList err: fullPath=$fullPath, trueGoToReposFalseGoToChangeList=$trueGoToReposFalseGoToChangeList, err=${e.localizedMessage}",
            )
          }
        }
      }

  val showInRepos = { fullPath: String -> findRepoThenGoToReposOrChangList(fullPath, true) }

  val showInChangeList = { fullPath: String -> findRepoThenGoToReposOrChangList(fullPath, false) }

  val showViewAndSortDialog = rememberSaveable { mutableStateOf(false) }
  val viewAndSortState =
      mutableCustomStateOf(stateKeyTag, "viewAndSortState") {
        settingsSnapshot.value.files.defaultViewAndSort
      }
  val viewAndSortStateBuf =
      mutableCustomStateOf(stateKeyTag, "viewAndSortStateBuf") {
        settingsSnapshot.value.files.defaultViewAndSort
      }
        ...
        textCompose = {
          ScrollableColumn {
            SingleSelection(
                ...
                onClick = { idx, item ->
                  viewAndSortStateBuf.value = viewAndSortStateBuf.value.copy(sortMethod = item.code)
                },
                minHeight = MyStyleKt.RadioOptions.middleHeight,
            )

            Spacer(Modifier.height(height))
            MyCheckBox2(stringResource(R.string.ascend), viewAndSortStateBuf.value.ascend) {
                newValue ->
              viewAndSortStateBuf.value = viewAndSortStateBuf.value.copy(ascend = newValue)
            }

            MyCheckBox2(
                stringResource(R.string.folder_first),
                viewAndSortStateBuf.value.folderFirst,
            ) { newValue ->
      ...
      doJobThenOffLoading {
        ...
        if (
            onlyForThisFolderStateBuf.value != onlyForThisFolderState.value ||
                viewAndSortStateBuf.value.equals(viewAndSortState.value).not()
        ) {
          ...
          settingsSnapshot.value =
              SettingsUtil.update(requireReturnSnapshotOfUpdatedSettings = true) {
                if (onlyForThisFolderStateBuf.value) {
                  ...
                  // try remove, if hase dir specified sort method, if hasn't remove is ok as well,
    ...
    if (isLoading.value) {
      ...
      Column(modifier = Modifier.fillMaxSize().padding(contentPadding)) {
          ...
          LazyRow(
            ...
            breadList.forEachIndexedBetter { idx, it ->
              item {
                ...
                //                            val curPathIsRootAndCurItemIsRoot = curPathIsRoot &&
                // curItemIsRoot
                val textColor =
                    if (it.fullPath.startsWith(currentPath() + separator)) Color.Gray
                ...
                Text(
                    ...
                    color = textColor,
                    fontWeight =
                        if (it.fullPath == currentPath()) FontWeight.Bold else FontWeight.Normal,
                    modifier =
                  ...
                  Column {
                    ...
                    DropdownMenu(
                      ...
                      DropdownMenuItem(
                          ...
                          onClick = {
                            breadCrumbDropDownMenuExpandState.value = false
                            // bread crumb dto lack some info for faster loading, so need requrey a
                            // new dto when show details
                            // 面包屑的dto是缩水的，为了加载快而没查最后修改时间和大小等在面包屑用不到的信息，因此显示前需要重新查下dto
                            initDetailsDialog(
                                listOf(
                                    FileItemDto.genFileItemDtoByFile(
                                        File(it.fullPath),
                                        activityContext,
                                    )
                                )
          ...
          val scrollToCurPath =
              remember {
                    derivedStateOf {
                      ...
                      val indexOfCurPath =
                          currentPathBreadCrumbList.value.indexOfFirst {
                            it.fullPath == currentPath()
                          }
                      if (indexOfCurPath != -1) {
        ...
        if (hasErr || folderIsEmpty) {
          Column(
              modifier =
                  Modifier.baseVerticalScrollablePageModifier(contentPadding, errScrollState)
                      .padding(MyStyleKt.defaultItemPadding),
          ...
          val enableFilter =
              filterModeActuallyEnabled(filterOn = filesPageSimpleFilterOn.value, keyword = keyword)

          val lastNeedRefresh = rememberSaveable { mutableStateOf("") }
          val currentPathFileList =
              filterTheList(
                      ...
                      DirSearchUtil.realBreadthFirstSearch(
                          ...
                          matchedCallback = { idx, item ->
                            filterList.value.add(
                                FileItemDto.genFileItemDtoByFile(item, activityContext)
                            )
                          },
                          canceled = canceled,
                      )
            ...
            FileListItem(
                  ...
                  if (isFileChooser.not()) {
                    ...
                    if (!isFileSelectionMode.value && !isPasteMode.value && !isImportMode.value) {
                      ...
                      // 如果处于选择模式，长按执行连续选择
                        ...
                        isFileSelectionMode.value && !isPasteMode.value && !isImportMode.value
                    ) {
                      //
                      // haptic.performHapticFeedback(HapticFeedbackType.LongPress)
              ...
              if (isFileSelectionMode.value) { // 选择模式，切换选择
                ...
                if (it.isFile) {
                  ...
                  if (
                      RegexUtil.equalsOrEndsWithExt(
                          it.name,
                          SettingsUtil.obtainEditorFileAssociationList(),
                      )
                  ) {
                    // 请求打开文件
                    val expectReadOnly = false
                    ...
                    // MimeType.ANY.intentType) {
                    if (mimeType.startsWith("text/")) { // 文本类型，直接用内部编辑器打开
                      val expectReadOnly = false
                      requireInnerEditorOpenFile(it.fullPath, expectReadOnly)
                    } else if (
                        mimeType == MimeType.ANY.intentType
                    ) { // Any, */* ， 无后缀或没检测出具体类型就会是这个
                      ...
                      val openSuccess =
                          FsUtils.openFile(activityContext, it.toFile(), mimeType, readOnly = false)
  ...
  //    val copyOrMoveOrExportFileErrList = mutableCustomStateListOf(stateKeyTag,
  // "copyOrMoveOrExportFileErrList") { mutableListOf<PasteResult>() }
  ...
  val copyOrMoveOrExportFile =
      copyOrMoveOrExportFile@{
          srcList: List<FileItemDto>,
          targetFullPath: String,
          requireDeleteSrc: Boolean ->
        ...
        doJobThenOffLoading(loadingOn = loadingOn, loadingOff = loadingOff) {
          val ret =
              FsUtils.copyOrMoveOrExportFile(
                  srcList.map { it.toFile() },
                  File(targetFullPath),
                  requireDeleteSrc,
              )
          if (ret.hasError()) {
  ...
  val itemListForExport =
      mutableCustomStateListOf(stateKeyTag, "itemListForExport") { listOf<FileItemDto>() }
  ...
  val chooseDirLauncher =
      rememberLauncherForActivityResult(MyOpenDocumentTree()) { uri ->
        // 执行导出
        if (uri != null) {
  ...
  if (showSafExportDialog.value || showSafImportDialog.value) {
    val importOrExportText =
        if (showSafExportDialog.value) stringResource(R.string.export)
        else stringResource(R.string.import_str)
    val closeDialog = {
      showSafExportDialog.value = false
      showSafImportDialog.value = false
    }
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            MySelectionContainer {
              CardButton(
                  onClick = { chooseDirLauncher.launch(null) },
                  content = {
                    Text(
                        text =
                            if (choosenSafUri.value == null) stringResource(R.string.select_path)
                            else choosenSafUri.value.toString(),
                        color =
                            UIHelper.getCardButtonTextColor(
                                enabled = true,
      ...
      val loadingText =
          activityContext.getString(
              if (trueExportFalseImport) R.string.exporting else R.string.importing
          )

      doJobThenOffLoading {
        val uri = choosenSafUri.value!!
        val conflictStrategy =
            if (safImportExportOverwrite.value)
                FsUtils.CopyFileConflictStrategy.OVERWRITE_FOLDER_AND_FILE
            else FsUtils.CopyFileConflictStrategy.SKIP
        ...
        //            appContext.contentResolver.openOutputStream(chosenDir?.createFile("*/*",
        // "test.txt")?.uri!!)
        try {
          ...
          if (trueExportFalseImport) {
            FsUtils.recursiveExportFiles_Saf(
                ...
                targetDir = chosenDir,
                srcFiles =
                    itemListForExport.value.map<FileItemDto, File> { it.toFile() }.toTypedArray(),
        ...
        textCompose = {
          ScrollableColumn {
            ...
            SingleLineCardButton(
                text = if (cancellableActRunning.value) "Cancel!" else "Diff!",
                enabled = true,
            ) {
            ...
            if (safDiffResultStr.value.isNotBlank()) {
              ...
              MySelectionContainer {
                CardButton(
                    ...
                    maxHeight = 500,
                    onClick = {},
                    content = { Text(safDiffResultStr.value) },
                )
              }
            }
  ...
  val chooseDirLauncherThenExport =
      rememberLauncherForActivityResult(MyOpenDocumentTree()) exportSaf@{ uri ->
        // 执行导出
        if (uri != null) {
          SafUtil.takePersistableRWPermission(activityContext.contentResolver, uri)
          ...
          doJobThenOffLoading {
            ...
            try {
              ...
              FsUtils.recursiveExportFiles_Saf(
                  ...
                  targetDir = chosenDir,
                  srcFiles =
                      selectedItems.value.map<FileItemDto, File> { it.toFile() }.toTypedArray(),
                  canceled = { requireCancelAct.value },
              )
              // throw RuntimeException("测试异常！")  passed
          ...
          try {
            ...
            selctedDirs.forEachBetter { dirPath ->
              try {
                ...
                //                            Msg.requireShowLongDuration(e.localizedMessage ?:
                // "err")
                MyLog.e(
                    TAG,
                    "init repo in FilesPage err: path=${dirPath}, err=${e.localizedMessage}",
                )
              }
            }
            ...
            Msg.requireShowLongDuration(
                replaceStringResList(
                    activityContext.getString(R.string.n_inited),
                    listOf("" + successCnt),
                )
            )
          } finally {
            if (enableFilterState.value.not()) {
              ...
              filterList.toList().forEachIndexedBetter { idx, item ->
                if (selctedDirs.contains(item.fullPath)) {
                  filterList[idx] =
                      FileItemDto.genFileItemDtoByFile(File(item.fullPath), activityContext)
                }
              }
            }
          }
      ...
      ConfirmDialog(
          ...
          textCompose = {
            Column(
              ...
              MyCheckBox(
                  text = stringResource(R.string.paths_are_repo_parent_dir),
                  value = isReposParentFolderForImport,
              )
        ...
        doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.importing)) {
          ...
          try {
            selctedDirs.forEachBetter { dirPath ->
              val result =
                  AppModel.dbContainer.repoRepository.importRepos(
                      dir = dirPath,
                      isReposParent = isReposParentFolderForImport.value,
                  )
            ...
            MyLog.e(
                TAG,
                "import repo from FilesPage err: importRepoResult=$importRepoResult, err=" +
          ...
          MySelectionContainer {
            ScrollableColumn {
                  ...
                  Text(
                      text =
                          replaceStringResList(
                              ...
                              listOf(
                                  "" + details_AllCount.intValue,
                                  "" + details_FoldersCount.intValue,
                                  "" + details_FilesCount.intValue,
                              ),
                          )
                  )
                }
              ...
              if (itemList.size == 1) {
                ...
                Row { Text(text = stringResource(R.string.name) + ": " + item.name) }
                    ...
                    Text(
                        text =
                            replaceStringResList(
                                ...
                                listOf(
                                    replaceStringResList(
                                        stringResource(R.string.folder_n_file_m),
                                        listOf("" + item.folderCount, "" + item.fileCount),
                                    )
                                ),
                            )
                    )
                ...
                Row { Text(text = stringResource(R.string.path) + ": " + item.fullPath) }
  ...
  val goToParentAndScrollToItem = { item: FileItemDto ->
    ...
    changeStateTriggerRefreshPage(
        ...
        StateRequestType.goToParentAndScrollToItem,
        item.fullPath,
    )
  }
  ...
  if (showSelectedItemsShortDetailsDialogForImportMode.value) {
    SelectedFileItemsDialog(
        ...
        // because usually only can share files, even can try import folder, but will got an err at
        // the end,
    ...
    GitIgnoreDialog(
        ...
        getRepository = {
          try {
            ...
            if (repoWillUse == null) {
              Msg.requireShowLongDuration(
                  activityContext.getString(R.string.err_dir_is_not_a_git_repo)
              )
            }
        ...
        getIgnoreItems = { repoWorkDirFullPath: String ->
          ...
          selectedItemList.map {
            IgnoreItem(
                pathspec = getFilePathUnderParent(repoWorkDirFullPath, it.fullPath),
                isFile = it.isFile,
            )
          }
        },
  ...
  val countNumOnClickForSelectAndPasteModeBottomBar = {
    ...
    //            sb.appendLine("${it.name}, ${if(it.isDir) "dir" else "file"},
    // ${it.fullPath}").appendLine()
    //        }
  ...
  if (isFileChooserAndSingleDirType || isFileSelectionMode.value) {
    val selectionModeIconList =
        if (isFileChooserAndSingleDirType) {
          listOf(
    ...
    val selectionModeIconOnClickList =
        if (isFileChooserAndSingleDirType) {
          listOf(confirm@{ confirmForChooser(currentPath()) })
        } else {
          listOf<() -> Unit>(
              delete@{ initDelFileDialog(selectedItems.value) },
              move@{ setPasteModeThenShowPasteBar(pasteMode_Move) },
              copy@{ setPasteModeThenShowPasteBar(pasteMode_Copy) },
              selectAll@{ selectAll() },
          )
        }

    val selectionModeIconEnableList =
        if (isFileChooserAndSingleDirType) {
          listOf(
    ...
    val selectionModeMoreItemTextList =
        (listOf(
            stringResource(id = R.string.ignore),
            stringResource(id = R.string.remove_from_git),
            stringResource(id = R.string.details),
            stringResource(R.string.share),
            if (proFeatureEnabled(importReposFromFilesTestPassed))
                stringResource(id = R.string.import_as_repo)
            else "", // empty string will be ignore when display menu items
            if (proFeatureEnabled(initRepoFromFilesPageTestPassed))
                stringResource(id = R.string.init_repo)
            else "",
            stringResource(R.string.add_storage_path),
            if (showImportForBottomBar) stringResource(R.string.import_str)
            else "", // 底栏没必要显示import，避免以后修改，保留代码，用开关变量控制是否显示
            stringResource(R.string.export),
        ))

    val selectionModeMoreItemOnClickList =
        (listOf(
            ignore@{ showIgnoreDialog.value = true },
            removeFromGit@{ showRemoveFromGitDialog.value = true },
            details@{ initDetailsDialog(selectedItems.value.toList()) },
            share@{ shareFiles(selectedItems.value) },
            importAsRepo@{
    ...
    val selectionModeMoreItemEnableList =
        (listOf(
            ...
            //            {selectedItems.value.indexOfFirst{it.isDir} != -1}  //enable import as
            // repo. (if has dirs in selected items, then enable else disbale) (after clicked then
            // check better than check at every time selected list change)
  ...
  if (isImportMode.value) {
        ...
        listOf(
            importFiles@{
              doJobThenOffLoading(
                  loadingOn = loadingOn,
                  loadingOff = loadingOff,
                  loadingText = activityContext.getString(R.string.importing),
              ) {
                ...
                requireImportUriList.value.toList().forEachBetter forEach@{ it: Uri? ->
                  try {
                    if (it != null && it.path != null && it.path!!.length > 0) {
                      ...
                      //                                val srcDocumentFile =
                      // FsUtils.getDocumentFileFromUri(appContext, it)

                      val src = File(it.path!!)
                      if (src.isDirectory) { // 不支持拷贝目录
                        failedCnt++
                        sb.appendLine(
                            "'${it.path}'" + ": is a directory, only support import files!"
                        )
                        return@forEach
                      }

                      // x
                      // 作废)执行到这有可能获取srcDocumentFile失败，其值为null；也有可能成功，且判断其是文件，下面不再做判断，直接拷贝，反正有异常捕获兜底
                      ...
                      if (srcFileName == null) { // 获取真实文件名失败
                        ...
                        MyLog.w(
                            TAG,
                            "#importFiles@: getFileRealNameFromUri() return null, will use src.name:${srcFileName}",
                        )
                        if (srcFileName.isNullOrEmpty()) { // 获取文件名失败
                          ...
                          MyLog.w(
                              TAG,
                              "#importFiles@:src.name is null or empty, will use a random name:${srcFileName}",
                          )
                        }
                      }
                      ...
                      curTarget =
                          ...
                          ) // 获得target后，立刻创建一个拷贝，用来在异常时判断是否更新了target来决定是否删除target
                      //
                    ...
                    // 检查目标文件是否存在，如果存在，删除 。
                    // 算了，不删了，让用户自己看着办吧！如果拷贝失败，有可能依然创建了文件，但文件大小为0，但如果不为0呢？我还得判断，算了，不管了，让用户自己看着办吧！算了，还是删一下吧！
                    failedCnt++
                    sb.appendLine(
                        (it?.path ?: "/fileNameIsNull/") + ": error: " + e.localizedMessage
                    )
                    ...
                    try {
                      if (curTarget != null) {
                        ...
                        if (
                            curFilePath.isNotBlank() &&
                                curFilePath != previousFilePath &&
                                curTarget!!.exists() &&
                                curTarget!!.length() <= 0
                        ) {
                          curTarget!!.delete()
    ...
    val countNumOnClickForImportMode = {
      ...
      //            }
      //
      //            selectedItemsShortDetailsStrForImportMode.value =
      // sb.removeSuffix("\n").toString()
  ...
  if (showImportExportErrorDialog.value) {
    val closeDialog = {
      showImportExportErrorDialog.value = false
      importExportErrorMsg.value = ""
    }
    ...
    ConfirmDialog(
        ...
        textCompose = {
          CopyScrollableColumn {
            Row { Text(text = stringResource(id = R.string.will_export_files_to) + ":") }
            ...
            Spacer(modifier = Modifier.height(10.dp))
            Row { Text(text = stringResource(id = R.string.are_you_sure)) }
          }
        },
      ...
      if (ret.hasError() || ret.data == null || !ret.data!!.exists()) {
        Msg.requireShowLongDuration(
            activityContext.getString(R.string.get_default_export_folder_failed_plz_choose_one)
        )
  ...
  if (isPasteMode.value) {
    val iconList =
        listOf(
            ...
            if (pasteMode.intValue == pasteMode_Move) Icons.Filled.ContentCut
            else Icons.Filled.ContentCopy,
        )
    val iconTextList =
        listOf(
            stringResource(R.string.paste),
            if (pasteMode.intValue == pasteMode_Move) stringResource(R.string.cut)
        ...
        listOf(
            paste@{
              copyOrMoveOrExportFile(
                  selectedItems.value,
                  currentPath(),
                  pasteMode.intValue == pasteMode_Move,
              ) // 最后一个参数代表是否删除源，如果是move，则删除
              Unit
            },
            cutOrCopyIndicator@{},
        )
  ...
  //                      Row {
  //                          Text(text = it.name+(if(it.isDirectory) File.separator else ""))
  // //如果是目录，后面加个 "/"
  //                      }
  //                  }
  //
  ...
  //                        if(target.exists()) {
  //                            target.de
  // //这里本来想如果存在则删除，但是，不行，如果是目录的话，不行，得合并而不是覆盖！算了，如果重名直接自动重命名好了，覆盖太麻烦了
  //                        }else {
  //                            src.copyRecursively(target, false)
  // //false，禁用覆盖，不过，只有文件存在时才需要覆盖，而上面其实已经判断过了，所以执行到这，target肯定不存在，也用不着覆盖，但以防万一，这个值传false，避免错误覆盖文件
  ...
  if (filesPageRequestFromParent.value == PageRequest.safDiff) {
    PageRequest.clearStateThenDoAct(filesPageRequestFromParent) { initSafDiffDialog() }
  }

  if (filesPageRequestFromParent.value == PageRequest.safImport) {
    PageRequest.clearStateThenDoAct(filesPageRequestFromParent) { initSafImportDialog() }
  }
    ...
    PageRequest.clearStateThenDoAct(filesPageRequestFromParent) {
      ...
      if (curPathReadable) {
        ...
        if (subFiles == null || subFiles.isEmpty()) { // 空文件夹没什么好导出的
          ...
          initSafExportDialog(
              subFiles.map { FileItemDto.genFileItemDtoByFile(it, activityContext) }
          )
        }
      } else {
        Msg.requireShow(activityContext.getString(R.string.err_read_path_failed))
  ...
  // {
  if (
      PageRequest.DataRequest.isDataRequest(filesPageRequestFromParent.value, PageRequest.goToIndex)
  ) {
  ...
  // disabled reason: maybe cause unexpected reload, if search mode on, may take long time to do a
  // non-senese searching again
  ...
  LaunchedEffect(needRefreshFilesPage.value) {
    ...
    try {
      ...
      if (requestType == StateRequestType.goToParentAndScrollToItem && requestData != null) {
        File(requestData).canonicalFile.parent?.let { updateCurrentPath(it) }
      }
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.loading)) {
        try {
          ...
          if (requestType == StateRequestType.goToParentAndScrollToItem && requestData != null) {
            ...
            // scroll to item
            val targetIndex =
                currentPathFileList.value.toList().indexOfFirst { it.fullPath == requestData }
            if (targetIndex >= 0) {
...
private suspend fun doInit(
  ...
  updateCurrentPath(
      if (currentPath().isBlank()) {
        ...
        // make path canonical first, elst dir/ will return once dir, that make must press 2 times
        // back for back to parent dir
  ...
  if (
      currentDir.canRead() && currentDir.isFile
  ) { // 注：若文件不存在 isDirectory和isFile 都为假，所以这个判断并不能确定目标路径就一定是个存在的文件
  ...
  // !currentDir.canonicalPath.startsWith(repoBaseDirPath)) {  //如果当前目录不存在，将路径设置为仓库根目录

  // because now(2024-09-23) support external path, so doesn't check startsWith repoBaseDirPath
  // anymore
  //        if(currentDir.canRead() && (!currentDir.exists() || !currentDir.isDirectory)) {
  // //如果当前目录不存在，将路径设置为仓库根目录
  ...
  if (currentDir.canRead() && !currentDir.isDirectory) { // 如果当前目录不存在，将路径设置为仓库根目录
    ...
    // 一边自己跳转到主页，一边提示无效path，产生一种主页是无效path的错觉，另人迷惑，故废弃
  ...
  if (lastOpenedPathFromSettings != currentPath()) {
    ...
    SettingsUtil
        .update { // 这里并不是把页面的settingsSnapshot状态变量完全写入到配置文件，而是获取一份当下最新设置项的拷贝，然后修改我在这个代码块里修改的变量，再写入文件，所以，在这修改的设置项其实和页面的设置项可能会有出入，但我只需要currentPath关联的一个值而已，所以有差异也无所谓
          it.files.lastOpenedPath = currentPath()
        }
  }

  // 文件列表排序算法
  val (viewAndSortOnlyForThisFolder, viewAndSort) =
      getViewAndSortForPath(currentPath(), settingsSnapshot.value)
  ...
  val comparator = { o1: FileItemDto, o2: FileItemDto -> // 不能让比较器返回0，不然就等于“去重”了，就会少文件
    ...
    var compareResult =
        if (sortMethod == SortMethod.NAME.code) {
          sortByName()
  ...
  if (curFileFromCurPathAlreadySelected && curFileFromCurPathFileDto != null) {
    ...
    // 下次渲染时请求滚动页面到对应条目
    filesPageRequestFromParent.value =
        PageRequest.DataRequest.build(PageRequest.goToIndex, "" + indexForScrollTo)
  }
  ...
  // 值等于0，需要初始化
  //    if(currentPathBreadCrumbList.intValue==0) {
  // //刚创建页面的时候，面包屑列表为空，需要初始化一下，后续由点击目录的onClick函数维护面包屑状态

  //        val willUpdateList = if(currentPathBreadCrumbList.intValue==1)
  // currentPathBreadCrumbList1 else currentPathBreadCrumbList2
  // //初始化时更新列表1，列表2由后续点击面包屑后的onClick函数更新
  ...
  //        if(lastUsedPath.isBlank() || curDirPath==FsUtils.rootPath ||
  // currentPathBreadCrumbList.value.isEmpty() || lastUsedPath.startsWith(curDirPath).not()) {

  val separator = Cons.slashChar

  // if breadCrumblist empty or last item full path not starts with current path, recreate the
  // breadcrumblist
  ...
  if (breadCrumbPathNotCoverdCurPath(curBreadCrumbList, curDirPath, separator)) {
    ...
    if (
        !(splitPath.size == 1 && splitPath[0].isEmpty())
    ) { // 如果当前路径非root，则添加面包屑，否则不需要添加，因为上面已经添加了rootpath，这里再添加只会添加个空元素，无意义
      var lastPathName =
          StringBuilder(
              40
      ...
      for (pathName in splitPath) { // 更新面包屑
        ...
        // 调用genFileItemDtoByFile(dto.toFile())即可
  ...
  // since require manage storage permission, no more need this, users can simple copy file at Files
  // page between external and internal storage
  // 检查是否请求导入文件
  ...
  // set err if has
  //        if(!File(currentPath.value).canRead()) {  // can't read dir, usually no permission for
  // dir or dir doesn't exist
  setErr(
      if (currentDir.canRead() && currentDir.isDirectory) ""
      else activityContext.getString(R.string.err_read_path_failed)
  )
}
...
private fun getBackHandler(
  ...
  val backHandlerOnBack: () -> Unit = {
    if (isFileChooser.not() && isFileSelectionMode.value) {
      ...
      resetSearchVars()
      //        }else if
      // (currentPath.value.startsWith(FsUtils.getExternalStorageRootPathNoEndsWithSeparator()+"/"))
      // { //如果在文件管理器页面且不在仓库根目录
    } else if (
        ceilingPaths.contains(currentPath()).not()
    ) { // 如果在文件管理器页面且未抵达任一天花板目录，则打开上级目录，否则显示“再按返回则退出”的提示
      // 返回上级目录
      goToPath(
          currentPath()
              .let { it.substring(0, it.lastIndexOf(Cons.slashChar).coerceAtLeast(0)) }
              .ifEmpty { FsUtils.rootPath }
      )
...
private fun breadCrumbPathNotCoverdCurPath(
    curBreadCrumbList: List<FileItemDto>,
    curDirPath: String,
    separator: Char,
): Boolean {
  return if (curBreadCrumbList.isEmpty()) {
    ...
    val breadCrumbPathForCompare =
        if (breadCrumbLastItemPath.endsWith(separator)) breadCrumbLastItemPath
        else "$breadCrumbLastItemPath$separator"
    val curDirPathForCompare =
        if (curDirPath.endsWith(separator)) curDirPath else "$curDirPath$separator"
