...
class RepoRepositoryImpl(private val dao: RepoDao) : RepoRepository {
  @Deprecated("不用这个") override fun getAllStream(): Flow<List<RepoEntity?>> = dao.getAllStream()

  @Deprecated("不用这个") override fun getStream(id: String): Flow<RepoEntity?> = dao.getStream(id)
  ...
  override suspend fun insert(item: RepoEntity) {
    ...
    if (isRepoNameExist(item.repoName)) {
      MyLog.w(
          TAG,
          "#$funName: warn: item's repoName '${item.repoName}' already exists! operation abort...",
      )
  ...
  private fun addTimeStampIfErrMsgIsNotBlank(errMsg: String): String {
    ...
    return if (errMsg.isBlank() || errMsg.endsWith(timestampSuffix)) errMsg
    else "$errMsg (${getNowInSecFormatted()}$timestampSuffix"
  }

  override suspend fun delete(
      item: RepoEntity,
      requireDelFilesOnDisk: Boolean,
    ...
    if (requireTransaction) {
      AppModel.dbContainer.db.withTransaction {
        act()
        //            throw
        // RuntimeException("测试抛异常是否会“回滚”以及“不执行异常后位于本代码块外的代码”，结论：1会回滚，2不会执行异常后的代码无论是在本代码块内还是外。两者都符合直觉，很好。")
      }
    } else {
      act()
    }
    ...
    if (requireDelFilesOnDisk) {
      ...
      Libgit2Helper.removeRepoFiles(
          repoFullPath,
          createEmptyWorkDirAfterRemove = item.parentRepoId.isNotBlank(),
      )
    }
  ...
  override suspend fun update(item: RepoEntity, requeryAfterUpdate: Boolean) {
    ...
    if (isRepoNameAlreadyUsedByOtherItem(item.repoName, item.id)) {
      MyLog.w(
          TAG,
          "#$funName: warn: item's repoName '${item.repoName}' already used by other item! operation abort...",
      )
      throw RuntimeException("#$funName err: repoName already exists")
    }
  ...
  override suspend fun getByNameOrId(
      repoNameOrId: String,
  ...
  override suspend fun getByFullSavePath(
      fullSavePath: String,
      onlyReturnReadyRepo: Boolean,
      requireSyncRepoInfoWithGit: Boolean,
  ): RepoEntity? {
  ...
  override suspend fun getAll(updateRepoInfo: Boolean): List<RepoEntity> {
    ...
    if (updateRepoInfo) {
      ...
      list.forEachBetter { Libgit2Helper.updateRepoInfo(it, settings = settings) }
    }
  ...
  override suspend fun cloneDoneUpdateRepoAndCreateRemote(item: RepoEntity) {
    ...
    db.withTransaction {
      ...
      // 但正常来说应该是不需要删的
      remoteRepository.deleteByRepoId(
          item.id
      ) // 这个其实正常来说不需要调用，但也有可能发生并发问题，导致错误调用这个代码块两次，那样remote表的记录会有错误，所以删一下保险，正常来说刚克隆完的仓库本来就是要有且只有一个remote origin的
    ...
    if (
      ...
      try {
        Repository.open(dirFile.canonicalPath).use { repo ->
          ...
          val repoWorkdirPath = Libgit2Helper.getRepoWorkdirNoEndsWithSlash(repo)
          if (
              repos.indexOfFirst { it.fullSavePath == repoWorkdirPath } != -1
          ) { // repo already exists
            existed = 1
  ...
  /**
   * @param addRepoToThisListIfSuccess when possible import more than 1 repo, should update all
   *   repos list after every imported for avoid add repo redundant, in that case, should pass all
   *   repos list to this param; but if add only 1 repo, needn't this param yet, pass null or ignore
   *   is ok
   */
  private suspend fun importSingleRepo(
    ...
    try {
      ...
      //                repoEntity.fullSavePath = repoDir.canonicalPath // bad, cause "repo/.git"
      // and "repo", both can open as repo, and is same repo, but difference path
      ...
      AppModel.dbContainer.db.withTransaction {
        ...
        // insert remotes
        remoteEntityList.forEachBetter { remote -> remoteDb.insert(remote) }
      }

      // in most case, this list is all repos list, if success, add repo into it, or dont add if is
      // not necessary
      try {
        addRepoToThisListIfSuccess?.add(repoEntity)
      } catch (addToListException: Exception) {
        // concurrent exception, maybe, but is ok, because repo is inserted, so ignore add to list
        // exception
        MyLog.e(
            TAG,
            "#$funName: err when add repo to list, err=${addToListException.localizedMessage}",
        )
      }
  ...
  override suspend fun isRepoNameAlreadyUsedByOtherItem(
      repoName: String,
      excludeId: String,
  ): Boolean {
