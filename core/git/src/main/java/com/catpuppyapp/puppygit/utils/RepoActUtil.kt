...
object RepoActUtil {
  ...
  private fun throwRepoBusy(prefix: String, repoName: String) {
    ...
    //        throwWithPrefix(prefix, RuntimeException("repo '$repoName' busy, plz try again
    // later"))
      ...
      doJobThenOffLoading {
        ...
        try {
          ...
          Libgit2Helper.doActWithRepoLock(
              repoFromDb,
              waitInMillSec = waitInMillSecIfApiBusy,
              onLockFailed = { throwRepoBusy(prefix, repoFromDb.repoName) },
          ) {
            syncSingle(
          ...
          notiSender
              ?.sendErrNotification
              ?.invoke(repoFromDb.repoName, errMsg, Cons.selectedItem_ChangeList, repoFromDb.id)

          MyLog.e(
              TAG,
              "#$funName: route:$routeName, repoName=${repoFromDb.repoName}, err=${e.stackTraceToString()}",
          )
        } finally {
  ...
  private suspend fun syncSingle(
      ...
      sendSuccessNotification:
          ((title: String?, msg: String?, startPage: Int?, startRepoId: String?) -> Unit)?,
      autoCommit: Boolean,
      routeName: String,
  ...
  private suspend fun pullSingle(
    ...
    try {
      ...
      Repository.open(repoFromDb.fullSavePath).use { gitRepo ->
        ...
        val (username, email) =
            if (gitUsernameFromUrl.isNotBlank() && gitEmailFromUrl.isNotBlank()) {
              Pair(gitUsernameFromUrl, gitEmailFromUrl)
            } else {
              val (gitUsernameFromConfig, gitEmailFromConfig) =
                  Libgit2Helper.getGitUsernameAndEmail(gitRepo)
        ...
        // merge没报错，基本算是成功了，不过可能是up to date啥也没干，也可能fast forward，也可能发生了merge，后面不会提示太详细，仅区分up to
        // date和pull successfully(fast-forwarded or merged)

        val successMsg =
            if (mergeRet.code == Ret.SuccessCode.upToDate) { // up to date, no fast-forward or merge
              // 显示 already up to date之类的，btw 如果显示already up to
              // date，代表没发生merge，文件应该没修改，不需要重新加载(20250216
              // markor不会在文件在外部被修改后自动重载，如果文件变化，需要手动重载，不过obsidian会，所以若嫌麻烦可以用obsidian)
              "$prefix: Already up-to-date"
            } else { // fast-forwarded or merged
              "pull successfully"
            }

        sendSuccessNotification?.invoke(
            repoFromDb.repoName,
            successMsg,
    ...
    try {
      ...
      Repository.open(repoFromDb.fullSavePath).use { gitRepo ->
        ...
        if (autoCommit) {
          ...
          if (username == null || username.isBlank() || email == null || email.isBlank()) {
            ...
            if (Libgit2Helper.hasConflictItemInRepo(gitRepo)) {
              ...
              MyLog.d(
                  TAG,
                  "#$funName: api=$routeName, repoName=${repoFromDb.repoName}, err=$errMsg",
              )
              ...
              if (!Libgit2Helper.indexIsEmpty(gitRepo)) {
                ...
                if (ret.hasError()) {
                  MyLog.d(
                      TAG,
                      "#$funName: api=$routeName, repoName=${repoFromDb.repoName}, create commit err: ${ret.msg}, exception=${ret.exception?.stackTraceToString()}",
                  )
        ...
        if (!force && upstream.isPublished) {
          val (ahead, behind) =
              Libgit2Helper.getAheadBehind(
                  gitRepo,
                  Oid.of(upstream.localOid),
                  Oid.of(upstream.remoteOid),
              )
