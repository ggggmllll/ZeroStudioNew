...
class RemoteDto {
  ...
  /**
   * NOTE: the `credentialId` and `pushCredentialId` maybe is match by domain, in that case, the
   * `actuallyCredentialIdWhenCredentialIdIsMatchByDomain` and
   * `actuallyPushCredentialIdWhenCredentialIdIsMatchByDomain` will save the actually using id, and
   * the crenentialName/Pass/Type 's value is related about the actually credential/pushCredential
   * id if credentialId is match by domain, this will replace to the value of
   * `actuallyCredentialIdWhenCredentialIdIsMatchByDomain` related credential
   */
  ...
  @Ignore var actuallyCredentialIdWhenCredentialIdIsMatchByDomain: String = ""
  @Ignore var actuallyPushCredentialIdWhenCredentialIdIsMatchByDomain: String = ""

  // Ignore的作用是从数据库查dto的时候忽略此字段
  @Ignore var branchMode: Int = Cons.dbRemote_Fetch_BranchMode_All
  @Ignore var branchListForFetch: List<String> = listOf()

  /** when push url empty, will use fetch url, that case should set this value to true* */
  @Ignore var pushUrlTrackFetchUrl: Boolean = false
