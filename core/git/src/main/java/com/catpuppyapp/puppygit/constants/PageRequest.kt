...
object PageRequest {
  ...
  const val showSyntaxHighlightingSelectLanguageDialogForCurItem =
      "showSyntaxHighlightingSelectLanguageDialogForCurItem"
  ...
  const val backFromExternalAppAskReloadFile =
      "backFromExternalAppAskReloadFile" // 在内置编辑器请求打开外部文件，再返回，会发出此请求，询问用户是否想重新加载文件
  ...
  const val switchBetweenFirstLineAndLastEditLine =
      "switchBetweenFirstLineAndLastEditLine" // 实现双击时在返回顶部和返回上次编辑行之间切换
  const val switchBetweenTopAndLastPosition =
      "switchBetweenTopAndLastPosition" // 实现双击时在 返回顶部和上个可见行之间切换
  ...
  fun getRequestThenClearStateThenDoAct(
      state: MutableState<String>,
      act: (request: String) -> Unit,
  ) {
