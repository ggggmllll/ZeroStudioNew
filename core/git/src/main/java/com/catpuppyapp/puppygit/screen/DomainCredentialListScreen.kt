...
fun DomainCredentialListScreen(
  ...
  val list =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "list",
          initValue = listOf<DomainCredentialDto>(),
      )
  val filterList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
  ...
  val curCredential =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curCredential",
          initValue = DomainCredentialDto(),
      )
  ...
  fun initCreateOrEditDialog(
      isCreateParam: Boolean,
      curDomainNameParam: String,
      curDomainCredItemIdParam: String,
      curCredentialId: String,
      curSshCredentialId: String,
  ) {
    ...
    selectedCredentialIdx.intValue =
        indexOfHttp.coerceAtLeast(0) // if not found, set to 0, else use found index
    selectedSshCredentialIdx.intValue =
        indexOfSsh.coerceAtLeast(0) // if not found, set to 0, else use found index
        ...
        textCompose = {
          ScrollableColumn {
            ...
            CredentialSelector(
                ...
                selectedCredentialIdx,
                stringResource(R.string.http_s_credential),
            )
            CredentialSelector(
                credentialList.value,
                selectedSshCredentialIdx,
                stringResource(R.string.ssh_credential),
            )
          }
        },
      ...
      doJobThenOffLoading {
        try {
          ...
          if (isCreate.value) {
            ...
            val old =
                dcDb.getById(curDomainCredItemId.value)
                    ?: throw RuntimeException("invalid id for update")
              ...
              if (filterModeOn.value) {
                ...
                Column(
                  ...
                  if (remote.value.id.isNotEmpty()) { // 隐含 remoteId.isNotEmpty() 为 true
                    titleSecondaryString.value =
                        stringResource(id = R.string.link_mode) +
                            ": [" +
                            remote.value.remoteName +
                            ":${curRepo.value.repoName}]"
                    ScrollableRow {
                      Text(
                          text = titleSecondaryString.value,
                          maxLines = 1,
    ...
    PullToRefreshBox(
      ...
      if (showBottomSheet.value) {
        BottomSheet(showBottomSheet, sheetState, curCredential.value.domain) {
          ...
          // text=stringResource(R.string.unlink_all)){
        ...
        val list =
            filterTheList(
                ...
                match = { idx: Int, it: DomainCredentialDto ->
                  it.domain.contains(keyword, ignoreCase = true) ||
                      (it.credName?.contains(keyword, ignoreCase = true) == true)
                },
            )
  ...
  LaunchedEffect(needRefresh.value) {
    ...
    try {
      ...
      // loadingText=activityContext.getString(R.string.loading)) job@{
      doJobThenOffLoading(initLoadingOn, initLoadingOff) {
        ...
        val credentialListFromDb =
            credentialDb.getAll(includeNone = true, includeMatchByDomain = false)
