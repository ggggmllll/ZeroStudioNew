...
object SafUtil {
  ...
  fun uriUnderSafDir(uri: Uri): Pair<Boolean, String?> {
    ...
    // 方法1，代码简单，死板：
    //        return
    // uri.toString().startsWith("content://${FsUtils.getAuthorityOfUri()}/internal/PuppyGitRepos/PuppyGit-Data/saf/")
   ...
   * 例如 "content://com.catpuppyapp.puppygit.play.pro/internal/abc.txt"，对应
   * /storage/emulated/0/Android/com.catpuppyapp.puppygit.play.pro/files/PuppyGitRepos/abc.txt 其中
   * internal 对应的实际路径在 res/xml/file_paths.xml里配置，external的路径和上述路径的区别就是把internal以及其对应的实际路径替换下。
   */
  @Deprecated("")
  fun appCreatedUriToPath(uri: Uri): String? {
    ...
    return if (uriStr.startsWith(getAppInternalUriPrefix())) {
      AppModel.externalFilesDir.canonicalPath +
          "/" +
          uriStr.substring(getAppInternalUriPrefix().length)
    } else if (uriStr.startsWith(getAppExternalUriPrefix())) {
      FsUtils.getExternalStorageRootPathNoEndsWithSeparator() +
          "/" +
          uriStr.substring(getAppExternalUriPrefix().length)
    } else {
      null
    }
  }
  ...
  fun takePersistableRWPermission(contentResolver: ContentResolver, uri: Uri): Boolean {
    return try {
      ...
      MyLog.d(
          TAG,
          "#takePersistableRWPermission() try take RW permissions err: ${e.stackTraceToString()}",
      )
      false
    }
  }
  ...
  fun takePersistableReadOnlyPermission(contentResolver: ContentResolver, uri: Uri): Boolean {
    return try {
      ...
      MyLog.d(
          TAG,
          "#takePersistableReadOnlyPermission() try take ReadOnly permissions err: ${e.stackTraceToString()}",
      )
      false
    }
  }
  ...
  fun releasePersistableRWPermission(contentResolver: ContentResolver, uri: Uri): Boolean {
    return try {
      ...
      MyLog.d(
          TAG,
          "#releasePersistableRWPermission() try release RW permissions err: ${e.stackTraceToString()}",
      )
      false
    }
  }
  ...
  fun releasePersistableReadOnlyPermission(contentResolver: ContentResolver, uri: Uri): Boolean {
    return try {
      ...
      MyLog.d(
          TAG,
          "#releasePersistableReadOnlyPermission() try release ReadOnly permissions err: ${e.stackTraceToString()}",
      )
      false
    }
  }
