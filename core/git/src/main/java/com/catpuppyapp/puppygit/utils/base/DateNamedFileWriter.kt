...
open class DateNamedFileWriter(
    private val logTagOfSubClass:
        String, // 日志tag，因为这个类并不直接使用，往往作为工具类的基类，所以log tag用其子类的，这样能知道是哪个类发生了错误
    private val fileNameTag:
        String, // 文件名标签，可用来区分这文件是干嘛的，例如带"2025-02-02#Log.txt"，#后面就是标签，而Log表明这文件记录的是日志
    ...
    private val maxErrCount: Int =
        5, // 若写入出错，最多重试次数，每次重试都会重新initWriter，若重试超过次数还是写入失败，将终止协程，再次执行类的init函数才会重新创建协程
) {
  private val writeLock = Mutex()
  private val writeChannel =
      Channel<String>(capacity = channelBufferSize, onBufferOverflow = BufferOverflow.SUSPEND) {
        /* onUndeliveredElement, 未交付的元素会调用此方法，一般用来执行关流之类的善后操作，不过我这用不上 */
      }

  private val fileNameDateFormatter: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")
  protected val contentTimestampFormatter: DateTimeFormatter =
      DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss") // 日志的输出格式
      ...
      for (f in fileList) { // Objects.requireNonNull(param) ，param为null则抛异常
        ...
        try {
          val dateStrFromFileName =
              getDateFromFileName(f.name) // 返回值类似 2024-04-08，和文件名中包含的日期格式必须匹配，否则会解析失败
          val fileCreatedDateInDay =
              LocalDate.from(fileNameDateFormatter.parse(dateStrFromFileName)).toEpochDay()
          val diffInDay = todayInDay - fileCreatedDateInDay // 计算今天和文件名日期相差的天数

          // debug
          //                    System.out.println("diffInDay:::"+diffInDay+",
          // now:"+logfile.format(new Date())+" other:"+dateStrFromFileName);
          // debug
