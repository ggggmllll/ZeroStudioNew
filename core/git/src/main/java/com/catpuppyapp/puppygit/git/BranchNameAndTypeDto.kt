...
class BranchNameAndTypeDto {
  ...
  var symbolicTargetFullName: String =
      "" // 符号引用全名，例如 refs/remotes/origin/abc or refs/heads/abc, 若是符号引用，值为其名，否则为空
  ...
  fun isRemoteNameAmbiguous():
      Boolean { // 如果返回true，说明远程分支的remote存在歧义，无法成功解析，如果执行删除远程分支等操作，需要用户指定一个分支。另外，本地分支调用此方法无意义，会百分百返回true
    ...
    return if (alreadyUpToDate()) {
      ...
      if (searchable) {
        ...
        replaceStringResList(
            activityContext.getString(R.string.ahead_n_behind_m),
            listOf("" + ahead, "" + behind),
        )
      }
    }
  ...
  fun getBranchNameNoRemotePrefixOrEmptyStrIfAmbiguous(): String {
    ...
    return Libgit2Helper.removeRemoteBranchShortRefSpecPrefixByRemoteName(
        remotePrefixFromShortName + "/",
        shortName,
    ) ?: ""
  }
  ...
  fun isUpstreamValid(): Boolean {
    // 注：`upstream?.isPublished != true` 不可写成 `xxx?.xxx == false`，因为 ?.
    // 除了布尔值还隐含了null，所以如果写成等于false，值为null时就会返回假，那就和值为true时效果一样了
    // 必须 有上游 且 上游的remote不为空 且 上游的branchRefsHeadsRefSpec不为空 且 已发布（推送到远程仓库），上游才算有效
    //        return !(upstream == null || upstream?.remote.isNullOrBlank() ||
    // upstream?.branchRefsHeadsFullRefSpec.isNullOrBlank() || upstream?.isPublished != true)
  ...
  fun isUpstreamAlreadySet(): Boolean {
    return !(upstream == null ||
        upstream?.remote.isNullOrBlank() ||
        upstream?.branchRefsHeadsFullRefSpec.isNullOrBlank())
  }
