...
data class FileItemDto(
  ...
  var cachedShortLastModifiedTime: String? = null
    ...
    get() =
        field
            ?: getShortTimeIfPossible(lastModifiedTime).let {
              field = it
              it
            }
  ...
  companion object {
    ...
    fun updateFileItemDto(fdto: FileItemDto, file: File, activityContext: Context) {
      try {
        ...
        //            fdto.mime = MimeType.guessFromPath(if(fdto.isDir)
        // fdto.fullPath+MimeType.separator else fdto.fullPath)
        // //这个MimeType类依赖路径末尾的分隔符来判断类型，但默认我存的路径末尾都没分隔符，所以判断下，如果是目录，给它加上分隔符
        ...
        if (fdto.isDir) { // 目录，没有大小，暂不递归计算文件大小，先显示："[Folder]"
          fdto.sizeInHumanReadable =
              "[" +
                  activityContext.getString(R.string.folder) +
                  "]" // 文件夹大小不是递归显示内部文件总和，所以显示没意义，改成Folder，指示条目是个目录，让界面看着不那么空，就行了
        } else { // 读文件大小，但不读目录大小
          fdto.sizeInHumanReadable = getHumanReadableSizeStr(fdto.sizeInBytes)
        }

        val fileAttributes = getFileAttributes(file.canonicalPath)
        //            fdto.lastModifiedTimeInSec = file.lastModified()/1000  //File的lastModified()
        // 默认返回的是毫秒数，需要转换成秒数，不然后面的日期格式化会出问题
        fdto.lastModifiedTimeInSec = fileAttributes?.lastModifiedTime()?.to(TimeUnit.SECONDS) ?: 0
        //            println("fdto.lastModifiedTimeInSec ==
        // file.lastModified()/1000:"+(fdto.lastModifiedTimeInSec ==(file.lastModified()/1000) ))
        // // true
