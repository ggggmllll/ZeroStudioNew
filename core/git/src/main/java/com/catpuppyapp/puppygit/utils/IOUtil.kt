...
object IOUtil {
  ...
  /**
   * @param len 注意是len不是end index，换算公式为： len = endIndex - startIndex + 1 = endIndex - (startIndex -
   *   1)： endIndex = len + (startIndex-1) = len + startIndex - 1 例如： 要比较 索引0 到 索引8，则len应为 8-0+1=9；
  ...
  fun bytesAreEquals(left: ByteArray, right: ByteArray, startIndex: Int, len: Int): Boolean {
    ...
    //        val endIndex = startIndex+len-1  // endIndex = len + (startIndex-1) = len + startIndex
    // -1
