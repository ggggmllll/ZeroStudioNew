...
fun RemoteListScreen(
  ...
  val curObjInState =
      mutableCustomStateOf(keyTag = stateKeyTag, keyName = "curObjInState", initValue = RemoteDto())
  ...
  if (showCreateRemoteDialog.value) {
    CreateRemoteDialog(
        ...
        onErr = { e ->
          ...
          val errMsgPrefix =
              "$actionDesc err: remoteName='${remoteNameForCreate.value}', remoteUrl=${remoteUrlForCreate.value}, err="
        ...
        try {
          Repository.open(curRepo.value.fullSavePath).use { repo ->
            ...
            if (ret.success() || noExist) {
              ...
              remoteDb.delete(
                  RemoteEntity(id = remoteWillDel.remoteId)
              ) // delete其实是根据id删除的，所以只设id即可
            } else {
  ...
  if (showViewDialog.value) {
    CopyableDialog(
        ...
        onCancel = { showViewDialog.value = false },
    ) { // 复制到剪贴板
  ...
  if (showSetUrlDialog.value) {
    ConfirmDialog(
        ...
        title =
            if (isPushUrl.value) stringResource(id = R.string.set_push_url)
            else stringResource(id = R.string.set_url),
          ...
          ScrollableColumn {
            MySelectionContainer {
              Row {
                ...
                Text(
                    text = curObjInState.value.remoteName,
            ...
            TextField(
                ...
                label = {
                  Text(
                      if (isPushUrl.value) stringResource(id = R.string.push_url)
                      else stringResource(R.string.url)
                  )
                },
            )
            ...
            if (isPushUrl.value) {
              MySelectionContainer {
                Text(
                    text = stringResource(R.string.leave_it_empty_will_use_url),
      ...
      try {
        ...
        MyLog.e(
            TAG,
            "$setUrlPrefix in RemoteList err: remoteName=${remoteName}, newUrl=$newUrl, oldUrl=$oldUrl, err is:\n${e.stackTraceToString()}",
        )
        val err1 = e.localizedMessage ?: activityContext.getString(R.string.unknown_err)
        ...
        if (showSetUrlDialog.value) { // 弹窗开着，直接在弹窗显示错误信息
          ...
          // 记录错误到数据库
          val errWillSave = "$setUrlPrefix for remote '$remoteName' err: $err1"
          doJobThenOffLoading { createAndInsertError(repoId, errWillSave) }
  ...
  val doFetch: suspend (String?, RepoEntity) -> Boolean =
  ...
  // filterListState.value.firstVisibleItemIndex else lazyListState.firstVisibleItemIndex } }
  //    ScrollListener(
    ...
    if (showBottomSheet.value) {
      BottomSheet(showBottomSheet, sheetState, curObjInState.value.remoteName) {
        BottomSheetItem(
          ...
          // e.g. "fetching origin..."
          val fetchingRemoteLoadingText =
              replaceStringResList(
                  activityContext.getString(R.string.fetching_remotename),
                  listOf(curObjInState.value.remoteName),
              )
          // 执行fetch
        ...
        if (dev_EnableUnTestedFeature || shallowAndSingleBranchTestPassed) {
          ...
          BottomSheetItem(
              ...
              enabled = isPro,
              text =
                  if (isPro) stringResource(R.string.set_branch_mode)
                  else stringResource(R.string.set_branch_mode_pro_only),
          ) {
        ...
        BottomSheetItem(
            sheetState = sheetState,
            showBottomSheet = showBottomSheet,
            text = stringResource(R.string.set_url),
        ) {
          // 设置remote url
          // 设置回显上次的url
          val oldUrl = curObjInState.value.remoteUrl
          urlTextForSetUrlDialog.value = oldUrl // 用来存储用户输入的url的状态变量
          oldUrlTextForSetUrlDialog.value =
              oldUrl // 保存旧值，在这设置后就不会更新了，这个变量用来在保存前检查新旧url是否相同，若相同就不更新了
        ...
        if (UserUtil.isPro()) {
          BottomSheetItem(
              sheetState = sheetState,
              showBottomSheet = showBottomSheet,
              text = stringResource(R.string.set_push_url),
          ) {
            // 设置pushurl
            // 设置回显上次的url
            val oldUrl =
                if (curObjInState.value.pushUrlTrackFetchUrl) "" else curObjInState.value.pushUrl
            urlTextForSetUrlDialog.value = oldUrl // 用来存储用户输入的url的状态变量
            oldUrlTextForSetUrlDialog.value =
                oldUrl // 保存旧值，在这设置后就不会更新了，这个变量用来在保存前检查新旧url是否相同，若相同就不更新了
          ...
          BottomSheetItem(
              sheetState = sheetState,
              showBottomSheet = showBottomSheet,
              text = stringResource(R.string.link_credential),
          ) {
            // 跳转到凭据列表页面
            // curObjInState当初应命名为“LongPressedItem”之类的名字，现在这我经常分不清哪个状态变量存储长按条目
            navController.navigate(
                Cons.nav_CredentialManagerScreen + "/${curObjInState.value.remoteId}"
            )
          }
        }

        //              //20241130: 改成前往凭据关联页面绑定None实现解绑凭据了
        //                BottomSheetItem(sheetState=sheetState, showBottomSheet=showBottomSheet,
        // text=stringResource(R.string.unlink_credential)){
        //                    //解除关联凭据
        //                    showUnlinkCredentialDialog.value=true
        //                }

        // 添加remote和删除remote配套，都是pro
        // feature，不能只开放其中一个，不然会有问题，想像一下，你就一个remote，而你删了它，但没法创建，是不是很不合理？所以删除和添加应该配套，一个是pro
        // feature另一个也必须是
        if (proFeatureEnabled(createRemoteTestPassed)) {
          BottomSheetItem(
              sheetState = sheetState,
      ...
      if (isLoading.value || list.value.isEmpty()) {
        ...
        MyLazyColumn(
          ...
          RemoteItem(showBottomSheet, curObjInState, idx, it, lastClickedItemKey) { // onClick
            ...
            //                    sb.append(appContext.getString(R.string.push_url)+":
            // "+(it.pushUrl.ifEmpty { it.remoteUrl }))  // no more check pushUrl empty need after
            // 20241007
            // after 20241007, if need, pushUrl will replaced to fetch url when querying, so reached
            // here, directly show pushUrl is ok
            sb.append(activityContext.getString(R.string.push_url) + ": " + it.pushUrl)

            sb.appendLine()
            sb.appendLine()
            sb.append(
                activityContext.getString(R.string.fetch_credential) +
                    ": " +
                    it.getLinkedFetchCredentialName()
            )
            sb.appendLine()
            sb.appendLine()
            sb.append(
                activityContext.getString(R.string.push_credential) +
                    ": " +
                    it.getLinkedPushCredentialName()
            )
            sb.appendLine()
            sb.appendLine()
            sb.append(
                activityContext.getString(R.string.branch_mode) +
                    ": " +
                    (if (it.branchMode == Cons.dbRemote_Fetch_BranchMode_All)
                        activityContext.getString(R.string.all)
                    else activityContext.getString(R.string.custom))
            )
            if (it.branchMode != Cons.dbRemote_Fetch_BranchMode_All) {
              sb.appendLine()
              sb.appendLine()
              sb.append(
                  (if (it.branchListForFetch.size > 1) activityContext.getString(R.string.branches)
                  else activityContext.getString(R.string.branch)) + ": ${it.branchListForFetch}"
              )
            }
