...
fun CredentialRemoteListScreen(
  ...
  val curItemInPage =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curItemInPage",
          initValue = CredentialEntity(),
      )
  val list =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "list",
          initValue = listOf<RemoteDtoForCredential>(),
      )
  val filterList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
  ...
  val needOverrideLinkItem =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "needOverrideLinkItem",
          initValue = RemoteDtoForCredential(),
      )
  ...
  if (showUnLinkAllDialog.value) {
    ConfirmDialog(
        title = stringResource(id = R.string.unlink_all),
  ...
  if (showOverrideLinkDialog.value) {
    ConfirmDialog(
        title = stringResource(id = R.string.override_link),
  ...
  if (showLinkOrUnLinkDialog.value) {
    LinkOrUnLinkCredentialAndRemoteDialog(
        ...
        onErrCallback = { e ->
          ...
          val errMsgPrefix =
              "${linkOrUnlinkDialogTitle.value} err: remote='${curItem.value.remoteName}', credential=${curItemInPage.value.name}, err="
          ...
          MyLog.e(
              TAG,
              "#LinkOrUnLinkCredentialAndRemoteDialog err: $errMsgPrefix${e.stackTraceToString()}",
          )
        },
  ...
  val initLoadingOff = { initLoading.value = false }
            ...
            title = {
              if (filterModeOn.value) {
                ...
                Column(
                  ...
                  ScrollableRow {
                    titleString.value =
                        if (isShowLink) stringResource(R.string.linked_remotes)
                        else stringResource(R.string.unlinked_remotes)
                    Text(text = titleString.value, maxLines = 1, overflow = TextOverflow.Ellipsis)
                  }
                  ScrollableRow {
            ...
            actions = {
              if (!filterModeOn.value) {
                Row {
                  ...
                  val linkOrUnLinkAll =
                      if (isShowLink) stringResource(R.string.unlink_all)
                      else stringResource(R.string.link_all)
                  LongPressAbleIconBtn(
                  ...
                  if (isShowLink) {
                    LongPressAbleIconBtn(
                        ...
                        //                                    iconColor =
                        // UIHelper.getIconEnableColorOrNull(!isShowLink)
                    ) {
      ...
      if (list.value.isEmpty()) {
        if (initLoading.value) {
          ...
          PageCenterIconButton(
              ...
              elseContent = { Text(stringResource(R.string.item_list_is_empty)) },
          )
        }
      } else {
        ...
        MyLazyColumn(
          ...
          RemoteItemForCredential(
              ...
              actText =
                  if (isShowLink) stringResource(R.string.unlink)
                  else stringResource(R.string.link),

              // x 废弃，容易令人困惑) 如果是 None页面 且 是关联模式 且
              // 条目fetch和push凭据id都为空，则不需要显示unlink，因为在无凭据条目列表将条目unlink到无凭据没有意义，执行了也没效果
              //                actAction = if(isNonePage && isShowLink &&
              // it.credentialId.isNullOrEmpty() && it.pushCredentialId.isNullOrEmpty()) null else
              // ({
                  ...
                  if (isNonePage && isShowLink) null
                      ...
                      ({
                        ...
                        linkOrUnlinkDialogTitle.value =
                            if (requireDoLink.value) activityContext.getString(R.string.link)
                            else
                                activityContext.getString(
                                    R.string.unlink
                                ) // (不建议，不方便记Err)若空字符串，将会自动根据requireDoLink的值决定使用link还是unlink作为title
                        showLinkOrUnLinkDialog.value = true

                        //                if(isShowLink) {  //如果是显示已关联条目的页面，点击取关直接执行
                        //                    doUnLink(it.remoteId)
                        //                }else{  //如果是显示未关联条目的页面，检查是否已关联其他凭据，如果没有，直接关联，如果关联了，询问是否覆盖
                        //                    if(it.credentialName==null ||
                        // it.credentialName!!.isBlank()) {  //没关联其他凭据
                        //                        doLink(it.remoteId)
  ...
  LaunchedEffect(needRefresh.value) {
    try {
      doJobThenOffLoading(
        ...
        curItemInPage.value =
            credentialDb.getById(credentialId, includeNone = true, includeMatchByDomain = true)
                ?: CredentialEntity(id = "")
        val listFromDb =
            if (isShowLink) {
