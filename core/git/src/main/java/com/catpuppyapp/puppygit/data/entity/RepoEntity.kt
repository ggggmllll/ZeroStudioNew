...
data class RepoEntity(
    @PrimaryKey var id: String = getShortUUID(),

    //    var testMigra:String="",

    // repo push or pull etc time
    /**
     * usually repo name is not blank, but maybe is blank, no enforce check, e.g. if you import a
     * empty name folder as repo, then will create a RepoEntity with blank name
     */
    ...
    var lastUpdateTime: Long =
        getSecFromTime(), // 本来是最后更新仓库workStatus状态的时间，后来改成最后检查更新的时间了，只有fetchpullpush这类和服务器通信更新或检查更新的操作才会更新这个时间，所以实际上这个字段成了最后与服务器通信的时间
    ...
    var hasUncheckedErr: Int =
        Cons
            .dbCommonFalse, // 这个变量其实没什么意义，直接检查latestUncheckedErrMsg是否为空或空白不就知道是否有错误了？不为空就有错，为空就没错，不就行了？为什么还要加这个变量？
    var latestUncheckedErrMsg: String = "",

    // 克隆时关联的credentialId，克隆完成后就没用了，到时候会根据不同的remote使用不同的credentialId，这个id在克隆结束后被会设置为
    // 默认remote(origin) 的credentialId
    var credentialIdForClone: String = "",
    var cloneUrl: String = "",

    // 仓库默认状态为 active，可通过仓库设置页面(不是公用仓库选项，是仓库私有的选项)设置为
    // inactive，如果是inactive，将不会自动检查仓库状态，在changelist也不能查看仓库状态(在changelist切换仓库，没这个仓库)
    @Deprecated("实际上没用到这个字段") var isActive: Int = Cons.dbCommonTrue,
    ...
    @Deprecated("已废弃") var storageDirId: String = StorageDirCons.DefaultStorageDir.puppyGitRepos.id,
    @Embedded var baseFields: BaseFields = BaseFields(),

    // 临时状态，仓库在repo list页面执行操作时会设置此状态，例如：fetching,
    // pushing等，不往数据库存，且每次从db查数据后会更新此值，在卡片显示时需要用到此变量，另外，若此变量不为空，代表仓库正在执行某些操作，隐含仓库有效（没删除，状态正常）
    //    @IgnoredOnParcel
    //    @Ignore
  ...
  // 忽略的字段应放构造器外面
  @Ignore var gitRepoState: Repository.StateT? = null

  @Ignore var parentRepoName: String = ""
   ...
   */
  @Ignore var parentRepoValid: Boolean = false

  @Ignore var otherText: String? = null
   ...
   */
  @Ignore var pendingTask: RepoPendingTask = RepoPendingTask.NONE
  ...
  var lastCommitHashShort: String? = null // 最后提交hash，短
    ...
    get() {
      ...
      return Libgit2Helper.getShortOidStrByFull(lastCommitHash).let {
        field = it
        it
      }
    }
  ...
  @Ignore private var lastUpdateTimeFormattedCached: String? = null

  @Ignore private var cached_OneLineLastestUnCheckedErrMsg: String? = null

  fun getCachedOneLineLatestUnCheckedErrMsg(): String =
      (cached_OneLineLastestUnCheckedErrMsg
          ?: Libgit2Helper.zipOneLineMsg(latestUncheckedErrMsg).let {
            cached_OneLineLastestUnCheckedErrMsg = it
            it
          })
  ...
  fun updateCommitDateTime(settings: AppSettings) {
    try {
      Repository.open(fullSavePath).use { repo -> updateCommitDateTimeWithRepo(repo, settings) }
    } catch (e: Exception) {
      MyLog.e(
          TAG,
          "#updateCommitDateTime: resolve commit hash failed! hash=$lastCommitHash, err=${e.localizedMessage}",
      )
    }
  }
  ...
  fun updateCommitDateTimeWithRepo(repo: Repository, settings: AppSettings) {
    lastCommitDateTime =
        try {
          getShortTimeIfPossible(
              Libgit2Helper.getSingleCommitSimple(
                      repo,
                      repoId = id,
                      commitOidStr = lastCommitHash,
                      settings,
                  )
                  .dateTime
          )
          ...
          MyLog.e(
              TAG,
              "#updateCommitDateTimeWithRepo: resolve commit hash failed! hash=$lastCommitHash, err=${e.localizedMessage}",
          )
          ""
        }
  }
  ...
  fun cachedLastUpdateTime(): String {
    return lastUpdateTimeFormattedCached
        ?: getShortTimeIfPossible(lastUpdateTime).let {
          lastUpdateTimeFormattedCached = it
          it
        }
  }
  ...
  @Ignore private var cached_appRelatedPath: String? = null

  fun cachedAppRelatedPath() =
      cached_appRelatedPath
          ?: FsUtils.getPathWithInternalOrExternalPrefix(fullPath = fullSavePath).let {
            cached_appRelatedPath = it
            it
          }

  @Ignore var latestCommitMsg = ""
  @Ignore private var cached_oneLineLatestCommitMsg: String? = null

  fun getOrUpdateCachedOneLineLatestCommitMsg() =
      cached_oneLineLatestCommitMsg
          ?: Libgit2Helper.zipOneLineMsg(latestCommitMsg).let {
            cached_oneLineLatestCommitMsg = it
            it
          }
}
