...
data class AppSettings(
    /**
     * 警告： 1 不要删除字段 2 不要修改字段名称或类型 3 不要修改字段默认值
     *
     * 仅建议：添加字段
     *
    ...
    @Deprecated("改成存到prefman了") var firstUse: Boolean = true,
    ...
    val storageDir: StorageDir =
        StorageDir(), // 将来会出一个StorageDir页面，用来管理存储仓库的目录，有内部，有外部，还有ProgramData(puppygitdata，这个类型不一定展示，感觉没必要展示给用户，但实际在代码里存在)
    // var settings ? Settings页面的设置？叫这个名字好像不太好，再考虑下吧

    // storagePaths for clone
    @Deprecated("instead by StoragePaths") val storagePaths: MutableList<String> = mutableListOf(),
    // last selected of storage paths
    @Deprecated("instead by StoragePaths") var storagePathLastSelected: String = "",
    ...
    @Deprecated("instead by `PrefMan.Key` same name field") var theme: Int = 0,
    @Deprecated("instead by `PrefMan.Key` same name field") var logLevel: Char = 'w',
    @Deprecated("instead by `PrefMan.Key` same name field") var logKeepDays: Int = 3,
    ...
    /** 提交历史页面是否使用RTL布局，因为提交树默认是镜像的，但我又实在不想单独写了提交树页面， 所以添加一个按钮，点击可切换左右布局，RTL开启时提交树就可变成非镜像的了 */
