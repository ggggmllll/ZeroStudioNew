...
class PassEncryptHelper {
  companion object {
    ...
    // key = ver, value = 加密解密器
    val encryptorMap: Map<Int, Encryptor> =
        mapOf(
    ...
    private fun chooseMasterPasswordOrDefaultPass(
        encryptorVersion: Int,
      ...
      return if (masterPassword.isEmpty() || (encryptorVersion >= 1 && encryptorVersion <= 4)) {
        // 如果用户没设置主密码，或者用户设置了，但传的主密码有误，则尝试使用默认密码。 ps
        // 如果用户设置了主密码但调用此函数时传的空值，那么会解密失败，然后报错。除非用户设置的密码刚好和默认密码一致....不过一般不会那么巧除非刻意为之，所以不用处理
    ...
    fun encryptWithSpecifyEncryptorVersion(
        encryptorVersion: Int,
        raw: String,
        masterPassword: String,
    ): String {
      return encryptorMap[encryptorVersion]!!.encrypt(
          raw,
          chooseMasterPasswordOrDefaultPass(encryptorVersion, masterPassword),
      )
    }

    fun decryptWithSpecifyEncryptorVersion(
        encryptorVersion: Int,
        encryptedStr: String,
        masterPassword: String,
    ): String {
      return encryptorMap[encryptorVersion]!!.decrypt(
          encryptedStr,
          chooseMasterPasswordOrDefaultPass(encryptorVersion, masterPassword),
      )
    }
