...
object ChangeListFunctions {
  ...
  suspend fun doCommit(
    ...
    //        commitBtnTextForCommitDialog.value = appContext.getString(if(requireDoSync)
    // R.string.sync else R.string.commit)
    ...
    //        requireShowToast("test context
    // getsTring:"+appContext.getString(R.string.n_files_staged))
    ...
    Repository.open(curRepoFromParentPage.fullSavePath).use { repo ->
      ...
      if (readyCreateCommit.hasError()) {
        // 20240819
        // 支持index为空时创建提交，因为目前实现了amend而amend可仅修改之前的提交且不提交新内容，所以index非空检测就不能强制了，显示个提示就够了，但仍允许提交
      ...
      // do commit
      val ret =
          if (repoState.intValue == Repository.StateT.REBASE_MERGE.bit) { // 执行rebase continue
            loadingText.value =
                activityContext.getString(R.string.rebase_continue) + Cons.oneChar3dots
            ...
            loadingText.value =
                activityContext.getString(R.string.cherrypick_continue) + Cons.oneChar3dots
            ...
            Libgit2Helper.createCommit(
                ...
                //                    indexItemList = if(fromTo == Cons.gitDiffFromHeadToIndex)
                // itemList else null,
                // 20250218:
                // 因为现在在diff页面或者filehistory有可能影响到index的itemlist，所以就算itemlist来自index页面我也无法确保那个列表就一定靠谱，所以这里传null，重查下
                indexItemList = null,
                amend = amendCommit,
      ...
      if (ret.hasError()) { // 创建commit失败
        ...
        Libgit2Helper.cleanRepoState(
            repo
        ) // 清理仓库状态，例如存在冲突的时候如果创建完不清理状态，会一直处于merge state，就是pc git 显示 merging的情况
    ...
    // 设置好email后，弹窗，提示将创建提交，是否确定，有个勾选框“我想填写提交信息”，勾选显示一个输入框可填写提交信息，不勾选将自动生成提交信息“更新了n个文件，列举前3个，最后加句
    // commit msg generated by PuppyGit”
    // 创建提交
  ...
  suspend fun doFetch(
    ...
    // x 废弃，逻辑已经改了) 执行isReadyDoSync检查之前要先do
    // fetch，想象一下，如果远程创建了一个分支，正好和本地的关联，但如果我没先fetch，那我检查时就get不到那个远程分支是否存在，然后就会先执行push，但可能远程仓库已经领先本地了，所以push也可能失败，但如果先fetch，就不会有这种问题了
    Repository.open(curRepoFromParentPage.fullSavePath).use { repo ->
      ...
      try {
        ...
        if (remoteName == null || remoteName.isBlank()) {
          ...
          if (remoteName == null || remoteName.isBlank()) { // fetch不需合并，只需remote有效即可，所以只检查remote
            requireShowToast(
                activityContext.getString(R.string.err_upstream_invalid_plz_try_sync_first)
            )
        ...
        showErrAndSaveLog(
            TAG,
            "#doFetch() err: " + e.stackTraceToString(),
  ...
  suspend fun doMerge(
    ...
    try {
      ...
      showErrAndSaveLog(
          logTag = TAG,
          logMsg =
              "#doMerge(trueMergeFalseRebase=$trueMergeFalseRebase) err: " + e.stackTraceToString(),
  ...
  suspend fun doSync(
    ...
    Repository.open(curRepoFromParentPage.fullSavePath).use { repo ->
      ...
      if (!hasUpstream) { // 不存在上游，弹窗设置一下
        ...
        // 显示弹窗
        initSetUpstreamDialog(
            Libgit2Helper.getRemoteList(repo),
            curBranchShortName,
            curBranchFullName,
        ...
        try {
          ...
          if (pushSuccess) {
            requireShowToast(
                activityContext.getString(
                    if (trueMergeFalseRebase) R.string.sync_merge_success
                    else R.string.sync_rebase_success
                )
            )
          } else {
            requireShowToast(
                activityContext.getString(
                    if (trueMergeFalseRebase) R.string.sync_merge_failed
                    else R.string.sync_rebase_failed
                )
            )
          }
          ...
          showErrAndSaveLog(
              TAG,
              "#doSync() err: " + e.stackTraceToString(),
              "sync err: " + e.localizedMessage,
              requireShowToast,
              curRepoFromParentPage.id,
          )
        }
  ...
  fun doStage(
    ...
    val msg = replaceStringResList(nFilesStagedStrRes, listOf(actuallyStageList.size.toString()))
  ...
  fun changeListDoRefresh(
      stateForRefresh: MutableState<String>,
      whichRepoRequestRefresh: RepoEntity,
  ) {
    changeStateTriggerRefreshPage(
        stateForRefresh,
        requestType = StateRequestType.withRepoId,
        data = whichRepoRequestRefresh.id,
    )
  }
    ...
    try {
      ...
      if (!fetchSuccess) {
        ...
        if (!mergeSuccess) {
          requireShowToast(
              activityContext.getString(
                  if (trueMergeFalseRebase) R.string.merge_failed else R.string.rebase_failed
              )
          )
        } else {
          requireShowToast(
              activityContext.getString(
                  if (trueMergeFalseRebase) R.string.pull_merge_success
                  else R.string.pull_rebase_success
              )
          )
        }
      }
      ...
      showErrAndSaveLog(
          logTag = TAG,
          logMsg =
              "doPull(trueMergeFalseRebase=$trueMergeFalseRebase) err: " + e.stackTraceToString(),
          showMsg =
              activityContext.getString(
                  if (trueMergeFalseRebase) R.string.pull_merge_failed
                  else R.string.pull_rebase_failed
              ) + ": " + e.localizedMessage,
          showMsgMethod = requireShowToast,
          repoId = curRepo.id,
      )
  ...
  suspend fun doAccept(
    ...
    loadingText.value =
        (if (acceptTheirs) activityContext.getString(R.string.accept_theirs)
        else activityContext.getString(R.string.accept_ours)) + Cons.oneChar3dots
    ...
    Repository.open(repoFullPath).use { repo ->
      val acceptRet =
          if (repoState.intValue == Repository.StateT.MERGE.bit) {
      ...
      if (acceptRet.hasError()) {
        ...
        val stageSuccess =
            if (existConflictItems.isEmpty()) { // 列表为空，无需stage，直接返回true即可
              true
            } else { // 列表有条目，执行stage
  ...
  fun createPath(
    ...
    Repository.open(curRepo.fullSavePath).use { repo ->
      ...
      val (reverse: Boolean, tree1: Tree?, tree2: Tree?) =
          if (fromTo == Cons.gitDiffFromIndexToWorktree || fromTo == Cons.gitDiffFromHeadToIndex) {
            Triple(false, null, null)
          } else if (
              Libgit2Helper.CommitUtil.isLocalCommitHash(leftCommit) ||
                  Libgit2Helper.CommitUtil.isLocalCommitHash(rightCommit)
          ) {
            treeToWorkTree = true
            ...
            val tree1 =
                Libgit2Helper.resolveTree(repo, leftCommit)
                    ?: throw RuntimeException("resolve tree1 failed, 12978960")
            val tree2 =
                Libgit2Helper.resolveTree(repo, rightCommit)
                    ?: throw RuntimeException("resolve tree2 failed, 17819020")
            Triple(reverse, tree1, tree2)
          }
