...
fun BranchItem(
  ...
  Column(
    ...
    Row(
      ...
      ScrollableRow {
        Text(
            ...
            color =
                if (thisObj.isCurrent) MyStyleKt.DropDownMenu.selectedItemColor()
    ...
    Row(
      ...
      InLineIcon(icon = Icons.Filled.Commit, tooltipText = stringResource(R.string.last_commit))
    ...
    //                Text(text = if (thisObj.isCurrent) stringResource(R.string.yes) else
    ...
    Row(
      ...
      ScrollableRow {
        Text(
            text = thisObj.getTypeString(activityContext, false),
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = defaultFontWeight,
        )
    ...
    // 显示上游信息
    //        if(thisObj.type == Branch.BranchType.LOCAL && thisObj.upstream!=null) {
    // //其实只要是local就一定有upstream，不会是null，顶多里面没值
    ...
    //                //上游已发布：是|否
    //                Text(
    //                    text = if (thisObj.upstream!!.isPublished) stringResource(R.string.yes)
    ...
    if (thisObj.type == Branch.BranchType.LOCAL) {
      if (thisObj.isUpstreamAlreadySet()) {
        Row(
          ...
          InLineIcon(icon = Icons.Filled.Cloud, tooltipText = stringResource(R.string.upstream))
      ...
      if (thisObj.isUpstreamValid()) {
        Row(
          ...
          InLineIcon(icon = Icons.Filled.Info, tooltipText = stringResource(R.string.status))
          ...
          ScrollableRow {
            Text(
                ...
                fontWeight = defaultFontWeight,
                color =
                    if (thisObj.alreadyUpToDate()) MyStyleKt.TextColor.getHighlighting()
                    else Color.Unspecified,
            )
          }
        }
    ...
    if (thisObj.isSymbolic) {
      Row(
        ...
        InLineIcon(icon = Icons.Filled.Link, tooltipText = stringResource(R.string.symbolic_target))

        //                Text(text = stringResource(R.string.symbolic_target) +": ")

        ScrollableRow {
          Text(
              text = thisObj.symbolicTargetShortName,
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
    ...
    Row(
      ...
      ScrollableRow {
        Text(
            text = thisObj.getOther(activityContext, false),
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = defaultFontWeight,
        )
