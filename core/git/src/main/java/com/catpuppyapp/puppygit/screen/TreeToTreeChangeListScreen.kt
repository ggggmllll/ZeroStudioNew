...
fun TreeToTreeChangeListScreen(
  ...
  val commit1OidStrState =
      rememberSaveable(commit1OidStrCacheKey) {
        mutableStateOf(
            (NaviCache.getByType<String>(commit1OidStrCacheKey) ?: "").ifBlank {
  ...
  val commit2OidStr =
      rememberSaveable(commit2OidStrCacheKey) {
        (NaviCache.getByType<String>(commit2OidStrCacheKey) ?: "").ifBlank {
          Cons.git_AllZeroOidStr
        }
      }

  val commitForQueryParents =
      rememberSaveable(commitForQueryParentsCacheKey) {
        NaviCache.getByType<String>(commitForQueryParentsCacheKey) ?: ""
      }

  val commitParentList =
  ...
  val titleDesc = rememberSaveable {
    mutableStateOf(NaviCache.getByType<String>(titleCacheKey) ?: "")
  }
  ...
  val changeListRefreshRequiredByParentPage = rememberSaveable {
    mutableStateOf("TreeToTree_ChangeList_refresh_init_value_63wk")
  }
  ...
  val repoState = rememberSaveable {
    mutableIntStateOf(Repository.StateT.NONE.bit)
  ...
  val actuallyLeftCommitDto =
      mutableCustomStateOf(stateKeyTag, "actuallyLeftCommitDto") { CommitDto() }
  val actuallyRightCommitDto =
      mutableCustomStateOf(stateKeyTag, "actuallyRightCommitDto") { CommitDto() }
  val initInfoDialog = {
    runCatching {
      ...
      Repository.open(curRepo.fullSavePath).use { repo ->
        val (left, right) =
            Libgit2Helper.getLeftRightCommitDto(
                repo,
                actuallyLeftCommit,
                actuallyRightCommit,
                repoId,
  ...
  if (showInfoDialog.value) {
    RepoInfoDialog(
        changeListCurRepo.value,
        showInfoDialog,
        prependContent = {
          Row { InDialogTitle(titleDesc.value) }
          ...
          Row { InDialogTitle(stringResource(R.string.repo)) }
        },
    )
  }
  ...
  Scaffold(
        ...
        TopAppBar(
            ...
            title = {
              if (changeListPageFilterModeOn.value) {
                ...
                val titleText =
                    Libgit2Helper.getLeftToRightDiffCommitsText(
                        commit1OidStrState.value,
                        commit2OidStr,
                        swap.value,
                    )
                ...
                TitleDropDownMenu(
                    ...
                    switchDropDownMenuShowHide = {
                      showParentListDropDownMenu.apply { value = !value }
                    },
                    titleFirstLine = {
                      Text(
                          text = titleText,
                    ...
                    titleRightIcon = {
                      Icon(
                          imageVector =
                              if (showParentListDropDownMenu.value) Icons.Filled.ArrowDropDown
                    ...
                    itemOnClick = {
                      ...
                      // 切换父提交则退出选择模式(现在20240420没用，但日后可能在TreeToTree页面也添加多选功能，比如可选择文件checkout or hard
                      // reset到worktree之类的，所以这里先把需要退出选择模式的逻辑写上)(20240818有用了)
            ...
            actions = {
              if (!changeListPageFilterModeOn.value) {
                Row {
                  ...
                  //
                  //                            ) {
                  //                                UIHelper.scrollToItem(scope,
                  // changeListPageItemListState, 0)
                  //                            }
                  ...
                  if (
                      UserUtil.isPro() &&
                          (dev_EnableUnTestedFeature || commitsTreeToTreeDiffReverseTestPassed)
                  ) {
                    LongPressAbleIconBtn(
                      ...
                      Msg.requireShow(
                          activityContext.getString(
                              if (swap.value) R.string.swap_commits_on
                              else R.string.swap_commits_off
                          )
                      )
    ...
    ChangeListInnerPage(
        ...
        hasNoConflictItems =
            changeListPageHasNoConflictItems, // 这选项是worktree和Index页面用的，TreeToTree其实用不到这个选项，只是占位
