    ...
    if (intent?.action == Intent.ACTION_SCREEN_OFF) {
      ...
      job.value = doJob job@{
        try {
          ...
          AutomationUtil.groupReposByPushDelayTime(lastPackage, repoList, settings)
              .forEachBetter forEach@{ pushDelayInSec, repoList ->
                ...
                if (pushDelayInSec >= 0L) {
                  ...
                  AutomationService.pushRepoList(
                      sessionId = "ScrOffAutoPush_" + generateRandomString(),
                      settings,
                      repoList,
                      targetPackageName = lastPackage,
                      currentPackageName = "screen off",
                  )
                } else {
