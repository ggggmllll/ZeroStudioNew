...
@Composable
fun RequireMasterPasswordScreen(requireMasterPassword: MutableState<Boolean>) {
  ...
  val inputPassCallback = {
    ...
    doJobThenOffLoading(loadingOn, loadingOff, loadingText.value) {
      try {
        ...
        errMsg.value =
            e.localizedMessage
                ?: (activityContext.getString(R.string.wrong_password) + ", (err msg is null)")
      }
    }
  ...
  Scaffold { contentPadding ->
    Column(
      ...
      PasswordTextFiled(
          ...
          keyboardActions = KeyboardActions(onGo = { inputPassCallback() }),
      ...
      ClickableText(
          ...
          color =
              if (loading.value) UIHelper.getDisableTextColor()
      ...
      // ok btn
      Button(enabled = loading.value.not(), onClick = { inputPassCallback() }) {
