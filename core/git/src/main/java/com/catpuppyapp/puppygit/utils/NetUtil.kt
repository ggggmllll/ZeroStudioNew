...
object NetUtil {
  ...
  fun checkApiRunning(urlString: String, timeoutInSec: Int = 5): Ret<Unit?> {
    ...
    val future: Future<Ret<Unit?>> =
        executor.submit<Ret<Unit?>> {
          ...
          try {
            ...
            return@submit Ret.createSuccess(
                null,
                "success got response with status code '$responseCode'",
            )

            //                if (responseCode == HttpURLConnection.HTTP_OK) {
            // response 200 status code
            //                    return@submit Ret.createSuccess(null, "success with HTTP_OK 200
            // status")
            //                } else {
