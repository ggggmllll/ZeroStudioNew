...
fun AutomationInnerPage(
  ...
  val updateServiceStatus = { runningStatus.value = getServiceStatus() }

  val appsFilterKeyword = mutableCustomStateOf(stateKeyTag, "appsFilterKeyword", TextFieldValue(""))
  val reposFilterKeyword =
      mutableCustomStateOf(stateKeyTag, "reposFilterKeyword", TextFieldValue(""))

  val addedAppList = mutableCustomStateListOf(stateKeyTag, "addedAppList") { listOf<AppInfo>() }
  val notAddedAppList =
      mutableCustomStateListOf(stateKeyTag, "notAddedAppList") { listOf<AppInfo>() }
  val appListLoading = rememberSaveable { mutableStateOf(SharedState.defaultLoadingValue) }

  val progressNotify = rememberSaveable {
    mutableStateOf(settingsState.value.automation.showNotifyWhenProgress)
  }
  //    val errNotify = rememberSaveable {
  // mutableStateOf(settingsState.value.automation.showNotifyWhenErr) }
  //    val successNotify = rememberSaveable {
  // mutableStateOf(settingsState.value.automation.showNotifyWhenSuccess) }
  ...
  val selectedRepoList =
      mutableCustomStateListOf(stateKeyTag, "selectedRepoList") { listOf<RepoEntity>() }
  val unselectedRepoList =
      mutableCustomStateListOf(stateKeyTag, "unselectedRepoList") { listOf<RepoEntity>() }
  ...
  val initSelectReposDialog = { packageName: String, appName: String ->
    ...
    doJobThenOffLoading(
      ...
      try {
        val packageNameLinkedRepos =
            AutomationUtil.getRepoIds(SettingsUtil.getSettingsSnapshot().automation, packageName)
        val removedNonExistsPackageNameLinkedRepos = packageNameLinkedRepos.toMutableList()
        val repos =
            AppModel.dbContainer.repoRepository.getAll(updateRepoInfo = false).toMutableList()
        ...
        SettingsUtil.update { s ->
          s.automation.packageNameAndRepoIdsMap.set(
              packageName,
              removedNonExistsPackageNameLinkedRepos,
          )
        }
      } catch (e: Exception) {
  ...
  val repoOfRepoOfAppDialog =
      mutableCustomStateOf(stateKeyTag, "repoOfRepoOfAppDialog") { RepoEntity(id = "") }
  val initRepoOfAppSettingsDialog = { appPackageName: String, targetRepo: RepoEntity ->
    ...
    val packageNameAndRepoSettings =
        AutomationUtil.getAppAndRepoSpecifiedSettings(appPackageName, targetRepo.id)
  ...
  if (showRepoOfAppSettingsDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          Column(
            ...
            MySelectionContainer {
              DefaultPaddingText(
                  stringResource(R.string.leave_empty_to_use_global_settings),
            ...
            TextField(
                ...
                label = { Text(stringResource(R.string.pull_interval_in_seconds)) },
            )

            MySelectionContainer {
              Text(
                  text = stringResource(R.string.pull_interval_note),
            ...
            TextField(
                ...
                label = { Text(stringResource(R.string.push_delay_in_seconds)) },
            )

            MySelectionContainer {
              Text(
                  text = stringResource(R.string.push_delay_note),
                  fontWeight = FontWeight.Light,
                  modifier = Modifier.padding(padding),
              )
            }
      ...
      doJobThenOffLoading {
        // 解析
        val newPullInterval =
            PackageNameAndRepoSettings.formatPullIntervalBeforeSaving(
                pullIntervalBufOfRepoOfAppDialog.value
            )
        val newPushDelay =
            PackageNameAndRepoSettings.formatPushDelayBeforeSaving(
                pushDelayBufOfRepoOfAppDialog.value
            )
        ...
        settingsState.value =
            SettingsUtil.update(requireReturnSnapshotOfUpdatedSettings = true) {
              it.automation.packageNameAndRepoAndSettingsMap.put(
                  PackageNameAndRepo(
                          appPackageNameOfRepoOfAppDialog.value,
                          repoOfRepoOfAppDialog.value.id,
                      )
                      .toKey(),
  ...
  val filterRepos = { keyword: String, list: List<RepoEntity> ->
    list.filter {
      it.repoName.contains(keyword, ignoreCase = true) || it.id.contains(keyword, ignoreCase = true)
    }
  }
                            ...
                            SettingsUtil.update(requireReturnSnapshotOfUpdatedSettings = true) {
                              it.automation.packageNameAndRepoIdsMap.let {
                                it.put(
                                    ...
                                    // 按仓库名排序，然后把id存上
                                    //
                                    // selectedRepoList.value.toList().sortedBy { it.repoName }.map
                                    // { it.id }
                                    // keep added order
                              ...
                              it.automation.packageNameAndRepoAndSettingsMap.remove(
                                  PackageNameAndRepo(
                                          packageNameForSelectReposDialog.value,
                                          clickedRepo.id,
                                      )
  ...
  val pullIntervalInSec = rememberSaveable {
    mutableStateOf(settingsState.value.automation.pullIntervalInSec.toString())
  }
  val pushDelayInSec = rememberSaveable {
    mutableStateOf(settingsState.value.automation.pushDelayInSec.toString())
  }
  val pullIntervalOrPushDelayInSecBuf =
      mutableCustomStateOf(stateKeyTag, "pullIntervalOrPushDelayInSecBuf") { TextFieldValue("") }
  ...
  if (showSetPullInternalOrPushDelayDialog.value) {
    ...
    val title =
        if (truePullIntervalFalsePushDelay) stringResource(R.string.pull_interval)
        else stringResource(R.string.push_delay)
    val label =
        if (truePullIntervalFalsePushDelay) stringResource(R.string.pull_interval_in_seconds)
        else stringResource(R.string.push_delay_in_seconds)
    ...
    ConfirmDialog2(
      ...
      doJobThenOffLoading {
        // 解析
        val newValue =
            parseLongOrDefault(pullIntervalOrPushDelayInSecBuf.value.text, default = null)
    ...
    LazyColumn(
        ...
        SettingsContentSwitcher(
            ...
            onClick = {
              ...
              Msg.requireShowLongDuration(
                  activityContext.getString(R.string.please_find_and_enable_disable_service_for_app)
              )
      ...
      item {
        SettingsContentSwitcher(
            ...
            right = { Switch(checked = progressNotify.value, onCheckedChange = null) },
            onClick = {
              val newValue = !progressNotify.value

              // save
              progressNotify.value = newValue
              SettingsUtil.update { it.automation.showNotifyWhenProgress = newValue }
            },
        )
      }
      ...
      // pull interval
      item {
        SettingsContent(onClick = { initPullIntervalOrPushDelayDialog(true) }) {
          Column {
            Text(stringResource(R.string.pull_interval), fontSize = itemFontSize)
            Text(
                appendSecondsUnit(pullIntervalInSec.value),
                fontSize = itemDescFontSize,
                fontWeight = FontWeight.Light,
            )
          }
      ...
      // push delay
      item {
        SettingsContent(onClick = { initPullIntervalOrPushDelayDialog(false) }) {
          Column {
            Text(stringResource(R.string.push_delay), fontSize = itemFontSize)
            Text(
                appendSecondsUnit(pushDelayInSec.value),
                fontSize = itemDescFontSize,
                fontWeight = FontWeight.Light,
            )
          }
      ...
      item {
        SettingsContent(onClick = { ActivityUtil.openUrl(activityContext, automationDocUrl) }) {
          Column { Text(stringResource(R.string.document), fontSize = itemFontSize) }
        }
      }
      ...
      //                    Text(stringResource(R.string.success_notification), fontSize =
      // itemFontSize)
      //                }
      //
      //                Icon(
      ...
      //                    contentDescription = if(successNotify.value)
      ...
      //                    contentDescription = if(errNotify.value) stringResource(R.string.enable)
      ...
      item {
        SelectionColumn(
          ...
          Text(
              stringResource(
                  R.string.will_auto_pull_push_linked_repos_when_selected_apps_enter_exit
              ),
      ...
      item {
        Row(modifier = Modifier.fillMaxWidth().padding(MyStyleKt.defaultItemPadding)) {
      ...
      if (appListLoading.value) {
        item {
          LoadingTextUnScrollable(
              stringResource(R.string.loading),
              PaddingValues(top = addItemBarHeight + 30.dp, bottom = 30.dp),
          )
        }
      } else {
        ...
        item {
          SettingsTitle(
              stringResource(R.string.selected_str) + "(" + filteredAddedAppList.size + ")"
          )
        }
            ...
            item {
              ...
              AppItem(
                  ...
                  trailIcons = { containerModifier ->
                    Row(
                        modifier = containerModifier.fillMaxWidth().padding(end = iconEndPadding),
                        verticalAlignment = Alignment.CenterVertically,
                        horizontalArrangement = Arrangement.End,
                    ) {
                      SizeIcon(
                      ...
                      SizeIcon(
                          ...
                          modifier =
                              Modifier.clickable {
                                ...
                                val appPackageAndRepoKeyPrefix =
                                    PackageNameAndRepo(appInfo.packageName).toKeyPrefix()
                                // 保存，从列表移除
                                settingsState.value =
                                    SettingsUtil.update(
                                        requireReturnSnapshotOfUpdatedSettings = true
                                    ) {
                                      it.automation.packageNameAndRepoIdsMap.remove(
                                          appInfo.packageName
                                      )

                                      // update app and repo specified settings
                                      val newPackageNameAndRepoAndSettingsMap =
                                          mutableMapOf<String, PackageNameAndRepoSettings>()
                                      ...
                                      it.automation.packageNameAndRepoAndSettingsMap =
                                          newPackageNameAndRepoAndSettingsMap
                                    }!!
                              },
                          imageVector = Icons.Filled.HorizontalRule,
                          contentDescription = stringResource(R.string.delete),
        ...
        item {
          SettingsTitle(
              stringResource(R.string.unselected) + "(" + filteredNotAddedAppList.size + ")"
          )
        }
                      ...
                      SizeIcon(
                          ...
                          modifier =
                              Modifier.clickable {
                                ...
                                // 保存，添加到列表
                                SettingsUtil.update {
                                  it.automation.packageNameAndRepoIdsMap.put(
                                      appInfo.packageName,
                                      listOf(),
                                  )
                                }
  ...
  LaunchedEffect(needRefreshPage.value) {
    ...
    doJobThenOffLoading {
      ...
      // update app list
      val (userAddedAppList, userNotAddedAppList) =
          AutomationUtil.getSelectedAndUnSelectedAppList(activityContext, newSettings.automation)
