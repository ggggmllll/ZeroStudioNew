...
fun ChangeListPageActions(
  ...
  //    val hasTmpStatus = changeListCurRepo.value.tmpStatus.isNotBlank()
  // //为了避免和repoCard执行的操作冲突，检查下此变量
    ...
    DropdownMenu(
        ...
        DropdownMenuItem(
            ...
            onClick = {
              ...
              requireDoActFromParentShowTextWhenDoingAct.value =
                  activityContext.getString(R.string.staging)
              requireDoActFromParent.value = true
              enableAction.value =
                  false // 禁用顶栏的按钮，避免用户重复操作，不过libgit2应该本身有避免重复执行会冲突的操作的机制，但我最好还是再控制一下，避免发生冲突才是最佳
      ...
      if (repoUnderMerge) {
        ...
        DropdownMenuItem(
            ...
            //                    enabled = enableMenuItem && hasNoConflictItems && repoUnderMerge,
            // //废弃有冲突则禁用的方案，容易让用户困惑“怎么才能continue
            // merge？”，改成点击后再检测是否有冲突了，若有，提示用户先解决冲突，这样用户就能通过和app互动来得知怎么 continue merge了
            enabled = enableMenuItem,
            text = { Text(stringResource(R.string.merge_continue)) },
            onClick = {
              Cache.set(
                  Cache.Key.changeListInnerPage_requireDoActFromParent,
                  PageRequest.mergeContinue,
              )
              requireDoActFromParentShowTextWhenDoingAct.value =
                  activityContext.getString(R.string.continue_merging)
              requireDoActFromParent.value = true
              enableAction.value = false
        ...
        DropdownMenuItem(
            ...
            onClick = {
              Cache.set(
                  Cache.Key.changeListInnerPage_requireDoActFromParent,
                  PageRequest.mergeAbort,
              )
              requireDoActFromParentShowTextWhenDoingAct.value =
                  activityContext.getString(R.string.aborting)
              requireDoActFromParent.value = true
              enableAction.value = false
      ...
      if (repoState.intValue == Repository.StateT.CHERRYPICK.bit) {
        ...
        DropdownMenuItem(
            ...
            onClick = {
              Cache.set(
                  Cache.Key.changeListInnerPage_requireDoActFromParent,
                  PageRequest.cherrypickContinue,
              )
              requireDoActFromParentShowTextWhenDoingAct.value =
                  activityContext.getString(R.string.loading)
              requireDoActFromParent.value = true
