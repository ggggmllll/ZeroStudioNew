      ...
      val ret =
          Repository.open(repoFullPath).use { repo ->
            if (hash == Cons.git_IndexCommitHash) {
              val entryId =
                  repo.index().getEntryByPath(relativePath, Index.Stage.NORMAL)?.id
                      ?: throw RuntimeException("resolve entry failed")
              val blob = Blob.lookup(repo, entryId) ?: throw RuntimeException("resolve blob failed")
              val savePath =
                  File(baseSavePath, genSavePath(entryId.toString(), fileName)).canonicalPath
              SaveBlobRet(code = LibgitTwo.saveBlobToPath(blob, savePath), savePath = savePath)
            } else {
              Libgit2Helper.saveFileOfCommitToPath(
                  repo,
                  hash,
                  relativePath,
                  genFilePath = { entry -> genSavePath(entry.id().toString(), fileName) },
              )
            }
          }
