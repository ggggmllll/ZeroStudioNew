...
class SearchOn : Search() {
  /**
   * reverse为true时，从末尾向开头搜索（从右到左）；为false时，从开头向末尾搜索（从左到右）。 主要思路：
   * 尝试在add中查找del，如果匹配不到，则更新add索引，如果匹配到，则将匹配到的内容添加为“伪删除和伪添加列表”， 然后更新索引，继续匹配，直到匹配到add或del的末尾
   *
   * 时间复杂度O(n) ，如果调用时采用`正向搜索无匹配则反向搜索 或 反向搜索无匹配则正向搜索`的逻辑，则时间复杂度翻倍为O(2n)。 ps: n 为addArr的长度
   *
   * 可匹配举例： addArr: 123, delArr:13，能匹配到1和3 add: 12345, del:
   * 012345，forwardSearch无法匹配，但reverseSearch能匹配到12345 add: 12345或123b45, del: 123a45,
   * forwardSearch匹配123，reverseSearch匹配45。注：时间复杂度为O(nm)的版本能匹配到这种字符串，能成功匹配到123和45。
   *
   * 本函数正向逆向均无法匹配，但O(nm)时间复杂度版本可匹配的情况： add: a123a, del: b123b add: 123, del:b123b
   */
  /*
  ...
  */
  override fun <T : CharSequence> doSearch(
    ...
    while (true) {
      if (addCur == addEnd && delCur == delEnd) { // implicit `matching` is true
        ...
        break
          ...
          delCur == delEnd
      ) { // note2 //implicit `matching` is true, because delCur only changed when matching is true
        addToList(delStart, delEnd, false, delList, lastDel)
    ...
    return IndexModifyResult(
        matched = hasSameChars,
        matchedByReverseSearch = reverse,
        addList,
        delList,
    )
  }
