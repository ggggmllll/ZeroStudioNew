...
// more bigger, will have more chance to use `firstVisibleLineIndex`, else, will more chance use
// `lineIndex`,
...
data class FileEditedPos(
   ...
   *
   * known bug: due to the `defaultRestoreOffset` is a guessed value, so, maybe `lineIndex`
   * invisible but still return the `firstVisibleLineIndex`, in that case, when scroll to make
   * lineIndex visible, the software keyboard will popup.
   */
  fun getLineIdxForRestoreView() =
      if ((firstVisibleLineIndex - lineIndex).absoluteValue < defaultRestoreOffset)
