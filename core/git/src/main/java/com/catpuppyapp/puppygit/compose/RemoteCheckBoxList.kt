...
fun RemoteCheckBoxList(
  ...
  val removeItem = { name: String -> selectedList.remove(name) }
  ...
  Row(
    ...
    Row(modifier = Modifier.fillMaxWidth(0.8f)) {
      MyTriCheckBox(
          text = stringResource(R.string.all) + " (${selectedList.size}/${itemList.size})",
        ...
        if (selectAll.value != ToggleableState.On) {
          ...
          checkedListState.forEachIndexedBetter { idx, it ->
            it.value = true
    ...
    Row(horizontalArrangement = Arrangement.End) {
      LongPressAbleIconBtn(
          ...
          icon =
              if (showChildren.value) Icons.Filled.KeyboardArrowDown
