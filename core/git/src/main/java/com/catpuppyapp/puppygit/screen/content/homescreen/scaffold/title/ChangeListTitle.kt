    ...
    TitleDropDownMenu(
          ...
          Text(
              ...
              text =
                  (if (dbIntToBool(changeListCurRepo.value.isDetached))
                      Libgit2Helper.genDetachedText(changeListCurRepo.value.lastCommitHashShort)
                  else
                      Libgit2Helper.genLocalBranchAndUpstreamText(
                          changeListCurRepo.value.branch,
                          changeListCurRepo.value.upstreamBranch,
                      )) +
        ...
        menuItem = { r, selected ->
          DropDownMenuItemText(
              ...
              text2 =
                  r.gitRepoState.let {
                    if (it == null) stringResource(R.string.invalid)
                    else if (it != Repository.StateT.NONE) it.toString() else ""
                  },
          )
        },
  ...
  LaunchedEffect(needReQueryRepoList.value) {
    try {
      doJobThenOffLoading {
        ...
        // 如果请求跳转到目标仓库，跳转
        val (requestType, targetRepoFullPath) =
            getRequestDataByState<String?>(needReQueryRepoList.value)
        if (
            requestType == StateRequestType.jumpAfterImport &&
                targetRepoFullPath.let { it != null && it.isNotBlank() }
        ) {
          readyRepoListFromDb
              .find { it.fullSavePath == targetRepoFullPath }
              ?.let { goToChangeListPage(it) }
        }
      }
