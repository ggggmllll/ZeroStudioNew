...
object TextMateUtil {
    ...
    themes.forEach { name ->
      ...
      themeRegistry.loadTheme(
          ThemeModel(
              ...
              .apply { isDark = name != PLTheme.THEME_LIGHT }
      )
    }
  }
  ...
  fun forEachSpanResult(
      rawText: String,
      spans: List<Span>,
      foreach: (start: Int, end: Int, spanStyle: SpanStyle) -> Unit,
  ) {
    var start = 0
    var spanIdx = 1
    ...
    while (spanIdx <= spans.size && start < rawText.length) {
      ...
      if (start < 0) { // this should never happened
        MyLog.i(
            TAG,
            "should never happened, plz check the code: invalid `start` index when apply syntax highlight styles: start=$start, endExclusive=$endExclusive, rawText.length=${rawText.length}",
        )
      ...
      if (endExclusive > rawText.length) { // this maybe happens
        if (AppModel.devModeOn) {
          MyLog.d(
              TAG,
              "invalid `end` index when apply syntax highlight styles: start=$start, endExclusive=$endExclusive, rawText.length=${rawText.length}",
          )
        }
      ...
      if (start >= endExclusive) {
        MyLog.i(
            TAG,
            "should never happened, plz check the code: empty range when apply syntax highlight styles: start=$start, endExclusive=$endExclusive, rawText.length=${rawText.length}",
        )
      ...
      //                val backgroundColor = Color(RendererUtils.getBackgroundColor(curSpan,
      // colorScheme))
    ...
    if (start < rawText.length) {
      foreach(start, rawText.length, MyStyleKt.emptySpanStyle)
    }
  }
}
