...
object AppMigrator {
  ...
  suspend fun sinceVer48(): Boolean {
    ...
    val systemDefaultTimeOffsetInSec =
        (AppModel.getSystemTimeZoneOffsetInMinutesCached() * 60).toLong()
    ...
    try {
      ...
      AppModel.dbContainer.db.withTransaction {
        ...
        AppModel.dbContainer.domainCredentialRepository.subtractTimeOffset(
            systemDefaultTimeOffsetInSec
        )
  ...
  suspend fun sinceVer122(): Boolean {
    ...
    try {
      ...
      PrefMan.set(
          ...
          PrefMan.Key.globalGitConfigUser,
          settings.globalGitConfig.username,
      )
      PrefMan.set(
          AppModel.realAppContext,
          PrefMan.Key.globalGitConfigEmail,
          settings.globalGitConfig.email,
      )
      PrefUtil.setGlobalGitConfigPullWithRebase(
          AppModel.realAppContext,
          settings.globalGitConfig.pullWithRebase,
      )
    } catch (e: Exception) {
      MyLog.e(
          TAG,
          "#$funName migrate git global config from settings.json to preference failed: ${e.stackTraceToString()}",
      )
    }
