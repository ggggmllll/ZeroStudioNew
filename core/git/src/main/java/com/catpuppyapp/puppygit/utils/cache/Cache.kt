...
object Cache : CacheStoreImpl() {
  ...
  object Key {
    ...
    const val changeListInnerPage_requireDoActFromParent =
        "changeListInnerPage_requireDoActFromParent"

    const val repoTmpStatusPrefix =
        "repo_tmp_status" // prefix+keyseparator+repoId，例如 "repo_tmp_status:a29388d9988"
    ...
    val diffableList_of_fromDiffScreenBackToWorkTreeChangeList =
        "diffableList_of_fromDiffScreenBackToWorkTreeChangeList"
    //        val diffableList_of_fromDiffScreenBackToIndexChangeList =
    // "diffableList_of_fromDiffScreenBackToIndexChangeList"
  }

  /**
   * 这个估计占不了多少内存，所以就不清了，如果清的话有点麻烦，需要避免清掉当前editor打开的文件的key，不然可能会有多个协程保存同一个文件 generate cache key for
   * save lock, you can use this key get lock obj from Cache for saving file
   *
  ...
  /**
   * 一般这个不用清，占不了多少内存，用户估计打开不了几个文件 TODO 如果清的话有点麻烦，需要避免清掉当前editor打开的文件的key，不然可能会有多个协程保存同一个文件，考虑下怎么避免
   */
   ...
   * 前缀的作用是用来清理内存时标识哪些是子页面的状态，具体实现就是在返回顶级页面后子页面已经全部弹出导航栈，这时就清理所有包含子页面前缀的key，释放内存； 随机数则是为了避免
   * a->b-c->b 这样的路径上有重复的子页面实例出现导致状态冲突。
   *
   * 顶级页面和子页面和inner page的区别： 顶级页面和子页面都是在导航组件导航进入的，其返回也是导航返回的（naviUp()），inner page本质上是组件
   ...
   * 每个组件都应该有各自的stateKeyTag，子组件若需要，往下传递即可，inner page本质上也是组件而不是页面（不可导航进入）， 所以应使用此函数为其生成stateKeyTag。
   */
  @Composable
