  ...
  Column(
    ...
    Row(
      ...
      MyCard(
          // 0.9f 占父元素宽度的百分之90
          modifier =
              Modifier.clickable(enabled = enableLoadMore) { // 如果有更多，则启用点击加载更多，否则禁用
                onClick()
              },
      ) {
        Box(
          ...
          Row(
            ...
            Text(
                text = text,
                color =
                    if (enableLoadMore) MyStyleKt.TextColor.enable
    ...
    if (enableAndShowLoadToEnd) {
      ...
      MyCard(
          // 0.9f 占父元素宽度的百分之90
          modifier =
              Modifier.clickable { // 如果有更多，则启用点击加载更多，否则禁用
                loadToEndOnClick()
              },
      ) {
        Row(
            modifier = textLineModifier.fillMaxWidth().height(buttonHeight.dp),
            horizontalArrangement = Arrangement.Center,
            verticalAlignment = Alignment.CenterVertically,
        ) {
          Text(text = loadToEndText, color = MyStyleKt.TextColor.enable)
        }
      }
    }
