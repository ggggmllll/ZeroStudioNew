...
object Libgit2Helper {
  object CommitUtil {
    ...
    fun mayGoodCommitHash(c: String): Boolean {
      return c.isNotBlank() &&
          c != Cons.git_AllZeroOidStr &&
          Cons.gitSha1HashMinLen1Regex.matches(c)
    }
  }
  ...
  object ShallowManage {
    ...
    fun createShallowBak(repo: Repository) {
      ...
      val puppyGitDirUnderGitCanonicalPath =
          AppModel.PuppyGitUnderGitDirManager.getDir(repoDotGitDir).canonicalPath
      val bak1File =
          File(puppyGitDirUnderGitCanonicalPath, bak1) // repo/.git/puppygit目录/shallow.1.bak
      val bak2File =
          File(puppyGitDirUnderGitCanonicalPath, bak2) // repo/.git/puppygit目录/shallow.2.bak
  ...
  object RebaseHelper {
    ...
    fun saveRepoCurBranchNameOrDetached(repo: Repository) {
      ...
      val puppyGitDirUnderGitCanonicalPath =
          AppModel.PuppyGitUnderGitDirManager.getDir(getRepoGitDirPathNoEndsWithSlash(repo))
              .canonicalPath
      val file = File(puppyGitDirUnderGitCanonicalPath, fileName_beforeBranch)
      if (!file.exists()) {
        file.createNewFile()
      }
      file.bufferedWriter().use { it.write(name) }
    }
  ...
  object SubmoduleDotGitFileMan {
    private fun genSubmoduleDotGitFilePath(
        parentFullPath: String,
        submodulePathUnderParent: String,
    ): String {
      return File(parentFullPath, File(submodulePathUnderParent, ".git").canonicalPath)
          .canonicalPath
    }

    fun backupDotGitFileForSubmodule(parentFullPath: String, submodulePathUnderParent: String) {
      val submoduleDotGitFullPath =
          genSubmoduleDotGitFilePath(parentFullPath, submodulePathUnderParent)
      try {
        ...
        MyLog.e(
            TAG,
            "#backupDotGitFileForSubmodule: backup git file failed, path=$submoduleDotGitFullPath",
        )
      }
    }
    ...
    fun restoreDotGitFileForSubmodule(parentFullPath: String, submodulePathUnderParent: String) {
      ...
      try {
        ...
        MyLog.e(
            TAG,
            "#restoreDotGitFileForSubmodule: restore git file failed, path=$submoduleDotGitFullPath",
        )
      }
    }
  }
  ...
  private fun getCredentialCb(
      credentialType: Int,
      credentialEntity: CredentialEntity,
  ): (url: String?, usernameFromUrl: String?, allowedTypes: Int?) -> Credential {
    ...
    return { url: String?, usernameFromUrl: String?, allowedTypes: Int? ->
      // 也可用allowedType判断决定返回什么类型的Credential，allowedTypes是个bitmask值，参考：libgit2 src/examples/common.c
      // 函数 int cred_acquire_cb()
      if (credentialType == Cons.dbCredentialTypeHttp) { // type http
        Credential.userpassPlaintextNew(usernameOrPrivateKey, passOrPassphrase)
      } else { // type SSH
        // 用户名先设置成广泛使用的默认名 git
        ...
        // params: username, publickey, privatekey,
        // passphrase。其中username和privatekey必须有，passphrase如果设置了就有，否则没有，publickey在客户端不需要。
        Credential.sshKeyMemoryNew(usernameForSsh, null, usernameOrPrivateKey, passphraseOrNull)
      }
    }
  }

  /**
   * a callback for Apply patch, for ignore submodule, because it can't be applied and make apply
   * abort
   */
  private fun getIgnoreSubmoduleApplyDeltaCallback(
      submodulePathList: List<String>
  ): (delta: Diff.Delta) -> Int {
    val skip = 1
    // >0, skip
    ...
    val cb: (delta: Diff.Delta) -> Int = cb@{ delta ->
      // get path of delta, btw Delta is a pair of old and new file, but most time can get path of
      // oldFile
      ...
      if (submodulePathList.contains(path)) {
        MyLog.d(
            TAG,
            "#getIgnoreSubmoduleApplyDeltaCallback: SKIP: target is a submodule '$path', apply it will abort the procedure",
        )
        return@cb skip
      }
  ...
  private fun getDefaultStatusOptTypeSet(): EnumSet<Status.OptT> {
    return EnumSet.of(
        ...
        // TODO submodule
        // ：有待验证此选项是否可以正常处理submodule的path，期望的是可以把submodule路径直接列出来。若不能，需要自己遍历目录，将submodule和file都当作叶子节点处理，不展开submodule文件夹
        Status.OptT
            .OPT_RECURSE_UNTRACKED_DIRS, // 包含untracked目录下的每个文件而不是只列出目录。（不设这个的话，如果目录里全是untracked文件，则不会列出其中的文件，只会列出目录名，还得我自己遍历目录文件，麻烦）
        //            Status.OptT.OPT_RENAMES_HEAD_TO_INDEX,
        // //把index条目的rename检测关了，这个会把相似度高的文件的删除和新增合并成一个renamed条目，还得检查oldfile和newfile，麻烦，不如直接把新增和删除列出来，反正用户自己知道是否重命名
  ...
  private fun getDefaultDiffOptionsFlags(): EnumSet<Diff.Options.FlagT> {
    return EnumSet.of(
        ...
        //                Diff.Options.FlagT.IGNORE_SUBMODULES,
        // //忽略submodules，把所有submodules都当作unmodified。
        ...
        //                ,Diff.Options.FlagT.REVERSE  // 反转比较的两者，例如：index to worktree 变成 worktree
        // to index
    )
  }
  ...
  private fun getRepoStatusList(
    ...
    // e.g. EnumSet.of(Status.OptT.OPT_INCLUDE_UNTRACKED, Status.OptT.OPT_RENAMES_HEAD_TO_INDEX,
    // Status.OptT.OPT_SORT_CASE_INSENSITIVELY)

    if (repo.isBare) {
      throw RuntimeException(
          "Cannot report status on bare repository: " + getRepoWorkdirNoEndsWithSlash(repo)
      )
    }
  ...
  fun checkIndexIsEmptyAndGetIndexList(
      repo: Repository,
      repoId: String,
      onlyCheckEmpty: Boolean,
  ): Pair<Boolean, List<StatusTypeEntrySaver>?> {
    ...
    if (
        !index.hasConflicts() && onlyCheckEmpty
    ) { // 如果没冲突，列表肯定不为空，但如果没设置仅检查是否为空的参数，则继续执行，取出不包含冲突条目的index条目列表
    ...
    val (_, statusMap) =
        runBlocking {
          statusListToStatusMap(repo, repoStatusList, repoId, Cons.gitDiffFromHeadToIndex)
        }
  ...
  fun indexIsEmpty(repo: Repository): Boolean {
    ...
    val (isEmpty, _) =
        checkIndexIsEmptyAndGetIndexList(repo = repo, repoId = "", onlyCheckEmpty = true)
    return isEmpty
  }
  ...
  fun getRepoCanonicalPath(repo: Repository, itemUnderRepoRelativePath: String): String {
    ...
    return getRepoWorkdirNoEndsWithSlash(repo) +
        slashChar +
        (itemUnderRepoRelativePath.trim(slashChar))
  }
  ...
  fun getRelativePathUnderRepo(
      repoPathNoFileSeparatorAtEnd: String,
      fileFullPath: String,
  ): String? {
    try {
      return fileFullPath.substring(
          fileFullPath.indexOf(repoPathNoFileSeparatorAtEnd) +
              repoPathNoFileSeparatorAtEnd.length +
              1
      )
    } catch (e: Exception) {
  ...
  // 参数说明：lineStartNum就是diffitem的行号，实际上是连续行的起始行号；originType就是 + -
  // 这类代表添加删除行的符号；content就是diffItem的content
  // 举例：输入 12、+、 ["line1content","line2content","line3content"]，返回["+12:line1content",
  // "+13:line2content","+14:line3content"]
  //        fun handleDiffItemContentByLine(lineStartNumber:Int, originType:String,
  ...
  /**
   * return: { index:{path:status, ...}, workdir:{path:status, ...}, conflict:{path:status} }，
   ...
   */
  @Deprecated(
      "实测这方法比原来的慢，不建议使用，实际上会先在jni循环一轮，" +
  ...
  suspend fun statusListToStatusMap_LoadListInJni(
    ...
    val submodulePathList =
        getSubmodulePathList(repo) // submodule name == it's path, so this list is path list too
    ...
    for (i in allStatusEntryDtos) {
      val oldFilePath =
          (if (trueIndex2WorktreeFalseHead2Index) i.indexToWorkDirOldFilePath
          else i.headToIndexOldFilePath) ?: ""
      val newFilePath =
          (if (trueIndex2WorktreeFalseHead2Index) i.indexToWorkDirNewFilePath
          else i.headToIndexNewFilePath) ?: ""
      var path = newFilePath
      var fileSize =
          (if (trueIndex2WorktreeFalseHead2Index) i.indexToWorkDirNewFileSize
          else i.headToIndexNewFileSize) ?: 0L

      val status = i.statusFlagToSet()
      // status有可能有多个状态，例如：同时包含INDEX_NEW和WT_MODIFIED两种状态
      ...
      if (status.contains(Status.StatusT.CONFLICTED)) { // index或worktree都会包含冲突条目
        ...
        if (mustPath.isNotEmpty()) {
          ...
          if (!f.exists() && removeNonExistsConflictItems) {
            MyLog.w(
                TAG,
                "#$funName(): removed a Non-exists conflict item from git, file '$mustPath' may delete after it become conflict item",
            )
          ...
          MyLog.w(
              TAG,
              "#$funName(): conflict item with empty path!, repoWorkDir at '$repoWorkDirPath'",
          )
        }
      } else {
        ...
        if (fromTo == Cons.gitDiffFromHeadToIndex) { // index
          if (status.contains(Status.StatusT.INDEX_NEW)) {
            ...
            //                            println("newFIle:::"+newFile?.size?:0) //
            // 0，无法获取已删除文件的大小，但提交后再diff，就能获取到大小了，可能是bug
      ...
      // xTODO(未来实现submodule的时候再做这个):
      // 需要区分submodule和文件夹，如果是submodule，不遍历StatusEntry.itemType为submodule，当作文件夹条目列出来且不可diff
      // xTODO: 如果path是个路径: 递归遍历，把里面所有文件都作为一个条目添加到map，每个条目的type都和路径的type一样，例如文件夹是untracked，则里面
      // 所有的条目都是untracked
      val canonicalPath = getRepoCanonicalPath(repo, path)
      val fileType = getRepoPathSpecType(path)
      //                statusTypeSaver.itemType = fileType

      //                statusTypeSaver.itemType= if(submodulePathList.contains(path) &&
      // File(canonicalPath).isDirectory) Cons.gitItemTypeSubmodule else fileType
      statusTypeSaver.itemType =
          if (submodulePathList.contains(path)) Cons.gitItemTypeSubmodule else fileType
      ...
      statusTypeSaver.fileName =
          getFileNameFromCanonicalPath(canonicalPath) // or File(canonicalPath).name
      ...
      // 目前：libgit2
      // 1.7.1有bug，status获取已删除文件有可能大小为0(并非百分百，若index为空，有可能获取到已删除文件的真实大小)，但实际不为0，所以这里检查下，如果大小等于0且类型是删除，用diffTree查询一下
      ...
      if (
          statusTypeSaver.fileSizeInBytes == 0L &&
              statusTypeSaver.changeType == Cons.gitStatusDeleted
      ) {
        val diffItem =
            getSingleDiffItem(
                ...
                // only check file size, most time very fast, no need set channel, the channel only
                // require when loading a huge content
                loadChannel = null,
    ...
    MyLog.d(
        TAG,
        "#$funName(): change list load method: end at $debugExeTime_End, spent: ${debugExeTime_End - debugExeTime_Start}",
    )
  ...
  suspend fun statusListToStatusMap_legacy(
    ...
    for (i in 0 until entryCnt) {
      ...
      //                    statusTypeSaver.fileSizeInBytes = file?.size?.toLong()?:0L
      //                }
      //                else {  //目录类型，遍历添加，不过 20240407 之后 添加了
      // Status.OptT.OPT_RECURSE_UNTRACKED_DIRS 后，应该不会执行到这个分支了，不过也没删的必要，就这样不动即可
      ...
      //                            subEntry.relativePathUnderRepo =
      // getRelativePathUnderRepo(repo.workdir().toString(), f.canonicalPath)
      ////                            statusTypeSaver.fileSizeInBytes = file?.size?.toLong()?:0L
      // //对于目录来说，这个参数不准确，不过，不会进入到这个代码块了，因为开启了递归遍历目录的flag
      //                            subs.add(subEntry)
      //                        }
      //                    }
  ...
  fun getGitUrlType(gitUrl: String): Int {
    ...
    return if (
        gitUrl.let {
          it.startsWith(Cons.gitUrlHttpsStartStr, ignoreCase = true) ||
  ...
  fun getTreeToTreeChangeList(
    ...
    // submodule，应该去submodule的commit
    // history里点击它的commit来diff，父模块顶多就是提示一下submodule关联的hash不一样了之类的，不提供文件diff。

    val ret =
        mutableListOf<
            StatusTypeEntrySaver
        >() // 如果担心有重复条目，可用Set存一下，然后addAll到list，不要把返回值改成Set，因为页面还要兼容其他类型的diff，而其他类型的diff用的都是List存条目
    ...
    val diff =
        if (treeToWorkTree) Diff.treeToWorkdir(repo, tree1, options)
        else Diff.treeToTree(repo, tree1, tree2, options)
    ...
    diff.foreach(
        { delta: Diff.Delta, progress: Float ->
          ...
          */
          ...
          stes.fileName =
              getFileNameFromCanonicalPath(stes.relativePathUnderRepo) // 用相对路径或完整路径都能取出文件名
          stes.fileParentPathOfRelativePath =
              getParentPathEndsWithSeparator(stes.relativePathUnderRepo)
          // hm, if a folder was submodule dir, but users remove it, then create a same name file,
          // the file type will become "type changed", and actually the file is not submodule
          // anymore
          //   so, here check the path is dir or not, if not, dont set type to submodule, but this
          // check may will create many File objects, wasted memory......
          //                    stes.itemType=
          // if(submodulePathList.contains(stes.relativePathUnderRepo) &&
          // File(stes.canonicalPath).isDirectory) Cons.gitItemTypeSubmodule else
          // Cons.gitItemTypeFile
          val fileType = getRepoPathSpecType(stes.relativePathUnderRepo)
          stes.itemType =
              if (submodulePathList.contains(stes.relativePathUnderRepo)) Cons.gitItemTypeSubmodule
              else fileType
          ...
          if (oldFileOid.isNullOrEmptyOrZero && !newFileOid.isNullOrEmptyOrZero) { // 新增
            ...
            //                        println("全0？：${stes.fileName},
            // ${oldFileOid.isNullOrEmptyOrZero}, ${newFileOid.isNullOrEmptyOrZero}")
            //                        stes.changeType = Cons.gitStatusUntracked
            stes.changeType = Cons.gitStatusDeleted
            stes.fileSizeInBytes = oldFile.size.toLong()
            //                        println("oldFile.size=${oldFile.size}")  // old size is ok,
            // new size is 0
        ...
        { delta, binary -> 0 },
        { delta: Diff.Delta?, hunk: Diff.Hunk -> 0 },
        { delta: Diff.Delta?, hunk: Diff.Hunk?, line: Diff.Line -> 0 },
    )
  ...
  fun revparseSingle(repo: Repository, revspec: String): GitObject? {
    try {
      ...
      MyLog.e(
          TAG,
          "#revparseSingle() error, params are (revspec=$revspec),\nerr is: " +
  ...
  fun getMergeHeads(repo: Repository): MutableList<Oid> {
    ...
    forEachMergeHeads(repo) { mergeHeadList.add(it) }
  ...
  fun readyForContinueMerge(repo: Repository, activityContext: Context): Ret<String?> {
    try {
      ...
      if (repo.index().hasConflicts()) {
        return Ret.createError(
            null,
            activityContext.getString(R.string.plz_resolve_conflicts_first),
        )
      }
  ...
  fun readyForContinueRebase(repo: Repository, activityContext: Context): Ret<Oid?> {
    ...
    try {
      ...
      //                if(getMergeHeads(repo).isEmpty()) {
      //                    return Ret.createError(null,
      // appContext.getString(R.string.no_merge_head_found))
      //                }
  ...
  fun rebaseGetCurCommitRet(repo: Repository): Ret<Commit?> {
    try {
      ...
      return Ret.createError(
          null,
          e.localizedMessage ?: "err when get cur commit of rebase",
          exception = e,
      )
    }
  }
  ...
  /**
   * @param overwriteAuthorForFirstCommit
   *   执行continue时覆盖提交的作者用户名和邮箱，只针对continue的对象也就是rebase的当前commit有效，仅当skipFirst为假时有效
  ...
  fun rebaseContinue(
      ...
      username: String,
      email: String,
      overwriteAuthorForFirstCommit: Boolean = false,
      commitMsgForFirstCommit: String = "",
    ...
    // 提交上次中断的 pick
    if (
        !skipFirst && !indexIsEmpty(repo)
    ) { // 有可能提交完了，没执行next，所以仍卡在当前commit；也有可能next了，但没commit，所以卡在当前提交，无法判断，所以这里检查下index是否为空，若为空就不创建提交了。注意：如果rebase前的分支存在空提交，这样做有可能会忽略空提交哦！
      // 为null用原提交信息，否则用传入的参数的值
      rebase.commit(
          if (overwriteAuthorForFirstCommit) rebaseCommitter else null,
          rebaseCommitter,
          null,
          commitMsgForFirstCommit.ifBlank { null },
      )
    }
    ...
    val headId =
        repo.head()?.id()
            ?: return Ret.createError(null, "rebase: get new oid failed after finished rebase")

    // 我发现只要正确处理Ac.fromRef和lookup，就不需要手动恢复分支
    // 如果执行rebase之前非detached Head，恢复一下之前的分支
    //            if(rebaseOriginName!=null) {
    // //不为null，rebase前处于某个分支，需要恢复分支；若为null，rebase前为detached HEAD，无需恢复分支
    //                //切换到之前分支
    //                checkoutLocalBranchThenUpdateHead(repo, rebaseOriginName, force=false,
    // updateHead=true)
    //
  ...
  fun rebaseAccept(
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#rebaseAccept err: params are: pathSpecList=$pathSpecList, acceptTheirs=$acceptTheirs\n" +
              e.stackTraceToString(),
      )
      return Ret.createError(
          null,
          "rebase accept ${if(acceptTheirs) "theirs" else "ours"} err: " + e.localizedMessage,
      )
    }
  }

  fun cherrypickAccept(
      repo: Repository,
      pathSpecList: List<String>,
      acceptTheirs: Boolean,
  ): Ret<String?> {
    try {
      ...
      MyLog.e(
          TAG,
          "#cherrypickAccept err: params are: pathSpecList=$pathSpecList, acceptTheirs=$acceptTheirs\n" +
              e.stackTraceToString(),
      )
      return Ret.createError(
          null,
          "cherrypick accept ${if(acceptTheirs) "theirs" else "ours"} err: " + e.localizedMessage,
      )
    }
  }

  private fun cherrypickAcceptTheirs(
      repo: Repository,
      checkoutOptions: Checkout.Options,
  ): Ret<String?> {
    val cpHeadCommitId =
        getCherryPickHeadCommit(repo).data?.id()
            ?: return Ret.createError(null, "resolve cur cherrypick commit err!")

    val targetTree =
        resolveTree(repo, cpHeadCommitId.toString())
            ?: return Ret.createError(null, "resolve cur cherrypick commit to tree err!")
   ...
   */
  private fun rebaseAcceptTheirs(
      repo: Repository,
      checkoutOptions: Checkout.Options,
  ): Ret<String?> {
    ...
    val curRebaseCommitId =
        curRebase?.id ?: return Ret.createError(null, "resolve cur rebase commit err!")

    val targetTree =
        resolveTree(repo, curRebaseCommitId.toString())
            ?: return Ret.createError(null, "resolve cur rebase commit to tree err!")
    Checkout.tree(repo, targetTree, checkoutOptions)
  ...
  fun mergeAccept(
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#mergeAccept err: params are: pathSpecList=$pathSpecList, acceptTheirs=$acceptTheirs\n" +
              e.stackTraceToString(),
      )
      return Ret.createError(
          null,
          "merge accept ${if(acceptTheirs) "theirs" else "ours"} err: " + e.localizedMessage,
      )
    }
  }
  ...
  private fun mergeAcceptTheirs(repo: Repository, checkoutOptions: Checkout.Options): Ret<String?> {
    ...
    if (mergeHeadList.size != 1) {
      return Ret.createError(
          null,
          "too less or many merge head, expect 1, but found ${mergeHeadList.size}",
      )
    }
    ...
    val mergeHeadTree =
        resolveTree(repo, mergeHead.toString())
            ?: return Ret.createError(null, "resolve merge head to tree err!")
  ...
  // x 加了个获取diff条目列表的函数，和这个配合即可，不用改这个函数了)
  // 废案)写个获取一个列表的diffitem的方法，返回一个List<DiffItem>，然后把这个single方法内部调用列表方法，只不过列表里只设置一个条目，写单个和列表逻辑都一样，但列表更通用
  ...
  suspend fun getSingleDiffItem(
      ...
      // 用来判断要diff还是patch，如果是修改类型，用patch；若是新增和删除，用diff？好像不用，都用patch就行？patch貌似只要是改变的文件，都能处理，无论新增删除修改，但如果是没修改的就返回null
      tree1: Tree? = null,
      tree2: Tree? = null,
      diffOptionsFlags: EnumSet<Diff.Options.FlagT> = getDefaultDiffOptionsFlags(),
    ...
    val diffItem =
        DiffItemSaver(
            relativePathUnderRepo = relativePathUnderRepo,
            fromTo = fromTo,
            languageScope = Box(languageScope),
        )
    ...
    // because no workdirToTree, so use treeToWorkdir+reverse instead, so must set reverse if want
    // to diff workTree to tree
    if (reverse) {
      opFlags.add(Diff.Options.FlagT.REVERSE)
    }
    ...
    val diff =
        if (fromTo == Cons.gitDiffFromIndexToWorktree) {
          ...
          if (headTree == null) {
            MyLog.w(
                TAG,
                "#$funName(): require diff from head to index, but resolve HEAD tree failed!",
            )
        ...
        //                val headTree:Tree? = resolveHeadTree(repo)
        //                if(headTree==null) {
        //                    MyLog.w(TAG, "#$funName(): require diff from head to worktree, but
        // resolve HEAD tree failed!")
        ...
        //            }
          ...
          MyLog.d(
              TAG,
              "#$funName(): require diff from tree to tree, tree1Oid=${tree1?.id().toString()}, tree2Oid=${tree2?.id().toString()}, reverse=$reverse",
          )
          //                println("treeToWorkTree:${treeToWorkTree},
          // tree1Oid=${tree1?.id().toString()}, tree2Oid=${tree2?.id().toString()},
          // reverse=$reverse")
          if (treeToWorkTree) {
            if (tree1 == null) {
    ...
    // delta hunk line
    // 区别：delta包含新旧文件路径（仓库下相对路径）和类型（主要是是否二进制）之类的信息；hunk包含一个内容片段；line组成hunk中的内容（但一个line实际可能包含多个行）。
    ...
    // 判断文件实际的修改类型
    if (
        diffItem.oldFileOid == Cons.git_AllZeroOidStr &&
            diffItem.newFileOid != Cons.git_AllZeroOidStr
    ) {
    ...
    // 检查文件大小，如果超过限制或者调用者请求仅检查文件大小(status列表可能会这么干，仅用此方法检查某个文件大小但不获取内容，正常来说是不需要的，但libgit2
    // 1.7.1有bug，有些情况无法获取已删除文件的大小，所以需要这么干)，则不查询内容
    //            diffItem.isFileSizeOverLimit =
    // isFileSizeOverLimit(diffItem.getEfficientFileSize())
    ...
    // x 废弃，一方面不限制大小了，另一方面就算取一下性能也不会有多大影响）用来存储puppyLine和rawLine对，这样可以实现先统计大小，若没超，则取line
    // content，否则不取的逻辑
    ...
    for (i in 0 until numHunks) {
      ...
      for (j in 0 until lineCnt) {
        if (loadChannel != null) {
          if (
              ++checkChannelLinesCount > checkChannelLinesLimit ||
                  checkChannelContentSizeCount > checkChannelSizeLimit
          ) {
        ...
        line.origin.let {
          ...
          if (it == Line.OriginType.ADDITION) {
            ...
            hunkAndLines.deletedLinesCount++
              ...
              it == Line.OriginType.ADD_EOFNL ||
                  it == Line.OriginType.DEL_EOFNL ||
                  it == Line.OriginType.CONTEXT_EOFNL
          ) {
        ...
        //                    pLine.content = LibgitTwo.getContent(line.contentLen,
        // LibgitTwo.jniLineGetContent(line.rawPointer))

        // 获取行号以及更新最大行号（用来对齐行号那列）
        pLine.lineNum =
            pLine.getAValidLineNum().let {
              if (it > diffItem.maxLineNum) diffItem.maxLineNum = it
              it
            }
        ...
        //
        // //假如有howManyLines是1，那最多会取两行，因为howManyLines是行号标识符，但有可能存在一行数据或多行数据最后一行不以行号结尾的情况，如果用 <
        // 来判断，可能漏一行，用 <= 则不会，只是不知道多出的一行会不会是错误的信息？
    ...
    //                pLine.oldLineNum= line.oldLineno /** Line number in old file or -1 for added
    // line */
    //                pLine.newLineNum=line.newLineno  /** Line number in new file or -1 for deleted
    // line */
    //                pLine.rawContent = line.content
    ...
    ////           (不对，用patch输出也不对劲，patch和diff.foreach()还有patch.print()，基本上，都一样)
    // 这个东西不能用来输出，乱七八糟，和git diff输出也不一样，实际上，git diff输出是patch（不知道算不算格式，类似一个事实上的diff规范），git
    // diff的输出可以当作patch的输入用来更新文件
    //            diff.foreach(
    //                { delta: Diff.Delta, progress: Float ->
    //    //                var f = delta.oldFile
    //    //                if (f == null) {
    ...
    ////                    println("==================")
    //                    val hunkAndLines =
    //                        diffItem.hunks.get(diffItem.hunks.size - 1)
    // //一个hunk调用n次linecb，所以每次被调用的linecd都和最后一个存的hunk有关
    ...
    ////                            line.oldLineno,
    ////                            line.newLineno
    ////                        ) + "," + line.oldLineno + "to" + line.newLineno + ":::" +
  ...
  /**
   * return .git folder, no ends with slash note: sometimes a folder meaning regular .git folder,
   * but is not named .git, and sometimes .git is a file include relative path to real .git foler,
  ...
  fun getRepoGitDirPathNoEndsWithSlash(repo: Repository): String {
    // I am not sure, but in my tests, this api always ends with "/" even in windows(windows default
    // separator is "\")
  ...
  fun getRepoIgnoreFilePathNoEndsWithSlash(
      repo: Repository,
      createIfNonExists: Boolean = false,
  ): String {
  ...
  fun removeFromGit(repoIndex: Index, relativePathUnderRepo: String, isFile: Boolean) {
    // .git有可能是文件，所以不等于.git的条件不能省略，不然当.git是个文件时，就会对其执行remove了（不会出错但无意义）
    // starts with ".git/" 也不能省略，这个判断适用于.git/目录下的路径
    // 实际效果：如果选中的是.git目录（或文件）本身，相对路径末尾没/，所以在判断路径是否等于.git那里会短路，不会执行后面的starts with .git/
    // 判断，所以.git/其实仅适用于忽略.git内部的文件而无法忽略.git目录（或文件）本身
    if (
        relativePathUnderRepo.isNotEmpty() &&
            relativePathUnderRepo != ".git" &&
            !relativePathUnderRepo.startsWith(".git/")
    ) {
      //            if (relativePathUnderRepo.isNotEmpty()) {  // 其实不忽略.git也无所谓，但.git及其下文件执行remove
      // from git无效，所以忽略更好
  ...
  fun getGitRemoteFetchRefSpec(
      remote: String = Cons.gitDefaultRemoteOrigin,
      branch: String = Cons.gitFetchAllBranchSign,
  ): String {
    ...
    try {
      ...
      if (
          fetch_BranchMode == Cons.dbRemote_Fetch_BranchMode_All ||
              fetch_BranchMode == Cons.dbRemote_Fetch_BranchMode_SingleBranch
      ) { // fetch all或者fetch 单分支(singlebranch)
        val refSpec = getGitRemoteFetchRefSpec(remote, branchOrBranches.trim())
        // 删除remote下所有fetch条目
        config.deleteMultivar(key, Cons.regexMatchAll)
        appendMultiVarToGitConfig(
            config,
            key,
            refSpec,
        ) // 因为前面已经删除了所有的fetch条目，所以这里不会出现多个fetch，因此用设置单个条目的方法 `config.setString(key, refSpec)` 也可以。
      } else if (
  ...
  fun getGitUserNameAndEmailFromRepo(repo: Repository): Pair<String, String> {
    ...
    return Pair<String, String>(
        config.getString(Cons.gitConfigKeyUserName).orElse(""),
        config.getString(Cons.gitConfigKeyUserEmail).orElse(""),
    )
  }
  ...
  fun saveGitUsernameAndEmailForRepo(
      repo: Repository,
      requireShowErr: (String) -> Unit,
      username: String,
      email: String,
  ...
  /**
   * get username and email from repo, if repo not set, try get from global git config, else return
   * empty string
   *
    ...
    return try {
      genCommitMsg(repo, itemList, msgTemplate).let {
        ...
        if (it.hasError() || it.data.isNullOrBlank()) {
          MyLog.w(
              TAG,
              "#genCommitMsgNoFault: generate commit msg err! will use fallback commit msg! errCode=${it.code}, errMsg=${it.msg}, commitMsgRet.data='${it.data}'",
          )
        }
   ...
   *
   * 注：如果期望index为空返回失败，应该在调用此方法前检测， 而不是依赖这个方法的返回值，因为目前这个方法已经不会在index为空时返回错误了
  ...
  private fun genCommitMsg(
    ...
    if (actuallyItemList.isNullOrEmpty()) { // 如果itemList为null或一个元素都没有，查询一下实际的index列表
      val (isIndexEmpty, indexItemList) =
          checkIndexIsEmptyAndGetIndexList(repo, "", onlyCheckEmpty = false) // 这里期望获得列表，所以仅检查空传假
      if (
          repoState != Repository.StateT.MERGE && (isIndexEmpty || indexItemList.isNullOrEmpty())
      ) { // 如果实际的index没条目，直接返回错误
        MyLog.w(
            TAG,
            "#genCommitMsg(): WARN: repo state may incorrect, state is not MERGE but index is empty, params are: repoState=$repoState, isIndexEmpty=$isIndexEmpty, indexItemList.isNullOrEmpty()=${indexItemList.isNullOrEmpty()}",
        )
  ...
  private fun genCommitMsgLegacy(
    ...
    // inclue "PuppyGit"
    //            val summary = (if(repoState==Repository.StateT.MERGE) "Conclude Merge" else
    // if(repoState==Repository.StateT.REBASE_MERGE) "Rebase" else
    // if(repoState==Repository.StateT.CHERRYPICK) "Cherrypick" else "Update $filesNum
    // ${if(filesNum>1) "files" else "file"} by PuppyGit") + (":\n")
    // no "PuppyGit"
    val summary =
        (if (repoState == Repository.StateT.MERGE) "Conclude Merge"
        else if (repoState == Repository.StateT.REBASE_MERGE) "Rebase"
        else if (repoState == Repository.StateT.CHERRYPICK) "Cherrypick"
        else "Update $allFilesCount ${if(allFilesCount>1) "files" else "file"}") + (":\n")
  ...
  //        fun genCommitMsg_deprecated_at_20240425(repo:Repository, itemList:
  // List<StatusTypeEntrySaver>?=null): Ret<String?> {
  //            var actuallyItemList = itemList
  //            if(actuallyItemList == null) { //如果itemList为null，查询一下实际的index列表
  //                val (isIndexEmpty, indexItemList) = checkIndexIsEmptyAndGetIndexList(repo, "",
  // onlyCheckEmpty = false)  //这里期望获得列表，所以仅检查空传假
  //                if(isIndexEmpty || indexItemList.isNullOrEmpty()) {
  //                    MyLog.d(TAG, "#genCommitMsg() error! isIndexEmpty = "+isIndexEmpty+",
  // indexItemList.isNullOrEmpty() = "+indexItemList.isNullOrEmpty())
  //                    return Ret.createError(null, "Index is empty!",Ret.ErrCode.indexIsEmpty)
  //                }
  //
  ...
  //            val commitMsg = "Updated " + actuallyItemList.size +" files:" +fileListStr+"...and
  // more. -- by PuppyGit"
  ...
  private fun doCreateCommit(
    ...
    val resultCommitOid =
        if (amend) {
          ...
          val author =
              if (overwriteAuthorWhenAmend) sign
              else null // if null will keep origin commit author info
          val committer = sign // committer will always is actually committer, even is amend

          Commit.amend(
              headCommitRet.data!!,
              curBranchFullRefSpec,
              author,
              committer,
              messageEncoding,
              msg,
          ...
          val author =
              if (
                  !overwriteAuthorWhenAmend &&
                      (repo.state() == Repository.StateT.CHERRYPICK ||
                          repo.state() == Repository.StateT.REBASE_MERGE)
              ) { // use origin commit's username and email
                val commitRet =
                    if (repo.state() == Repository.StateT.CHERRYPICK) getCherryPickHeadCommit(repo)
                    else /*Rebase*/ rebaseGetCurCommitRet(repo)
  ...
  fun isReadyCreateCommit(repo: Repository, activityContext: Context): Ret<Oid?> {
    ...
    if (hasConflictItemInRepo(repo)) {
      return Ret.createError(
          null,
          activityContext.getString(R.string.plz_resolve_conflicts_first),
          Ret.ErrCode.hasConflictsNotStaged,
      )
    }
    ...
    if (repoState == Repository.StateT.MERGE && getMergeHeads(repo).isEmpty()) {
      return Ret.createError(
          ...
          activityContext.getString(
              R.string
                  .repo_state_is_merge_but_no_merge_head_found_plz_use_abort_merge_to_clean_state
          ),
      )
      ...
      if (rebaseGetCurCommit(repo) == null) {
        return Ret.createError(
            ...
            activityContext.getString(
                R.string
                    .repo_state_is_rebase_but_no_rebase_head_found_plz_use_abort_rebase_to_clean_state
            ),
        )
      }
      ...
      if (getCherryPickHeadCommit(repo).data == null) {
        return Ret.createError(
            ...
            activityContext.getString(
                R.string
                    .repo_state_is_cherrypick_but_no_cherrypick_head_found_plz_use_abort_cherrypick_to_clean_state
            ),
        )
      }
    }
    ...
    if (
        repoState != Repository.StateT.MERGE &&
            repoState != Repository.StateT.REBASE_MERGE &&
            repoState != Repository.StateT.CHERRYPICK &&
            indexIsEmpty1
    ) {
      return Ret.createError(
          null,
          activityContext.getString(R.string.index_is_empty),
          Ret.ErrCode.indexIsEmpty,
      )
    }
  ...
  // 通过检查才创建提交
  //    fun createCommitIfPassedCheck(repo: Repository, activityContext:Context, msg: String,
  // username: String, email: String, settings: AppSettings):Ret<Oid?> {
  ...
  /**
   * 直接创建提交，需用调用者自行检查index是否为空，是否存在冲突条目等 若想commit后清除仓库state，需自行操作，本函数不负责
   *
  ...
  fun createCommit(
    ...
    val needAddParentsByFunc =
        (parents == null && !firstCommit) // parent若为null将自动添加parents (ps:会处理merge时多head的情况)
    ...
    if (
        firstCommit.not() && (needAddParentsByFunc || needQueryRefNameByFunc)
    ) { // 只有这两种情况需要查询head，所以判断一下
      ...
      if (needAddParentsByFunc) {
        ...
        if (curBranchLatestCommit != null) {
          ...
          return Ret.createError(
              null,
              "get current HEAD latest commit failed",
              Ret.ErrCode.createCommitFailedByGetRepoHeadCommitFaild,
          )
        }
    ...
    // 如果msg为空，自动生成提交信息
    val msg =
        if (msg.isBlank()) { // 这里检测必须用isBlank()别用isEmpty()，因为空白提交信息有可能提交失败（没测试，但我猜测提交信息可能会trim()）
          if (amend) {
  ...
  fun getUpstreamOfBranch(repo: Repository, shortBranchName: String): Upstream {
    try {
      ...
      val localBranchRef =
          resolveRefByName(repo, refsHeadsBranchName, trueUseDwimFalseUseLookup = false)

      if (localBranchRef == null) {
        // 抛异常，然后会记日志，然后返回给调用者一个空的Upstream
        throw RuntimeException(
            "resolve shortBranchName to reference failed! shortBranchName=${shortBranchName}, refsHeadsBranchName=$refsHeadsBranchName"
        )
      }
      ...
      if (
          remoteFromConfig.isBlank() || fullRefSpecFromConfig.isBlank()
      ) { // 没remote 或 没远程分支，等于没有有效上游，直接返回空上游，请求用户设置一个
        return u // 返回空上游
      }
      ...
      u.pushRefSpec =
          u.downstreamLocalBranchRefsHeadsFullRefSpec + ":" + u.branchRefsHeadsFullRefSpec

      // 用配置文件里的值拼接成远程分支名，输出例如：origin/main 这样的远程分支名，如果传参为空字符串，则返回空字符串
      u.remoteBranchShortRefSpec =
          getUpstreamRemoteBranchShortNameByRemoteAndBranchRefsHeadsRefSpec(
              u.remote,
              u.branchRefsHeadsFullRefSpec,
          )
      ...
      u.remoteBranchShortRefSpecNoPrefix =
          getShortRefSpecByRefsHeadsRefSpec(u.branchRefsHeadsFullRefSpec)
              ?: "" // 或调用 removeRemoteShortRefSpecPrefix(u.remote, u.remoteBranchShortRefSpec)
      ...
      if (
          u.remote.isNotBlank() && u.branchRefsHeadsFullRefSpec.isNotBlank()
      ) { // 如果配置文件里不为空，则查询下有无实际远程分支
        // 方法1：检测能否取得上游分支引用并获得短名称，若能，分支肯定存在，也就是已发布
        //                val remoteBranchShortName =
        // getUpstreamRemoteBranchShortRefSpecByLocalBranchShortName(repo, shortBranchName)
        //                if(remoteBranchShortName!=null && remoteBranchShortName.isNotBlank()){
        //                    u.isPublished = true
        ////                    u.remoteBranchShortRefSpec = remoteBranchShortName
        // //这个值应该和上面设置的值一样，所以更新不更新无所谓
        //                }
  ...
  fun setUpstreamForBranch(repo: Repository, upstream: Upstream, setForBranchName: String = "") {
    setUpstreamForBranchByRemoteAndRefspec(
        repo,
        upstream.remote,
  ...
  fun setUpstreamForBranchByRemoteAndRefspec(
      repo: Repository,
      remote: String?,
      fullBranchRefSpec: String?,
      targetBranchShortName: String = "",
  ): Boolean {
    if (remote.isNullOrBlank() || fullBranchRefSpec.isNullOrBlank()) {
      MyLog.d(
          TAG,
          "#setUpstreamForBranchByRemoteAndRefspec, bad upstream, remote=" +
              remote +
              ", fullBranchRefSpec=" +
              fullBranchRefSpec,
      )
    ...
    val branchWhichWillUpdate =
        if (targetBranchShortName.isBlank()) {
  ...
  fun getUpstreamRemoteBranchShortRefSpecByLocalBranchShortName(
      repo: Repository,
      localBranchShortBranchName: String,
  ): String? {
    val localBranchRef =
        resolveBranch(repo, localBranchShortBranchName, Branch.BranchType.LOCAL) // 取出本地分支的Reference
    if (localBranchRef == null) {
      return null
    }
  ...
  fun isUpstreamActuallyExistOnLocal(
      repo: Repository,
      remote: String,
      branchFullRefSpec: String,
  ): Boolean {
    // 把 remote和全分支名（例如origin和refs/heads/main），拼接成需要检查的远程分支名（例如：origin/main）
    val needCheckRemoteBranchName =
        getUpstreamRemoteBranchShortNameByRemoteAndBranchRefsHeadsRefSpec(remote, branchFullRefSpec)
    //            MyLog.d(TAG,"#isUpstreamActuallyExistOnLocal(),
    // needCheckRemoteBranchName="+needCheckRemoteBranchName)
    try {
      val ref =
          resolveRefByName(
              repo,
  ...
  fun getRemoteList(repo: Repository): List<String> {
    try {
      ...
      MyLog.e(
          TAG,
          "#getRemoteList() err, will return an empty list!\nException is:${e.stackTraceToString()}",
      )
      return listOf()
    }
  }
  ...
  fun addToIndexThenWriteToDisk(repo: Repository, pathSpecList: List<String>) {
    val index = repo.index()
    pathSpecList.forEachBetter { index.add(it) }
    // 写入硬盘
    index.write()
  }
  ...
  fun removePathSpecListFromIndexThenWriteToDisk(repo: Repository, pathSpecList: List<IgnoreItem>) {
    ...
    pathSpecList.forEachBetter {
      removeFromIndexThenWriteToDisk(
          repoIndex,
          it,
  ...
  // 参数 requireWriteToDisk 应用场景：
  // 例如外部有个列表，想删完所有条目后写入一次，那调用这个方法时就可以传false，然后自己写入即可；如果是单次调用本方法，且希望本方法保存修改，则传true
  // 如果移除的文件path是仓库.git目录下的，则不会从git移除文件，也不会报错，就跟没移过一样，所以调用此方法前不用手动排除.git目录下的文件
  fun removeFromIndexThenWriteToDisk(
      index: Index,
      ignoreItem: IgnoreItem,
      requireWriteToDisk: Boolean = false,
  ) {
  ...
  fun stageStatusEntryAndWriteToDisk(repo: Repository, list: List<StatusTypeEntrySaver>) {
    ...
    list.forEachBetter {
      try {
        ...
        MyLog.e(
            TAG,
            "#stageStatusEntryAndWriteToDisk err: fileName=$fileName, pathUnderRepo=${it.relativePathUnderRepo}, err=${e.localizedMessage}",
        )
  ...
  // 入参requireUnshallow如果为真，会执行unshallowfetch，如果为假，会判断仓库是否是shallow，如果是则取出其depth值，否则会fetch
  // all(depth=0，fetch的默认depth)
  fun fetchRemoteListForRepo(
    ...
    for (remoteAndCredentials in remoteList) {
      try {
        ...
        // TODO searchKey(多remote相关待测试) :目前的机制是多remote的情况下只有第一次unshallow成功会删除
        // shallow和shallow.1.bak文件，后续不会再删除。但是，对每个remote执行fetch都会用DepthT.UNSHALLOW，【我不太确定unshallow的时候如果shallow文件不存在，是否会报错，等以后实现了多remote，测试一下】
        if (requireUnshallow) { // 请求unshallow
          fetchOpts.depth = FetchOptions.DepthT.UNSHALLOW
        } else {
          fetchOpts.depth = FetchOptions.DepthT.FULL // 默认值

          // 不需要判断是否is shallow，克隆成功后再fetch直接用默认值full即可，如果再设shallow，又会找不到object
          // id，而且unshallow都救不回来！但如果不设，报找不到object id，unshallow还能抢救一下
        ...
        fetchOpts.prune =
            pruneType // prune会删除本地存在但远程已经不存在的分支以及tag，fetch tag时最好关了这个，不然会把本地有远程没有的tag删掉
        ...
        if (credential != null) { // 如果不是null，设置下验证凭据的回调
          setCredentialCbForRemoteCallbacks(
              callbacks,
              getCredentialTypeByUrl(remoteFetchUrl),
              credential,
          )
        }
        ...
        if (repoIsShallow && !shallowFile.exists()) {
          if (
              requireUnshallow
          ) { // 执行到这，说明仓库原本是shallow且请求unshallow，且fetch完后shallow文件不存在了，说明unshallow成功，所以可以删除bak1了
            ShallowManage.deleteBak1(
                repo
            ) // unshallow会删除shallow文件，我这再删除下 shallow.1.bak， 之后，就不会再恢复shallow文件了
        ...
        MyLog.e(
            TAG,
            "fetchRemoteListForRepo err: remoteName=${remoteAndCredentials.remoteName}, err=${e.stackTraceToString()}",
        )

        // now just throw, maybe in future, set errCallback and successCallback, then can continue
        // fetch next even this failed
        throw e
      }
    }
  }
  ...
  fun setCertCheckCallback(
    ...
    // to 'PuppyGitData/cert-user' dir to allow trusted hosts, so, no reason to set a custom cert
    // check callback for tls(https)
    if (isSshUrl(url)) {
      if (SettingsUtil.sshAllowUnknownHosts()) { // allow unknown host, no check, just passed
        ...
        callbacks.setCertificateCheckCb cb@{ cert, valid, hostname ->
          ...
          if (valid) {
            MyLog.d(
                TAG,
                "libgit2 think cert of '$hostname' is valid, the cert maybe in the 'known_hosts' file, certCb will return 1 to honor libgit2's decide, then the connection should allow",
            )
            return@cb 1
          }
          ...
          //                            check db, if in the allow list, allow, else reject and ask
          // user
          //                            add certData and allow and reject callbacks to the popup
          // dialog list, it will show to user, then do act which user choosen
          //                            the popup dialog should wait 5s then can do allow for avoid
          // mistake touched
          //                        }
          ...
          doJobThenOffLoading {
            KnownHostRequestStateMan.addToList(SshAskUserUnknownHostRequest(sshCert))
          }
          ...
          // reject at here, wait user response, then user need re-try the action before requested,
          // add allow and reject callbacks to the request too complex, and maybe can't refresh
          // view, so, let use do the action again by self, better
          return@cb -1
        }
      }
    } else { // http url
  ...
  fun isHttpSslVerifyEnabled(repo: Repository?): Boolean {
    ...
    return try {
      getRepoConfigForRead(repo).getBool(Cons.gitConfigKeyHttpSslVerify).orElse(globalSettings)
    } catch (_: Exception) {
      globalSettings
    }
  }
  ...
  suspend fun unshallowRepo(
    ...
    try {
      ...
      val remoteAndCredentialPairList =
          genRemoteCredentialPairList(
              remoteDtoListFromDb,
              credentialDb,
      ...
      // TODO searchKey(多remote相关待测试) :
      // 需要测试，支持添加多个remote后，新添加的remote是否直接fetch所有提交，如果是，这的代码就不用改，否则，取消注释下面的代码，改成先对origin执行unshallow，然后对所有分支执行 DepthT=UNSHALLOW的fetch
      val singleOriginList =
          listOf(remoteAndCredentialPairList.first { it.remoteName == Cons.gitDefaultRemoteOrigin })
  ...
  fun genRemoteCredentialPairList(
      list: List<RemoteDto>,
      credentialDb: CredentialRepository,
      requireFetchCredential: Boolean,
      requirePushCredential: Boolean,
  ): List<RemoteAndCredentials> {
    // remote名和凭据组合的列表
    ...
    list.forEachBetter { // 添加remote名和凭据进列表
      ...
      if (requirePushCredential) {
        ...
        // 用户名和密码至少一个不为空才创建凭据
        if (
            it.pushCredentialVal?.isNotBlank() == true ||
                it.pushCredentialPass?.isNotBlank() == true
        ) {
          credential = CredentialEntity()
  ...
  fun setCredentialCbForRemoteCallbacks(
      remoteCallbacks: Remote.Callbacks,
      credentialType: Int,
      credentialEntity: CredentialEntity,
  ) {
    //            用户名和密码如果都为空就不用设置了，如果一个为空，还是要设置，有的可只设token，例如gitlab就是(电脑是，手机不知道行不行)；有的可只设private
    // key，就是只有私钥没passphrase的情况，所以这两个可只有一个为空，但不能两个都为空，两个都为空就没有任何意义了。
    //            MyLog.d(TAG,"#setCredentialForFetchOptions():
    // username.isBlank():"+usernameOrPrivateKey.isBlank()+",
    // passOrPassphrase.isBlank():"+passOrPassphrase.isBlank())
    if (credentialEntity.maybeIsValid()) { // 可能是有效凭据
      ...
      MyLog.w(
          TAG,
          "#setCredentialCbForRemoteCallbacks(): call method with empty username/privatekey and password/passphrase",
      )
    }
  }
  ...
  fun setAllowUnknownHostsForCertificatesCheck(remoteCallbacks: Remote.Callbacks) {
    remoteCallbacks.setCertificateCheckCb { cert, valid, hostname ->
      // the `cert` idk the details, it should can help get the cert sha-256 or other hash for
      // fingerprints
      // if host in the know_hosts, `valid` should be true, else false
      // the "hostname" like "github.com" or "1.2.3.4"
      // 0 allow, -1 reject, 1 make libgit2 decide(I guess simple use the param 'valid', if true,
      // allow, else reject)
      // if no set this callback, the libgit2 will allow hosts in the known_hosts, and reject
      // others(same as return 1 at here)
      0
    }
  }

  suspend fun getRemoteCredential(
      remoteDb: RemoteRepository,
      credentialDb: CredentialRepository,
      repoId: String,
      remoteName: String,
      trueFetchFalsePush: Boolean,
      masterPassword: String = AppModel.masterPassword.value,
  ): CredentialEntity? {
    ...
    val credential =
        credentialDb.getByIdWithDecryptAndMatchByDomain(
            id = credentialId,
            url = remoteUrl,
  ...
  fun mergeOneHead(
      ...
      username: String,
      email: String,
      requireMergeByRevspec: Boolean = false,
      revspec: String = "",
      settings: AppSettings,
  ): Ret<Oid?> {
    return mergeOrRebase(
        repo,
        targetRefName,
        username,
        email,
        requireMergeByRevspec,
        revspec,
  ...
  private fun startRebase(
    ...
    if (state == null || (state != Repository.StateT.NONE)) {
      return Ret.createError(
          null,
          "repo state is not 'NONE'",
          Ret.ErrCode.rebaseFailedByRepoStateIsNotNone,
      )
    }
    ...
    // 分析我们的head和要合并的分支的状态，是否已经是最新，能否fast-forward之类的，Merge.analysis()
    // 这个方法不需要你传headRef，它会自己查，然后内部也是调用 Merge.analysisRef()
    ...
    // 能fast-forward
    if (
        analySet.contains(Merge.AnalysisT.UNBORN) // unborn是指HEAD还没创建，比如刚克隆的时候？不太清楚这种情况具体的发生场景
      ...
      if (theirHeads.size != 1) {
        return Ret.createError(
            null,
            "theirHeads.size!=1 when do fast-forward",
            Ret.ErrCode.fastforwardTooManyHeads,
        )
      }
      ...
      return doFastForward(
          repo,
          targetId,
          analySet.contains(Merge.AnalysisT.UNBORN),
          caller = "rebase",
      )
    }
    ...
    if (analySet.contains(Merge.AnalysisT.NORMAL)) {
      if (preferenceSet.contains(Merge.PreferenceT.FASTFORWARD_ONLY)) {
        return Ret.createError(
            null,
            "config is fast-forward only, but need merge",
            Ret.ErrCode.mergeFailedByConfigIsFfOnlyButCantFfMustMerge,
        )
      }
      ...
      val srcAnnotatedCommit =
          if (headDetachedBeforeRebase) AnnotatedCommit.lookup(repo, head.id()!!)
          else AnnotatedCommit.fromRef(repo, head)
      ...
      for (i in 0..<allOpCount) {
        ...
        if (!repo.index().hasConflicts()) { // 无冲突，提交，这里不检查index是否为空，强制创建提交 //
          ...
          return Ret.createError(
              ...
              Ret.ErrCode.mergeFailedByAfterMergeHasConfilts,
          ) // 和merge用同一个冲突错误代码
        }
      }

      // 全部rebase完成
      // 注：rebase.origHeadName 如果通过 AnnotatedCommit.lookup
      // 创建，此值为null，否则为分支名。但是rebase.ontoName则无论通过AC.lookup创建还是fromRef创建都不会返回null，前者返回提交号，后者返回分支名
      //                val rebaseOriginName = rebase.origHeadName()  //用户恢复之前的分支
      ...
      val headId =
          repo.head()?.id()
              ?: return Ret.createError(null, "rebase:get new oid err after finish rebase")

      // 只要在非detachedHead时通过fromRef创建AnnotatedCommit就不需要手动恢复分支，finish会自动恢复
      // 如果执行rebase之前非detached Head，恢复一下之前的分支
      //                if(!headDetachedBeforeRebase) {
      //                    if(rebaseOriginName==null) {  //用非fromRef
      // 方式创建的AnnotatedCommit就会为null，这时就没法恢复之前的分支了，除非你操作前记过
      //                        return Ret.createError(null, "rebase:origin name is null, can't
      // restore branch to before rebase")
      //                    }
      //
      //                    //切换到之前分支
      //                    checkoutLocalBranchThenUpdateHead(repo, rebaseOriginName, force=false,
    ...
    MyLog.e(
        TAG,
        "#$funName:rebase failed, unknown analysis set of repo: repo state before rebase=`$state` analysis set=`$analySet`, preference set=`$preferenceSet`, codepos=12614088",
    )
  ...
  private fun initRebaseOptions(rebaseOptions: Rebase.Options) {
    ...
    //                checkoutOpts.strategy = EnumSet.of(Checkout.StrategyT.FORCE,
    // Checkout.StrategyT.ALLOW_CONFLICTS)
    // 不会覆盖本地未提交的文件（推荐）
    checkoutOpts.strategy = EnumSet.of(Checkout.StrategyT.SAFE, Checkout.StrategyT.ALLOW_CONFLICTS)
  }

  // 即使targetRefName传长引用名在冲突文件里显示的依然是短引用名，所以，若想提高dwim找到引用的机率，建议这个值传长引用名（例如
  // refs/heads/abc），不过，其实合并冲突的文件里显示的是长引用名还是短引用名无所谓，都比hash好，所以就算不为了在冲突文件里显示得更友好，也建议这个传长引用名。
  fun mergeOrRebase(
      repo: Repository,
      targetRefName: String,
      username: String,
    ...
    MyLog.d(
        TAG,
        "#mergeOneHead(): targetRefName=$targetRefName, revspec=$revspec, requireMergeByRevspec=$requireMergeByRevspec",
    )

    if (username.isBlank() || email.isBlank()) {
      MyLog.w(
          TAG,
          "#$funName(): can't start merge, because `username` or `email` is blank!, username.isBlank()=${username.isBlank()}, email.isBlank()=${email.isBlank()}",
      )
      return Ret.createError(
          null,
          "username or email is blank!",
          Ret.ErrCode.usernameOrEmailIsBlank,
      )
    }
    ...
    //                return Ret.createError(null, "index has uncommitted changes!
    // ${if(trueMergeFalseRebase) "merge" else "rebase"} abort")
    //            }

    val targetAnnotatedCommit =
        if (requireMergeByRevspec) {
          ...
          if (targetRef == null) {
            return Ret.createError(
                null,
                "resolve targetRefName '$targetRefName' to Reference failed!",
                Ret.ErrCode.resolveReferenceError,
            )
          }
  ...
  private fun mergeManyHeads(
    ...
    if (state != Repository.StateT.NONE) {
      ...
      if (
          state == Repository.StateT.MERGE &&
              getMergeHeads(repo).isEmpty() &&
              !hasConflictItemInRepo(repo) &&
              getIndexStatusList(repo).entryCount() == 0
      ) {
        MyLog.w(
            TAG,
            "#mergeManyHeads(): repo state is 'MERGE' , but no conflict items and Index is empty! maybe is wrong state, will clean repo state, old state is: '$state', new state will be 'NONE'",
        )
        cleanRepoState(repo)
      } else {
        ...
        return Ret.createError(
            null,
            "merge failed! repo state is: '$state', expect 'NONE'",
            Ret.ErrCode.mergeFailedByRepoStateIsNotNone,
        )
      }
    }
    //            val ourHeadRef = repo.head()
    //            MyLog.d(TAG, "ourHeadRef == null:::"+(ourHeadRef==null))
    //            val analysisResult = Merge.analysisForRef(repo, ourHeadRef, theirHeads)
    // //需要我们自己取出head传给它，这样会增加jni中c和java的通信，也更容易出bug，所以不要用这个方法，能直接全在c做的就全在c做，全在java做的就全在java做，非必要，不通信(jni)
    ...
    if (analySet.contains(Merge.AnalysisT.NORMAL)) { // 可以且需要合并
      ...
      // 不会覆盖本地未提交的文件（推荐）
      checkoutOpts.strategy =
          EnumSet.of(Checkout.StrategyT.SAFE, Checkout.StrategyT.ALLOW_CONFLICTS)
    ...
    if (hasConflictItemInRepo(repo)) {
      ...
      if (headCommit == null) {
        return Ret.createError(
            null,
            "get current HEAD latest commit failed",
            Ret.ErrCode.mergeFailedByGetRepoHeadCommitFaild,
        )
      }
      ...
      for (ac in theirHeads) {
        ...
        //                    MyLog.d(TAG, "#mergeManyHeads() , ac.ref():"+Reference.dwim(repo,
        // ac.ref())?.shorthand())  //能取出本地(例如main)或远程分支名(例如origin/main)，但这个还得jni，没必要

        // 首先尝试从 refs/heads/本地分支名 这样的refspec查找本地分支名，如果找不到，尝试从 refs/remotes/远程仓库名/远程分支名
        // 这样的refspec查找远程分支名
        ...
        var branchNameOrRefShortHash =
            getShortRefSpecByRefsHeadsRefSpec(
                ref
            ) // ac.ref()返回的是完整refspec，例如 refs/heads/main，取出来的应该是短refspec，例如 main
        if (
            branchNameOrRefShortHash == null || branchNameOrRefShortHash.isBlank()
        ) { // 没找到本地分支名，可能是远程分支
          branchNameOrRefShortHash = getShortRefSpecByRefsRemotesRefSpec(ref) // 尝试取出远程分支名
          if (
              branchNameOrRefShortHash == null || branchNameOrRefShortHash.isBlank()
          ) { // 本地分支名和远程分支名都没找到，用 短commit_hash 代替
      ...
      // 创建提交
      //                val commitResult = doCreateCommit(repo, msg, username, email,
      // branchFullRefName, parent)
      val commitResult =
          createCommit(
              repo = repo,
              msg = msg,
      ...
      if (commitResult.hasError()) {
        return Ret.createError(
            null,
            "merge failed: " + commitResult.msg,
            Ret.ErrCode.mergeFailedByCreateCommitFaild,
        )
      }
      ...
      return Ret.createSuccess(
          commitResult.data,
          "merge success, new commit oid: " + commitResult.data.toString(),
      )
    }
  }
  ...
  private fun doFastForward(
    ...
    val targetRef =
        if (isUnborn) { // HEAD non-exist，需要查找一下
          val headRef = Reference.lookup(repo, "HEAD")
          val symbolicRef =
              headRef?.symbolicTarget()
                  ?: throw RuntimeException(
                      "doFastForward() failed by headRef.symbolicTarget() return null"
                  )
    ...
    val newTargetRef =
        targetRef.setTarget(targetOid, "$caller: ${targetRef.name()} fast-forward to $targetOid")
    ...
    return if (newTargetRef != null) {
      ...
      Ret.createSuccess(
          ...
          successCode = Ret.SuccessCode.fastForwardSuccess,
      )
    } else {
  ...
  suspend fun updateDbAfterMergeSuccess(
      mergeResult: Ret<Oid?>,
      appContext: Context,
      repoId: String,
      msgNotifier: (String) -> Unit,
      trueMergeFalseRebase: Boolean,
  ) {
    if (
        mergeResult.code == Ret.SuccessCode.upToDate
    ) { // 合并成功，但什么都没改，因为into的那个分支已经领先或者和mergeTarget拥有相同的最新commit了(换句话说：接收合并的那个分支要么比请求合并的分支新，要么和它一样)
      ...
      val fastforwardPrefix =
          if (mergeResult.code == Ret.SuccessCode.fastForwardSuccess) "(FF) " else ""

      // 显示成功通知
      msgNotifier(
          fastforwardPrefix +
              appContext.getString(
                  if (trueMergeFalseRebase) R.string.merge_success else R.string.rebase_success
              )
      )
    }
  }
  ...
  fun removeRemoteBranchShortRefSpecPrefixByRemoteName(
      remoteName: String,
      refspec: String,
  ): String? {
  ...
  fun removeRemoteBranchShortRefSpecPrefixBySeparator(refspec: String): String? {
    return removeGitRefSpecPrefix(
        "/",
        refspec,
    ) // 我想了下，这个函数就算用 / 作为前缀，也能正常工作，例如传 origin/abc，返回 abc，但如果传 refs/heads/abc ，则会返回 heads/abc
  }
  ...
  fun push(
    ...
    //            if(repo.headDetached()) {
    //                return Ret.createError(null, "push failed: head detached!",
    // Ret.ErrCode.headDetached)
    //            }
    ...
    // //传空列表，将使用配置文件中的设置，但实际上有点问题，测试了下没把我想推送的分支推上去，所以还是不要传空列表了
  ...
  /**
   * 针对每个remotes推送一遍refspecs 注意：这个函数没有force参数！如果要force
   * push，需要调用前将对应RemoteAndCredentials的的forcePush设为true
   *
   * @return 如果全推送成功，failedlist空，否则非空
   */
  fun pushMulti(
      repo: Repository,
      remotes: List<RemoteAndCredentials>,
      refspecs: List<String>,
  ): List<PushFailedItem> {
    ...
    for (rc in remotes) {
      try {
        ...
        // 应该由调用者负责log
        //                    MyLog.e(TAG, "$funName err: remoteName=${rc.remoteName},
  ...
  fun getUpstreamRemoteBranchShortNameByRemoteAndBranchRefsHeadsRefSpec(
      remote: String,
      branchFullRefSpec: String,
  ): String {
  ...
  fun getBranchList(
      repo: Repository,
      branchType: Branch.BranchType = defaultBranchTypeForList,
      excludeRemoteHead: Boolean = false,
  ): List<BranchNameAndTypeDto> {
    ...
    while (b != null) {
      ...
      //                if(!(b.key.isBranch || b.key.isRemote)) {  // if not refs/heads/xxx or
      // refs/remotes/xxx, continue
      //                    // throw RuntimeException("shou not be here 3333344444")  //test
      // ，测试了下，问题不出在这里，没抛异常
      //                    continue
      //                }
      ...
      try { // 解析oid，若是符号引用会解析出实际指向的对象指向的commit的oid，一般符号引用顶多两层即可抵达commit，例如 HEAD->main->commit
        //                        bnat.oidStr  = b.key.peel(GitObject.Type.COMMIT).id().toString()
        // //20240427:不要用resolve！经过测试，我发现有时候peel能解析出对象，但resolve会返回null。20240825：更新，这个问题已解决，是git24j里用>0当作有效指针判断导致的，实际c指针在java里可能为负数，因为java没unsigned类型，但还是建议用peel解析commit，经我测试，peel比resolve更强大，能解析annotated tags为commit，但resolve不能) or b.key.resolve().id().toString()，
        bnat.oidStr =
            b.key.peel(GitObject.Type.COMMIT)?.id()?.toString()
                ?: throw RuntimeException(
                    "resolve branch to direct ref(commit) failed, branch is: ${bnat.fullName}"
                ) // or b.key.resolve().id().toString()
      } catch (e: Exception) {
      ...
      //                        MyLog.d(TAG, "#getBranchList: origin/HEAD, point
      // to=${b.key.symbolicTarget()}")
      //                    }
      //                }
      ...
      if (b.value == Branch.BranchType.LOCAL) {
        ...
        if (upstream.remoteOid.isNotBlank() && upstream.localOid.isNotBlank()) {
          ...
          //                        if(upstream.localOid != bnat.oidStr) {
          // //如果这俩值不相等，说明解析分支的引用解析错了，测试时可启用此代码，正常来说，永远不会进入此代码块
          //                            throw RuntimeException("upstream.localOid != bnat.oidStr is
          // true, something wents wrong!")  //test
          //                        }
          // test code block end

          val (ahead, behind) =
              getAheadBehind(repo, Oid.of(upstream.localOid), Oid.of(upstream.remoteOid))
          bnat.ahead = ahead
          bnat.behind = behind
        }
  ...
  fun isBranchNameAlreadyExists(
      repo: Repository,
      branchName: String,
  ): Boolean { // 主要用来在创建分支时如果分支已存在，则提示用户改名
    return resolveRefByName(repo, branchName) != null
  }

  fun createLocalBranchBasedHead(
      repo: Repository,
      branchName: String,
    ...
    return createLocalBranchBasedOidStr(
        repo,
        branchName,
        currentHeadLatestCommitHash,
        overwriteIfExisted,
    )
  }

  // 创建成功返回: 分支长名，分支短名，完整hash 。 例如：refs/heads/main , main , abc11111111111...省略
  fun createLocalBranchBasedOidStr(
      repo: Repository,
      branchName: String,
      oidStr: String,
      overwriteIfExisted: Boolean,
    ...
    val newBranch = Branch.create(repo, branchName, commit, overwriteIfExisted)

    // 创建成功返回: 分支长名，分支短名，完整hash。
    return Ret.createSuccess(
        Triple(newBranch.name(), newBranch.shorthand(), commit.id().toString())
    )
  }

  // checkout分支并使HEAD指向分支
  fun checkoutLocalBranchThenUpdateHead(
      repo: Repository,
      branchName: String,
      force: Boolean = false,
      updateHead: Boolean = true,
  ): Ret<Oid?> {
    return checkoutBranchThenUpdateHead(
        repo,
        branchName,
        force,
        detachHead = false,
  ...
  // checkout远程分支然后把HEAD变成detached head。(这种实现方式和pc git行为一致，不过libgit2的example里checkout
  // remote分支不会使仓库变成detached head)
  fun checkoutRemoteBranchThenDetachHead(
      repo: Repository,
      branchName: String,
      force: Boolean = false,
      updateHead: Boolean = true,
  ...
  fun checkoutCommitThenDetachHead(
      repo: Repository,
      commitHash: String,
      force: Boolean = false,
      updateHead: Boolean = true,
    ...
    if (updateHead) {
      ...
      val targetCommitOid =
          checkRet.data
              ?: return Ret.createError(
                  null,
                  "checkout success but detach head failed, because new commit id is invalid",
                  Ret.ErrCode.checkoutSuccessButDetacheHeadFailedByNewCommitInvalid,
              )
  ...
  private fun checkoutBranchThenUpdateHead(
      repo: Repository,
      branchName: String,
      force: Boolean = false,
      detachHead: Boolean,
  ...
  private fun checkoutByHash(
    ...
    val strategy =
        if (force) EnumSet.of(Checkout.StrategyT.FORCE) else EnumSet.of(Checkout.StrategyT.SAFE)
    ...
    if (errno < 0) {
      ...
      return Ret.createError(
          null,
          "Checkout Tree err(errno=$errno)!",
          Ret.ErrCode.checkoutTreeError,
      )
    }
  ...
  fun resolveRefByName(
    ...
    try {
      MyLog.v(
          TAG,
          "#resolveRefByName(refNameShortOrFull=$refNameShortOrFull, trueUseDwimFalseUseLookup=$trueUseDwimFalseUseLookup)",
      )

      // 注：如果使用lookup必须全名查找，例如refs/heads/main 或
      // refs/remotes/origin/main，而且lookup不会解引用对象，如果ref是个符号引用(symbolic reference)，就会返回一个符号引用
      val ref =
      ...
      MyLog.d(
          TAG,
          "#resolveRefByName(): resolve refname err! refname=" +
              refNameShortOrFull +
              ", trueUseDwimFalseUseLookup=$trueUseDwimFalseUseLookup, err is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  /**
   * @param tryResolveRefToDirect this only make difference when Reference pointed to a symbolic
   *   target
   */
  fun resolveRefByName2(
    ...
    try {
      //                MyLog.d(TAG,
      // "#resolveRefByNameRetVersion(refNameShortOrFull=$refNameShortOrFull,
      // trueUseDwimFalseUseLookup=$trueUseDwimFalseUseLookup)")
      ...
      return Ret.createSuccess(
          if (tryResolveRefToDirect) ref?.resolve() else ref
      ) // resolve reference to direct ref, direct ref is point to commit, no symbolicTarget
    } catch (e: Exception) {
  ...
  fun resolveGitObject(repo: Repository, targetOid: Oid, type: GitObject.Type): GitObject? {
    try {
      ...
      MyLog.e(
          TAG,
          "#resolveGitObject(): resolve GitObject err! targetOid=$targetOid, type=${type.name} \n Exception is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  fun resolveCommitByHash(repo: Repository, shortOrLongHash: String): Commit? {
    try {
      ...
      MyLog.d(
          TAG,
          "#resolveCommitByHash() error, param is (shortOrLongHash=$shortOrLongHash):\nerr is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  fun resolveBranchRemotePrefix(repo: Repository, fullRemoteBranchRefSpec: String): String {
    try {
      // 注：好像，有时候，符号引用解析出来会是空指针，但有时候就不会，原因不明，可能跟single
      // branch有关，但我没验证过，参见，未解决的问题文档，标题：“Libgit2Helper#resolveBranchRemotePrefix解析出空指针问题 20240510”
      val remoteName =
          Branch.remoteName(repo, fullRemoteBranchRefSpec)
              ?: "" // 后面必须做空值判断：因为返回值为String而不是String?，所以，如果remoteName为null，会报异常，然后进入异常代码块，最后返回空字符串
      MyLog.d(
          TAG,
          "#resolveBranchRemotePrefix: in: fullRemoteBranchRefSpec=$fullRemoteBranchRefSpec; out: remoteName=$remoteName",
      )
      return remoteName
    } catch (e: Exception) {
      MyLog.e(
          TAG,
          "#resolveBranchRemotePrefix() error, param is (fullRemoteBranchRefSpec=$fullRemoteBranchRefSpec) :\nerr is: " +
              e.stackTraceToString(),
      )
      return ""
    }
  ...
  fun resolveRemote(repo: Repository, remoteName: String): Remote? {
    try {
      ...
      MyLog.e(
          TAG,
          "#resolveRemote() error, param is (remoteName=$remoteName):\nerr is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  fun getRemoteFetchBranchList(remote: Remote): Pair<Boolean, List<String>> {
    ...
    try {
      ...
      val isAllRealValue =
          branchNameList.size == 1 && branchNameList[0] == Cons.gitFetchAllBranchSign
  ...
  fun resolveBranch(
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#resolveBranch() error, params are (branchShortName=$branchShortName, type=${type.name}),\nerr is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  fun resolveTree(repo: Repository, revspec: String): Tree? {
    try {
      val tree =
          if (revspec == Cons.git_IndexCommitHash) {
            Tree.lookup(repo, repo.index().writeTree())
          } else {
            Tree.lookup(
                repo,
                Revparse.lookup(repo, "$revspec^{tree}").getFrom().id(),
                GitObject.Type.TREE,
            ) as? Tree
          }
      ...
      MyLog.e(
          TAG,
          "#resolveTree() error, params are (revspec=$revspec),\nerr is: " + e.stackTraceToString(),
      )
      return null
    }
  }
  ...
  fun deleteBranch(repo: Repository, branchNameShortOrFull: String): Ret<String?> {
    ...
    if (ref == null) {
      return Ret.createError(
          null,
          "resolve branch name '$branchNameShortOrFull' to reference failed!",
          Ret.ErrCode.resolveReferenceError,
      )
    }
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#deleteBranch(): delete branch '$branchNameShortOrFull' error:\n" +
              e.stackTraceToString(),
      )
      return Ret.createError(
          null,
          "delete branch '$branchNameShortOrFull' error: ${e.localizedMessage}",
          Ret.ErrCode.deleteBranchErr,
      )
    }
  }

  // 如果解析remote前缀不存在歧义，可调用这个方法，否则需让用户输入具体remote名，然后调用deleteRemoteBranchByRemoteAndRefsHeadsBranchRefSpec()
  // 参数remoteBranchFullRefSpec，预期，类似：refs/remotes/origin/branchname，不要带冒号，本函数执行push的时候会自己加，ps
  // “:远程分支名” 的意思是本地没你的下游了，自我了断吧，所以作用就是删除冒号后面的远程分支名
  fun deleteRemoteBranch(
      repo: Repository,
      remoteBranchFullRefSpec: String,
      credential: CredentialEntity?,
  ): Ret<String?> {
    ...
    if (remote.isNullOrBlank()) {
      MyLog.d(
          TAG,
          "#deleteRemoteBranch(): resolve remote prefix failed, param: remoteBranchFullRefSpec=$remoteBranchFullRefSpec",
      )
      return Ret.createError(
          null,
          "resolve remote name from remote branch full refspec failed!",
          Ret.ErrCode.resolveRemotePrefixFromRemoteBranchFullRefSpecFailed,
      )
    }
    // 取出分支名
    val remoteBranchShortName =
        removeGitRefSpecPrefix("refs/remotes/$remote/", remoteBranchFullRefSpec)
    val remoteBranchFullName = "refs/heads/$remoteBranchShortName"

    return deleteRemoteBranchByRemoteAndRefsHeadsBranchRefSpec(
        repo,
        remote,
  ...
  fun deleteRemoteBranchByRemoteAndRefsHeadsBranchRefSpec(
      repo: Repository,
      remote: String,
      refsHeadsRefspec: String,
      credential: CredentialEntity?,
  ): Ret<String?> {
    try {
      if (remote.isNullOrBlank()) {
      ...
      //                if(remoteObj==null) {
      //                    return Ret.error(null, "resolve remote failed!",
      // Ret.ErrCode.resolveRemoteFailed)
      //                }
      // :refspec，等于删除远程分支
      ...
      MyLog.e(
          TAG,
          "#deleteRemoteBranchByRemoteAndRefsHeadsBranchRefSpec() error:params are(remote=$remote, refsHeadsRefspec=$refsHeadsRefspec), err=" +
              e.stackTraceToString(),
      )
      // e.g. "del 'refs/heads/abc' for remote 'origin' err: err msg"
      return Ret.createError(
          null,
          "del branch '$refsHeadsRefspec' for remote '$remote' err: ${e.localizedMessage}",
          exception = e,
      )
    }
  }
  ...
  fun getSingleCommit(
    ...
    // 后面如果出错会返回这个dto
    val errReturnDto =
        CommitDto(oidStr = commitOidStr, shortOidStr = getShortOidStrByFull(commitOidStr))
    ...
    //            val shallowOidList =
    // ShallowManage.getShallowOidList(repo.workdir().toString()+File.separator+".git")
  ...
  // 返回值 (nextOid,
  // CommitDtoList)，nextOid就是CommitDtoList列表里最后一个元素之后的Oid，用来实现加载更多，如果不存在下一个元素，则是null，意味着已经遍历到提交树的最初提交了
    ...
    while (next != null) {
      try {
        ...
        if (commitDto == null) {
          ...
          if (commit != null) {
            ...
            for ((idx, node) in draw_lastOutputNodes.value.withIndex()) {
              // 更新当前节点
              val newNode =
                  DrawCommitNode.transOutputNodesToInputs(
                          idx = idx,
                          node = node,
                      ...
                      .let {
                        ...
                        it.mergedList.forEachIndexedBetter { idx, node ->
                          ...
                          newMergedList.add(
                              ...
                              node.copy(
                                  circleAtHere = it.circleAtHere,
                                  endAtHere = it.endAtHere,
                                  outputIsEmpty = it.outputIsEmpty,
                                  startAtHere = it.startAtHere,
                              )
                          )
                        }
            ...
            if (drawInputs.isNotEmpty()) {
              ...
              while (--reservedIdx >= 0) {
                ...
                if (hasInputLineToOutput || curNode.endAtHere.not()) {
                  ...
                  MyLog.v(
                      TAG,
                      "#$funName: commitDrawNodeInfo inputs to outputs: hash=${c.oidStr}, node=$curNode",
                  )
                }
              }
            }
              ...
              for (idx in (if (drawInputs.isEmpty()) 0 else 1)..<c.parentOidStrList.size) {
                ...
                val newNode =
                    DrawCommitNode(
                        ...
                        circleAtHere =
                            idx == 0 &&
                                drawInputs
                                    .isEmpty(), // 如果是HEAD first commit，inputs为空，则在这画圆圈，否则由上面的输入节点画
                        fromCommitHash = c.oidStr,
                        toCommitHash = p,
                    )
                ...
                DrawCommitNode.getAnInsertableIndex(drawOutputs, p).let { pos ->
                  ...
                  if (pos.index >= 0) { // 索引有效，替换
                    drawOutputs[pos.index] =
                        if (pos.isMergedToPrevious) { // 非空节点，但画线时可和前一条线合并
                          // 合流：若合流，当前线会和上一条线合并。代码实现上就直接把当前节点添加到对应位置的节点的mergedList就行了，类似sub节点
                          drawOutputs[pos.index].let {
                            it.copy(
                                mergedList = it.mergedList.toMutableList().apply { add(newNode) }
                            )
                          }
                        } else { // 空节点
                          newNode
        ...
        if (++checkChannelCount > checkChannelFrequency) {
          // TODO 这个检测可以改成 delay(1)
          // ，然后再外部希望终止任务时，调用下job.cancel()即可，另外delay(1)的作用是响应cancel()使代码块抛出canceledException()
          // 20250430：现在外部没靠job.cancel()取消任务，还是用的channel，不过加个delay没坏处，可响应compose
          // 用的scope的取消“信号”，虽然我不确定compose销毁时是否会对scope.launch的任务调用cancel()。。。。。。
          delay(1)
  ...
  /**
   * @return last(previous) tree entry oid, maybe is null but still has more commits, casue if
   *   commit no entry will return null, but it maybe exists in subsequent commits
  ...
  suspend fun getFileHistoryList(
    ...
    while (next != null) {
      try {
        ...
        //                        MyLog.e(TAG, "#getCommitList: get channel result err:
        // ${cherr.localizedMessage}")
        //                        break
        //                    }
                ...
                if (entryOidStr != lastVersionEntryOid) {
                  ...
                  if (AppModel.devModeOn) {
                    if (
                      ...
                      throw RuntimeException(
                          "#getFileHistoryList() err: Wrong State: lastCommit=${lastCommit?.id().toString()}, lastVersionEntryOid=$lastVersionEntryOid"
                      )
                    }
                  }
  ...
  fun getEntryOrNullByPathOrName(tree: Tree, path: String, byName: Boolean): Tree.Entry? {
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#getEntryOrNullByPathOrName err: path=$path, byName=$byName, err=${e.stackTraceToString()}",
      )
      return null
    }
  }
  ...
  fun getDateTimeStrOfCommit(commit: Commit, settings: AppSettings): String {
    ...
    val minuteOffset =
        readTimeZoneOffsetInMinutesFromSettingsOrDefault(settings, commit.timeOffset())

    val secOffset = minuteOffset * 60 // commit.timeOffset() 返回的是分钟偏移量，需要转换成秒给java的对象使用
    val formattedTimeStr =
        time.atOffset(ZoneOffset.ofTotalSeconds(secOffset)).format(Cons.defaultDateTimeFormatter)
  ...
  fun getRepoOnBranchOrOnDetachedHash(repo: RepoEntity): String {
    return if (dbIntToBool(repo.isDetached))
        "[${repo.repoName} on ${repo.lastCommitHashShort}(Detached)]"
    else "[${repo.repoName} on ${repo.branch}]"
  }
  ...
  suspend fun doCheckoutBranchThenUpdateDb(
    ...
    val checkoutRet =
        if (checkoutType == Cons.checkoutType_checkoutRefThenUpdateHead) { // checkout本地分支
          // checkout分支并更新HEAD
          checkoutLocalBranchThenUpdateHead(repo, fullBranchNameOrFullHash, force, updateHead)
        } else if (
            checkoutType == Cons.checkoutType_checkoutRefThenDetachHead
        ) { // checkout 远程分支/Tag
    ...
    if (updateHead) {
      ...
      val lastCommitHash =
          getShortOidStrByFull(
              checkoutRet.data.toString()
          ) // 执行到这，checkout肯定成功，data为commit Oid，应该不会为null
      ...
      //                if(checkoutType != Cons.checkoutTypeCommit) {  //如果checkoutType不是commit(是
      // 本地分支 或 远程分支)，解析ref（一般是分支名），取出其commitId
      //                    //查最新的commit hash
      //                    // resolveRefByName() 用dwim()实现，应该会找到一个直接引用
      //                    val resolveRefByName = resolveRefByName(repo,
      // shortBranchNameOrShortHash);
      ...
      //                        return Ret.createError(null,
      // appContext.getString(R.string.err_when_querying_branch_commit_id) , Ret.ErrCode.refIsNull)
      //                    }
      //
      ////                if(resolveRefByName.symbolicTarget()!=null) { //
      // 如果id是null的话，需要peel出commit，不过应该不需要，因为resolveRefByName()用dwim()实现的，应该会取出直接引用
      ////
      ////                }
      //                    //把commithash改成现在的分支最新新commithash
      //                    lastCommitHash =
      // resolveRefByName.id().toString().substring(Cons.gitShortCommitHashRange)
      //                }
      ...
      if (checkoutType == Cons.checkoutType_checkoutRefThenUpdateHead) { // 本地分支
        ...
        repoDb.updateBranchAndCommitHash(
            ...
            upstreamBranch =
                upstreamBranchShortNameParam, // 这个参数是当前分支的上游，如果是新建分支，应该传空，因为新创建的分支，肯定没上游，因为我没写创建时设置上游的功能
        )
  ...
  fun doCreateBranch(
    ...
    // 这个如果检查，不会发生本地和远程分支同名的情况，例如同时存在 refs/remotes/origin/abc 和
    // refs/heads/origin/abc，好处是不容易混淆，坏处是增加了限制；
    ...
    //                    return
    // Ret.createError(null,appContext.getString(R.string.branch_already_exists),
    // Ret.ErrCode.branchAlreadyExists)
    //                }
    //            }
    ...
    val result =
        if (basedHead) {
          createLocalBranchBasedHead(
              repo,
              branchNameParam,
              overwriteIfExisted,
          ) // 内部也调用的 #createLocalBranchBasedOidStr，所以返回值和 createLocalBranchBasedOidStr 匹配
        } else { // 基于remote创建分支！
          ...
          if (createByRef) { // 如果是用引用创建分支，会查一下其commitOid，并给refOidStr重新赋值
            ...
            if (baseRef == null) {
              ...
              return Ret.createError(
                  null,
                  activityContext.getString(R.string.resolve_reference_failed),
                  Ret.ErrCode.refIsNull,
              )
            }
  ...
  fun isRepoStatusNotReady(repoFromDb: RepoEntity): Boolean {
    return repoFromDb.workStatus == Cons.dbRepoWorkStatusNotReadyNeedClone ||
        repoFromDb.workStatus == Cons.dbRepoWorkStatusNotReadyNeedInit
  }
  ...
  /**
   * @param requireQueryParentInfo query parent Info by parentRepoId, used for set parent repo info,
   *   e.g. parentRepoName, if pass false to this value, all repo's parentXXXX filed will not
   *   update, that means they maybe invalid, except parentRepoId 如果不传此值，将不会查询parent
   *   repo，也不会更新当前仓库的parent相关字段(例如parentRepoName)，这时，除了parentRepoId以外的其他parent相关字段都可能不准确
   */
  suspend fun updateRepoInfo(
    ...
    try {
      ...
      Repository.open(repoFromDb.fullSavePath).use { repo ->
        ...
        //                        repo.state()
        //                    }catch (e:Exception) {
        //                        MyLog.e(TAG, "#$funName: query repo state err:
        // ${e.stackTraceToString()}")
        //                        null
        //                    }
        ...
        if (!dbIntToBool(repoFromDb.isDetached)) { // 只有非detached才有upstream
          ...
          if (
              upstream.isPublished &&
                  upstream.remote.isNotBlank() &&
                  upstream.branchRefsHeadsFullRefSpec.isNotBlank()
          ) {
            // 非detached时，检查 ahead behind commit，来决定是否提示用户需要同步
            val localOid = head?.id()
            val upstreamOid =
                resolveRefByName(repo, upstream.remoteBranchRefsRemotesFullRefSpec)?.id()
            if (localOid != null && upstreamOid != null) {
              ...
              if (
                  repoFromDb.ahead == 0 && repoFromDb.behind == 0
              ) { // 本地不领先也不落后上游，最新。 or `ahead<1 && behind<1`
        ...
        if (hasConflictItemInRepo(repo)) { // 有冲突条目，这个检查起来很快，所以就在这直接查了
          ...
          //                            tmpStatusIfHave =
          // AppModel.activityContext.getString(R.string.loading)
          ...
          if (
              repoState == Repository.StateT.MERGE
          ) { // 不一定有冲突条目，但仓库处于merge状态，这个必须处理，不然changelist能看出来在merge状态，仓库页面卡片还显示错误的up-to-date，之前遇到过几次，容易让人迷惑
      ...
      // TODO 这里可设个特殊的仓库状态，卡片显示查询仓库信息出错，可选操作为
      // 刷新、删除，和克隆错误的区别在于无法编辑仓库信息，或者也可支持重新编辑克隆信息并克隆（需要提醒用户会删除之前克隆时创建的本地仓库目录）
      // 这里不插入了，因为这个函数经常调用的关系，如果仓库一更新错且没及时处理，会生成很多调错误记录
      //                createAndInsertError(repoFromDb.id, "update repo info err!")
      // 不过log还是要记下的
      MyLog.d(
          TAG,
          "#$funName() failed: repoId=${repoFromDb.id}, repo.toString()=${repoFromDb.toString()}\n Exception is: ${e.stackTraceToString()}",
      )
    }
  }
  ...
  fun getAheadBehind(repo: Repository, localOid: Oid, upstreamOid: Oid): Pair<Int, Int> {
    try {
      ...
      MyLog.d(
          TAG,
          "#getAheadBehind() failed!, localOid=${localOid}, upstreamOid=${upstreamOid}\n Exception is:${e.stackTraceToString()}",
      )
      // 查不出来就当up-to-date吧
      return Pair(0, 0)
    }
  ...
  fun findRepoByPath(
      underGitRepoPath: String,
      cellingDir: String = FsUtils.getExternalStorageRootPathNoEndsWithSeparator(),
  ): Repository? {
    // 最后一个参数是用系统分隔符(linux 是 :, windows可能是 ;)分隔的目录列表，搜索如果遇到上限目录就会停止
    try { // 如果找到，返回仓库引用，否则返回null

      // 可正常处理submodule（子模块），如果入参是一个子模块下的路径，会返回子模块的仓库引用而不是父的。（因此用在remove from
      // git功能时，不会用父仓库移除子模块下的文件。）
  ...
  fun getRepoStateText(repoStateIntValue: Int, appContext: Context): String {
    ...
    return if (repoStateIntValue == Cons.gitRepoStateInvalid) {
      // Repository state is null, that means repo maybe broken or corrupt or donesn't exists
      // anymore
  ...
  fun getAllTags(repoId: String, repo: Repository, settings: AppSettings): List<TagDto> {
    ...
    Tag.foreach(repo) { name, oidStr ->
      ...
      if (commit != null) {
        tagDto.pointedCommitDto =
            createSimpleCommitDto(commit = commit, repoId = repoId, settings = settings)
      }
  ...
  fun fetchAllTags(
      repo: Repository,
      repoFromDb: RepoEntity,
      remoteAndCredentials: List<RemoteAndCredentials>,
      force: Boolean = false,
  ) {
    val refspecs =
        Array(1) {
          if (force) "+${Cons.gitAllTagsRefspecForFetchAndPush}"
          else Cons.gitAllTagsRefspecForFetchAndPush
        } // 这+号好像还是有点用的
    //            println("refspecs:${refspecs.contentToString()}")  // refspec没问题，就是单纯不支持 +
    // 作为force，他妈的，还得配合autodownloadtag选项才能实现强制覆盖tags
    ...
    fetchRemoteListForRepo(
        ...
        repoFromDb,
        requireUnshallow = false,
        refspecs,
        pruneType =
            FetchOptions.PruneT
                .UNSPECIFIED, // FetchOptions.PruneT.UNSPECIFIED作用是遵循配置文件prune，默认情况下可避免删除远程有本地没有的tags，除非用户改配置文件强制启用prune
  ...
  //        fun pushAllTags(repo:Repository, remoteAndCredentials:List<RemoteAndCredentials>, force:
  // Boolean=false, delMode:Boolean=false) {
  //            val refspecs = if(delMode) listOf(Cons.gitDelAllTagsRefspecForFetchAndPush) else
  // listOf(if(force) "+${Cons.gitAllTagsRefspecForFetchAndPush}" else
  // Cons.gitAllTagsRefspecForFetchAndPush)
  ...
  fun createRemote(repo: Repository, remoteName: String, url: String): Ret<Remote?> {
    return try {
      val remote = Remote.create(repo, remoteName, url)
      Ret.createSuccess(remote)
    } catch (e: Exception) {
      MyLog.e(
          TAG,
          "#createRemote err: remoteName=$remoteName, url=$url, err=" + e.stackTraceToString(),
      )
  ...
  fun stashList(repo: Repository, out: MutableList<StashDto>): List<StashDto> {
    Stash.foreach(repo) { index: Int, message: String?, stashId: Oid ->
  ...
  fun getReflogList(
      repo: Repository,
      name: String,
      out: MutableList<ReflogEntryDto>,
      settings: AppSettings,
  ): List<ReflogEntryDto> {
    val timeZoneOffsetMins =
        readTimeZoneOffsetInMinutesFromSettingsOrDefaultNullable(settings, null)
  ...
  fun cherrypick(
      repo: Repository,
      targetCommitFullHash: String,
      parentCommitFullHash: String = "",
      pathSpecList: List<String>? = null,
      cherrypickOpts: Cherrypick.Options? = null,
   ...
   * 解决完冲突后调用这个
   *
   * @param overwriteAuthor
   *   overwriteAuthor为真则新提交的author会变成执行cherrypick的那个人的信息而不是原提交的信息；若为假则使用原始提交作者信息
   * @param msg 如果为空，将会使用原提交的msg
   */
  fun cherrypickContinue(
      activityContext: Context,
      repo: Repository,
      msg: String = "",
      username: String,
      email: String,
      autoClearState: Boolean = true,
      overwriteAuthor: Boolean,
      settings: AppSettings,
  ): Ret<Oid?> {
    ...
    val ret =
        createCommit(
            ...
            cleanRepoStateIfSuccess = autoClearState,
            settings = settings,
        )
   ...
   * @return 返回Ret对象，包含:文件路径 和
   *   diff内容。（当outFile不为null时，返回值必包含文件路径；当returnDiffContent为true时，返回值必包含diff内容(patch内容）（补丁内容））
   */
  fun savePatchToFileAndGetContent(
    ...
    try {
      ...
      val diff =
          if (fromTo == Cons.gitDiffFromIndexToWorktree) {
            ...
            if (headTree == null) {
              ...
              return Ret.createError(
                  null,
                  "require diff from head to index, but resolve HEAD tree failed!",
              )
            }
          ...
          //                    val headTree:Tree? = resolveHeadTree(repo)
          //                    if(headTree==null) {
          //                        MyLog.w(TAG, "#$funName(): require diff from head to worktree,
          // but resolve HEAD tree failed!")
          //                        return Ret.createError(null, "require diff from head to
          // worktree, but resolve HEAD tree failed!")
          //                    }
          //
      ...
      // 返回操作成功
      return Ret.createSuccess(
          PatchFile(outFileFullPath = outFile?.canonicalPath, content = retContent)
      )
    } catch (e: Exception) {
      MyLog.e(TAG, "#$funName err: ${e.stackTraceToString()}")
  ...
  fun applyPatchFromFile(
    ...
    try {
      ...
      // 取出patch内容
      var content = ""
      inputFile.bufferedReader().use { content = it.readText() }
      // 检查内容是否为空
      if (content.isBlank()) {
  ...
  fun getHeadCommit(repo: Repository): Ret<Commit?> {
    try {
      ...
      val commit =
          resolveCommitByHash(repo, head.id().toString())
              ?: return Ret.createError(null, "HEAD commit is null")
  ...
  fun resolveTag(repo: Repository, oidFullOrShortStr: String): Tag? {
    try {
      ...
      MyLog.e(
          TAG,
          "#resolveTag() error, params are (oidFullOrShortStr=$oidFullOrShortStr}),\nerr is: " +
              e.stackTraceToString(),
      )
      return null
    }
  ...
  fun resolveCommitOidByRef(repo: Repository, shortOrFullRefSpec: String): Oid? {
    try {
      ...
      MyLog.d(
          TAG,
          "#resolveCommitOidByRef() error, params are (shortOrFullRefSpec=$shortOrFullRefSpec}),\nerr is: " +
              e.stackTraceToString(),
      )
  ...
  /**
   * @return if success, return Ret success and non-null Oid; else return err and null data and err
   *   msg, and exception(if have)
   */
  fun resolveCommitOidByRef2(repo: Repository, shortOrFullRefSpec: String): Ret<Oid?> {
    try {
      ...
      //                MyLog.e(TAG, "#resolveCommitOidByRef() error, params are
      // (shortOrFullRefSpec=$shortOrFullRefSpec}),\nerr is:"+e.stackTraceToString())
  ...
  fun resolveCommitByRef(repo: Repository, shortOrFullRefSpec: String): Commit? {
    try {
      ...
      MyLog.d(
          TAG,
          "#resolveCommitByRef() error, params are (shortOrFullRefSpec=$shortOrFullRefSpec}),\nerr is: " +
              e.stackTraceToString(),
      )
  ...
  fun resolveCommitByHashOrRef(repo: Repository, hashOrBranchOrTag: String): Ret<Commit?> {
    ...
    try {
      ...
      MyLog.d(
          TAG,
          "#$funName() error, params are (hashOrBranchOrTag=$hashOrBranchOrTag}),\nerr is: " +
              e.stackTraceToString(),
      )
      return Ret.createError(
          null,
          e.localizedMessage ?: "resolve commit err: param=$hashOrBranchOrTag",
          exception = e,
      )
    }
  }
  ...
  fun checkoutFiles(
    ...
    try {
      ...
      if (targetCommitHash == Cons.git_IndexCommitHash) { // checkout index files
        ...
        val targetTree =
            resolveTree(repo, targetCommitHash)
                ?: return Ret.createError(null, "resolve target tree failed")
        Checkout.tree(repo, targetTree, checkoutOptions)
      }
      ...
      MyLog.e(
          TAG,
          "#$funName err: targetCommitHash=$targetCommitHash, err=${e.stackTraceToString()}",
      )
  ...
  fun rebaseCurOfAllFormatted(
    ...
    try {
      ...
      // eg: "(2/6)"
      return prefix +
          (rebase.operationCurrent() + 1) +
          split +
          rebase.operationEntrycount() +
          suffix
    } catch (e: Exception) {
      return ""
    }
  ...
  fun isValidGitRepo(repoFullPath: String): Boolean {
    ...
    try {
      ...
      // should use Repository.open() check dir is or not a repo, if open success, is a repo, else
      // not.
      ...
      // the .git file include a relative path to ".git" folder(maybe is not named .git folder, but
      // meant same)
    ...
    //         under codes only is not reliable, if .git is a file, it will return false, but the
    // dir maybe is a git repo though
    //            if(!dir.isDirectory) {
    //                return false
    //            }
    //
  ...
  fun createDotGitFileIfNeed(
      parentRepoFullPathNoSlashSuffix: String,
      subRepoPathUnderParentNoSlashPrefixAndSuffix: String,
      force: Boolean,
  ) {
    //            .gitfile content should like: "gitdir: ../../.git/modules/"
    val subFullPath =
        parentRepoFullPathNoSlashSuffix + Cons.slash + subRepoPathUnderParentNoSlashPrefixAndSuffix
  ...
  /**
   * e.g. parent is "/abc/def", sub is "def", will return "../", if sub is not under parent, return
   * null, if sub and parent are same, return empty str
   */
  private fun getGoToParentRelativePathFromSub(parent: String, subFullPath: String): String? {
    ...
    val levelCount = subRelativePath.count { it == Cons.slashChar }
  ...
  fun addSubmodule(repo: Repository, remoteUrl: String, relativePathUnderParentRepo: String) {
    //            if true, will create .git file under submodule, else will create .git folder, pc
    // git default create .git file, so recommend set it to true
    val useGitlink = true
  ...
  fun getSubmoduleDtoList(
      repo: Repository,
      invalidUrlAlertText: String,
      predicate: (submoduleName: String) -> Boolean = { true },
  ): List<SubmoduleDto> {
    val parentWorkdirPathNoSlashSuffix = getRepoWorkdirNoEndsWithSlash(repo)
    //            val parentDotGitModuleFile = File(parentWorkdirPathNoSlashSuffix,
    // Cons.gitDotModules)
   ...
   */
  fun removeRepoFilesForGoodRepo(
      workDirPath: String,
      createEmptyWorkDirAfterRemove: Boolean = false,
  ) {
    // 删除.git文件夹
    ...
    kotlin.runCatching {
      ...
      val repoGitDirPath =
          Repository.open(workDirPath).use { repo ->
  ...
  /**
   * @param createEmptyWorkDirAfterRemove
   *   在删除仓库workdir后创建空目录，一般用来在删除submodule之后避免报错，非submodule使用默认值false即可
   */
  fun removeRepoFiles(workDirPath: String, createEmptyWorkDirAfterRemove: Boolean = false) {
    ...
    if (dotGitDir != null) {
      ...
      MyLog.d(
          TAG,
          "remove '.git' file err: '.git' file doesn't exist or doesn't include a valid path",
      )
    }

    // delete workdir (note: must delete this after delete .git folder, because this will delete
    // .git file, if order change, it will can't find .git folder, then .git folder will keep
  ...
  /**
   * remove submodule info from .gitmodules remove submodule info from .git/config delete
   * submodule's .git folder under .git/modules delete submodule folder on parent repo's workdir
   *
   * update parent's index ( this method don't do this, it control by user ) done
   */
  fun removeSubmodule(
      deleteFiles: Boolean,
    ...
    // del files on disk
    if (deleteFiles) {
      // if not delete the config, re-create the submodule folder for avoid show submdules deleted
      // in git status
    ...
    if (deleteConfigs) {
      ...
      if (parentConfig.isNotBlank()) {
        ...
        if (parentConfigFile.exists()) {
          MyLog.d(
              TAG,
              "will delete submodule key from parent repo config at: ${parentConfigFile.canonicalPath}",
          )
      ...
      if (gitmoduleFile.exists()) {
        MyLog.d(
            TAG,
            "will delete submodule key from submodule config at: ${gitmoduleFile.canonicalPath}",
        )
  ...
  /**
   * should make sure when expect NONE credential, set `specifiedCredential` to null; if want to
   * match by domain, set it to match by domain credential entity; if want to use specified
   * credential but NONE and match by domain both, set it to expect credential
   */
  suspend fun cloneSubmodules(
      repo: Repository,
      recursive: Boolean,
      depth: Int,
    ...
    submoduleNameList.forEachBetter forEach@{ name ->
      // sync .gitmodules info(e.g. remoteUrl) to parent repos .git/config and submodules
      // .git/config
      // sm.sync();  // update parent repo's .git/config and submodules .git/config, if use this,
      // need not do init again yet, but if do init again, nothing bad though
      // sm.init(true);  // only update .git/config,
      // 如果传参为false，将不会更新已存在条目，即使与.gitmodules里的信息不匹配，建议传true，强制更新为.gitmodules里的内容
      // if(true) return 0;
      ...
      // if repo already cloned, will not clone again, if want to re-clone, should delete then do
      // clone again
      ...
      try {
        ...
        if (specifiedCredential != null) {
          // only 2 cases possible in this block, credential is match by domain or a specified
          // credential
          if (
              SpecialCredential.MatchByDomain.credentialId == specifiedCredential.id
          ) { // match by domain, need query
            val credentialByDomain =
                credentialDb.getByIdWithDecryptAndMatchByDomain(specifiedCredential.id, smUrl)
            if (credentialByDomain != null) {
              callbacks.setCredAcquireCb(
                  getCredentialCb(getCredentialTypeByUrl(smUrl), credentialByDomain)
              )
            }
          } else { // specified credential, no query need
            callbacks.setCredAcquireCb(
                getCredentialCb(getCredentialTypeByUrl(smUrl), specifiedCredential)
            )
          }
        }
      } catch (e: Exception) {
        MyLog.e(
            TAG,
            "#cloneSubmodules: set credential for submodule '$name' err: ${e.localizedMessage}",
        )
      }
      ...
      val subRepo: Repository? =
          if (isCloned) {
            ...
            SubmoduleDotGitFileMan.restoreDotGitFileForSubmodule(
                repoFullPathNoSlashSuffix,
                submodulePath,
            )

            //                    createDotGitFileIfNeed(repoFullPathNoSlashSuffix,
            // submodulePath.removePrefix(Cons.slash).removeSuffix(Cons.slash), force = false)
      ...
      if (subRepo == null) {
        MyLog.e(
            TAG,
            "#cloneSubmodules: clone submodule '$name' for '${File(repoFullPathNoSlashSuffix).name}' err",
        )
        return@forEach
      }
      ...
      try {
        ...
        MyLog.e(
            TAG,
            "#cloneSubmodules: do addFinalize() for submodule '$name' err: ${e.localizedMessage}",
        )
      }
      ...
      if (recursive) {
        // does not support filter submodule's submodule, so predicate always return true when reach
        // here
        cloneSubmodules(
            subRepo,
            recursive,
  ...
  /**
   * only need do repoInit before clone a existed submodule after parent repo cloned, other case may
   * failed, and if add submodule by app, need not call this method, because after add, it will do
   * repoInit implicit.
   *
  ...
  fun submoduleRepoInit(parentRepoFullPath: String, sm: Submodule) {
    try {
      // true will create a ".git" file under submodule workdir, and .git folder will under parent
      // repo's ".git/modules" folder;
      // false will create .git folder under submodule workdir.
      // because pc git default is true, so here is true too.
      // btw: it will not affect submodules file, just difference way to saving submodules .git
      // folder.
      val useGitlink = true
      sm.repoInit(useGitlink)

      // if init repo success, backup the .git file, because it may delete by
      // libgit2...................
  ...
  // the param `superParentRepo` is the very top level repo for this submodule can be reached, it
  // will used
    ...
    submoduleNameList.forEachBetter { submoduleName ->
      ...
      try {
        ...
        try {
          ...
          MyLog.e(
              TAG,
              "#updateSubmodule: set credential for submodule '$submoduleName' err: ${e.localizedMessage}",
          )
        }
        ...
        //                    MyLog.e(TAG,"#updateSubmodule: update submodule '$submoduleName' err:
        // ${e.localizedMessage}")
      }
    }
  }
  ...
  /**
   * read path from .git file, content in .git file should like: "gitdir:
   * ../../.git/modules/your/submodule/path", it's relative path to it's ".git" folder
  ...
  fun getDotGitDirByWorkDirPath(workDirPath: String): File? {
    try {
      ...
      MyLog.d(
          TAG,
          "#getDotGitDirByWorkDirPath: dotGitFile.canonicalPath=${dotGitFile.canonicalPath}",
      )
      ...
      dotGitFile.bufferedReader().use {
        while (true) {
          ...
          if (line.startsWith(prefix)) {
            ...
            MyLog.d(
                TAG,
                "#getDotGitDirByWorkDirPath: got .git dir path from .git file: '$gitDirPath'",
            )
            break
          }
        }
      ...
      return gitDirPath.let {
        ...
        else if (it.startsWith('/')) File(it) else File(dotGitFile.canonicalFile.parent!!, it)
      }
    } catch (e: Exception) {
      return null
    }
  ...
  /**
   * delete top level item like [xxxx] or [xxx "abc"] and all it's sub items from git config style
   * file
   */
    ...
    try {
      ...
      gitConfig.bufferedWriter().use { writer ->
        if (newLines.isEmpty()) {
          ...
          newLines.forEachBetter { line -> writer.write("$line\n") }
        }
      }
    } catch (_: Exception) {}
  }
  ...
  /**
   * support most 3 levels, e.g. "submodule.abc.url" or "user.name" or "url", but is not perfect
   * handle in-formal style, example: if your config's has [abc "k1"], it will not handle correctly,
   * because between abc and "k1" has more than 1 spaces
   */
  ...
  private fun getValueFromGitConfigByKeyArr2or3Level(
      configFile: File,
      key1: String,
      key2: String,
  ): String {
    ...
    configFile.bufferedReader().use { br ->
      ...
      while (line != null) {
        ...
        if (matched) {
          ...
          if (
              idx > 0 && rightStartIdx < line.length
          ) { // idx == 0 or < 0 are invalid, because left of "=" must  has at least 1 char
  ...
  fun getStatusOfSubmodule(parent: Repository, smName: String): Set<Submodule.StatusT> {
    return try {
      ...
      MyLog.e(
          TAG,
          "#getStatusOfSubmodule: get status of submodule '$smName' err: ${e.localizedMessage}",
      )
      setOf()
    }
  }
  ...
  /**
   * @return if submodule has uncommitted changes in it's index or workdir, will return true, else
   *   return false
   */
  ...
  /**
   * @param isShowingCommitListForHEAD the commit history showing for HEAD or not, only true allow
   *   do squash
   */
  fun squashCommitsCheckBeforeShowDialog(
      repo: Repository,
      targetFullOidStr: String,
      isShowingCommitListForHEAD: Boolean,
  ): Ret<SquashData?> {
    try {
  ...
  fun squashCommits(
      ...
      username: String,
      email: String,
      currentBranchFullNameOrHEAD:
          String, // current branch name, or "HEAD"(when detached or first commit)
      settings: AppSettings,
  ): Ret<Oid?> {
    try {
        ...
        if (repo2ndQuery.workStatus == Cons.dbRepoWorkStatusNotReadyNeedClone) {
          ...
          if (branch.isNotBlank()) {
            ...
            if (dbIntToBool(repo2ndQuery.isSingleBranch)) {
              ...
              options.setRemoteCreateCb { repo, name, url ->
                // name = "origin", url= "xxxx.git"。 这的url其实就是用户设置的那个克隆url，而name其实也只有 origin
                // 一种可能(？不太确定，但应该是）
                MyLog.d(TAG, "in cloneOptions.setRemoteCreateCb: name=$name, url=$url")
                // 这里的 `name`，其实就是 "origin"，基本不会有其他可能
                val singleBranchRefSpec =
                    Libgit2Helper.getGitRemoteFetchRefSpec(remote = name, branch = branch)
                MyLog.d(
                    TAG,
                    "in cloneOptions.setRemoteCreateCb: singleBranchRefSpec=$singleBranchRefSpec",
                )
                // 测试能否通过repo获取当前仓库名，如果能，即使用户没填分支，也可启用singlebranch
                //                                        MyLog.d(TAG, "RemoteCreateCb: test get
                // current branch name:"+Libgit2Helper.resolveHEAD(repo)?.name())  //
                // 期望能获得分支名，结果是null，这时 repo.headUnborun()
                // 为true，head还没出生呢！不过也有其他办法实现不填branch也可以singlebranch那就是先成功克隆仓库，然后取出head指向的分支，然后再删除仓库再重新克隆，设置之前取出的分支为singlebranch的分支就行了，但没必要，不如让用户填一个，不填就别开single branch
                Remote.createWithFetchspec(repo, name, url, singleBranchRefSpec)
              }
            }
          }

          // TODO
          // 这个depth好使不好使，有待测试，如果加了depth就只能遍历到对应深度的commit的话，就没问题，否则可能需要进一步修改，暂时先不管了，写commit列表的时候再说，如果到时候加了depth的仓库能遍历出多于depth的记录数，就有问题，到时再回头改这里
          if (hasDepth) {
          ...
          */
          ...
          if (credentialId.isNotBlank()) {
            ...
            val credentialDb = AppModel.dbContainer.credentialRepository
            //                                val credentialFromDb =
            // credentialDb.getByIdWithDecrypt(credentialId)
            val credentialFromDb =
                credentialDb.getByIdWithDecryptAndMatchByDomain(
                    id = credentialId,
                    url = repo2ndQuery.cloneUrl,
                )
            if (credentialFromDb != null) {
              val credentialType = Libgit2Helper.getCredentialTypeByUrl(cloneUrl)
              // 设置验证凭据的回调
              Libgit2Helper.setCredentialCbForRemoteCallbacks(
                  callbacks,
                  credentialType,
                  credentialFromDb,
              )
              ...
              //
              // Libgit2Helper.setCredentialForFetchOptions(options.fetchOpts, credentialType,
              // usernameOrPrivateKey, passOrPassphrase)
              //                                    options.fetchOpts.callbacks.setCredAcquireCb {
              // url: String, usernameFromUrl: String, allowedTypes: Int ->
              //                                        if (credentialType ==
              // Cons.dbCredentialTypeHttp) {  //type http
              ...
              //                                            val usernameForSsh =
              //                                                usernameFromUrl.ifBlank {
              // Cons.gitWellKnownSshUserName }
              //                                            val passphraseOrNull =
              //                                                passOrPassphrase.ifBlank { null }
              // // pass?pass:null
              //
            ...
            //                                    options.fetchOpts.callbacks.setCertificateCheckCb{
            // cert:Cert,  valid:Boolean, host:String ->
          ...
          // warn: if don't check tje url type, actually this the certificate callback affect
          // https(tls) too,
          //  https no need set this, if want to allow unknown host for https(e.g. user used a
          // self-signed cert),
          //  can copy the cert into the user-cert folder, then can connect self-signed cert with
          // https
          ...
          try {
            ...
            Clone.cloneRepo(cloneUrl, savePath, options).use { clonedRepo ->
              ...
              if (dbIntToBool(repo2ndQuery.isRecursiveCloneOn)) {
                // 疑惑：这个东西能共享吗？
                val submoduleUpdateOptions = Submodule.UpdateOptions.createDefault()
                /*
                 * Allow fetching from the submodule's default remote if the target
                 * commit isn't found. Enabled by default.
                 */
                //                                        submoduleUpdateOptions.allowFetch = true
                // //默认就开了，不用再开
                ...
                Submodule.foreach(clonedRepo) { submodule, submoduleName ->
                  ...
                  ) // 克隆好像执行这个就行了，后续检查更新(fetch)的时候可能需要执行下面的update和sync方法
                  //                                                    submodule.sync()
                  //                                                    submodule.update(true,
                  // submoduleUpdateOptions)
                  //                                                    submodule.init(true)
                  // //true强制覆盖已存在  //这个好像不用执行，以后检查更新(fetch)应该也不需要执行这个
                  0
                }
              }
              ...
              //                                    if (branch.isBlank()) {
              // //用户没填branch的情况，不对，其实不管用户填没填branch，这里都该获取下实际的branch
              // 更新分支信息
              ...
              //                                    else {  //用户填了branch的情况
              //                                        //println("走这了：：："+
              // Cons.gitDefaultRemoteOriginStartStrPrefix+repo2ndQuery.branch)
              //                                        val branchRef = Reference.lookup(clonedRepo,
              // Cons.gitDefaultRemoteOriginStartStrPrefix+repo2ndQuery.branch)
              //                                        if (branchRef != null) {
              //                                            if (branchRef.type() ==
              // Reference.ReferenceType.SYMBOLIC) {
              //                                                fullBranchRefStr =
              // branchRef.symbolicTarget()  //branch name
              //                                            } else if (branchRef.type() ==
              ...
              //                                            SortT.TIME,
              ////                                            SortT.REVERSE
              // //如果加这个，就是从最旧的commit开始找了，也就是升序
              //                                        )
              //                                    )
              //                                    //获取当前分支的引用；如果当前分支为空，则可能是detached
              // HEAD，这时获取commitOid；否则获取HEAD兜底
              ////                                    if(repo2ndQuery.branch.isNotBlank() &&
              // fullBranchRefStr!=null) {
              ...
              //                                        .toString()  // commit hash is Oid(Git
              // Object ID)

              //                                    test_checkAndPrintDepth(clonedRepo)

              // set remoteName and remoteUrl fields
              //                                    val defaultRemoteName =
              // Remote.list(clonedRepo)[0]  // remote "origin"
              //                            val defaultRemoteName = Cons.gitDefaultRemoteOrigin
              // //"origin"就是默认的名字，根本不用执行上面的查询
              ...
              //                                    repo2ndQuery.upstreamBranch =
              // Libgit2Helper.getUpstreamRemoteBranchShortRefSpecByLocalBranchShortName(clonedRepo,
              // repo2ndQuery.branch)?:""
              // 方法2：这个方法没缺陷，只要配置文件有，即使没发布也能查出来
              repo2ndQuery.upstreamBranch =
                  Libgit2Helper.getUpstreamOfBranch(clonedRepo, repo2ndQuery.branch)
                      .remoteBranchShortRefSpec
            ...
            repo2ndQuery.createErrMsg = e.localizedMessage ?: unknownErrWhenCloning
            // 如果出错，删除仓库目录
            deleteIfFileOrDirExist(repoDir)
  ...
  fun createSignature(name: String, email: String, settings: AppSettings): Signature {
    ...
    // val offsetMinutes = readTimeZoneOffsetInMinutesFromSettingsOrDefault(settings,
  ...
  fun hasUncommittedChanges(repo: Repository): Boolean {
    ...
    // 在这个页面 调用
    // getWorktreeStatusTypeEntryList()并不需要repoId，这个只是用来设置到StatusEntrySaver的，但这里仅检测是否为空，用不到此值
    return hasConflictItemInRepo(repo) ||
        indexIsEmpty(repo).not() ||
        getWorktreeChangeList(repo, getWorkdirStatusList(repo), repoId = "").isNotEmpty()
  }

  suspend fun doActWithRepoLockIfPredicatePassed(
      curRepo: RepoEntity,
      predicate: (RepoEntity) -> Boolean,
      act: suspend () -> Unit,
  ) {
  ...
  suspend fun doActWithRepoLock(
      curRepo: RepoEntity,
      waitInMillSec: Long = 0,
      onLockFailed: (lock: Mutex) -> Unit = {},
      act: suspend () -> Unit,
  ) {
  ...
  fun getWorktreeChangeList(
    ...
    val (_, statusMap) =
        runBlocking {
          statusListToStatusMap(
              repo,
              rawStatusList,
              repoIdFromDb = repoId,
              Cons.gitDiffFromIndexToWorktree,
          )
        }
  ...
  fun getSimpleCommitDto(
      repo: Repository,
      commitHashOrRef: String,
      repoId: String,
      settings: AppSettings,
  ): Ret<CommitDto?> {
    return try {
      ...
      val commitDto =
          if (
              commitHashOrRef.let {
                it == Cons.git_AllZeroOidStr ||
                    it == Cons.git_LocalWorktreeCommitHash ||
                    it == Cons.git_IndexCommitHash
              }
          ) {
  ...
  fun getLeftRightCommitDto(
      repo: Repository,
      leftHashOrRef: String,
      rightHashOrRef: String,
      repoId: String,
      settings: AppSettings,
  ): Pair<CommitDto, CommitDto> {
    val leftRet =
        Libgit2Helper.getSimpleCommitDto(repo, commitHashOrRef = leftHashOrRef, repoId, settings)
    val left =
        if (leftRet.hasError())
            CommitDto(
                oidStr = leftHashOrRef,
                shortOidStr = Libgit2Helper.getShortOidStrByFull(leftHashOrRef),
            )
        else leftRet.data!!

    val rightRet =
        Libgit2Helper.getSimpleCommitDto(repo, commitHashOrRef = rightHashOrRef, repoId, settings)
    val right =
        if (rightRet.hasError())
            CommitDto(
                oidStr = rightHashOrRef,
                shortOidStr = Libgit2Helper.getShortOidStrByFull(rightHashOrRef),
            )
        else rightRet.data!!
  ...
  fun zipOneLineMsg(msg: String, limitedCharsCount: Int = 500): String {
    ...
    for (c in msg) {
      ...
      if ((c == '\n' || c == '\r' || c.isWhitespace()) && lastAppendChar == ' ') {
        continue
      }
  ...
  fun saveFileOfCommitToPath(
      repo: Repository,
      refOrHash: String,
      relativePath: String,
      genFilePath: (entry: Tree.Entry) -> String,
  ): SaveBlobRet {
    val tree =
        resolveTree(repo, refOrHash)
            ?: return SaveBlobRet(code = SaveBlobRetCode.ERR_RESOLVE_TREE_FAILED)
    val entry =
        getEntryOrNullByPathOrName(tree, relativePath, byName = false)
            ?: return SaveBlobRet(code = SaveBlobRetCode.ERR_RESOLVE_ENTRY_FAILED)
  ...
  fun saveEntryToPath(repo: Repository, entry: Tree.Entry, savePath: String): SaveBlobRet {
    val blob =
        Blob.lookup(repo, entry.id())
            ?: return SaveBlobRet(code = SaveBlobRetCode.ERR_RESOLVE_BLOB_FAILED)
  ...
  suspend fun forcePushLeaseCheckPassedOrThrow(
    ...
    // 解析本地引用的值
    val expectedCommitOidRet =
        Libgit2Helper.resolveCommitByHashOrRef(repo, forcePush_expectedRefspecForLease)

    if (expectedCommitOidRet.hasError()) {
      throw RuntimeException(
          "force push with lease canceled: resolve expected refspec failed, expected refspec is `$forcePush_expectedRefspecForLease`"
      )
    }
    ...
    val credential =
        Libgit2Helper.getRemoteCredential(
    ...
    // 查fetch后的数据
    val latestUpstreamOidStr =
        Libgit2Helper.resolveCommitOidByRef(repo, remoteBranchRefsRemotesFullRefSpec).toString()
    // upstream oid str is null-string-able, but expected oid string is not null or
    // null-string-able,
    //  so if both are equals, the upstream oid must not null able, hence here no more null-check
    // needed
    // 两个若相等，两者必然都非null
    ...
    MyLog.d(
        TAG,
        "#$funName: force push with lease: upstream.remoteBranchRefsRemotesFullRefSpec=${remoteBranchRefsRemotesFullRefSpec}, latestUpstreamOid=$latestUpstreamOidStr, expectedCommitOid=$expectedCommitOidStr, expectedCommitOid==latestUpstreamOid is `$expectedEqualsToLatest`",
    )

    if (!expectedEqualsToLatest) {
      throw RuntimeException(
          "force push canceled: upstream didn't match the expected refspec, upstream is `$latestUpstreamOidStr`, expected is `$expectedCommitOidStr`"
      )
    }
  }
