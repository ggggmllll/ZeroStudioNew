...
class SimilarCompareImpl : SimilarCompare {
    ...
    if (result == null || (result.matched.not() && degradeToCharMatchingIfMatchByWordFailed)) {
      ...
      val reverse =
          searchDirection == SearchDirection.REVERSE ||
              searchDirection == SearchDirection.REVERSE_FIRST

      val reverseMatchIfNeed =
          searchDirection == SearchDirection.REVERSE_FIRST ||
              searchDirection == SearchDirection.FORWARD_FIRST

      // 用On算法最坏的情况是正向匹配一次，然后逆向匹配一次，时间复杂度为 O(2n)，正常情况时间复杂度为O(n)，O(nm)若匹配两次，也会翻倍，最坏时间复杂度变成O(2nm)
      result =
          if (requireBetterMatching) {
      ...
      if (reverseMatchIfNeed && !result.matched) {
        result =
            if (requireBetterMatching) betterSearch.doSearch(addWillUse, delWillUse, !reverse)
            else search.doSearch(addWillUse, delWillUse, !reverse)
      }
    }
   ...
   * @param treatNoWordMatchAsNoMatched if true, will return no match when only spaces and
   *   punctuations matched ( haven't any non-space words matched )
   */
  private fun <T : CharSequence> doMatchByWords(
    ...
    var matched =
        doEqualsMatchWordsOrSpaces(addIter, delIter, addIndexResultList, delIndexResultList)
    ...
    val addSpaceIter =
        NoCopyIterator(
            srcList = addWordSpacePair.spaces as MutableList,
            isReversed = !isAppendContentMaybe,
        )
    val delSpaceIter =
        NoCopyIterator(
            srcList = delWordSpacePair.spaces as MutableList,
            isReversed = !isAppendContentMaybe,
        )

    // match spaces by equals
    matched =
        doEqualsMatchWordsOrSpaces(
            addSpaceIter,
            delSpaceIter,
            addIndexResultList,
            delIndexResultList,
    ...
    if (requireBetterMatching && (wordsNotAllMatched || spacesNotAllMatched)) {
      if (wordsNotAllMatched) {
        matched =
            doIndexOfMatchWordsOrSpaces(addIter, delIter, addIndexResultList, delIndexResultList) ||
                matched
      }

      if (spacesNotAllMatched) {
        matched =
            doIndexOfMatchWordsOrSpaces(
                addSpaceIter,
                delSpaceIter,
                addIndexResultList,
                delIndexResultList,
    ...
    // when reached here, all matched items already added in the `addIndexResultList` and
    // `delIndexResultList`,
    //  and not-matched items in the `addStillNotMatchedList` and `delStillNotMatchedList`
    //  and the order is not sorted yet

    // add not matched items, these items not matched by equals nor by indexOf, they are really
    // `modified` items
    ...
    // sortedWith return new List and keep origin list unchanged, sortWith sort in place, here no
    // need new list, so using sortWith
    // 根据索引从小到大，原地排序
  ...
  private fun <T : CharSequence> maybeIsAppendContent(
    ...
    if (add.getLen() < expectedAppendCount || del.getLen() < expectedAppendCount) {
      return false
    }
      ...
      while (delIter.hasNext()) {
        ...
        if (addStr.length > delStr.length) {
          ...
          if (indexOf != -1) {
            ...
            addIndexResultList.add(
                IndexStringPart(start = aStartIndex, end = aEndIndex, modified = false)
            )

            delIndexResultList.add(
                IndexStringPart(start = d.index, end = d.index + delStr.length, modified = false)
            )
          ...
          if (indexOf != -1) {
            ...
            addIndexResultList.add(
                IndexStringPart(start = a.index, end = a.index + addStr.length, modified = false)
            )

            val dStartIndex = d.index + indexOf
            val dEndIndex = dStartIndex + addStr.length
            delIndexResultList.add(
                IndexStringPart(start = dStartIndex, end = dEndIndex, modified = false)
            )
  ...
  private fun <T : CharSequence> getWordAndIndexList(
      compareParam: CompareParam<T>,
      requireBetterMatching: Boolean,
  ): WordsSpacesPair {
    // 拼接相邻的非单词字符（空格、标点符号）：
...
private data class WordsSpacesPair(val words: List<WordAndIndex>, val spaces: List<WordAndIndex>)
...
private val wordSeparators =
    listOf(
        ...
        // 20250413
        // 重新启用：这个可以当作按句子匹配，当按单词匹配打开的时候，对中文就按句子匹配，否则按字匹配，若不加中文标点符号，匹配中文时只能按单字匹配，虽然有时候匹配率还好，但有时候不如按句子匹配，所以启用这个，这样可多一种不同的匹配方式
        '，',
        '。',
        '、',
        '；',
