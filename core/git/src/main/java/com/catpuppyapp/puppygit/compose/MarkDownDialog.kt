...
fun MarkDownDialog(
  ...
  ConfirmDialog3(
      ...
      textCompose = {
        ScrollableColumn {
          // use code font for both, if want to view text with system font, can use details dialog
          // instead
          if (previewModeOn.value) {
            if (loadingForLetMarkDownContainerRefreshAfterSwitchFont.value) {
              ...
              Text(text = text, fontFamily = font)
            } else {
            ...
            MySelectionContainer { Text(text = rawOrHighlightedText(), fontFamily = font) }
          }
        }
      },
      customCancel = {
        ScrollableRow {
          IconButton(onClick = switchFonts) {
            Icon(
                imageVector =
                    if (useSystemFonts.value) Icons.Filled.FontDownloadOff
                    else Icons.Filled.FontDownload,
                contentDescription = "switch use system fonts or not",
            )
          }

          IconButton(onClick = switchBetweenRawAndRenderedContent) {
            Icon(
                imageVector =
                    if (previewModeOn.value) Icons.AutoMirrored.Filled.Notes
  ...
  LaunchedEffect(previewModeOn.value) {
    if (!previewModeOn.value && highlightedMarkdownText.value == null) {
      ...
      MarkDownSyntaxHighlighter(text) { highlightedMarkdownText.value = it }.analyze()
    }
  }
}
