  ...
  Column(
      ...
      modifier =
          Modifier.fillMaxWidth()
              ...
              .then(
                  // 如果条目被选中，切换高亮颜色
                  if (isItemInSelected(thisObj))
                      Modifier.background(
                          MaterialTheme.colorScheme.primaryContainer

                          // then 里传
                          // Modifier不会有任何副作用，还是当前的Modifier(即调用者自己：this)，相当于什么都没改，后面可继续链式调用其他方法
                      )
    ...
    Row(
      ...
      InLineIcon(icon = Icons.AutoMirrored.Filled.Label, tooltipText = stringResource(R.string.tag))

      //            Text(text = stringResource(R.string.name) +": ")

      ScrollableRow {
        Text(
            text = thisObj.shortName,
            maxLines = 1,
    ...
    Row(
      ...
      InLineIcon(icon = Icons.Filled.Commit, tooltipText = stringResource(R.string.target))
    ...
    if (pointedCommit != null) {
      Row(
        ...
        InLineIcon(icon = Icons.Outlined.Person, tooltipText = stringResource(R.string.author))

        ScrollableRow {
          Text(
              text = pointedCommit.getFormattedAuthorInfo(),
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
      ...
      Row(
        ...
        InLineIcon(icon = Icons.Outlined.CalendarMonth, tooltipText = stringResource(R.string.date))

        ScrollableRow {
          Text(
              text = pointedCommit.dateTime,
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
    ...
    Row(
      ...
      InLineIcon(icon = Icons.Filled.Category, tooltipText = stringResource(R.string.type))

      //            Text(text = stringResource(R.string.type) +": ")

      ScrollableRow {
        Text(
            text = thisObj.getType(activityContext, false),
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = defaultFontWeight,
        )
    ...
    if (thisObj.isAnnotated) {
      Row(
        ...
        InLineIcon(icon = Icons.Filled.Person, tooltipText = stringResource(R.string.tagger))

        //                Text(text = stringResource(R.string.tagger) +": ")

        ScrollableRow {
          Text(
              text = thisObj.getFormattedTaggerNameAndEmail(),
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
              ...
              text =
                  if (shouldShowTimeZoneInfo)
                      TimeZoneUtil.appendUtcTimeZoneText(
                          thisObj.getFormattedDate(),
                          thisObj.originTimeOffsetInMinutes,
                      )
                  else thisObj.getFormattedDate(),
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
      ...
      Row(
        ...
        SingleLineClickableText(text = thisObj.getCachedOneLineMsg()) { showItemMsg(thisObj.msg) }
      }
    }
  }
}
