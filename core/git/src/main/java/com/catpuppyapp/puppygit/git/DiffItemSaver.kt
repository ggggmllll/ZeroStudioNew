...
// 【larger value bad performance but accurate result, smaller value has good performance but
// in-accurate result】
...
object PuppyLineOriginType {
  ...
  fun isEofLine(line: PuppyLine) =
      line.originType.let { it == CONTEXT_EOFNL || it == ADD_EOFNL || it == DEL_EOFNL }
}
...
data class DiffItemSaver(
    ...
    internal val languageScope: Box<PLScope> =
        if (SettingsUtil.isDiffSyntaxHighlightEnabled()) Box(PLScope.AUTO) else Box(PLScope.NONE),
) {
  fun getAndUpdateScopeIfIsAuto(
      fileNameForGuessLangScope: String,
      scope: PLScope = languageScope.value,
  ): PLScope {
    val scope =
        if (scope == PLScope.AUTO) {
  ...
  // true means changed and scope valid, false means no change, null means changed but newScope is
  // invalid
  ...
  fun fileName() =
      cachedFileName
          ?: getFileNameFromCanonicalPath(relativePathUnderRepo).let {
            cachedFileName = it
            it
          }
  ...
  fun syntaxDisabledOrNoMoreMem(noMoreMemToaster: OneTimeToast): Boolean {
    ...
    if (
        ...
        //            ||
        noMoreHeapMemThenDoAct { noMoreMemToaster.show(StrCons.syntaxHightDisabledDueToNoMoreMem) }
    ) {
      operateStylesMapWithWriteLock { it.clear() }
      return true
    }
  ...
  fun generateFakeIndexForGroupedLines() {
    for (h in hunks) {
      ...
      for ((_, lines) in h.groupedLines) {
        // 顺序是context/del/add
        lines.get(Diff.Line.OriginType.CONTEXT.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
        lines.get(Diff.Line.OriginType.CONTEXT_EOFNL.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
        lines.get(Diff.Line.OriginType.DELETION.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
        lines.get(Diff.Line.OriginType.DEL_EOFNL.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
        lines.get(Diff.Line.OriginType.ADDITION.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
        lines.get(Diff.Line.OriginType.ADD_EOFNL.toString())?.let {
          it.fakeIndexOfGroupedLine = ++index
        }
      }
    }
...
class PuppyHunkAndLines(val diffItemSaver: DiffItemSaver) {
  ...
  // {linNum: Unit}, if map.get(lineNum) != null, means already showed add or del line as context,
  // need not show one more
  ...
  class MergeAddDelLineResult(
      ...
      // if already showed this line as context, next time same line will not showed again, eg: if
      // +12 and -12 only diff at end of line has "\n" or not, then only show context line 12 once
      // instead by field `data`
      //        val alreadyShowedAsContext:Boolean,

      // 是否需要显示此行为context
      // need show this line as context or not, if not, will show origin line, else set line origin
      // type to context, then show it
  ...
  @Deprecated(
      "this method has better performance but bad matching, recommend use `linkCompareTargetForLine` to instead of"
  )
  fun linkCompareTargetForLineByContextOffset(puppyLine: PuppyLine, changeType: String) {
    ...
    //  so, only added line need handle, when find the compare target (related deleted line), update
    // the deleted line as well
    // 删除行和添加行都不需要找比较目标，仅添加行需要找，找到后把对应的删除行也关联上
    if (
        changeType == Cons.gitStatusModified && puppyLine.originType == PuppyLineOriginType.ADDITION
    ) {
      var foundDel = false
      var size = lines.size
      while (--size >= 0) {
        ...
        if (ppLine.originType == PuppyLineOriginType.CONTEXT) {
          if (foundDel) {
            ...
            val guessedLine =
                groupedLines.get(guessedRelatedLineNum)?.get(PuppyLineOriginType.DELETION)
          ...
          // if found context, it's finished, this line if has a matched deleted line, already
          // handle at upside, else, none matched with it
          break
  ...
  fun linkCompareTargetForLine(puppyLine: PuppyLine, changeType: String) {
    ...
    if (
        changeType == Cons.gitStatusModified &&
            deletedLinesCount > 0 &&
            puppyLine.originType == PuppyLineOriginType.ADDITION
    ) {
      ...
      for (line in lines) {
        ...
        // if remove `line.roughlyMatchedCount < targetRoughlyMatchedCount` can be better for
        // matching(both strings have more matched chars), but bad for performance, better don't
        // remove this condition, if want to better matching, you can increase
        // `targetRoughlyMatchedCount`
        // 如果移除 `line.roughlyMatchedCount < targetRoughlyMatchedCount`
        // ，可能会提高匹配率，让相同字符更多的两个字符串互相关联，但会降低性能，最好不要移除此判断条件，而是通过提高 `targetRoughlyMatchedCount` 来增加匹配率
        if (
            line.originType == PuppyLineOriginType.DELETION &&
                line.roughlyMatchedCount < targetRoughlyMatchedCount
        ) {
          val roughMatchCnt =
              CmpUtil.roughlyMatch(
                  puppyLine.getContentNoLineBreak(),
                  line.getContentNoLineBreak(),
                  targetRoughlyMatchedCount,
              )
          // these two strings matched more chars than the old two lines,
          //  so, unlink old lines and link new lines
          if (
              (roughMatchCnt > maxRoughMatchCnt && roughMatchCnt > line.roughlyMatchedCount)

              // current line never linked any other lines, link it first, then will unlink if have
              // better matched target
            ...
            // if enable this break, can be improve performance, better keep this and adjust
            // `targetRoughlyMatchedCount` to control better matching or better performance
   ...
   *
   * TODO: 目前仅发现过相同行号的两行末尾换行符不同，如果以后发现有不同行号的行，则需要改用 `keyAndLineMap` 取代 `groupedLine`
   *   做判断，`mergedAddDelLine`也需要改成依赖line key而不是行号的，可以如果发现匹配，则把两个line key都添加上
   * TODO: only found same line num with line break not match, if found difference line number with
   *   content all matched but line break, need use `keyAndLineMap` replaced `groupedLine` to check,
   *   also need change the `mergedAddDelLine`, if matched, add two key of liens into it
   */
    ...
    if (
      ...
      return MergeAddDelLineResult(
          ...
          line =
              ...
              else del.copy(originType = Diff.Line.OriginType.CONTEXT.toString()),
      )
    } else {
  ...
  //        return add!=null && del!=null &&
  // add.getContentNoLineBreak().equals(del.getContentNoLineBreak())
  //    }

  fun getModifyResult(
      line: PuppyLine,
      requireBetterMatchingForCompare: Boolean,
    ...
    val modifyResult2 =
        CmpUtil.compare(
            add =
                StringCompareParam(add.getContentNoLineBreak(), add.getContentNoLineBreak().length),
            del =
                StringCompareParam(del.getContentNoLineBreak(), del.getContentNoLineBreak().length),
  ...
  fun clearStyles() {
    diffItemSaver.operateStylesMapWithWriteLock { styleMap ->
      lines.forEachBetter { styleMap.remove(it.key) }
    }
  }
}
...
data class PuppyLine(
    ...
    // the default compare target, it was match the compare target with line num, but sometimes line
    // number same content totally non-related, in that case need calculate the offset the pair them
    ...
    //    private val lines:MutableList<PuppyLine> = mutableListOf();
    // //针对content处理，把content的每个行都存成了一个PuppyLine（content默认只有多个连续行的第一个行有行号
    //
    //    /**
    ...
    // 会返回两行，但有时候这和函数又不会返回多余的行，我凌乱了，目前先这样吧
  ...
  companion object {
    fun mergeStringAndStylePartList(
        stringPartList: List<IndexStringPart>,
        stylePartList: List<LineStylePart>,
        modifiedBgColorSpanStyle: SpanStyle,
    ): List<LineStylePart> {
        ...
        while (iterator.hasNext()) {
          ...
          retStylePartList.add(
              LineStylePart(
                  ...
                  style =
                      if (stringPart.modified) stylePart.style.merge(modifiedBgColorSpanStyle)
                      else stylePart.style,
              )
          )
          ...
          if (reachedEnd) {
            if (start < stringPart.end) {
              stringPartMutableList.add(
                  0,
                  IndexStringPart(start, stringPart.end, stringPart.modified),
              )
            }
            break
          }
      ...
      if (retStyleLastEndIndex < stringPartLastEndIndex) {
        retStylePartList.add(
            LineStylePart(
                ...
                style =
                    if (lastStringPart.modified) modifiedBgColorSpanStyle
                    else MyStyleKt.emptySpanStyle,
            )
        )
      }
  ...
  fun getContentNoLineBreak(): String { // not safe for concurrency
    // replace eof to empty string to match UI behavior(display empty line instead of eof text like
    // "\No New Line End Of File")
    return contentNoBreak
        ?: (if (!isEOF()) content.removeSuffix("\n").removeSuffix("\r") else "").let {
          contentNoBreak = it
          it
        }
  }
