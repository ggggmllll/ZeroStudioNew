...
object Cons {
  ...
  //        @Deprecated("改用：StorageDirCons.DefaultStorageDir.allRepoParentsDir.name")
  // //20240527：禁用，sd相关
  const val defaultAllRepoParentDirName = "PuppyGitRepos"
  //        @Deprecated("改用：StorageDirCons.DefaultStorageDir.puppyGitDataDir.name")
  // //20240527：禁用，sd相关
  const val defalutPuppyGitDataUnderAllReposDirName = "PuppyGit-Data"

  const val defaultFileSnapshotDirName = "FileSnapshot"
  const val defaultEditCacheDirName =
      "EditCache" // 编辑缓存目录，存储编辑文件时的content缓存文件的目录，用于在app突然崩溃，或者手机突然没电，但没保存手动保存文件时恢复内容，实现机制就是某行内容一update就自动保存
  const val defaultLogDirName = "Log"
  const val defaultSubmoduleDotGitFileBakDirName =
      "SmGitBak" // backup submodule's .git file, Sm=Submodule Git=git file
  ...
  //        const val defaultInternalStorageDirsParentDirName = "InternalStorageDir"
  // //20240527：禁用，sd相关
  ...
  val dateTimeFormatter_yyyyMMddHHmm: DateTimeFormatter =
  ...
  // special commit hash, 'LOCAL' and 'INDEX' reserved by PuppyGit , 'HEAD' reserved by Git, don't
  // name your branch/tag to these
  ...
  const val selectedItem_Never =
      -1 // 这个变量永远不会是这个值，这个值只是为了在页面里配合关联的状态变量实现永真判断的，目前20240411用在切换页面更新配置文件中记住的页面值上
  ...
  // 仅遵循旧代码规范而加的这个变量，实际上点这个直接退出，不会切换页面，也不会记住最后退出页面
  ...
  const val dbRepoWorkStatusNoHEAD =
      10 // 仓库有效，但没创建任何提交，这时需要创建文件并提交以使仓库正常工作，创建了仓库但没创建任何文件任何提交任何分支，就会是这个状态
  ...
  const val dbRemote_Fetch_BranchMode_SingleBranch =
      1 // 20240510: 这个实际上弃用了，因为CustomBranches如果仅指定1个分支，其实就是single branch了
  ...
  //        const val dbRepoShallowStatusSetDepthButBigThanCommitCountsSoShallowOff=3
  // //设置了depth，但depth比实际的提交数大，所以并没有shallow
  // clone，这种情况需要在设置了depth的仓库在克隆完后检查仓库.git目录是否存在shallow文件，如果存在则是此情况，否则是普通的shallow仓库，即dbRepoShallowStatusIsShallow状态
  ...
  const val dbSettingsUsedForCommonGitConfig =
      6 // 公用的gitconfig文件，相当于pc上用户目录下的.gitconfig，也可以说是全局gitconfig
  ...
  const val gitDefaultRemoteOriginStartStrPrefix =
      gitDefaultRemoteStartStrPrefix + gitDefaultRemoteOrigin + "/" // "refs/remotes/origin/"
  ...
  const val gitDefaultFetchRefSpecBranchReplacer =
      "+refs/heads/$gitBranchPlaceholder:refs/remotes/$gitDefaultRemoteOrigin/$gitBranchPlaceholder"
  // 可替换remote和branch为你期望的值
  const val gitFetchRefSpecRemoteAndBranchReplacer =
      "+refs/heads/$gitBranchPlaceholder:refs/remotes/$gitRemotePlaceholder/$gitBranchPlaceholder"
  ...
  val gitSha1HashRegex =
      "^[0-9a-f]{${gitShortCommitHashRange.endInclusive},$maxGitSha1HashStrLength}$".toRegex()
  ...
  // git status
  const val gitStatusUnmodified =
      "Unmodified" // 未修改，未加载修改内容，或两个对象完全一样时，就会是这样，只能在Diff页面看到这个状态，因为cl页面不会列出未修改的文件
  ...
  val gitItemTypeDirStr = "Folder"
  // use 'Folder' instead 'Dir' for distinguish with 'Dirty', else if input 'dir' as filter text,
  // will show dirty sub and dirs
  val gitItemTypeSubmoduleStr = "Subm"

  val gitSubmoduleDirtyStr =
      "Dirty" // for item type submodule, when dirty, show this at ChangeList item
  ...
  val gitDiffFromTreeToTree = "4"
  // 例如：一个提交和另一个提交比较，就属于这个类型
  ...
  val isReadyDoSyncCheckResult_ReadyDoPushAndRemoteRefSpecNonExists =
      3 // pc git 需要带 -u 选项的情况，远程仓库不存在本地分支关联的上游分支

  // 最大支持的大小需要测试下
  //        val editorFileSizeMaxLimit = 2000000L  // 2MB，在20240421之前是1mb，后来优化了保存机制，改成2mb了, instead
  // by settings
  //        val editorFileSizeMaxLimitForHumanReadable = "2MB"  // 文件大小限制人类可读的描述方式

  // diffItem
  // 的所有line.content总和，如果超过这个大小，就不显示了，有时候存在一个文件大小很大，但其diffcontent很少的情况，这时候其实获取diff内容并不慢，所以单独限制一个diff
  // content大小
  ...
  /**
   * 等同于: checkout reference then detach HEAD，适用: tag/branch等引用类型 old name: checkoutTypeRemoteBranch
   */
  val checkoutType_checkoutRefThenDetachHead = 2

  /**
   * 等同于：checkout commit then detach
   * HEAD，实际上适用任何类型，只要peel出commit即可，但有缺陷，不知道checkout的是哪个分支或tag之类的，只知道分支号，所以如果知道引用还是尽量用引用好 old name:
   * checkoutTypeCommit
   */
