  ...
  Row(
    ...
    ListItemRow {
      ...
      ListItemToggleButton(
          ...
          // //diff提交时，禁用点击图标启动长按模式，按钮会变灰色，太难看了，弃用，改成点击后判断是否需要执行操作了，若不需要直接返回
        ...
        IconOfItem(
            ...
            defaultIconWhenLoadFailed =
                if (item.changeType == Cons.gitStatusDeleted)
                    ImageVector.vectorResource(R.drawable.outline_unknown_document_24)
                else if (itemIsDir) Icons.Filled.Folder else null,
            contentDescription =
                if (item.changeType == Cons.gitStatusDeleted) null
                else if (itemIsDir) stringResource(R.string.folder_icon)
      ...
      Column {
        ...
        Text(
            text = item.getChangeListItemSecondLineText(isDiffToLocal),
    ...
    val dropDownMenuExpandState = rememberSaveable {
      mutableStateOf(false)
    } // typo: "Expend" should be "Expand"
