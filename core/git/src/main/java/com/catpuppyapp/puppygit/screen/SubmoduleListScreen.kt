...
fun SubmoduleListScreen(
  ...
  val initCreateDialog = { showCreateDialog.value = true }
    ...
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            ...
            TextField(
                ...
                singleLine = true,
                onValueChange = { pathForCreate.value = it },
            ...
            TextField(
                ...
                singleLine = true,
                onValueChange = { remoteUrlForCreate.value = it },
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.creating)) {
        try {
          ...
          MyLog.e(
              TAG,
              "#CreateDialog err: path=${pathForCreate.value}, url=${remoteUrlForCreate.value}, err=${e.stackTraceToString()}",
          )
        } finally {
  ...
  val switchItemSelected = { item: SubmoduleDto ->
    ...
    UIHelper.selectIfNotInSelectedListElseRemove(
        ...
        contains = containsForSelectedItems,
    )
    // 开启选择模式
  ...
  val getDetail = { item: SubmoduleDto ->
    ...
    sb.appendLine(activityContext.getString(R.string.name) + ": " + item.name)
        .appendLine()
        .appendLine(activityContext.getString(R.string.url) + ": " + item.remoteUrl)
        .appendLine()
        .appendLine(activityContext.getString(R.string.target) + ": " + item.targetHash)
        .appendLine()
        .appendLine(activityContext.getString(R.string.location) + ": " + item.location.toString())
        .appendLine()
        .appendLine(activityContext.getString(R.string.path) + ": " + item.relativePathUnderParent)
        .appendLine()
        .appendLine(activityContext.getString(R.string.full_path) + ": " + item.fullPath)
        .appendLine()
        .appendLine(
            activityContext.getString(R.string.status) + ": " + item.getStatus(activityContext)
        )
        .appendLine()
  ...
  if (showSetUrlDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.set_url),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            TextField(
                ...
                singleLine = true,
                onValueChange = { urlForSetUrlDialog.value = it },
                label = { Text(stringResource(R.string.url)) },
            )
          }
        },
          ...
          Repository.open(curRepo.value.fullSavePath).use { repo ->
            ...
            if (sm == null) {
              Msg.requireShowLongDuration(
                  activityContext.getString(R.string.resolve_submodule_failed)
              )
              return@act
            }
  ...
  if (showSyncConfigDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.sync_configs),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            ...
            MyCheckBox(
                text = stringResource(R.string.submodule_config),
                value = syncSubmoduleConfig,
            )
          }
        },
  ...
  if (showInitRepoDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.init_repo),
        requireShowTextCompose = true,
        textCompose = {
          CopyScrollableColumn {
            Text(
                stringResource(R.string.will_do_init_repo_for_selected_submodules),
  ...
  if (showRestoreDotGitFileDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          CopyScrollableColumn {
            Text(
                stringResource(R.string.will_try_restore_git_file_for_selected_submodules),
                fontWeight = FontWeight.Light,
            )
            Spacer(Modifier.height(10.dp))
  ...
  if (showReloadDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            MySelectionContainer { Text(stringResource(R.string.reload_submodule_note)) }
            Spacer(Modifier.height(15.dp))
    ...
    ResetDialog(
        ...
        onOk = { resetType ->
          ...
          doJobThenOffLoading(
              loadingOn,
              loadingOff,
              activityContext.getString(R.string.resetting),
          ) {
            try {
              selectedItemList.value.toList().forEachBetter {
  ...
  if (showImportToReposDialog.value) {
    ConfirmDialog2(
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.importing)) {
        ...
        val importList =
            selectedItemList.value.toList() // just import all selected, will fail if must fail
        ...
        try {
          ...
          MyLog.e(
              TAG,
              "import repos from SubmoduleListPage err: importRepoResult=$importRepoResult, err=" +
  ...
  if (showDeleteDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.delete),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            MyCheckBox(
                text = stringResource(R.string.del_config),
                value = deleteConfigForDeleteDialog,
            )
            ...
            MyCheckBox(
                text = stringResource(R.string.del_files),
                value = deleteFilesForDeleteDialog,
            )
          ...
          Repository.open(curRepo.value.fullSavePath).use { repo ->
            ...
            selectedItemList.value.toList().forEachBetter { smdto ->
              try {
                ...
                val errPrefix =
                    "del submodule err: delConfig=${deleteConfigForDeleteDialog.value}, delFiles=${deleteFilesForDeleteDialog.value}, err="
                val errMsg = e.localizedMessage
                ...
                MyLog.e(
                    TAG,
                    "#DeleteDialog err: delConfig=${deleteConfigForDeleteDialog.value}, delFiles=${deleteFilesForDeleteDialog.value}, err=${e.stackTraceToString()}",
                )
              }
            }
          }
  ...
  if (showCloneDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.clone),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            //
            ...
            if (recursiveClone.value) {
              MySelectionContainer {
                DefaultPaddingText(
                    stringResource(R.string.recursive_clone_submodule_nested_loop_warn),
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.cloning)) {
        try {
          ...
          val credential =
              if (SpecialCredential.MatchByDomain.credentialId == selectedCredential.id)
                  selectedCredential.copy()
              else credentialDb.getByIdWithDecrypt(selectedCredential.id)

          //                val defaultInvalidIdx = -1

          // not cloned, and not do other job for submodule
          //                    val willCloneList = selectedList.filter{!it.cloned &&
          // it.tempStatus.isBlank()}

          //                val nameIndexMap = mutableMapOf<String, Int>()
          // set status to cloning for selected items
          //                willCloneList.forEach { selectedItem ->
          //                    val curItemIdx = allItems.indexOfFirst { selectedItem.name ==
          // it.name }
          ...
          //                        try {
          //                            val selectedIdx = selectedItemList.value.indexOfFirst {
          // it.name == selectedItem.name }
          //                            if(selectedIdx!=-1) {  //current item selected, update it's
          // info in selectedItemList
            ...
            willCloneList.forEachBetter { selectedItem ->
              try {
                ...
                Libgit2Helper.cloneSubmodules(
                    ...
                    recursive,
                    depth,
                    specifiedCredential = credential,
                    submoduleNameList = listOf(selectedItem.name),
              ...
              //                        val curItemIdx =
              // nameIndexMap.getOrDefault(selectedItem.name, defaultInvalidIdx)
              //                        doActIfIndexGood(curItemIdx, allItems) { itemWillUpdate ->
              //                            val newItem = itemWillUpdate.copy(tempStatus =
              // resultMsg, cloned = Libgit2Helper.isValidGitRepo(itemWillUpdate.fullPath))
              //                            allItems[curItemIdx] = newItem
              //
              //                            try {
              //                                // make selectedList info more real-time, but may
              // cause ConcurrentException, most time should be fine
              //                                val selectedIdx =
              // selectedItemList.value.indexOfFirst { it.name == selectedItem.name }
              //                                if(selectedIdx!=-1) {  //current item selected,
              // update it's info in selectedItemList
              //                                    selectedItemList.value[selectedIdx] = newItem
              //                                }
              //                            }catch (_:Exception) {
              //
  ...
  if (showUpdateDialog.value) {
    ConfirmDialog2(
        title = activityContext.getString(R.string.update),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            CredentialSelector(credentialList.value, selectedCredentialIdx)
            ...
            if (recursiveUpdate.value) {
              MySelectionContainer {
                DefaultPaddingText(
                    stringResource(R.string.recursive_update_submodule_nested_loop_warn),
                    color = MyStyleKt.TextColor.danger(),
                )
              }
            }
            ...
            selectedItemList.forEachBetter {
              try {
                ...
                createAndInsertError(
                    curRepo.id,
                    "update submodule '${it.name}' err: ${e.localizedMessage}",
                )
      ...
      if (list.value.isEmpty()) { // 无条目，显示可创建或fetch
        FullScreenScrollableColumn(contentPadding) {
          if (isInitLoading.value) {
            ...
            Row { Text(text = stringResource(R.string.no_submodules_found)) }
            ...
            filterTheList(
                ...
                match = { idx: Int, it: SubmoduleDto ->
                  it.name.contains(keyword, ignoreCase = true) ||
                      ...
                      //                            || it.fullPath.contains(keyword, ignoreCase =
                      // true)  //完整路径肯定有相同前缀，仅相对路径（it.name）不同，所以仅过滤name即可，不需要过滤完整路径
        ...
        if (multiSelectionMode.value) {
          ...
          val iconOnClickList: List<() -> Unit> =
              listOf( // index页面的底栏选项
                  delete@{ initDelDialog() },
                  update@{ initUpdateDialog() },
                  clone@{ initCloneDialog() },
                  selectAll@{
                    //                        val list = if(enableFilterState.value)
                    // filterList.value else list.value

                    list.forEachBetter { selectItem(it) }
          ...
          val moreItemEnableList: List<() -> Boolean> =
              (listOf(
                  ...
                  }, // sync config （同步.gitmodules 里的内容到父仓库或子仓库的配置文件）
          ...
          val moreItemOnClickList: List<() -> Unit> =
              (listOf(
                    ...
                    try {
                      if (selectedItemList.value.isNotEmpty()) {
                        clipboardManager.setText(
                            AnnotatedString(selectedItemList.value[0].fullPath)
                        )
                        Msg.requireShow(activityContext.getString(R.string.copied))
                      } else {
                        Msg.requireShowLongDuration(
                            activityContext.getString(R.string.no_item_selected)
                        )
                      }
                    } catch (e: Exception) {
                  ...
                  importToRepos@{ showImportToReposDialog.value = true },
                  resetToTarget@{ showResetToTargetDialog.value = true },
                    ...
                    try {
                      Repository.open(item.fullPath).use { subRepo ->
                        ...
                        if (File(configPath).exists().not()) {
                          Msg.requireShowLongDuration(
                              activityContext.getString(R.string.file_doesnt_exist)
                          )
                          return@editConfig
                        }
  ...
  LaunchedEffect(needRefresh.value) {
    try {
      ...
      val pageChanged = { refreshId != needRefresh.value }
        ...
        if (!repoId.isNullOrBlank()) {
          ...
          if (repoFromDb != null) {
            ...
            Repository.open(repoFromDb.fullSavePath).use { repo ->
              val items =
                  Libgit2Helper.getSubmoduleDtoList(
                      repo,
                      invalidUrlAlertText =
                          activityContext.getString(R.string.submodule_invalid_url_err),
                  )
          ...
          val credentialListFromDb =
              credentialDb.getAll(includeNone = true, includeMatchByDomain = true)
