...
class StatusTypeEntrySaver() : ItemKey {
  ...
  // 文件在仓库下的相对路径的父路径，例如 abc/123.txt，文件名是123.txt，父路径是abc/
  ...
  fun getMime(): MimeType {
    if (mime == null) {
      ...
      mime =
          MimeType.guessFromFileName(
              fileName
          ) // guessFromPath() 默认匹配不上就会返回 MimeType.TEXT_PLAIN ，所以这里不用做空值判断，也不用加空值则返回text plain的逻辑
    }
    return mime!!
  }
  ...
  fun getChangeTypeAndSuffix(isDiffToLocal: Boolean): String {
    if (changeTypeAndSuffix == null) {
      ...
      changeTypeAndSuffix =
          ((item.changeType ?: "") +
              (if (item.itemType == Cons.gitItemTypeSubmodule)
                  ", ${Cons.gitItemTypeSubmoduleStr+(if(isDiffToLocal && item.dirty) ", ${Cons.gitSubmoduleDirtyStr}" else "")}"
              else ""))
    }
  ...
  fun getItemTypeString(): String {
    if (itemTypeString == null) {
      itemTypeString =
          if (itemType == Cons.gitItemTypeDir) {
  ...
  fun maybeIsFileAndExist(): Boolean {
    return itemType == Cons.gitItemTypeFile ||
        (itemType != Cons.gitItemTypeDir && itemType != Cons.gitItemTypeSubmodule)
  }

  fun maybeIsDirAndExist(): Boolean {
    return itemType == Cons.gitItemTypeDir ||
        itemType == Cons.gitItemTypeSubmodule ||
