...
class MainActivity : BaseComposeActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContent { InitContent(applicationContext) { MainCompose() } }
  }
  ...
  override fun onResume() {
    ...
    // compose 可通过对应的get方法获取到 Activity 的生命周期事件
    // 仅当 pause后才设置on resume以忽略on create时第一个 on resume事件从而避免 compose(例如EditorInnerPage)的
    // LaunchedEffect和生命周期函数被重复调用
    // 会重复调用是因为Activity on create时会创建compose，然后就会触发LaunchedEffect，同时还会触发compose生命周期on
    // resume，而切换到后台再切换回来时，
    // 只会触发compose on resume，所以，忽略创建Activity时的第一个on resume就能避免重复触发了，换句话说，得先切到后台后再触发compose on
    // resume才执行就没问题了
...
private fun MainCompose() {
  ...
  val loadingText = rememberSaveable {
    mutableStateOf(activityContext.getString(R.string.launching))
  }
  ...
  val sshAskUserUnknownHostRequestList =
      mutableCustomStateListOf(
          stateKeyTag,
          "sshAskUserUnknownHostRequestList",
          listOf<SshAskUserUnknownHostRequest>(),
      )
  val currentSshAskUserUnknownHostRequest =
      mutableCustomStateOf<SshAskUserUnknownHostRequest?>(
          stateKeyTag,
          "currentSshAskUserUnknownHostRequest",
          null,
      )

  val iTrustTheHost = rememberSaveable { mutableStateOf(false) }
  val showSshDialog = rememberSaveable { mutableStateOf(false) }
  val closeSshDialog = { showSshDialog.value = false }
  ...
  if (showRandomLoadingTextDialog.value) {
    CopyableDialog2(
        ...
        //            onDismiss = {},  //x 算了，这个弹窗不重要，应该设置的关闭比保持开启更容易才合理)
        // 避免如果有人想点文字，狂点屏幕，然后显示弹窗，但点到外部，弹窗消失，所以把这个dismiss函数设为空
    ...
    try {
      ...
      doJobThenOffLoading {
        ...
        if (requireMasterPassword.value.not()) {
          // 更新下loading文案
          //                    loadingText.value =
          // activityContext.getString(R.string.checking_creds_migration)

          AppModel.dbContainer.credentialRepository.migrateEncryptVerIfNeed(
