...
object FileOpenHistoryMan {
   ...
   * @param requireClearOldSettingsEditedHistory if true, will clear settings remembered file edited
   *   position, caller should check before pass this value to avoid unnecessary clear
   */
  ...
  private fun readHistoryFromFile() {
    ...
    try {
      ...
      //            saveHistory(curHistory)  // when read from file failed, save a new history to
      // file
      ...
      MyLog.e(
          TAG,
          "#readHistoryFromFile: read err, file content empty or corrupted, will return a new history, err is: ${e.localizedMessage}",
      )
    }
  }
  ...
  /** update the `lastUsedTime` and return the original object which passed in 更新最后使用时间并返回传入的原始对象 */
  ...
  private fun saveHistory(newHistory: FileOpenHistory) {
    doJobThenOffLoading {
      try {
        ...
        lock.withLock { JsonUtil.j.encodeToStream(newHistory, getFile().outputStream()) }
      } catch (e: Exception) {
  ...
  /**
   * clear file last edited positions in settings, the filed is
   * `Settings.Editor.filesLastEditPosition`
   */
  private fun clearOldSettingsHistory() {
    try {
      SettingsUtil.update { it.editor.filesLastEditPosition.clear() }
    } catch (e: Exception) {
