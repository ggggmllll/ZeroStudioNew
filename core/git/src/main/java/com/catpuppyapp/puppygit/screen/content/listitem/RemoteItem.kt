  ...
  Column(
    ...
    Row(
      ...
      ScrollableRow {
        Text(
            text =
                if (curObj.pushUrlTrackFetchUrl) stringResource(R.string.use_url)
                else curObj.pushUrl,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = defaultFontWeight,
    ...
    Row(
      ...
      ScrollableRow {
        Text(
            text = curObj.getLinkedFetchCredentialName(),
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = defaultFontWeight,
        )
    ...
    if (curObj.branchMode != Cons.dbRemote_Fetch_BranchMode_All) {
      Row(
        ...
        Text(
            text =
                (if (curObj.branchListForFetch.size > 1) stringResource(R.string.branches)
                else stringResource(R.string.branch)) + ": "
        )

        ScrollableRow {
          Text(
              text = curObj.branchListForFetch.toString(),
              maxLines = 1,
              overflow = TextOverflow.Ellipsis,
              fontWeight = defaultFontWeight,
          )
