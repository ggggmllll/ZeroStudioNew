...
class HttpService : BaseService() {
  companion object {
    ...
    private suspend fun runNewHttpServer(settings: AppSettings) {
      ...
      // 创建新的
      val newServer = HttpServer(settings.httpService.listenHost, settings.httpService.listenPort)
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    ...
    if (action == command_stop) { // stop
      ...
      copyTextToClipboard(
          ...
          text =
              genHttpHostPortStr(
                  ...
                  settings.httpService.listenPort.toString(),
              ),
      )
      ...
      doJobThenOffLoading { doActWithLock { runNewHttpServer(settings) } }
  ...
  override fun onDestroy() {
    ...
    doJobThenOffLoading { doActWithLock { stopCurrentServer() } }
