...
fun CreateRemoteDialog(
  ...
  AlertDialog(
      title = { DialogTitle(stringResource(R.string.create)) },
      text = {
        ScrollableColumn {
          TextField(
        ...
        TextButton(
            onClick = {
              ...
              doJobThenOffLoading(
                  loadingOn,
                  loadingOff,
                  activityContext.getString(R.string.creating),
              ) {
                try {
                  ...
                  Repository.open(repoPath).use { repo ->
                    ...
                    val exists =
                        ret.msg.startsWith("remote") &&
                            ret.msg.endsWith("already exists") // remote 'xxxx' already exists
                    ...
                    if (ret.success() || exists) {
                      val remoteUrl =
                          if (exists) { // 若remote存在，读取配置文件中的url，否则使用用户输入的url
