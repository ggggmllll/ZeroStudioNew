...
fun RepoInnerPage(
  ...
  val repoCountEachRow =
      remember(configuration.screenWidthDp) {
        UIHelper.getRepoItemsCountEachRow(configuration.screenWidthDp.toFloat())
      }
  ...
  val initLoadingText = rememberSaveable {
    mutableStateOf(activityContext.getString(R.string.loading))
  }
  ...
  if (showSetGlobalGitUsernameAndEmailDialog.value) {
    ...
    AskGitUsernameAndEmailDialog(
        title =
            if (showWelcomeToNewUser.value) stringResource(R.string.welcome)
            else stringResource(R.string.user_info),
        text =
            if (showWelcomeToNewUser.value)
                stringResource(R.string.welcome_please_set_git_username_and_email)
            else setGlobalGitUsernameAndEmailStrRes,
        ...
        onOk = {
          ...
          doJobThenOffLoading {
            userInfoRepoList.value.toList().forEachBetter { curRepo ->
              //                    MyLog.d(TAG,
              // "curRepo.value.fullSavePath::"+curRepo.value.fullSavePath)
              Repository.open(curRepo.fullSavePath).use { repo ->
                // save email and username
        ...
        textCompose = {
          MySelectionContainer {
            Column(
              ...
              MyCheckBox(
                  text = stringResource(R.string.the_path_is_a_repos_parent_dir),
  ...
  val doActAndLogErr:
      suspend (curRepo: RepoEntity, actName: String, act: suspend () -> Unit) -> Unit =
      { curRepo, actName, act ->
        try {
          act()
        } catch (e: Exception) {
  ...
  val doFetch: suspend (String?, RepoEntity) -> Unit =
      doFetch@{ remoteNameParam: String?, curRepo: RepoEntity
        -> // 参数的remoteNameParam如果有效就用参数的，否则自己查当前head分支对应的remote
        Repository.open(curRepo.fullSavePath).use { repo ->
            ...
            if (remoteName == null || remoteName.isBlank()) { // fetch不需合并，只需remote有效即可，所以只检查remote
              throw RuntimeException(
                  activityContext.getString(
                      R.string.err_upstream_invalid_plz_go_branches_page_set_it_then_try_again
                  )
              )
  ...
  suspend fun doMerge(upstreamParam: Upstream?, curRepo: RepoEntity): Unit {
    ...
    Repository.open(curRepo.fullSavePath).use { repo ->
      ...
      if (Libgit2Helper.isUpstreamInvalid(upstream)) { // 如果调用者没传有效的upstream，查一下
        ...
        upstream =
            Libgit2Helper.getUpstreamOfBranch(
                repo,
                shortBranchName,
      ...
      if (mergeResult.hasError()) {
        ...
        if (mergeResult.code == Ret.ErrCode.mergeFailedByAfterMergeHasConfilts) {
          ...
          //                            throw
          // RuntimeException(appContext.getString(R.string.merge_has_conflicts))
          //                        }else {
          //                            throw
          // RuntimeException(appContext.getString(R.string.rebase_has_conflicts))
          //                        }
        }
      ...
      ) // 最后一个参数是合并成功或者不需要合并(uptodate)的信息提示函数，这个页面就不要在成功时提示了，合并完刷新下页面显示在仓库卡片上就行了
    }
  }

  val doPush: suspend (Upstream?, RepoEntity) -> Unit =
      doPush@{ upstreamParam: Upstream?, curRepo: RepoEntity ->
        Repository.open(curRepo.fullSavePath).use { repo ->
          ...
          if (Libgit2Helper.isUpstreamInvalid(upstream)) { // 如果调用者没传有效的upstream，查一下
            val shortBranchName =
                Libgit2Helper.getRepoCurBranchShortRefSpec(repo) // 获取当前分支短名，例如 main
            upstream =
                Libgit2Helper.getUpstreamOfBranch(
                    repo,
                    shortBranchName,
          ...
          MyLog.d(
              TAG,
              "#doPush: upstream.remote=" +
                  upstream!!.remote +
                  ", upstream.branchFullRefSpec=" +
                  upstream!!.branchRefsHeadsFullRefSpec,
          )
      ...
      repoList.forEachBetter { curRepo ->
        ...
        repoLock.withLock {
          ...
          if (repoFromDb.workStatus == Cons.dbRepoWorkStatusCloneErr) {
            ...
            //                                    repoFromDb.tmpStatus =
            // appContext.getString(R.string.cloning)
  ...
  val doCloneSingle = { targetRepo: RepoEntity -> doClone(listOf(targetRepo)) }
  ...
  val doSync: suspend (RepoEntity) -> Unit = doSync@{ curRepo: RepoEntity ->
    Repository.open(curRepo.fullSavePath).use { repo ->
      ...
      val isUpstreamExistOnLocal =
          Libgit2Helper.isUpstreamActuallyExistOnLocal(
              repo,
              upstream.remote,
              upstream.branchRefsHeadsFullRefSpec,
          )
      ...
      // 如果不存在上游，只需执行push，相当于pc的 git push with -u
      // (--set-upstream)，但上面已经把remote和branch设置到gitconfig里了(执行到这里都能取出来upstream所以肯定设置过了，在
      // 对话框或分支管理页面设置的)，所以这里正常推送即可，不用再设置什么
  ...
  val getCurActiveList = { if (enableFilterState.value) filterList.value else repoList.value }
  ...
  /**
   * the `_idx` was `idx`, but then changed to find idx by repoId, so deprecated it, kept for
   * backward compatible
   */
  val doActAndSetRepoStatus: suspend (Int, String, String, suspend () -> Unit) -> Unit =
      { _idx: Int, repoId: String, status: String, act: suspend () -> Unit ->
          ...
          doJobThenOffLoading {
            ...
            if (reQueriedRepoInfo.pendingTask == RepoPendingTask.NEED_CHECK_UNCOMMITED_CHANGES) {
              ...
              checkGitStatusAndUpdateItemInList(
                  ...
                  pageChanged = { needRefreshRepoPage.value != curRefreshValue },
              )
            ...
            val curRepoInMenu =
                curRepo
                    .value // 这样修改的话，即使在下面赋值时用户长按了其他仓库也能正常工作，只是下面的赋值操作失去意义而已，但不会并发冲突，也不会显示或执行错误，但如果直接给curRepo.value赋值，则就有可能出错了，比如可能覆盖用户长按的仓库，发生用户长按了仓库a，但显示的却是仓库b的状态的情况
            if (
                curRepoInMenu.id == reQueriedRepoInfo.id
            ) { // 如果，当前长按菜单显示的是当前修改的仓库，则更新下其tmpStatus以更新其菜单项的启用/禁用状态，例如执行fetch时，pull/push/fetch都禁用，但操作执行完后应重新启用，若不做这个检测，则只有重新长按菜单才会更新启用/禁用状态，有点不方便
              // 注意这里更新的只有和长按菜单相关的字段而已，其他字段无需更新
              curRepoInMenu.tmpStatus =
                  reQueriedRepoInfo.tmpStatus // 不出意外的话，这的tmpStatus应为空字符串，但保险起见，还是不要直接用空字符串赋值比较好
  ...
  if (showDelRepoDialog.value) {
    ConfirmDialog2(
        ...
        // '"+willDeleteRepo.value.repoName+"' ?"+"\n"+
        // stringResource(R.string.will_delete_repo_and_all_its_files_on_disk),
        requireShowTextCompose = true,
        textCompose = {
          ScrollableColumn {
            SelectionRow { Text(text = stringResource(id = R.string.delete_repos) + ":") }
        ...
        okTextColor =
            if (requireDelFilesOnDisk.value) MyStyleKt.TextColor.danger() else Color.Unspecified,
      ...
      doJobThenOffLoading {
        ...
        try {
          ...
          var updatedPackageNameAndRepoSettingsMap =
              settings.automation.packageNameAndRepoAndSettingsMap
          ...
          settings.automation.packageNameAndRepoAndSettingsMap =
              updatedPackageNameAndRepoSettingsMap
  ...
  val repoNameForRenameDialog =
      mutableCustomStateOf(stateKeyTag, "repoNameForRenameDialog") { TextFieldValue("") }
  ...
  if (showRenameDialog.value) {
    ...
    ConfirmDialog(
        ...
        cancelBtnText = stringResource(R.string.cancel),
        okBtnEnabled =
            repoNameForRenameDialog.value.text.isNotBlank() &&
                errMsgForRenameDialog.value.isEmpty() &&
                repoNameForRenameDialog.value.text != curRepo.repoName,
      ...
      doJobThenOffLoading {
        try {
          ...
          createAndInsertError(
              curRepo.id,
              "err: rename repo '${curRepo.repoName}' to ${repoNameForRenameDialog.value} failed, err message is '$errmsg'",
          )
  ...
  if (showUnshallowDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            MySelectionContainer {
              Row { Text(text = stringResource(R.string.will_do_unshallow_for_repos) + ":") }
            }
            MySelectionContainer {
              Row(
  ...
  val statusClickedRepo =
      mutableCustomStateOf(keyTag = stateKeyTag, keyName = "statusClickedRepo", RepoEntity(id = ""))
    ...
    if (targetRepo.id.isBlank()) {
      ...
      ConfirmDialog(
          ...
          text =
              stringResource(
                  R.string
                      .will_go_to_changelist_then_you_can_continue_or_abort_your_merge_rebase_cherrpick
              ),
  ...
  val doActAfterSetUpstreamSuccess =
      mutableCustomStateOf<(() -> Unit)?>(stateKeyTag, "doActAfterSetUpstreamSuccess") { null }
  val setUpstreamOnFinally =
      mutableCustomStateOf<(() -> Unit)?>(stateKeyTag, "setUpstreamOnFinally") { null }
  val showClearForSetUpstreamDialog = rememberSaveable { mutableStateOf(false) }

  val initSetUpstreamDialog: suspend (RepoEntity, String, (() -> Unit)?) -> Unit =
      { targetRepo, onOkText, actAfterSuccess ->
        try {
          ...
          Repository.open(targetRepo.fullSavePath).use { repo ->
            val headRef =
                Libgit2Helper.resolveHEAD(repo) ?: throw RuntimeException("resolve HEAD failed")
          ...
          setUpstreamOnFinally.value =
              if (actAfterSuccess != null) null
              else {
                { refreshSpecifedRepos(listOf(targetRepo)) }
              }
          ...
          MyLog.e(
              TAG,
              "init set upstream dialog err: targetRepo='${targetRepo.repoName}', err=${e.stackTraceToString()}",
          )
        }
      }
  ...
  if (showSetUpstreamForLocalBranchDialog.value) {
    ...
    SetUpstreamDialog(
        ...
        closeDialog = { showSetUpstreamForLocalBranchDialog.value = false },
          ...
          ScrollableColumn {
            ...
            Box(
              ...
              Row(
                ...
                MySelectionContainer { Text(text = pullurl) }
              }
            ...
            Box(
              ...
              Row(
                ...
                MySelectionContainer { Text(text = pushurl) }
              }
            ...
            Box(
              ...
              Row(
                ...
                MySelectionContainer { Text(text = syncurl) }
              }
  ...
  val selectedSingle = { getSelectedFilesCount() == 1 }

  val hasSelectedItems = { getSelectedFilesCount() > 0 }
  ...
  val repoCardTitleOnClick = { item: RepoEntity -> switchItemSelected(item) }
  ...
  val isRepoGoodAndActEnabled = { curRepo: RepoEntity ->
    ...
    // 如果有设临时状态，说明在执行某个操作，比如正在fetching，所以这时应该不允许再执行fetch或pull之类的操作，我做了处理，即使用户去cl页面执行，也无法绕过此限制( ?
    // 不确定，cl页面好像已经无视tmpStatus了，而且没加mutex，直接依靠libgit2底层的锁（好像有））
    ...
    val hasTmpStatus =
        curRepo.pendingTask != RepoPendingTask.NEED_CHECK_UNCOMMITED_CHANGES &&
            curRepo.tmpStatus.isNotBlank()
  ...
  val backHandlerOnBack =
      ComposeHelper.getDoubleClickBackHandler(
  ...
  BackHandler(
      enabled = isBackHandlerEnable.value,
      onBack = {
  ...
  PullToRefreshBox(
    ...
    if (repoList.value.isEmpty()) { // 无仓库，显示添加按钮
      if (isInitLoading.value) {
        FullScreenScrollableColumn(contentPadding) { Text(initLoadingText.value) }
          ...
          filterTheList(
              ...
              match = { idx: Int, it: RepoEntity ->
                it.repoName.contains(keyword, ignoreCase = true) ||
                    ...
                    it.getOrUpdateCachedOneLineLatestCommitMsg()
      ...
      MyLazyColumn(
        ...
        Row(
          ...
          chunkedList.forEachIndexedBetter { subListIdx, element ->
            ...
            // 如果有必要细分状态，可以改成这样: if(it.workStatus==cloningStatus) go cloningCard, else if other
            // status, go other card, else go normal RepoCard
            ...
            RepoCard(
              ...
              -> // 这个是点击status的callback，这个status其实可以不传，因为这里的lambda能捕获到数组的元素，就是当前仓库
              ...
              if (
                  status == Cons.dbRepoWorkStatusMerging ||
                ...
                if (dbIntToBool(curRepo.isDetached)) { // detached, can't sync
                  ...
                  if (curRepo.upstreamBranch.isBlank()) { // 无上游，先设置，再同步
                    doTaskOrShowSetUsernameAndEmailDialog(curRepo) {
                      doJobThenOffLoading {
                        initSetUpstreamDialog(
                            curRepo,
                            activityContext.getString(R.string.save_and_sync),
                        ) {
                          doActWithLockIfRepoGoodAndActEnabled(curRepo) {
                            doActAndSetRepoStatus(
                                invalidIdx,
                                curRepo.id,
                                activityContext.getString(R.string.syncing),
                            ) {
                              doActAndLogErr(curRepo, "sync") { doSync(curRepo) }
                            }
                          }
                        }
                      }
                ...
                doTaskOrShowSetUsernameAndEmailDialog(curRepo) {
                  doActWithLockIfRepoGoodAndActEnabled(curRepo) {
                    doActAndSetRepoStatus(
                      ...
                      doActAndLogErr(curRepo, "pull") { doPull(curRepo) }
                    }
                  }
                }
                ...
                doActWithLockIfRepoGoodAndActEnabled(curRepo) {
                  doActAndSetRepoStatus(
                    ...
                    doActAndLogErr(curRepo, "push") { doPush(null, curRepo) }
                  }
                }
  ...
  if (isSelectionMode.value) {
    ...
    val selectionModeIconOnClickList =
        listOf<() -> Unit>(
            sync@{
              val needSetUpstreamBeforeSync = { curRepo: RepoEntity ->
                curRepo.upstreamBranch.isBlank() && !dbIntToBool(curRepo.isDetached)
              }
              val expectRepos = { it: RepoEntity ->
                it.upstreamBranch.isNotBlank() && !dbIntToBool(it.isDetached)
              }
              ...
              if (list.size == 1) {
                ...
                if (expectRepos(curRepo)) {
                  doTaskOrShowSetUsernameAndEmailDialog(curRepo) { task(curRepo) }
                ...
                list.filter { expectRepos(it) }.forEachBetter { curRepo -> task(curRepo) }
              }
            ...
            push@{
              selectedItems.value
                  .toList()
                  .filter { it.upstreamBranch.isNotBlank() && !dbIntToBool(it.isDetached) }
                  .forEachBetter { curRepo ->
                    doActWithLockIfRepoGoodAndActEnabled(curRepo) {
                      doActAndSetRepoStatus(
                          invalidIdx,
                          curRepo.id,
            ...
            pull@{
              ...
              if (list.size == 1) {
                ...
                list.filter { expectRepos(it) }.forEachBetter { task(it) }
              }
            },
            fetch@{
              selectedItems.value
                  ...
                  .forEachBetter { curRepo ->
                    doActWithLockIfRepoGoodAndActEnabled(curRepo) {
                      ...
                      doActAndSetRepoStatus(
                        ...
                        doActAndLogErr(curRepo, "fetch") { doFetch(null, curRepo) }
                      }
                    }
                  }
            },
    ...
    val bottomBarIconDefaultEnable = {
      hasSelectedItems() &&
          selectedItems.value.any { it.upstreamBranch.isNotBlank() && !dbIntToBool(it.isDetached) }
    }
    ...
    val selectionModeMoreItemTextList =
        (listOf(
            ...
            //        stringResource(R.string.export_files), //
            // multi/single，如果多选仓库，强制为不同仓库创建文件夹名，如果单选仓库，会显示一个“清空”的按钮，若勾选，执行导入前会先清空git仓库；执行导出前会先清空输出目录。还有，这个导入导出只支持saf，非saf的可通过本app的Files页面完成。
    ...
    val selectionModeMoreItemOnClickList =
        (listOf(
            refresh@{ refreshSpecifedRepos(selectedItems.value) },
            ...
            changelist@{
              ...
              doActIfRepoGood(curRepo) { goToChangeListPage(curRepo) }
            },
            userInfo@{ showSetUserInfoDialog(selectedItems.value.filter { isRepoGood(it) }) },
            rename@{
              ...
              // init rename dialog
              repoNameForRenameDialog.value =
                  TextFieldValue(
                      text = selectedRepo.repoName,
                      selection = TextRange(0, selectedRepo.repoName.length),
                  )
              errMsgForRenameDialog.value = ""
            ...
            setUpstream@{
              doJobThenOffLoading {
                initSetUpstreamDialog(
                    ...
                    activityContext.getString(R.string.save),
                    null,
                )
              }
            ...
            unshallow@{
              ...
              val unshallowableList =
                  selectedItems.value.filter { curRepo ->
                    val repoStatusGood =
                        curRepo.gitRepoState != null &&
                            !Libgit2Helper.isRepoStatusNotReadyOrErr(curRepo)
            ...
            stash@{
              val curRepo = selectedItems.value.first()
              doActIfRepoGood(curRepo) { doJobThenOffLoading { goToStashPage(curRepo.id) } }
            },
            reflog@{
              val curRepo = selectedItems.value.first()
              doActIfRepoGood(curRepo) {
            ...
            api@{
              ...
              val token =
                  settings.httpService.tokenList.let { if (it.isEmpty()) "" else it.first() }

              val sbpull =
                  StringBuilder("${genHttpHostPortStr(host, port.toString())}/pull?token=$token")
              val sbpush =
                  StringBuilder("${genHttpHostPortStr(host, port.toString())}/push?token=$token")
              val sbsync =
                  StringBuilder("${genHttpHostPortStr(host, port.toString())}/sync?token=$token")
            ...
            //            selectedItems.value.forEach {
            //                sb.append(HttpServer.getApiJson(it,
            // SettingsUtil.getSettingsSnapshot()))
            //                sb.append(spliter)
            //            }
            //
            //            initDetailsDialog(activityContext.getString(R.string.api),
            // sb.removeSuffix(spliter).toString())
            //        },
            ...
            details@{
              ...
              selectedItems.value.forEachBetter {
                sb.append(activityContext.getString(R.string.name))
                    ...
                    .append(lb)
                    .append(lb)
                sb.append(activityContext.getString(R.string.id))
                    .append(": ")
                    .append(it.id)
                    .append(lb)
                    .append(lb)
                sb.append(activityContext.getString(R.string.state))
                    .append(": ")
                    .append(it.getRepoStateStr(activityContext))
                    .append(lb)
                    .append(lb)
              ...
              initDetailsDialog(
                  activityContext.getString(R.string.details),
                  sb.removeSuffix(spliter).toString(),
              )
            },
            delete@{ requireDelRepo(selectedItems.value.toList()) },
        ))

    val selectionModeMoreItemEnableList =
        (listOf(
            refresh@{ hasSelectedItems() },
            changelist@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            //        importFiles@{
            ...
            userInfo@{ hasSelectedItems() && selectedItems.value.any { isRepoGood(it) } },
            rename@{ selectedSingle() },
            setUpstream@{
              if (selectedSingle()) {
            ...
            clone@{
              // 至少选中一个需要克隆的仓库才显示此按钮
              hasSelectedItems() &&
                  selectedItems.value.any { it.workStatus == Cons.dbRepoWorkStatusCloneErr }
            },
            unshallow@{
              hasSelectedItems() && selectedItems.value.any { dbIntToBool(it.isShallow) }
            },
            remotes@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            tags@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            stash@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            reflog@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            submodules@{ selectedSingle() && isRepoGood(selectedItems.value.first()) },
            editConfig@{ selectedSingle() },
            api@{ hasSelectedItems() },
            details@{ hasSelectedItems() },
...
private suspend fun doInit(
  ...
  val pageChanged = { refreshId != latestRefreshId.value }

  val specifiedRefreshRepoList: MutableList<RepoEntity> =
      if (specifiedRefreshRepoList.isNotEmpty()) {
  ...
  // complex code for update item and remove non-exist item, but the effect just no-difference with
  // clear then addAll, so, disable it
  ...
  for ((idx, item) in repoDtoList.value.toList().withIndex()) {
    ...
    if (
        specifiedRefreshRepoList.isNotEmpty() &&
            specifiedRefreshRepoList.find { it.id == item.id } == null
    ) {
      continue
    }
...
private fun updateRepoListByIndexOrId(
    newItem: RepoEntity,
    idx: Int,
    list: MutableList<RepoEntity>,
    expectListSize: Int,
) {
  ...
  doJobThenOffLoading {
    try {
      Repository.open(item.fullSavePath).use { repo ->
        ...
        MyLog.d(
            TAG,
            "#checkRepoGitStatus: repoName=${item.repoName}, repoId=${item.id}, needCommit=$needCommit, pageChanged=${pageChanged()}",
        )

        // 如果页面没改变（没重新刷新） 且 仓库没有在执行其他操作（例如 pull），则 更新列表
        if (
            pageChanged().not() &&
                RepoStatusUtil.getRepoStatus(item.id).let { it == null || it.isBlank() }
        ) {
          val newRepo = item.copyAllFields(settings)
          // 操作已经执行完毕，清空需要执行的操作
