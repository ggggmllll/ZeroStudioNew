...
interface RepoDao {
  ...
  @Query("SELECT * from repo WHERE id = :id") fun getStream(id: String): Flow<RepoEntity?>

  @Insert suspend fun insert(item: RepoEntity)

  // update by id
  @Update suspend fun update(item: RepoEntity)

  // delete by id
  @Delete suspend fun delete(item: RepoEntity)

  @Query("SELECT id from repo where repoName=:repoName LIMIT 1")
  suspend fun getIdByRepoName(repoName: String): String?

  /**
   * 更新时用来检查仓库名是否冲突的，如果其他条目存在相同仓库名，就冲突，排除id是需要更新的仓库的id，修改仓库信息时，自己的名字可以和自己的名字一样，所以检查时需要排除自己的id for
   * check repo name exists, excludeId used for exclude update item, because update a Repo to same
   * name is ok, in that case, should exclude itself id when checking name exist
   */
  ...
  @Query("SELECT * from repo where id=:id") suspend fun getById(id: String): RepoEntity?
  ...
  //    @Query("SELECT * from repo where baseIsDel=0 and storageDirId=:storageDirId ORDER BY
  // baseCreateTime DESC")
  ...
  //    @Query("SELECT * from repo WHERE baseIsDel=0 ORDER BY baseCreateTime DESC")
  @Query("SELECT * from repo ORDER BY baseCreateTime DESC") suspend fun getAll(): List<RepoEntity>

  @Query(
      "update repo set credentialIdForClone = :newCredentialIdForClone where credentialIdForClone = :oldCredentialIdForClone"
  )
  suspend fun updateCredentialIdByCredentialId(
      oldCredentialIdForClone: String,
      newCredentialIdForClone: String,
  )

  @Query(
      "update repo set hasUncheckedErr = :hasUncheckedErr , latestUncheckedErrMsg = :latestUncheckedErrMsg where id = :repoId"
  )
  suspend fun updateErrFieldsById(
      repoId: String,
      hasUncheckedErr: Int,
  ...
  @Query(
      "update repo set branch = :branch , lastCommitHash = :lastCommitHash , isDetached=:isDetached, upstreamBranch=:upstreamBranch where id = :repoId"
  )
  suspend fun updateBranchAndCommitHash(
      repoId: String,
      branch: String,
  ...
  @Query(
      "update repo set lastCommitHash = :lastCommitHash , isDetached=:isDetached where id = :repoId"
  )
  ...
  @Query(
      "UPDATE repo set baseCreateTime = baseCreateTime-(:offsetInSec), baseUpdateTime = baseUpdateTime-(:offsetInSec), lastUpdateTime = lastUpdateTime-(:offsetInSec)"
  )
