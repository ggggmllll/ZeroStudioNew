...
object SnapshotUtil : SnapshotCreator {
  ...
  */
  private val fileSnapshotDisable_FileNamePlaceHolder =
      "FileSnapshotDisable-name" // 文件快照禁用时，一律返回此文件名
  private val fileSnapshotDisable_FilePathPlaceHolder =
      "FileSnapshotDisable-path" // 文件快照禁用时，一律返回此路径
  private val contentSnapshotDisable_FileNamePlaceHolder =
      "ContentSnapshotDisable-name" // 内容快照禁用时，一律返回此文件名
  private val contentSnapshotDisable_FilePathPlaceHolder =
      "ContentSnapshotDisable-path" // 内容快照禁用时，一律返回此路径
  ...
  fun init(
      enableFileSnapshotForEditorInitValue: Boolean,
      enableContentSnapshotForEditorInitValue: Boolean,
      enableFileSnapshotForDiffInitValue: Boolean,
  ) {
  ...
  override fun createSnapshotByContentAndGetResult(
    ...
    if (!enableContentSnapshotForEditor) {
      return Ret.createSuccess(
          Pair(
              contentSnapshotDisable_FileNamePlaceHolder,
              contentSnapshotDisable_FilePathPlaceHolder,
          )
      )
    }
    ...
    try {
      if (
          (trueUseContentFalseUseEditorState && fileContent!!.isNotEmpty()) ||
              (trueUseContentFalseUseEditorState.not() && editorState!!.contentIsEmpty().not())
      ) {
        val (snapshotFileName, snapFileFullPath, snapFile) =
            getSnapshotFileNameAndFullPathAndFile(srcFileName, flag)
      ...
      return Ret.createError(
          null,
          "save file snapshot failed: ${e.localizedMessage}",
          Ret.ErrCode.saveFileErr,
      )
    }
  }

  override fun createSnapshotByFileAndGetResult(
      srcFile: FuckSafFile,
      flag: SnapshotFileFlag,
  ): Ret<Pair<String, String>?> {
    // 如果未开启文件快照功能 或 开启了diff文件但flag不是diff文件快照 或 开启了editor文件快照但flag不是editor文件快照 则 直接返回成功
    if (
        (!enableFileSnapshotForEditor && !enableFileSnapshotForDiff) ||
            (!enableFileSnapshotForEditor &&
                enableFileSnapshotForDiff &&
                !flag.isDiffFileSnapShot()) ||
            (enableFileSnapshotForEditor &&
                !enableFileSnapshotForDiff &&
                !flag.isEditorFileSnapShot())
    ) {
      return Ret.createSuccess(
          Pair(fileSnapshotDisable_FileNamePlaceHolder, fileSnapshotDisable_FilePathPlaceHolder)
      ) // 1是file name，2是file path
    }
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#$funName() err: srcFileName=$srcFileNameForLog, snapshotFileName=$snapshotFileNameForLog, err=${e.stackTraceToString()}",
      )
      return Ret.createError(
          null,
          "save file snapshot failed: ${e.localizedMessage}",
   ...
   */
  private fun genSnapshotFileName(
      srcFileName: String,
      flag: String,
      timestamp: String,
      uid: String,
    ...
    //        return
    // sb.append("-").append(flag).append("-").append(getNowInSecFormatted(Cons.dateTimeFormatterCompact)).append("-").append(getShortUUID(len = uidLen)).append(".bak").toString()
