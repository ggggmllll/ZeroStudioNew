...
class CustomStateSaveable<T>(private val holder: Holder<MutableState<T>>) {
...
class CustomBoxSaveable<T>(private val holder: Holder<Box<T>>) {
  var value: T = holder.data.value
    get() {
      return holder.data.value
    }
...
class CustomStateListSaveable<T>(private val holder: Holder<SnapshotStateList<T>>) {
...
class CustomStateMapSaveable<K, V>(private val holder: Holder<SnapshotStateMap<K, V>>) {
...
@Composable
fun <T> mutableCustomStateOf(
    keyTag: String,
    keyName: String,
    initValue: T,
    inputs: Array<Any?> = arrayOf(),
): CustomStateSaveable<T> {
  val stateHolder =
      rememberSaveable(inputs = inputs, saver = getSaver()) {
...
@Composable
fun <T> mutableCustomStateListOf(
    keyTag: String,
    keyName: String,
    initValue: List<T>,
    inputs: Array<Any?> = arrayOf(),
): CustomStateListSaveable<T> {
  val stateHolder =
      rememberSaveable(inputs = inputs, saver = getSaver()) {
...
@Composable
fun <K, V> mutableCustomStateMapOf(
    keyTag: String,
    keyName: String,
    initValue: Map<K, V>,
    inputs: Array<Any?> = arrayOf(),
): CustomStateMapSaveable<K, V> {
  val stateHolder =
      rememberSaveable(inputs = inputs, saver = getSaver()) {
