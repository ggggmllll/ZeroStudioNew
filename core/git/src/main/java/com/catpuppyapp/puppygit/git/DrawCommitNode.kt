...
// 避免并发冲突，没用普通的list
private val cachedColors =
    ConcurrentMap<Int, Color>().apply {
...
data class DrawCommitNode(
  ...
  fun toStringForView(): String {
    return "from: $fromCommitHash\nto: $toCommitHash\ncircleAtHere: $circleAtHere\nstartAtHere: $startAtHere\nendAtHere: $endAtHere\ninputIsEmpty: $inputIsEmpty\noutputIsEmpty: $outputIsEmpty\n\n" +
        (if (mergedList.isEmpty()) ""
          ...
          mergedList.forEachBetter { sb.append(it.toStringForView()) }
  ...
  companion object {
    ...
    fun localAheadUpstreamColor(inDarkTheme: Boolean = Theme.inDarkTheme) =
        if (inDarkTheme) unPushedColorDark else unPushedColorLight
    ...
    fun getAnInsertableIndex(list: List<DrawCommitNode>, toCommitHash: String): InsertablePosition {
      ...
      for ((idx, node) in list.withIndex()) {
        ...
        if (
            node.outputIsEmpty ||
                (node.toCommitHash == toCommitHash).let {
                  isMergedToPrevious = it
                  it
                }
        ) {
          index = idx
      ...
      return InsertablePosition(isMergedToPrevious, index)
    }

    fun getNodeColorByIndex(i: Int): Color {
      // 缓存颜色的集合得是可变的，运行时如果发现颜色索引不存在，会插入个新颜色存进去
      // alpha越大越不透明，范围 0-255，128是半透明
      return cachedColors.get(i)
          ?: UIHelper.getRandomColorForDrawNode(alpha = alpha).let {
    ...
    fun transOutputNodesToInputs(
        node: DrawCommitNode,
        currentCommitOidStr: String,
        idx: Int,
        circleAt: Box<Int>,
    ): DrawCommitNode {
