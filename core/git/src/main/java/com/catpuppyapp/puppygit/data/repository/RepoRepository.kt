...
interface RepoRepository {
  ...
  /**
   * Delete item from the data source 注：requireTransaction:
   * 我不确定room的事务策略是怎样，不知道若已存在事务是加入还是新建，所以用这个变量来控制，若外部调用者有事务，传false即可
   */
  suspend fun delete(
      item: RepoEntity,
      requireDelFilesOnDisk: Boolean = false,
      requireTransaction: Boolean = true,
  )
  ...
  suspend fun getByFullSavePath(
      fullSavePath: String,
      onlyReturnReadyRepo: Boolean = false,
      requireSyncRepoInfoWithGit: Boolean = true,
  ): RepoEntity?
  ...
  /**
   * @param repoNamePrefix specific a prefix for repo name, e.g. when import submodule of repos,
   *   default prefix is "sub_"
   * @param parentRepoId when import a submodule from a repo, should set this id to parent repo's id
   */
  suspend fun importRepos(
      dir: String,
