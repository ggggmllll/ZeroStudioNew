...
fun CredentialNewOrEdit(
  ...
  val credentialInThisPage =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "credentialInThisPage",
          initValue = CredentialEntity(id = ""),
      )
  val oldCredential =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "oldCredential",
          initValue = CredentialEntity(id = ""),
      )
  ...
  val doSave: () -> Unit = {
    doJobThenOffLoading launch@{
      ...
      if (
          !isEditMode.value || credentialNameText != oldCredentialName
      ) { // 只有当非编辑模式或者新旧名称不一样时才检查是否重名(新建模式必检查是否重名，编辑模式允许和当前条目旧名一样)
        // 检查是否重名
      ...
      val credentialForSave =
          if (isEditMode.value) credentialInThisPage.value else CredentialEntity()
      ...
      if (isEditMode.value) {
        ...
        if (
            oldPass != newPass
        ) { // 新旧密码如果不一样，说明用户编辑了密码，这时候需要重新加密；如果一样，说明依然是编辑前的加密过的密码，则不需要加密；（不太可能发生的极特殊情况：用户输入的新密码原文和加密后的密码字符串完全一样！这样就会出问题，连接时会使用“解密”后的错误密码）
  ...
  Scaffold(
    ...
    Column(
      ...
      TextField(
          ...
          label = { Text(stringResource(R.string.username_or_private_key)) },
      )
      ...
      if (isEditMode.value && oldPassIsEmpty.value.not()) {
        Row(modifier = Modifier.padding(horizontal = 10.dp)) {
          Text(
              text =
                  stringResource(
                      R.string.don_t_touch_the_password_passphrase_if_you_don_t_want_to_update_it
                  ),
  ...
  LaunchedEffect(Unit) {
    ...
    doJobThenOffLoading(
      ...
      if (
          credentialId != null && credentialId.isNotBlank() && credentialId != "null"
      ) { // 如果是编辑模式，查询仓库信息
        ...
        credentialSelectedOption.intValue =
            if (credentialType.intValue == Cons.dbCredentialTypeHttp) optNumHttp
            else optNumSsh // 设置凭据类型选中项为数据库中的类型
  ...
  isReadyForSave.value =
      ((!showCredentialNameAlreadyExistsErr.value) && (credentialName.value.text.isNotBlank())
      //           && ((credentialType.intValue==Cons.dbCredentialTypeSsh &&
      // credentialVal.value.isNotBlank()) || (credentialType.intValue==Cons.dbCredentialTypeHttp &&
