...
object PatchUtil {
  ...
  fun maybeIsPatchLine(str: String) =
      isFileHeader(str) || isAdd(str) || isDelete(str) || isHunkHeader(str)
  ...
  fun isFileHeader(str: String) =
      str.startsWith("diff --git") ||
          str.startsWith("---") ||
          str.startsWith("+++") ||
          str.startsWith("index") ||
          str.startsWith("new file")

  fun getColorOfLine(str: String, inDarkTheme: Boolean): Color? =
      if (isFileHeader(str)) {
        if (inDarkTheme) {
