...
data class HttpServiceSettings(
     ...
     *
     * will use first token in the list when copy api config in the Repos Screen note: if token list
     * is empty, will reject all request
     */
    ...
    @Deprecated("ipWhiteList and token fair enough") var ipBlackList: List<String> = listOf(),

    /**
     * show notify if server do act err
     *
     * 注意：这是系统通知，不是toast！ 最好能够实现：点击可启动app并前往仓库页面，定位到发生错误的仓库卡片
     */
    @Deprecated("now, only using show notify when progress") var showNotifyWhenErr: Boolean = true,
    ...
    /** show notify if server acting 进度通知，例如正在推送、正在拉取之类的 */
