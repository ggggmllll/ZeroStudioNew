...
fun ChangeListInnerPage(
    ...
    goToFilesPage: (path: String) -> Unit =
        {}, // 跳转到Files页面以浏览仓库文件，只有主页需要传这个参数，Index和TreeToTree页面不需要
    ...
    goToRepoPage: (targetRepoId: String) -> Unit =
        {}, // only show workdir changes at ChangeList need this
  ...
  // for detect page changed, avoid loading repo1, switched repo2, repo2 loaded, then repo1 loaded,
  // page show wrong items of repo1
  //    val pageId = remember {
  ...
  val repoId =
      remember(repoId, curRepoFromParentPage.value.id) {
            derivedStateOf { if (repoId.isBlank()) curRepoFromParentPage.value.id else repoId }
          }
          .value // must call .value, else derived block may not executing

  //    val isDiffToLocal = fromTo == Cons.gitDiffFromIndexToWorktree ||
  // commit1OidStr==Cons.git_LocalWorktreeCommitHash ||
  ...
  val localAtDiffRight =
      remember(fromTo, commit1OidStr, commit2OidStr, swap) {
        derivedStateOf {
          (fromTo == Cons.gitDiffFromIndexToWorktree ||
              (if (swap) commit1OidStr == Cons.git_LocalWorktreeCommitHash
              else (commit2OidStr == Cons.git_LocalWorktreeCommitHash)))
        }
      }
  ...
  val doActWithLock: suspend (curRepo: RepoEntity, act: suspend () -> Unit) -> Unit =
      { curRepo, act ->
  ...
  val afterSetUsernameAndEmailSuccessCallback =
      mutableCustomStateOf<(() -> Unit)?>(stateKeyTag, "afterSetUsernameAndEmailSuccessCallback") {
        null
      }
      ...
      Repository.open(curRepo.fullSavePath).use { repo ->
        if (Libgit2Helper.repoUsernameAndEmailInvaild(repo)) {
          Msg.requireShowLongDuration(
              activityContext.getString(R.string.plz_set_username_and_email_first)
          )
  ...
  val pleaseSetUsernameAndEmailBeforeCommit =
      stringResource(R.string.please_set_username_email_before_commit)
  ...
  val initRebaseSkipDialog = { curRepo: RepoEntity ->
    doTaskOrShowSetUsernameAndEmailDialog(curRepo) { showRebaseSkipDialog.value = true }
  }
  ...
  //    val needRefreshChangeListPage = remember {
  // mutableStateOf("justForTriggerLaunchedEffectAfterNaviBack") }
  // must make sure the init value 100% difference with `refreshRequiredByParentPage`, so dont use
  // empty string as init value
  //    val lastRequireRefreshValue = rememberSaveable {
  // mutableStateOf("TheValueNever=RequireRefresh")}
  //    val changeListPageWorktreeItemList = remember { mutableStateListOf<StatusTypeEntrySaver>() }
  //    val itemList = mutableCustomStateOf(value = mutableListOf<StatusTypeEntrySaver>())
  // //这个反正旋转屏幕都会清空，没必要用我写的自定义状态存储器
  //    val changeListPageConflictItemList = mutableCustomStateOf(value =
  // mutableListOf<StatusTypeEntrySaver>())
  //    val openRepoFailedErrStrRes = stringResource(R.string.open_repo_failed)
  val curRepoUpstream =
      mutableCustomStateOf(
          keyTag = stateKeyTag,
          keyName = "curRepoUpstream",
          initValue = Upstream(),
      )
  ...
  //    val selFilePathListJsonObjStr = remember{ mutableStateOf("{}") }
  // //key是文件名，所以这个列表只能存储相同目录下的文件，不同目录有可能名称冲突，但由于选择模式只能在当前目录选择，所以这个缺陷可以接受。json格式:{fileName:canonicalPath}
  ...
  val filesPageAddFileToSelectedListIfAbsentElseRemove: (StatusTypeEntrySaver) -> Unit =
  ...
  val selectedListIsNotEmpty: () -> Boolean = { selectedItemList.value.isNotEmpty() }
  ...
  val loadingOn = { text: String ->
    ...
    // 这里不能刷新，页面会闪烁，会看不到中间状态，忘了之前怎么写的了，总之这里不用刷新
  }
  val loadingOff = {
  ...
  val mustSelectAllConflictBeforeCommitStrRes =
      stringResource(R.string.must_resolved_conflict_and_select_them_before_commit)
  ...
  val upstreamRemoteOptionsList =
      mutableCustomStateListOf(
          keyTag = stateKeyTag,
          keyName = "upstreamRemoteOptionsList",
          initValue = listOf<String>(),
      )
  val upstreamSelectedRemote = rememberSaveable {
    mutableIntStateOf(0)
  } // 默认选中第一个remote，每个仓库至少有一个origin remote，应该不会出错
  // 默认选中为上游设置和本地分支相同名
  ...
  val afterSetUpstreamSuccessCallback =
      mutableCustomStateOf<(() -> Unit)?>(stateKeyTag, "afterSetUpstreamSuccessCallback") { null }
  val setUpstreamOnFinallyCallback =
      mutableCustomStateOf<(() -> Unit)?>(stateKeyTag, "setUpstreamOnFinallyCallback") { null }
  val initSetUpstreamDialog: (List<String>, String, String, String, (() -> Unit)?) -> Unit =
      { remoteList, curBranchShortName, curBranchFullName, onOkText, successCallback ->
        // 设置远程名
        ...
        setUpstreamOnFinallyCallback.value =
            if (successCallback != null) null
            else {
              { changeListRequireRefreshFromParentPage(curRepoFromParentPage.value) }
            }
  ...
  val doStageAll_2 =
      { paramList: List<StatusTypeEntrySaver>, curRepo: RepoEntity, requireCloseBottomBar: Boolean
        ->
        // 第2个参数代表使用参数提供的列表，第3个参数是当前仓库所有的changelist列表，这样就实现了stage all
        ChangeListFunctions.doStage(
            curRepo = curRepo,
  ...
  val forcePush_curRepo =
      mutableCustomStateOf(stateKeyTag, "forcePush_curRepo") { RepoEntity(id = "") }
  ...
  if (forcePush_ShowDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            MySelectionContainer {
              PaddingText(
                  stringResource(
                      R.string.will_force_overwrite_remote_branch_even_it_is_ahead_to_local
                  ),
        ...
        okBtnEnabled =
            forcePush_pushWithLease.value.not() ||
                forcePush_expectedRefspecForLease.value.isNotEmpty(),
        ...
        doActWithLock(curRepo) {
          try {
            ...
            showErrAndSaveLog(
                ...
                showMsg =
                    activityContext.getString(R.string.push_force_failed) +
                        ": " +
                        e.localizedMessage,
  ...
  if (requireDoActFromParent.value) { // pull
    ...
    doJobThenOffLoading(
      ...
      val requireAct =
          Cache.syncGetThenDel(Cache.Key.changeListInnerPage_requireDoActFromParent)
      ...
      if (requireAct == PageRequest.editIgnoreFile) {
        try {
          Repository.open(curRepo.fullSavePath).use { repo ->
            //                        val ignoreFilePath =
            // IgnoreMan.getFileFullPath(Libgit2Helper.getRepoGitDirPathNoEndsWithSlash(repo))
            val ignoreFilePath =
                Libgit2Helper.getRepoIgnoreFilePathNoEndsWithSlash(repo, createIfNonExists = true)
          ...
          MyLog.e(
              TAG,
              "get ignore file for repo '${curRepo.repoName}' err: ${e.stackTraceToString()}",
          )
        }
          ...
          doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.syncing)) {
            doActWithLock(curRepo) {
              try {
                ...
                showErrAndSaveLog(
                    TAG,
                    "require Sync(Merge) error: " + e.stackTraceToString(),
                    activityContext.getString(R.string.sync_merge_failed) +
                        ": " +
                        e.localizedMessage,
                    requireShowToast,
                    curRepo.id,
          ...
          doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.syncing)) {
            doActWithLock(curRepo) {
              try {
                ...
                showErrAndSaveLog(
                    TAG,
                    "require Sync(Rebase) error: " + e.stackTraceToString(),
                    activityContext.getString(R.string.sync_rebase_failed) +
                        ": " +
                        e.localizedMessage,
                    requireShowToast,
                    curRepo.id,
        ...
        doActWithLock(curRepo) {
          if (requireAct == PageRequest.fetch) {
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require fetch error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.fetch_failed) + ": " + e.localizedMessage,
                  requireShowToast,
                  curRepo.id,
              )
            } finally {
            ...
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require stage_all error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.stage_all_failed) + ": " + e.localizedMessage,
                  requireShowToast,
                  curRepo.id,
              )
            } finally {
            ...
            // workTreeToIndex的提交所有和Index页面的提交的区别在于：workTreeToIndex在提交前会先执行stage
            // all，并且itemList传null以触发强制重查indexList

            try {
              // 如果是从DiffScreen返回带来的列表，仅针对在diff页面查看的条目执行操作
              val listForStage =
                  if (actFromDiffScreen_tmp)
                      (Cache.getByType<List<StatusTypeEntrySaver>>(
                          Cache.Key.diffableList_of_fromDiffScreenBackToWorkTreeChangeList
                      ) ?: listOf())
                  else itemList.value.toList()

              if (AppModel.devModeOn) {
                MyLog.d(
                    TAG,
                    "actFromDiffScreen=$actFromDiffScreen_tmp, listForStage.isEmpty() = ${listForStage.isEmpty()}",
                )
              }
              ...
              showErrAndSaveLog(
                  TAG,
                  "require commit all error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.commit_err) + ": " + e.localizedMessage,
                  requireShowToast,
                  curRepo.id,
              )
            }
            ...
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require commit error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.commit_failed) + ": " + e.localizedMessage,
                  requireShowToast,
                  curRepo.id,
              )
            }
            ...
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require Continue Merge error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.continue_merge_err) +
                      ": " +
                      e.localizedMessage,
                  requireShowToast,
                  repoId,
            ...
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require Rebase Continue error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.rebase_continue_err) +
                      ": " +
                      e.localizedMessage,
                  requireShowToast,
                  repoId,
            ...
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require Cherrypick Continue error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.cherrypick_continue_err) +
                      ": " +
                      e.localizedMessage,
                  requireShowToast,
                  repoId,
  ...
  val getActuallyList = { if (enableFilterState.value) filterList.value else itemList.value }

  val getActuallyListState = { if (enableFilterState.value) filterListState else itemListState }
  ...
  if (showRebaseSkipDialog.value) {
    val closeDialog = { showRebaseSkipDialog.value = false }
          ...
          try {
            ...
            Repository.open(repoFullPath).use { repo ->
              ...
              if (usernameFromConfig.isBlank() || emailFromConfig.isBlank()) {
                ...
                initSetUsernameAndEmailDialog(curRepo) { showRebaseSkipDialog.value = true }
              } else {
                val readyRet =
                    Libgit2Helper.rebaseSkip(
                        repo,
                        activityContext,
                        usernameFromConfig,
                        emailFromConfig,
  ...
  if (
      showMergeAbortDialog.value || showRebaseAbortDialog.value || showCherrypickAbortDialog.value
  ) {
    val closeDialog = {
      showMergeAbortDialog.value = false
      showRebaseAbortDialog.value = false
    ...
    ConfirmDialog(
      ...
      //            val nullMergeTrueRebaseFalseCherrypick = if(showMergeAbortDialog.value) null
      // else if(showRebaseAbortDialog.value) true else false
      val nullMergeTrueRebaseFalseCherrypick =
          if (showMergeAbortDialog.value) null else showRebaseAbortDialog.value
        ...
        doActWithLock(curRepo) {
          if (nullMergeTrueRebaseFalseCherrypick == null) { // merge abort
            try {
              ...
              showErrAndSaveLog(
                  TAG,
                  "require abort_merge error: " + e.stackTraceToString(),
                  activityContext.getString(R.string.abort_merge_failed) +
                      ": " +
                      e.localizedMessage,
                  requireShowToast,
                  curRepo.id,
            ...
            try {
              ...
              showErrAndSaveLog(
                  ...
                  activityContext.getString(R.string.cherrypick_abort_err) +
                      ": " +
                      e.localizedMessage,
                  requireShowToast,
                  repoId,
  ...
  if (showMergeAcceptTheirsOrOursDialog.value) {
    ...
    ConfirmDialog(
        title =
            if (acceptTheirs) stringResource(R.string.accept_theirs)
            else stringResource(R.string.accept_ours),
        text = stringResource(R.string.ask_do_operation_for_selected_conflict_items),
        okTextColor = MyStyleKt.TextColor.danger(),
        onCancel = { showMergeAcceptTheirsOrOursDialog.value = false },
    ) { // onOk
  ...
  if (showSetUpstreamDialog.value) {
    ...
    SetUpstreamDialog(
        ...
        closeDialog = { showSetUpstreamDialog.value = false },
        onCancel = {
    ...
    RequireCommitMsgDialog(
        ...
        onOk = { curRepo, msgOrAmendMsg, requirePush, requireSync ->
          ...
          doJobThenOffLoading(
              loadingOn,
              loadingOff,
              activityContext.getString(R.string.committing),
          ) {
            doActWithLock(curRepo) {
              try {
                ...
                //                            return@launch
                //                        }
                //                        val requireDoSync:Boolean =
                // Cache.getByType<Boolean>(Cache.Key.changeListInnerPage_RequireDoSyncAfterCommit)?:false

                // 执行commit
                //                        val commitSuccess = doCommit(false, cmtMsg,
                ...
                val commitSuccess =
                    ChangeListFunctions.doCommit(
                        ...
                        email = email,
                        requireShowToast = requireShowToast,
                        pleaseSetUsernameAndEmailBeforeCommit =
                            pleaseSetUsernameAndEmailBeforeCommit,
                        initSetUsernameAndEmailDialog = initSetUsernameAndEmailDialog,
                        amendCommit = amendCommit,
                        overwriteAuthor = overwriteAuthor,
                ...
                if (commitSuccess) { // 提交成功
                  ...
                  if (requireSync) {
                    ...
                    ChangeListFunctions.doSync(
                        ...
                        loadingOff = loadingOff,
                        requireCloseBottomBar = true,
                        trueMergeFalseRebase =
                            !PrefUtil.getGlobalGitConfigPullWithRebase(AppModel.realAppContext),
                        curRepoFromParentPage = curRepo,
                        requireShowToast = requireShowToast,
                        activityContext = activityContext,
                        bottomBarActDoneCallback = bottomBarActDoneCallback,
                  ...
                  if (requireSync) {
                    requireShowToast(
                        activityContext.getString(R.string.sync_canceled_by_commit_failed)
                    )
                  } else if (requirePush) {
                    requireShowToast(
                        activityContext.getString(R.string.push_canceled_by_commit_failed)
                    )
                  }
                }
  ...
  if (showUsernameAndEmailDialog.value) {
    ...
    AskGitUsernameAndEmailDialogWithSelection(
        ...
        onErrorCallback = { e ->
          ...
          MyLog.e(
              TAG,
              "set username and email err (from ChangeList page): ${e.stackTraceToString()}",
          )
        },
        onFinallyCallback = {},
        onSuccessCallback = {
    ...
    ConfirmDialog(
        ...
        textCompose = {
          CopyScrollableColumn {
            ...
            Text(
                text =
                    stringResource(R.string.will_cherrypick_changes_of_selected_files_are_you_sure),
                modifier = Modifier.padding(horizontal = MyStyleKt.defaultHorizontalPadding),
            )
            ...
            if (ret.hasError()) {
              ...
              if (ret.code != Ret.ErrCode.alreadyUpToDate) { // 如果错误码不是 Already up-to-date ，就log下
                ...
                createAndInsertError(
                    repoId,
                    "cherrypick files changes of '$shortParent..$shortTarget' err: " + ret.msg,
                )
              }
            } else {
    ...
    GitIgnoreDialog(
        ...
        getIgnoreItems = { _: String -> // 参数是仓库workdir完整路径，是用来取仓库相对路径的，这里不需要，因为已经有仓库下相对路径
          ...
          selectedItemList.map {
            IgnoreItem(pathspec = it.relativePathUnderRepo, isFile = it.toFile().isFile)
          }
        },
        onCatch = { e: Exception ->
        ...
        onFinally = { changeListRequireRefreshFromParentPage(curRepo) },
    )
  }

  val showCreatePatchDialog = rememberSaveable { mutableStateOf(false) }
  val savePatchPath = rememberSaveable {
    mutableStateOf("")
  } // 给这变量一赋值app就崩溃，原因不明，报的是"java.lang.VerifyError: Verifier rejected class"之类的错误，日，后来升级gradle解决了
    ...
    ConfirmDialog(
        ...
        textCompose = {
          ScrollableColumn {
            MySelectionContainer {
              Text(
                  text = stringResource(R.string.will_create_patch_for_selected_files_are_you_sure)
              )
            }
          }
        },
        ...
        try {
          ...
          val savePatchRet =
              ChangeListFunctions.createPath(
                  ...
                  fromTo =
                      if (
                          left == Cons.git_IndexCommitHash &&
                              right == Cons.git_LocalWorktreeCommitHash
                      )
  ...
  if (showCheckoutFilesDialog.value) {
    ConfirmDialog2(
        ...
        textCompose = {
          ScrollableColumn {
            //                    PaddingText(stringResource(R.string.target)+":", fontWeight =
            ...
            SingleSelection(
                ...
                text = { idx, item ->
                  ...
                  activityContext.getString(if (isLeft) R.string.left else R.string.right) +
                      ": " +
                      shortHash +
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.checking_out)) {
        doActWithLock(curRepo) job@{
          val pathSpecs = selectedItemList.map { it.relativePathUnderRepo }
          Repository.open(curRepo.fullSavePath).use { repo ->
            val checkoutTargetHash =
                if (
                    targetHash == Cons.git_IndexCommitHash
                ) { // if checkout index, no need resolve it to hash
                  targetHash
                } else {
  ...
  //    val firstVisible = remember {derivedStateOf { if(enableFilterState.value)
  // filterListState.value.firstVisibleItemIndex else itemListState.firstVisibleItemIndex }}
  //    ScrollListener(
  ...
  val selectAll = {
    ...
    list.toList().forEachBetter { selectItem(it) }
  ...
  if (showUnstageConfirmDialog.value) {
    ConfirmDialog(
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.unstaging)) {
        doActWithLock(curRepo) { doUnstage(curRepo, selectedItemList.value.toList()) }
      }
    }
  }

  val credentialList =
      mutableCustomStateListOf(stateKeyTag, "credentialList", listOf<CredentialEntity>())
  val selectedCredentialIdx = rememberSaveable { mutableIntStateOf(0) }

  //    val fullPathForImport = StateUtil.getRememberSaveableState("")
  val importList =
      mutableCustomStateListOf(stateKeyTag, "importList", listOf<StatusTypeEntrySaver>())
  // 导入仓库后跳转到对应仓库
      ...
      doJobThenOffLoading(loadingOn, loadingOff, activityContext.getString(R.string.importing)) {
        ...
        try {
          importList.forEachBetter {
            val result =
                repoDb.importRepos(
                    dir = it.canonicalPath,
                    isReposParent = false,
                    repoNameSuffix = repoNameSuffix,
          ...
          Msg.requireShowLongDuration(
              replaceStringResList(
                  activityContext.getString(R.string.n_imported),
                  listOf("" + importRepoResult.success),
              )
          )
        } catch (e: Exception) {
          ...
          MyLog.e(
              TAG,
              "import repo(s) from ChangeList err: importRepoResult=$importRepoResult, err=" +
                  e.stackTraceToString(),
          )
        } finally {
          // require refresh repo list for go to submodule after import
          // since only worktree(ChangeList) can go to sub, so only need pass this param to
          // ChangeList page, index and treeToTree page no need yet
            ...
            if (importSuccess.value && jumpAfterImportRepo.value && importList.isNotEmpty()) {
              ...
              if (firstItem != null) {
                changeStateTriggerRefreshPage(
                    needReQueryRepoList,
                    requestType = StateRequestType.jumpAfterImport,
                    data = firstItem.canonicalPath,
                )
              }
            } else {
  ...
  val getSelectedFilesCount = { selectedItemList.value.size }
  ...
  if (showOpenAsDialog.value) {
    OpenAsDialog(
        ...
        fileName = openAsDialogFileName.value,
        filePath = openAsDialogFilePath.value,
    ) {
      // onClose
  ...
  fun diffFiles(
      curItem: StatusTypeEntrySaver,
      itemListOrFilterList: List<StatusTypeEntrySaver>,
      commit1OidStr: String,
      commit2OidStr: String,
      fromTo: String,
  ) {
    ...
    //                                SharedState.homeChangeList_itemList = if(enableFilter)
    // filterList.value else itemList.value
    SharedState.homeChangeList_itemList = itemList.value
    SharedState.homeChangeList_indexHasItem = changeListPageHasIndexItem

    // 准备导航到diff页面的参数
    //                                val diffableList = itemList.filter {item -> item.changeType ==
    // Cons.gitStatusModified || item.changeType == Cons.gitStatusNew || item.changeType ==
    // Cons.gitStatusDeleted}
  ...
  val menuKeyTextList =
      listOf(
          ...
          Libgit2Helper.getLeftToRightFullHash(
              if (fromTo == Cons.gitDiffFromHeadToIndex) Cons.git_HeadCommitHash
              else stringResource(R.string.left),
              stringResource(R.string.local),
          ),
          Libgit2Helper.getLeftToRightFullHash(
              if (fromTo == Cons.gitDiffFromHeadToIndex) stringResource(R.string.index)
              else stringResource(R.string.right),
              stringResource(R.string.local),
          ),
  ...
  val menuKeyActList =
      listOf(
          ...
          importAsRepo@{ initImportAsRepo(listOf(it)) },
          goToSub@{ goToSub(it) },
      )
  val menuKeyEnableList: List<(StatusTypeEntrySaver) -> Boolean> =
      listOf(
          openEnabled@{
            true
          }, // 改成点击后动态检测文件是否存在，要不然在index页面，即使文件状态为删除，但worktree实际可能存在文件，这时文件应该仍可编辑，但却点不了按钮
          openAsEnabled@{ true },

          // 只有worktree的cl页面支持在Files页面显示文件，index页面由于是二级页面，跳转不了，干脆禁用了
          showInFilesEnabled@{
            fromTo == Cons.gitDiffFromIndexToWorktree
          }, // 对所有条目都启用showInFiles，不过会在点击后检查文件是否存在，若不存在不会跳转
          leftToLocal@{
            fromTo == Cons.gitDiffFromTreeToTree || fromTo == Cons.gitDiffFromHeadToIndex
          },
          rightToLocal@{
            fromTo == Cons.gitDiffFromTreeToTree || fromTo == Cons.gitDiffFromHeadToIndex
          },
          ...
          goToSub@{
            fromTo == Cons.gitDiffFromIndexToWorktree && it.toFile().isDirectory
          }, // only Worktree page can switch repo, and only dir maybe is repo
      )
    ...
    SelectedFileItemsDialog(
        ...
        fileIconOnClick = {
          ...
          if (!found.value && enableFilterState.value) {
            val index = itemList.value.indexOfFirst { item -> predicate(item) }
  ...
  val countNumOnClickForBottomBar = {
    ...
    // ${it.relativePathUnderRepo.removeSuffix(it.fileName)}").appendLine()
    //        }
  ...
  if (showRevertAlert.value) {
    ConfirmDialog(
        ...
        text =
            stringResource(R.string.will_revert_modified_or_deleted_and_rm_new_files_are_you_sure),
        okTextColor = MyStyleKt.TextColor.danger(),
      ...
      doJobThenOffLoading(
        ...
        doActWithLock(curRepo) { doRevert(curRepo, selectedItemList.value.toList()) }
      }
    }
  }
      ...
      doJobThenOffLoading(loadingOn = loadingOn, loadingOff = loadingOff) {
        ...
        try {
          ...
          Repository.open(curRepoFromParentPage.fullSavePath).use { repo ->
            ...
            MyLog.d(
                TAG,
                "#RemoveFromGitDialog: will remove files from repo workdir: '${repoWorkDirFullPath}'",
            )
    ...
    if (isLoading.value) {
      ...
      if (hasError.value) { // 有错误则显示错误（例如无仓库、无效树id都会导致出错）
        FullScreenScrollableColumn(contentPadding) {
          MySelectionContainer { Text(errMsg.value, color = MyStyleKt.TextColor.error()) }
        }
        ...
        if (itemList.value.isEmpty()) { // 列表为空
          FullScreenScrollableColumn(contentPadding) {
            if (fromTo == Cons.gitDiffFromIndexToWorktree) { // worktree
              MySelectionContainer { Text(text = stringResource(R.string.work_tree_clean)) }
                ...
                Row(modifier = Modifier.padding(top = 10.dp)) {
                  if (changeListPageHasIndexItem.value) { // index不为空
                    ClickableText(
                        ...
                        modifier =
                            MyStyleKt.ClickableText.modifierNoPadding.clickable { // 导航到Index页面
              ...
              if (!dbIntToBool(curRepoOnUi.isDetached)) {
                ...
                Column(
                  ...
                  if (
                      curRepoOnUi.ahead != 0 ||
                          curRepoOnUi.behind != 0
                          // 上游为空，显示sync
                          || curRepoUpstreamOnUi.branchRefsHeadsFullRefSpec.isBlank()
                          // 上游不为空，但未发布，显示sync
                          ||
                          (curRepoUpstreamOnUi.branchRefsHeadsFullRefSpec.isNotBlank() &&
                              !curRepoUpstreamOnUi.isPublished)
                  ) {
                    // 查询下仓库是否领先或落后于上游
                    if (curRepoOnUi.ahead != 0) {
                      MySelectionContainer {
                        Row {
                          Text(
                              text = stringResource(R.string.local_ahead) + ": " + curRepoOnUi.ahead
                          )
                        }
                      }
                    }
                    if (curRepoOnUi.behind != 0) {
                      MySelectionContainer {
                        Row { // 换行
                          Text(
                              text =
                                  stringResource(R.string.local_behind) + ": " + curRepoOnUi.behind
                          )
                        }
                      }
                    }
                    ...
                    if (curRepoOnUi.behind != 0) {
                      Row(
                        ...
                        LongPressAbleIconBtn(
                          ...
                          doTaskOrShowSetUsernameAndEmailDialog(curRepo) {
                            doJobThenOffLoading(
                                loadingOn,
                                loadingOff,
                                activityContext.getString(R.string.rebasing),
                            ) {
                              doActWithLock(curRepo) {
                        ...
                        //                                               // Text(text = splitSign,
                        // modifier = Modifier.padding(horizontal = splitHorizonPadding))
                        ...
                        LongPressAbleIconBtn(
                          ...
                          doTaskOrShowSetUsernameAndEmailDialog(curRepo) {
                            doJobThenOffLoading(
                                loadingOn,
                                loadingOff,
                                activityContext.getString(R.string.merging),
                            ) {
                              doActWithLock(curRepo) {
                                ChangeListFunctions.doMerge(
                                    requireCloseBottomBar = true,
                  ...
                  Row(
                    ...
                    val syncText =
                        stringResource(
                            if (upstreamNotSet) R.string.set_upstream_and_sync else R.string.sync
                        )
                    //                                    val syncIcon = if(upstreamNotSet)
                    // Icons.Filled.CloudSync else Icons.Filled.Sync
                    //                                    val syncIcon = if(upstreamNotSet)
                    // Icons.Filled.CloudSync else
                    ...
                    LongPressAbleIconBtn(
                      ...
                      doTaskOrShowSetUsernameAndEmailDialog(curRepo) {
                        doJobThenOffLoading(
                            loadingOn,
                            loadingOff,
                            activityContext.getString(R.string.syncing),
                        ) {
                          doActWithLock(curRepo) {
                            try {
                              //                                                   //
                              ...
                              ChangeListFunctions.doSync(
                                  ...
                                  requireCloseBottomBar = true,
                                  trueMergeFalseRebase =
                                      !PrefUtil.getGlobalGitConfigPullWithRebase(
                                          AppModel.realAppContext
                                      ),
                                  curRepoFromParentPage = curRepo,
                                  requireShowToast = requireShowToast,
                                  activityContext = activityContext,
                              ...
                              showErrAndSaveLog(
                                  ...
                                  showMsg =
                                      activityContext.getString(R.string.sync_failed) +
                                          ": " +
                                          e.localizedMessage,
                                  showMsgMethod = requireShowToast,
                                  repoId = curRepo.id,
                    ...
                    if (!upstreamNotSet) {
                      LongPressAbleIconBtn(
                        ...
                        doJobThenOffLoading(
                            loadingOn,
                            loadingOff,
                            activityContext.getString(R.string.pushing),
                        ) {
                          doActWithLock(curRepo) {
                            try {
                              //                                                            val
                              // success = doPush(true, null)
                              val success =
                                  ChangeListFunctions.doPush(
                                      requireCloseBottomBar = true,
                                      upstreamParam = null,
                              ...
                              showErrAndSaveLog(
                                  ...
                                  showMsg =
                                      activityContext.getString(R.string.push_failed) +
                                          ": " +
                                          e.localizedMessage,
                                  showMsgMethod = requireShowToast,
                                  repoId = curRepo.id,
              ...
              if (
                  !dbIntToBool(curRepoOnUi.isDetached) &&
                      curRepoUpstreamOnUi.branchRefsHeadsFullRefSpec.isNotBlank()
              ) {
                MySelectionContainer {
                  Row(Modifier.padding(top = 10.dp)) {
                    //                                LongPressAbleIconBtn(
                    //                                    iconModifier = iconModifier,
                    //                                    tooltipText = stringResource(id =
                    // R.string.check_update),
                    //                                    icon =  Icons.Filled.Downloading,
                    //                                    iconContentDesc = stringResource(id =
                    // R.string.check_update),
                    //
                    //                                ) {}
                    ClickableText(
                        ...
                        modifier =
                            MyStyleKt.ClickableText.modifierNoPadding.clickable {
                              val curRepo = curRepoFromParentPage.value
                              // fetch
                              doJobThenOffLoading(
                                  loadingOn,
                                ...
                                doActWithLock(curRepo) {
                                  try {
                                    ...
                                    if (fetchSuccess) {
                                      requireShowToast(
                                          activityContext.getString(R.string.fetch_success)
                                      )
                                    } else {
                                      requireShowToast(
                                          activityContext.getString(R.string.fetch_failed)
                                      )
                                    }
                                  } catch (e: Exception) {
                                    showErrAndSaveLog(
                ...
                Row(
                  ...
                  LongPressAbleIconBtn(
                    ...
                    navController.navigate(
                        Cons.nav_BranchListScreen + "/" + curRepoFromParentPage.value.id
                    )
                  }
              ...
              MySelectionContainer { Text(text = stringResource(id = R.string.index_clean)) }
          ...
          val enableFilter =
              filterModeActuallyEnabled(
                  filterOn = changeListPageFilterModeOn.value,
                  keyword = keyword,
              )

          val lastNeedRefresh = rememberSaveable { mutableStateOf("") }
          val itemListOrFilterList =
              filterTheList(
          ...
          MyLazyColumn(
            ...
            ChangeListItem(
                ...
                isFileSelectionMode = isFileSelectionMode,
                //                            filesPageAddFileToSelectedListIfAbsentElseRemove =
                // filesPageAddFileToSelectedListIfAbsentElseRemove,
                ...
                onLongClick = lc@{
                      ...
                      if (
                          fromTo == Cons.gitDiffFromTreeToTree &&
                              !proFeatureEnabled(treeToTreeBottomBarActAtLeastOneTestPassed())
                      ) {
                        return@lc
                      }
              ...
              if (isFileSelectionMode.value) {
                ...
                //                        Cache.Map.set(Cache.Map.Key.diffScreen_UnderRepoPath,
                // item.relativePathUnderRepo)
                ...
                // on worktree page, if new file(untracked), open with editor instead view
                // difference
                //                            else if(isWorktreePage && it.changeType ==
                // Cons.gitStatusNew){
                //                                val initMergeMode = false
                //                                openFileWithInnerEditor(it.canonicalPath,
                // initMergeMode)
                //                            }
                else { // 非冲突条目，预览diff
          ...
          if (isFileSelectionMode.value) {
            ...
            val iconTextList: List<String> =
                if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    listOf( // changelist page
                        //        stringResource(id =
                        // R.string.commit_selected_and_index_items_then_sync),
                    ...
                    listOf( // index page
                        //            stringResource(id =
                        // R.string.commit_all_index_items_then_sync),
                        stringResource(id = R.string.commit_all_index_items),
                        stringResource(id = R.string.select_all),
                    )
                else
                    listOf( // if(fromTo == Cons.gitDiffFromTreeToTree)
                        stringResource(R.string.import_as_repo),
            ...
            val enableAcceptOursTheirs = {
              ...
              (repoState == Repository.StateT.MERGE.bit ||
                  repoState == Repository.StateT.REBASE_MERGE.bit ||
                  repoState == Repository.StateT.CHERRYPICK.bit) && hasConflictItemsSelected()
            }

            val iconEnableList: List<() -> Boolean> =
                if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    listOf( // worktree页面
                    ...
                    listOf( // index页面
                        { true }, // commit
                        //                        {changeListPageHasIndexItem.value},  //x
                        // 废弃，因为支持amend了，就算index无条目也可提交）commit，只有当存在index条目时才启用
                        { true }, // select all
                    )
                else
                    listOf( //  if(fromTo == Cons.gitDiffFromTreeToTree)
                        ...
                        }, // cherrypick，只有存在parents的情况下才可cherrypick
                        selectedListIsNotEmpty, // create patch
                        { true }, // select all
                    )

            val moreItemEnableList: List<() -> Boolean> =
                (if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    listOf( // worktree page
                    ...
                    listOf( // tree to tree page
                    ))

            val iconOnClickList: List<() -> Unit> =
                if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    listOf( // ChangeList页面的底栏选项 ( worktree页面 )
                        commit@{
                          val curRepo = curRepoFromParentPage.value
                          doJobThenOffLoading(
                              loadingOn = loadingOn,
                              loadingOff = loadingOff,
                              loadingText = activityContext.getString(R.string.committing),
                          ) {
                            doActWithLock(curRepo) {
                              val stageSuccess =
                                  ChangeListFunctions.doStage(
                              ...
                              if (!stageSuccess) {
                                bottomBarActDoneCallback(
                                    activityContext.getString(R.string.stage_failed),
                                    curRepo,
                                )
                              } else {
                  ...
                  listOf( // tree to tree页面的底栏选项
                      importAsRepo@{ initImportAsRepo(selectedItemList.value.toList()) },
                      checkout@{
                        val curRepo = curRepoFromParentPage.value
                        // when diff a commit to local (commit..local), if I clicked the checkout,
                        // actually I expect checkout the left version,
                        //  in other cases, I expect checkout the right version
                        val target =
                            if (localAtDiffRight.value) getCommitLeft() else getCommitRight()
                        ...
                        if (commitParentList.isNotEmpty()) {
                          ...
                          Msg.requireShowLongDuration(
                              activityContext.getString(
                                  R.string.cherrypick_only_work_for_diff_to_parents
                              )
                          )
                        }
                      },
                      createPatch@{ showCreatePatchDialog.value = true },
                      selectAll@{
                        // impl select all
                        selectAll()
                      },
            ...
            val moreItemTextList =
                (if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    ...
                    listOf(
                        stringResource(R.string.unstage),
                        stringResource(R.string.create_patch),
                        stringResource(R.string.import_as_repo),
                    )
                else
                    listOf( // tree to tree，无选项
                    ))

            val moreItemOnClickList: List<() -> Unit> =
                (if (fromTo == Cons.gitDiffFromIndexToWorktree)
                    listOf(
                        acceptOurs@{
                          if (!UserUtil.isPro()) {
                            Msg.requireShowLongDuration(
                                activityContext.getString(R.string.this_feature_is_pro_only)
                            )
                            return@acceptOurs
                          }
                        ...
                        createPatch@{ showCreatePatchDialog.value = true },
                        ignore@{ showIgnoreDialog.value = true },
                        removeFromGit@{ showRemoveFromGitDialog.value = true },
                        importAsRepo@{ initImportAsRepo(selectedItemList.value.toList()) },
                    )
                else if (fromTo == Cons.gitDiffFromHeadToIndex)
                    listOf(
                        unstage@{ showUnstageConfirmDialog.value = true },
                        createPatch@{ showCreatePatchDialog.value = true },
                        importAsRepo@{ initImportAsRepo(selectedItemList.value.toList()) },
                    )
                else
                    listOf( // fromTo == Cons.gitDiffFromTreeToTree
                    ))
  ...
  LaunchedEffect(refreshRequiredByParentPage) {
    ...
    try {
      ...
      // if navi to Difference page or internal Editor then navi back, actually most time no need
      // reload page
      if (naviTarget.value == Cons.ChangeListNaviTarget_NoNeedReload) {
        ...
        // 最后一个else是TreeToTree
        val lastClickedItemKey =
            if (fromTo == Cons.gitDiffFromIndexToWorktree)
                SharedState.homeChangeList_LastClickedItemKey.value
            else if (fromTo == Cons.gitDiffFromHeadToIndex)
                SharedState.index_LastClickedItemKey.value
            else SharedState.treeToTree_LastClickedItemKey.value
        ...
        if (requestType == StateRequestType.indexToWorkTree_CommitAll) {
          Cache.set(
              Cache.Key.changeListInnerPage_requireDoActFromParent,
              PageRequest.indexToWorkTree_CommitAll,
          )
      ...
      // 初始化
      doJobThenOffLoading(
          loadingOn = { loadingText -> loadingOnByToken(loadingToken, loadingText) },
          loadingOff = { loadingOffByToken(loadingToken) },
          loadingText = activityContext.getString(R.string.loading),
      ) {
        try {
          changeListInit(
          ...
          showErrAndSaveLog(
              logTag = TAG,
              logMsg =
                  "#LaunchedEffect: $prefix, params are: fromTo=${fromTo}, commit1OidStr=${commit1OidStr}, commit2OidStr=${commit2OidStr}, curRepo=${curRepo}\nerr is: " +
                      e.stackTraceToString(),
...
private suspend fun changeListInit(
  ...
  // 没必要给整个变量重新赋值
  //            curRepoFromParentPage.value = RepoEntity(id = "", fullSavePath = "", repoName =
  // curRepoFromParentPage.value.repoName)
  ...
  val credentialListFromDb =
      AppModel.dbContainer.credentialRepository.getAll(
          includeNone = true,
          includeMatchByDomain = true,
      )
  ...
  if (fromTo == Cons.gitDiffFromTreeToTree) {
    ...
    Repository.open(repoFromDb.fullSavePath).use { repo ->
      ...
      //                            isDiffToHead.value = commit1OidStr == headCommitId ||
      // commit2OidStr == headCommitId
      //                        }
      //                    }
      ...
      if (commit1OidStr == commit2OidStr) {
        ...
        // 如果1或2是worktree ( local )，则用 treeToWorkTree 函数
          ...
          Libgit2Helper.CommitUtil.isLocalCommitHash(commit1OidStr) ||
        ...
        if (isActuallyIndexToLocal.not() && tree == null) {
          ...
          setErrMsg(
              activityContext.getString(R.string.error_invalid_commit_hash) +
                  " '$leftCommit', (err at: 3277)"
          )
          return
        }
        ...
        val cl =
            if (isActuallyIndexToLocal) {
              ...
              Libgit2Helper.getTreeToTreeChangeList(
                  repo,
                  repoId,
                  tree!!,
                  null,
        ...
        if (tree1 == null) {
          MyLog.w(
              TAG,
              "#$funName, tree to tree diff, query tree1 err! commit1OidStr=$commit1OidStr",
          )
          setErrMsg(
              activityContext.getString(R.string.error_invalid_commit_hash) +
                  " '$commit1OidStr', (err at: 5145)"
          )
          return
        }
        ...
        if (tree2 == null) {
          MyLog.w(
              TAG,
              "#$funName, tree to tree diff, query tree2 err! commit2OidStr=$commit2OidStr",
          )
          setErrMsg(
              activityContext.getString(R.string.error_invalid_commit_hash) +
                  " '$commit2OidStr', (err at: 7136)"
          )
          return
        }
        ...
        // 只有和parents比较时才需要查询parents（目前20240807只通过点击commit
        // item触发）；其他情况（手动输入两个commit、长按commit条目出现的diff to
        // local）都不需要查询commit，这时把commitForQueryParents直接传空字符串就行
    ...
    //                val settings =
    // settingsRepository.getOrInsertByUsedFor(Cons.dbSettingsUsedForChangeList)
    //                    ?: return@launch
    // 这个?:不是三元表达式，这里的作用是如果?:左边的内容为null，返回其右边的内容
    //                val changeListSettings =
    // MoshiUtil.changeListSettingsJsonAdapter.fromJson(settings.jsonVal) ?: ChangeListSettings()
    ...
    if (!isRepoReadyAndPathExist(curRepoFromParentPage.value)) {
      ...
      if (lastUsedRepoId.isBlank()) { // 没有之前使用的repoid
        ...
        if (
            repoFromDb == null || !isRepoReadyAndPathExist(repoFromDb)
        ) { // 进入这个分支可能是之前changelist页面使用的仓库被删除了，所以id无效了
          needQueryRepoFromDb = true
        } else { // 正常来说，如果是非第一次打开这个页面，并且之前选过有效仓库，会进入到这里查询出上次使用的仓库
          curRepoFromParentPage.value =
              repoFromDb // 执行到这，repoFromDb必然非null，否则会在上面的isRepoReadyAndPathExist()判断返回假而进入上面的if语句
        }
      }
      ...
      if (
          repoFromDb == null || !isRepoReadyAndPathExist(repoFromDb)
      ) { // 等于null，需要重查一个就绪的仓库，不过一般进入到这查出的仓库就不会等于null，这个判断只是以防万一
        needQueryRepoFromDb = true
      } else { // 在其他页面更新过仓库信息后，会进入到这里，例如在分支页面修改了仓库分支，或者在changelist页面标题栏切换了仓库
        curRepoFromParentPage.value =
            repoFromDb // 更新下仓库信息。(执行到这，repoFromDb必然非null，否则会在上面的isRepoReadyAndPathExist()判断返回假而进入上面的if语句)
      }
    }
    ...
    //            MyLog.d(TAG, "ChangeListInnerPage#Init: queryed Repo
    // id:"+curRepoFromParentPage.value.id)

    Repository.open(curRepoFromParentPage.fullSavePath).use { gitRepository ->
      // 废弃，新的逻辑是把冲突条目列在其他条目上面就行了：确认实现这的逻辑： 如果有冲突，会提示先去解决冲突
      //                changeListPageHasConflictItem.value =
      // Libgit2Helper.hasConflictItemInRepo(gitRepository)
      ...
      if (
          repoState.intValue == Repository.StateT.REBASE_MERGE.bit &&
      ...
      if (fromTo == Cons.gitDiffFromHeadToIndex) {
        itemList.value.clear()
        indexList?.let { itemList.value.addAll(it) }
      }

      // 最后查worktree是否为空，这个查的最慢，放最后。
      // 20240504:
      // 先查worktree，再查index是否为空，因为worktree有可能更改index。20250123：worktree修改index？用户不操作会自动修改？不可能啊，总之又改成先查index和仓库状态了，最后再查worktree列表，因为查worktree列表最慢
      // 检测index是否为空，如果不为空，会在图标有红点提示(最好红点，高亮图标也行)，如果worktree status为空(包含conflict条目) 且
      // index不为空，则会提示用户可去index区查看status
      ...
      if (fromTo == Cons.gitDiffFromIndexToWorktree) { // 查询worktree页面条目，就是从首页抽屉打开的changelist
        ...
        if (hasWorktreeItem) {
          val worktreeItems =
              Libgit2Helper.getWorktreeChangeList(
                  gitRepository,
                  wtStatusList,
                  curRepoFromParentPage.id,
              )
        ...
        curRepoUpstream.value =
            Libgit2Helper.getUpstreamOfBranch(gitRepository, curRepoFromParentPage.branch)
      }
    }
  }
  ...
  val pageChangedNeedAbort =
      updateSelectedList(
          selectedItemList = selectedItemList.value,
          itemList = itemList.value,
          quitSelectionMode = quitSelectionMode,
          match = { oldSelected, item ->
            oldSelected.relativePathUnderRepo == item.relativePathUnderRepo
          },
...
private fun getBackHandler(
  ...
  val backHandlerOnBack: () -> Unit = {
    if (isFileSelectionMode.value) {
      ...
      if (
          fromTo != Cons.gitDiffFromIndexToWorktree
      ) { // TreeToTree or Index，非WorkTree页面，非顶级页面，点击返回上级页面
        doJobWithMainContext { naviUp() }
      } else { // WorkTree，顶级页面，双击退出app
        // 如果在两秒内按返回键，就会退出，否则会提示再按一次可退出程序
        if (
            backStartSec.longValue > 0 && getSecFromTime() <= backStartSec.longValue
        ) { // 大于0说明不是第一次执行此方法，那检测是上次获取的秒数，否则直接显示“再按一次退出app”的提示
          exitApp()
        } else {
