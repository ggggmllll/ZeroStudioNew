...
fun EditorTitle(
  ...
  if (editorPageShowingFilePath.value.isNotBlank()) {
    ...
    //        val filePath = getFilePathStrBasedRepoDir(editorPageShowingFilePath.value,
    // returnResultStartsWithSeparator = true)

    val filePathNoFileNameNoEndSlash =
        FsUtils.getPathWithInternalOrExternalPrefixAndRemoveFileNameAndEndSlash(
    ...
    Column(
                      ...
                      if (isPreviewModeOn) {
                        runBlocking {
                          defaultTitleDoubleClick(
                              scope,
                              previewNavStack.getCurrentScrollState(),
                              previewLastScrollPosition,
                          )
                        }
                      } else {
                    ...
                    onLongClick =
                        if (isPreviewModeOn.not())
                            ({
                              ...
                              // because value represent disable when it's true, so, need a `not()`
                              // call to know on/off
                              Msg.requireShow(
                                  activityContext.getString(R.string.software_keyboard) +
                                      ": ${onOffText(newValue.not())}"
                              )
                            })
                        else null,
                ) { // onClick
      ...
      if (editorSearchMode) {
        FilterTextField(
            ...
            showClear = false,
            containerModifier =
                Modifier.fillMaxWidth().onPreviewKeyEvent opke@{ keyEvent ->
        ...
        ScrollableRow {
          if (isPreviewModeOn) {
            ...
            if (disableSoftKb.value) {
              SmallIcon(
                  ...
                  contentDescription =
                      stringResource(R.string.software_keyboard) +
                          ": ${onOffText(disableSoftKb.value.not())}",
              )
            }
          ...
          Text(
              ...
              //                        color = if(editorPageMergeMode)
              // MyStyleKt.ChangeListItemColor.getConflictColor(inDarkTheme) else Color.Unspecified
          )
        }
        ScrollableRow {
          Text(
    ...
    // 菜单列表
    DropdownMenu(expanded = dropDownMenuExpandState.value, onDismissRequest = { closeMenu() }) {
      DropdownMenuItem(
      ...
      if (UserUtil.isPro() && (dev_EnableUnTestedFeature || editorMergeModeTestPassed)) {
        DropdownMenuItem(
            ...
            trailingIcon = { SimpleCheckBox(editorPageMergeMode.value) },
            onClick = {
      ...
      DropdownMenuItem(
          ...
          trailingIcon = { SimpleCheckBox(patchModeOn.value) },
          onClick = {
            val newValue = !patchModeOn.value
            patchModeOn.value = newValue

            // 更新配置文件
            SettingsUtil.update { it.editor.patchModeOn = newValue }
      ...
      DropdownMenuItem(
          ...
          trailingIcon = { SimpleCheckBox(readOnly.value) },
          onClick = {
    ...
    if (recentFileListFilterModeOn) {
      ...
      ScrollableRow(
          modifier =
              Modifier.combinedClickable(
                      ...
                      onDoubleClick = {
                        defaultTitleDoubleClick(
                            scope,
                            getActuallyRecentFilesListState(),
        ...
        Text(
            text =
                stringResource(
                    if (recentFileListIsEmpty) R.string.editor else R.string.recent_files
                ),
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
        )
