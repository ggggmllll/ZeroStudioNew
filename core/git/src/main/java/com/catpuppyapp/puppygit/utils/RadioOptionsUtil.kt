...
class RadioOptionsUtil {
  companion object {
    ...
    //        //有些单选框，没编号属性，只能对比字符串判断是否选中，如果用户语言奇怪，可能会很麻烦，所以强制在所有选项前加个数字编号，一般编号只需要占1位，选项类似：1:abc,
    // 2:def
    ...
    // 如果是已确定项的那种list，直接把idx和value都写到代码里即可，例如：listOf(Pair(optNumNoCredential, noCredential),
    // Pair(optNumNewCredential,newCredential), Pair(optNumSelectCredential,selectCredential))
