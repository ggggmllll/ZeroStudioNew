  ...
  doJobThenOffLoading job@{
    try {
      ...
      repo.use {
        ...
        val repoGitDirEndsWithSlash =
            Libgit2Helper.getRepoGitDirPathNoEndsWithSlash(it) + Cons.slash
        ...
        if (repoFromDb == null) {
          Msg.requireShowLongDuration(
              activityContext.getString(R.string.plz_import_repo_then_try_again)
          )
          return@job
        }
...
suspend fun goToFileHistoryByRelativePathWithMainContext(
    repoId: String,
...
fun naviToFileHistoryByRelativePath(repoId: String, relativePathUnderRepo: String) {
  ...
  doJobWithMainContext {
    AppModel.navController.navigate(
        Cons.nav_FileHistoryScreen + "/" + repoId + "/" + fileRelativePathKey
    )
  }
}
...
fun getLoadText(
    loadedCount: Int,
    actuallyEnabledFilterMode: Boolean,
...
fun openFileWithInnerSubPageEditor(
  ...
  doJobWithMainContext job@{
    if (
        onlyGoToWhenFileExists &&
            FilePath(filePath).toFuckSafFile(context).isActuallyReadable().not()
    ) {
    ...
    AppModel.navController.navigate(
        Cons.nav_SubPageEditor + "/$goToLine/$initMergeMode/$initReadOnly/$filePathKey"
    )
  }
}
...
suspend fun initSearch(
  ...
  withMainContext { token.value = tokenForThisSession }
  ...
  return {
    if (AppModel.devModeOn) {
      ...
      MyLog.v(
          TAG,
          "token.value==tokenForThisSession is '${token.value==tokenForThisSession}', if is false, may something wrong: token.value=${token.value}, tokenForThisSession=$tokenForThisSession",
      )
    }
  ...
  return if (enableFilter) {
    ...
    if (
        pageRefreshed ||
            ...
            (lastListSize != null && curListSize != lastListSize.intValue) ||
        ...
        (customTask
                ?: {
                  ...
                  search(
                      src = list,
                      match = match,
                      matchedCallback = { idx, item -> filterList.add(item) },
                      canceled = canceled,
                  )
                })
            .invoke()
...
fun getFilesScreenTitle(currentPath: String, activityContext: Context): String {
  ...
  return if (trimedSlashCurPath == FsUtils.getAppDataRootPathNoEndsWithSeparator()) {
    ...
    runCatching { FsUtils.splitParentAndName(currentPath).second }
        .getOrDefault("")
        .ifEmpty { activityContext.getString(R.string.files) }
  }
}
...
fun getEditorStateOnChange(
  ...
  return { newState, trueSaveToUndoFalseRedoNullNoSave, clearRedoStack, caller, from ->
    ...
    if (lastState.maybeNotEquals(newState)) {
      // if content changed, reset remembered last cursor at column，then next time navigate line by
      // Down/Up key will update the column to the expect value
...
suspend fun goToStashPage(repoId: String) {
  withMainContext { AppModel.navController.navigate(Cons.nav_StashListScreen + "/" + repoId) }
}

fun goToTreeToTreeChangeList(
    title: String,
    repoId: String,
    commit1: String,
    commit2: String,
...
fun goToCommitListScreen(
  ...
  doJobWithMainContext {
    ...
    // 注：如果fullOidKey传null，会变成字符串
    // "null"，然后查不出东西，返回空字符串，与其在导航组件取值时做处理，不如直接传空字符串，不做处理其实也行，只要“null“作为cache
    // key取不出东西就行，但要是不做处理万一字符串"null"作为cache key能查出东西，就歇菜了，总之，走正常流程取得有效cache key，cache value传空字符串，即可
    AppModel.navController.navigate(
...
fun goToDiffScreen(
  ...
  doJobWithMainContext {
    ...
    val isMultiMode: Boolean =
        if (DiffFromScreen.isFromFileHistory(fromScreen)) false
        else !DevFeature.singleDiff.state.value
  ...
  return { newPath: String ->
    ...
    try {
      try {
        ...
        lastPressedPath.value =
            if (
                newPath.length < oldPath.length &&
                    startIndexForFindEndOfLastPath < oldPath.length &&
              ...
              val indexOfSlash =
                  oldPath.indexOf(Cons.slashChar, startIndex = startIndexForFindEndOfLastPath)
              // 截出当前目录在新目录中的path，例如当前目录为 /abc/def 新目录为 /abc，则找到def，若当前目录为 /abc/def/ghi，新目录为
              // /abc，则截取 /abc/def
        ...
        MyLog.d(
            TAG,
            "#$funName: resolve `lastPressedPath` failed! oldPath='$oldPath', newPath='$newPath', err=${e.stackTraceToString()}",
        )
      }
      ...
      MyLog.e(
          TAG,
          "#$funName: change path failed! oldPath='$oldPath', newPath='$newPath', err=${e.stackTraceToString()}",
      )
...
fun goToErrScreen(repoId: String) {
  doJobWithMainContext { AppModel.navController.navigate(Cons.nav_ErrorListScreen + "/" + repoId) }
}
