...
@Composable
fun FileListItem(
    /** 若不为空字符串，将针对所有条目以此path为基础显示其相对路径，否则不显示。 应用场景：在递归搜索时使用此变量 */
  ...
  val alpha = 0.6f
  val iconColor =
      if (item.isHidden) LocalContentColor.current.copy(alpha = alpha)
      else LocalContentColor.current
  val fontColor =
      if (item.isHidden) {
        if (inDarkTheme) Color.White.copy(alpha = alpha) else Color.Black.copy(alpha = alpha)
  ...
  Row(
              ...
              .then(
                  if (isItemInSelected(item)) {
                    Modifier.background(MaterialTheme.colorScheme.primaryContainer)
                      ...
                      lastPathByPressBack.value == item.fullPath
                  ) { // show light background for last clicked path
    ...
    ListItemRow {
      ...
      // JSONObject(selFilePathListJsonObjStr.value).has(item.name), onCheckedChange = {
      //                                            addIfAbsentElseRemove(item)
      //                                        }) {
      //                                            Icon(imageVector = Icons.Outlined.CheckCircle,
      // contentDescription = stringResource(R.string.file_checked_indicator_icon))
      //                                        }
      //                                    }

      ListItemToggleButton(

          //                enabled = fromTo!=Cons.gitDiffFromTreeToTree,
          // //diff提交时，禁用点击图标启动长按模式，按钮会变灰色，太难看了，弃用
        ...
        IconOfItem(
            ...
            contentDescription =
                if (item.isDir) stringResource(R.string.folder_icon)
                else stringResource(R.string.file_icon),
            iconColor = iconColor,
        )
      }
      ...
      Column {
        Row { Text(text = item.name, fontSize = 20.sp, color = fontColor) }

        Row { Text(item.getShortDesc(), fontSize = 12.sp, color = fontColor) }

        if (fullPathOfTopNoEndSlash.isNotBlank()) {
          // 末尾必须分别移除path和/，若合并移除"path/"，当path和当前路径相同时会漏
          val relativePath =
              item.fullPath.removePrefix(fullPathOfTopNoEndSlash).removePrefix(Cons.slash).let {
                getParentPathEndsWithSeparator(it, trueWhenNoParentReturnEmpty = true)
              }

          if (relativePath.isNotEmpty()) {
            Row { Text(text = relativePath, fontSize = 12.sp, color = fontColor) }
          }
        }
      }
    }
