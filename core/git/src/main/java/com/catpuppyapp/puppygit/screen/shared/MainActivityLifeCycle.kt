...
fun doActIfIsExpectLifeCycle(
    expectLifeCycle: MainActivityLifeCycle,
    nextLifeCycle: MainActivityLifeCycle = MainActivityLifeCycle.NONE,
    act: () -> Unit,
) {
  MyLog.d(
      TAG,
      "#doActIfIsExpectLifeCycle(): current life cycle is '${currentMainActivityLifeCycle.value}'",
  )
...
fun setByPredicate(
    lifeCycle: MainActivityLifeCycle,
    predicate: (current: MainActivityLifeCycle) -> Boolean,
) {
