...
class EncryptUtil {
  companion object {
    ...
    fun encryptString(plaintext: String, password: String): String {
      ...
      val pwSpec: PBEKeySpec =
          PBEKeySpec(password.toCharArray(), salt, PBKDF2_ITERATIONS, ALGORITHM_KEY_SIZE)
      ...
      val ciphertextAndNonce: ByteArray =
    ...
    fun decryptString(base64CiphertextAndNonceAndSalt: String, password: String): String {
      // Decode the base64.
      val ciphertextAndNonceAndSalt: ByteArray =
          Base64.getDecoder().decode(base64CiphertextAndNonceAndSalt)

      // Retrieve the salt and ciphertextAndNonce.
      val salt: ByteArray = ByteArray(PBKDF2_SALT_SIZE)
      val ciphertextAndNonce: ByteArray =
          ByteArray(ciphertextAndNonceAndSalt.size - PBKDF2_SALT_SIZE)
