...
object CertMan {
  ...
  const val defaultCertUserDirName =
      "cert-user" // 存放用户证书的目录，自签证书之类的可以放这里，app会自动加载，自签证书并不是在加密上有问题，只是不被公众信任，用户如果自签，也并非一定不安全，另外：这块功能我没测试
  ...
  /** @certBundleDir 存放app内置证书的目录，app会从内置资源文件中将证书内容读取到这个目录 @certUserDir 存放user certs的目录 */
