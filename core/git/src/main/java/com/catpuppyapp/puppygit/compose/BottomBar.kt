...
fun BottomBar(
    ...
    iconVisibleList: List<() -> Boolean> =
        listOf(), // 这个默认就用空list()让所有条目都可以显示就很好，不要把默认行为调整成启用则显示禁用则隐藏，因为这个按钮本来就只显示4、5个，用户可能会习惯按某个位置对应某个功能，若默认隐藏禁用按钮，可能会按错
    ...
    moreItemVisibleList: List<() -> Boolean> =
        moreItemEnableList, // 这个菜单条目默认启用即显示，禁用即隐藏，这样感觉比较节省心智，因为有可能有很多菜单条目而且这个菜单用户一般是先看一下才会按，就算菜单条目变化一般也不会按错
    reverseMoreItemList: Boolean = false,
) {
  val scope = rememberCoroutineScope()

  val dropDownMenuExpandState = rememberSaveable { mutableStateOf(false) }
  val showDropDownMenu = { dropDownMenuExpandState.value = true }
  val closeDropDownMenu = { dropDownMenuExpandState.value = false }
  val switchDropDownMenu = { dropDownMenuExpandState.value = !dropDownMenuExpandState.value }
  ...
  val isIconHidden = { idx: Int -> iconVisibleList.isNotEmpty() && !iconVisibleList[idx]() }
  ...
  Box(
      modifier =
          Modifier.fillMaxSize()
              ...
              //   and I am not sure, if bottom bar lost focus, it can response the key event? if it
              // can't, no need add it.
    ...
    Row(
      ...
      Row(
        ...
        if (showSelectedCount) {
          ...
          Text(
              ...
              modifier =
                  MyStyleKt.ClickableText.modifier
                      .clickable(enabled = countNumOnClickEnabled) { countNumOnClick() }
      ...
      Box {
        ...
        val offsetForPinnedAndMenuIcons =
            baseIconSize.value
                .let { pinnedIconList.size * it + (if (visibleMoreIcon) it else 0F) }
                .coerceAtLeast(0f)
                .dp
