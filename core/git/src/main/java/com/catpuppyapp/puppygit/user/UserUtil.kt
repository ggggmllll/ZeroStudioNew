...
object UserUtil {
  ...
  // 也可这样写：modifyUser:User.()->Unit
  // ，限定函数作用域为传给modifyUser的入参user，函数里用this指代当前user实例，相当于给传入的函数Bind了user实例作为this
  // needUpdateUserByLock若为true，百分百不出错，但如果调用者正确更新状态，则没必要执行updateUserByLock
  fun updateUser(
      requireReturnChangedUser: Boolean = false,
      needUpdateUserByLock: Boolean = true,
    ...
    if (needUpdateUserByLock) {
      doJobThenOffLoading { updateUserChannel.send(userForUpdate) }
    }
  ...
  /**
   * isPro的使用方法： 1
   * 在MainCompose里创建rememberSaveable的isPro状态变量，然后调用UserInfo.updateIsProState()把状态赋值UserInfo实例 2
   * 在需要获取isPro的地方调用UserInfo.isPro()；在需要修改的地方调用 setIsPro()。 3 有待补充
   */
  ...
  /** 此方法应只在MainCompose里调用一次 期望的入参是由rememberXXXX系列方法生成的user的各种state变量 */
  fun updateUserStateToRememberXXXForPage(newIsProState: MutableState<Boolean>) {
