...
object UIHelper {
  ...
  object Size {
    ...
    @Composable
    fun editorVirtualSpace(
        configuration: Configuration = AppModel.getCurActivityConfig()
    ): Pair<Dp, Dp> {
    ...
    /**
     * 获取键盘高度。 注：(?) 需要在Activity#onCreate()执行 `WindowCompat.setDecorFitsSystemWindows(window,
     * false)` 才能获取到有效高度， 否则只能获取到0，但我没充分测试。
     */
    @Composable
  ...
  fun getTitleColor(enabled: Boolean, inDarkTheme: Boolean = Theme.inDarkTheme) =
      if (enabled) Color.Unspecified else getDisableBtnColor(inDarkTheme)

  fun getFontColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
    return if (inDarkTheme) MyStyleKt.TextColor.darkThemeFontColor
    else MyStyleKt.TextColor.fontColor
  }
  ...
  fun getIconEnableColorOrNull(enable: Boolean): Color? {
    ...
    val color =
        if (Theme.inDarkTheme) MaterialTheme.colorScheme.inversePrimary
        else MaterialTheme.colorScheme.primary
    //        val enableColor = Color(red=(color.red+0.3f).coerceAtMost(1f), green =
    // (color.green+0.3f).coerceAtMost(1f), blue = (color.blue+0.3f).coerceAtMost(1f), alpha = 1f)
    // 如果是darkTheme，提升整体亮度；否则只增加蓝色
    val enableColor =
        if (Theme.inDarkTheme)
            Color(
  ...
  // 长按选中连续条目
  fun <T> doSelectSpan(
      itemIdxOfItemList: Int,
      item: T,
      selectedItems: List<T>,
      itemList: List<T>,
      switchItemSelected: (T) -> Unit,
      selectIfNotInSelectedListElseNoop: (T) -> Unit,
  ) {
    // 如果 已选条目列表为空 或 索引无效，选中条目，然后返回
    if (
        selectedItems.isEmpty() || itemIdxOfItemList < 0 || itemIdxOfItemList > itemList.lastIndex
    ) {
    ...
    if (
        startIndex >= endIndexExclusive ||
            startIndex < 0 ||
            startIndex > itemList.lastIndex ||
            endIndexExclusive < 0 ||
            endIndexExclusive > itemList.size
    ) {
      return
    }

    // 选中范围内的条目 左闭右开 [startIndex, endIndexExclusive)
    // list.forEach(selectIfNotInSelectedListElseNoop) 等于
    // list.forEach{selectIfNotInSelectedListElseNoop(it)}
    //        itemList.subList(startIndex, endIndexExclusive).forEach
    // {selectIfNotInSelectedListElseNoop(it)}  //需要拷贝列表，bad
  ...
  fun <T> selectIfNotInSelectedListElseRemove(
      item: T,
      selectedItems: MutableList<T>,
      contains: (srcList: List<T>, curItem: T) -> Boolean = { srcList, curItem ->
        srcList.contains(curItem)
      },
      remove: (srcList: MutableList<T>, curItem: T) -> Boolean = { srcList, curItem ->
        srcList.remove(curItem)
      },
  ) {
    if (contains(selectedItems, item)) {
  ...
  fun <T> scrollByPredicate(
      scope: CoroutineScope,
      list: List<T>,
      listState: LazyListState,
      offset: Int = Cons.scrollToItemOffset,
      animation: Boolean = false,
      predicate: (idx: Int, item: T) -> Boolean,
  ) {
  ...
  fun switchBetweenTopAndLastVisiblePosition(
      coroutineScope: CoroutineScope,
      listState: LazyListState,
  ...
  /** 获取高亮条目闪烁时间（换个角度来说，即“解除高亮倒计时”）。 应用场景：在列表定位某个条目后，短暂高亮那个条目以便用户发现 */
  ...
  @Composable
  fun getHighlightingBackgroundColor(
      baseColor: Color = MaterialTheme.colorScheme.inversePrimary
  ): Color {
  ...
  fun defaultCardColor(): Color {
    ...
    return if (Theme.inDarkTheme) MaterialTheme.colorScheme.surfaceBright
  ...
  fun getBackgroundColorForMergeConflictSplitText(
    ...
    val retColor =
        if (curExpectConflictStrMatched) {
          if (text.startsWith(nextExpectConflictStr)) {
            expectConflictStrDto.curConflictStr =
                if (nextExcept == 0) settings.editor.conflictStartStr
                else if (nextExcept == 1) settings.editor.conflictSplitStr
                else settings.editor.conflictEndStr
            ...
            if (nextExcept == 0) startLineBgColor
            else if (nextExcept == 1) splitLineBgColor else endLineBgColor
          } else {
            //                //if(curExpect==0) startBgColor else if(curExpect==1) splitBgColor
            // else endBgColor
          ...
          if (text.startsWith(curExpectConflictStr)) {
            ...
            if (curExpect == 0) startLineBgColor
            else if (curExpect == 1) splitLineBgColor else endLineBgColor
  ...
  fun getCheckBoxByState(state: Boolean) =
      if (state) Icons.Filled.CheckBox else Icons.Filled.CheckBoxOutlineBlank

  fun getIconForSwitcher(state: Boolean) =
      if (state) Icons.Filled.ToggleOn else Icons.Filled.ToggleOff

  fun getColorForSwitcher(state: Boolean) =
      if (state) Color(0xFF0090FF) else if (Theme.inDarkTheme) Color.LightGray else Color.Gray

  fun getChangeListTitleColor(repoStateValue: Int): Color {
    return if (
        repoStateValue == Repository.StateT.MERGE.bit ||
            repoStateValue == Repository.StateT.REBASE_MERGE.bit ||
            repoStateValue == Repository.StateT.CHERRYPICK.bit
    )
  ...
  fun getDividerColor(inDarkTheme: Boolean = Theme.inDarkTheme): Color {
    return if (inDarkTheme) Color.DarkGray.copy(alpha = 0.2f)
    else Color.LightGray.copy(alpha = 0.2f)
  }

  fun getChangeTypeColor(type: String): Color {
    if (type == Cons.gitStatusNew) {
      return if (Theme.inDarkTheme) MyStyleKt.ChangeListItemColor.changeTypeAdded_darkTheme
      else MyStyleKt.ChangeListItemColor.changeTypeAdded
    }
    if (type == Cons.gitStatusDeleted) {
      return if (Theme.inDarkTheme) MyStyleKt.ChangeListItemColor.changeTypeDeleted_darkTheme
      else MyStyleKt.ChangeListItemColor.changeTypeDeleted
    }
    if (type == Cons.gitStatusModified) {
      return if (Theme.inDarkTheme) MyStyleKt.ChangeListItemColor.changeTypeModified_darkTheme
      else MyStyleKt.ChangeListItemColor.changeTypeModified
    }
    if (type == Cons.gitStatusConflict) {
      return if (Theme.inDarkTheme) MyStyleKt.ChangeListItemColor.changeTypeConflict_darkTheme
      else MyStyleKt.ChangeListItemColor.changeTypeConflict
    }
  ...
  fun getRunningStateColor(runningStatus: Boolean?): Color {
    return if (runningStatus == null) MyStyleKt.TextColor.error()
    else if (runningStatus) MyStyleKt.TextColor.getHighlighting() else Color.Unspecified
  }

  fun getRunningStateText(context: Context, runningStatus: Boolean?): String {
    return if (runningStatus == null) context.getString(R.string.unknown_state)
    else if (runningStatus) context.getString(R.string.running)
    else context.getString(R.string.stopped)
  }

  fun getTextForSwitcher(context: Context, runningStatus: Boolean?): String {
    return if (runningStatus == true) context.getString(R.string.enable)
    else context.getString(R.string.disable)
  }

  fun getCardButtonTextColor(enabled: Boolean, inDarkTheme: Boolean): Color {
    return if (enabled) MyStyleKt.TextColor.enable
    else if (inDarkTheme) MyStyleKt.TextColor.disable_DarkTheme else MyStyleKt.TextColor.disable
  }
  ...
  fun getHunkColor(inDarkTheme: Boolean = Theme.inDarkTheme) =
      if (inDarkTheme) MyStyleKt.Diff.hunkHeaderColorBgInDarkTheme
      else MyStyleKt.Diff.hunkHeaderColorBgInLightTheme
  ...
  fun getNaviBarsPadding(
    ...
    val naviBarsLeftHeight =
        WindowInsets.navigationBars.getLeft(density, direction).coerceAtLeast(0)
    val naviBarsRightHeight =
        WindowInsets.navigationBars.getRight(density, direction).coerceAtLeast(0)
  ...
  fun getMatchedTextBgColorForDiff(
      inDarkTheme: Boolean = Theme.inDarkTheme,
      line: PuppyLine,
  ): Color {
    if (line.originType == PuppyLineOriginType.ADDITION) { // 添加行
      return if (inDarkTheme) MyStyleKt.Diff.hasMatchedAddedLineBgColorForDiffInDarkTheme
      else MyStyleKt.Diff.hasMatchedAddedLineBgColorForDiffInLightTheme
    } else if (line.originType == PuppyLineOriginType.DELETION) { // 删除行
      return if (inDarkTheme) MyStyleKt.Diff.hasMatchedDeletedLineBgColorForDiffInDarkTheme
      else MyStyleKt.Diff.hasMatchedDeletedLineBgColorForDiffInLightTheme
  ...
  fun getDiffLineBgColor(line: PuppyLine, inDarkTheme: Boolean): Color {
    if (line.originType == PuppyLineOriginType.ADDITION) { // 添加行
      return if (inDarkTheme) MyStyleKt.Diff.addedLineBgColorForDiffInDarkTheme
      else MyStyleKt.Diff.addedLineBgColorForDiffInLightTheme
    } else if (line.originType == PuppyLineOriginType.DELETION) { // 删除行
      return if (inDarkTheme) MyStyleKt.Diff.deletedLineBgColorForDiffInDarkTheme
      else MyStyleKt.Diff.deletedLineBgColorForDiffInLightTheme
    } else if (line.originType == PuppyLineOriginType.HUNK_HDR) { // hunk header
      // 注：后来hunk header并没用这的颜色
      return Color.Gray
    } else if (line.originType == PuppyLineOriginType.CONTEXT) { // 上下文
