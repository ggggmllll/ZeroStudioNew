...
fun AskGitUsernameAndEmailDialogWithSelection(
  ...
  val setUserAndEmailForCurRepo =
      stringResource(R.string.set_for_current_repo) + " (${curRepo.repoName})"
  ...
  val optNumSetUserAndEmailForGlobal =
      0 // 为全局设置用户名和密码，值是对应选项在选项列表中的索引，这个变量其实相当于是索引的别名。关联列表：usernameAndEmailDialogOptionList
  val optNumSetUserAndEmailForCurRepo = 1 // 为当前仓库设置用户名和密码
  // num: optText，例如 "1: 选项1文本"
  //    val usernameAndEmailDialogOptionList =
  // listOf(RadioOptionsUtil.formatOptionKeyAndText(optNumSetUserAndEmailForGlobal,setUserAndEmailForGlobal), RadioOptionsUtil.formatOptionKeyAndText(optNumSetUserAndEmailForCurRepo,setUserAndEmailForCurRepo))
  // 这个列表两个条目，一个代表为全局设置用户名和邮箱，另一个代表为当前仓库设置用户名和邮箱
  val optionsList =
      listOf( // idx关联选项：optNumSetUserAndEmailForGlobal, optNumSetUserAndEmailForCurRepo
  ...
  AlertDialog(
      title = { DialogTitle(stringResource(R.string.user_info)) },
      text = {
        ScrollableColumn {
