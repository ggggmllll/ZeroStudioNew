...
fun MimeType.Companion.guessFromExtension(extension: String): MimeType {
  ...
  return extensionToMimeTypeOverrideMap[extension]
      ...
      ?: GENERIC // 如果文件类型未知，当作常规文件处理，GENERIC 调用 intentType 会返回
                 // */*（好像因为做了map映射？），有点反直觉，我还以为会返回字节流octet那个类型。另外，由于我有open
                 // as弹窗，所以未知类型用户可选用内部编辑器打开或外部程序打开，操作我感觉还算方便，所以废弃“无匹配mime就当作text类型”的方案
...
// @see
// https://android.googlesource.com/platform/frameworks/base/+/master/mime/java-res/android.mime.types
...
private val extensionToMimeTypeOverrideMap =
    mapOf(
            // Fixes
...
private val mimeTypeToIntentMimeTypeMap =
    listOf(
