...
class CredentialRepositoryImpl(private val dao: CredentialDao) : CredentialRepository {
  ...
  override suspend fun getAllWithDecrypt(
      includeNone: Boolean,
      includeMatchByDomain: Boolean,
    ...
    prependSpecialItemIfNeed(
        list = all,
        includeNone = includeNone,
        includeMatchByDomain = includeMatchByDomain,
    )
  ...
  private fun prependSpecialItemIfNeed(
      list: MutableList<CredentialEntity>,
      includeNone: Boolean,
      includeMatchByDomain: Boolean,
  ) {
  ...
  override suspend fun insertWithEncrypt(item: CredentialEntity, masterPassword: String) {
    ...
    Cons.credentialInsertLock.withLock {
      ...
      if (isCredentialNameExist(item.name)) {
        MyLog.w(
            TAG,
            "#insertWithEncrypt(): Credential name exists, item will NOT insert! name is: '${item.name}'",
        )
  ...
  override suspend fun insert(item: CredentialEntity) {
    ...
    Cons.credentialInsertLock.withLock {
      if (isCredentialNameExist(item.name)) { // 如果名称已经存在则不保存
        MyLog.w(
            TAG,
            "#insert(): Credential name exists, item will NOT insert! name is:" + item.name,
        )
        throw RuntimeException("#$funName err: name already exists")
      }
      dao.insert(item)
  ...
  override suspend fun updateWithEncrypt(
  ...
  override suspend fun getByIdAndMatchByDomain(id: String, url: String): CredentialEntity? {
    ...
    return getByIdAndMatchByDomainAndDecryptOrNoDecrypt(
        id,
        url,
        decryptPass = false,
        masterPassword = "",
  ...
  override suspend fun getByIdWithDecryptAndMatchByDomain(
      id: String,
      url: String,
      masterPassword: String,
  ): CredentialEntity? {
  ...
  private suspend fun getByIdAndMatchByDomainAndDecryptOrNoDecrypt(
      id: String,
      url: String,
      decryptPass: Boolean,
      masterPassword: String,
    ...
    if (id == SpecialCredential.MatchByDomain.credentialId) {
      ...
      if (domain.isNotBlank()) {
        ...
        val credId =
            if (Libgit2Helper.isSshUrl(url)) domainCred.sshCredentialId else domainCred.credentialId
  ...
  //    }
  //
  //    override suspend fun getHttpList(includeNone:Boolean, includeMatchByDomain:Boolean):
  // List<CredentialEntity> {
  //        val list = dao.getListByType(Cons.dbCredentialTypeHttp).toMutableList()
  //
  //        prependSpecialItemIfNeed(list = list, includeNone = includeNone, includeMatchByDomain =
  // includeMatchByDomain)
  //
  //        return list
  //    }
  ...
  override fun encryptPassIfNeed(item: CredentialEntity?, masterPassword: String) {
    ...
    if (item != null) {
      ...
      if (item.pass.isNotEmpty()) {
        ...
        // masterPassword)
        item.pass =
            PassEncryptHelper.encryptWithSpecifyEncryptorVersion(
                curEncryptorVersion,
                item.pass,
                masterPassword,
            )
  ...
  override fun decryptPassIfNeed(item: CredentialEntity?, masterPassword: String) {
    if (item != null && item.pass.isNotEmpty()) {
      ...
      // masterPassword)
      item.pass =
          PassEncryptHelper.decryptWithSpecifyEncryptorVersion(
              item.encryptVer,
              item.pass,
              masterPassword,
          )
  ...
  override suspend fun updateMasterPassword(
    ...
    reEncryptCredentials(
        ...
        decryptFailedCallback = { cred, exception -> decryptFailedList.add(cred.name) },
    )
   ...
   *
   * 迁移密码本质上只有一个主密码，但为了兼容更新主密码的函数，所以添加了old和new两个主密码参数，但内部实际不对新旧主密码是否相同做检测， 如果只是想升级加密器版本，old和new
   * master password传一样即可
   *
   * Actually migrate only has one master password, no old or new, but for compatible
   * `updateMasterPassword()`, it has 2 params represents old and new password, if just want to
   * upgrade encryptor by version, just passing same value for old and new master passwords
   */
    ...
    AppModel.dbContainer.db.withTransaction {
      ...
      for (c in credentialList) {
        ...
        try {
          ...
          MyLog.w(
              TAG,
              "decrypt password failed, credentialName=${c.name}, err=${e.localizedMessage}",
          )
  ...
  override suspend fun migrateEncryptVerIfNeed(masterPassword: String) {
    val needUpdateEncryptVerCredList =
        getByEncryptVerNotEqualsTo(PassEncryptHelper.passEncryptCurrentVer)
    ...
    reEncryptCredentials(
        ...
        masterPassword,
        masterPassword,
        decryptFailedCallback = { c, e -> failedList.add(c) },
    )
    ...
    if (failedList.isNotEmpty()) {
      ...
      for (i in failedList) {
        ...
        sb.append("(")
            .append("name=${i.name}, ")
            .append("encryptVer=${i.encryptVer}")
            .append(")")
            .append(split)
      }
      ...
      MyLog.e(
          TAG,
          "#migrateEncryptVerIfNeed: these credentials migrate failed: ${sb.removeSuffix(split)}",
      )
    }
  }
