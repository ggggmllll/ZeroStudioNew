...
fun LinkOrUnLinkCredentialAndRemoteDialog(
  ...
  val fetchChecked = rememberSaveable {
    mutableStateOf(
        if (targetAll) true
        else if (requireDoLink) thisItem.credentialId != curItemInPage.value.id
        else thisItem.credentialId == curItemInPage.value.id
    )
  }
  val pushChecked = rememberSaveable {
    mutableStateOf(
        if (targetAll) true
        else if (requireDoLink) thisItem.pushCredentialId != curItemInPage.value.id
        else thisItem.pushCredentialId == curItemInPage.value.id
    )
  }
  ...
  AlertDialog(
      title = {
        DialogTitle(
            title.ifEmpty {
              if (requireDoLink) stringResource(R.string.link) else stringResource(R.string.unlink)
            }
        )
      },
      text = {
      ...
      confirmButton = {
        TextButton(
            ...
            onClick = onOk@{
                  ...
                  doJobThenOffLoading {
                    try {
                      val targetCredentialId =
                          if (requireDoLink) curCredentialId
                          else SpecialCredential.NONE.credentialId

                      if (requireDoLink) { // link
                        if (targetAll) {
                          if (fetchChecked.value && pushChecked.value) {
                            remoteDb.updateAllFetchAndPushCredentialId(
                                targetCredentialId,
                                targetCredentialId,
                            )
                        ...
                        if (targetAll) { // unlink all by current credentialId
                          if (fetchChecked.value && pushChecked.value) {
                            remoteDb.updateFetchAndPushCredentialIdByCredentialId(
                                curCredentialId,
                                curCredentialId,
                                targetCredentialId,
                                targetCredentialId,
                            ...
                            remoteDb.updateCredentialIdByCredentialId(
                                curCredentialId,
                                targetCredentialId,
                            )
                          } else { // pushChecked.value is true
                            remoteDb.updatePushCredentialIdByCredentialId(
                                curCredentialId,
                                targetCredentialId,
                            )
                          }
