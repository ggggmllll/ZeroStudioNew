...
object FsUtils {
  ...
  // 换了种解析方法，这个没用了
  //    const val encodedFileUriPathPrefix = "file%3A%2F%2F%2F"  // web 那个url encoder或url
  // component编码过的
  // "file://"，由于安卓的uri在.net包下，可能在设计上想用来网络传输所以用web的url组件编码，呵呵，这破saf的破uri，本地用都这么恶心，还想web用，想屁呢
  ...
  const val appExportFolderNameUnderDocumentsDirShowToUser =
      "Documents/${appExportFolderName}" // 显示给用户看的路径
  ...
  object FileMimeTypes {
    ...
    val typeList =
        listOf(
            ...
            MimeTypeAndDescText("application/zip") {
              it.getString(R.string.file_open_as_type_archive)
            },
  ...
  /**
   * get authority for gen uri for file note: the value must same as provider.android:authorities in
   * AndroidManifest.xml
   */
  ...
  fun getUriForFile(context: Context, file: File): Uri {
    val uri = FileProvider.getUriForFile(context, getAuthorityOfUri(), file)
  ...
  fun openFile(
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#openFile(): try open file(path=${file.canonicalPath}) err! params is: mimeType=$mimeType, readOnly=$readOnly\n" +
  ...
  fun getExportDirUnderPublicDocument(): Ret<File?> {
    return createDirUnderPublicExternalDir(
        dirNameWillCreate = appExportFolderName,
        publicDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS),
    )
  }

  private fun createDirUnderPublicExternalDir(
      dirNameWillCreate: String,
      publicDir: File =
          Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS),
  ): Ret<File?> {

    // 20240424改用saf导出文件了，saf从安卓4.0(ndk19)开始支持，兼容性更好
    // 经过我的测试，api 26，安卓8.0.0
    // 并不能访问公开目录(Documents/Pictures)之类的，所以这个判断没什么卵用，就算通过了，也不一定能获取到公开目录，貌似至少安卓9才行
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
      return Ret.createError(
          null,
          "Doesn't support export file to public dir on android version lower than 9",
          Ret.ErrCode.doesntSupportAndroidVersion,
      )
    }
  ...
  fun getANonExistsFile(file: File): File {
    ...
    return File(
        parent +
            getANonExistsName(fileName, exists = { newName -> File(parent + newName).exists() })
    )
  }

  /**
   * @return Pair(name, ext) e.g. input "abc.txt", return "abc" and ".txt"; 但是如果.在开头，则不视为后缀，例如 input
   *   ".git", return ".git" and ""
   */
  ...
  /**
   * 本函数适用于从"/"开头的绝对路径和非"/"开头的相对路径拆分去父路径和文件名 本函数应该不会报错，但若担心可 runCatching{}
   *
   * @return Pair(parentPath, fileName), e.g. input "/abc/def/123", will return Pair("/abc/def/",
   *   "123")，并不会移除父路径末尾的'/'，所以可以直接把parentPath和新文件名拼接获得同目录下的新文件对象
   */
  fun splitParentAndName(canonicalPath: String): Pair<String, String> {
    // `File("/").name` return empty String, so if path equals rootPath, should return empty string
    // as file name for compatible
    ...
    return if (
        fileNameStartAt >= canonicalPath.length
    ) { // bad index, out of bound，执行到这，若此判断为真，"/"必然在路径末尾
      // no file name yet, canonicalPath like "path/" or "/path/"  but is not "/", cause "/" already
  ...
  fun copyOrMoveOrExportFile(
      srcList: List<File>,
      destDir: File,
      requireDeleteSrc: Boolean,
  ): Ret<List<PasteResult>?> {
    ...
    // 目标路径不能是文件
    if (
        !destDir.isDirectory || destDir.isFile
    ) { // 其实这俩判断一个就行了，不过我看两个方法的实现不是简单的一个是另一个的取反，所以我索性两个都用了
      return Ret.createError(
          null,
          "target is a file but expect dir!",
          Ret.ErrCode.targetIsFileButExpectDir,
      )
    }
    ...
    srcList.forEachBetter forEach@{
      ...
      try {
        // 1
        // 源不能不存在(例如，我在选择模式下对某个复制到“剪贴板”的文件执行了重命名，粘贴时就会出现源不存在的情况(这种情况实际已经解决，现在20240601选中文件后重命名会把已选中列表对应条目也更新))
        // 2 源和目标不能相同(否则会无限递归复制)
        // 3 源不能是目标文件夹的父目录(否则会无限递归复制)
        if (
            (!src.exists()) ||
                (src.isDirectory && destDir.canonicalPath.startsWith(src.canonicalPath))
        ) {
        ...
        //                MyLog.e(TAG, "#copyOrMoveOrExportFile() err: src=${src.canonicalPath},
        // target=${target?.canonicalPath}, err=${e.stackTraceToString()}")
  ...
  fun saveFile(fileFullPath: String, text: String, charsetName: String?) {
    ...
    FileOutputStream(fileFullPath).use { fos ->
      ...
      fos.bufferedWriter(EncodingUtil.resolveCharset(charsetName)).use { it.write(text) }
    }
  }
  ...
  fun saveFileAndGetResult(fileFullPath: String, text: String, charsetName: String?): Ret<Unit?> {
    try {
      ...
      //            val retFileName = if(fileName.isEmpty())
      // getFileNameFromCanonicalPath(fileFullPath) else fileName
      return Ret.createSuccess(null)
    } catch (e: Exception) {
      MyLog.e(TAG, "#saveFileAndGetResult() err: " + e.stackTraceToString())
      return Ret.createError(
          null,
          "save file failed: ${e.localizedMessage}",
          Ret.ErrCode.saveFileErr,
      )
    }
  }
  ...
  fun readFile(fileFullPath: String, charset: Charset? = null): String {
    val br =
        FileInputStream(fileFullPath)
            .bufferedReader(
                charset
                    ?: EncodingUtil.resolveCharset(
  ...
  fun recursiveExportFiles_Saf(
    ...
    for (f in srcFiles) {
      ...
      if (targetFileBeforeCreate != null) { // 文件已存在
        if (conflictStrategy == CopyFileConflictStrategy.SKIP) {
          ...
          if (targetFileBeforeCreate.isDirectory) { // 递归删除目录及其子文件
            recursiveDeleteFiles_Saf(
                contentResolver,
                targetFileBeforeCreate,
                targetFileBeforeCreate.listFiles() ?: arrayOf<DocumentFile>(),
                canceled,
            )
          } else { // 删除文件
          ...
          targetName =
              getANonExistsName(
                  targetName,
                  exists = { newName -> filesUnderExportDir.find { it.name == newName } != null },
              )
        }
      }
      ...
      if (f.isDirectory) {
        ...
        f.inputStream().use { ins -> output.use { outs -> ins.copyTo(outs) } }
      }
    }
  }
  ...
  fun recursiveImportFiles_Saf(
    ...
    for (f in srcFiles) {
      ...
      if (targetFileBeforeCreate != null) { // 文件已存在
        if (conflictStrategy == CopyFileConflictStrategy.SKIP) {
          ...
          targetName =
              getANonExistsName(
                  targetName,
                  exists = { newName -> filesUnderImportDir.find { it.name == newName } != null },
              )
        }
      }
      ...
      if (f.isDirectory) {
        ...
        inputStream.use { ins -> outputStream.use { outs -> ins.copyTo(outs) } }
      }
    }
  }
  ...
  fun simpleSafeFastSave(
    ...
    try {
      ...
      contentAndFileSnapshotPathPair =
          Pair(contentRet.data?.second ?: "", fileRet.data?.second ?: "")
      ...
      return Ret.createSuccess(
          Triple(
              writeContentToTargetFileSuccess,
              contentAndFileSnapshotPathPair.first,
              contentAndFileSnapshotPathPair.second,
          )
      )
    } catch (e: Exception) {
      ...
      return Ret.createError(
          ...
          "SSFS: save err: " + e.localizedMessage,
      )
    }
  }
  ...
  fun getDoSaveForEditor(
      ...
      snapshotedFileInfo:
          CustomStateSaveable<
              FileSimpleDto
          >, // 用来粗略判断是否已创建文件的快照，之所以说是粗略判断是因为其只能保证打开一个文件后，再不更换文件的情况下不会重复创建同一文件的快照，但一切换文件就作废了，即使已经有某个文件的快照，还是会重新创建其快照
    ...
    val doSave: suspend () -> Unit = doSave@{
      ...
      // here use Box is for reduce changes the code, because `Box.value` in code is same as
      // `State.value`, is convenience to replace
      //   here expect get current state pointed instance, if get the value then assign to a
      // variable, must remove all .value, so use a Box to avoid many changes
      ...
      try {
        ...
        if (filePath.isEmpty()) {
          ...
          if (
              editorPageTextEditorState.value.contentIsEmpty().not() && !isContentSnapshoted.value
          ) {
            MyLog.w(
                pageTag,
                "#$funName: filePath is empty, but content is not empty, will create content snapshot with a random filename...",
            )
            val flag = SnapshotFileFlag.editor_content_FilePathEmptyWhenSave_Backup
            val contentSnapRet =
                SnapshotUtil.createSnapshotByContentWithRandomFileName(
                    fileContent = null,
            ...
            if (contentSnapRet.hasError()) {
              MyLog.e(
                  pageTag,
                  "#$funName: create content snapshot for empty path failed:" + contentSnapRet.msg,
              )
        ...
        if (targetFile.exists()) {
          ...
          if (newDto != editorPageFileDto.value) { // editor正在编辑的文件被外部修改过
            ...
            if (newDto != snapshotedFileInfo.value) { // 未存被外部修改过的文件快照
              MyLog.w(
                  pageTag,
                  "#$funName: warn! file maybe modified by external! will create a snapshot before save...",
              )
              val snapRet =
                  SnapshotUtil.createSnapshotByFileAndGetResult(
                      targetFile,
                      SnapshotFileFlag.editor_file_BeforeSave,
                  )
              ...
              if (snapRet.hasError()) {
                ...
                MyLog.e(
                    pageTag,
                    "#$funName: create file snapshot for '$filePath' failed:" + snapRet.msg,
                )

                // 虽然为源文件创建快照失败了，但如果没有为当前内容创建快照，则为当前用户编辑的内容(content)创建个快照
                if (
                    editorPageTextEditorState.value.contentIsEmpty().not() &&
                        !isContentSnapshoted.value
                ) {
                  val contentSnapRet =
                      SnapshotUtil.createSnapshotByContentAndGetResult(
                          srcFileName = targetFile.name,
                          ...
                          flag =
                              SnapshotFileFlag
                                  .editor_content_CreateSnapshotForExternalModifiedFileErrFallback,
                      )
                  if (contentSnapRet.hasError()) {
                    MyLog.e(
                        pageTag,
                        "#$funName: create content snapshot for '$filePath' failed:" +
                            contentSnapRet.msg,
                    )
                    ...
                    throw RuntimeException(
                        "save origin file snapshot err, but save content snapshot success"
                    )
                  }
                } else {
                  // 如果备份文件失败但内容快照已创建
                  throw RuntimeException(
                      "save origin file snapshot err, and content is empty or snapshot already exists"
                  )
                }
              } else { // 创建文件快照成功
                // 更新已创建快照的文件信息
        ...
        if (ret.hasError()) {
          ...
          if (
            ...
            if (snapRet.hasError()) {
              MyLog.e(
                  pageTag,
                  "#$funName: save content snapshot for '$filePath' failed:" + snapRet.msg,
              )
            ...
            MyLog.w(
                pageTag,
                "#$funName: save file failed, but content is empty or already snapshoted, so will not create snapshot for it",
            )
  ...
  // folderDesc描述folder类型，为可选参数，例如folder为快照目录，则期望的folderDesc为"snapshot
  // folder"，记日志的时候会用到此参数，理论上，不传也行，但建议传(强制！)
  fun delFilesOverKeepInDays(keepInDays: Int, folder: File, folderDesc: String) {
    ...
    try {
      ...
      val predicate = predicate@{ f: File ->
        ...
        // 文件最后修改时间，起始时间1970-01-01,
        // 时区GMT(UTC)。（ps：最后修改时间单位默认竟然是毫秒？我一直以为是秒，不过，应该取决于平台，例如在linux上，可能记的就是秒，我也不确定）
      ...
      MyLog.w(
          TAG,
          "#$funName: end: del expired files for '$folderDesc' done, success deleted: $successDeletedCount",
      )
    } catch (e: Exception) {
    ...
    try {
      ...
      for (f in files) {
        try {
          ...
          MyLog.e(
              TAG,
              "#$funName: del file '${f.name}' for $folderDesc err: " + e.stackTraceToString(),
          )
        }
      }
   ...
   */
  @Deprecated(
      "没必要检查是否只读，无所谓，用户编辑这些目录下的文件也不是不行，而且每次我打开这些目录下的文件，第一件事就是取消只读，不然光标无法显示，搜索后因为jetpackcompose限制难以同时实现关键字高亮和文字可编辑，搞得查找文字后就算找到了我也不知道关键字在哪，而且也没法用光标辅助定位上次我查看的位置，总之就是很恶心"
  )
  ...
  fun calculateFolderSize(fileOrFolder: File, itemsSize: MutableLongState) {
    if (fileOrFolder.isDirectory) {
      ...
      if (!list.isNullOrEmpty()) {
        list.forEachBetter { calculateFolderSize(it, itemsSize) }
      }
    } else { // file
  ...
  /**
   * ceiling path of app, if reached those path in Files explorer, should show 'press back again to
   * exit' rather than go to parent dir
   *
   * note: the externalFilesDir "/storage/emulated/0/Android/data/your_package_name" should not in
   * this list, cause it actually is subdir of external storage dir "/storage/emulated/0",
   * 不加进列表的话，切换目录方便，可直接从App的外部data目录返回到外部存储目录
   */
  ...
  /**
   * 返回的不一定是真实路径，例如 termux用这种方式解析路径会出现两个/ 例如
   * "/storage/emulated/0/tree//data/data/com.termux/files/home/Repo_export"，这是因为termux在路径中间加了两个/，懒得处理了，反正不支持使用saf克隆仓库
  ...
  fun getRealPathFromUri(uri: Uri): String {
    return try {
      ...
      // 例如：uri为：content://com.android.externalstorage.documents/tree/primary%3ARepos, 则uri.path为
      // /tree/primary:Repos，但不保证一定是canonical path，可能中间出现两个 "//" 也可能有其他东西
      // 这两个trim都有意义，尤其最后的trimEnd，当uriPathString为空字符串时，可去掉中间拼接的slash
      (getExternalStorageRootPathNoEndsWithSeparator() +
              Cons.slash +
              uriPathString.let { it.substring(it.indexOf(":") + 1).trim(Cons.slashChar) })
          .trimEnd(Cons.slashChar)
    } catch (_: Exception) {
      ""
    }
  }
  ...
  fun internalExternalPrefixPathToRealPath(path: String): String {
    return if (path.startsWith(appDataPathPrefix)) {
      (getAppDataRootPathNoEndsWithSeparator() + Cons.slash + removeAppDataPrefix(path)).trimEnd(
          Cons.slashChar
      )
    } else if (path.startsWith(internalPathPrefix)) {
      (getInternalStorageRootPathNoEndsWithSeparator() +
              Cons.slash +
              removeInternalStoragePrefix(path))
          .trimEnd(Cons.slashChar)
    } else if (path.startsWith(externalPathPrefix)) {
      (getExternalStorageRootPathNoEndsWithSeparator() +
              Cons.slash +
              removeExternalStoragePrefix(path))
          .trimEnd(Cons.slashChar)
    } else if (path.startsWith(innerPathPrefix)) {
      (getInnerStorageRootPathNoEndsWithSeparator() + Cons.slash + removeInnerStoragePrefix(path))
          .trimEnd(Cons.slashChar)
    } else {
      path
    }
  ...
  @Deprecated(
      "this too complex, only trim line break then use `File(path).cononicalPath` is enough"
  )
  ...
  /**
   * @return eg. input parent="/abc/def", fullPath="/abc/def/123", will return "/123"; if fullPath
   *   not starts with parent, will return origin `fullPath`
   */
  ...
  /**
   * eg: fullPath = /storage/emulated/0/repos/abc, return "External:/abc" eg (wrong path): fullPath
   * = /storage/emulated/0//repos/abc, return "External://repos/abc" ("External:/" + "/repos/abc")
   * eg: fullPath = /storage/emulated/0/Android/path-to-app-internal-repos-folder/abc, return
   * "Internal:/abc" eg: fullPath = /data/data/app.package.name/files/abc, return origin path eg
   * (matched internal/external storage prefix): fullPath = /storage/emulated/0, return "External:/"
   */
  fun getPathWithInternalOrExternalPrefix(fullPath: String): String {
    ...
    return if (
      ...
      externalPathPrefix +
          ((getPathAfterParent(parent = externalStorageRoot, fullPath = fullPath)).removePrefix(
              "/"
          ))
    } else if (fullPath.startsWith(innerStorageRoot)) {
      innerPathPrefix +
          ((getPathAfterParent(parent = innerStorageRoot, fullPath = fullPath)).removePrefix("/"))
  ...
  /**
   * 替换多个行到指定行号，若源文件无EOFNL，执行完此函数后，会添加 replace lines to specified line number, if origin file no
   * EOFNL, after this function, will add it
   *
   * note: if target line doesn't exist, will append content to the EOF, and delete/append content
   * is not available for EOF
   *
   * @param file the file which would be replaced
   * @param startLineNum the line number start from 1 or -1 represent EOF, when you want to append
   *   content to the end of file, use -1 or a line number which bigger than the file actually lines
   *   is ok
  ...
  private suspend fun replaceOrInsertOrDeleteLinesToFile(
    ...
    if (settings.diff.createSnapShotForOriginFileBeforeSave) {
      val snapRet =
          SnapshotUtil.createSnapshotByFileAndGetResult(file, SnapshotFileFlag.diff_file_BeforeSave)
      if (snapRet.hasError()) {
        // 其实加不加括号都行 ?: 优先级高于throw
        throw (snapRet.exception
            ?: RuntimeException(snapRet.msg.ifBlank { "err: create snapshot failed" }))
      }
    }

    val tempFile =
        FuckSafFile.fromFile(
            createTempFile(
                "${TempFileFlag.FROM_DIFF_SCREEN_REPLACE_LINES_TO_FILE.flag}-${file.name}"
            )
        )
    var found = false
    ...
    file.bufferedReader(charsetName).use { reader ->
      tempFile.bufferedWriter(charsetName).use { writer ->
        ...
        while (true) {
          ...
          // note: if target line num is EOF, this if never execute, but will append content to the
          // EOF of target file after looped
  ...
  suspend fun replaceLinesToFile(
      file: FuckSafFile,
      startLineNum: Int,
      newLines: List<String>,
      settings: AppSettings,
  ) {
    replaceOrInsertOrDeleteLinesToFile(
        file,
        startLineNum,
        newLines,
        trueInsertFalseReplaceNullDelete = false,
        settings,
    )
  }

  suspend fun insertLinesToFile(
      file: FuckSafFile,
      startLineNum: Int,
      newLines: List<String>,
      settings: AppSettings,
  ...
  suspend fun prependLinesToFile(
      file: FuckSafFile,
      startLineNum: Int,
      newLines: List<String>,
      settings: AppSettings,
    ...
    replaceOrInsertOrDeleteLinesToFile(
        ...
        startLineNum,
        newLines,
        trueInsertFalseReplaceNullDelete = true,
        settings,
    )
  }

  suspend fun appendLinesToFile(
      file: FuckSafFile,
      startLineNum: Int,
      newLines: List<String>,
      settings: AppSettings,
  ...
  suspend fun deleteLineToFile(file: FuckSafFile, lineNum: Int, settings: AppSettings) {
    replaceOrInsertOrDeleteLinesToFile(
        ...
        lineNum,
        newLines = listOf(),
        trueInsertFalseReplaceNullDelete = null,
        settings,
    )
  }
  ...
  fun createTempFile(prefix: String, suffix: String = ".tmp"): File {
    return File(
        AppModel.getOrCreateExternalCacheDir().canonicalPath,
        "$prefix-${generateRandomString(8)}$suffix",
    )
  }

  /**
   * 读取文件中的行到list， 支持 \r 或 \n 或 \r\n 分割的文件， 正常情况下，一个文件只有一种类型的换行符，用这个函数没问题，如果是个混用各种换行符的奇葩文件，行数会乱。
   *
   * @param addNewLineIfFileEmpty if true, when file is empty, will add a empty str as element to
   *   list, else will return an empty list. set it to true, if you expect this function has same
   *   behavior with `String.lines()`
  ...
  fun readLinesFromFile(
    ...
    // readLines() api 说不能用于 huge
    // files?我看源代码好像也是用readLine一行行读的，我自己写好像差不多，不过不会创建迭代器之类的，可能稍微快一点点，但应该也不能用于huge files吧？大概
    //        File(filePath).bufferedReader().readLines()

    // 根据文件大小估算一下行数，没任何考证，我随便打开了几个文件，感觉file.length / 1000 *
    // 7差不多就是行数，而直接右移7位和/1000*7结果差不多，所以就这么写了，shr是有符号右移，如果是负数，移完还是负数，不看符号的话，值如果不超Int最大值，无符号和有符号右移的区别就在于符号不同，值一般是一样的
    val estimateLineCount =
        (file.length() shr 7)
            .toInt()
            .coerceAtLeast(10)
            .coerceAtMost(
                10000
            ) // chatgpt说10000个空Object对象大概470k，不会超过1mb，可以接受，不过java创建的数组默认值不是null吗？所以可能连470k都占不了
    //        val estimateLineCount = (file.length() / 1000 *
    // 7).toInt().coerceAtLeast(10).coerceAtMost(10000)
  ...
  fun appendTextToFile(file: File, text: String, charset: Charset = StandardCharsets.UTF_8) {
    ...
    filerWriter.buffered().use { writer -> writer.write(text) }
  }
  ...
  fun readShortContent(
    ...
    return try {
      ...
      MyLog.d(
          TAG,
          "readShortContent of file err: fileIoPath=${file.path.ioPath}, err=${e.localizedMessage}",
      )
      ""
    }
  }

  fun getPathWithInternalOrExternalPrefixAndRemoveFileNameAndEndSlash(
      path: String,
      fileName: String,
  ): String {
    ...
    return if (path.startsWith(contentUriPathPrefix)) {
      ...
      getPathWithInternalOrExternalPrefix(path.removeSuffix(fileName).trimEnd(Cons.slashChar))
          .ifBlank { Cons.slash }
    }
  }

  fun translateContentUriToRealPath(
      uri: Uri,
      appContext: Context = AppModel.realAppContext,
      mode: String = "rw",
  ): String? {
  ...
  fun shareFiles(activityContext: Context, files: List<FileItemDto>) {
    ...
    val intent = uris.createSendStreamIntent(activityContext, mimeTypes).withChooser()
