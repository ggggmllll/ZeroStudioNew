...
fun SetUpstreamDialog(
    ...
    onCancel: () -> Unit =
        closeDialog, // 点击取消或返回或空白处执行的操作，一般和closeDialog逻辑一样，但也可能会执行比closeDialog更多的操作
  ...
  AlertDialog(
      title = { DialogTitle(stringResource(R.string.set_upstream_title)) },
      text = {
        ScrollableColumn {
          SelectionRow { Text(text = stringResource(R.string.set_upstream_for_branch) + ":") }

          SelectionRow(
              modifier =
                  Modifier.fillMaxWidth().padding(horizontal = MyStyleKt.defaultHorizontalPadding),
          ...
          //                }
          SelectionRow { Text(text = stringResource(R.string.select_a_remote) + ":") }
          ...
          SelectionRow { Text(stringResource(R.string.upstream_branch_name) + ":") }
          ...
          TextField(
              ...
              singleLine = true,
              onValueChange = { upstreamBranchShortName.value = it },
      ...
      confirmButton = {
        TextButton(
            onClick = onOk@{
                  ...
                  val remote =
                      try {
                        ...
                        MyLog.e(
                            callerTag,
                            "#$funName: err when get remote by index from remote list of '$repoName': remoteIndex=$selectedRemoteIndex, remoteList=$remoteList\nerr info:${e.stackTraceToString()}",
                        )
                  ...
                  //                    doJobThenOffLoading(loadingOn, loadingOff,
                  ...
                  doJobThenOffLoading {
                    try {
                      ...
                      Repository.open(repoFullPath).use { repo ->
                        ...
                        MyLog.d(
                            callerTag,
                            "#$funName: set upstream dialog #onOk(): repo is '$repoName', will write to git config: remote=$remote, branch=$branch",
                        )
                      ...
                      if (isCurrentBranchOfRepo) {
                        ...
                        MyLog.d(
                            callerTag,
                            "#$funName: set upstream dialog #onOk(): upstreamBranchShortName=$upstreamBranchShortName",
                        )

                        AppModel.dbContainer.repoRepository.updateUpstream(
                            repoId,
                      ...
                      if (setUpstreamSuccess) {
                        ...
                        throw RuntimeException(
                            "unknown error, code '1c3f943a8e'"
                        ) // 这code是用来定位出错原因在源代码中的位置的
                      }
                    } catch (e: Exception) {
                      // 显示通知
                      onErrorCallback(e)
            ...
            enabled =
                (!(!upstreamBranchShortNameSameWithLocal.value &&
                    upstreamBranchShortName.value.isBlank())) && remoteList.isNotEmpty(),
        ) {
          Text(text = onOkText)
        }
      },
