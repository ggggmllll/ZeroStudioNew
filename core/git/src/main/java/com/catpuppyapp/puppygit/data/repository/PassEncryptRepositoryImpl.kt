...
class PassEncryptRepositoryImpl(private val dao: PassEncryptDao) : PassEncryptRepository {
  ...
  //        if(ver != PassEncryptHelper.passEncryptCurrentVer) {
  // //ver不等于当前ver说明是旧版加密方式，迁移一下，否则无需执行操作
  //            //为迁移密码准备新旧密钥
  ...
  //                    c.pass = PassEncryptHelper.encryptWithCurrentEncryptor(raw, masterPassword =
  // "")  //用新加密器加密密码
  //                    credentialDb.update(c)  //更新db
  //                }
  //
  //                //更新passEncryptEntity
  //                passEncryptEntity.ver = PassEncryptHelper.passEncryptCurrentVer
  // //更新版本，下次就不会再执行迁移了，直到版本再度更新
