...
data class Editor(
    ...
    // TODO
    // 在设置页面加个选项，可清除此map，否则此map只有在文件不存在时(打开时检查)，才清除打开位置，或者实现一个定期未打开就清除其记录的机制，我已经在FileEditedPos里记录在最后使用条目的时间，可用来实现定期不用删除机制
    ...
    /**
     * how much file opened history and last edit position will remembered if change this value,
     * require restart app for affect
     */
    ...
    var restoreLastEditColumn: Boolean =
        true, // 打开文件时，定位到上次编辑列。注意：会弹出键盘！（我理想中的情况是不会弹出键盘，因为自动弹出键盘有可能会使文本内容被键盘意外读取而泄漏隐私，但没找到合适的方案隐藏键盘。）
    ...
    /**
     * when click file name from `Files` which match the patterns will open in internal Editor as
     * default, else will show open with dialog
     */
    var fileAssociationList: List<String> = SettingsCons.editor_defaultFileAssociationList,

    /**
     * recent file list max limit, should less than FileHistoryLimit, but if over it, will not cause
     * an err，超了历史记录数也不会报错，只是顶多显示历史记录那么多个文件
     */
    ...
    /**
     * set true to disable software keyboard, due to I haven't a keyboard available for phone, so,
     * this settings haven't tested
     */
    ...
    /**
     * if true, input opened symbol, will auto add closed, e.g. input '{', auto add '}', support
     * surround selection range, e.g. selection 'abc', then input '"', will output '"abc"', (doesn't
     * include single quote sign)
     */
    var autoCloseSymbolPair: Boolean = true,

    /**
     * in selection mode, batch operate lines, if over this count, will use full analyze instead of
     * incremental analyze
     */
