...
// @Deprecated("may crashed if use this in dialog")  // 20241003 update: new version of jetpack
// compose are fixed this bug
@Composable
fun <T> SingleSelectList(
    ...
    basePadding: (defaultHorizontalPadding: Dp) -> PaddingValues = { defaultHorizontalPadding ->
      PaddingValues(horizontal = defaultHorizontalPadding)
    },
    ...
    selectedOptionValue: T? =
        if (
            selectedOptionIndex != null &&
                isGoodIndexForList(selectedOptionIndex.intValue, optionsList)
        )
            optionsList[selectedOptionIndex.intValue]
        else null,
    ...
    // for the formatter, index will be null if value not in the list, and index maybe invalid, like
    // -1,-2 something... usually happened when you was selected a item, but remove it from list
    // later, the the selected item will haven't index of the list, should handle this case, if you
    // overwrite this formatter
    menuItemFormatter: (index: Int?, value: T?) -> String = { index, value ->
      value?.toString() ?: ""
    },
    menuItemOnClick: (index: Int, value: T) -> Unit = { index, value ->
      selectedOptionIndex?.intValue = index
    },
    menuItemSelected: (index: Int, value: T) -> Boolean = { index, value ->
      selectedOptionIndex?.intValue == index
    },
  ...
  Surface(
      // 0.9f 占父元素宽度的百分之90
      modifier =
          Modifier.padding(basePadding(MyStyleKt.defaultHorizontalPadding))
              .clickable { expandDropdownMenu.value = !expandDropdownMenu.value }
              .onSizeChanged {
                // unit is pixel
    ...
    Box(
      ...
      Column(
          modifier =
              Modifier.fillMaxWidth().padding(end = trailIconWidth).align(Alignment.CenterStart),
      ...
      Row(
          modifier = Modifier.width(trailIconWidth).align(Alignment.CenterEnd),
          verticalAlignment = Alignment.CenterVertically,
          horizontalArrangement = Arrangement.Center,
      ) {
        Icon(
            imageVector =
                if (expandDropdownMenu.value) Icons.Filled.ArrowDropDown
    ...
    DropdownMenu(
        ...
        //            modifier =
        // dropDownMenuModifier.width((containerSize.value.width/2).coerceAtLeast(MyStyleKt.DropDownMenu.minWidth).dp),
        //            modifier = dropDownMenuModifier.widthIn(min =
        // MyStyleKt.DropDownMenu.minWidth),
        modifier =
            dropDownMenuModifier.width(UIHelper.pxToDpAtLeast0(containerSize.value.width, density)),
      ...
      for ((index, value) in optionsList.withIndex()) {
        ...
        Column(modifier = Modifier.dropDownItemContainerColor(selected).fillMaxSize()) {
                    ...
                    if (menuItemTrailIcon != null)
                        ({
                          IconButton(
                              ...
                              onClick = { menuItemTrailIconOnClick(index, value) },
                          ) {
                            Icon(
