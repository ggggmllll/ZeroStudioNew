...
abstract class Search {
  ...
  abstract fun <T : CharSequence> doSearch(
      add: CompareParam<T>,
      del: CompareParam<T>,
      reverse: Boolean,
  ): IndexModifyResult

  protected fun getAddListMethod(
      reverse: Boolean
  ): (Int, Int, Boolean, LinkedList<IndexStringPart>, IndexStringPart?) -> IndexStringPart? {
    return if (reverse)
        {
            start: Int,
            end: Int,
            modified: Boolean,
            list: LinkedList<IndexStringPart>,
            lastItem: IndexStringPart? ->
          var last = lastItem
          val len = start - end
          if (len > 0) {
