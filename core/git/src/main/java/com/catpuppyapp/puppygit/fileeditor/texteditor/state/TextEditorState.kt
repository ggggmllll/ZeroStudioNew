...
class TextEditorState(
    ...
    private val onChanged:
        suspend (
            ...
            trueSaveToUndoFalseRedoNullNoSave: Boolean?,
            clearRedoStack: Boolean,
            caller: TextEditorState,
            from: EditorStateOnChangeCallerFrom?,
        ) -> Unit,
  ...
  /**
   * x 已修复） 20241203: 发现有bug，无法找到 " w "
   * 这样字符，已经修复，在无匹配时将被查找的文件中的字符串索引向后移一位再和关键字从头匹配即可（反向查找时是向前移一位然后和关键字末尾匹配）
   *
   * @param toNext : if `toNext` is false, will search to previous 缺陷：不支持查找包含换行符的关键字
  ...
  fun doSearch(keyword: String, toNext: Boolean, startPos: SearchPos): SearchPosResult {
    ...
    try {
      ...
      val curPos =
          startPos.copy(
              lineIndex =
                  if (goodIndex) startPos.lineIndex
                  else {
                    if (toNext) 0 else f.size - 1
                  }
          )
      var curText = getCurTextOfIndex(curPos.lineIndex, f)
      curPos.columnIndex =
          if (goodIndex) startPos.columnIndex
          else {
            if (toNext) 0 else curText.length - 1
          }
      val endPosOverThis = curPos.copy() // 越过这一行这一列，就查到开头了，结束了
      var curIndexOfKeyword = if (toNext) 0 else keyword.length - 1

      curPos.columnIndex =
          if (goodIndex && isGoodIndexForStr(curPos.columnIndex, curText)) {
            curPos.columnIndex
          } else {
            if (toNext) {
              0
      ...
      while (true) {
        //                println("在搜索:lineIndex=${curPos.lineIndex},
        // columnIndex=${curPos.columnIndex}")  //test1791022120240812
        // matched 1 char
        if (char != null && char == charOfKeyword) {
          ...
          if (!isGoodIndexForStr(curIndexOfKeyword, keyword)) {
            ...
            val foundColumnIdx =
                if (toNext) curPos.columnIndex + 1 - keyword.length else curPos.columnIndex
            ...
            if (!isGoodIndexForStr(nextColumn, curText)) {
              ...
              if (!isGoodIndexForList(nextLineIdx, f)) {
                // the file at least have an empty line, so the `f` never will be empty, so the else
                // block is safe
                // 文件至少有个空行，所以f永远不会为空，所以else代码块至少为0，不会负数
                nextLineIdx = if (toNext) 0 else f.size - 1
              }

              // if is empty line, the else case will got -1, but is fine, when next search, if use
              // this index, will reset,
              //   let this index coerceAtLeast(0) doesn't make sense, due to 0 or -1, all is
              // invalid index for an empty line
              nextColumn = if (toNext) 0 else getCurTextOfIndex(nextLineIdx, f).length - 1
            }
            //                        println("posResult:${SearchPosResult(foundPos =
            // curPos.copy(lineIndex = foundLineIdx, columnIndex = foundColumnIdx), nextPos =
            // SearchPos(lineIndex =  nextLineIdx, columnIndex = nextColumn))}")
            // //test1791022120240812
          ...
          // if last is matched, keep target text index at same place for matching to head or tail
          // of keyword
        ...
        if (!isGoodIndexForStr(curPos.columnIndex, curText)) { // 需要换行
          if (looped) { // 过了最初查找的那行且需要换行，说明查找完了，可返回了
            //                        val lineOverLooped = if(toNext) curPos.lineIndex >
            // endPosOverThis.lineIndex else curPos.lineIndex < endPosOverThis.lineIndex
            //                        if(lineOverLooped) {
        ...
        //                    val end = if(toNext) curPos.columnIndex >= endPosOverThis.columnIndex
        // else curPos.columnIndex <= endPosOverThis.columnIndex
        //                    if(end){
        //                        return SearchPosResult.NotFound
        //                    }
        //                }
      ...
      MyLog.e(
          TAG,
          "#$funName err: keyword=$keyword, toNext=$toNext, startPos=$startPos, err=${e.stackTraceToString()}",
      )
      return SearchPosResult.NotFound
    }
  }
  ...
  suspend fun splitNewLine(
      ...
      updater:
          ((
              newLinesRange: IntRange,
              newFields: MutableList<MyTextFieldState>,
              newSelectedIndices: MutableList<Int>,
          ) -> Unit)? =
          null,
  ) {
    lock.withLock {
      ...
      newFields[targetIndex] =
          oldLine.copy(value = splitFirstLine).apply {
            if (newLineAtOldLineHead) { // 在旧行头部添加了一换行符
              // 如果新行开头是空行，必然是新行，所以强制更新而不用if none则更新
              updateLineChangeType(LineChangeType.NEW)

              // 这里用oldLine或this的changeType都行，因为
              // this是oldLine的copy，而且没有拷贝changeType，所以他们两个的changeType应该相同，但从逻辑上来说，应该用oldLine的
              // 为真则是在旧行中间添加的换行符，否则是在旧行末尾（无需处理，维持旧行changeType即可）
            } else if (
                oldLine.changeType == LineChangeType.NONE &&
                    this.value.text.let { t ->
                      oldLine.value.text.let { oldT ->
                        // the String.equals() should compare length first, if didn't match,
                        //   then compare the string? if it is, the length check at here can be
                        // remove
      ...
      val newSplitFieldStates = newSplitFieldValues.map {
        MyTextFieldState(value = it, changeType = LineChangeType.NEW)
      }
  ...
  // //光标位置，有可能在行末尾，这时和text.length相等，并不会越界
  //            val maxOfPosition = textFieldValue.text.length
  //            if (splitPosition < 0 || splitPosition > maxOfPosition) {
  // //第2个判断没错，就是大于最大位置，不是大于等于，没写错，光标位置有可能在行末尾，这时其索引和text.length相等，所以只有大于才有问题
  //                throw InvalidParameterException("splitPosition '$splitPosition' out of range
  // '[0, $maxOfPosition]'")
  //            }
  //
  //            val newText = textFieldValue.text
  ...
  //                    updateLineChangeTypeIfNone(if(newLineAtOldLineHead) this.changeType else
  // LineChangeType.NEW)
  //                }
  //            )
  //
  ...
  /**
   * @param textChanged if already checked text change before calling this method, then can pass the
   *   value to avoid redundant check.
   */
  suspend fun updateField(
    ...
    val act = suspend p@{
      ...
      val textChanged =
          textChanged
              ?: oldField.value.text.let {
                it.length != newText.length || it != newText
              } // 旧值 != 新值
  ...
  /**
   * append close pair if need NOTE: make sure call this only when field text changed, else will
   * have unexpected chars inserted
   *
   * background: add this feature is not I want actually, but if user input an opened symbol(e.g.
   * "), it will take big effect for syntax highlighting, maybe let it very slow, so I add this
   * feature to let the pair closed if possible
   */
  private fun appendClosePairIfNeedNoCatch(
    ...
    if (
        ...
        ||
            (!oldSelection.collapsed &&
                (oldText.length - oldSelection.end + oldSelection.start == newText.length))
    ) {
      return newField
    }
    ...
    //        // selection maximum value is length, so need subtract 2 to get the char before user
    // last input
    ...
    // user selected text, then input opened symbol, e.g. select abc then input ", expected output
    // "abc"
    if (!oldSelection.collapsed) {
      ...
      newSelectionRange =
          TextRange(
              start = newSelectionRange.start,
              end = newSelectionRange.end + oldSelectedText.length,
          )
    ...
    val textAddedClosedPair =
        StringBuilder().let {
          it.append(leftTextOfCursor)
          it.append(midText)
          // if start index is length of string, will not throw an exception, but returned empty
          // string
          it.append(rightText)
          it.toString()
        }
    ...
    return newField.copy(
        value = newField.value.copy(text = textAddedClosedPair, selection = newSelectionRange)
    )
  }
  ...
  private fun resolveClosedCharOfPair(openedCharOfPair: Char): String? {
    var closedPair =
        codeEditor?.myLang?.symbolPairs?.matchBestPairBySingleChar(openedCharOfPair)?.close
    if (closedPair == null) {
  ...
  private fun updateStyles(
    ...
    try {
      ...
      if (
          howManyLinesNeedUpdate >= halfLines ||
      ...
      if (baseStyles == null) {
        MyLog.d(
            TAG,
            "#updateStyles: Styles of current field '$fieldsId' not found, maybe not exists or theme/languageScop are not matched, will re-run analyze after user stop input for a while",
        )
        ...
        //            doJobThenOffLoading {
        //                // onChange will call after updateStyles() finished, so here no need call
        // it again
  ...
  private suspend fun appendOrReplaceFields(
      targetIndex: Int,
      text: String,
      trueAppendFalseReplace: Boolean,
  ) {
    if (!isGoodIndexForList(targetIndex, fields)) {
      MyLog.d(
          TAG,
          "#appendOrReplaceFields(): invalid index: $targetIndex, list.size=${fields.size}",
      )
      return
    }
    ...
    val updater =
        {
          ...
          for (i in newFields.indices) {
            if (
                !(((trueAppendFalseReplace && i <= newLinesRange.start) ||
                    (trueAppendFalseReplace.not() && i < newLinesRange.start)) ||
                    i > newLinesRange.endInclusive)
            ) {
    ...
    if (trueAppendFalseReplace) {
      splitNewLine(
          targetIndex,
          TextFieldValue(fields[targetIndex].value.text + lb + text),
          updater = updater,
      )
    } else {
      if (text.contains(lb)) {
  ...
  suspend fun deleteNewLine(targetIndex: Int) {
    lock.withLock {
      ...
      val concatText =
          StringBuilder(toText.length + fromText.length).append(toText).append(fromText).toString()
      ...
      val toTextFieldState =
          newFields[targetIndex - 1].copy(value = concatTextFieldValue).apply {
            val newChangeType =
                if (toText.isEmpty() && fromText.isNotEmpty()) {
                  ...
                  if (toField.changeType == LineChangeType.NONE) LineChangeType.UPDATED
                  else toField.changeType
                }
      ...
      updateStyles(2, newState) { baseStyles ->
        ...
        updateStylesAfterDeletedLineBreak(
            fields,
            baseStyles,
            toLineIdx,
            ignoreThis = false,
  ...
  // 若只传columnStartIndexInclusive，相当于定位到某一行的某一列；若传columnStartIndexInclusive 和
  // columnEndIndexExclusive，相当于选中某行的某一段文字
  ...
  suspend fun selectPrevOrNextField(
    ...
    lock.withLock {
      ...
      if ((isNext && selectedIndex >= fields.lastIndex) || (isNext.not() && selectedIndex <= 0)) {
        return
      }
      ...
      // A thing I am not sure: when layout direction is RTL, maybe need use min instead of max?
      // (not important, because most time navigate by Down/Up key without selection of text)
      val targetColumn =
          curField.value.selection.max.let {
  ...
  /**
   * 区域选中有bug，高亮也有bug，就算选中或高亮，后面会自动触发TextField
   * OnValueChange，然后就丢失选中或样式了，不过写代码的时候如果需要选中区域或高亮区域，正常传参就行，这样以后修复bug就不需要再改了
   */
  private fun selectFieldInternal(
    ...
    val selection =
        provideTextFieldValue?.selection
            ?: run {
              val textLenOfTarget = targetInFieldList.value.text.length
              val columnStartIndexInclusive =
                  columnStartIndexInclusive.coerceAtMost(textLenOfTarget).coerceAtLeast(0)
              val columnEndIndexExclusive =
                  columnEndIndexExclusive.coerceAtMost(textLenOfTarget).coerceAtLeast(0)
    ...
    // 高亮关键字不聚焦是为了避免有的语言（例如英语）输入法，明明没编辑，也会触发onValueChange导致字段被更新，然后高亮闪一下消失，但不聚焦又不会弹出键盘和定位光标，用起来不方便，所以取消，如果想避免高亮关键字闪一下就消失，可开只读模式再搜索
    ...
    //                        it.copy(value = it.value.copy(selection = TextRange.Zero, text =
    // it.value.text))
    //                    }
    //                }
    //            }
    ...
    if (isMultipleSelectionMode && requireSelectLine) { // 多选模式，添加选中行列表或从列表移除
      //            ret_fields = if(isMutableFields) (ret_fields as MutableList) else
      // ret_fields.toMutableList()
      ret_selectedIndices =
          if (isMutableSelectedIndices) (ret_selectedIndices as MutableList)
          else ret_selectedIndices.toMutableList()
      ...
      if (provideTextFieldValue != null) {
        if (provideTextFieldValue != targetInFieldList.value) {
          ret_fields =
              if (isMutableFields) (ret_fields as MutableList) else ret_fields.toMutableList()
          ret_fields[targetIndex] = targetInFieldList.copy(value = provideTextFieldValue)
        }
        ...
        if (selectionRangeChanged) {
          ret_fields[targetIndex] =
              targetInFieldList.copy(value = targetInFieldList.value.copy(selection = selection))
        }
      }
    }
  ...
  private fun splitTextsByNL(
    ...
    for ((index, text) in lines.withIndex()) {
      ret.add(
          if (
              maybeIsPaste
          ) { // if it is pasted content, doesn't add intent to avoid unexpected intents
          ...
          if (direction == FindDirection.UP) {
            ...
            if (!isGoodIndexForList(tempStartIndex, list)) {
              throw RuntimeException(
                  "bad tempStartIndex: $tempStartIndex , list.size: ${list.size}"
              )
            }
  ...
  suspend fun deleteLineByIndices(
      ...
      // if deleteSelected, should be true, if is accept ours/theirs call this method, should be
      // false,
      //   if not sure, use true is safe, will only deleted indices lines and clear selected
      // indices;
    ...
    lock.withLock {
      ...
      if (newFields.isEmpty()) {
        ...
        val newLinesCoveredOldIndex =
            if (
                lastDeletedIndex != null && lastDeletedIndex < newFields.size
        ...
        newFields[focusingLineIdx] =
            newFields[focusingLineIdx].let {
              it.copy(
                  // if new lines covered last deleted index, then move cursor to new line's first
                  // column, else move to last
                  value =
                      it.value.copy(
                          selection =
                              if (newLinesCoveredOldIndex) TextRange.Zero
                              else TextRange(it.value.text.length)
                      )
              )
            }
      }
      ...
      val newState =
          internalCreate(
              ...
              fieldsId = newId(),
              selectedIndices =
                  if (trueClearAllSelectedIndicesFalseOnlyClearWhichDeleted) {
                    listOf()
                  } else {
      ...
      // if deleted lines count more than rest of lines, then re-analyze syntax highlight may better
      // than incremental update,
      // else deleted lines
  ...
  //                selectedIndices = selectedIndices,
  //                isMultipleSelectionMode = isMultipleSelectionMode,
  // //拷贝和删除不要退出选择模式(准确来说是不要改变是否处于选择模式，若以后以非多选模式创建CopiedState，也不会自动进入选择模式)，这样用户可间接实现剪切功能，因为选择很多行有时候很废力，所以除非用户明确按取消，否则不要自动解除选择模式
  //                focusingLineIdx = focusingLineIdx
  //
  //            )
  //
  ...
  suspend fun deleteSelectedLines() {
    ...
    deleteLineByIndices(
        selectedIndices,
        trueClearAllSelectedIndicesFalseOnlyClearWhichDeleted = true,
    )
  }
      ...
      val newState =
          internalCreate(
              ...
              isMultipleSelectionMode =
                  isMultipleSelectionMode, // 一般来说此值在这会是true，不过，这里的语义是“不修改是否选择模式”，所以把这个字段传过去比直接设为true要合适
              focusingLineIdx = focusingLineIdx,
          )
  ...
  /** 计算行索引对应的以像素为单位大概滚动位置。 注：只能算个大概，如果有图片之类的，文字可能只占一行，但预览可能占很多行，这时滚动可能会有较大偏差 */
  fun lineIdxToPx(
      lineIndex: Int,
      fontSizeInPx: Float,
      screenWidthInPx: Float,
    ...
    val (oneLineHowManyChars, luckyOffset) =
        getOneLineHowManyCharsAndLuckyOffset(
            lineHeight = lineHeight,
            screenWidthInPx = screenWidthInPx,
  ...
  /** 计算索引对应的大概行索引。 注：只能算个大概，如果有图片之类的，文字可能只占一行，但预览可能占很多行，这时滚动可能会有较大偏差 */
  fun pxToLineIdx(
      targetPx: Int,
      fontSizeInPx: Float,
      screenWidthInPx: Float,
  ...
  private fun getOneLineHowManyCharsAndLuckyOffset(
      lineHeight: Float,
      screenWidthInPx: Float,
      screenHeightInPx: Float,
      indexToPx: Boolean,
  ...
  fun focusingLineIndexInvalid() =
      focusingLineIdx == null || focusingLineIdx < 0 || focusingLineIdx >= fields.size

  /**
   * 若forceAppend为假：如果当前行为空行，用text替换当前行；否则追加到当前行后面。 若forceAppend为真：强制追加，无论当前行是否为空行。
   *
   * targetIndex: `focusingLineIdx` 有效则用它，无效则用 `selectedIndices` 最后一个条目，两个都无效则直接返回
   *
   * If `forceAppend` is false: If the current line is empty, replace the current line with text;
   * otherwise, append text to the end of the current line. If `forceAppend` is true: Force append,
   * regardless of whether the current line is empty or not.
   *
   * targetIndex: will use `focusingLineIdx` if it is valid, or last item of `selectedIndices`, if
   * both are invalid, will return
   */
  suspend fun appendTextToLastSelectedLine(
    ...
    val lastSelectedIndexOfLine =
        if (focusedLineInvalid) selectedIndices.last() else (focusingLineIdx!!)
    if (lastSelectedIndexOfLine < 0 || lastSelectedIndexOfLine >= fields.size) {
      MyLog.w(
          TAG,
          "#appendTextToLastSelectedLine: invalid index '$lastSelectedIndexOfLine' of `fields`",
      )
      return
    }
    ...
    val trueAppendFalseReplace =
        if (forceAppend) true else fields[lastSelectedIndexOfLine].value.text.isNotEmpty()

    // 执行添加，到这才真正修改Editor的数据
    appendOrReplaceFields(
        targetIndex = lastSelectedIndexOfLine,
        text = text,
        trueAppendFalseReplace = trueAppendFalseReplace,
    )
   ...
   *
   * WARN: Don't cache this function's returned value, because: 1.(the main reason) this function at
   * most call twice, one for syntax highlighting, one for saving, but if cache it, it will cache
   * for every instance, usually, it will duplicate `n * undoStack.size`, but after syntax
   * highlighting finished, we only need another 1 copy for saving, so other copies just waste
   * memory. 2.(this just so so) usually users edit a small file, so `getAllText()` will run very
   * fast, so no need cache the returned value (第1个理由是主要原因，第2个其实有点牵强，总之，主要还是浪费内存，所以不建议缓存)
   */
  ...
  /** 此函数不比较fields数组，缺点是返回结果并不准确(真或假都不靠谱)，优点是快 用来粗略判断当前状态是否需要入撤销栈（undoStack） */
  fun maybeNotEquals(other: TextEditorState): Boolean {
    ...
    //        return this.fieldsId != other.fieldsId || this.isMultipleSelectionMode !=
    // other.isMultipleSelectionMode || this.selectedIndices != other.selectedIndices
    ...
    lock.withLock {
      ...
      val sfiRet =
          selectFieldInternal(
              ...
              targetIndex = targetIndex,
              option =
                  if (goToTop) SelectionOption.FIRST_POSITION else SelectionOption.LAST_POSITION,
          )
    ...
    return lock.withLock {
      val handled =
          try {
            ...
            MyLog.e(
                TAG,
                "$TAG#handleTabIndent err: replace ${if(trueTabFalseShiftTab) "TAB" else "SHIFT+TAB"} to indent spaces err: ${e.stackTraceToString()}",
            )
            false
          }
  ...
  private fun doTab(tabIndentSpacesCount: Int, f: MyTextFieldState): HandleTabRet {
    ...
    val newText =
        sb.append(tabToSpaces(tabIndentSpacesCount))
            .append(fv.text.substring(cursorAt, fv.text.length))
            .toString()
    // at least 1char, because even set tabIndentSpacesCount to less than 0, it still will insert a
    // real tab
    val cursorOffset = tabIndentSpacesCount.coerceAtLeast(1)
    val newSelection =
        if (fv.selection.collapsed) TextRange(cursorAt + cursorOffset)
        else
            TextRange(
                start = fv.selection.start + cursorOffset,
                end = fv.selection.end + cursorOffset,
            )
   ...
   */
  private fun doShiftTab(tabIndentSpacesCount: Int, f: MyTextFieldState): HandleTabRet {
    val fv = f.value
    // if empty or not starts with tab or space, return
    if (
        fv.text.let {
          it.isEmpty() ||
              (it.startsWith(IndentChar.SPACE.char).not() &&
                  it.startsWith(IndentChar.TAB.char).not())
        }
    ) {
    ...
    var leftSub = ""
    val text =
        if (fv.selection.collapsed && fv.selection.start.let { it > 0 && it < fv.text.length }) {
    ...
    val (newTextTmp, removedCount) =
        if (
            tabIndentSpacesCount < 1 || text.startsWith(IndentChar.TAB.char)
        ) { // remove a tab or a space
    ...
    return if (removedCount < 1) {
      ...
      val newSelection =
          if (fv.selection.collapsed) {
            if (useSubString) {
              // if use sub string, we operate the sub string which on the right side of the text,
              // so the cursor haven't moved
            ...
            TextRange(
                start = (fv.selection.start - removedCount).coerceAtLeast(0),
                end = (fv.selection.end - removedCount).coerceAtLeast(0),
            )
          }
  ...
  suspend fun applySyntaxHighlighting(
      stylesResult: StylesResult,
      requireCallOnChange: Boolean = true,
  ) {
    ...
    MyLog.d(
        TAG,
        "#$funName: StylesResult may will be apply: $stylesResult, spans.LineCount=$spansLineCount, fields.size=${fields.size}",
    )
    ...
    //            MyLog.w(TAG, "#$funName: will not apply stylesResult, because spans.lineCount
    // doesn't match the fields.size: spans.lineCount=$spansLineCount, fields.size=${fields.size}")
    ...
    // already applied
    //        if(stylesResult.from == StylesResultFrom.CODE_EDITOR && stylesResult.uniqueId ==
    // temporaryStyles?.uniqueId) {
    //            MyLog.d(TAG, "already applied styles (from code editor),
    // uniqueId=${stylesResult.uniqueId}")
    //            return
    //        }
    ...
    lock.withLock {
      ...
      // if has err, just return, then will not save the styles map, so UI will show a normal text
      ...
      try {
        ...
        fields.forEachIndexedBetter { idx, value ->
          ...
          val annotatedString =
              codeEditor.generateAnnotatedStringForLine(value, spans).let {
                codeEditor.ifSameReturnCachedAnnotatedString(value.syntaxHighlightId, it)
              }

          syntaxHighlightStorage.put(value.syntaxHighlightId, annotatedString)
        }
      } catch (e: Exception) {
        MyLog.e(
            TAG,
            "#$funName: apply styles err: fieldsId=$fieldsId, stylesResult=$stylesResult, err=${e.localizedMessage}",
        )
        e.printStackTrace()
      ...
      codeEditor?.putSyntaxHighlight(
          fieldsId,
          SyntaxHighlightResult(inDarkTheme = inDarkTheme, storage = syntaxHighlightStorage),
      )
    }

    // must free this instance's lock before try apply style, else will dead lock
    if (requireCallOnChange) {
      // in this case, first param `nextState` even is  `this`, but it will not be applied, will use
      // copied latest state as new state
      codeEditor?.doActWithLatestEditorState("#applySyntaxHighlighting") { latestState ->
        if (latestState.fieldsId == fieldsId) {
          ...
          onChanged(
              ...
              false,
              this,
              EditorStateOnChangeCallerFrom.APPLY_SYNTAX_HIGHLIGHTING,
          )
        } else {
          MyLog.d(
              TAG,
              "editor state already changed, ignore style update request by previous style's apply syntax highlighting method, newStateFieldsId=${latestState.fieldsId}, currentAppliedFieldsId=${fieldsId}",
          )
        }
      }
    }
  ...
  fun tryGetStylesResult(): StylesResult? {
    ...
    // have a little bug (its acceptable, no fix still ok): if users clicked undo/redo, then change
    // content,
  ...
  private fun updateStylesAfterDeletedLineBreak(
    ...
    if (
        !isGoodIndexForList(startLineIndex, baseFields) ||
            !isGoodIndexForList(endLineIndexInclusive, baseFields)
    ) {
      MyLog.d(
          TAG,
          "$funName: bad index: start=$startLineIndex, end=$endLineIndexInclusive, list.size=${baseFields.size}",
      )
      return
    }
    ...
    val start =
        CharPosition(startLineIndex, baseFields[startLineIndex].value.text.length, startIdxOfText)
    ...
    MyLog.d(
        TAG,
        "#$funName: start=$start, end=$end, baseFields.size=${baseFields.size}, newState.fields.size=${newTextEditorState.fields.size}, spansCount=${stylesResult.styles.spans.lineCount}",
    )

    //        println("baseFields[insertIndex].value.text:
    // ${baseFields.getOrNull(end.line)?.value?.text}")
    ...
    MyLog.d(
        TAG,
        "#$funName: adjusted on delete, spans.lineCount = ${stylesResult.styles.spans.lineCount}, baseFields.size = ${baseFields.size}",
    )
  ...
  //   explain: delete index 8 will not affect index before 8, so need descend, else the delete
  // logic must change
  private fun updateStylesAfterDeleteLine(
    ...
    val endLineIndex =
        if (keepLine) startLineIndex else (startLineIndex + 1).coerceAtMost(baseFields.lastIndex)
    MyLog.d(
        TAG,
        "$funName: startLineIndex: $startLineIndex, endLineIndex=$endLineIndex, list.size=${baseFields.size}",
    )
    if (
        !isGoodIndexForList(startLineIndex, baseFields) ||
            !isGoodIndexForList(endLineIndex, baseFields)
    ) {
      MyLog.d(
          TAG,
          "$funName: bad range: startLineIndex: $startLineIndex, endLineIndex=$endLineIndex, list.size=${baseFields.size}",
      )
      return
    }
    ...
    val start =
        CharPosition(
            startLineIndex,
            if (keepLine) 0 else baseFields[startLineIndex].value.text.length,
            startIdxOfText,
        )
    ...
    MyLog.d(
        TAG,
        "#$funName: start=$start, end=$end, baseFields.size=${baseFields.size - 1}, newState.fields.size=${newTextEditorState.fields.size}, spansCount=${stylesResult.styles.spans.lineCount}",
    )
    ...
    MyLog.d(
        TAG,
        "#$funName: adjusted on delete, spans.lineCount = ${stylesResult.styles.spans.lineCount}, baseFields.size = ${baseFields.size - 1}",
    )
  ...
  private fun updateStylesAfterInsertLine(
    ...
    MyLog.d(
        TAG,
        "$funName: startLineIndex=$startLineIndex, baseFields.size=${baseFields.size}, $stylesResult",
    )
    ...
    if (startLineIndex >= baseFields.size) {
      MyLog.d(
          TAG,
          "$funName: startLineIndex '$startLineIndex' is invalid, baseFields.size=${baseFields.size}",
      )
      return
    }

    // the insert position is start of the line, and the line should clear by `afterDelete` before
    // insert,
    ...
    val end =
        CharPosition(
            insertIndex,
            newTextEditorState.fields.get(insertIndex).value.text.length,
            startIdxOfText + insertedContent.length,
        )
    ...
    MyLog.d(
        TAG,
        "#$funName: adjusted on insert, spans.lineCount = ${stylesResult.styles.spans.lineCount}, baseFields.size = ${baseFields.size}",
    )
    ...
    if (lang != null) {
      codeEditor.sendUpdateStylesRequest(
          StylesUpdateRequest(
              ignoreThis = ignoreThis,
              targetEditorState = newTextEditorState,
              act = { lang.analyzeManager.insert(start, end, selectedText /*, it*/) },
          )
      )
    }
  }

  private fun reAnalyzeBetterThanIncremental(
      baseFields: List<MyTextFieldState>,
      ignoreThis: Boolean,
  ): Boolean {
    return ignoreThis.not() &&
        (baseFields.isEmpty() || (baseFields.size == 1 && baseFields[0].value.text.isBlank()))
  }
  ...
  fun getIndexOfText(
    ...
    // due to find to line's start will - 1, so if find index of start, the lineIdx range is [0,
    // lastIndexOfFields - 1],
    //  so, if the lineIdx == lastIndexOfFields is true, it must find end of the line, hence, should
    // - 1 for remove last line break count for the last line
  ...
  suspend fun moveCursor(
    ...
    if (
        trueToLeftFalseRight &&
            ((headOrTail && atLineHead) || (!headOrTail && atLineHead && atFileHead))
    ) {
      return
    }
    ...
    if (
        !trueToLeftFalseRight &&
            ((headOrTail && atLineTail) || (!headOrTail && atLineTail && atFileTail))
    ) {
      return
    }

    if (atLineHead && trueToLeftFalseRight) {
      selectField(targetIndex = targetFieldIndex - 1, option = SelectionOption.LAST_POSITION)
      return
    }

    if (atLineTail && !trueToLeftFalseRight) {
      selectField(targetIndex = targetFieldIndex + 1, option = SelectionOption.FIRST_POSITION)
      return
    }
    ...
    lock.withLock {
      val newTextRange =
          if (trueToLeftFalseRight) {
            if (headOrTail) TextRange(0) else TextRange((textRange.start - 1).coerceAtLeast(0))
          } else {
            if (headOrTail) TextRange(textFieldState.value.text.length)
            else TextRange((textRange.start + 1).coerceAtMost(textFieldState.value.text.length))
          }

      val newFields = fields.toMutableList()
      newFields[targetFieldIndex] =
          newFields[targetFieldIndex].copy(
              value = textFieldState.value.copy(selection = newTextRange)
          )
  ...
  /**
   * if selected all, replace; else append to the last selected line after pasted, will select
   * pasted content
   */
  suspend fun paste(
      text: String,
  ...
  suspend fun asUnsaved() {
    lock.withLock {
      ...
      // create new fieldsId for enable save button, but needn't update the undo stack because
      // content no change
  ...
  companion object {
    ...
    fun fuckSafFileToFields(file: FuckSafFile, charsetName: String?) =
        linesToFields(FsUtils.readLinesFromFile(file, charsetName, addNewLineIfFileEmpty = true))
...
private fun createInitTextFieldStates(list: List<String>) =
    if (list.isEmpty()) {
