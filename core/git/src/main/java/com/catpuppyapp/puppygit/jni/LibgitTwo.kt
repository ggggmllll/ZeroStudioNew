...
object LibgitTwo {
  ...
  // 指向clone后产生的git仓库对象的指针，c里一般用int做返回值来指示函数执行是否成功。然后传入一个指针对象用来存储需要返回的对象，例如：int
  // createStudent(Student* ret)，其中，int是返回值，但函数创建的Student结构体存储在ret指向的内存中
  //    private val jniCRepoPtr: Long = 0

  external fun jniLibgitTwoInit()

  external fun jniClone(
      url: String?,
      local_path: String?,
      jniCloneOptionsPtr: Long,
  ...
  */
  ...
  /**
   * jniCRetParamRepoPtr: url: 要clone的url local_path: 仓库本地路径 jniOptionsPtr:
   * clone命令的选项，是个指针，指向jni创建的options对象
   *
