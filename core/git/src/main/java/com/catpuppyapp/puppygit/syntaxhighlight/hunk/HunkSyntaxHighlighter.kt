...
class HunkSyntaxHighlighter(
  ...
  private fun doAnalyzeNoLock(scope: PLScope) {
    ...
    MyLog.w(
        TAG,
        "will run full syntax highlighting analyze(at $TAG, filename: ${hunk.diffItemSaver.fileName()})",
    )
    ...
    try {
      ...
      MyLog.e(
          TAG,
          "#doAnalyzeNoLock() err: call `TextMateUtil.setReceiverThenDoAct()` err: fileRelativePath=${hunk.diffItemSaver.relativePathUnderRepo} err=${e.stackTraceToString()}",
      )
    }
  }
