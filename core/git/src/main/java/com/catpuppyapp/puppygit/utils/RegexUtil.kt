...
object RegexUtil {
  ...
  private const val extFlagLenSubOne =
      extFlagLen - 1 // 减1是为了保留后缀名的"."，例如"*.txt"，substring 后得到 ".txt"，不减1就变成 "txt" 了

  private const val slash = Cons.slashChar
  private const val spaceChar = ' '

  /**
   * *.ext 可匹配后缀，多个*.可匹配不同后缀，只要目标匹配任一后缀就返回真，若有其他关键词例如"*.md *.txt filename_part1
   * part2"则会变成目标匹配任一后缀且包含其他所有关键词则返回真
   *
   * example: val input = "TARGET_FILE.md".lowercase() val pattern = listOf( "tar", // match, true
   * "tar fi*.md", // match, true "tar file *.md", // match, true "t?r", // no-match, false, '?' is
   * not treat as placeholder here, 这个不会把?当作占位符 "*.md", // match, true )
   *
   *     for (p in pattern) {
  ...
  fun matchWildcard(target: String, keyword: String, ignoreCase: Boolean = true): Boolean {
    ...
    for (k in splitKeyword) {
      ...
      if (
          k.length > extFlagLen && k.startsWith(extMatchFlag, ignoreCase = ignoreCase)
      ) { // 有后缀名。（注意："*." 会被认为无后缀名，然后去匹配else的 keyword part部分）
        needMatchExt = true

        if (!extMatched) { // 若已匹配后缀成功则不需要再匹配
          extMatched =
              target.endsWith(
                  k.substring(extFlagLenSubOne),
                  ignoreCase = ignoreCase,
              ) // `extFlagLen - 1` to keep '.', e.g. "*.txt" will got ".txt"
        }
  ...
  fun matchByPredicate(
      target: String,
      keywordList: List<String>,
      predicate: (target: String, keyword: String) -> Boolean,
  ): Boolean {
