...
fun CreateBranchDialog(
    ...
    //    onOk: (branchName:String, baseRefSpec:String, basedHead:Boolean, createByRef:Boolean,
    // needCheckout:Boolean, forceCheckout:Boolean, overwriteIfExist:Boolean) -> Unit,
) {
  ...
  //    val textUnderCheckout =
  // stringResource(R.string.warn_please_commit_your_change_before_checkout_or_merge)

  val optHEAD = 0
  val optCommit = 1
  val selectedOption = rememberSaveable { mutableIntStateOf(optHEAD) }
  val createMethodList =
      listOf(activityContext.getString(R.string.head), activityContext.getString(R.string.commit))
  ...
  // 返回值为：分支长名，短名，完整hash
  val doCreateBranch:
      suspend (String, Boolean, String, Boolean, Boolean) -> Ret<Triple<String, String, String>?> =
      doCreateBranch@{
  ...
  val doCheckoutBranch: suspend (String, String, String, force: Boolean) -> Ret<Oid?> =
        ...
        ScrollableColumn {
          ...
          if (selectedOption.intValue == optCommit) {
            ...
            TextField(
                ...
                singleLine = true,
                onValueChange = { userInputHash.value = it },
      ...
      confirmButton = {
        TextButton(
            onClick = {
              ...
              // 准备参数
              // branchName:String, baseRefSpec:String, basedHead:Boolean, createByRef:Boolean,
              ...
              val createByRef =
                  false // 如果basedHead为true，此值被忽略，否则，如果此值为true，根据分支名（引用名）查提交号，为false，代表入参是提交号直接使用提交号。在这里，如果basedHead为true，此值无意义，如果basedHead为假，我需要此值为假以使用用户输入的提交号，所以此值在此设为常量假即可
              ...
              doJobThenOffLoading(
                ...
                try {
                  // 支持基于分支名、tag名创建提交
                  val actuallyBaseRefSpecCommitHash =
                      if (!basedHead) { // 如果选的不是基于HEAD创建提交，查询一下
                        var r = baseRefSpec
                        // 解析一下refspec
                        Repository.open(curRepo.fullSavePath).use { repo ->
                  ...
                  val createBranchRet =
                      doCreateBranch(
                          branchName,
                          basedHead,
                          actuallyBaseRefSpecCommitHash,
                          createByRef,
                          overwriteIfExist.value,
                      )

                  if (createBranchRet.hasError()) { // 创建分支失败
                    throw RuntimeException(
                        activityContext.getString(R.string.create_branch_err) +
                            ": " +
                            createBranchRet.msg
                    )
                  }
                  ...
                  if (needCheckout) {
                    ...
                    if (checkoutRet.hasError()) {
                      throw RuntimeException(
                          activityContext.getString(R.string.checkout_error) +
                              ": " +
                              checkoutRet.msg
                      )
                    }
            ...
            enabled =
                branchName.value.isNotBlank() &&
                    (if (selectedOption.intValue == optCommit) userInputHash.value.isNotBlank()
                    else true),
        ) {
