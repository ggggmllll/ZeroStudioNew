...
object FileValidator {
  ...
  /**
   * 判断是否为视频或音频 原理：使用 Android MediaExtractor 解析媒体轨道。 性能：只分析媒体容器结构（Container Structure），不解码帧。
   *
  ...
  /** 判断是否为归档文件 (Zip, Jar, Apk, Docx, etc.) 原理：尝试作为 ZipInputStream 打开并读取第一个 Entry。 读取头部结构。 */
  ...
  /**
   * 【高精度】判断是否为纯文本文件 原理：读取前段字节，检查是否存在“非法二进制字符”。 纯文本文件（UTF-8/ASCII等）不应包含 NULL 字节 (0x00) 或大量不可见控制字符。
   *
   * 读取最多 2KB 数据。
   */
