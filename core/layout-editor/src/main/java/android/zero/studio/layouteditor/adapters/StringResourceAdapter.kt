...
import android.zero.studio.layouteditor.databinding.LayoutValuesItemBinding
import android.zero.studio.layouteditor.databinding.LayoutValuesItemDialogBinding
import android.zero.studio.layouteditor.utils.FileUtil.writeFile
...
class StringResourceAdapter(
  ...
  class VH(var binding: LayoutValuesItemBinding) : RecyclerView.ViewHolder(binding.root) {
    var stringName: TextView = binding.name
    var stringValue: TextView = binding.value
  }

  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
    return VH(LayoutValuesItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))
  }
    ...
    popupMenu.setOnMenuItemClickListener { item: MenuItem ->
      ...
      when (id) {
        R.id.menu_copy_name -> {
          ...
          make(v, "${v.context.getString(R.string.copied)} ${stringList[position].name}")
              .setSlideAnimation()
              .showAsSuccess()
          true
        }
        ...
        R.id.menu_delete -> {
          MaterialAlertDialogBuilder(v.context)
              ...
              .setMessage(String.format("Do you want to remove %s?", stringList[position].name))
              .setNegativeButton(R.string.no, null)
              .setPositiveButton(R.string.yes) { _, _ ->
                val name = stringList[position].name
                if (name == "default_string") {
                  make(v, v.context.getString(R.string.msg_cannot_delete_default, "string"))
                      .setFadeAnimation()
  ...
  private fun editString(v: View, pos: Int) {
    ...
    builder.setPositiveButton(R.string.okay) { dlg: DialogInterface?, i: Int ->
