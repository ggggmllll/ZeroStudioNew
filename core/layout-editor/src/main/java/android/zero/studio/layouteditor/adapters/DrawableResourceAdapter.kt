...
import android.zero.studio.layouteditor.activities.PreviewDrawableActivity
import android.zero.studio.layouteditor.adapters.models.DrawableFile
import android.zero.studio.layouteditor.databinding.LayoutDrawableItemBinding
import android.zero.studio.layouteditor.databinding.TextinputlayoutBinding
import android.zero.studio.layouteditor.managers.ProjectManager.Companion.instance
import android.zero.studio.layouteditor.utils.FileUtil.deleteFile
import android.zero.studio.layouteditor.utils.FileUtil.getLastSegmentFromPath
...
class DrawableResourceAdapter(private val drawableList: MutableList<DrawableFile>) :
  ...
  inner class VH(var binding: LayoutDrawableItemBinding) : RecyclerView.ViewHolder(binding.root) {
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
    return VH(LayoutDrawableItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))
  }
  ...
  override fun onBindViewHolder(holder: VH, position: Int) {
    ...
    TooltipCompat.setTooltipText(holder.binding.root, name.substring(0, name.lastIndexOf(".")))
    ...
    holder.binding.menu.setOnClickListener {
      showOptions(it, holder.absoluteAdapterPosition, holder)
    }
      ...
      when (id) {
        R.id.menu_copy_name -> {
          ClipboardUtils.copyText(
              drawableList[position].name.substring(0, drawableList[position].name.lastIndexOf("."))
          )
          ...
          MaterialAlertDialogBuilder(v.context)
              ...
              .setPositiveButton(R.string.yes) { _, _ ->
                ...
                if (name.substring(0, name.lastIndexOf(".")) == "default_image") {
                  make(v, v.context.getString(R.string.msg_cannot_delete_default, "image"))
                      .setFadeAnimation()
  ...
  private fun rename(v: View, position: Int, holder: VH) {
    ...
    val lastSegment = getLastSegmentFromPath(drawableList[position].path)
    ...
    builder.setPositiveButton(R.string.rename) { _, _ ->
      if (
        ...
        if (
            drawableList[position].name.endsWith(".xml") ||
