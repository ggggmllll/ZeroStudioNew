...
import android.zero.studio.layouteditor.adapters.models.FontItem
import android.zero.studio.layouteditor.databinding.LayoutFontItemBinding
...
class FontResourceAdapter(private val fontList: MutableList<FontItem>) :
  ...
  class VH(var binding: LayoutFontItemBinding) : RecyclerView.ViewHolder(binding.root) {
    var fontName = binding.name
    var fontLook = binding.fontLook
  }

  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
    return VH(LayoutFontItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))
  }
  ...
  override fun onBindViewHolder(holder: VH, position: Int) {
    ...
    holder.itemView.setOnClickListener {
      ...
      make(holder.binding.root, "${it.context.getString(R.string.copied)} $fontName")
          .setSlideAnimation()
          .showAsSuccess()
    }
          ...
          MaterialAlertDialogBuilder(v.context)
              ...
              .setPositiveButton(R.string.yes) { _, _ ->
                ...
                if (name.substring(0, name.lastIndexOf(".")) == "default_font") {
                  make(v, v.context.getString(R.string.msg_cannot_delete_default, "font"))
                      .setFadeAnimation()
  ...
  private fun rename(v: View, position: Int, holder: VH) {
    ...
    @Suppress("DEPRECATION") builder.setView(bind.root, padding, padding, padding, padding)
    builder.setTitle(R.string.rename_font)
    builder.setNegativeButton(R.string.cancel) { _, _ -> }
    builder.setPositiveButton(R.string.rename) { _, _ ->
      if (
          fontList[position].name.substring(0, fontList[position].name.lastIndexOf(".")) ==
              "default_font"
      ) {
