...
import android.zero.studio.layouteditor.adapters.PagerAdapter
import android.zero.studio.layouteditor.adapters.models.DrawableFile
import android.zero.studio.layouteditor.databinding.ActivityResourceManagerBinding
import android.zero.studio.layouteditor.fragments.resources.ColorFragment
import android.zero.studio.layouteditor.fragments.resources.DrawableFragment
import android.zero.studio.layouteditor.fragments.resources.FontFragment
import android.zero.studio.layouteditor.fragments.resources.StringFragment
...
class ResourceManagerActivity : BaseActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding = ActivityResourceManagerBinding.inflate(layoutInflater)
    ...
    if (ProjectManager.instance.openedProject == null) {
      ...
      if (extras != null && extras.containsKey(Constants.EXTRA_KEY_PROJECT)) {
        ProjectManager.instance.openProject(extras.getParcelable(Constants.EXTRA_KEY_PROJECT))
      }
    }
    // loadDrawables()
    ...
    if (id == R.id.menu_add) {
      ...
      if (fragment != null) {
        when (fragment) {
          ...
          is StringFragment -> {
            val it = Intent().setClass(this, ShowXMLActivity::class.java)
            it.putExtra(ShowXMLActivity.EXTRA_KEY_XML, ProjectManager.instance.stringsXml)
            startActivity(it)
          }
  ...
  private fun launchPhotoPicker() {
    if (isPhotoPickerAvailable(this)) {
      ...
      pickMedia!!.launch(PickVisualMediaRequest.Builder().setMediaType(ImageOnly).build())
    } else {
    ...
    if (uri != null) {
      ...
      if (fragment is DrawableFragment) {
        try {
          ...
          if (drawable.isVector) fragment.addDrawable(uri)
          else
