...
import android.zero.studio.layouteditor.managers.SharedPreferencesKeys
          ...
          MaterialAlertDialogBuilder(requireContext())
              ...
              .setSingleChoiceItems(themes, themes.indexOf(selectedThemeValue)) { d, w ->
                PreferencesManager.prefs
                    .edit()
                    .putString(SharedPreferencesKeys.KEY_APP_THEME, themes[w])
                    .apply()
      ...
      MaterialAlertDialogBuilder(requireContext())
          ...
          .setNegativeButton(R.string.cancel) { d, _ ->
            preference
                ?.sharedPreferences
                ?.edit()
                ?.putBoolean(preference.key, !PreferencesManager.isApplyDynamicColors)
                ?.apply()
            preference?.isChecked = PreferencesManager.isApplyDynamicColors
            d.cancel()
          }
          .setPositiveButton(R.string.okay) { _, _ -> AppUtils.relaunchApp(true) }
          .show()
      true
    }
  }
