package android.zero.studio.layouteditor.editor.convert
...
class ConvertImportedXml(private val xml: String?) {
  ...
  fun getXmlConverted(context: Context): String? {
    ...
    if (isWellFormed(xml)) {
      ...
      val matcher = Pattern.compile(pattern).matcher(xml.toString())
      try {
        while (matcher.find()) {
          val fullTag = matcher.group(0)?.replace("<", "")
          val widgetName = matcher.group(2)

          val classes = JSONObject(FileUtil.readFromAsset("widgetclasses.json", context))
