...
import android.zero.studio.layouteditor.databinding.LayoutColorItemBinding
...
class ColorResourceAdapter(
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
    return VH(LayoutColorItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))
  }
  ...
  override fun onBindViewHolder(holder: VH, position: Int) {
    ...
    holder.colorPreview.setImageDrawable(drawCircle(Color.parseColor(colorList[position].value)))
  ...
  private fun editColor(v: View, pos: Int) {
    ...
    etValue.setOnClickListener {
      val dialog: ColorPickerDialog.Builder =
          ColorPickerDialog.Builder(v.context)
              .setTitle("Choose Color")
              .setPositiveButton(
                  v.context.getString(R.string.confirm),
                  ColorEnvelopeListener { envelope, _ -> etValue.setText("#${envelope.hexCode}") },
              )
    ...
    builder.setPositiveButton(R.string.okay) { _, _ ->
      if (
          colorList[pos].name == "default_color" && etName.getText().toString() != "default_color"
      ) {
