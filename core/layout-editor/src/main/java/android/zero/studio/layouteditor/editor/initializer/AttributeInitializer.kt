package android.zero.studio.layouteditor.editor.initializer
...
class AttributeInitializer {
  ...
  fun applyAttribute(target: View, value: String, attribute: HashMap<String, Any>) {
    ...
    if (value.startsWith("@+id/") && viewAttributeMap[target]!!.contains("android:id")) {
      for (view in viewAttributeMap.keys) {
        ...
        for (key in map!!.keySet()) {
          ...
          if (
              mValue.startsWith("@id/") &&
                  mValue == viewAttributeMap[target]!!.getValue("android:id").replace("+", "")
          ) {
