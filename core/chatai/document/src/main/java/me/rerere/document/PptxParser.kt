...
private data class SlideContent(val slideNumber: Int, val content: String, val notes: String = "")
    ...
    return try {
      ZipFile(file).use { zipFile ->
        ...
        slideEntries.forEachIndexed { index, entry ->
          ...
          val slideContent = zipFile.getInputStream(entry).use { stream -> parseSlideXml(stream) }

          // Try to get notes for this slide
          val notesEntry = zipFile.getEntry("ppt/notesSlides/notesSlide${slideNumber}.xml")
          val notes =
              if (notesEntry != null) {
                zipFile.getInputStream(notesEntry).use { stream -> parseNotesXml(stream) }
              } else ""
      ...
      when (parser.eventType) {
        XmlPullParser.START_TAG -> {
          when (parser.name) {
            ...
            "lvl" -> {
              parser.getAttributeValue(null, "val")?.let { level = it.toIntOrNull() ?: 0 }
            }
          }
        }
