...
private data class ListInfo(val level: Int, val isNumbered: Boolean, val number: Int)

private data class ParagraphProperties(val listInfo: ListInfo?, val headingLevel: Int)
        ...
        XmlPullParser.START_TAG -> {
          when (parser.name) {
            "pStyle" -> {
              ...
              if (
                  styleVal?.startsWith("Heading") == true || styleVal?.startsWith("heading") == true
              ) {
              ...
              while (parser.next() != XmlPullParser.END_DOCUMENT) {
                when (parser.eventType) {
                  XmlPullParser.START_TAG ->
                      when (parser.name) {
                        "ilvl" ->
                            listLevel = parser.getAttributeValue(null, "val")?.toIntOrNull() ?: 0
