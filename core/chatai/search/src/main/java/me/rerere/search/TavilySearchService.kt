...
object TavilySearchService : SearchService<SearchServiceOptions.TavilyOptions> {
  ...
  override fun Description() {
    ...
    TextButton(onClick = { urlHandler.openUri("https://app.tavily.com/home") }) {
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (response.isSuccessful) {
            val response = response.body.string().let { json.decodeFromString<SearchResponse>(it) }
  ...
  override suspend fun scrape(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          val body = buildJsonObject { put("urls", buildJsonArray { add(url) }) }
          val apiKey = keyRoulette.next(serviceOptions.apiKey, serviceOptions.id.toString())
          val request =
              Request.Builder()
          ...
          val response = httpClient.newCall(request).await()
          if (response.isSuccessful) {
            val response = response.body.string().let { json.decodeFromString<ScrapeResponse>(it) }
            return@withContext Result.success(
                ScrapedResult(
                    urls =
                        response.results.map {
