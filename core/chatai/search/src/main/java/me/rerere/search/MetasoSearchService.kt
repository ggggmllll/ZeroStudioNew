...
object MetasoSearchService : SearchService<SearchServiceOptions.MetasoOptions> {
  ...
  override fun Description() {
    Text(
        buildAnnotatedString {
          append("秘塔搜索: ")
          withLink(LinkAnnotation.Url("https://metaso.cn/")) { append("https://metaso.cn/") }
        }
    )
  }
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (response.isSuccessful) {
            ...
            val searchResponse =
                runCatching { json.decodeFromString<MetasoSearchResponse>(bodyRaw) }
                    .onFailure {
                      it.printStackTrace()
  ...
  data class MetasoSearchResponse(
      ...
      @SerialName("searchParameters") val searchParameters: MetasoSearchParameters,
      @SerialName("webpages") val webpages: List<MetasoWebpage>,
  )
