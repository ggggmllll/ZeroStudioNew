...
object JinaSearchService : SearchService<SearchServiceOptions.JinaOptions> {
  ...
  override fun Description() {
    ...
    TextButton(onClick = { urlHandler.openUri("https://jina.ai/") }) {
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (response.isSuccessful) {
            val responseData =
                response.body.string().let { json.decodeFromString<JinaSearchResponse>(it) }

            return@withContext Result.success(
                SearchResult(
                    items =
                        responseData.data.take(commonOptions.resultSize).map {
                          SearchResultItem(title = it.title, url = it.url, text = it.description)
                        }
                )
            )
          } else {
  ...
  override suspend fun scrape(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          val responseData =
              response.body.string().let { json.decodeFromString<JinaScrapeResponse>(it) }
  ...
  @Serializable data class JinaUsage(val tokens: Int)

  @Serializable
  data class JinaScrapeResponse(val code: Int, val status: Int, val data: JinaScrapeData)
