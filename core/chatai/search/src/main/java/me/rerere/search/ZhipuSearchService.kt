...
object ZhipuSearchService : SearchService<SearchServiceOptions.ZhipuOptions> {
  ...
  override fun Description() {
    ...
    TextButton(
        onClick = { urlHandler.openUri("https://bigmodel.cn/usercenter/proj-mgmt/apikeys") }
    ) {
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (response.isSuccessful) {
            ...
            val response =
                runCatching { json.decodeFromString<ZhipuDto>(bodyRaw) }
                    .onFailure {
                      it.printStackTrace()
                      println(bodyRaw)
  ...
  @Serializable
  data class ZhipuDto(@SerialName("search_result") val searchResult: List<ZhipuSearchResultDto>)
  ...
  data class ZhipuSearchResultDto(
      ...
      @SerialName("refer") val refer: String?,
