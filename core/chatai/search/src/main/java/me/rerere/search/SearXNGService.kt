...
object SearXNGService : SearchService<SearchServiceOptions.SearXNGOptions> {
  ...
  override suspend fun search(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          require(serviceOptions.url.isNotBlank()) { "SearXNG URL cannot be empty" }
          ...
          val request =
              Request.Builder()
                  ...
                  .apply {
                    // 添加HTTP Basic Auth支持
                    if (
                        serviceOptions.username.isNotBlank() && serviceOptions.password.isNotBlank()
                    ) {
                      header(
                          "Authorization",
          ...
          if (response.isSuccessful) {
            ...
            val searchResponse =
                runCatching { json.decodeFromString<SearXNGResponse>(bodyRaw) }
                    .onFailure {
                      it.printStackTrace()
            ...
            // 转换为标准格式，取前 N 个结果
            val items =
                searchResponse.results.take(commonOptions.resultSize).map { result ->
                  SearchResultItem(title = result.title, url = result.url, text = result.content)
                }
  ...
  data class SearXNGResponse(
      ...
      @SerialName("number_of_results") val numberOfResults: Int,
      @SerialName("results") val results: List<SearXNGResult>,
  )
  ...
  data class SearXNGResult(
      ...
      @SerialName("parsed_url") val parsedUrl: List<String> = emptyList(),
