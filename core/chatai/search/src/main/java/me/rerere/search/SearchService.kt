...
@Serializable data class SearchCommonOptions(val resultSize: Int = 10)
...
sealed class SearchServiceOptions {
  ...
  class BingLocalOptions(override val id: Uuid = Uuid.random()) : SearchServiceOptions()
  ...
  @Serializable
  @SerialName("exa")
  data class ExaOptions(override val id: Uuid = Uuid.random(), val apiKey: String = "") :
      SearchServiceOptions()
    ...
    enqueue(
        object : Callback {
          ...
          override fun onResponse(call: Call, response: Response) {
            continuation.resume(response) { cause, _, _ -> response.closeQuietly() }
          }
        }
    )
  }
