...
class PromptInjectionTransformerTest {
  ...
  ) = Assistant(modeInjectionIds = modeInjectionIds, lorebookIds = lorebookIds)
  ...
  ) = Lorebook(id = id, name = name, enabled = enabled, entries = entries)

  private fun getMessageText(message: UIMessage): String {
    return message.parts.filterIsInstance<UIMessagePart.Text>().joinToString("") { it.text }
  }
  ...
  fun `disabled mode injection should not be applied`() {
    ...
    val injection =
        createModeInjection(id = injectionId, enabled = false, content = "Should not appear")
  ...
  fun `mode injection with AFTER_SYSTEM_PROMPT should append to system message`() {
    ...
    val messages = listOf(UIMessage.system("Original system prompt"), UIMessage.user("Hello"))
  ...
  fun `injection without existing system message should create new system message`() {
    ...
    val messages = listOf(UIMessage.user("Hello"), UIMessage.assistant("Hi!"))
  ...
  fun `lorebook with keyword match should trigger injection`() {
    ...
    val regexInjection =
        createRegexInjection(keywords = listOf("magic"), content = "Magic system explanation")
    val lorebook = createLorebook(id = lorebookId, entries = listOf(regexInjection))

    val messages = listOf(UIMessage.system("System prompt"), UIMessage.user("Tell me about magic"))
  ...
  fun `lorebook without keyword match should not trigger injection`() {
    val lorebookId = Uuid.random()
    val regexInjection =
        createRegexInjection(keywords = listOf("magic"), content = "Should not appear")
    val lorebook = createLorebook(id = lorebookId, entries = listOf(regexInjection))

    val messages =
        listOf(UIMessage.system("System prompt"), UIMessage.user("Tell me about science"))
  ...
  fun `lorebook with constantActive should always trigger`() {
    ...
    val messages = listOf(UIMessage.system("System prompt"), UIMessage.user("Any message"))
  ...
  fun `different entries should use their own scanDepth`() {
    ...
    val lorebook = createLorebook(id = lorebookId, entries = listOf(shallowEntry, deepEntry))
  ...
  fun `disabled world book should not trigger`() {
    ...
    val regexInjection =
        createRegexInjection(keywords = listOf("magic"), content = "Should not appear")
    val lorebook =
        createLorebook(id = lorebookId, enabled = false, entries = listOf(regexInjection))
  ...
  fun `combined mode injection and world book should both apply`() {
    ...
    val modeInjection = createModeInjection(id = modeId, content = "Mode content")

    val regexInjection =
        createRegexInjection(keywords = listOf("hello"), content = "WorldBook content")
    val lorebook = createLorebook(id = lorebookId, entries = listOf(regexInjection))

    val messages = listOf(UIMessage.system("System prompt"), UIMessage.user("hello world"))

    val result =
        transformMessages(
            messages = messages,
            assistant =
                createAssistant(modeInjectionIds = setOf(modeId), lorebookIds = setOf(lorebookId)),
  ...
  fun `injection after ASSISTANT with tools should work normally`() {
    ...
    // 注入应该在最后一条用户消息之前
    val injectedIndex = result.indexOfFirst { getMessageText(it).contains("Bottom injection") }
    val lastUserIndex = result.indexOfLast {
      it.role == MessageRole.USER && getMessageText(it) == "Thanks!"
    }
