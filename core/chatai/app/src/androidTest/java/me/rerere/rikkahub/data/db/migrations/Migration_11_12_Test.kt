...
class Migration_11_12_Test {
  ...
  fun migrate11To12_migratesSimpleConversationCorrectly() {
    ...
    val conversationCursor =
        db.query("SELECT nodes FROM conversationentity WHERE id = ?", arrayOf(conversationId))
  ...
  fun migrate11To12_handlesBranchedMessages() {
    ...
    // 验证结果
    val cursor =
        db.query("SELECT * FROM message_node WHERE conversation_id = ?", arrayOf(conversationId))
  ...
  fun migrate11To12_handlesEmptyConversations() {
    ...
    val conversationCursor =
        db.query("SELECT id FROM conversationentity WHERE id = ?", arrayOf(conversationId))
  ...
  fun migrate11To12_handlesMultipleConversations() {
    ...
    // 验证第一个对话的节点数
    val cursor1 =
        db.query("SELECT * FROM message_node WHERE conversation_id = ?", arrayOf(conversationId1))
    assertEquals("Conversation 1 should have 1 message node", 1, cursor1.count)
    cursor1.close()

    // 验证第二个对话的节点数
    val cursor2 =
        db.query("SELECT * FROM message_node WHERE conversation_id = ?", arrayOf(conversationId2))
  ...
  fun migrate11To12_handlesVeryLargeConversations() {
                    ...
                    listOf(
                        UIMessage(
                            ...
                            parts =
                                listOf(
                                    UIMessagePart.Text(
                                        "Message $i with some content to increase size " +
                                            "x".repeat(100)
                                    )
                                ),
                        ),
                        UIMessage(
                            ...
                            parts =
                                listOf(
                                    UIMessagePart.Text(
                                        "Response $i with some content to increase size " +
                                            "y".repeat(100)
                                    )
                                ),
                            modelId = Uuid.random(),
    ...
    val largeConvCursor =
        db.query("SELECT nodes FROM conversationentity WHERE id = ?", arrayOf(largeConversationId))
    assertTrue(largeConvCursor.moveToFirst())
    val largeConvNodes = largeConvCursor.getString(0)
    largeConvCursor.close()

    // 验证普通对话的 nodes 应该被清空
    val normalConvCursor =
        db.query("SELECT nodes FROM conversationentity WHERE id = ?", arrayOf(normalConversationId))
