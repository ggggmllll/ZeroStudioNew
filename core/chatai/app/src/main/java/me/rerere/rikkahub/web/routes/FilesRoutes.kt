...
fun Route.filesRoutes(filesManager: FilesManager, context: Context) {
  route("/files") {
    ...
    delete("/{id}") {
      val idParam = call.pathParameters["id"] ?: throw BadRequestException("Missing file id")
      val id = idParam.toLongOrNull() ?: throw BadRequestException("Invalid file id")
    ...
    get("/id/{id}") {
      ...
      val entity = filesManager.get(id) ?: throw NotFoundException("File not found")
    ...
    get("/path/{path...}") {
      val relativePath =
          call.pathParameters.getAll("path")?.joinToString("/")
      ...
      val contentType =
          if (!managedFileMime.isNullOrBlank()) {
            managedFileMime
          } else {
            when (file.extension.lowercase()) {
              ...
              "config",
              "diff",
              "patch" -> ContentType.Text.Plain.toString()
              "md" -> ContentType("text", "markdown").toString()
              "html",
              "htm" -> ContentType.Text.Html.toString()
              "css" -> ContentType.Text.CSS.toString()
              "xml" -> ContentType.Text.Xml.toString()
              "csv" -> ContentType.Text.CSV.toString()
              "yml",
              "yaml" -> ContentType("text", "yaml").toString()
              "ini",
              "toml",
              "env" -> ContentType.Text.Plain.toString()
              "go",
              "py",
              "rs",
              "java",
              ...
              "sql",
              "graphql",
              "proto" -> ContentType.Text.Plain.toString()
  ...
  route("/assets") {
    get("/{path...}") {
      ...
      try {
        ...
        call.respondOutputStream { inputStream.use { it.copyTo(this) } }
      } catch (_: Exception) {
...
private fun ManagedFileEntity.toUploadedFileDto(filesManager: FilesManager) =
    UploadedFileDto(
        id = id,
