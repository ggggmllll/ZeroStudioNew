  ...
  LaunchedEffect(reasoning.reasoning, loading) {
    if (loading) {
      ...
      if (state.expandState.expanded) {
        state.expandState =
            if (settings.displaySetting.autoCloseThinking) ReasoningCardState.Collapsed
...
private fun ReasoningContent(
  ...
  Column(
      modifier =
          Modifier.fillMaxWidth().let { contentModifier ->
            if (isPreview) {
              contentModifier
...
private fun ReasoningTitle(title: String) {
  AnimatedContent(
    ...
    Text(
        ...
        style = MaterialTheme.typography.titleSmall,
        color = MaterialTheme.colorScheme.secondary,
        modifier = Modifier.padding(horizontal = 4.dp).shimmer(true),
    )
  }
}
