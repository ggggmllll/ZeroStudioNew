...
fun Route.conversationRoutes(
  ...
  route("/conversations") {
    ...
    get {
      ...
      val conversations =
          conversationRepo.getConversationsOfAssistant(settings.assistantId).first().map {
              conversation ->
    ...
    // POST /api/conversations/{id}/messages/{messageId}/edit - Edit a message as a new branch
    // version
    ...
    // POST /api/conversations/{id}/nodes/{nodeId}/select - Switch branch selection for a message
    // node
    ...
    post("/{id}/regenerate") {
      ...
      val message =
          node?.messages?.find { it.id == messageId }
      ...
      try {
        ...
        val errorEvents =
            chatService.errors
                ...
                .map { events -> ConversationStreamPayload.BatchErrors(events) }
          ...
          when (payload) {
            is ConversationStreamPayload.Conversation -> {
              ...
              if (nodeDiff != null) {
                ...
                val json =
                    JsonInstant.encodeToString(
                        ConversationSnapshotEvent(seq = sequence, conversation = currentDto)
                    )
            ...
            is ConversationStreamPayload.BatchErrors -> {
              payload.messages.forEach { message ->
                val json = JsonInstant.encodeToString(ErrorEvent(message = message))
