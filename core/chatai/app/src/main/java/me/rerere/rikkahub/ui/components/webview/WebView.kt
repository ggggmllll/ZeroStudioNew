    ...
    AndroidView(
        factory = { context ->
          WebView(context).apply {
            ...
            state.interfaces.forEach { (name, obj) -> addJavascriptInterface(obj, name) }
          }
        },
        modifier = Modifier.fillMaxWidth(), // Make WebView fill the width
        onReset = {
          state.interfaces.forEach { (name, _) -> it.removeJavascriptInterface(name) }
          Log.d(TAG, "AndroidView: Resetting WebView")
        },
        update = { webView ->
          state.webView = webView
          state.interfaces.forEach { (name, obj) -> webView.addJavascriptInterface(obj, name) }
          ...
          when (val content = state.content) {
            is WebContent.Url -> {
              val url = content.url
              // Only load new URL if it's different from the current one or if the state forces
              // reload
            ...
            is WebContent.Data -> {
              ...
              // For simplicity, we might just reload it every time the update block runs with Data
              // content.
              // A more complex check could involve comparing `content.data` with a previously
              // stored value.
            ...
            WebContent.NavigatorOnly -> {
              // NO-OP: State changes related to navigation are handled by the methods in
              // WebViewState
            }
          }
          onUpdated(webView)
...
class WebViewState(
  ...
  internal var forceReload: Boolean by
      mutableStateOf(false) // Internal state to force URL reload if needed
