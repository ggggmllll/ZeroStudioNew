...
/**
 * Draws an arbitrary composable into a bitmap mainScope has to be Dispatcher.Main because it has to
 * perform layout and measurement calculations on the UI thread.
 */
class BitmapComposer(private val mainScope: CoroutineScope) {
  /**
   * Renders an arbitrary Composable View into a Bitmap.
   *
   * @param activity The host activity that is needed to attach the composable content to the view
   *   hierarchy.
   * @param width Optional width of the bitmap in device-independent pixels. Try to provide a width
   *   or height value for better results.
   * @param height Optional height of the bitmap in device-independent pixels. Try to provide a
   *   width or height value for better results.
  ...
  suspend fun composableToBitmap(
    ...
    mainScope.launch {
      ...
      val contentHeightInPixels =
          (screenDensity.density * (height ?: MAX_HEIGHT).value).roundToInt()
      ...
      val composeView =
          ComposeView(activity).apply {
            ...
            setContent { CompositionLocalProvider(LocalExportContext provides true) { content() } }
          }
      ...
      Handler(Looper.getMainLooper()).post {
        ...
        Handler(Looper.getMainLooper())
            .postDelayed(
                ...
                100,
            ) // delay to allow ComposeView to finish rendering
      }
    }
  }
}
