...
object ImageUtils {
   ...
   */
  fun loadOptimizedBitmap(context: Context, uri: Uri, maxSize: Int = 1024): Bitmap? {
    return runCatching {
   ...
   * @return 二维码内容，解析失败返回null
   */
  fun decodeQRCodeFromUri(context: Context, uri: Uri, maxSize: Int = 1024): String? {
  ...
  fun getTavernCharacterMeta(context: Context, uri: Uri): Result<String> = runCatching {
    val metadata =
        context.contentResolver.openInputStream(uri)?.use { ImageMetadataReader.readMetadata(it) }
    ...
    val pngDirectory =
        metadata.getDirectoriesOfType(PngDirectory::class.java).firstOrNull { directory ->
  ...
  data class ImageInfo(val width: Int, val height: Int, val mimeType: String?)
}
