...
interface FavoriteDAO {
  @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun upsert(favorite: FavoriteEntity)
  ...
  @Query(
      "SELECT substr(ref_key, length('node:' || :conversationId || ':') + 1) FROM favorites WHERE ref_key LIKE 'node:' || :conversationId || ':%'"
  )
  ...
  @Query("DELETE FROM favorites WHERE id = :id") suspend fun deleteById(id: String): Int
}
