...
private fun TextViewPreview() {
  MaterialTheme {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
      ...
      Text(
          text =
              buildAnnotatedString {
                ...
                withStyle(SpanStyle(fontSize = 39.sp)) { append("BIG TEXT") }
                append("ahah")
              },
          style = style,
      )
      ...
      ViewText(text = spannableString, modifier = Modifier.fillMaxWidth(), style = style)
    }
  }
}

private fun TextView.setComposeTextStyle(density: Density, textStyle: TextStyle) {
  with(density) {
    // text color
    ...
    val typefaceStyle =
        getAndroidTypefaceStyle(fontWeight = textStyle.fontWeight, fontStyle = textStyle.fontStyle)
    val finalTypeface =
        when (textStyle.fontFamily) {
          FontFamily.SansSerif,
          null -> Typeface.create(Typeface.SANS_SERIF, typefaceStyle)
          ...
          FontFamily.Cursive ->
              Typeface.create(Typeface.SANS_SERIF, typefaceStyle) // Cursive 没有直接映射，回退到 SansSerif
...
private fun getAndroidTypefaceStyle(fontWeight: FontWeight?, fontStyle: FontStyle?): Int {
