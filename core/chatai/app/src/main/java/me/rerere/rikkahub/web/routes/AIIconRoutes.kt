...
fun Route.aiIconRoutes(context: Context) {
  route("/ai-icon") {
    get {
      val name =
          call.request.queryParameters["name"]?.trim() ?: throw BadRequestException("Missing name")
      if (name.isEmpty()) {
        ...
        runCatching {
              context.assets.open(assetPath).use { input ->
                ...
                call.response.header(
                    ...
                    resolveContentType(iconPath).toString(),
                )
                call.respondOutputStream { input.copyTo(this) }
              }
            }
            .onSuccess {
              return@get
