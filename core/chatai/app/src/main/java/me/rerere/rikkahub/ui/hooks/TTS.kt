...
/**
 * Composable function to remember and manage custom TTS state. Uses user-configured TTS providers
 * instead of system TTS.
 */
@Composable
fun rememberCustomTtsState(): CustomTtsState {
  ...
  val ttsState = remember {
    CustomTtsStateImpl(context = context.applicationContext, settingsStore = settingsStore)
  }
  ...
  DisposableEffect(ttsState) { onDispose { ttsState.cleanup() } }
...
interface CustomTtsState {
  ...
  /**
   * Speaks the given text using the selected TTS provider. Long texts will be automatically chunked
   * and queued.
   */
...
private class CustomTtsStateImpl(
  ...
  override val isSpeaking: StateFlow<Boolean>
    get() = controller.isSpeaking

  override val error: StateFlow<String?>
    get() = controller.error

  override val currentChunk: StateFlow<Int>
    get() = controller.currentChunk

  override val totalChunks: StateFlow<Int>
    get() = controller.totalChunks
