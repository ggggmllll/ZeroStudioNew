...
fun EmojiBurstHost(
  ...
  BoxWithConstraints(modifier = modifier) {
    ...
    val onBurst: (Offset) -> Unit = onBurst@{ origin ->
      ...
      val start = Offset(origin.x.coerceIn(minX, maxX), origin.y.coerceIn(minY, maxY))
      pendingBursts.add(
          BurstRequest(
              center = start,
    ...
    LaunchedEffect(Unit) {
      ...
      while (true) {
        ...
        if (pendingBursts.isNotEmpty()) {
          ...
          while (iterator.hasNext()) {
            ...
            repeat(spawnCount) {
              ...
              val jitter = Offset(cos(jitterAngle) * jitterRadius, sin(jitterAngle) * jitterRadius)
              val spawn =
                  Offset(
        ...
        repeat(2) {
          for (i in 0 until count) {
            for (j in i + 1 until count) {
              ...
              positions[i] =
                  Offset(positions[i].x - nx * overlap / 2f, positions[i].y - ny * overlap / 2f)
              positions[j] =
                  Offset(positions[j].x + nx * overlap / 2f, positions[j].y + ny * overlap / 2f)
        ...
        for (i in 0 until count) {
          ...
          val shouldRemove = age >= lifetimeSeconds && position.y >= maxY && speed < 200f
          if (!shouldRemove) {
            particles.add(
                current[i].copy(
                    position = position,
