...
val dataSourceModule = module {
  ...
  single<OkHttpClient> {
    ...
    OkHttpClient.Builder()
        ...
        .addInterceptor { chain ->
          ...
          val requestBuilder =
              originalRequest.newBuilder().addHeader(HttpHeaders.AcceptLanguage, acceptLang)

          if (originalRequest.header(HttpHeaders.UserAgent) == null) {
            requestBuilder.addHeader(
                HttpHeaders.UserAgent,
                "RikkaHub-Android/${BuildConfig.VERSION_NAME}",
            )
          }
  ...
  single { WebDavSync(settingsStore = get(), json = get(), context = get(), httpClient = get()) }
