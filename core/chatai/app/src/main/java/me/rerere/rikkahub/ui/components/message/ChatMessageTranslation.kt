    ...
    Column(
      ...
      LazyColumn(
        ...
        items(languages) { language ->
          Card(onClick = { onLanguageSelected(language) }, shape = MaterialTheme.shapes.medium) {
            Row(
        ...
        item {
          Card(onClick = { onClearTranslation() }, shape = MaterialTheme.shapes.medium) {
            Row(
                verticalAlignment = Alignment.CenterVertically,
                horizontalArrangement = Arrangement.spacedBy(16.dp),
                modifier = Modifier.padding(16.dp).fillMaxWidth(),
...
@Composable
fun CollapsibleTranslationText(content: String, onClickCitation: (String) -> Unit) {
  if (content.isNotBlank()) {
    ...
    Row(
      ...
      // 折叠/展开按钮
      IconButton(onClick = { isCollapsed = !isCollapsed }, modifier = Modifier.size(32.dp)) {
        Icon(
            imageVector = if (isCollapsed) HugeIcons.ArrowDown01 else HugeIcons.ArrowUp01,
            contentDescription =
                if (isCollapsed) stringResource(R.string.expand_translation)
                else stringResource(R.string.collapse_translation),
            modifier = Modifier.size(16.dp),
