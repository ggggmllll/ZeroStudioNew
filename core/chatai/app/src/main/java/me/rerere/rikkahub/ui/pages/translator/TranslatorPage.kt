...
fun TranslatorPage(vm: TranslatorVM = koinViewModel()) {
  ...
  // 处理错误
  LaunchedEffect(Unit) {
    vm.errorFlow.collect { error -> toaster.show(error.message ?: "错误", type = ToastType.Error) }
  }

  Scaffold(
      topBar = {
        TopAppBar(
            title = { Text(stringResource(R.string.translator_page_title)) },
      ...
      bottomBar = {
        BottomBar(
            ...
            onCancelTranslation = { vm.cancelTranslation() },
            onLanguageSelected = { vm.updateTargetLanguage(it) },
    ...
    Column(
      ...
      Column {
        ...
        FilledTonalButton(
            onClick = {
              scope.launch {
                clipboard.getClipEntry()?.clipData?.getText()?.let { vm.updateInputText(it) }
              }
            }
        ) {
      ...
      Crossfade(translating) { isTranslating ->
        if (isTranslating) {
          LinearWavyProgressIndicator(modifier = Modifier.padding(8.dp).fillMaxWidth())
        } else {
          HorizontalDivider()
        }
      }
      ...
      AnimatedVisibility(translatedText.isNotBlank()) {
        FilledTonalButton(
            onClick = {
              scope.launch {
                clipboard.setClipEntry(ClipEntry(ClipData.newPlainText(null, translatedText)))
              }
            }
        ) {
          Icon(HugeIcons.Clipboard, null)
...
@Composable
private fun LanguageSelector(targetLanguage: Locale, onLanguageSelected: (Locale) -> Unit) {
