...
fun LogPage() {
  ...
  Scaffold(
    ...
    UnifiedLogList(logs = logs, modifier = Modifier.fillMaxSize().padding(contentPadding))
  }
}
...
private fun UnifiedLogList(logs: List<LogEntry>, modifier: Modifier = Modifier) {
  ...
  selectedLog?.let { log ->
    ModalBottomSheet(onDismissRequest = { selectedLog = null }, sheetState = sheetState) {
...
private fun RequestLogDetail(log: LogEntry.RequestLog) {
  ...
  LazyColumn(
    ...
    log.responseCode?.let { code -> item { DetailSection("Status Code", code.toString()) } }

    log.durationMs?.let { duration -> item { DetailSection("Duration", "${duration}ms") } }

    log.error?.let { error -> item { DetailSection("Error", error) } }
    ...
    if (log.requestHeaders.isNotEmpty()) {
      ...
      log.requestHeaders.forEach { (key, value) -> item { HeaderItem(key, value) } }
    }
    ...
    log.requestBody?.let { body ->
      item {
        ...
        if (jsonElement != null) {
          ...
          Text(text = body, fontFamily = JetbrainsMono, modifier = Modifier.padding(top = 4.dp))
        }
      }
    }
    ...
    if (log.responseHeaders.isNotEmpty()) {
      ...
      log.responseHeaders.forEach { (key, value) -> item { HeaderItem(key, value) } }
    }
  }
}
...
private fun DetailSection(label: String, value: String) {
  Column {
    ...
    Text(text = value, style = MaterialTheme.typography.bodyMedium, fontFamily = JetbrainsMono)
  }
}
...
private fun HeaderItem(key: String, value: String) {
  Column(modifier = Modifier.padding(vertical = 2.dp)) {
    ...
    Text(text = value, style = MaterialTheme.typography.bodySmall, fontFamily = JetbrainsMono)
  }
}
