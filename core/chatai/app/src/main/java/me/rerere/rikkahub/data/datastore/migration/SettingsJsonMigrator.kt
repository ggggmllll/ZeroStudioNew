 ...
 *
 * DataStore migration 作用于分散的 key-value 存储，而备份文件中的 settings.json 是整个
 * [me.rerere.rikkahub.data.datastore.Settings] 对象的序列化结果。 此工具类负责在反序列化前对旧格式的 JSON 执行等价的迁移操作。
 */
object SettingsJsonMigrator {

  /** 对 settings JSON 字符串依次应用所有版本的迁移。 若发生异常则返回原始 JSON，不中断恢复流程。 */
  fun migrate(settingsJson: String): String {
    return runCatching {
          ...
          root["assistants"]?.let { element ->
            ...
            if (extractedQuickMessages.isNotEmpty()) {
              ...
              val existingArray =
                  existing?.let {
                    runCatching {
                          JsonInstant.parseToJsonElement(JsonInstant.encodeToString(it))
                              as? JsonArray
                        }
                        .getOrNull()
                  } ?: JsonArray(emptyList())
              ...
              val merged =
                  JsonArray(
                      existingArray +
                          extractedQuickMessages.filter { e ->
        ...
        .onFailure { Log.e(TAG, "migrate: Failed to migrate settings JSON, using original", it) }
