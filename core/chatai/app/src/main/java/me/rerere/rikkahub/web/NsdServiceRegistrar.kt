...
class NsdServiceRegistrar(private val context: Context) {
  ...
  suspend fun register(
      ...
      withContext(Dispatchers.IO) {
        ...
        try {
          ...
          multicastLock =
              wifiManager?.createMulticastLock("jmdns-lock")?.apply {
          ...
          val serviceInfo =
              ServiceInfo.create(serviceType, serviceName, port, "RikkaHub Web Server")
  ...
  suspend fun unregister() = withContext(Dispatchers.IO) { cleanup() }
  ...
  private fun cleanup() {
    runCatching {
        ...
        .onFailure { Log.w(TAG, "Failed to close JmDNS", it) }
    jmdns = null
