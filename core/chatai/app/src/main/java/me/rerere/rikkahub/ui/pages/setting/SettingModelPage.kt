...
fun SettingModelPage(vm: SettingVM = koinViewModel()) {
  ...
  Scaffold(
    ...
    LazyColumn(
      ...
      item { DefaultChatModelSetting(settings = settings, vm = vm) }

      item { DefaultTitleModelSetting(settings = settings, vm = vm) }

      item { DefaultSuggestionModelSetting(settings = settings, vm = vm) }

      item { DefaultTranslationModelSetting(settings = settings, vm = vm) }

      item { DefaultOcrModelSetting(settings = settings, vm = vm) }

      item { DefaultCompressModelSetting(settings = settings, vm = vm) }
    }
  }
}

@Composable
private fun DefaultTranslationModelSetting(settings: Settings, vm: SettingVM) {
...
@Composable
private fun DefaultSuggestionModelSetting(settings: Settings, vm: SettingVM) {
  var showModal by remember { mutableStateOf(false) }
  ModelFeatureCard(
      title = {
        Text(text = stringResource(R.string.setting_model_page_suggestion_model), maxLines = 1)
      },
...
@Composable
private fun DefaultTitleModelSetting(settings: Settings, vm: SettingVM) {
  var showModal by remember { mutableStateOf(false) }
  ModelFeatureCard(
    ...
    ModalBottomSheet(
      ...
      Column(
        ...
        FormItem(
          ...
          TextButton(
              onClick = { vm.updateSettings(settings.copy(titlePrompt = DEFAULT_TITLE_PROMPT)) }
          ) {
            Text(stringResource(R.string.setting_model_page_reset_to_default))
          }
        }
...
@Composable
private fun DefaultChatModelSetting(settings: Settings, vm: SettingVM) {
  ModelFeatureCard(
...
@Composable
private fun DefaultOcrModelSetting(settings: Settings, vm: SettingVM) {
  var showModal by remember { mutableStateOf(false) }
  ModelFeatureCard(
    ...
    ModalBottomSheet(
      ...
      Column(
        ...
        FormItem(
          ...
          TextButton(
              onClick = { vm.updateSettings(settings.copy(ocrPrompt = DEFAULT_OCR_PROMPT)) }
          ) {
            Text(stringResource(R.string.setting_model_page_reset_to_default))
          }
        }
...
@Composable
private fun DefaultCompressModelSetting(settings: Settings, vm: SettingVM) {
  var showModal by remember { mutableStateOf(false) }
  ModelFeatureCard(
