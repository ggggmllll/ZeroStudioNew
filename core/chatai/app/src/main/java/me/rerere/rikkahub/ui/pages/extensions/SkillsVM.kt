  ...
  fun importSkillFromGitHub(repoUrl: String, onResult: (Boolean, String) -> Unit) {
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        val listed =
            listFilesRecursively(info.owner, info.repo, info.branch, info.path, info.path, files)
        if (!listed) {
        ...
        val skillMdContent =
            downloadText(skillMdEntry.second)
                ?: run {
  ...
  private fun listFilesRecursively(
    ...
    for (i in 0 until array.length()) {
      ...
      when (type) {
        "file" -> {
          val downloadUrl =
              item.optString("download_url").takeIf { it.isNotBlank() } ?: return false
