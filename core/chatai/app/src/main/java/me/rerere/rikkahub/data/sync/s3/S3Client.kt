...
class S3Client(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          val response: HttpResponse =
              httpClient.request(signed.url) {
                method = HttpMethod.Put
                headers { signed.headers.forEach { (k, v) -> append(k, v) } }
                setBody(data)
              }
  ...
  suspend fun getObjectStream(key: String): Result<InputStream> =
      withContext(Dispatchers.IO) {
        runCatching {
          val path = "/${key.trimStart('/')}"
          val signed =
  ...
  suspend fun downloadObjectToFile(key: String, targetFile: File): Result<Unit> =
      withContext(Dispatchers.IO) {
        runCatching {
          val path = "/${key.trimStart('/')}"
          val signed =
  ...
  suspend fun headObject(key: String): Result<S3ObjectMetadata> =
      withContext(Dispatchers.IO) {
        runCatching {
          val path = "/${key.trimStart('/')}"
          val signed =
            ...
            when {
              inContents -> {
                when (currentTag) {
                  ...
                  "LastModified" ->
                      currentLastModified = runCatching { Instant.parse(text) }.getOrNull()
