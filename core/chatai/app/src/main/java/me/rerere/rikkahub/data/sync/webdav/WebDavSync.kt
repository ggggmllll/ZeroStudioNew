...
class WebDavSync(
  ...
  suspend fun backup(config: WebDavConfig) =
      withContext(Dispatchers.IO) {
  ...
  suspend fun listBackupFiles(config: WebDavConfig): List<WebDavBackupItem> =
      withContext(Dispatchers.IO) {
        ...
        resources
            .filter {
              !it.isCollection &&
                  it.displayName.startsWith("backup_") &&
                  it.displayName.endsWith(".zip")
            }
            .map { resource ->
              WebDavBackupItem(
                  href = resource.href,
  ...
  private fun restoreSkillEntry(zipIn: ZipInputStream, entryName: String) {
    ...
    val skillDir =
        SkillPaths.resolveSkillDir(skillsRoot, skillName)
    ...
    try {
      ...
      Log.i(
          TAG,
          "restoreFromBackupFile: Restored skill file $entryName (${targetFile.length()} bytes)",
      )
    } catch (e: Exception) {
