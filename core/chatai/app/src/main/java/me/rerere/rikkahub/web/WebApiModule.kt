...
/**
 * Configure Web API for the Ktor application. This should be called from app module when starting
 * the web server.
 *
 * Example usage:
 * ```
      ...
      jwt("auth-jwt") {
        ...
        authHeader { call ->
          extractAccessToken(
              ...
              ?.let { token -> HttpAuthHeader.Single("Bearer", token) }
        }
        validate { credential ->
          val currentPassword = settingsStore.settingsFlow.value.webServerAccessPassword
          if (currentPassword.isBlank()) {
            ...
            credential.payload.subject
                ...
                ?.let { io.ktor.server.auth.jwt.JWTPrincipal(credential.payload) }
          }
        }
        challenge { _, _ ->
          val currentPassword = settingsStore.settingsFlow.value.webServerAccessPassword
...
private fun extractAccessToken(authorizationHeader: String?, queryToken: String?): String? {
  return extractBearerToken(authorizationHeader) ?: queryToken?.trim()?.takeIf { it.isNotEmpty() }
}
