...
class McpManager(
      ...
      settingsStore.settingsFlow
          ...
          .collect { mcpServerConfigs ->
            runCatching {
                  ...
                  toAdd.forEach { cfg ->
                    appScope.launch {
                      runCatching { addClient(cfg) }.onFailure { it.printStackTrace() }
                    }
                  }
  ...
  suspend fun callTool(toolName: String, args: JsonObject): List<UIMessagePart> {
    ...
    if (client == null)
        return listOf(
            UIMessagePart.Text("Failed to execute tool, because no such mcp client for the tool")
        )
  ...
  private suspend fun convertImageContentToFilePart(image: ImageContent): UIMessagePart.Image {
    ...
    val ext =
        android.webkit.MimeTypeMap.getSingleton().getExtensionFromMimeType(image.mimeType) ?: "bin"
    val entity =
        filesManager.saveUploadFromBytes(
            bytes = bytes,
  ...
  private suspend fun sync(config: McpServerConfig) {
    ...
    settingsStore.update { old ->
      old.copy(
          mcpServers =
              old.mcpServers.map { serverConfig ->
                ...
                clients.put(config.clone(commonOptions = common.copy(tools = tools)), client)

                // 返回新的serverConfig，更新到settings store
                serverConfig.clone(commonOptions = common.copy(tools = tools))
              }
      )
    }
  ...
  suspend fun syncAll() =
      withContext(Dispatchers.IO) {
        clients.keys.toList().forEach { config ->
          runCatching { sync(config) }.onFailure { it.printStackTrace() }
        }
      }
  ...
  suspend fun removeClient(config: McpServerConfig) =
      withContext(Dispatchers.IO) {
        ...
        toRemove.forEach { entry ->
          runCatching { entry.value.close() }.onFailure { it.printStackTrace() }
  ...
  private fun scheduleReconnect(config: McpServerConfig) {
    ...
    if (currentAttempt > MAX_RECONNECT_ATTEMPTS) {
      ...
      appScope.launch { setStatus(config, McpStatus.Error("连接断开，已达最大重连次数")) }
      return
    }
    ...
    Log.i(
        TAG,
        "Scheduling reconnect for ${config.commonOptions.name}, attempt $currentAttempt/$MAX_RECONNECT_ATTEMPTS, delay ${delayMs}ms",
    )
    ...
    reconnectJobs[configId] = appScope.launch {
      try {
        ...
        // 检查配置是否仍然启用
        val currentConfig =
            settingsStore.settingsFlow.value.mcpServers.find {
              it.id == configId && it.commonOptions.enable
            }

        if (currentConfig == null) {
          Log.i(
              TAG,
              "Config disabled or removed, cancelling reconnect for ${config.commonOptions.name}",
          )
          return@launch
        }
  ...
  private suspend fun reconnectClient(config: McpServerConfig) =
      withContext(Dispatchers.IO) {
        // 先关闭旧客户端
