...
public class StreamableHttpClientTransport(
  ...
  /**
   * Sends one or more messages with optional resumption support. This is the main send method that
  ...
  private suspend fun startSseSession(
      ...
      sseSession =
          client.sseSession(
            ...
            (resumptionToken ?: lastEventId)?.let {
              headers.append(MCP_RESUMPTION_TOKEN_HEADER, it)
            }
            requestBuilder()
          }
    ...
    sseJob =
        scope.launch(CoroutineName("StreamableHttpTransport.collect#${hashCode()}")) {
  ...
  private suspend fun collectSse(
    ...
    try {
      session.incoming.collect { event ->
        ...
        Log.d(
            TAG,
            "Client received SSE event: event=${event.event}, data=${event.data}, id=${event.id}",
        )
        when (event.event) {
          null,
          "message" ->
