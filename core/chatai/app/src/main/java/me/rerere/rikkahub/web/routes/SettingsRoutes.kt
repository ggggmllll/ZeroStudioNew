...
fun Route.settingsRoutes(settingsStore: SettingsStore) {
  route("/settings") {
    post("/assistant") {
    ...
    post("/search/enabled") {
      ...
      settingsStore.update { settings -> settings.copy(enableWebSearch = request.enabled) }
      call.respond(HttpStatusCode.OK, mapOf("status" to "ok"))
    }
    ...
    post("/model/built-in-tool") {
      ...
      settingsStore.update { settings ->
        ...
        settings.copy(
            providers = settings.providers.map { provider -> provider.editModel(updatedModel) }
        )
      }
    ...
    post("/favorite-models") {
      ...
      settingsStore.update { settings -> settings.copy(favoriteModels = favoriteModelIds) }
      call.respond(HttpStatusCode.OK, mapOf("status" to "ok"))
    }
...
private fun parseBuiltInTool(tool: String): BuiltInTools {
  return when (tool.trim().lowercase(Locale.ROOT)) {
    ...
    "url_context",
    "url-context",
    "urlcontext" -> BuiltInTools.UrlContext
