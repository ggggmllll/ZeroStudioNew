...
@Composable
fun WebDavTab(vm: BackupVM, onShowRestartDialog: () -> Unit) {
        ...
        item(
            ...
            supportingContent = {
              OutlinedTextField(
                  ...
                  onValueChange = { updateWebDavConfig(webDavConfig.copy(username = it.trim())) },
                  singleLine = true,
              )
            },
        )
          ...
          onClick = {
            scope.launch {
              try {
                ...
                e.printStackTrace()
                toaster.show(
                    context.getString(R.string.backup_page_connection_failed, e.message ?: ""),
        ...
        supportingContent = {
          Column(
            ...
            Row(
              ...
              TextButton(onClick = { onDelete(item) }, enabled = !isRestoring) {
                Text(stringResource(R.string.backup_page_delete))
              }
              Button(onClick = { onRestore(item) }, enabled = !isRestoring) {
                if (isRestoring) {
