...
class WebDavClient(
      ...
      withContext(Dispatchers.IO) {
        runCatching {
          ...
          if (!response.status.isSuccess()) {
            ...
            throw WebDavException(
                "Failed to put file: ${response.status}",
                response.status.value,
                errorBody,
            )
          }
  ...
  suspend fun getStream(path: String): Result<InputStream> =
      withContext(Dispatchers.IO) {
        runCatching {
          val url = config.buildUrl(path)
          ...
          if (!response.status.isSuccess()) {
            ...
            throw WebDavException(
                "Failed to get stream: ${response.status}",
                response.status.value,
                errorBody,
            )
          }
  ...
  suspend fun downloadToFile(path: String, targetFile: File): Result<Unit> =
      withContext(Dispatchers.IO) {
        runCatching {
          val url = config.buildUrl(path)
  ...
  suspend fun head(path: String): Result<WebDavResourceInfo> =
      withContext(Dispatchers.IO) {
        runCatching {
          val url = config.buildUrl(path)
          Log.d(TAG, "HEAD: $url")
          ...
          if (!response.status.isSuccess()) {
            throw WebDavException(
                "Resource not found: ${response.status}",
                response.status.value,
                "",
            )
          }
  ...
  suspend fun mkcol(path: String): Result<Unit> =
      withContext(Dispatchers.IO) {
        runCatching {
          val url = config.buildUrl(path)
          Log.d(TAG, "MKCOL: $url")
  ...
  suspend fun propfind(
      path: String = "",
      depth: Int = 1,
  ): Result<List<WebDavResourceInfo>> =
      withContext(Dispatchers.IO) {
        runCatching {
          val url = config.buildUrl(path)
          ...
          if (!response.status.isSuccess() && response.status.value != 207) {
            ...
            throw WebDavException(
                "Failed to propfind: ${response.status}",
                response.status.value,
                errorBody,
            )
          }
  ...
  suspend fun ensureCollectionExists(path: String = ""): Result<Unit> =
      withContext(Dispatchers.IO) {
        runCatching {
  ...
  suspend fun list(path: String = ""): Result<List<WebDavResourceInfo>> =
      withContext(Dispatchers.IO) {
        runCatching {
  ...
  private fun parseLastModified(dateString: String?): Instant? {
    ...
    return try {
      ...
      try {
        ...
        ZonedDateTime.parse(dateString, DateTimeFormatter.ofPattern("EEEE, dd-MMM-yy HH:mm:ss zzz"))
            .toInstant()
      } catch (e: Exception) {
        try {
          // ISO 8601
