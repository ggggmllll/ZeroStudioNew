...
data class UIMessage(
  ...
  private fun appendChunk(chunk: MessageChunk): UIMessage {
    ...
    return message?.let { delta ->
            ...
            when (deltaPart) {
                  ...
                  if (lastPart is UIMessagePart.Reasoning) {
                    ...
                    acc.dropLast(1) +
                        UIMessagePart.Reasoning(
                            ...
                            .also { it.metadata = deltaPart.metadata ?: lastPart.metadata }
                  } else {
              ...
              is UIMessagePart.Tool -> {
                if (deltaPart.toolCallId.isBlank()) {
                  ...
                  val existsPart =
                      acc.find { it is UIMessagePart.Tool && it.toolCallId == deltaPart.toolCallId }
      ...
      // Handle annotations
      val newAnnotations = delta.annotations.ifEmpty { annotations }
      copy(
          parts = newParts,
  ...
  companion object {
    fun system(prompt: String) =
        UIMessage(role = MessageRole.SYSTEM, parts = listOf(UIMessagePart.Text(prompt)))

    fun user(prompt: String) =
        UIMessage(role = MessageRole.USER, parts = listOf(UIMessagePart.Text(prompt)))

    fun assistant(prompt: String) =
        UIMessage(role = MessageRole.ASSISTANT, parts = listOf(UIMessagePart.Text(prompt)))
  }
}
...
fun List<UIMessage>.handleMessageChunk(chunk: MessageChunk, model: Model? = null): List<UIMessage> {
  require(this.isNotEmpty()) { "messages must not be empty" }
...
sealed class ToolApprovalState {
  @Serializable @SerialName("auto") data object Auto : ToolApprovalState()

  @Serializable @SerialName("pending") data object Pending : ToolApprovalState()

  @Serializable @SerialName("approved") data object Approved : ToolApprovalState()
...
sealed class UIMessagePart {
  ...
  @Serializable
  @SerialName("text")
  data class Text(val text: String, override var metadata: JsonObject? = null) : UIMessagePart()

  @Serializable
  @SerialName("image")
  data class Image(val url: String, override var metadata: JsonObject? = null) : UIMessagePart()

  @Serializable
  @SerialName("video")
  data class Video(val url: String, override var metadata: JsonObject? = null) : UIMessagePart()

  @Serializable
  @SerialName("audio")
  data class Audio(val url: String, override var metadata: JsonObject? = null) : UIMessagePart()
  ...
  data class Tool(
    ...
    val isPending: Boolean
      get() = approvalState is ToolApprovalState.Pending
 ...
 *
 * WARNING: This function is intended for migration only. Do not use for new messages as it may
 * break the semantic order when a message contains multiple Reasoning/Text parts.
 */
@Deprecated(
    message =
        "Only use for migration. May break semantic order for messages with multiple Reasoning/Text parts.",
...
/**
 * Migrate legacy ToolCall parts to new Tool type within a single message. This converts ToolCall
 * parts to Tool parts with empty output.
 */
...
/**
 * Migrate TOOL role messages into previous ASSISTANT messages by merging ToolResult parts into
 * corresponding Tool parts. Returns the migrated list with TOOL messages removed.
 */
@Suppress("DEPRECATION")
      ...
      if (result.isNotEmpty() && result.last().role == MessageRole.ASSISTANT) {
        ...
        val updatedParts =
            lastAssistant.parts.map { part ->
              if (part is UIMessagePart.Tool && !part.isExecuted) {
                val matchingResult = toolResults.find { result ->
                  result.toolCallId == part.toolCallId
                }
                if (matchingResult != null) {
                  part.copy(
                      output =
                          listOf(UIMessagePart.Text(json.encodeToString(matchingResult.content)))
                  )
                } else {
                  part
                }
...
/**
 * Migrate legacy TOOL role messages at the MessageNode level. This handles the case where TOOL
 * messages are stored in separate MessageNodes by merging ToolResult parts into the previous
 * ASSISTANT node's Tool parts.
 *
                    ...
                    if (!part.isExecuted) {
                      ...
                      if (matchingResult != null) {
                        part.copy(
                            output =
                                listOf(
                                    UIMessagePart.Text(json.encodeToString(matchingResult.content))
                                )
                        )
                      } else part
                    } else part
...
sealed class UIMessageAnnotation {
  ...
  data class UrlCitation(val title: String, val url: String) : UIMessageAnnotation()
}
