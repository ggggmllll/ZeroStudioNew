...
/**
 * 使用服务账号（email + private key PEM）换取 Google OAuth2 Access Token。 构造时传入 OkHttpClient；调用时传 email、私钥
 * PEM 与 scopes。
 */
class ServiceAccountTokenProvider(private val http: OkHttpClient) {
