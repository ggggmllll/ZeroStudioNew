...
class ModelGroup internal constructor(private val members: List<ModelSelector>) : ModelSelector {
...
private fun parseTokenSpec(spec: String): TokenSpec {
  val options = spec.split('|').map { it.trim().lowercase() }.filter { it.isNotEmpty() }.toSet()
