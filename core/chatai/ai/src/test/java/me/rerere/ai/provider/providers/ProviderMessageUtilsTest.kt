...
class ProviderMessageUtilsTest {
  ...
  fun `only text parts should return single Content group`() {
    val parts = listOf(UIMessagePart.Text("Hello"), UIMessagePart.Text("World"))
    val result = groupPartsByToolBoundary(parts)
  ...
  fun `only executed tools should return single Tools group`() {
    val parts = listOf(createExecutedTool("call1", "tool1"), createExecutedTool("call2", "tool2"))
    val result = groupPartsByToolBoundary(parts)
  ...
  fun `unexecuted tool should be in Content group`() {
    val parts = listOf(UIMessagePart.Text("Hello"), createUnexecutedTool("call1", "tool1"))
    val result = groupPartsByToolBoundary(parts)
  ...
  fun `text then tool should create Content then Tools groups`() {
    val parts = listOf(UIMessagePart.Text("Hello"), createExecutedTool("call1", "tool1"))
    val result = groupPartsByToolBoundary(parts)
  ...
  fun `tool then text should create Tools then Content groups`() {
    val parts = listOf(createExecutedTool("call1", "tool1"), UIMessagePart.Text("Result"))
    val result = groupPartsByToolBoundary(parts)
