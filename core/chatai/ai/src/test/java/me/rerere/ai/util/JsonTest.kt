...
class JsonTest {
  ...
  fun `mergeCustomBody with simple keys should merge correctly`() {
    val originalJson = buildJsonObject { put("existingKey", "existingValue") }
  ...
  fun `mergeCustomBody should override existing simple keys`() {
    val originalJson = buildJsonObject { put("key1", "oldValue") }

    val customBodies = listOf(CustomBody("key1", JsonPrimitive("newValue")))
  ...
  fun `mergeCustomBody should merge nested JsonObjects`() {
    ...
    val customBodies = listOf(CustomBody("config", nestedJsonValue))
  ...
  fun `mergeCustomBody should handle deeply nested JsonObjects`() {
    val originalJson = buildJsonObject {
      put(
          "level1",
          buildJsonObject { put("level2", buildJsonObject { put("setting1", "original") }) },
      )
    }
    ...
    val customBodies = listOf(CustomBody("level1", nestedValue))
  ...
  fun `mergeCustomBody should ignore empty keys`() {
    val originalJson = buildJsonObject { put("key1", "value1") }
