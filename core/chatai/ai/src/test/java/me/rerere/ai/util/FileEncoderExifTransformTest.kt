...
class FileEncoderExifTransformTest {
  ...
  fun `all supported exif orientations should map to expected transform`() {
    ...
    assertEquals(
        ...
        mapExifOrientationToTransform(ORIENTATION_FLIP_HORIZONTAL),
    )
    assertEquals(
        ExifTransformType.ROTATE_180,
        mapExifOrientationToTransform(ORIENTATION_ROTATE_180),
    )
    assertEquals(
        ExifTransformType.FLIP_VERTICAL,
        mapExifOrientationToTransform(ORIENTATION_FLIP_VERTICAL),
    )
    ...
    assertEquals(
        ...
        mapExifOrientationToTransform(ORIENTATION_TRANSVERSE),
    )
    assertEquals(
        ExifTransformType.ROTATE_270,
        mapExifOrientationToTransform(ORIENTATION_ROTATE_270),
    )
  }
