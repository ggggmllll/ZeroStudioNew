...
class TomlTextDocumentService(private val server: TomlLanguageServer) : TextDocumentService {
    ...
    private fun notifyUnsupported(feature: String) {
        server.getClient()?.showMessage(
            MessageParams(MessageType.Info, "TOML: 当前不支持 $feature 功能")
        )
    }
    ...
    override fun completion(params: CompletionParams): CompletableFuture<Either<List<CompletionItem>, CompletionList>> {
        return CompletableFuture.supplyAsync {
            Either.forLeft(TomlCompletionProvider.compute(getContent(params.textDocument.uri), params.position))
        }
    }
    ...
    override fun documentHighlight(params: DocumentHighlightParams): CompletableFuture<MutableList<out DocumentHighlight>> {
        return CompletableFuture.supplyAsync {
            TomlFeatureEngine.computeHighlight(getContent(params.textDocument.uri), params.position).toMutableList()
        }
    }

    override fun definition(params: DefinitionParams): CompletableFuture<Either<List<Location>, List<LocationLink>>> {
        return CompletableFuture.supplyAsync {
            Either.forLeft(TomlFeatureEngine.computeDefinition(getContent(params.textDocument.uri), params.textDocument.uri, params.position))
        }
    }
    ...
    override fun rename(params: RenameParams): CompletableFuture<WorkspaceEdit> {
        return CompletableFuture.supplyAsync {
            TomlFeatureEngine.computeRename(getContent(params.textDocument.uri), params.textDocument.uri, params.position, params.newName)
        }
    }
    ...
    override fun codeAction(params: CodeActionParams): CompletableFuture<MutableList<Either<Command, CodeAction>>> {
        return CompletableFuture.supplyAsync {
            TomlFeatureEngine.computeCodeAction(getContent(params.textDocument.uri), params).toMutableList()
        }
    }
