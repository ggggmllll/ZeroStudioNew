...
/**
 * 轻量级 SDK 安装管理器。
 * 采用 Kotlin 下载 (保证高可用度与精准进度回报) + Termux Shell (高效强健解压)。
 * @author android_zero
 */
object SdkInstallerManager {
    ...
    suspend fun downloadAndInstall(
        ...
        // Download File
        val tempArchive = File(Environment.TMP_DIR, "sdk_dl_${System.currentTimeMillis()}_${File(urlStr).name}")
        ...
        val cmdResult = TermuxCommand.run(context) {
    ...
    /** 卸载任务 */
    suspend fun deletePackage(node: SdkTreeNode, onLog: (String) -> Unit): Boolean = withContext(Dispatchers.IO) {
        val destDir = getDestDir(node)
