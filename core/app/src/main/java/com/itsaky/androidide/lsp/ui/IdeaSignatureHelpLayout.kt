...
class IdeaSignatureHelpLayout : SignatureHelpLayout {
    ...
    override fun createView(inflater: LayoutInflater): View {
        ...
        signatureText = TextView(context).apply {
            textSize = 13f
    ...
    override fun renderSignatures(signatureHelp: SignatureHelp) {
        ...
        if (nameEnd < signature.label.length) {
            ...
            for (i in params.indices) {
                ...
                val paramStr = if (param.label.isLeft) {
                    param.label.left ?: ""
                } else {
                ...
                // IDEA: 高亮且加粗当前活跃参数
                if (i == activeParam) {
                    builder.setSpan(ForegroundColorSpan(highlightColor), spanStart, builder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                    builder.setSpan(StyleSpan(Typeface.BOLD), spanStart, builder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                } else {
                    builder.setSpan(ForegroundColorSpan(textColor), spanStart, builder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                }
