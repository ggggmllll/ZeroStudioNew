...
/**
 * A collection of extension functions to add missing or compatibility APIs to the sora-editor
 * library classes without modifying their source code.
 */

// --- Content Extensions for missing APIs ---

/** Creates a CharPosition from a byte offset. Assumes UTF-16 encoding where 1 char = 2 bytes. */
...
fun Cursor.addSelection(startLine: Int, startColumn: Int, endLine: Int, endColumn: Int) {
  if (!this.isSelected) {
    ...
    val newStartColumn =
        if (newStartLine == currentStart.line) min(currentStart.column, startColumn)
        else if (newStartLine == startLine) startColumn else currentStart.column

    val newEndLine = max(currentEnd.line, endLine)
    val newEndColumn =
        if (newEndLine == currentEnd.line) max(currentEnd.column, endColumn)
        else if (newEndLine == endLine) endColumn else currentEnd.column
