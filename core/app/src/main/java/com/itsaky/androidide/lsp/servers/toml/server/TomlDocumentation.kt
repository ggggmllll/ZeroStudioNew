...
object TomlDocumentation {
    val SCHEMA_DOCS = mapOf(
        ...
        // ==========================================
        "package" to "*(Cargo specific)* Root node for project configuration. Defines metadata for the Rust project (name, version, edition).",
        "dependencies" to "*(General/Cargo)* Core dependencies. Libraries required for the project to run.",
        "dev-dependencies" to "*(Cargo specific)* Development dependencies. Required only for tests, examples, or benchmarks.",
        "build-dependencies" to "*(Cargo specific)* Build dependencies. Libraries used specifically for the `build.rs` script.",
        "features" to "*(Cargo specific)* Conditional compilation. Defines optional feature sets and their associated dependencies.",
        "workspace" to "*(Cargo specific)* Workspace mode. Manages multiple related crates within a single repository.",
        "profile" to "*(Cargo specific)* Compiler profiles. Defines optimization levels, LTO, and panic strategies for `dev` or `release` builds.",
        "target" to "*(Cargo specific)* Platform-specific dependencies. Configures dependencies for specific OS or architectures.",
        "patch" to "*(Cargo specific)* Dependency patching. Overrides dependency sources globally, often used for upstream bug fixes.",
        "lib" to "*(Cargo specific)* Library target configuration. Controls attributes of the generated `.rlib` or `.so`.",
        "bin" to "*(Cargo specific)* Binary targets. Defines the entry point and name for executable files.",
        "plugins" to "*(Gradle/Plugins)* Block for defining Gradle plugins or generalized build plugins.",
        "versions" to "*(Gradle Version Catalog)* Defines shared version numbers to be referenced across the build files.",
        "libraries" to "*(Gradle Version Catalog)* Defines exact artifacts (group:name:version) to be used as dependencies.",
        "bundles" to "*(Gradle Version Catalog)* Groups multiple libraries together to be declared as a single dependency block."
    )
}
