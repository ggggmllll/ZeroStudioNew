...
/**
 * A foreground service to handle downloading and installing SDK components in the background. It
 * communicates its state (progress, completion, failure) back to the UI using
...
class SdkManagerService : Service() {
    ...
    withContext(Dispatchers.IO) {
      URL(url).openStream().use { input ->
        FileOutputStream(tempFile).use { output ->
          val totalBytes =
              try {
                URL(url).openConnection().contentLengthLong
              } catch (e: Exception) {
                -1L
              }
          var bytesCopied = 0L
        ...
        TarArchiveInputStream(xzIn).use { tarIn ->
          ...
          while (entry != null && isActive) { // Check for cancellation
            ...
            if (entry.isDirectory) {
              ...
              destFile.outputStream().buffered().use { tarIn.copyTo(it) }
            }
  ...
  private fun broadcastUpdate(url: String, status: Int, progress: Int = 0, error: String? = null) {
    val intent =
        Intent(ACTION_SDK_MANAGER_UPDATE).apply {
  ...
  private fun createNotificationChannel() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      val channel =
          NotificationChannel(CHANNEL_ID, "SDK下载", NotificationManager.IMPORTANCE_LOW).apply {
