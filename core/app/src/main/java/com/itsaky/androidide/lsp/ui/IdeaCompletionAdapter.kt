 ...
 *
 * Item 布局结构: [Icon] [Label(匹配高亮)] [Detail(签名)] ....... [Type(右对齐)]
 *
 * @author android_zero
 */
class IdeaCompletionAdapter : EditorCompletionAdapter() {
  ...
  private fun bindData(holder: ViewHolder, item: CompletionItem, isSelected: Boolean) {
    ...
    if (item.deprecated) {
      holder.labelView.paintFlags =
          holder.labelView.paintFlags or android.graphics.Paint.STRIKE_THRU_TEXT_FLAG
    } else {
      holder.labelView.paintFlags =
          holder.labelView.paintFlags and android.graphics.Paint.STRIKE_THRU_TEXT_FLAG.inv()
    }
