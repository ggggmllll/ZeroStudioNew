...
/**
 * Toggles the read-only mode for the editor. This action is checkable, reflecting the current
 * state.
 */
class ToggleReadOnlyAction(context: Context, override val order: Int) :
    EditorRelatedAction(), CheckableAction {
  ...
  /**
   * Prepares the action's state. The checked status is synced directly with the editor's current
   * editable state, which is the source of truth.
   */
