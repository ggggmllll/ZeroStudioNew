...
class ToggleCommentAction(context: Context, override val order: Int) : EditorRelatedAction() {
  ...
  /**
   * Executes the action to toggle comments in the editor. It retrieves the current file from the
   * action data and passes it to the operation handler to apply the correct comment style.
   *
   * @param data The [ActionData] containing the current context, including the editor instance and
   *   the active file.
   * @return `true` if the operation was successful, `false` otherwise.
   */
  override suspend fun execAction(data: ActionData): Boolean {
    val editor = data.getEditor() ?: return false
    // Get the currently active file from the action data. This is crucial for determining the file
    // type.
