...
/**
 * 独立的智能导包分析器。
 * 解析文本中未解析的类引用，并在 AndroidIDE 层面提供修复建议。
 *
 * @author android_zero
 */
class KotlinImportAnalyzer {
    ...
    /**
     * 发现给定文本中的潜在缺失导入，并构建 Diagnostic。
     * （这个方法可以被 sora-editor-lsp 诊断系统结合使用）。
     */
            ...
            classRefRegex.findAll(line).forEach { match ->
                ...
                if (!isAlreadyResolved(className, existingImports) && !isKnownType(className)) {
                    ...
                    if (possibleFqns.isNotEmpty()) {
                        ...
                        val message = "Unresolved reference: $className. Could be: ${possibleFqns.joinToString(", ")}"
                        
                        // 使用 Warning 级别和特殊 Code 以便和 KLS 标准报错区分并被 QuickFix 拦截
                        val diag = Diagnostic(range, message, DiagnosticSeverity.Warning, "androidide-import-analyzer", "missing_import")
    ...
    private fun shouldSkipLine(line: String): Boolean {
        ...
        return t.startsWith("import ") || t.startsWith("package ") || t.startsWith("//") || t.startsWith("/*")
    }

    private fun isAlreadyResolved(className: String, existingImports: Set<String>): Boolean {
        return existingImports.any { it.endsWith(".$className") || it.endsWith(".*") || it == className }
    }

    private fun isKnownType(className: String): Boolean {
        val standardTypes = setOf("Int", "Long", "String", "Boolean", "Float", "Double", "Char", "Any", "Unit", "List", "Map")
