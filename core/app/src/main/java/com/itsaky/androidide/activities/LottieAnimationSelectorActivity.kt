...
/** About: Used for loading, importing, and exporting Lottie animation files @author：android_zero */
class LottieAnimationSelectorActivity : BaseIDEActivity() {
            ...
            try {
              ...
              val fileName =
                  uri.lastPathSegment?.substringAfterLast('/')
                      ?: "imported_animation_${System.currentTimeMillis()}.json"
              ...
              Toast.makeText(this, getString(R.string.import_failed, e.message), Toast.LENGTH_LONG)
                  .show()
            }
          }
        }
    ...
    when (item.itemId) {
      R.id.action_import -> {
        ...
        try {
          ...
          Toast.makeText(this, getString(R.string.error_file_picker, e.message), Toast.LENGTH_SHORT)
              .show()
        }
        return true
      }
  ...
  private fun loadAnimations() {
    ...
    if (Environment.LOTTIE_ANIMATION_DIR.exists() && Environment.LOTTIE_ANIMATION_DIR.isDirectory) {
      Environment.LOTTIE_ANIMATION_DIR.listFiles { _, name -> name.endsWith(".json") }
          ?.forEach { animationList.add(LottieAnimation(it.name, it.absolutePath, false)) }
    }
  ...
  private fun showExportDialog() {
    ...
    AlertDialog.Builder(this)
        ...
        .setItems(names) { _, which -> exportAnimation(animationList[which]) }
    ...
    try {
      ...
      if (animation.isAsset) {
        ...
        assets.open(animation.path).use { input ->
          FileOutputStream(finalDestFile).use { output -> input.copyTo(output) }
        }
      } else {
      ...
      Toast.makeText(
              this,
              getString(R.string.export_success_to, finalDestFile.absolutePath),
              Toast.LENGTH_LONG,
          )
          .show()
    } catch (e: Exception) {
  ...
  inner class LottieAnimationAdapter : RecyclerView.Adapter<LottieAnimationAdapter.ViewHolder>() {
    ...
    private var selectedPosition: Int = animationList.indexOfFirst {
      it.path == GeneralPreferences.lottieAnimation
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
      val binding =
          ItemLottieAnimationBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
      ...
      holder.itemView.setOnClickListener {
        ...
        if (selectedPosition != RecyclerView.NO_POSITION) {
          ...
          Toast.makeText(
                  this@LottieAnimationSelectorActivity,
                  getString(R.string.animation_set_to, item.name),
                  Toast.LENGTH_SHORT,
              )
              .show()
        }
    ...
    inner class ViewHolder(binding: ItemLottieAnimationBinding) :
