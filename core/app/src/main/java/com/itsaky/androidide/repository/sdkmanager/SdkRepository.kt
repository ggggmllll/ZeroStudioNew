...
/**
 * SDK 仓库解析层。 负责抓取 JSON、剥离版本下划线、映射真实物理路径校验版本。
 *
 * @author android_zero
 */
...
object SdkRepository {
  ...
  private fun parseVersion(jsonKey: String): String {
    val noPrefix =
        if (jsonKey.startsWith("_") || jsonKey.startsWith("-")) jsonKey.substring(1) else jsonKey
  ...
  private fun buildGroupNode(
    ...
    val groupNode =
        SdkTreeNode(name = groupName, isGroup = true, level = 0, isExpanded = isExpandedDefault)
  ...
  suspend fun getSdkPlatformsTree(): List<SdkTreeNode> =
  ...
  /** 获取 SDK Tools 数据树 */
  suspend fun getSdkToolsTree(): List<SdkTreeNode> =
      withContext(Dispatchers.IO) {
        val manifest = api.getManifest()
        ...
        // Platform-Tools
        buildGroupNode("Android SDK Platform-Tools", "platform-tools", manifest.platformTools, arch)
            ?.let { list.add(it) }
        ...
        val jdkList =
            listOf(
                "11" to manifest.jdk11,
                "17" to manifest.jdk17,
                "21" to manifest.jdk21,
                "22" to manifest.jdk22,
                "23" to manifest.jdk23,
                "24" to manifest.jdk24,
                "25" to manifest.jdk25,
                "26" to manifest.jdk26,
            )
