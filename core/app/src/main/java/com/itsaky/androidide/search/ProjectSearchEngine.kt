...
object ProjectSearchEngine {
  ...
  /** 执行搜索，返回结果流 */
  fun search(config: SearchConfig): Flow<List<SearchResultItem>> =
      flow {
  ...
  private fun getFilesForScope(
      config: SearchConfig,
      workspace: com.itsaky.androidide.projects.IWorkspace,
  ): Sequence<File> {
    return when (config.scope) {
      ...
      SearchScope.ALL,
      SearchScope.SCOPE_CUSTOM -> {
        // 利用 ProjectManager 的索引获取所有 Source Files，比 IO 遍历快
        val allSources =
            workspace
                .getSubProjects()
                .flatMap {
                  if (it is com.itsaky.androidide.projects.ModuleProject) it.getSourceDirectories()
                  else emptySet()
                }
      ...
      BufferedReader(FileReader(file)).use { reader ->
        ...
        while (line != null) {
          ...
          while (matcher.find()) {
            ...
            results.add(TextMatchResult(file, lineIndex, line!!, range, preview, isCrowded))
          }
  ...
  private fun createPreview(line: String, start: Int, end: Int): CharSequence {
    ...
    if (highlightStart >= 0 && highlightEnd <= span.length) {
      span.setSpan(
          BackgroundColorSpan(COLOR_HIGHLIGHT_BG),
          highlightStart,
          highlightEnd,
          Spannable.SPAN_EXCLUSIVE_EXCLUSIVE,
      )
      span.setSpan(
          ForegroundColorSpan(COLOR_HIGHLIGHT_FG),
          highlightStart,
          highlightEnd,
          Spannable.SPAN_EXCLUSIVE_EXCLUSIVE,
      )
