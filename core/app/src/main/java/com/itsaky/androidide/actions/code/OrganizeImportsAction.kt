 ...
 * @author android_zero
 */
class OrganizeImportsAction(context: Context, override val order: Int) : EditorActionItem {
   ...
   * @param ignoredKeywords Keywords to ignore/strip during sorting (usually same as
   *   importKeywords + "static").
  ...
  private val languages =
      HashMap<String, LanguageProfile>().apply {
        // Java, Groovy
        val javaStyle =
            LanguageProfile(listOf("import "), "package ", listOf("import ", "static "), ";")
        put("java", javaStyle)
        ...
        // C# (uses 'using')
        val csharpStyle =
            LanguageProfile(listOf("using "), "namespace ", listOf("using ", "static "), ";")
        put("cs", csharpStyle)
        ...
        val pythonStyle =
            LanguageProfile(listOf("import ", "from "), null, listOf("import ", "from "), null)
        put("py", pythonStyle)
        put("pyi", pythonStyle)
        ...
        // Lua
        val luaStyle =
            LanguageProfile(listOf("require ", "local "), null, listOf("require ", "local "), null)
        put("lua", luaStyle)
      }
  ...
  private fun organizeWholeFile(content: Content, profile: LanguageProfile) {
    ...
    try {
      ...
      // B. Adjust insertion point if it was below deleted lines (unlikely for imports, but safe to
      // check)
      ...
      // If based on firstImportIndex, that index is now logically "empty" or pointing to the next
      // line.
  ...
  /** Context-aware comparator. Extracts meaningful content based on the LanguageProfile. */
