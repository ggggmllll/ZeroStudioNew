 ...
 *
 * This action serves as a container for a sub-menu that includes operations like copy, cut, delete,
 * duplicate, change case, manage indentation, toggle comments, jump to a specific line, navigate
 * cursor history, and toggle read-only mode.
 *
 ...
 */
class EditorEditLineMenuAction(context: Context, override val order: Int) :
  ...
  /**
   * Prepares the action by updating its state based on the current context. This action is made
   * invisible if no editor is available.
   *
