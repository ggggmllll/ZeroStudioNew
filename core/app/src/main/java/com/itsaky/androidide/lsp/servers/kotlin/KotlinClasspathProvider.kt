...
/**
 * Kotlin 类路径提取器。
 * 用于扫描和收集工作区中所有模块的依赖库、SDK 路径及生成的源码，为 KLS 提供完全绕过 Gradle 解析的纯净类路径。
 *
 * @author android_zero
 */
class KotlinClasspathProvider {
    ...
    fun getClasspath(): String {
        return cachedClasspathString ?: getClasspathList().joinToString(File.pathSeparator).also { cachedClasspathString = it }
    }
    ...
    fun getClasspathList(): List<String> {
        cachedClasspathList?.let { return it }
        ...
        LOG.info("Total classpath entries collected: ${classpaths.size}, Valid existing: ${existingPaths.size}")
    ...
    private fun addKotlinScriptingJars(classpaths: MutableSet<String>) {
        try {
            ...
            val gradleCacheDir = File(System.getProperty("user.home", ""), ".gradle/caches/modules-2/files-2.1/org.jetbrains.kotlin")
