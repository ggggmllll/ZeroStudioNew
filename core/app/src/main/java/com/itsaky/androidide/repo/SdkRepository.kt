 ...
 * - Refactored getSdkPackages to use checkPackageStatus for accurate installation and update
 *   status.
 * - Corrected logic for locating OpenJDK installation directory.
 */
interface SdkManifestApi {
  @GET("android-zeros/androidide-tools/main/manifest.json") suspend fun getManifest(): SdkManifest
}
...
object SdkRepository {
            ...
            packages =
                listOf(
                    checkPackageStatus(
                        ...
                        SdkPackageItem(
                            name = "Android SDK Platform",
                            version = "Latest", // Platform doesn't have a specific version in this
                            // manifest
      ...
      val buildToolsItems =
          versions
              .map { (versionKey, url) ->
                ...
                val item =
                    SdkPackageItem(name = cleanVersion, version = cleanVersion, downloadUrl = url)
  ...
  private fun isNewerVersion(remoteVersion: String, localVersion: String?): Boolean {
    ...
    if (remoteVersion == "Latest")
        return false // Cannot determine update for "Latest" tags without more info.
