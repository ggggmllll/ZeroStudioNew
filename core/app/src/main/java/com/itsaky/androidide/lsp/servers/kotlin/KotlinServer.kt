 ...
 *
 * [执行器]：使用 AndroidIDE 内置的 Termux 环境，通过 [Environment.BASH_SHELL] 执行器。 [运行方式]：利用
 * [AbstractSilentCommandRunner] 静默拉起 KLS 启动脚本。 [通信方式]：脚本附加 `--tcpServerPort` 参数，通过
 * [SocketStreamProvider] 提供标准的 [java.io.InputStream] 和 [java.io.OutputStream] 供 LSP4J 协议通信。
 *
 * @author android_zero
 */
class KotlinServer : BaseLspServer() {
  ...
  override fun isInstalled(context: Context): Boolean {
    ...
    if (
        !Environment.KOTLIN_LSP_LIBS_JAR_DIR.exists() ||
            !Environment.KOTLIN_LSP_LIBS_JAR_DIR.isDirectory
    ) {
    ...
    val missingJars =
        KotlinServerConstants.REQUIRED_LIB_JARS.filter { jarName ->
  ...
  override fun getConnectionFactory(): LspConnectionFactory {
    return LspConnectionFactory { workingDir ->
      ...
      //  构造静默命令执行器实例
      val runner =
          object : AbstractSilentCommandRunner(context) {
            ...
            override fun onCommandFailed(commandLabel: String, result: SilentCommandResult) {
              LOG.error(
                  "KLS process failed or stopped. ExitCode: ${result.exitCode}, Error: ${result.stderr} / ${result.internalError}"
              )
            }
          }
