 ...
 *
 * KLS 默认返回的 Snippet 可能是： `makeText(${1:p0}, ${2:p1}, ${3:p2})`
 *
 * 我们利用 detail 字段中的签名 `(context: Context, text: CharSequence, duration: Int)`， 将其转换为：
...
object KotlinSnippetTransformer {
  ...
  private val BLOCK_PLACEHOLDER_REGEX =
      Regex("(block|lambda|action|init|body|builder|func)", RegexOption.IGNORE_CASE)
  ...
  /** 从函数签名中提取参数名列表。 输入示例: "foo(bar: String, baz: Int): Unit" 输出: ["bar", "baz"] */
