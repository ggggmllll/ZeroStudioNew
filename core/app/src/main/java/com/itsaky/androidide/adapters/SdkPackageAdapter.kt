...
/**
 * An adapter for the SDK Manager RecyclerView, which handles displaying expandable groups
 * (SdkPackageGroup) and individual package items (SdkPackageItem).
 *
 * @param onGroupClick A lambda function to be invoked when a group header is clicked.
 * @param onActionClick A lambda function to be invoked when the action button (download/delete) on
 *   an item is clicked.
 */
class SdkPackageAdapter(
  ...
  /**
   * ViewHolder for displaying an SdkPackageGroup. Handles click events to expand/collapse the
   * group.
   */
  class GroupViewHolder(
    ...
    fun bind(group: SdkPackageGroup) {
      ...
      binding.root.setOnClickListener { onGroupClick(group.name) }
    }
  }
      ...
      when {
        item.isDownloading -> {
          ...
          binding.progressBar.progress =
              if (item.downloadProgress >= 0) item.downloadProgress else 0
        }
        item.isInstalled -> {
...
/**
 * DiffUtil.ItemCallback implementation for efficiently updating the list. It compares items to
 * determine if the list content has changed.
 */
class SdkDiffCallback : DiffUtil.ItemCallback<Any>() {
  override fun areItemsTheSame(oldItem: Any, newItem: Any): Boolean {
    return when {
      ...
      oldItem is SdkPackageItem && newItem is SdkPackageItem ->
  ...
  override fun areContentsTheSame(oldItem: Any, newItem: Any): Boolean {
    // Since our data classes are used for state management (copy), a simple equality check is
