...
class SelectAllAction(context: Context, override val order: Int) : BaseEditorAction() {
  ...
  private fun customSelectAll(editor: CodeEditor) {
    ...
    if (
        cursor.isSelected &&
            ...
            cursor.right().line == lastLine &&
            cursor.right().column == lastColumn
    ) {
      return
    }

    // 调用底层 setSelectionRegion 实现精确全选
    editor.setSelectionRegion(0, 0, lastLine, lastColumn, false, SelectionChangeEvent.CAUSE_UNKNOWN)
  }
  ...
  companion object {
    ...
    /** 启发式符号配对。支持同种符号（如 `"`）和不同符号（如 `()`, `[]`）， 并且支持正确的嵌套深度计数逻辑（防止内部的 `()` 截断了外部 `()` 的匹配）。 */
