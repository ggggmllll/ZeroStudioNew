  ...
  suspend fun connect() =
      withContext(Dispatchers.IO) {
        ...
        runCatching {
              ...
              // 初始化或获取 LspProject
              val lspProject =
                  projectCache.getOrPut(projectPath) {
  ...
  suspend fun requestDocumentSymbols(): List<Either<SymbolInformation, DocumentSymbol>> {
    ...
    return withContext(Dispatchers.Default) {
      ...
      lspEd.requestManager
          ...
          ?.get(Timeout[Timeouts.SYMBOLS].toLong(), TimeUnit.MILLISECONDS) ?: emptyList()
    }
  }
  ...
  suspend fun requestDocumentHighlight(): List<DocumentHighlight> {
    ...
    return withContext(Dispatchers.Default) {
      ...
      lspEd.requestManager
          ...
          ?.get(Timeout[Timeouts.DOC_HIGHLIGHT].toLong(), TimeUnit.MILLISECONDS) ?: emptyList()
    }
  }
