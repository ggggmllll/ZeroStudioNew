...
/**
 * ViewModel for [com.itsaky.androidide.fragments.TemplateListFragment]. Manages the state for the
 * template selection UI, including categories, templates, and layout configuration.
 *
 * Work Flow:
 * 1. On initialization, it fetches all registered categories and their templates from
 *    [ITemplateProvider].
...
class TemplateListViewModel : ViewModel() {
  ...
  private val _templatesByCategory =
      MutableStateFlow<Map<TemplateCategory, List<Template<*>>>>(emptyMap())
  ...
  /**
   * Loads categories and templates from the [ITemplateProvider] on a background thread. Updates the
   * StateFlows, which will trigger a recomposition in the UI.
   */
  fun loadTemplates() {
