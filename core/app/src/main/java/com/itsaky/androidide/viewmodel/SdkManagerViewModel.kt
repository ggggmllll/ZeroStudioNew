...
/**
 * ViewModel for SdkManagerActivity. It fetches SDK package information, handles user interactions
 * like downloading or deleting packages, and communicates status updates to the UI.
 *
 * @author android_zero
 */
class SdkManagerViewModel : ViewModel() {
  ...
  fun startDownload(context: Context, item: SdkPackageItem) {
    val intent =
        Intent(context, SdkManagerService::class.java).apply {
          ...
          val destSubPath =
              when {
                item.name.contains("Build-Tools", ignoreCase = true) ->
                    "build-tools/${item.version}"
                item.name.contains("Platform-Tools", ignoreCase = true) -> "platform-tools"
                item.name.contains("Platform", ignoreCase = true) ->
                    "." // Extract to sdk root, it contains 'platforms' dir
                item.name.contains("Command-line Tools", ignoreCase = true) -> "cmdline-tools"
                item.name.contains("JDK", ignoreCase = true) ->
                    Environment.JAVA_HOME.absolutePath // Use absolute path for JDK
  ...
  fun deletePackage(context: Context, item: SdkPackageItem) {
    MaterialAlertDialogBuilder(context)
        ...
        .setPositiveButton(android.R.string.ok) { _, _ -> executeDeletion(item) }
        .show()
  }
    ...
    viewModelScope.launch(Dispatchers.IO) {
      ...
      try {
        val fileToDelete =
            when {
              item.name.contains("Build-Tools", ignoreCase = true) ->
                  File(Environment.ANDROID_HOME, "build-tools/${item.version}")
              item.name.contains("Platform-Tools", ignoreCase = true) ->
                  File(Environment.ANDROID_HOME, "platform-tools")
              // item.name.contains("Platform", ignoreCase = true) -> File(Environment.ANDROID_HOME,
              // "platforms")
              item.name.contains("Platform") ->
                  File(Environment.ANDROID_HOME, "platforms/android-${item.version}")
              item.name.contains("Command-line Tools", ignoreCase = true) ->
                  File(Environment.ANDROID_HOME, "cmdline-tools")
          ...
          packages =
              group.packages.map { pkg ->
                if (pkg.downloadUrl == url) {
                  when (status) {
                    SdkManagerService.STATUS_STARTED,
                    SdkManagerService.STATUS_EXTRACTING ->
                        pkg.copy(isDownloading = true, downloadProgress = -1) // Indeterminate
                    SdkManagerService.STATUS_PROGRESS ->
                        pkg.copy(isDownloading = true, downloadProgress = progress)
