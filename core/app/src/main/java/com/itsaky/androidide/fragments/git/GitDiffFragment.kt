...
class GitDiffFragment : BaseGitPageFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val dummyLines = listOf(
            ...
            DiffLine(12, 13, "    override fun onCreate(savedInstanceState...) {", DiffType.CONTEXT)
        )
    ...
    // --- 数据模型 ---
    enum class DiffType { ADD, DELETE, CONTEXT, HUNK_HEADER }
    data class DiffLine(val oldLine: Int, val newLine: Int, val content: String, val type: DiffType)

    // --- 适配器 ---
    inner class DiffAdapter(private val lines: List<DiffLine>) : RecyclerView.Adapter<DiffAdapter.ViewHolder>() {
