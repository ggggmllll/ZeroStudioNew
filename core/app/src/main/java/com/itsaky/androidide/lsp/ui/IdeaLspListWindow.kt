...
/**
 * 通用 LSP 列表窗口。 提供真正的 IDE 悬浮窗体验，替代占屏幕的 BottomSheet。
 *
 * @author android_zero
 */
class IdeaLspListWindow(private val editor: CodeEditor) :
  ...
  init {
    ...
    val gd =
        GradientDrawable().apply {
          ...
          setStroke(
              (1 * dp).toInt(),
              editor.colorScheme.getColor(EditorColorScheme.COMPLETION_WND_CORNER),
          )
        }
    ...
    listView.adapter =
        object : BaseAdapter() {
          ...
          override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            // 处理大纲树形结构的缩进
            view.setPadding(
                (12 * dp + item.indentLevel * 16 * dp).toInt(),
                (8 * dp).toInt(),
                (12 * dp).toInt(),
                (8 * dp).toInt(),
            )
            ...
            if (!item.detail.isNullOrEmpty()) {
              ...
              detailView.setTextColor(
                  editor.colorScheme.getColor(EditorColorScheme.COMPLETION_WND_TEXT_SECONDARY)
              )
            } else {
  ...
  fun showList(newItems: List<LspWindowItem>, onClick: (LspWindowItem) -> Unit) {
    ...
    val height =
        (editor.height * 0.6f)
            .toInt()
            .coerceAtMost((items.size * 40 * dp).toInt() + (16 * dp).toInt())
    ...
    return LinearLayout(ctx).apply {
      ...
      val icon =
          ImageView(ctx).apply {
            id = 1
            layoutParams =
                LinearLayout.LayoutParams((16 * dp).toInt(), (16 * dp).toInt()).apply {
