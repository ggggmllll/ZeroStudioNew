...
class SmaliTextDocumentService(private val server: SmaliLanguageServer) : TextDocumentService {
  ...
  override fun didClose(params: DidCloseTextDocumentParams) =
      SmaliDocumentCache.remove(params.textDocument.uri)
  ...
  override fun completion(
    ...
    val items =
        listOf(".class", ".super", ".field", ".method", ".end method").map {
  ...
  override fun hover(params: HoverParams): CompletableFuture<Hover> {
    val value =
        MarkupContent(
            "markdown",
            "Smali token at `${params.position.line}:${params.position.character}`",
        )
  ...
  override fun documentSymbol(
    ...
    val symbols =
        lines
                ...
                line.trimStart().startsWith(".class") ->
                    SymbolInformation(
                        ...
                        Location(
                            ...
                            Range(Position(idx, 0), Position(idx, line.length)),
                        ),
                    )
                line.trimStart().startsWith(".method") ->
                    SymbolInformation(
                        "method",
                        SymbolKind.Method,
                        Location(
            ...
            .map { Either.forLeft<SymbolInformation, DocumentSymbol>(it) }
            .toMutableList()
        ...
        object : BaseErrorListener() {
          override fun syntaxError(
            ...
            diagnostics +=
                Diagnostic(
                    Range(
                        Position((line - 1).coerceAtLeast(0), charPositionInLine.coerceAtLeast(0)),
                        Position(
                            (line - 1).coerceAtLeast(0),
                            (charPositionInLine + 1).coerceAtLeast(0),
                        ),
                    ),
