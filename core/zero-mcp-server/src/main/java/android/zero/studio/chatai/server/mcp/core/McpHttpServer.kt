...
/**
 * 嵌入式 MCP HTTP 服务器
 * 监听端口，处理 JSON-RPC 请求并分发给 ToolManager
 *
 * @author android_zero
 */
class McpHttpServer(
    ...
    override fun serve(session: IHTTPSession): Response {
        ...
        if (method == Method.POST && (uri == "/ZeroStudio" || uri == "/")) {
            ...
            try {
                ...
                val res = newFixedLengthResponse(Response.Status.OK, "application/json", gson.toJson(errorResponse))
                corsHeaders.forEach { (k, v) -> res.addHeader(k, v) }
                return res
            }
        }

        return newFixedLengthResponse(Response.Status.OK, MIME_HTML, "<html><body><h1>Android MCP Server is Running</h1><p>Workspace: ${workspace.name}</p></body></html>")
    }
    ...
    private fun handleRpc(request: JsonObject): JsonObject? {
        ...
        when (method) {
            ...
            "tools/call" -> {
                ...
                val params = if (paramsElement != null && !paramsElement.isJsonNull) paramsElement.asJsonObject else JsonObject()
                
                val toolName = params.get("name")?.asString ?: "unknown"
                
                val argsElement = params.get("arguments")
                val args = if (argsElement != null && !argsElement.isJsonNull) argsElement.asJsonObject else JsonObject()
