...
/**
 * MCP 工具管理器
 * 实现了 AI 编程所需的工具集：文件读写、代码搜索、项目结构分析。
 * 
 * @author android_zero
 */
object McpToolManager {
    ...
    fun getToolDefinitions(): JsonArray {
        ...
        // 列出文件 (ls)
        tools.add(createToolDef("ls", "列出当前工作区的文件和文件夹结构，用于了解项目目录。",
            """{"type":"object", "properties":{"path":{"type":"string", "description":"相对路径，为空则列出根目录"}}}"""))

        // 读取文件 (read_file) - AI 读取代码上下文
        tools.add(createToolDef("read_file", "读取源代码文件内容 (.java, .kt, .xml, .gradle 等)。",
            """{"type":"object", "properties":{"path":{"type":"string"}}, "required":["path"]}"""))

        // 写入文件 (write_file) - AI 写代码核心能力
        tools.add(createToolDef("write_file", "创建新文件或覆盖现有文件内容。用于编写代码、生成配置文件。",
            """{"type":"object", "properties":{"path":{"type":"string"}, "content":{"type":"string"}}, "required":["path", "content"]}"""))

        //全局搜索 (search_code)
        tools.add(createToolDef("search_code", "在项目中递归搜索代码片段、类名或关键词。",
            """{"type":"object", "properties":{"keyword":{"type":"string"}}, "required":["keyword"]}"""))

        //获取项目概览 (get_project_structure)
        tools.add(createToolDef("get_project_structure", "快速分析项目结构，识别 src, res, Manifest, Gradle 等关键位置。",
    ...
    fun handleCall(name: String, args: JsonObject, rootDir: File): String {
        return try {
            when (name) {
                ...
                "write_file" -> writeFile(rootDir, args.get("path")?.asString, args.get("content")?.asString)
    ...
    private fun listFiles(root: File, subPath: String?): String {
        ...
        // 安全检查：防止访问工作区外部
        if (!target.canonicalPath.startsWith(root.canonicalPath)) return "Error: Access denied (Outside workspace)"
    ...
    private fun searchCode(root: File, keyword: String?): String {
        ...
        root.walkTopDown()
            ...
            .forEach { file ->
                ...
                } catch (e: Exception) { /* Ignore binary or unreadable files */ }
            }
    ...
    private fun analyzeProject(root: File): String {
        ...
        val criticalPaths = listOf(
            ...
            "AndroidManifest.xml", "build.gradle", "build.gradle.kts", "settings.gradle"
        )
